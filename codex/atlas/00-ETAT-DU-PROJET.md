...
fichier) et a inventé le sens du sigle RAG. Corrigés par cette section et par un lexique, **les deux corrections CONFIRMÉES par re-test le 21/09**. **Le soir : tes notes locales entrent dans sa mémoire** (`atlas-apprendre.bat`, CONFIRMÉ sur une note test). Prochaine couche : le RAG, qui règle la lenteur.
...
2. **Deux réponses d'une ligne** pour le dossier statut : as-tu aujourd'hui une activité
   ...
   *(et, pour que ton IA locale grandisse : une note dans `%USERPROFILE%\ATLAS\corpus`, puis
   `%USERPROFILE%\atlas-apprendre.bat` — c'est ça qui la met à jour. Détail dans `memoire/MACHINE.md`.)*
...
| 3 | Mémoire | ✅ **CONFIRMÉE** (21/09) : mémoire du dépôt **et** notes locales via `atlas-apprendre.bat`. Limite mesurée : 1re réponse lente sur CPU → argument du RAG |
