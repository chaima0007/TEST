...
# SPÉCIFICITÉS DU PROJET — chaima0007/TEST

- **Dépôt** : chaima0007/TEST. Branche de travail de la session Libre & Accomplis :
  `claude/libre-accomplis-system-9wvsnf` (PR #6). Jamais de commit direct sur la branche principale.
- **Stack** : Next.js (version modifiée — lire les docs embarquées, voir @AGENTS.md),
  TypeScript, Prisma, Tailwind/PostCSS.
- **Vérifications avant push** (Parcours 4) : `npm run lint` puis `npx tsc --noEmit` puis
  `npm run build` ; tests s'ils touchent au code applicatif. Changements documentation seule :
  pas de build requis.
- **Pièges connus** :
  - Ce Next.js n'est PAS celui des données d'entraînement — lire `node_modules/next/dist/docs/`
    avant d'écrire du code.
  - Le connecteur Google Drive ne peut pas éditer un Google Doc existant : créer un document
    « — partie N », jamais de doublon couvrant les mêmes actions.
  - Horloges conteneur/Drive parfois décalées de la date réelle : toujours dater manuellement
    les documents (date fournie par le système, pas par l'horloge locale).
- **Coordination inter-services (Drive)** : dossier `LibreEtAccomplis/` sur le Google Drive —
  protocole détaillé dans `docs/libre-et-accomplis/PROTOCOLE_AUDIT_DRIVE.md` (un audit par
  travail effectué, titres normalisés, synopsis obligatoire, lecture du Drive en début de session).
