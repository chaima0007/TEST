...
# DÉPÔT PARTAGÉ (La Loi Avec Moi + outillage Caelum) — spécificités du projet

*(Sous le bloc CODEX, conformément au §15.4. Les règles métier ci-dessous ne sont pas une version
concurrente du protocole : ce sont les spécificités de ce dépôt. Les protocoles de PROCESSUS
antérieurs — « Drive d'abord », livraison v2 — sont remplacés par le bloc CODEX ci-dessus.)*

@AGENTS.md
@docs/protocols/wave-development-protocol.md

## Dépôt et branche
- Dépôt : `chaima0007/TEST` — **partagé entre plusieurs sessions et projets**.
- Branche de travail de cette session : `claude/swarm-50-agent-architecture-3l6cno`.
  `main` est la branche principale : jamais de push direct dessus (§10).
- Une session = une branche. Contrôle anti-collision avant toute écriture (voir commandes).

## Stack
- Next.js 16 (App Router, Tailwind v4) à la racine (outillage Caelum) et dans `laloiavecmoi/`.
- Corpus juridique : `data/belgium/*.json`, **miroité** dans `laloiavecmoi/data/belgium/` — les deux
  copies doivent rester identiques, sans exception.
- Outillage Python dans `scripts/`.

## Vérifications avant push (Parcours 4)
```bash
python3 scripts/env_integrity_check.py       # branche correcte, dossiers clés, corpus complet
python3 scripts/branch_guard.py --check      # anti-collision entre sessions
python3 scripts/loi_reference_audit.py       # 100 % des réponses avec une loi concrète, 0 vague
python3 scripts/source_trust_protocol.py     # 0 source « officiel » hors tier1
python3 scripts/legal_content_verifier.py    # chaque fait sourcé officiellement
python3 scripts/norm_hierarchy_guard.py      # aucun conflit temporel ni territorial
python3 scripts/liens_fiches_guard.py        # 0 lien mort vers une fiche
python3 scripts/build_health_protocol.py     # sécurité des routes, doublons, taille sidebar
python3 scripts/constants_monitor.py         # constantes des moteurs dans les bornes
diff -q data/belgium laloiavecmoi/data/belgium   # miroirs identiques
```

## Pièges connus de ce dépôt
- **Le conteneur est régulièrement recyclé** : le clone local peut retomber sur un état ancien et
  divergent. Toujours vérifier l'état réel par `git ls-remote` avant de conclure (§5.1). Le travail
  poussé sur `origin` n'est jamais perdu ; `git reset --hard` est refusé par l'environnement.
- **Double miroir obligatoire** : toute modification d'une fiche doit être appliquée aux deux copies,
  sinon l'application sert une version périmée.
- **Le routage `/loi/[domaine]` matche le champ `module` du JSON, pas le nom de fichier** — 11 fiches
  ont un nom différent de leur module. Un lien écrit de tête casse en silence.
- **Sources** : droit de l'UE → EUR-Lex ; droit belge → `ejustice.just.fgov.be` ; jamais une
  fédération professionnelle ni un blog comme base légale d'une fiche.
- **Réforme du préavis** : en vigueur depuis le 01/08/2026 (loi du 3 juin 2026) — le régime dépend de
  la **date de début d'exécution du contrat**, pas de la date de notification du préavis.
- Ce dépôt est **public** : rien qui touche à l'invention couverte par l'i-DEPOT n'y entre.
