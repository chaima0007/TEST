...
> Ajouté sous le bloc CODEX, conformément au §15.4. Rien d'autre que le local ici.

- **Dépôt :** `chaima0007/TEST` — application **CompeteIQ** (SaaS de veille concurrentielle). **Projet en pause.** Repo de test, **aucun site live**.
- **Stack :** Next.js 16.2.9 · React 19.2 · Prisma 7.8 / SQLite (libSQL) · next-auth v5 (beta) · bcryptjs · Tailwind v4. ⚠️ AGENTS.md prévient : ce Next.js a des breaking changes — lire `node_modules/next/dist/docs/` avant de coder.
- **Commandes de vérification avant push (Parcours 4) :** `npm run lint` · `npm run typecheck` · `npm run build`. (Pas de suite de tests présente à ce jour — NON VÉRIFIÉ qu'un test runner soit configuré.)
- **Pièges connus :** données concurrents **codées en dur** dans `lib/data.ts` (mock, pas de vraie source) ; le pitch (`app/pitch/page.tsx`) vante « crawling temps réel » et « NLP » **non implémentés** — ne pas les présenter comme réels.
- **⚠️ Confidentialité :** ce dépôt est **PUBLIC** (VÉRIFIÉ via API GitHub, 2026-07-17 : `"private": false`). Action humaine en attente : passer en privé (Settings → Danger Zone → Make private). Voir `/codex/A-DECIDER.md`.
- **État PI :** audit de brevetabilité fait → **0 invention brevetable** (rapport `docs/audits/AUDIT_BREVETS_2026-07-17_2056.md`). Cohérent avec §11.

<!-- Spécificité locale Next.js conservée : -->
@AGENTS.md
