...
### Bloc unique. En tête du CLAUDE.md de CHAQUE projet, intégralement, sans rien retirer.
...
# SPÉCIFIQUE PROJET — competeiq

> Ajouté sous le bloc CODEX conformément au §15.4. Ne remonte jamais au-dessus du protocole.

- **Dépôt / branche de travail** : `chaima0007/TEST` — branche `claude/shopify-app-development-ri1090`. Jamais de commit direct sur la branche principale (§8 Parcours 4).
- **Nature** : application **Next.js 16.2.9** (App Router) + intégration **Shopify**, embarquée dans l'admin Shopify.
- **Stack** : React 19.2.4 · TypeScript 5 · Prisma 7.8 (`@prisma/client`, adapter libsql) · next-auth 5 (beta) · Tailwind 4. Base : libsql/SQLite.
- **⚠️ Next.js modifié** : voir `AGENTS.md` — cette version porte des *breaking changes*. **Lire `node_modules/next/dist/docs/` AVANT d'écrire du code Next.** Ne pas coder de mémoire.
- **Commandes de vérification AVANT push (§8 Parcours 4)** — dans l'ordre :
  1. `npm run lint`
  2. `npm run typecheck`
  3. `npm run build`
  - DB (si schéma touché) : `npm run db:push` puis `npm run db:seed`.
- **Pièges connus** :
  - `overrides` épinglés dans `package.json` (`postcss`, `@hono/node-server`) — ne pas les retirer sans passer par le Parcours 1.
  - App embarquée = contraintes CSP / `X-Frame-Options` sur les routes embarquées : toute modif d'en-têtes passe par Sentinel-Sécurité + Gardien-Données.
  - Secrets Shopify / next-auth : jamais commités, jamais dans le bundle client (Conservateur-Secrets, §10).
- **Horloge du bac à sable** : a déjà dérivé (lu `2026-07`/`2026-07-17` lors de sessions précédentes). Toujours horodater via `TZ="Europe/Brussels" date` **et** recouper avec une source datée avant d'inscrire une date comme VÉRIFIÉE.
