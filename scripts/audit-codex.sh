...
# ── 1. Travail poussé mais pas livré (ERR-20260914-2022) ───────────────────────────────
...
# ── 5 bis. Fraîcheur du SHA cité dans ETAT.md (ERR-20260914-2022) ────────────
# « Un merge ne met pas à jour les registres » : la fraîcheur d'un fait d'état
# ne peut pas reposer sur la mémoire de celui qui part. Un fait d'état vit à UN
# SEUL endroit (ETAT.md) — une machine vérifie qu'il est encore juste.
sha_etat=$(grep -oE '\*\*`main` = `[0-9a-f]{7,40}`\*\*' ETAT.md 2>/dev/null | grep -oE '[0-9a-f]{7,40}' | head -1)
sha_main=$(git rev-parse --short origin/main 2>/dev/null)
if [ -z "$sha_etat" ]; then
  ko "ETAT.md ne cite aucun SHA de main — le fait d'état n'est plus vérifiable"
elif git merge-base --is-ancestor "$sha_etat" origin/main 2>/dev/null; then
  retard=$(git rev-list --count "$sha_etat"..origin/main 2>/dev/null || echo '?')
  if [ "$retard" = "0" ]; then ok "ETAT.md cite le main du jour ($sha_main)"
  elif [ "$retard" -le 10 ] 2>/dev/null; then ok "ETAT.md a $retard commit(s) de retard sur main — acceptable"
  else ko "ETAT.md cite $sha_etat, soit $retard commits de retard sur main ($sha_main) — à remettre à jour"; fi
else
  ko "le SHA cité dans ETAT.md ($sha_etat) n'est pas un ancêtre de main — fait d'état invalide"
fi
