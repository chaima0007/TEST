...
interface Entity {
  ...
  last_updated: string;
  biased_ai_deployment_score?: number;
  predictive_policing_bias_score?: number;
  hiring_algorithm_discrimination_score?: number;
  ai_accountability_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "biased_ai_deployment_score", label: "Déploiement d&apos;IA biaisée" },
  { key: "predictive_policing_bias_score", label: "Biais de police prédictive" },
  { key: "hiring_algorithm_discrimination_score", label: "Discrimination algorithmique à l&apos;embauche" },
  { key: "ai_accountability_gap_score", label: "Lacunes de responsabilité IA" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des discriminations algorithmiques et des droits humains : déploiement d&apos;IA biaisée, biais de police prédictive, discrimination algorithmique à l&apos;embauche et lacunes de responsabilité IA.</p>
              <p className="text-slate-400">Source : Algorithmic Discrimination Rights Engine · Wave 246</p>
            </div>
          )}
        </div>
      </div>
...
export default function AlgorithmicDiscriminationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/algorithmic-discrimination-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Algorithmic Discrimination Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">IA Biaisée · Police Prédictive · Discrimination à l&apos;Embauche · Responsabilité IA</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-indigo-700/40 text-indigo-300"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "#4338ca20" } : {}}>
