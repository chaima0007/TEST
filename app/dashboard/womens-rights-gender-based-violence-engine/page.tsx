...
export default function WomensRightsGenderBasedViolencePage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#500724";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", "modéré": "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/womens-rights-gender-based-violence-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_womens_rights_gender_based_violence_index"] as number) ?? 0;
