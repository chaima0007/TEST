...
interface Entity {
  ...
  estimated_torture_prevention_rights_index: number;
  last_updated: string;
  [key: string]: unknown;
}
...
interface DashData {
  ...
  avg_estimated_torture_prevention_rights_index?: number;
        ...
        <div style={{ padding: 24 }}>
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <div style={{ background: "#1e293b", borderRadius: 12, padding: 16, textAlign: "center" }}>
                  <div style={{ fontSize: 32, fontWeight: 700, color: ACCENT_VIS }}>{typeof entity.estimated_torture_prevention_rights_index === "number" ? entity.estimated_torture_prevention_rights_index.toFixed(2) : "—"}</div>
                  <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>Index Prévention Torture</div>
                </div>
              </div>
          ...
          {tab === "metriques" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ background: "#1e293b", borderRadius: 10, padding: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <span style={{ fontSize: 13, color: "#cbd5e1" }}>Index Prévention Torture</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: "white" }}>{typeof entity.estimated_torture_prevention_rights_index === "number" ? entity.estimated_torture_prevention_rights_index.toFixed(2) : "—"}</span>
                </div>
                <div style={{ height: 6, background: "#334155", borderRadius: 9999, overflow: "hidden" }}>
                  <div style={{ height: "100%", borderRadius: 9999, width: `${Math.min((entity.estimated_torture_prevention_rights_index ?? 0) * 10, 100)}%`, background: ACCENT_VIS }} />
                </div>
              </div>
            </div>
          )}
...
export default function TorturePreventionRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/torture-prevention-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ background: "#030712", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontSize: 14, color: ACCENT_VIS, animation: "pulse 2s infinite" }}>Initialisation Prévention Torture &amp; Droits Humains…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_torture_prevention_rights_index ?? avg(allEntities.map(e => e.estimated_torture_prevention_rights_index));
  ...
  const kpis = [
    ...
    { label: "Index Prévention Torture", value: avgIndex.toFixed(2) },
    ...
    <div style={{ minHeight: "100vh", background: "#030712", color: "white", padding: 24, display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0, letterSpacing: "-0.025em" }}>Prévention Torture &amp; Droits Humains</h1>
        </div>
        <p style={{ color: "#94a3b8", fontSize: 13, margin: "0 0 0 24px" }}>
          Torture Prevention Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 24 }}>
          ...
          <GaugeRing value={avgIndex * 10} label="Index Torture (×10)" />
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div style={{ fontSize: 11, color: "#64748b", marginTop: 8 }}>
              Index Prévention Torture: <span style={{ fontWeight: 500, color: ACCENT_VIS }}>{typeof e.estimated_torture_prevention_rights_index === "number" ? e.estimated_torture_prevention_rights_index.toFixed(2) : "—"}</span>
            </div>
