...
interface Entity {
  ...
  risk_level: string
  estimated_peacekeeping_accountability_index: number
  [key: string]: unknown
}
...
export default function PeacekeepingAccountabilityGovernancePage() {
  const [data, setData] = useState<DashData | null>(null)
  const [loading, setLoading] = useState(true)
  const [filter, setFilter] = useState("tous")

  useEffect(() => {
    fetch("/api/peacekeeping-accountability-governance-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Responsabilité des Opérations de Maintien de la Paix…</div>
      </div>
    )
  }
  ...
  const avgIndex = avg(allEntities.map(e => e.estimated_peacekeeping_accountability_index))
  ...
  const kpis = [
    ...
    { label: "Indice responsabilité", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Responsabilité des Opérations de Maintien de la Paix</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Abus et impunité dans les missions ONU — surveillance CSDDD
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <GaugeRing value={avgIndex * 10} label="Indice responsabilité (×10)" />
        ...
        {filtered.map(e => (
          <div key={e.id}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Indice responsabilité: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_peacekeeping_accountability_index === "number" ? e.estimated_peacekeeping_accountability_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
