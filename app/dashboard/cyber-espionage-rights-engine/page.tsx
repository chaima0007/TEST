...
interface Entity {
  ...
  last_updated: string;
  state_sponsored_espionage_score?: number;
  zero_day_exploit_misuse_score?: number;
  covert_surveillance_ops_score?: number;
  attribution_impunity_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "state_sponsored_espionage_score", label: "Espionnage étatique" },
  { key: "zero_day_exploit_misuse_score", label: "Exploitation zero-day" },
  { key: "covert_surveillance_ops_score", label: "Surveillance couverte" },
  { key: "attribution_impunity_score", label: "Impunité d&apos;attribution" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse du cyber-espionnage étatique et de l&apos;exploitation des droits numériques. Les opérations couvrent l&apos;espionnage sponsorisé par les États, l&apos;abus d&apos;exploits zero-day, la surveillance couverte et l&apos;impunité d&apos;attribution.</p>
              <p className="text-slate-400">Source : Cyber Espionage Rights Engine · Wave 242</p>
            </div>
          )}
        </div>
      </div>
...
export default function CyberEspionageRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/cyber-espionage-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Cyber Espionage Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Espionnage Étatique · Exploits Zero-Day · Surveillance Couverte · Impunité</p>
        </div>
