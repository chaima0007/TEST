...
interface Entity {
  ...
  estimated_lithium_mining_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_lithium_mining_rights_index?: number;
        ...
        <div className="p-6">
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_lithium_mining_rights_index === "number" ? entity.estimated_lithium_mining_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Mines de Lithium</div>
                </div>
              </div>
          ...
          {tab === "entites" && (
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                <div className="text-xs text-slate-400 mb-3">Index Mines de Lithium (sur 10)</div>
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-3 bg-slate-700 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${Math.min((entity.estimated_lithium_mining_rights_index / 10) * 100, 100)}%`, background: ACCENT }} />
                  </div>
                  <span className="text-sm font-bold text-white w-12 text-right">{typeof entity.estimated_lithium_mining_rights_index === "number" ? entity.estimated_lithium_mining_rights_index.toFixed(2) : "—"}</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-sm text-slate-300 space-y-1">
                  <div>Conditions de Travail & Sécurité Minière × 0.30</div>
                  <div>Impact Environnemental & Droits Fonciers × 0.25</div>
                  <div>Violations Communautés Autochtones × 0.25</div>
                  <div>Impunité Entreprises & Gouvernance Faible × 0.20</div>
                </div>
              </div>
              {entity.last_updated && (
...
export default function LithiumMiningRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/lithium-mining-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits dans les Mines de Lithium…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_lithium_mining_rights_index ?? avg(allEntities.map(e => e.estimated_lithium_mining_rights_index));
  ...
  const kpis = [
    ...
    { label: "Index Mines Lithium", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits dans les Mines de Lithium</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Lithium Mining Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <GaugeRing value={avgIndex * 10} label="Index Mines Lithium" />
        ...
        {filtered.map(e => (
          <div key={e.id ?? e.entity} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Mines Lithium: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_lithium_mining_rights_index === "number" ? e.estimated_lithium_mining_rights_index.toFixed(2) : "—"}</span>
            </div>
