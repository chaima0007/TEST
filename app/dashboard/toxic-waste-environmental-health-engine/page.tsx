...
interface TWEEntity {
  ...
  severity: string;
  estimated_toxic_waste_environmental_health_index: number;
...
const SEVERITY_STYLES: Record<string, { bg: string; color: string; label: string }> = {
  ...
  "élevé":  { bg: "#fffbeb", color: "#b45309", label: "Élevé" },
...
function EntityCard({ entity }: { entity: TWEEntity }) {
  const idx = entity.estimated_toxic_waste_environmental_health_index;
  return (
    <div
      style={{
        background: "#ffffff",
...
function SummaryBadges({ entities, avg }: { entities: TWEEntity[]; avg: number }) {
...
export default function ToxicWasteEnvironmentalHealthPage() {
  const [data, setData] = useState<TWEData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/toxic-waste-environmental-health-engine")
      .then((r) => r.json())
    ...
    <div style={{ padding: "24px 24px 48px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      >
        <div>
          ...
          >
            Déchets Toxiques &amp; Santé Environnementale
          </h1>
