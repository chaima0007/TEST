...
interface Entity {
  ...
  supply_chain_forced_labor_complicity_severity_score: number;
  environmental_destruction_community_displacement_scale_score: number;
  corporate_impunity_legal_accountability_gap_score: number;
  human_rights_due_diligence_failure_deficit_gap_score: number;
  estimated_corporate_accountability_human_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_corporate_accountability_human_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Complicité Travail Forcé Chaîne Appro.", value: entity.supply_chain_forced_labor_complicity_severity_score, weight: "0.30" },
    { label: "Destruction Env. & Déplacement Communautés", value: entity.environmental_destruction_community_displacement_scale_score, weight: "0.25" },
    { label: "Impunité Corporate & Responsabilité Juridique", value: entity.corporate_impunity_legal_accountability_gap_score, weight: "0.25" },
    { label: "Déficit Devoir Vigilance Droits Humains", value: entity.human_rights_due_diligence_failure_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_corporate_accountability_human_rights_index === "number" ? entity.estimated_corporate_accountability_human_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Responsabilité Corporate</div>
                </div>
              </div>
...
export default function CorporateAccountabilityHumanRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/corporate-accountability-human-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Responsabilité des Entreprises & Droits Humains…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_corporate_accountability_human_rights_index ?? avg(allEntities.map(e => e.estimated_corporate_accountability_human_rights_index));
  ...
  const avgSupplyChain = avg(allEntities.map(e => e.supply_chain_forced_labor_complicity_severity_score));
  const avgEnvDestruction = avg(allEntities.map(e => e.environmental_destruction_community_displacement_scale_score));
  const avgImpunity = avg(allEntities.map(e => e.corporate_impunity_legal_accountability_gap_score));
  const avgDueDiligence = avg(allEntities.map(e => e.human_rights_due_diligence_failure_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Responsabilité Corporate", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Responsabilité des Entreprises & Droits Humains</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Corporate Accountability Human Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgSupplyChain} label="Complicité Travail Forcé" />
          <GaugeRing value={avgEnvDestruction} label="Destruction Env. & Déplacement" />
          <GaugeRing value={avgImpunity} label="Impunité Corporate" />
          <GaugeRing value={avgDueDiligence} label="Devoir de Vigilance" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Responsabilité Corporate: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_corporate_accountability_human_rights_index === "number" ? e.estimated_corporate_accountability_human_rights_index.toFixed(2) : "—"}</span>
            </div>
