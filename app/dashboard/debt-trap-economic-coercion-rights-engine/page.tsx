...
interface Entity {
  ...
  country: string;
  sector: string;
  sovereign_debt_coercion_loss_autonomy_score: number;
  conditionality_austerity_social_rights_erosion_score: number;
  resource_extraction_dispossession_scale_score: number;
  debt_restructuring_access_justice_deficit_score: number;
  estimated_debt_trap_economic_coercion_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_debt_trap_economic_coercion_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Coercition Souveraineté Dette", value: entity.sovereign_debt_coercion_loss_autonomy_score, weight: "0.30" },
    { label: "Austérité Droits Sociaux", value: entity.conditionality_austerity_social_rights_erosion_score, weight: "0.25" },
    { label: "Extraction Ressources Spoliation", value: entity.resource_extraction_dispossession_scale_score, weight: "0.25" },
    { label: "Déficit Justice Restructuration", value: entity.debt_restructuring_access_justice_deficit_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.sovereign_debt_coercion_loss_autonomy_score === "number" ? entity.sovereign_debt_coercion_loss_autonomy_score.toFixed(1) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Coercition Souveraineté</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_debt_trap_economic_coercion_rights_index === "number" ? entity.estimated_debt_trap_economic_coercion_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Coercition Économique</div>
                </div>
              </div>
...
export default function DebtTrapEconomicCoercionRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/debt-trap-economic-coercion-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Pi&egrave;ge de la Dette &amp; Coercition &Eacute;conomique…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.sovereign_debt_coercion_loss_autonomy_score));
  const avgIndex = data?.avg_estimated_debt_trap_economic_coercion_rights_index ?? avg(allEntities.map(e => e.estimated_debt_trap_economic_coercion_rights_index));
  ...
  const avgDebt = avg(allEntities.map(e => e.sovereign_debt_coercion_loss_autonomy_score));
  const avgAusterity = avg(allEntities.map(e => e.conditionality_austerity_social_rights_erosion_score));
  const avgExtraction = avg(allEntities.map(e => e.resource_extraction_dispossession_scale_score));
  const avgJustice = avg(allEntities.map(e => e.debt_restructuring_access_justice_deficit_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Coercition Économique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Pi&egrave;ge de la Dette &amp; Coercition &Eacute;conomique</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Debt Trap &amp; Economic Coercion Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgDebt} label="Coercition Souveraineté Dette" />
          <GaugeRing value={avgAusterity} label="Austérité Droits Sociaux" />
          <GaugeRing value={avgExtraction} label="Extraction Ressources Spoliation" />
          <GaugeRing value={avgJustice} label="Déficit Justice Restructuration" />
        </div>
      </div>
          ...
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.sovereign_debt_coercion_loss_autonomy_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.sovereign_debt_coercion_loss_autonomy_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Coercition Économique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_debt_trap_economic_coercion_rights_index === "number" ? e.estimated_debt_trap_economic_coercion_rights_index.toFixed(2) : "—"}</span>
            </div>
