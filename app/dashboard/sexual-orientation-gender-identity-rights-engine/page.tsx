...
interface Entity {
  ...
  criminalization_legal_penalty_severity_score: number;
  state_sanctioned_violence_impunity_score: number;
  social_discrimination_hate_crime_exposure_score: number;
  legal_protection_recognition_gap_score: number;
  estimated_sogi_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_sogi_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Criminalisation & Pénalité", value: entity.criminalization_legal_penalty_severity_score, weight: "0.30" },
    { label: "Violence Étatique Impunité", value: entity.state_sanctioned_violence_impunity_score, weight: "0.25" },
    { label: "Discrimination & Crime Haine", value: entity.social_discrimination_hate_crime_exposure_score, weight: "0.25" },
    { label: "Lacune Protection Légale", value: entity.legal_protection_recognition_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_sogi_rights_index === "number" ? entity.estimated_sogi_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index SOGI Droits</div>
                </div>
              </div>
...
export default function SexualOrientationGenderIdentityRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/sexual-orientation-gender-identity-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation SOGI Droits…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_sogi_rights_index ?? avg(allEntities.map(e => e.estimated_sogi_rights_index));
  ...
  const avgCriminalization = avg(allEntities.map(e => e.criminalization_legal_penalty_severity_score));
  const avgStateViolence = avg(allEntities.map(e => e.state_sanctioned_violence_impunity_score));
  const avgDiscrimination = avg(allEntities.map(e => e.social_discrimination_hate_crime_exposure_score));
  const avgLegalGap = avg(allEntities.map(e => e.legal_protection_recognition_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index SOGI Droits", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Sexual Orientation & Gender Identity Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Criminalisation LGBTQ+, violence étatique et lacunes de protection SOGI — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles · Wave 181
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <GaugeRing value={avgStateViolence} label="Violence Étatique" />
          <GaugeRing value={avgDiscrimination} label="Discrimination" />
          <GaugeRing value={avgLegalGap} label="Lacune Légale" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index SOGI: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_sogi_rights_index === "number" ? e.estimated_sogi_rights_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
