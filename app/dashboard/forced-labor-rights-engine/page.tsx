      ...
      <div style={{ background:"#0f172a",border:"1px solid #1e293b",borderRadius:12,padding:24,maxWidth:600,width:"90%",maxHeight:"80vh",overflowY:"auto" }}
        ...
        {tab==="apercu" && (
          <div style={{ color:"#cbd5e1",fontSize:14,lineHeight:1.6 }}>
            <p>Score composite : <strong style={{ color:accent }}>{entity?.composite_score}</strong></p>
            <p>Niveau de risque : <strong style={{ color:accent }}>{entity?.risk_level}</strong></p>
            <p>Pays : {entity?.country}</p>
            <p>Pattern : {entity?.primary_pattern}</p>
          </div>
        )}
        {tab==="metriques" && (
          <div style={{ color:"#cbd5e1",fontSize:13 }}>
            {entity && Object.entries(entity)
              .filter(([k]) => k.endsWith("_score") && k!=="composite_score")
              .map(([k,v]) => (
        ...
        {tab==="sources" && (
          <div style={{ color:"#94a3b8",fontSize:13 }}>
            <p style={{ marginBottom:8 }}>Pattern principal : {entity?.primary_pattern}</p>
            <p>Domaine : {entity?.country}</p>
          </div>
        )}
      </div>
    </div>
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/forced-labor-rights-engine")
      .then(r => r.json())
  ...
  return (
    <div style={{ background:"#020617",minHeight:"100vh",color:"#f1f5f9",fontFamily:"system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth:1200,margin:"0 auto",padding:"32px 24px" }}>
        <h1 style={{ fontSize:28,fontWeight:700,color:ACCENT,marginBottom:8 }}>Travail Forcé, Esclavage Moderne &amp; Exploitation</h1>
        <p style={{ color:"#94a3b8",marginBottom:32 }}>CaelumSwarm™ · CSDDD 2024/1760 · Wave 222</p>
        <div style={{ display:"flex",gap:16,marginBottom:32 }}>
          <div style={{ background:"#0f172a",borderRadius:8,padding:20,flex:1,textAlign:"center" }}>
            <GaugeRing value={data.avg_composite} color={ACCENT} />
            <div style={{ color:"#94a3b8",fontSize:12,marginTop:8 }}>Score moyen composite</div>
            <div style={{ color:ACCENT,fontSize:18,fontWeight:700 }}>{data.avg_composite?.toFixed(2)}</div>
          </div>
          <div style={{ background:"#0f172a",borderRadius:8,padding:20,flex:1,display:"flex",flexDirection:"column",justifyContent:"center",gap:12 }}>
            <div style={{ display:"flex",justifyContent:"space-between" }}>
              <span style={{ color:"#94a3b8" }}>Total entités</span>
              <span style={{ color:ACCENT,fontWeight:700 }}>{data.total_entities}</span>
            </div>
            <div style={{ display:"flex",justifyContent:"space-between" }}>
              <span style={{ color:"#94a3b8" }}>Niveau critique</span>
              <span style={{ color:"#ef4444",fontWeight:700 }}>{data.risk_distribution?.critique ?? 0}</span>
            </div>
            <div style={{ display:"flex",justifyContent:"space-between" }}>
              <span style={{ color:"#94a3b8" }}>Confiance</span>
              <span style={{ color:"#22c55e",fontWeight:700 }}>{((data.confidence_score ?? 0.85)*100).toFixed(0)}%</span>
            </div>
          </div>
        </div>
