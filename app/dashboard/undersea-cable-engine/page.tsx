...
interface UCEntity {
  ...
  composite_score: number; cable_chokepoint_control_score: number;
  submarine_sabotage_capability_score: number; landing_station_surveillance_score: number;
  cable_build_monopoly_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_undersea_cable_index: number; last_updated: string;
}
interface UCSummary {
  ...
  entities: UCEntity[]; avg_estimated_undersea_cable_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: UCEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Contrôle des Points d'Étranglement", v: entity.cable_chokepoint_control_score, c: "#0284c7" },
              { l: "Capacité Sabotage Sous-Marin", v: entity.submarine_sabotage_capability_score, c: "#0369a1" },
              { l: "Surveillance Stations d'Atterrissage", v: entity.landing_station_surveillance_score, c: "#075985" },
              { l: "Monopole Construction Câbles", v: entity.cable_build_monopoly_score, c: "#0c4a6e" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Câbles Sous-Marins Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_undersea_cable_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Activation des protocoles de protection des infrastructures câbles critiques et surveillance accrue des zones sensibles</p><p>› Coordination avec alliés pour patrouilles sous-marines et protection des stations d'atterrissage stratégiques</p></>}
            {entity.risk_level === "élevé" && <><p>› Audit des dépendances en routes de câbles et développement de redondances via routes alternatives</p><p>› Surveillance renforcée des activités suspectes de navires près des infrastructures câbles critiques</p></>}
            {entity.risk_level === "modéré" && <><p>› Suivi des projets de construction de câbles et des acteurs dominants dans les consortiums câbles</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille sur les politiques de gouvernance des câbles sous-marins et les accords d'atterrissage</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function UnderseaCablePage() {
  const [data, setData] = useState<UCSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<UCEntity | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/undersea-cable-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données câbles sous-marins"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse des infrastructures câbles sous-marins…</div></div>;
  ...
  const avgScore = (k: keyof UCEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Undersea Cable Intelligence Engine</h1>
          <p className="text-slate-400 text-sm">Cartographie des vulnérabilités des câbles sous-marins et risques d'infrastructure critique · v{data.engine_version}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Sources de Données", v: data.data_sources.length, g: "from-sky-900 to-sky-800", t: "text-sky-300" },
            { l: "Confiance Analyse", v: `${Math.round(data.confidence_score * 100)}%`, g: "from-slate-800 to-slate-700", t: "text-slate-300" },
            { l: "Index Câbles Sous-Marins", v: data.avg_estimated_undersea_cable_index, g: "from-sky-900 to-sky-800", t: "text-sky-300" },
          ].map(({ l, v, g, t }) => (
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs de Vulnérabilité Câbles Sous-Marins Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("cable_chokepoint_control_score")} label="Chokepoints Câbles" color="#0284c7" />
            <GaugeRing value={avgScore("submarine_sabotage_capability_score")} label="Sabotage Sous-Marin" color="#0369a1" />
            <GaugeRing value={avgScore("landing_station_surveillance_score")} label="Surveillance Stations" color="#075985" />
            <GaugeRing value={avgScore("cable_build_monopoly_score")} label="Monopole Construction" color="#0c4a6e" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns Câbles Sous-Marins</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={data.total_entities} color={["#0284c7", "#0369a1", "#075985", "#0c4a6e", "#082f49"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        {data.critical_alerts.length > 0 && (
          <div className="rounded-xl p-4" style={{ backgroundColor: "#0284c710", border: "1px solid #0284c730" }}>
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Câbles Sous-Marins Critiques</h2>
            <ul className="space-y-1">{data.critical_alerts.map((a, i) => <li key={i} className="text-sm" style={{ color: "#38bdf8" }}>› {a}</li>)}</ul>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              onMouseEnter={e => (e.currentTarget.style.borderColor = "#0284c750")}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_undersea_cable_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Undersea Cable Intelligence Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
