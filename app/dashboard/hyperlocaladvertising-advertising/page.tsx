...
type Entity = {
  ...
  risk_level: string;
  estimated_hyperlocaladvertising_index: number;
};
...
const SIGNALS = [
  "Precise location consent",
  "POI data collection",
  "Geo-fence transparency",
  "Movement tracking limits",
  "Local audience profiling",
];

const ACTIONS = [
  "Enforce precise location consent",
  "Limit POI data collection",
  "Disclose geo-fencing",
  "Cap movement tracking",
  "Restrict local profiling",
];
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque hyper-local advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_hyperlocaladvertising_index.toFixed(2)} / 10`,
    `Conformité plateforme hyper-local : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 417 — Hyper-Local Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Hyper-Local Advertising · Wave 417</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: entity.estimated_hyperlocaladvertising_index * 10, color: COLOR },
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            {ACTIONS.map((action, i) => (
              <div key={i} style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(22,163,74,0.15)" }}>
...
export default function HyperlocaladvertisingAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/hyperlocaladvertising-advertising")
      .then((r) => r.json())
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Hyper-Local Advertising…
      </div>
    );
  }
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + e.estimated_hyperlocaladvertising_index, 0) / entities.length
    : 0;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Hyper-Local Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#16a34a99", margin: 0 }}>
          Caelum Partners · Wave 417 · Analyse publicitaire hyper-locale · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
        ...
        {filtered.map((entity, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
              ...
              <span style={{ color: COLOR, fontWeight: 600 }}>{entity.estimated_hyperlocaladvertising_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Hyper-Local Advertising Intelligence · Wave 417
      </div>
