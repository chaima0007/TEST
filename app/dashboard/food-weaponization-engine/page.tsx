...
interface FWEntity {
  ...
  composite_score: number; grain_export_control_score: number;
  fertilizer_monopolization_score: number; agricultural_land_grab_score: number;
  food_aid_coercion_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_food_weapon_index: number; last_updated: string;
}
interface FWSummary {
  ...
  entities: FWEntity[]; avg_estimated_food_weapon_index: number;
}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Contrôle Exportations Céréales", v: entity.grain_export_control_score, c: "#f59e0b" },
              { l: "Monopolisation Engrais", v: entity.fertilizer_monopolization_score, c: "#f97316" },
              { l: "Accaparement Terres Agricoles", v: entity.agricultural_land_grab_score, c: "#eab308" },
              { l: "Coercition Aide Alimentaire", v: entity.food_aid_coercion_score, c: "#dc2626" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index d'Arme Alimentaire Estimé</span>
              <span className="text-amber-400 font-bold">{entity.estimated_food_weapon_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Plan d'urgence de diversification des chaînes alimentaires stratégiques</p><p>› Sanctions sur les exportations coercitives et constitution de réserves souveraines</p></>}
            {entity.risk_level === "élevé" && <><p>› Réduction des dépendances en intrants agricoles critiques</p><p>› Renforcement des accords multilatéraux sur la sécurité alimentaire</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance des flux céréaliers et des prix des engrais</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille sur les stratégies d'influence alimentaire</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function FoodWeaponizationEnginePage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/food-weaponization-engine")
  ...
  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse de la weaponisation alimentaire mondiale…</div></div>;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Food Weaponization Intelligence</h1>
          <p className="text-slate-400 text-sm">Weaponisation alimentaire &amp; coercition par la faim — Caelum Partners · v{data.engine_version}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Index Arme Alimentaire Moy.", v: data.avg_estimated_food_weapon_index, g: "from-amber-900 to-amber-800", t: "text-amber-300" },
          ].map(({ l, v, g, t }) => (
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          ...
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("grain_export_control_score")} label="Contrôle Exportations Céréales" color="#f59e0b" />
            <GaugeRing value={avgScore("fertilizer_monopolization_score")} label="Monopolisation Engrais" color="#f97316" />
            <GaugeRing value={avgScore("agricultural_land_grab_score")} label="Accaparement Terres Agricoles" color="#eab308" />
            <GaugeRing value={avgScore("food_aid_coercion_score")} label="Coercition Aide Alimentaire" color="#dc2626" />
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Distribution Risques Weaponisation Alimentaire</h2>
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Weaponisation Alimentaire</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={data.total_entities} color={["#f59e0b", "#f97316", "#eab308", "#10b981", "#dc2626"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-amber-900/10 border border-amber-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-amber-400 mb-2">Alertes Weaponisation Alimentaire Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_food_weapon_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Food Weaponization Intelligence · {data.last_analysis}</p>
      </div>
    </div>
  );
}
