...
interface Entity {
  ...
  scholar_persecution_imprisonment_severity_score: number;
  curriculum_ideological_control_censorship_scale_score: number;
  university_autonomy_state_capture_score: number;
  girls_education_access_denial_gap_score: number;
  estimated_academic_freedom_education_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_academic_freedom_education_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Persécution & Emprisonnement Chercheurs", value: entity.scholar_persecution_imprisonment_severity_score, weight: "0.30" },
    { label: "Contrôle Idéologique Curriculums", value: entity.curriculum_ideological_control_censorship_scale_score, weight: "0.25" },
    { label: "Capture Étatique Universités", value: entity.university_autonomy_state_capture_score, weight: "0.25" },
    { label: "Refus Accès Éducation Filles", value: entity.girls_education_access_denial_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_academic_freedom_education_rights_index === "number" ? entity.estimated_academic_freedom_education_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Liberté Académique</div>
                </div>
              </div>
...
export default function AcademicFreedomEducationRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/academic-freedom-education-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Liberté Académique & Droit à l&apos;Éducation…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_academic_freedom_education_rights_index ?? avg(allEntities.map(e => e.estimated_academic_freedom_education_rights_index));
  ...
  const avgScholarPersecution = avg(allEntities.map(e => e.scholar_persecution_imprisonment_severity_score));
  const avgCurriculumControl = avg(allEntities.map(e => e.curriculum_ideological_control_censorship_scale_score));
  const avgUniversityCapture = avg(allEntities.map(e => e.university_autonomy_state_capture_score));
  const avgGirlsEducation = avg(allEntities.map(e => e.girls_education_access_denial_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Liberté Académique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Liberté Académique & Droit à l&apos;Éducation</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Academic Freedom & Education Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgScholarPersecution} label="Persécution Chercheurs" />
          <GaugeRing value={avgCurriculumControl} label="Contrôle Curriculums" />
          <GaugeRing value={avgUniversityCapture} label="Capture Universités" />
          <GaugeRing value={avgGirlsEducation} label="Accès Éducation Filles" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Liberté Académique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_academic_freedom_education_rights_index === "number" ? e.estimated_academic_freedom_education_rights_index.toFixed(2) : "—"}</span>
            </div>
