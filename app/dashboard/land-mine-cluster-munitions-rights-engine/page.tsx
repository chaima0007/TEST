...
interface Entity {
  ...
  active_landmine_civilian_casualty_severity_score: number;
  cluster_munition_unexploded_ordnance_scale_score: number;
  mine_ban_treaty_non_compliance_score: number;
  victim_assistance_demining_funding_deficit_gap_score: number;
  estimated_land_mine_cluster_munitions_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_land_mine_cluster_munitions_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Mines Actives & Victimes Civiles", value: entity.active_landmine_civilian_casualty_severity_score, weight: "0.30" },
    { label: "Sous-Munitions & Engins Non-Explosés", value: entity.cluster_munition_unexploded_ordnance_scale_score, weight: "0.25" },
    { label: "Non-Conformité Traité Interdiction Mines", value: entity.mine_ban_treaty_non_compliance_score, weight: "0.25" },
    { label: "Déficit Aide Victimes & Financement Déminage", value: entity.victim_assistance_demining_funding_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_land_mine_cluster_munitions_rights_index === "number" ? entity.estimated_land_mine_cluster_munitions_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Mines & Sous-Munitions</div>
                </div>
              </div>
...
export default function LandMineClusterMunitionsRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/land-mine-cluster-munitions-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Mines Antipersonnel & Sous-Munitions…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_land_mine_cluster_munitions_rights_index ?? avg(allEntities.map(e => e.estimated_land_mine_cluster_munitions_rights_index));
  ...
  const avgActiveLandmine = avg(allEntities.map(e => e.active_landmine_civilian_casualty_severity_score));
  const avgClusterMunition = avg(allEntities.map(e => e.cluster_munition_unexploded_ordnance_scale_score));
  const avgMineBan = avg(allEntities.map(e => e.mine_ban_treaty_non_compliance_score));
  const avgVictimAssistance = avg(allEntities.map(e => e.victim_assistance_demining_funding_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Mines & Sous-Munitions", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Mines Antipersonnel & Sous-Munitions</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Land Mine Cluster Munitions Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgActiveLandmine} label="Mines Actives & Victimes" />
          <GaugeRing value={avgClusterMunition} label="Sous-Munitions & UXO" />
          <GaugeRing value={avgMineBan} label="Non-Conformité Traité" />
          <GaugeRing value={avgVictimAssistance} label="Aide Victimes & Déminage" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Mines & Sous-Munitions: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_land_mine_cluster_munitions_rights_index === "number" ? e.estimated_land_mine_cluster_munitions_rights_index.toFixed(2) : "—"}</span>
            </div>
