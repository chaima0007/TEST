...
interface Entity {
  ...
  last_updated: string;
  safe_water_access_denial_score?: number;
  water_privatization_exclusion_score?: number;
  sanitation_crisis_score?: number;
  industrial_water_contamination_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "safe_water_access_denial_score", label: "Déni d&apos;accès à l&apos;eau potable" },
  { key: "water_privatization_exclusion_score", label: "Exclusion par privatisation de l&apos;eau" },
  { key: "sanitation_crisis_score", label: "Crise d&apos;assainissement" },
  { key: "industrial_water_contamination_score", label: "Contamination industrielle de l&apos;eau" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse du droit à l&apos;eau : déni d&apos;accès à l&apos;eau potable, exclusion par privatisation de l&apos;eau, crise d&apos;assainissement et contamination industrielle de l&apos;eau.</p>
              <p className="text-slate-400">Source : Right to Water Rights Engine · Wave 249</p>
            </div>
          )}
        </div>
      </div>
...
export default function RightToWaterRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-water-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Right to Water Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Accès à l&apos;eau · Privatisation · Assainissement · Contamination Industrielle</p>
        </div>
