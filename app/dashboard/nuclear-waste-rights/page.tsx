...
interface Entity {
  ...
  radioactive_contamination_community_exposure_score: number;
  storage_site_safety_rights_deficit_score: number;
  indigenous_land_nuclear_waste_displacement_score: number;
  regulatory_accountability_transparency_gap_score: number;
  estimated_nuclear_waste_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_nuclear_waste_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Contamination Radioactive & Exposition Communauté", value: entity.radioactive_contamination_community_exposure_score, weight: "0.30" },
    { label: "Déficit Droits Sécurité Sites Stockage", value: entity.storage_site_safety_rights_deficit_score, weight: "0.25" },
    { label: "Déplacement Terres Autochtones Déchets Nucléaires", value: entity.indigenous_land_nuclear_waste_displacement_score, weight: "0.25" },
    { label: "Lacune Responsabilité & Transparence Réglementaire", value: entity.regulatory_accountability_transparency_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_nuclear_waste_rights_index === "number" ? entity.estimated_nuclear_waste_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Déchets Nucléaires</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = Contamination×0.30 + Sécurité×0.25 + Déplacement×0.25 + Régulation×0.20
                </div>
              </div>
...
export default function NuclearWasteRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/nuclear-waste-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits contre les Déchets Nucléaires…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_nuclear_waste_rights_index ?? avg(allEntities.map(e => e.estimated_nuclear_waste_rights_index));
  ...
  const avgContamination = avg(allEntities.map(e => e.radioactive_contamination_community_exposure_score));
  const avgStorage = avg(allEntities.map(e => e.storage_site_safety_rights_deficit_score));
  const avgIndigenous = avg(allEntities.map(e => e.indigenous_land_nuclear_waste_displacement_score));
  const avgRegulatory = avg(allEntities.map(e => e.regulatory_accountability_transparency_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Déchets Nucléaires", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits contre les Déchets Nucléaires</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Nuclear Waste Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgContamination} label="Contamination Radioactive" />
          <GaugeRing value={avgStorage} label="Sécurité Sites Stockage" />
          <GaugeRing value={avgIndigenous} label="Terres Autochtones" />
          <GaugeRing value={avgRegulatory} label="Régulation & Transparence" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Déchets Nucléaires: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_nuclear_waste_rights_index === "number" ? e.estimated_nuclear_waste_rights_index.toFixed(2) : "—"}</span>
            </div>
