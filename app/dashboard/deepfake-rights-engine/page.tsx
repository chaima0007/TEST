...
interface Entity {
  ...
  last_updated: string;
  non_consensual_deepfake_score?: number;
  political_disinformation_deepfake_score?: number;
  identity_fraud_deepfake_score?: number;
  legal_framework_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "non_consensual_deepfake_score", label: "Deepfakes non consentis" },
  { key: "political_disinformation_deepfake_score", label: "Désinformation politique par deepfake" },
  { key: "identity_fraud_deepfake_score", label: "Fraude d&apos;identité par deepfake" },
  { key: "legal_framework_gap_score", label: "Lacune du cadre juridique" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des violations des droits liées aux deepfakes : contenus non consentis, désinformation politique, fraude d&apos;identité et lacunes du cadre juridique de protection.</p>
              <p className="text-slate-400">Source : Deepfake Rights Engine · Wave 244</p>
            </div>
          )}
        </div>
      </div>
...
export default function DeepfakeRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/deepfake-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Deepfake Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Deepfakes Non Consentis · Désinformation Politique · Fraude Identité · Cadre Juridique</p>
        </div>
