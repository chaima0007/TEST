...
interface Entity {
  ...
  protest_violent_repression_severity_score: number;
  assembly_ban_criminalization_scale_score: number;
  protest_leader_arrest_persecution_score: number;
  counter_terrorism_law_assembly_misuse_gap_score: number;
  estimated_freedom_of_assembly_protest_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_freedom_of_assembly_protest_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Répression Violente Protestations", value: entity.protest_violent_repression_severity_score, weight: "0.30" },
    { label: "Criminalisation Rassemblements", value: entity.assembly_ban_criminalization_scale_score, weight: "0.25" },
    { label: "Arrestation Dirigeants Manifestants", value: entity.protest_leader_arrest_persecution_score, weight: "0.25" },
    { label: "Abus Lois Anti-Terrorisme", value: entity.counter_terrorism_law_assembly_misuse_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_freedom_of_assembly_protest_rights_index === "number" ? entity.estimated_freedom_of_assembly_protest_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Liberté de Réunion</div>
                </div>
              </div>
...
export default function FreedomOfAssemblyProtestRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/freedom-of-assembly-protest-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Liberté de Réunion…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_freedom_of_assembly_protest_rights_index ?? avg(allEntities.map(e => e.estimated_freedom_of_assembly_protest_rights_index));
  ...
  const avgRepressionViolente = avg(allEntities.map(e => e.protest_violent_repression_severity_score));
  const avgCriminalisationRassemblements = avg(allEntities.map(e => e.assembly_ban_criminalization_scale_score));
  const avgArrestationDirigeants = avg(allEntities.map(e => e.protest_leader_arrest_persecution_score));
  const avgAbusLois = avg(allEntities.map(e => e.counter_terrorism_law_assembly_misuse_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Liberté de Réunion", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Liberté de Réunion & Droit de Manifester</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Freedom of Assembly & Protest Rights Engine — Répression des manifestations, criminalisation et abus des lois anti-terrorisme — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgRepressionViolente} label="Répression Violente" />
          <GaugeRing value={avgCriminalisationRassemblements} label="Criminalisation" />
          <GaugeRing value={avgArrestationDirigeants} label="Arrestation Dirigeants" />
          <GaugeRing value={avgAbusLois} label="Abus Lois Anti-Terror." />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Liberté de Réunion: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_freedom_of_assembly_protest_rights_index === "number" ? e.estimated_freedom_of_assembly_protest_rights_index.toFixed(2) : "—"}</span>
            </div>
