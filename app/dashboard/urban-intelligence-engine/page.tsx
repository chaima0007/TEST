...
type Entity = {
  ...
  city_type: string;
  urban_risk: string;
  urban_pattern: string;
  urban_severity: string;
  recommended_action: string;
  surveillance_score: number;
  resilience_score: number;
  social_score: number;
  governance_score: number;
  urban_composite: number;
  is_urban_crisis: boolean;
  requires_urban_intervention: boolean;
  urban_signal: string;
};
...
type Summary = {
  ...
  avg_urban_composite: number;
  urban_crisis_count: number;
  urban_intervention_count: number;
  avg_surveillance_score: number;
  avg_resilience_score: number;
  avg_social_score: number;
  avg_governance_score: number;
  avg_estimated_urban_risk_index: number;
};
...
const PATTERN_COLORS: Record<string, string> = {
  none: "#10b981",
  surveillance_state: "#7c3aed",
  infrastructure_collapse: "#1d4ed8",
  social_explosion: "#dc2626",
  algorithmic_oppression: "#0891b2",
  smart_city_monopoly: "#b45309",
};
const SEVERITY_COLORS: Record<string, string> = {
  urban_resilient: "#10b981",
  urban_tension: "#f59e0b",
  high_urban_risk: "#3b82f6",
  urban_emergency: "#dc2626",
};
const ACTION_COLORS: Record<string, string> = {
  no_action: "#10b981",
  urban_monitoring: "#06b6d4",
  urban_resilience_program: "#3b82f6",
  civil_liberties_audit: "#7c3aed",
  urban_emergency_intervention: "#dc2626",
};
...
const SEV_BADGE: Record<string, string> = {
  urban_resilient: "bg-emerald-900 text-emerald-300",
  urban_tension: "bg-amber-900 text-amber-300",
  high_urban_risk: "bg-blue-900 text-blue-300",
  urban_emergency: "bg-red-950 text-red-400",
};
      ...
      >
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ["Surveillance", entity.surveillance_score, "#7c3aed"],
              ["Résilience", entity.resilience_score, "#3b82f6"],
              ["Social", entity.social_score, "#dc2626"],
            ...
            <div className="col-span-2 bg-slate-900 border border-blue-700/20 rounded-lg p-3">
              <div className="text-amber-300/60 text-xs mb-1">Composite Urbain</div>
              <div className="text-white font-bold text-2xl">
                {entity.urban_composite.toFixed(1)}
              </div>
            </div>
          </div>
        )}
            ...
            <div className="mt-3 flex gap-2 flex-wrap">
              <span
                className={`px-2 py-0.5 rounded text-xs font-medium ${
                  RISK_BADGE[entity.urban_risk] || "bg-slate-700 text-slate-300"
                }`}
              >
                {entity.urban_risk}
              </span>
              <span
                className={`px-2 py-0.5 rounded text-xs font-medium ${
                  SEV_BADGE[entity.urban_severity] || "bg-slate-700 text-slate-300"
                }`}
              >
                {entity.urban_severity.replace(/_/g, " ")}
              </span>
            </div>
          </div>
        )}
        ...
        {tab === "action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-900 border border-blue-700/20 rounded-lg p-3">
              <div className="text-amber-300/60 text-xs mb-1">Pattern Urbain</div>
              <div className="text-white font-medium capitalize">
                {entity.urban_pattern.replace(/_/g, " ")}
              </div>
            </div>
            <div className="flex gap-2">
              {entity.is_urban_crisis && (
                <span className="px-2 py-1 rounded bg-red-950 text-red-400 text-xs font-medium">
                  CRISE URBAINE
                </span>
              )}
              {entity.requires_urban_intervention && (
...
export default function UrbanIntelligenceDashboard() {
  ...
  useEffect(() => {
    fetch("/api/urban-intelligence-engine")
      .then(r => r.json())
      .then(setData)
      .catch(console.error);
  }, []);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-amber-400 text-lg animate-pulse">
          Initialisation du Moteur d&apos;Intelligence Urbaine...
        </div>
      </div>
    );
  }
  ...
  const filtered = entities.filter(
    e =>
      (filter === "all" || e.urban_risk === filter) &&
      (patFilter === "all" || e.urban_pattern === patFilter)
  );

  const dists = [
    { title: "Niveau Risque Urbain",   counts: summary.risk_counts,     colors: RISK_COLORS     },
    { title: "Pattern Urbain",         counts: summary.pattern_counts,  colors: PATTERN_COLORS  },
    { title: "Sévérité Urbaine",       counts: summary.severity_counts, colors: SEVERITY_COLORS },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-amber-400">
          Intelligence Urbaine &amp; Villes Intelligentes — Module 308
        </h1>
        <p className="text-blue-400/60 text-sm mt-1">
          Surveillance · Résilience · Social · Gouvernance — Caelum Partners — Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          ["Crises Urbaines",               summary.urban_crisis_count,                           "text-red-400"],
          ["Interventions Requises",        summary.urban_intervention_count,                     "text-blue-400"],
          ["Composite Moyen",               summary.avg_urban_composite.toFixed(1),               "text-amber-300"],
          ["Indice Risque Urbain Moyen",    summary.avg_estimated_urban_risk_index.toFixed(2)+"/10","text-amber-400"],
          ["Score Gouvernance Moyen",       summary.avg_governance_score.toFixed(1),              "text-blue-300"],
        ].map(([l, v, c]) => (
          <div
            key={String(l)}
      ...
      <div className="bg-slate-900 border border-blue-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          <GaugeRing value={summary.avg_surveillance_score} label="Score Surveillance" color="#7c3aed" />
          <GaugeRing value={summary.avg_resilience_score}   label="Score Résilience"   color="#3b82f6" />
          <GaugeRing value={summary.avg_social_score}       label="Score Social"        color="#dc2626" />
          <GaugeRing value={summary.avg_governance_score}   label="Score Gouvernance"   color="#f59e0b" />
        </div>
      </div>
          ...
          <button
            ...
            className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
              filter === r
                ...
                : "bg-slate-900 border-blue-700/30 text-amber-400/70 hover:text-white"
            }`}
          >
            {r}
          </button>
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div className="flex gap-1 mb-3 flex-wrap">
              <span
                className={`px-2 py-0.5 rounded text-xs font-medium ${
                  RISK_BADGE[e.urban_risk] || "bg-slate-700 text-slate-300"
                }`}
              >
                {e.urban_risk}
              </span>
              <span
                className={`px-2 py-0.5 rounded text-xs font-medium ${
                  SEV_BADGE[e.urban_severity] || "bg-slate-700 text-slate-300"
                }`}
              >
                {e.urban_severity.replace(/_/g, " ")}
              </span>
            </div>
            <div className="text-2xl font-black text-white mb-1">
              {e.urban_composite.toFixed(1)}
            </div>
            <div className="text-xs text-amber-400/60 mb-2 capitalize">
              {e.urban_pattern.replace(/_/g, " ")}
            </div>
