...
interface DVEntity {
  ...
  composite_score: number; femicide_intimate_partner_score: number;
  legal_protection_gap_score: number; impunity_prosecution_failure_score: number;
  economic_coercion_dependency_score: number; risk_level: string; primary_pattern: string;
  key_signals: string[]; avg_estimated_domestic_violence_index: number; last_updated: string;
}
interface DVSummary {
  ...
  confidence_score: number; data_sources: string[]; entities: DVEntity[];
  avg_estimated_domestic_violence_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: DVEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "Aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Féminicide/Partenaire", v: entity.femicide_intimate_partner_score, c: "#ef4444" },
              { l: "Lacune Protection Jur.", v: entity.legal_protection_gap_score, c: "#f97316" },
              { l: "Impunité Poursuites", v: entity.impunity_prosecution_failure_score, c: "#a855f7" },
              { l: "Coercition Économique", v: entity.economic_coercion_dependency_score, c: "#f59e0b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Violence Domestique</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.avg_estimated_domestic_violence_index}</span>
            </div>
          </div>
        )}
        {tab === "Signaux" && (
        ...
        {tab === "Contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Mécanismes d'urgence pour les victimes de violence conjugale</p><p>› Hébergements d'urgence et protection immédiate des survivantes</p><p>› Mobilisation internationale contre l'impunité du féminicide</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des lois de protection contre la violence domestique</p><p>› Formation des forces de l'ordre à la violence de genre</p></>}
            {entity.risk_level === "modéré" && <><p>› Développement de programmes d'autonomisation économique</p><p>› Surveillance des lacunes dans le cadre légal de protection</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien des programmes de prévention de la violence</p><p>› Consolidation des mécanismes de signalement et soutien</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  violence_systemique: "#ef4444",
  impunite_structurelle: "#f97316",
  protection_insuffisante: "#f59e0b",
  tensions_genre: "#eab308",
  protection_exemplaire: "#10b981",
};

export default function DomesticViolenceEnginePage() {
  const [data, setData] = useState<DVSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<DVEntity | null>(null);

  useEffect(() => {
    fetch("/api/domestic-violence-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données violence domestique"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la violence domestique en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof DVEntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Domestic Violence Engine</h1>
          <p className="text-slate-400 text-sm mt-1">La violence domestique comme fracture silencieuse — quand le foyer devient danger et l'impunité système, c'est l'État lui-même qui faillit</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Moyen", v: `${data.avg_composite}%`, t: "text-rose-300" },
            ...
            { l: "Index Moyen VD", v: data.avg_estimated_domestic_violence_index, t: "text-rose-400" },
            { l: "Confiance", v: `${Math.round((data.confidence_score ?? 0) * 100)}%`, t: "text-slate-300" },
          ].map(({ l, v, t }) => (
            <div key={l} className="bg-slate-900 border border-rose-900/30 rounded-xl p-4">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 rounded-xl p-6 border border-rose-900/30">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Dimensions de la Violence — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("femicide_intimate_partner_score")} label="Féminicide/Partenaire" color={ACCENT} />
            <GaugeRing value={avgScore("legal_protection_gap_score")} label="Lacune Protection Jur." color="#f97316" />
            <GaugeRing value={avgScore("impunity_prosecution_failure_score")} label="Impunité Poursuites" color="#a855f7" />
            <GaugeRing value={avgScore("economic_coercion_dependency_score")} label="Coercition Économique" color="#f59e0b" />
          </div>
        </div>

        {/* Distribution Bars */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-rose-900/30">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-rose-900/30">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Violence Domestique</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-rose-950/20 border border-rose-900/30 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Critiques — Violence Domestique</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              className={`bg-slate-900 rounded-xl p-4 border cursor-pointer transition-all hover:border-rose-700/50 ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.avg_estimated_domestic_violence_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Domestic Violence Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
