...
interface Entity {
  ...
  last_updated: string;
  public_space_harassment_score?: number;
  gender_based_intimidation_score?: number;
  police_response_failure_score?: number;
  mobility_restriction_fear_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "public_space_harassment_score", label: "Harcèlement espace public" },
  { key: "gender_based_intimidation_score", label: "Intimidation basée sur le genre" },
  { key: "police_response_failure_score", label: "Défaillance réponse policière" },
  { key: "mobility_restriction_fear_score", label: "Restriction mobilité par la peur" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse du harcèlement de rue : harcèlement dans l&apos;espace public, intimidations basées sur le genre, défaillances des réponses policières et restrictions de mobilité induites par la peur.</p>
              <p className="text-slate-400">Source : Street Harassment Rights Engine · Wave 261</p>
            </div>
          )}
        </div>
      </div>
...
export default function StreetHarassmentRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/street-harassment-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Street Harassment Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Espace Public · Intimidation · Police · Mobilité Restreinte</p>
        </div>
