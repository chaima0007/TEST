...
interface Entity {
  ...
  last_updated: string;
  neural_data_exploitation_score?: number;
  cognitive_enhancement_coercion_score?: number;
  mental_privacy_violation_score?: number;
  thought_surveillance_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "neural_data_exploitation_score", label: "Exploitation données neurales" },
  { key: "cognitive_enhancement_coercion_score", label: "Coercition amélioration cognitive" },
  { key: "mental_privacy_violation_score", label: "Violation vie privée mentale" },
  { key: "thought_surveillance_score", label: "Surveillance des pensées" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la liberté cognitive : exploitation des données neurales, coercition à l&apos;amélioration cognitive, violations de la vie privée mentale et surveillance des pensées par des systèmes IA.</p>
              <p className="text-slate-400">Source : Cognitive Liberty Rights Engine · Wave 259</p>
            </div>
          )}
        </div>
      </div>
...
export default function CognitiveLibertyRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/cognitive-liberty-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Cognitive Liberty Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Données Neurales · Coercition Cognitive · Vie Privée Mentale · Surveillance</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-indigo-900/40 text-indigo-300"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "#6366f120" } : {}}>
