...
interface Entity {
  ...
  last_updated: string;
  reconstruction_exclusion_score?: number;
  transitional_justice_failure_score?: number;
  reparation_program_gap_score?: number;
  reconciliation_mechanism_absence_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "reconstruction_exclusion_score", label: "Exclusion de la reconstruction" },
  { key: "transitional_justice_failure_score", label: "Échec de la justice transitionnelle" },
  { key: "reparation_program_gap_score", label: "Lacunes des programmes de réparation" },
  { key: "reconciliation_mechanism_absence_score", label: "Absence de mécanismes de réconciliation" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits à la reconstruction post-conflit : exclusion des processus de reconstruction, échecs de la justice transitionnelle, lacunes des programmes de réparation et absence de mécanismes de réconciliation.</p>
              <p className="text-slate-400">Source : Post-Conflict Reconstruction Rights Engine · Wave 254</p>
            </div>
          )}
        </div>
      </div>
...
export default function PostConflictReconstructionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/post-conflict-reconstruction-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Post-Conflict Reconstruction Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Exclusion Reconstruction · Justice Transitionnelle · Réparations · Réconciliation</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#4b556320" } : {}}>
