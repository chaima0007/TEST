...
interface Entity {
  ...
  conflict_mineral_labor_abuse_score: number;
  artisanal_miner_rights_deficit_score: number;
  child_labor_exploitation_score: number;
  armed_group_coercion_score: number;
  estimated_coltan_mining_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_coltan_mining_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Abus Travail Minerais Conflits", value: entity.conflict_mineral_labor_abuse_score, weight: "0.30" },
    { label: "Déficit Droits Mineurs Artisanaux", value: entity.artisanal_miner_rights_deficit_score, weight: "0.25" },
    { label: "Exploitation Travail Enfants", value: entity.child_labor_exploitation_score, weight: "0.25" },
    { label: "Coercition Groupes Armés", value: entity.armed_group_coercion_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_coltan_mining_rights_index === "number" ? entity.estimated_coltan_mining_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Mines de Coltan</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = Minerais×0.30 + Artisanaux×0.25 + Enfants×0.25 + Coercition×0.20
                </div>
              </div>
...
export default function ColtanMiningRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/coltan-mining-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits dans les Mines de Coltan…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_coltan_mining_rights_index ?? avg(allEntities.map(e => e.estimated_coltan_mining_rights_index));
  ...
  const avgMinerais = avg(allEntities.map(e => e.conflict_mineral_labor_abuse_score));
  const avgArtisanaux = avg(allEntities.map(e => e.artisanal_miner_rights_deficit_score));
  const avgEnfants = avg(allEntities.map(e => e.child_labor_exploitation_score));
  const avgCoercition = avg(allEntities.map(e => e.armed_group_coercion_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Mines de Coltan", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits dans les Mines de Coltan</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Coltan Mining Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgMinerais} label="Minerais de Conflit" />
          <GaugeRing value={avgArtisanaux} label="Mineurs Artisanaux" />
          <GaugeRing value={avgEnfants} label="Travail Enfants" />
          <GaugeRing value={avgCoercition} label="Coercition Armée" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Mines Coltan: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_coltan_mining_rights_index === "number" ? e.estimated_coltan_mining_rights_index.toFixed(2) : "—"}</span>
            </div>
