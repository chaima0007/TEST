...
interface FRSEntity {
  ...
  mass_surveillance_deployment_scale_score: number;
  minority_targeting_bias_severity_score: number;
  legal_oversight_framework_absence_score: number;
  chilling_effect_dissent_suppression_score: number;
  estimated_facial_recognition_surveillance_index: number;
  risk_level: string;
...
interface FRSData {
  ...
  data_sources: string[];
  entities: FRSEntity[];
  avg_estimated_facial_recognition_surveillance_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function FacialRecognitionSurveillancePage() {
  const [data, setData] = useState<FRSData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<FRSEntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/facial-recognition-surveillance-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-cyan-400 animate-pulse">Chargement des données de surveillance…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "mass_surveillance_deployment_scale_score", label: "Déploiement Masse" },
    { key: "minority_targeting_bias_severity_score", label: "Biais Ciblage Minorités" },
    { key: "legal_oversight_framework_absence_score", label: "Absence Cadre Légal" },
    { key: "chilling_effect_dissent_suppression_score", label: "Effet Dissuasif Dissidence" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Surveillance par Reconnaissance Faciale</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Facial Recognition Surveillance Engine</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_facial_recognition_surveillance_index ?? "—" },
            { label: "Confidence", value: `${Math.round((data.confidence_score ?? 0) * 100)}%` },
          ].map(k => (
            <div key={k.label} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              <div className="text-xs text-slate-500 mb-1">{k.label}</div>
              <div className="text-xl font-bold text-cyan-400">{k.value}</div>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#22d3ee" />
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            ...
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#22d3ee", "#0891b2", "#06b6d4", "#67e8f9", "#a5f3fc"];
        ...
        {(data.critical_alerts?.length ?? 0) > 0 && (
          <div className="border border-cyan-500/30 bg-cyan-500/10 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-cyan-400 mb-2">Alertes Critiques</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => { setSelected(entity); setTab(0); }}
              className={`bg-slate-900 border rounded-xl p-4 cursor-pointer hover:border-cyan-500/50 transition-all ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#22d3ee" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_facial_recognition_surveillance_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Surveillance par Reconnaissance Faciale · {data.last_analysis}</p>
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Vue générale", "Sous-scores", "Alertes"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`px-6 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-cyan-500 text-cyan-400" : "text-slate-500 hover:text-slate-300"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#22d3ee" />
                    <div>
                      <div className="text-2xl font-bold text-cyan-400">{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Surveillance Reconnaissance Faciale</div>
                    <div className="text-lg font-bold text-cyan-400">{selected.estimated_facial_recognition_surveillance_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
                ...
                <ul className="space-y-2">
                  {(selected.key_signals ?? []).map((s, i) => (
                    <li key={i} className="flex gap-2 text-sm text-slate-300">
                      <span className="text-cyan-400 shrink-0 mt-0.5">▸</span>{s}
                    </li>
                  ))}
                </ul>
              )}
