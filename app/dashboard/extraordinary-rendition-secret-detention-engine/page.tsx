...
interface Entity {
  ...
  risk_level: string
  estimated_rendition_rights_index: number
}
...
export default function ExtraordinaryRenditionDashboard() {
  const [data, setData] = useState<DomainData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch("/api/extraordinary-rendition-secret-detention-engine")
      .then((r) => r.json())
      .then((d) => {
        setData(d.payload ?? d)
        setLoading(false)
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 8 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: "#f1f5f9" }}>
              Extraordinary Rendition &amp; Secret Detention
            </h1>
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8, gap: 8 }}>
                <span style={{ color: "#6366f1", fontSize: 11, fontWeight: 700, letterSpacing: 1 }}>{entity.id}</span>
                <RiskBadge level={entity.risk_level} />
              </div>
              <p style={{ margin: "0 0 10px", fontSize: 13, color: "#cbd5e1", lineHeight: 1.5 }}>{entity.name}</p>
              <div style={{ display: "flex", gap: 16 }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: ACCENT }}>{entity.estimated_rendition_rights_index.toFixed(2)}</div>
                  <div style={{ fontSize: 11, color: "#64748b" }}>Indice rendition</div>
                </div>
                <div>
