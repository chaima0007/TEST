...
interface Entity {
  ...
  last_updated: string;
  autonomy_suppression_score?: number;
  fpic_violation_score?: number;
  territorial_sovereignty_denial_score?: number;
  governance_interference_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "autonomy_suppression_score", label: "Suppression de l&apos;autonomie" },
  { key: "fpic_violation_score", label: "Violation du CLIP" },
  { key: "territorial_sovereignty_denial_score", label: "Refus de souveraineté territoriale" },
  { key: "governance_interference_score", label: "Ingérence dans la gouvernance" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de l&apos;autodétermination autochtone : suppression de l&apos;autonomie, violations du CLIP, refus de souveraineté territoriale et ingérence dans la gouvernance.</p>
              <p className="text-slate-400">Source : Indigenous Self Determination Rights Engine · Wave 248</p>
            </div>
          )}
        </div>
      </div>
...
export default function IndigenousSelfDeterminationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/indigenous-self-determination-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Indigenous Self Determination Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Autonomie · CLIP · Souveraineté Territoriale · Gouvernance</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#14532d20" } : {}}>
