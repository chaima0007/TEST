...
interface Entity {
  ...
  country: string;
  sector: string;
  trafficking_victim_scale_vulnerability_score: number;
  labor_exploitation_debt_bondage_severity_score: number;
  sex_trafficking_prosecution_impunity_score: number;
  victim_identification_support_deficit_score: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Échelle Victimes &amp; Vulnérabilité Trafic", value: entity.trafficking_victim_scale_vulnerability_score, weight: "0.30" },
    { label: "Exploitation Travail &amp; Servitude pour Dettes", value: entity.labor_exploitation_debt_bondage_severity_score, weight: "0.25" },
    { label: "Impunité Trafic Sexuel", value: entity.sex_trafficking_prosecution_impunity_score, weight: "0.25" },
    { label: "Déficit Identification Victimes", value: entity.victim_identification_support_deficit_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{entity.trafficking_victim_scale_vulnerability_score.toFixed(1)}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Victimes Trafic</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_human_trafficking_modern_slavery_index === "number" ? entity.estimated_human_trafficking_modern_slavery_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Trafic Humain</div>
                </div>
              </div>
...
export default function HumanTraffickingModernSlaveryEnginePage() {
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Trafic Humain &amp; Esclavage Moderne…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.trafficking_victim_scale_vulnerability_score));
  const avgIndex = data?.avg_estimated_human_trafficking_modern_slavery_index ?? avg(allEntities.map(e => e.estimated_human_trafficking_modern_slavery_index));
  ...
  const avgVictim = avg(allEntities.map(e => e.trafficking_victim_scale_vulnerability_score));
  const avgLabor = avg(allEntities.map(e => e.labor_exploitation_debt_bondage_severity_score));
  const avgSex = avg(allEntities.map(e => e.sex_trafficking_prosecution_impunity_score));
  const avgIdentification = avg(allEntities.map(e => e.victim_identification_support_deficit_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Trafic Humain", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Trafic Humain &amp; Esclavage Moderne</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Human Trafficking &amp; Modern Slavery Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgVictim} label="Échelle Victimes Trafic" />
          <GaugeRing value={avgLabor} label="Exploitation Travail &amp; Servitude" />
          <GaugeRing value={avgSex} label="Impunité Trafic Sexuel" />
          <GaugeRing value={avgIdentification} label="Déficit Identification Victimes" />
        </div>
      </div>
          ...
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.trafficking_victim_scale_vulnerability_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.trafficking_victim_scale_vulnerability_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Trafic Humain: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_human_trafficking_modern_slavery_index === "number" ? e.estimated_human_trafficking_modern_slavery_index.toFixed(2) : "—"}</span>
            </div>
