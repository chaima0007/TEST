...
interface Entity {
  ...
  last_updated: string;
  untouchability_practices_score?: number;
  caste_based_violence_score?: number;
  social_exclusion_score?: number;
...
const SUB_SCORES = [
  { key: "untouchability_practices_score", label: "Pratiques d&apos;intouchabilité" },
  { key: "caste_based_violence_score", label: "Violence basée sur la caste" },
  { key: "social_exclusion_score", label: "Exclusion sociale" },
  { key: "legal_protection_gap_score", label: "Déficit de protection légale" },
];
...
export default function CasteDiscriminationRightsDashboard() {
  ...
  useEffect(() => {
    fetch("/api/caste-discrimination-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Caste Discrimination Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Pratiques d&apos;Intouchabilité · Violence de Caste · Exclusion Sociale · Déficit Légal</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-[#78350f]/40 text-[#a05c20]"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "rgba(120,53,15,0.15)" } : {}}>
