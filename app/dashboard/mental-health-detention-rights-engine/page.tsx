...
interface Entity {
  ...
  last_updated: string;
  involuntary_hospitalization_score?: number;
  punitive_psychiatry_score?: number;
  patient_rights_violation_score?: number;
  indefinite_detention_mental_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "involuntary_hospitalization_score", label: "Hospitalisation involontaire" },
  { key: "punitive_psychiatry_score", label: "Psychiatrie punitive" },
  { key: "patient_rights_violation_score", label: "Violation droits patients" },
  { key: "indefinite_detention_mental_score", label: "Détention psychiatrique indéfinie" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la détention psychiatrique : hospitalisations involontaires abusives, psychiatrie punitive, violations des droits des patients et détentions psychiatriques indéfinies.</p>
              <p className="text-slate-400">Source : Mental Health Detention Rights Engine · Wave 262</p>
            </div>
          )}
        </div>
      </div>
...
export default function MentalHealthDetentionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/mental-health-detention-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Mental Health Detention Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Hospitalisation Forcée · Psychiatrie Punitive · Droits Patients · Détention</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-purple-900/40 text-purple-300"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "#7e22ce20" } : {}}>
