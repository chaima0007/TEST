...
interface Entity {
  ...
  state_enforced_disappearance_severity_score: number;
  family_notification_denial_body_concealment_scale_score: number;
  impunity_perpetrators_accountability_gap_score: number;
  truth_commission_reparation_mechanism_deficit_gap_score: number;
  estimated_forced_disappearances_extrajudicial_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_forced_disappearances_extrajudicial_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Disparitions Forcées par l'État", value: entity.state_enforced_disappearance_severity_score, weight: "0.30" },
    { label: "Déni Notification Famille & Corps Cachés", value: entity.family_notification_denial_body_concealment_scale_score, weight: "0.25" },
    { label: "Impunité des Auteurs", value: entity.impunity_perpetrators_accountability_gap_score, weight: "0.25" },
    { label: "Déficit Commissions Vérité & Réparations", value: entity.truth_commission_reparation_mechanism_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_forced_disappearances_extrajudicial_index === "number" ? entity.estimated_forced_disappearances_extrajudicial_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Disparitions Forcées</div>
                </div>
              </div>
...
export default function ForcedDisappearancesExtrajudicialEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/forced-disappearances-extrajudicial-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Disparitions Forcées & Exécutions Extrajudiciaires…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_forced_disappearances_extrajudicial_index ?? avg(allEntities.map(e => e.estimated_forced_disappearances_extrajudicial_index));
  ...
  const avgStateDisappearance = avg(allEntities.map(e => e.state_enforced_disappearance_severity_score));
  const avgFamilyDenial = avg(allEntities.map(e => e.family_notification_denial_body_concealment_scale_score));
  const avgImpunity = avg(allEntities.map(e => e.impunity_perpetrators_accountability_gap_score));
  const avgTruthCommission = avg(allEntities.map(e => e.truth_commission_reparation_mechanism_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Disparitions Forcées", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Disparitions Forcées & Exécutions Extrajudiciaires</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Forced Disappearances Extrajudicial Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgStateDisappearance} label="Disparitions Étatiques" />
          <GaugeRing value={avgFamilyDenial} label="Déni Famille & Corps" />
          <GaugeRing value={avgImpunity} label="Impunité Auteurs" />
          <GaugeRing value={avgTruthCommission} label="Commissions Vérité" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Disparitions Forcées: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_forced_disappearances_extrajudicial_index === "number" ? e.estimated_forced_disappearances_extrajudicial_index.toFixed(2) : "—"}</span>
            </div>
