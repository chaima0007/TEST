...
interface HREntity {
  ...
  homelessness_inadequate_housing_scale_score: number;
  housing_discrimination_marginalized_score: number;
  rent_speculation_financialization_gap_score: number;
...
export default function HousingRightsPage() {
  ...
  const [selected, setSelected] = useState<HREntity | null>(null);
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données droits au logement…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "forced_eviction_displacement_severity_score", label: "Expulsions Forcées" },
    { key: "homelessness_inadequate_housing_scale_score", label: "Sans-Abrisme" },
    { key: "housing_discrimination_marginalized_score", label: "Discrimination Logement" },
    { key: "rent_speculation_financialization_gap_score", label: "Spéculation Immobilière" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits au Logement</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Expulsions Forcées · Sans-Abrisme · Discrimination Logement · Spéculation Immobilière — surveillance des droits au logement
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Logement", value: data.avg_estimated_housing_rights_index ?? "—" },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Crise Logement</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#f97316", "#ef4444", "#eab308", "#7c3aed", "#10b981"];
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-500">idx {entity.estimated_housing_rights_index}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droits au Logement · {data.last_analysis}</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Logement</div>
                    <div className="text-lg font-bold" style={{ color: ACCENT }}>{selected.estimated_housing_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
