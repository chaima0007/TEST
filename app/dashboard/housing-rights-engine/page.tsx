...
interface HREntity {
  ...
  composite_score: number; forced_eviction_displacement_score: number;
  homelessness_shelter_denial_score: number; affordability_habitability_score: number;
  discriminatory_housing_score: number; risk_level: string; primary_pattern: string;
  key_signals: string[]; avg_estimated_housing_rights_index: number; last_updated: string;
}
interface HRSummary {
  ...
  confidence_score: number; data_sources: string[]; entities: HREntity[];
  avg_estimated_housing_rights_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: HREntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "Aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Expulsion Forcée", v: entity.forced_eviction_displacement_score, c: "#ef4444" },
              { l: "Sans-Abrisme/Abri", v: entity.homelessness_shelter_denial_score, c: "#f97316" },
              { l: "Inabordabilité", v: entity.affordability_habitability_score, c: "#a855f7" },
              { l: "Discrimination Logement", v: entity.discriminatory_housing_score, c: "#f59e0b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Droits au Logement</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.avg_estimated_housing_rights_index}</span>
            </div>
          </div>
        )}
        {tab === "Signaux" && (
        ...
        {tab === "Contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Mécanismes d'urgence pour la prévention des expulsions forcées</p><p>› Mobilisation de logements d'urgence pour les sans-abri</p><p>› Intervention légale contre les discriminations au logement</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des programmes de logement social</p><p>› Surveillance des pratiques d'expulsion et de discrimination</p></>}
            {entity.risk_level === "modéré" && <><p>› Développement de politiques de logement inclusives</p><p>› Renforcement de la résilience des communautés locataires</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien des standards minimaux de logement décent</p><p>› Consolidation des programmes de logement abordable</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  crise_logement_aigue: "#ef4444",
  inegalites_acces_logement: "#f97316",
  fragilite_immobiliere: "#f59e0b",
  tensions_logement: "#eab308",
  logement_exemplaire: "#10b981",
};

export default function HousingRightsEnginePage() {
  const [data, setData] = useState<HRSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<HREntity | null>(null);

  useEffect(() => {
    fetch("/api/housing-rights-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données droits au logement"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse des droits au logement en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof HREntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Housing Rights Engine</h1>
          <p className="text-slate-400 text-sm mt-1">Le logement comme droit fondamental — quand un toit devient un luxe, c'est la dignité humaine qui s'effondre</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Moyen", v: `${data.avg_composite}%`, t: "text-lime-300" },
            ...
            { l: "Index Moyen Logement", v: data.avg_estimated_housing_rights_index, t: "text-lime-400" },
            { l: "Confiance", v: `${Math.round((data.confidence_score ?? 0) * 100)}%`, t: "text-slate-300" },
          ].map(({ l, v, t }) => (
            <div key={l} className="bg-slate-900 border border-lime-500/20 rounded-xl p-4">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 rounded-xl p-6 border border-lime-500/20">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Dimensions des Violations du Droit au Logement — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("forced_eviction_displacement_score")} label="Expulsion Forcée" color={ACCENT} />
            <GaugeRing value={avgScore("homelessness_shelter_denial_score")} label="Sans-Abrisme/Abri" color="#f97316" />
            <GaugeRing value={avgScore("affordability_habitability_score")} label="Inabordabilité" color="#a855f7" />
            <GaugeRing value={avgScore("discriminatory_housing_score")} label="Discrimination Logement" color="#f59e0b" />
          </div>
        </div>

        {/* Distribution Bars */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-lime-500/20">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-lime-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Crise Immobilière</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-lime-900/10 border border-lime-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Critiques — Droits au Logement</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.avg_estimated_housing_rights_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Housing Rights Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
