...
interface Entity {
  ...
  country: string;
  sector: string;
  child_soldier_recruitment_scale_score: number;
  school_hospital_attack_civilian_harm_score: number;
  family_separation_displacement_trauma_score: number;
  child_protection_mechanism_absence_score: number;
  estimated_children_rights_armed_conflict_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_children_rights_armed_conflict_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Recrutement Enfants Soldats", value: entity.child_soldier_recruitment_scale_score, weight: "0.30" },
    { label: "Attaques Écoles & Hôpitaux", value: entity.school_hospital_attack_civilian_harm_score, weight: "0.25" },
    { label: "Séparation & Déplacement", value: entity.family_separation_displacement_trauma_score, weight: "0.25" },
    { label: "Absence Protection Enfants", value: entity.child_protection_mechanism_absence_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.child_soldier_recruitment_scale_score === "number" ? entity.child_soldier_recruitment_scale_score.toFixed(1) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Recrutement Enfants</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_children_rights_armed_conflict_index === "number" ? entity.estimated_children_rights_armed_conflict_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Enfants Conflit</div>
                </div>
              </div>
...
export default function ChildrenRightsArmedConflictEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/children-rights-armed-conflict-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Enfants en Conflits Armés…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.child_soldier_recruitment_scale_score));
  const avgIndex = data?.avg_estimated_children_rights_armed_conflict_index ?? avg(allEntities.map(e => e.estimated_children_rights_armed_conflict_index));
  ...
  const avgRecruitment = avg(allEntities.map(e => e.child_soldier_recruitment_scale_score));
  const avgSchoolAttacks = avg(allEntities.map(e => e.school_hospital_attack_civilian_harm_score));
  const avgDisplacement = avg(allEntities.map(e => e.family_separation_displacement_trauma_score));
  const avgProtection = avg(allEntities.map(e => e.child_protection_mechanism_absence_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Enfants Conflit", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Enfants en Conflits Armés</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Children Rights Armed Conflict Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgRecruitment} label="Recrutement Enfants Soldats" />
          <GaugeRing value={avgSchoolAttacks} label="Attaques Écoles & Hôpitaux" />
          <GaugeRing value={avgDisplacement} label="Séparation & Déplacement" />
          <GaugeRing value={avgProtection} label="Absence Protection Enfants" />
        </div>
      </div>
          ...
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.child_soldier_recruitment_scale_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.child_soldier_recruitment_scale_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Enfants: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_children_rights_armed_conflict_index === "number" ? e.estimated_children_rights_armed_conflict_index.toFixed(2) : "—"}</span>
            </div>
