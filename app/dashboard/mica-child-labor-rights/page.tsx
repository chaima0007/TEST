...
interface Entity {
  ...
  risk_level: string;
  estimated_mica_child_labor_index: number;
}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <h2 style={{ color: "#fff", fontSize: 20, fontWeight: 700 }}>Détails — Travail Enfant dans les Mines de Mica</h2>
        ...
        {tab === "analyse" && (
          <div style={{ color: "#cbd5e1", lineHeight: 1.7 }}>
            <p style={{ marginBottom: 12 }}>
              L&apos;extraction du mica implique massivement le travail des enfants, notamment en Inde
              et à Madagascar. Ces enfants travaillent dans des conditions dangereuses — tunnels instables,
              poussières toxiques — pour alimenter des chaînes d&apos;approvisionnement mondiales en
              cosmétiques, électronique et peintures industrielles.
            </p>
            <p style={{ marginBottom: 12 }}>
              Score composite moyen : <strong style={{ color: "#fff" }}>{data.avg_composite.toFixed(2)}</strong> — niveau critique
              indiquant une exploitation persistante malgré les engagements volontaires des entreprises.
            </p>
            <p>
              Les mécanismes de traçabilité et les audits de chaîne d&apos;approvisionnement restent
              insuffisants pour éradiquer cette forme de travail forcé infantile.
            </p>
          </div>
        )}
                ...
                <tbody>
                  {data.entities.map((e) => (
                    <tr key={e.entity} style={{ borderBottom: "1px solid #0f172a" }}>
                      ...
                      <td style={{ color: "#f1f5f9", textAlign: "right", padding: "8px 4px" }}>{e.estimated_mica_child_labor_index.toFixed(2)}</td>
        ...
        {tab === "methodologie" && (
          <div style={{ color: "#cbd5e1", lineHeight: 1.7 }}>
            <p style={{ marginBottom: 12 }}>
              <strong style={{ color: "#fff" }}>Moteur :</strong> mica_child_labor_rights_engine
            </p>
            <p style={{ marginBottom: 12 }}>
              <strong style={{ color: "#fff" }}>Sous-scores :</strong> Prévalence du travail enfant (×0.30) ·
              Cadre légal national (×0.25) · Traçabilité chaîne (×0.25) · Accès à l&apos;éducation (×0.20)
            </p>
            <p style={{ marginBottom: 12 }}>
...
export default function MicaChildLaborRightsDashboard() {
  ...
  useEffect(() => {
    fetch("/api/mica-child-labor-rights")
      .then((r) => r.json())
      .then((d) => {
        setData(d.payload ?? d);
        setLoading(false);
      ...
      <div style={{ maxWidth: 1024, margin: "0 auto" }}>
        ...
        <div style={{ borderLeft: `4px solid ${ACCENT}`, paddingLeft: 24, marginBottom: 32 }}>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: "#fff", margin: 0 }}>
            Droits — Travail Enfant dans les Mines de Mica
          </h1>
          <p style={{ color: "#94a3b8", marginTop: 8 }}>
            Droits de l&apos;Enfant · Mines Artisanales · Chaîne d&apos;Approvisionnement · Travail Forcé
          </p>
        </div>
        ...
        <div style={{ color: "#475569", fontSize: 12, textAlign: "right" }}>
          Moteur : mica_child_labor_rights_engine · Wave 278 · Caelum Partners
        </div>
      </div>
    </div>
  );
