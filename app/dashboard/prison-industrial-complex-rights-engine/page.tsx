...
interface Entity {
  ...
  mass_incarceration_racial_disparity_score: number;
  private_prison_profit_incentive_score: number;
  labor_exploitation_incarcerated_score: number;
  rehabilitation_access_denial_score: number;
  estimated_prison_industrial_complex_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_prison_industrial_complex_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Incarcération Masse & Disparité Raciale", value: entity.mass_incarceration_racial_disparity_score, weight: "0.30" },
    { label: "Profit Prisons Privées", value: entity.private_prison_profit_incentive_score, weight: "0.25" },
    { label: "Exploitation Travail Détenus", value: entity.labor_exploitation_incarcerated_score, weight: "0.25" },
    { label: "Déni Réhabilitation", value: entity.rehabilitation_access_denial_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                ...
                <div style={{ background: "rgba(30,41,59,0.5)", borderRadius: "12px", padding: "16px", textAlign: "center" }}>
                  <div style={{ fontSize: "30px", fontWeight: "bold", color: ACCENT }}>{typeof entity.estimated_prison_industrial_complex_index === "number" ? entity.estimated_prison_industrial_complex_index.toFixed(2) : "—"}</div>
                  <div style={{ fontSize: "12px", color: "#94a3b8", marginTop: "4px" }}>Index Complexe Carcéral</div>
                </div>
              </div>
...
export default function PrisonIndustrialComplexRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/prison-industrial-complex-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ background: "#020617", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontSize: "14px", color: ACCENT }}>Initialisation Complexe Carcéral Industriel &amp; Droits…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_prison_industrial_complex_index ?? avg(allEntities.map(e => e.estimated_prison_industrial_complex_index));
  ...
  const avgMassIncarceration = avg(allEntities.map(e => e.mass_incarceration_racial_disparity_score));
  const avgPrivatePrison = avg(allEntities.map(e => e.private_prison_profit_incentive_score));
  const avgLabor = avg(allEntities.map(e => e.labor_exploitation_incarcerated_score));
  const avgRehabilitation = avg(allEntities.map(e => e.rehabilitation_access_denial_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Complexe Carcéral", value: avgIndex.toFixed(2) },
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "white", padding: "24px", display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "4px" }}>
          ...
          <h1 style={{ fontSize: "24px", fontWeight: "bold", letterSpacing: "-0.025em" }}>Complexe Carcéral Industriel &amp; Droits</h1>
        </div>
        <p style={{ color: "#94a3b8", fontSize: "14px", marginLeft: "24px" }}>
          Prison Industrial Complex Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: "12px", padding: "20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: "24px" }}>
          <GaugeRing value={avgMassIncarceration} label="Incarcération & Racisme" />
          <GaugeRing value={avgPrivatePrison} label="Profit Prisons Privées" />
          <GaugeRing value={avgLabor} label="Exploitation Travail" />
          <GaugeRing value={avgRehabilitation} label="Déni Réhabilitation" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div style={{ fontSize: "12px", color: "#64748b", marginTop: "8px" }}>
              Index Complexe Carcéral: <span style={{ fontWeight: 500, color: ACCENT }}>{typeof e.estimated_prison_industrial_complex_index === "number" ? e.estimated_prison_industrial_complex_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
