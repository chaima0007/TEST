...
interface Entity {
  ...
  last_updated: string;
  humanitarian_goods_blockage_score?: number;
  medicine_access_denial_score?: number;
  civilian_economic_impact_score?: number;
  sanctions_exemption_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "humanitarian_goods_blockage_score", label: "Blocage biens humanitaires" },
  { key: "medicine_access_denial_score", label: "Refus d&apos;accès aux médicaments" },
  { key: "civilian_economic_impact_score", label: "Impact économique civil" },
  { key: "sanctions_exemption_gap_score", label: "Lacunes exemptions sanctions" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits civils sous régime de sanctions : blocage des biens humanitaires, refus d&apos;accès aux médicaments, impact économique sur les civils et lacunes dans les exemptions humanitaires.</p>
              <p className="text-slate-400">Source : Sanctions Civilian Rights Engine · Wave 247</p>
            </div>
          )}
        </div>
      </div>
...
export default function SanctionsCivilianRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/sanctions-civilian-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Sanctions Civilian Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Biens Humanitaires · Médicaments · Impact Économique · Exemptions</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-stone-700/40 text-stone-300"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "#44403c20" } : {}}>
