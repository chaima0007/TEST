...
type UMEEntity = {
  ...
  signal: string;
  ewaste_volume_growth: number;
  informal_recycling_rate: number;
};
...
type UMESummary = {
  ...
  avg_estimated_urban_mining_index: number;
};
...
const PATTERN_COLORS: Record<string, string> = {
  none: "#16a34a",
  toxic_ewaste_dumping_crisis: "#dc2626",
  critical_metal_supply_gap: "#7c3aed",
  informal_sector_health_catastrophe: "#b45309",
  extended_producer_responsibility_failure: "#0891b2",
  planned_obsolescence_acceleration: "#c2410c",
};
const SEVERITY_COLORS: Record<string, string> = {
  "surveillance_filière_déchets_électroniques": "#16a34a",
  "déficit_économie_circulaire_structurel": "#d97706",
  "crise_recyclage_déchets_électroniques_majeure": "#c2410c",
  "crise_minage_urbain_systémique": "#dc2626",
};
const ACTION_COLORS: Record<string, string> = {
  "veille_minage_urbain_continue": "#16a34a",
  "amélioration_collecte_et_traçabilité_déchets": "#d97706",
  "renforcement_filière_recyclage_électronique_accéléré": "#c2410c",
  "intervention_urgente_minage_urbain_critique": "#dc2626",
};
...
const SEV_BADGE: Record<string, string> = {
  "surveillance_filière_déchets_électroniques": "bg-green-900 text-green-300",
  "déficit_économie_circulaire_structurel": "bg-orange-900 text-orange-300",
  "crise_recyclage_déchets_électroniques_majeure": "bg-orange-950 text-orange-400",
  "crise_minage_urbain_systémique": "bg-red-950 text-red-400",
};

function DetailModal({ entity, onClose }: { entity: UMEEntity; onClose: () => void }) {
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <span className="ml-2 text-slate-500 text-xs">
              {entity.electronics_sector.replace(/_/g, " ")}
            </span>
          </div>
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ["Score Déchets Élec.",  entity.ewaste_score,      "#dc2626"],
              ["Score Récupération",   entity.recovery_score,    "#7c3aed"],
              ["Score Toxicité",       entity.toxicity_score,    "#b45309"],
            ...
            <div className="col-span-2 bg-slate-900 border border-orange-700/20 rounded-lg p-3">
              <div className="text-orange-500/60 text-xs mb-1">Score Composite Minage Urbain</div>
          ...
          <div className="bg-slate-900 border border-orange-700/20 rounded-lg p-4 text-sm text-slate-200 leading-relaxed">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="bg-slate-800 rounded p-2">
                <div className="text-orange-500/50 text-xs mb-0.5">Volume Déchets Élec.</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.ewaste_volume_growth * 100)}%
                </div>
              </div>
              <div className="bg-slate-800 rounded p-2">
                <div className="text-orange-500/50 text-xs mb-0.5">Recyclage Informel</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.informal_recycling_rate * 100)}%
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-900 border border-orange-700/20 rounded-lg p-3">
              <div className="text-orange-500/60 text-xs mb-1">Secteur Électronique</div>
...
export default function UrbanMiningDashboard() {
  ...
  const [selected, setSelected] = useState<UMEEntity | null>(null);

  useEffect(() => {
    fetch("/api/urban-mining-engine")
      .then(r => r.json())
      .then(setData)
      .catch(console.error);
  }, []);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-orange-500 text-lg animate-pulse">
          Initialisation du Moteur Minage Urbain...
        </div>
      </div>
    );
  }

  const { entities, summary, avg_ewaste } = data;
  ...
  const dists = [
    ...
    { title: "Patron Minage Urbain",   counts: summary.pattern_distribution,  colors: PATTERN_COLORS  },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-orange-500">
          Minage Urbain &amp; Recyclage Déchets Électroniques — Module 410
        </h1>
        <p className="text-slate-500/60 text-sm mt-1">
          Déchets · Récupération · Toxicité · Gouvernance — Caelum Partners — Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ["Total Entités",              summary.total,                                                        "text-orange-500"],
          ["Crise Minage Urbain",        summary.critical,                                                     "text-red-400"],
          ["Crise Majeure",              summary.high,                                                         "text-orange-700"],
          ["Composite Moyen",            `${summary.avg_composite.toFixed(1)}`,                                "text-slate-300"],
          ["Index Minage Urbain",        `${summary.avg_estimated_urban_mining_index.toFixed(2)}/10`,          "text-orange-500"],
          ["Déchets Élec. Moyen",        `${avg_ewaste.toFixed(1)}`,                                          "text-slate-300"],
        ].map(([l, v, c]) => (
          <div
            key={String(l)}
      ...
      <div className="bg-slate-900 border border-orange-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          <GaugeRing
            value={entities.reduce((s, e) => s + e.ewaste_score, 0) / (entities.length || 1)}
          ...
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.toxicity_score, 0) / (entities.length || 1)}
            label="Toxicité Moy."
            color="#b45309"
          />
          <GaugeRing
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {[
          "all",
          "none",
          "toxic_ewaste_dumping_crisis",
          "critical_metal_supply_gap",
          "informal_sector_health_catastrophe",
          "extended_producer_responsibility_failure",
          "planned_obsolescence_acceleration",
        ].map(p => (
          <button
            key={p}
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div className="text-xs text-slate-500 mb-2 capitalize">
              {e.electronics_sector.replace(/_/g, " ")}
            </div>
            ...
            <div className="text-xs text-orange-500/70 font-medium mb-2">
              Déchets: {e.ewaste_score.toFixed(1)} · Toxicité: {e.toxicity_score.toFixed(1)}
            </div>
