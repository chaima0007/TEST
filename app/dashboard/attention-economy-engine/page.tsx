...
interface AEEntity {
  ...
  composite_score: number; platform_capture_score: number;
  algorithmic_manipulation_score: number; cognitive_bandwidth_erosion_score: number;
  behavioral_modification_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_attention_index: number; last_updated: string;
}
interface AESummary {
  ...
  entities: AEEntity[]; avg_estimated_attention_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: AEEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Capture Plateformes", v: entity.platform_capture_score, c: "#ef4444" },
              { l: "Manipulation Algorithmique", v: entity.algorithmic_manipulation_score, c: "#f97316" },
              { l: "Érosion Bande Passante", v: entity.cognitive_bandwidth_erosion_score, c: "#eab308" },
              { l: "Modification Comportementale", v: entity.behavioral_modification_score, c: "#f59e0b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Attentionnel Caelum</span>
              <span className="text-amber-400 font-bold">{entity.estimated_attention_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Intervention d&apos;urgence contre la colonisation cognitive des plateformes</p><p>› Régulation immédiate des algorithmes de capture attentionnelle</p><p>› Programmes d&apos;urgence de désintoxication numérique collective</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des cadres réglementaires sur la manipulation algorithmique</p><p>› Programmes de souveraineté cognitive et d&apos;éducation numérique</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance accrue des vecteurs de capture attentionnelle</p><p>› Politiques de protection de la bande passante cognitive collective</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille sur les indicateurs attentionnels</p><p>› Consolidation des pratiques de souveraineté cognitive</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function AttentionEconomyPage() {
  const [data, setData] = useState<AESummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<AEEntity | null>(null);

  useEffect(() => {
    fetch("/api/attention-economy-engine")
      .then(r => r.json())
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données de l'économie de l'attention"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse de l&apos;économie de l&apos;attention mondiale…</div></div>;
  ...
  const avgScore = (k: keyof AEEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Attention Economy Engine — Capture Cognitive &amp; Disruption Attentionnelle</h1>
          <p className="text-slate-400 text-sm">Mesure de la capture attentionnelle : plateformes · algorithmes · bande passante cognitive · v{data.engine_version}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Index Attentionnel", v: data.avg_estimated_attention_index, g: "from-purple-900 to-purple-800", t: "text-purple-300" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs Attentionnels Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("platform_capture_score")} label="Capture Plateformes" color="#ef4444" />
            <GaugeRing value={avgScore("algorithmic_manipulation_score")} label="Manipulation Algorithmique" color="#f97316" />
            <GaugeRing value={avgScore("cognitive_bandwidth_erosion_score")} label="Érosion Bande Passante" color="#eab308" />
            <GaugeRing value={avgScore("behavioral_modification_score")} label="Modification Comportementale" color="#f59e0b" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Disruption Attentionnelle</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-amber-900/10 border border-amber-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-amber-400 mb-2">Alertes Colonisation Cognitive Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_attention_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Économie de l&apos;Attention &amp; Souveraineté Cognitive · {data.last_analysis}</p>
      </div>
    </div>
  );
}
