...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/transnational-repression-engine").then(r => r.json()).then(j => setData(j.payload ?? j));
  }, []);
  ...
  const PATTERN_LABELS: Record<string, string> = {
    ciblage_diaspora_systematique: "Ciblage Diaspora",
    assassinat_extraterritorial: "Assassinat Extraterritorial",
    surveillance_numerique_frontiere: "Surveillance Numérique",
    coercition_pays_accueil: "Coercition Pays Accueil",
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Transnational Repression Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Surveillance de la répression des dissidents en exil et violations de souveraineté</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    {["diaspora_targeting_intensity_score", "extraterritorial_assassination_score", "digital_transborder_surveillance_score", "host_country_coercion_score"].map(key => (
