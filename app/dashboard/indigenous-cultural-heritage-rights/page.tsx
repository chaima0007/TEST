...
interface Entity {
  ...
  sacred_site_violation_score?: number;
...
const SUB_SCORES = [
  ...
  { key: "land_dispossession_score", label: "Dépossession territoriale" },
  { key: "sacred_site_violation_score", label: "Violation de sites sacrés" },
  { key: "language_suppression_score", label: "Suppression linguistique" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits au patrimoine culturel autochtone : effacement culturel, dépossession territoriale, violation de sites sacrés et suppression linguistique affectant les peuples autochtones à travers le monde.</p>
              <p className="text-slate-400">Source : Indigenous Cultural Heritage Rights Engine · Wave 270</p>
            </div>
          )}
        </div>
      </div>
...
export default function IndigenousCulturalHeritageRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/indigenous-cultural-heritage-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits au Patrimoine Culturel Autochtone</h1>
          <p className="text-slate-400 mt-1 text-sm">Effacement Culturel · Dépossession Territoriale · Sites Sacrés · Suppression Linguistique</p>
        </div>
