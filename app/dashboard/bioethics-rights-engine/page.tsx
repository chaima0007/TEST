...
interface Entity {
  ...
  non_consensual_experimentation_severity_score: number;
  organ_harvesting_trafficking_scale_score: number;
  genetic_data_surveillance_coercion_score: number;
  medical_ai_algorithmic_discrimination_gap_score: number;
  estimated_bioethics_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_bioethics_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Expérimentation Non Consentie", value: entity.non_consensual_experimentation_severity_score, weight: "0.30" },
    { label: "Prélèvement Organes", value: entity.organ_harvesting_trafficking_scale_score, weight: "0.25" },
    { label: "Surveillance Génétique", value: entity.genetic_data_surveillance_coercion_score, weight: "0.25" },
    { label: "IA Médicale Discrimination", value: entity.medical_ai_algorithmic_discrimination_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_bioethics_rights_index === "number" ? entity.estimated_bioethics_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Bioéthique</div>
                </div>
              </div>
...
export default function BioethicsRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/bioethics-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Bioéthique…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_bioethics_rights_index ?? avg(allEntities.map(e => e.estimated_bioethics_rights_index));
  ...
  const avgExperimentation = avg(allEntities.map(e => e.non_consensual_experimentation_severity_score));
  const avgOrganHarvesting = avg(allEntities.map(e => e.organ_harvesting_trafficking_scale_score));
  const avgGeneticSurveillance = avg(allEntities.map(e => e.genetic_data_surveillance_coercion_score));
  const avgMedicalAI = avg(allEntities.map(e => e.medical_ai_algorithmic_discrimination_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Bioéthique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Bioethics Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Expérimentation non consentie, prélèvement organes et discrimination par IA médicale — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgExperimentation} label="Expérimentation NC" />
          <GaugeRing value={avgOrganHarvesting} label="Prélèvement Organes" />
          <GaugeRing value={avgGeneticSurveillance} label="Surveillance Génétique" />
          <GaugeRing value={avgMedicalAI} label="IA Médicale Discrim." />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Bioéthique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_bioethics_rights_index === "number" ? e.estimated_bioethics_rights_index.toFixed(2) : "—"}</span>
            </div>
