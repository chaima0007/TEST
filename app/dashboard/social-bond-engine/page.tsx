...
interface BondEntity {
  ...
  composite_score: number; impact_measurement_deficit_score: number;
  greenwashing_risk_score: number; investor_trust_erosion_score: number;
  regulatory_compliance_gap_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_bond_index: number; last_updated: string;
}
interface BondSummary {
  ...
  entities: BondEntity[]; avg_estimated_bond_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: BondEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Déficit mesure impact", v: entity.impact_measurement_deficit_score, c: "#ef4444" },
              { l: "Risque greenwashing", v: entity.greenwashing_risk_score, c: "#f97316" },
              { l: "Érosion confiance", v: entity.investor_trust_erosion_score, c: "#eab308" },
              { l: "Gap réglementaire", v: entity.regulatory_compliance_gap_score, c: "#8b5cf6" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Obligation</span>
              <span className="text-violet-400 font-bold">{entity.estimated_bond_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Audit d'impact indépendant immédiat</p><p>› Suspension temporaire des émissions</p><p>› Communication de crise investisseurs</p></>}
            {entity.risk_level === "élevé" && <><p>› Certification tierce partie sous 30 jours</p><p>› Révision des métriques ESG</p></>}
            {entity.risk_level === "modéré" && <><p>› Renforcement du reporting d'impact</p><p>› Alignement taxonomie UE</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien des standards actuels</p><p>› Extension des programmes d'impact</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function SocialBondPage() {
  const [data, setData] = useState<BondSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<BondEntity | null>(null);

  useEffect(() => {
    fetch("/api/social-bond-engine")
      .then(r => r.json())
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement obligations sociales"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse obligations sociales…</div></div>;
  ...
  const avgScore = (key: keyof BondEntity) => Math.round(data.entities.reduce((s, e) => s + (e[key] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Obligations Sociales à Impact — Intelligence ESG</h1>
          <p className="text-slate-400 text-sm">Surveillance mondiale des obligations sociales · v{data.engine_version}</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { l: "Entités", v: data.total_entities, g: "from-slate-700 to-slate-600", t: "text-white" },
            { l: "Score Moyen", v: `${data.avg_composite}%`, g: "from-violet-900 to-violet-800", t: "text-violet-300" },
            { l: "Alertes Critiques", v: data.critical_alerts.length, g: "from-red-900 to-red-800", t: "text-red-300" },
            { l: "Index Bond Moyen", v: data.avg_estimated_bond_index, g: "from-blue-900 to-blue-800", t: "text-blue-300" },
            { l: "Sources", v: data.data_sources.length, g: "from-teal-900 to-teal-800", t: "text-teal-300" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Indicateurs Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("impact_measurement_deficit_score")} label="Déficit Impact" color="#ef4444" />
            <GaugeRing value={avgScore("greenwashing_risk_score")} label="Risque Greenwashing" color="#f97316" />
            <GaugeRing value={avgScore("investor_trust_erosion_score")} label="Érosion Confiance" color="#eab308" />
            <GaugeRing value={avgScore("regulatory_compliance_gap_score")} label="Gap Réglementaire" color="#8b5cf6" />
          </div>
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_bond_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Obligations Sociales · {data.last_analysis}</p>
      </div>
    </div>
  );
}
