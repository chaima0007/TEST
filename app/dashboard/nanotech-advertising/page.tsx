...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque publicitaire nanotech : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_index.toFixed(2)} / 10`,
    `Conformité campagne nanotech : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 387 — Nanotech Advertising Intelligence`,
  ];
}
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        style={{ background: "#0a0f1e", border: "1px solid rgba(14,116,144,0.3)", borderRadius: 16, width: "100%", maxWidth: 520, padding: 24, boxShadow: "0 25px 50px rgba(0,0,0,0.7)" }}
        onClick={(e) => e.stopPropagation()}
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700, color: "#fff" }}>{entity.name}</div>
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Nanotech Advertising · Wave 387</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              <div key={label} style={{ background: "#1e293b", borderRadius: 10, padding: 14, border: "1px solid rgba(14,116,144,0.2)" }}>
            ...
            <div style={{ gridColumn: "span 2", background: "#1e293b", borderRadius: 10, padding: 14, border: "1px solid rgba(14,116,144,0.2)" }}>
        ...
        {tab === "signaux" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              ...
              `Indice Caelum Nanotech Advertising : ${entity.estimated_index.toFixed(2)} / 10`,
              `Campagne nanotech en ${entity.risk_level === "faible" ? "zone verte — performance nominale" : "zone d&apos;alerte — optimisation requise"}`,
            ].map((sig, i) => (
              <div key={i} style={{ background: "#1e293b", borderRadius: 8, padding: 12, fontSize: 13, color: "#e2e8f0", border: "1px solid rgba(14,116,144,0.15)", lineHeight: 1.5 }}>
                {sig}
              </div>
            ))}
          </div>
        ...
        {tab === "certs" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {certs.map((cert, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(14,116,144,0.15)" }}>
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(14,116,144,0.15)" }}>
              <div style={{ fontSize: 11, color: "#93c5fd", marginBottom: 4 }}>Action Prioritaire</div>
              <div>{entity.risk_level === "critique" ? "Audit immédiat des campagnes nanotech — conformité réglementaire urgente" : entity.risk_level === "élevé" ? "Révision des stratégies publicitaires — renforcement compliance" : entity.risk_level === "modéré" ? "Optimisation continue des métriques publicitaires nanotech" : "Maintien des bonnes pratiques — veille sectorielle"}</div>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(14,116,144,0.15)" }}>
...
export default function NanotechAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/nanotech-advertising")
      .then((r) => r.json())
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Nanotech Advertising…
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Nanotech Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#0e749099", margin: 0 }}>
          Caelum Partners · Wave 387 · Analyse publicitaire nanotech · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 32 }}>
        {[
          ...
          <div key={label} style={{ background: "#0f172a", border: "1px solid rgba(14,116,144,0.25)", borderRadius: 12, padding: "16px 14px" }}>
      ...
      {/* Gauges */}
      <div style={{ background: "#0f172a", border: "1px solid rgba(14,116,144,0.25)", borderRadius: 14, padding: 20, marginBottom: 24 }}>
        ...
        {FILTER_OPTIONS.map((opt) => (
          <button
            ...
            style={{
              ...
              border: `1px solid ${filter === opt ? COLOR : "rgba(14,116,144,0.3)"}`,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14, marginBottom: 24 }}>
        {filtered.map((entity, i) => (
          <div
            ...
            style={{
              background: "#0f172a", border: "1px solid rgba(14,116,144,0.25)", borderRadius: 12, padding: 16,
              cursor: "pointer", transition: "border-color 0.2s",
            }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = COLOR; }}
            onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(14,116,144,0.25)"; }}
          >
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Nanotech Advertising Intelligence · Wave 387
      </div>
