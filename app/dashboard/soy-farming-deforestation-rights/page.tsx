...
interface Entity {
  ...
  deforestation_land_rights_deficit_score: number;
  indigenous_territory_violation_score: number;
  monoculture_expansion_rights_score: number;
  agrochemical_contamination_rights_score: number;
  estimated_soy_farming_deforestation_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_soy_farming_deforestation_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déficit Droits Déforestation", value: entity.deforestation_land_rights_deficit_score, weight: "0.30" },
    { label: "Violation Territoire Indigène", value: entity.indigenous_territory_violation_score, weight: "0.25" },
    { label: "Droits Expansion Monoculture", value: entity.monoculture_expansion_rights_score, weight: "0.25" },
    { label: "Droits Contamination Agrochimique", value: entity.agrochemical_contamination_rights_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_soy_farming_deforestation_index === "number" ? entity.estimated_soy_farming_deforestation_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Soja-Déforestation</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = Déforestation×0.30 + Territoire×0.25 + Monoculture×0.25 + Agrochimique×0.20
                </div>
              </div>
...
export default function SoyFarmingDeforestationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/soy-farming-deforestation-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits — Soja et Déforestation Amazonienne…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_soy_farming_deforestation_index ?? avg(allEntities.map(e => e.estimated_soy_farming_deforestation_index));
  ...
  const avgDeforestation = avg(allEntities.map(e => e.deforestation_land_rights_deficit_score));
  const avgTerritoire = avg(allEntities.map(e => e.indigenous_territory_violation_score));
  const avgMonoculture = avg(allEntities.map(e => e.monoculture_expansion_rights_score));
  const avgAgrochimique = avg(allEntities.map(e => e.agrochemical_contamination_rights_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Soja-Déforestation", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits — Soja et Déforestation Amazonienne</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Soy Farming Deforestation Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgDeforestation} label="Déficit Déforestation" />
          <GaugeRing value={avgTerritoire} label="Territoire Indigène" />
          <GaugeRing value={avgMonoculture} label="Expansion Monoculture" />
          <GaugeRing value={avgAgrochimique} label="Contamination Agrochimique" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Soja-Déforestation: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_soy_farming_deforestation_index === "number" ? e.estimated_soy_farming_deforestation_index.toFixed(2) : "—"}</span>
            </div>
