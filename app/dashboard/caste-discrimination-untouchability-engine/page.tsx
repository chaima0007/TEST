...
interface Entity {
  ...
  caste_based_violence_atrocity_severity_score: number;
  untouchability_practice_occupation_segregation_scale_score: number;
  intercaste_marriage_honor_violence_score: number;
  caste_affirmative_action_legal_protection_deficit_gap_score: number;
  estimated_caste_discrimination_untouchability_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_caste_discrimination_untouchability_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Violence & Atrocités Basées sur la Caste", value: entity.caste_based_violence_atrocity_severity_score, weight: "0.30" },
    { label: "Pratiques Intouchabilité & Ségrégation", value: entity.untouchability_practice_occupation_segregation_scale_score, weight: "0.25" },
    { label: "Mariage Intercaste & Violence d&apos;Honneur", value: entity.intercaste_marriage_honor_violence_score, weight: "0.25" },
    { label: "Déficit Action Affirmative & Protection Juridique", value: entity.caste_affirmative_action_legal_protection_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_caste_discrimination_untouchability_index === "number" ? entity.estimated_caste_discrimination_untouchability_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Discrimination de Caste</div>
                </div>
              </div>
...
export default function CasteDiscriminationUntouchabilityEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/caste-discrimination-untouchability-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Discrimination de Caste & Intouchabilité…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_caste_discrimination_untouchability_index ?? avg(allEntities.map(e => e.estimated_caste_discrimination_untouchability_index));
  ...
  const avgCasteViolence = avg(allEntities.map(e => e.caste_based_violence_atrocity_severity_score));
  const avgUntouchability = avg(allEntities.map(e => e.untouchability_practice_occupation_segregation_scale_score));
  const avgIntercasteMarriage = avg(allEntities.map(e => e.intercaste_marriage_honor_violence_score));
  const avgAffirmativeAction = avg(allEntities.map(e => e.caste_affirmative_action_legal_protection_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Discrimination Caste", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Discrimination de Caste & Intouchabilité</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Caste Discrimination Untouchability Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgCasteViolence} label="Violence & Atrocités" />
          <GaugeRing value={avgUntouchability} label="Intouchabilité & Ségrégation" />
          <GaugeRing value={avgIntercasteMarriage} label="Mariage Intercaste" />
          <GaugeRing value={avgAffirmativeAction} label="Action Affirmative" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Discrimination Caste: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_caste_discrimination_untouchability_index === "number" ? e.estimated_caste_discrimination_untouchability_index.toFixed(2) : "—"}</span>
            </div>
