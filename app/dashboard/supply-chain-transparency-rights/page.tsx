...
interface Entity {
  ...
  estimated_supply_chain_transparency_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_supply_chain_transparency_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  const [tab, setTab] = useState<"analyse" | "entites" | "methodologie">("analyse");
  const tabs: { key: "analyse" | "entites" | "methodologie"; label: string }[] = [
        ...
        <div className="p-6">
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_supply_chain_transparency_rights_index === "number" ? entity.estimated_supply_chain_transparency_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Transparence Chaînes</div>
                </div>
              </div>
          ...
          {tab === "entites" && (
            <div className="space-y-3">
              <div className="bg-slate-800/50 rounded-xl p-4">
                <div className="text-xs text-slate-400 mb-3">Score Composite Global</div>
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-3 bg-slate-700 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${Math.min(entity.composite_score, 100)}%`, background: ACCENT }} />
                  </div>
                  <span className="text-sm font-bold text-white w-12 text-right">{entity.composite_score.toFixed(1)}</span>
                </div>
              </div>
              <div className="bg-slate-800/50 rounded-xl p-4">
                <div className="text-xs text-slate-400 mb-3">Index Transparence Chaînes (sur 10)</div>
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-3 bg-slate-700 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${Math.min((entity.estimated_supply_chain_transparency_rights_index / 10) * 100, 100)}%`, background: ACCENT }} />
                  </div>
                  <span className="text-sm font-bold text-white w-12 text-right">{typeof entity.estimated_supply_chain_transparency_rights_index === "number" ? entity.estimated_supply_chain_transparency_rights_index.toFixed(2) : "—"}</span>
                </div>
              </div>
            </div>
          )}
          {tab === "methodologie" && (
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                <div className="text-xs text-slate-400 mb-2">Pondération des sous-scores</div>
                <div className="text-sm text-slate-300 space-y-1">
                  <div>Opacité des Fournisseurs & Due Diligence × 0.30</div>
                  <div>Traçabilité des Produits & Certification × 0.25</div>
                  <div>Violations Droits Humains dans la Chaîne × 0.25</div>
                  <div>Mécanismes de Plainte & Remédiation × 0.20</div>
                </div>
              </div>
              {entity.last_updated && (
...
export default function SupplyChainTransparencyRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/supply-chain-transparency-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits à la Transparence des Chaînes d&apos;Approvisionnement…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_supply_chain_transparency_rights_index ?? avg(allEntities.map(e => e.estimated_supply_chain_transparency_rights_index));
  const rd = data?.risk_distribution ?? data?.distribution ?? {};
  ...
  const kpis = [
    ...
    { label: "Index Transparence Chaînes", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits à la Transparence des Chaînes d&apos;Approvisionnement</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Supply Chain Transparency Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgComposite} label="Score Global" />
          <GaugeRing value={avgIndex * 10} label="Index Transparence" />
          <GaugeRing value={countCritique > 0 ? (countCritique / Math.max(allEntities.length, 1)) * 100 : 0} label="Taux Critique" />
          <GaugeRing value={allEntities.length > 0 ? ((countCritique + countEleve) / allEntities.length) * 100 : 0} label="Taux Risque Élevé+" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map(e => (
          <div key={e.id ?? e.entity} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-sm leading-tight truncate">{e.name ?? e.entity}</div>
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Transparence: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_supply_chain_transparency_rights_index === "number" ? e.estimated_supply_chain_transparency_rights_index.toFixed(2) : "—"}</span>
            </div>
            {(e.key_signals?.length ?? 0) > 0 && (
