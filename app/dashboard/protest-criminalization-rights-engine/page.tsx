...
interface Entity {
  ...
  last_updated: string;
  anti_protest_legislation_score?: number;
  activist_imprisonment_score?: number;
  excessive_force_protest_score?: number;
  assembly_permit_denial_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "anti_protest_legislation_score", label: "Législation anti-manifestation" },
  { key: "activist_imprisonment_score", label: "Emprisonnement militants" },
  { key: "excessive_force_protest_score", label: "Force excessive manifestations" },
  { key: "assembly_permit_denial_score", label: "Refus permis de rassemblement" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la criminalisation des manifestations : législations répressives anti-manifestation, emprisonnement d&apos;activistes, usage excessif de la force et refus de permis de rassemblement.</p>
              <p className="text-slate-400">Source : Protest Criminalization Rights Engine · Wave 258</p>
            </div>
          )}
        </div>
      </div>
...
export default function ProtestCriminalizationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/protest-criminalization-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Protest Criminalization Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Législation Anti-Protest · Militants · Force Excessive · Rassemblement</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#ea580c20" } : {}}>
