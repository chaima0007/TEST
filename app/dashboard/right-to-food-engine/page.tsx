...
interface DashData { agent?: string; total_entities?: number; avg_composite?: number; confidence_score?: number; risk_distribution?: Record<string,number>; avg_estimated_right_to_food_index?: number; entities?: Entity[]; [key: string]: unknown; }
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-food-engine").then(r => r.json()).then(d => { setData(d); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Droit à l&apos;Alimentation</h1>
        <p className="text-slate-400 mt-1">Right to Food Engine — Analyse des risques d&apos;insécurité alimentaire et violations du droit à la nourriture</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
          ...
          <p className="text-2xl font-bold mt-1" style={{ color: ACCENT }}>{loading ? "…" : data.total_entities}</p>
        ...
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
          <p className="text-slate-400 text-sm">Confiance IA</p>
          <p className="text-2xl font-bold mt-1" style={{ color: ACCENT }}>{loading ? "…" : ((data.confidence_score ?? 0)*100).toFixed(0)}</p>
          <p className="text-slate-500 text-xs mt-1">%</p>
        </div>
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
          <p className="text-slate-400 text-sm">Indice Alimentaire Moyen</p>
          <p className="text-2xl font-bold mt-1" style={{ color: ACCENT }}>{loading ? "…" : data.avg_estimated_right_to_food_index?.toFixed(2) ?? "—"}</p>
            ...
            <div className="p-6">
              {tab === 0 && (
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm"><span className="text-slate-400">Pattern principal</span><span className="text-slate-200">{selected.primary_pattern?.replace(/_/g," ")}</span></div>
                  <div className="pt-3 border-t border-slate-800">
                    <p className="text-slate-400 text-xs mb-2">SECTEUR</p>
                    <p className="text-slate-300 text-sm">{selected.sector as string}</p>
                  </div>
                </div>
              )}
              {tab === 1 && (
                <ul className="space-y-4">
                  {(selected.key_signals ?? []).map((s, i) => (
                    <li key={i} className="bg-slate-800 rounded-lg p-3 text-sm text-slate-300 leading-relaxed">{s}</li>
                  ))}
                </ul>
              )}
              {tab === 2 && (
