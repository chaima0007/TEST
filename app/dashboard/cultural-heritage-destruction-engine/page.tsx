...
interface CulturalHeritageDestructionEntity {
  entity_id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  deliberate_destruction_scale_score: number;
  looting_trafficking_score: number;
  cultural_identity_erasure_score: number;
  impunity_accountability_gap_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_cultural_heritage_destruction_index: number;
  last_updated: string;
}

interface CulturalHeritageDestructionSummary {
  ...
  data_sources: string[];
  avg_estimated_cultural_heritage_destruction_index: number;
}

interface CulturalHeritageDestructionData {
  entities: CulturalHeritageDestructionEntity[];
  summary: CulturalHeritageDestructionSummary;
}
...
function EntityCard({ entity, onClick }: { entity: CulturalHeritageDestructionEntity; onClick: (e: CulturalHeritageDestructionEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.deliberate_destruction_scale_score}</p>
          <p className="text-[10px] text-gray-500">Destruction</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.looting_trafficking_score}</p>
          <p className="text-[10px] text-gray-500">Pillage</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.cultural_identity_erasure_score}</p>
          <p className="text-[10px] text-gray-500">Effacement</p>
        </div>
      </div>
...
function DetailModal({ entity, onClose }: { entity: CulturalHeritageDestructionEntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Destruction Délibérée à Grande Échelle", value: `${entity.deliberate_destruction_scale_score}/100` },
                  { label: "Pillage & Trafic", value: `${entity.looting_trafficking_score}/100` },
                  { label: "Effacement Identité Culturelle", value: `${entity.cultural_identity_erasure_score}/100` },
                  { label: "Impunité & Absence Responsabilité", value: `${entity.impunity_accountability_gap_score}/100` },
                  { label: "Indice Destruction Patrimoine", value: `${entity.estimated_cultural_heritage_destruction_index}/10` },
              ...
              <div className="bg-white/4 rounded-lg p-4">
                ...
                <p className="text-[11px] text-gray-500 mt-1.5">
                  Formule : destruction×0.30 + pillage×0.25 + effacement×0.25 + impunité×0.20
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "contexte" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 mt-1"><span className="text-gray-300 font-medium">Indice Destruction Patrimoine :</span> {entity.estimated_cultural_heritage_destruction_index}/10</p>
              </div>
              <div className={`rounded-xl border p-4 ${cfg.bg} ${cfg.border}`}>
                <p className={`text-sm font-semibold ${cfg.color}`}>{entity.primary_pattern}</p>
                <p className="text-xs text-gray-400 mt-2">Renforcer la protection internationale du patrimoine culturel, lutter contre le trafic et garantir la responsabilité des auteurs de destructions délibérées.</p>
              </div>
            </div>
          )}
        </div>
...
export default function CulturalHeritageDestructionEnginePage() {
  const [data, setData] = useState<CulturalHeritageDestructionData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<CulturalHeritageDestructionEntity | null>(null);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Destruction du Patrimoine Culturel</h1>
            <p className="text-sm text-gray-400 mt-1">
              Destruction &amp; Trafic du Patrimoine Culturel Mondial
            </p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Indice Destruction Patrimoine Moyen" value={`${s.avg_estimated_cultural_heritage_destruction_index ?? "—"}/10`} sub="cultural heritage index" accent="text-orange-400" />
              <KpiCard label="Sources de données" value={s.data_sources?.length ?? 0} sub="organismes ONU/UNESCO" accent="text-violet-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Destruction Patrimoine Culturel</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_cultural_heritage_destruction_index ?? 0} max={10} label="Indice Patrimoine" color={ACCENT} />
                </div>
              </div>
            ...
            {s.critical_alerts > 0 && (
              <div className="rounded-xl px-5 py-4 flex items-start gap-3" style={{ backgroundColor: "rgba(194,65,12,0.08)", border: "1px solid rgba(194,65,12,0.2)" }}>
                <div className="w-2 h-2 rounded-full mt-1.5 shrink-0 animate-pulse" style={{ backgroundColor: ACCENT }} />
                <div>
                  <p className="text-sm font-medium" style={{ color: ACCENT }}>
                    {s.critical_alerts} entité{s.critical_alerts > 1 ? "s" : ""} en situation critique de destruction du patrimoine culturel
                  </p>
