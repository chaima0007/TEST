...
interface Entity {
  ...
  last_updated: string;
  femicide_impunity_score?: number;
  law_enforcement_failure_score?: number;
  judicial_gender_bias_score?: number;
  state_negligence_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "femicide_impunity_score", label: "Impunité féminicide" },
  { key: "law_enforcement_failure_score", label: "Défaillance forces de l&apos;ordre" },
  { key: "judicial_gender_bias_score", label: "Biais de genre judiciaire" },
  { key: "state_negligence_score", label: "Négligence étatique" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la responsabilité pour féminicide : impunité systémique, défaillance des forces de l&apos;ordre, biais de genre dans le système judiciaire et négligence étatique face aux violences de genre.</p>
              <p className="text-slate-400">Source : Femicide Accountability Rights Engine · Wave 261</p>
            </div>
          )}
        </div>
      </div>
...
export default function FemicideAccountabilityRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/femicide-accountability-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Femicide Accountability Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Impunité · Forces de l&apos;Ordre · Biais Judiciaire · Négligence Étatique</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#be123c20" } : {}}>
