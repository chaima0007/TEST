...
function GaugeRing({ value, color }: { value: number; color: string }) {
  ...
  return (
    <svg viewBox="0 0 88 88" className="w-24 h-24">
      ...
      <text x={cx} y={cy + 5} textAnchor="middle" fontSize={14} fill={color} fontWeight="bold">
        {pct.toFixed(0)}%
      </text>
    </svg>
  );
...
export default function EnvironmentalDefenderDashboard() {
  const [data, setData] = useState<{ entities?: { entity: string; composite: number; level: string }[]; avg_composite?: number } | null>(null);

  useEffect(() => {
    fetch("/api/environmentaldefender").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);

  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Environmental Defender Rights</h1>
      {!data ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {(data.entities ?? []).map((e) => (
            <div key={e.entity} className="bg-white rounded-xl p-4 shadow text-center">
              <GaugeRing value={e.composite}
                color={e.level === "critique" ? "#ef4444" : e.level === "élevé" ? "#f97316" : e.level === "modéré" ? "#eab308" : "#22c55e"} />
      ...
      {data?.avg_composite && (
        <div className="mt-6 p-4 bg-gray-50 rounded-xl">
          <p className="font-bold">Score moyen: {data.avg_composite}</p>
        </div>
      )}
    </div>
  );
