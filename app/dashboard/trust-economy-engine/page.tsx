...
interface TEEntity {
  ...
  composite_score: number; institutional_trust_score: number;
  interpersonal_trust_score: number; market_confidence_score: number;
  digital_trust_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_trust_index: number; last_updated: string;
}
interface TESummary {
  ...
  entities: TEEntity[]; avg_estimated_trust_index: number;
}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 mb-4">
          {(["scores", "signaux", "actions"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-3 py-1 rounded-full text-xs border ${tab === t ? "bg-sky-500 border-sky-400 text-white" : "bg-slate-800 border-slate-700 text-slate-400"}`}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Déficit Institutionnel", v: entity.institutional_trust_score, c: "#ef4444" },
              { l: "Déficit Interpersonnel", v: entity.interpersonal_trust_score, c: "#f97316" },
              { l: "Déficit Marchés", v: entity.market_confidence_score, c: "#eab308" },
              { l: "Déficit Numérique", v: entity.digital_trust_score, c: "#0ea5e9" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Déficit Confiance</span>
              <span className="text-sky-400 font-bold">{entity.estimated_trust_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
          <ul className="space-y-2">
            {entity.key_signals.map((s, i) => (
              <li key={i} className="flex gap-2 text-sm text-slate-300"><span className="text-sky-400">›</span>{s}</li>
            ))}
          </ul>
        )}
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            <p className="text-xs text-slate-500 mb-2">Pattern: <span className="text-sky-400">{entity.primary_pattern}</span></p>
            {entity.risk_level === "critique" && <><p>› Restauration d&apos;urgence du capital-confiance institutionnel</p><p>› Médiation internationale et dialogue multi-parties prenantes</p><p>› Programmes d&apos;urgence de reconstruction de la confiance sociale</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des institutions garantes de la confiance</p><p>› Investissements ciblés dans la transparence et la redevabilité</p></>}
            {entity.risk_level === "modéré" && <><p>› Programmes de renforcement du capital-confiance</p><p>› Politiques de réduction du déficit de confiance émergent</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille sur les indicateurs de confiance</p><p>› Consolidation des mécanismes de confiance institutionnelle</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function TrustEconomyPage() {
  ...
  useEffect(() => {
    fetch("/api/trust-economy-engine")
      .then(r => r.json())
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données du capital-confiance"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse du capital-confiance mondial…</div></div>;
  ...
  const avgScore = (k: keyof TEEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Trust Economy Engine — Capital-Confiance &amp; Déficit Institutionnel</h1>
          <p className="text-slate-400 text-sm">Mesure du déficit de confiance : institutionnel · interpersonnel · marché · numérique · v{data.engine_version}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Index Déficit", v: data.avg_estimated_trust_index, g: "from-purple-900 to-purple-800", t: "text-purple-300" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Déficits de Confiance Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("institutional_trust_score")} label="Déficit Institutionnel" color="#ef4444" />
            <GaugeRing value={avgScore("interpersonal_trust_score")} label="Déficit Interpersonnel" color="#f97316" />
            <GaugeRing value={avgScore("market_confidence_score")} label="Déficit Marchés" color="#eab308" />
            <GaugeRing value={avgScore("digital_trust_score")} label="Déficit Numérique" color="#0ea5e9" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Déficit de Confiance</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-sky-900/10 border border-sky-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-sky-400 mb-2">Alertes Déficit Confiance Critiques</h2>
        ...
        <div className="flex gap-2 flex-wrap">
          {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-full text-sm border transition-all capitalize ${filter === f ? "bg-sky-500 border-sky-400 text-white" : "bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-500"}`}>
              {f}
            </button>
          ))}
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="h-1.5 bg-slate-800 rounded-full mb-2">
                <div className="h-full rounded-full bg-sky-500" style={{ width: `${entity.composite_score}%` }} />
              </div>
              <div className="flex justify-between text-xs text-slate-500">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span>idx {entity.estimated_trust_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Trust Economy &amp; Capital-Confiance · {data.last_analysis}</p>
      </div>
    </div>
  );
}
