...
interface Entity {
  ...
  violence_physique_meurtres_rituels_trafic_score: number;
  impunite_judiciaire_absence_poursuites_score: number;
  stigmatisation_sociale_exclusion_communautaire_score: number;
  acces_soins_protection_uv_soutien_psychosocial_score: number;
  estimated_albinism_persecution_rights_index: number;
  risk_level: string;
...
const SUB_SCORES = [
  { key: "violence_physique_meurtres_rituels_trafic_score", label: "Violence Physique & Meurtres Rituels" },
  { key: "impunite_judiciaire_absence_poursuites_score", label: "Impunité Judiciaire" },
  { key: "stigmatisation_sociale_exclusion_communautaire_score", label: "Stigmatisation & Exclusion" },
  { key: "acces_soins_protection_uv_soutien_psychosocial_score", label: "Accès Soins & Protection UV" },
];
        ...
        <div className="flex border-b border-slate-800">
          {["Aperçu", "Métriques"].map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 text-amber-400" : "text-slate-400 hover:text-white"}`}
          ...
          {tab === 0 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Indice Persécution Albinos</span>
                <span className="font-semibold" style={{ color: ACCENT_LIGHT }}>{entity.estimated_albinism_persecution_rights_index}/10</span>
              </div>
              <div className="flex items-center justify-between text-sm">
...
export default function AlbinismPersecutionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/albinism-persecution-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT_LIGHT }}>Persécution &amp; Droits des Personnes Albinos</h1>
          <p className="text-slate-400 mt-1 text-sm">Violence Rituelle · Mutilations · Stigmatisation · Protection Légale</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice APR</p>
        ...
        {filtered.map(e => {
          ...
          return (
            <button key={e.id} onClick={() => setSelected(e)}
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold uppercase ${cfg.color}`}>{cfg.label}</span>
                <span className="text-xs text-slate-500">Indice: <span className="font-bold" style={{ color: ACCENT_LIGHT }}>{e.estimated_albinism_persecution_rights_index}</span></span>
              </div>
            </button>
          );
        })}
