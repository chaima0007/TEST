...
interface Entity {
  ...
  journalist_killing_imprisonment_severity_score: number;
  media_censorship_state_capture_scale_score: number;
  surveillance_source_protection_violation_score: number;
  online_journalist_harassment_slapp_gap_score: number;
  estimated_press_freedom_journalist_protection_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_press_freedom_journalist_protection_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Meurtres & Emprisonnement Journalistes", value: entity.journalist_killing_imprisonment_severity_score, weight: "0.30" },
    { label: "Censure & Capture Médias", value: entity.media_censorship_state_capture_scale_score, weight: "0.25" },
    { label: "Surveillance & Sources Violées", value: entity.surveillance_source_protection_violation_score, weight: "0.25" },
    { label: "Harcèlement & SLAPP En Ligne", value: entity.online_journalist_harassment_slapp_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_press_freedom_journalist_protection_index === "number" ? entity.estimated_press_freedom_journalist_protection_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Liberté Presse</div>
                </div>
              </div>
...
export default function PressFreedomJournalistProtectionEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/press-freedom-journalist-protection-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Liberté de la Presse & Protection Journalistes…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_press_freedom_journalist_protection_index ?? avg(allEntities.map(e => e.estimated_press_freedom_journalist_protection_index));
  ...
  const avgKilling = avg(allEntities.map(e => e.journalist_killing_imprisonment_severity_score));
  const avgCensorship = avg(allEntities.map(e => e.media_censorship_state_capture_scale_score));
  const avgSurveillance = avg(allEntities.map(e => e.surveillance_source_protection_violation_score));
  const avgHarassment = avg(allEntities.map(e => e.online_journalist_harassment_slapp_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Liberté Presse", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Liberté de la Presse & Protection Journalistes</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Press Freedom & Journalist Protection Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgKilling} label="Meurtres & Emprisonnement" />
          <GaugeRing value={avgCensorship} label="Censure & Capture Médias" />
          <GaugeRing value={avgSurveillance} label="Surveillance & Sources" />
          <GaugeRing value={avgHarassment} label="Harcèlement & SLAPP" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Liberté Presse: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_press_freedom_journalist_protection_index === "number" ? e.estimated_press_freedom_journalist_protection_index.toFixed(2) : "—"}</span>
            </div>
