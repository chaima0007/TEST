...
interface PrisonLaborEntity {
  entity_id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  forced_labor_scale_score: number;
  below_minimum_wage_score: number;
  coercion_punishment_score: number;
  legal_protection_absence_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_prison_labor_index: number;
  last_updated: string;
}

interface PrisonLaborSummary {
  ...
  data_sources: string[];
  avg_estimated_prison_labor_index: number;
}

interface PrisonLaborData {
  entities: PrisonLaborEntity[];
  summary: PrisonLaborSummary;
}
...
function EntityCard({ entity, onClick }: { entity: PrisonLaborEntity; onClick: (e: PrisonLaborEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.forced_labor_scale_score}</p>
          <p className="text-[10px] text-gray-500">Travail Forcé</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.below_minimum_wage_score}</p>
          <p className="text-[10px] text-gray-500">Sous SMIC</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.coercion_punishment_score}</p>
          <p className="text-[10px] text-gray-500">Coercition</p>
        </div>
      </div>
...
function DetailModal({ entity, onClose }: { entity: PrisonLaborEntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Travail Forcé à Grande Échelle", value: `${entity.forced_labor_scale_score}/100` },
                  { label: "Rémunération Sous SMIC", value: `${entity.below_minimum_wage_score}/100` },
                  { label: "Coercition & Punition", value: `${entity.coercion_punishment_score}/100` },
                  { label: "Absence Protection Légale", value: `${entity.legal_protection_absence_score}/100` },
                  { label: "Indice Travail Carcéral", value: `${entity.estimated_prison_labor_index}/10` },
              ...
              <div className="bg-white/4 rounded-lg p-4">
                ...
                <p className="text-[11px] text-gray-500 mt-1.5">
                  Formule : travail forcé×0.30 + sous SMIC×0.25 + coercition×0.25 + protection légale×0.20
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "contexte" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 mt-1"><span className="text-gray-300 font-medium">Indice Travail Carcéral :</span> {entity.estimated_prison_labor_index}/10</p>
              </div>
              <div className={`rounded-xl border p-4 ${cfg.bg} ${cfg.border}`}>
                <p className={`text-sm font-semibold ${cfg.color}`}>{entity.primary_pattern}</p>
                <p className="text-xs text-gray-400 mt-2">Garantir des conditions de travail dignes, une rémunération équitable et des protections légales effectives pour les personnes détenues.</p>
              </div>
            </div>
          )}
        </div>
...
export default function PrisonLaborEnginePage() {
  const [data, setData] = useState<PrisonLaborData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<PrisonLaborEntity | null>(null);

  useEffect(() => {
    fetch("/api/prison-labor-engine")
      .then((r) => r.json())
      .then((d) => {
        setData(d.data ?? d);
        setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Travail Carcéral</h1>
            <p className="text-sm text-gray-400 mt-1">
              Travail en Détention &amp; Exploitation Pénitentiaire
            </p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Indice Travail Carcéral Moyen" value={`${s.avg_estimated_prison_labor_index ?? "—"}/10`} sub="prison labor index" accent="text-slate-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Travail Carcéral</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_prison_labor_index ?? 0} max={10} label="Indice Prison" color={ACCENT} />
                </div>
              </div>
            ...
            {s.critical_alerts > 0 && (
              <div className="rounded-xl px-5 py-4 flex items-start gap-3" style={{ backgroundColor: "rgba(71,85,105,0.15)", border: "1px solid rgba(71,85,105,0.4)" }}>
                <div className="w-2 h-2 rounded-full mt-1.5 shrink-0 animate-pulse" style={{ backgroundColor: ACCENT }} />
                <div>
                  <p className="text-sm font-medium" style={{ color: "#94a3b8" }}>
                    {s.critical_alerts} entité{s.critical_alerts > 1 ? "s" : ""} en situation critique de travail carcéral
                  </p>
                  <p className="text-xs mt-0.5 text-slate-500">
                    Intervention immédiate requise — garanties légales et conditions de travail dignes prioritaires
                  </p>
                </div>
              </div>
            )}
