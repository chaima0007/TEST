...
export default function RightToTruthTransitionalJusticePage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-truth-transitional-justice-engine")
      .then(r => r.json())
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droit à la Vérité &amp; Justice Transitionnelle</h1>
          <p className="text-slate-400 mt-1 text-sm">Vérité · Réconciliation · Réparations · Commissions & Tribunaux Internationaux</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice justice transit.</p>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#6366f11a" } : {}}>
          ...
          return (
            <button key={e.id} onClick={() => setSelected(e)}
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-slate-500">Indice justice transit.: <span className="font-bold" style={{ color: ACCENT }}>{e.estimated_transitional_justice_index}</span></span>
              </div>
            </button>
          )
        })}
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto"
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Indice justice transit.</span>
                <span className="font-semibold" style={{ color: ACCENT }}>{selected.estimated_transitional_justice_index}/10</span>
              </div>
              <div className="flex items-center justify-between text-sm">
