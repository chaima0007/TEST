...
interface Entity {
  ...
  country: string;
  sector: string;
  mass_biometric_collection_scale_score: number;
  facial_recognition_misidentification_harm_score: number;
  biometric_data_misuse_rights_violation_score: number;
  surveillance_state_chilling_effect_score: number;
  estimated_biometric_surveillance_state_control_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_biometric_surveillance_state_control_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Collecte Biométrique Masse", value: entity.mass_biometric_collection_scale_score, weight: "0.30" },
    { label: "Erreurs Reconnaissance Faciale", value: entity.facial_recognition_misidentification_harm_score, weight: "0.25" },
    { label: "Mésusage Données Biométriques", value: entity.biometric_data_misuse_rights_violation_score, weight: "0.25" },
    { label: "Effet Dissuasif Liberté", value: entity.surveillance_state_chilling_effect_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.mass_biometric_collection_scale_score === "number" ? entity.mass_biometric_collection_scale_score.toFixed(1) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Collecte Biométrique</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_biometric_surveillance_state_control_index === "number" ? entity.estimated_biometric_surveillance_state_control_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Surveillance Biométrique</div>
                </div>
              </div>
...
export default function BiometricSurveillanceStateControlEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/biometric-surveillance-state-control-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Surveillance Biométrique &amp; Contrôle Étatique…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.mass_biometric_collection_scale_score));
  const avgIndex = data?.avg_estimated_biometric_surveillance_state_control_index ?? avg(allEntities.map(e => e.estimated_biometric_surveillance_state_control_index));
  ...
  const avgBiometric = avg(allEntities.map(e => e.mass_biometric_collection_scale_score));
  const avgFacial = avg(allEntities.map(e => e.facial_recognition_misidentification_harm_score));
  const avgMisuse = avg(allEntities.map(e => e.biometric_data_misuse_rights_violation_score));
  const avgChilling = avg(allEntities.map(e => e.surveillance_state_chilling_effect_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Surveillance Biométrique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Surveillance Biométrique &amp; Contrôle Étatique</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Biometric Surveillance &amp; State Control Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgBiometric} label="Collecte Biométrique Masse" />
          <GaugeRing value={avgFacial} label="Erreurs Reconnaissance Faciale" />
          <GaugeRing value={avgMisuse} label="Mésusage Données Biométriques" />
          <GaugeRing value={avgChilling} label="Effet Dissuasif Liberté" />
        </div>
      </div>
          ...
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.mass_biometric_collection_scale_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.mass_biometric_collection_scale_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Surveillance Biométrique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_biometric_surveillance_state_control_index === "number" ? e.estimated_biometric_surveillance_state_control_index.toFixed(2) : "—"}</span>
            </div>
