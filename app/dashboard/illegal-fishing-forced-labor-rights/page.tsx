...
interface IFFLREntity {
  id: string;
  name: string;
  country: string;
  composite_score: number;
  forced_labor_at_sea_confinement_score: number;
  illegal_unreported_fishing_scale_score: number;
  debt_bondage_recruitment_pattern_score: number;
  flag_state_oversight_failure_score: number;
  estimated_illegal_fishing_forced_labor_rights_index: number;
  risk_level: string;
...
interface IFFLRData {
  ...
  data_sources: string[];
  entities: IFFLREntity[];
  avg_estimated_illegal_fishing_forced_labor_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function IllegalFishingForcedLaborRightsPage() {
  const [data, setData] = useState<IFFLRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<IFFLREntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/illegal-fishing-forced-labor-rights")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-blue-400 animate-pulse">Chargement des données pêche illégale…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "forced_labor_at_sea_confinement_score", label: "Travail Forcé Haute Mer" },
    { key: "illegal_unreported_fishing_scale_score", label: "Pêche INN à Grande Échelle" },
    { key: "debt_bondage_recruitment_pattern_score", label: "Recrutement Dette Servage" },
    { key: "flag_state_oversight_failure_score", label: "Défaillance État Pavillon" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits — Pêche Illégale et Esclavage Maritime</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Illegal Fishing &amp; Forced Labor Rights Engine</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_illegal_fishing_forced_labor_rights_index ?? "—" },
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#60a5fa" />
        ...
        {(data.critical_alerts?.length ?? 0) > 0 && (
          <div className="border border-blue-500/30 bg-blue-500/10 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-blue-400 mb-2">Alertes Critiques</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => { setSelected(entity); setTab(0); }}
              className={`bg-slate-900 border rounded-xl p-4 cursor-pointer hover:border-blue-500/50 transition-all ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#60a5fa" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_illegal_fishing_forced_labor_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droits Pêche Illégale &amp; Esclavage Maritime · {data.last_analysis}</p>
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Vue générale", "Sous-scores", "Alertes"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`px-6 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-blue-500 text-blue-400" : "text-slate-500 hover:text-slate-300"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#60a5fa" />
                    <div>
                      <div className="text-2xl font-bold text-blue-400">{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Droits Pêche Illégale &amp; Travail Forcé</div>
                    <div className="text-lg font-bold text-blue-400">{selected.estimated_illegal_fishing_forced_labor_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
