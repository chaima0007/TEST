...
interface Entity {
  ...
  last_updated: string;
  large_scale_ecosystem_destruction_score?: number;
  corporate_environmental_impunity_score?: number;
  indigenous_land_ecocide_score?: number;
  international_law_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "large_scale_ecosystem_destruction_score", label: "Destruction d&apos;écosystèmes à grande échelle" },
  { key: "corporate_environmental_impunity_score", label: "Impunité environnementale des entreprises" },
  { key: "indigenous_land_ecocide_score", label: "Écocide sur terres autochtones" },
  { key: "international_law_gap_score", label: "Lacunes du droit international" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de l&apos;écocide : destruction d&apos;écosystèmes à grande échelle, impunité environnementale des entreprises, écocide sur terres autochtones et lacunes du droit international.</p>
              <p className="text-slate-400">Source : Ecocide Rights Engine · Wave 248</p>
            </div>
          )}
        </div>
      </div>
...
export default function EcocideRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/ecocide-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Ecocide Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Destruction Écosystèmes · Impunité Corporate · Terres Autochtones · Droit International</p>
        </div>
