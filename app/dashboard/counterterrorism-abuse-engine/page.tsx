...
interface Entity {
  ...
  arbitrary_detention_torture_score: number;
  civil_rights_dismantlement_score: number;
  minorities_targeting_score: number;
  judicial_oversight_absence_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_counterterrorism_abuse_index: number;
  last_updated: string;
}
...
interface Data {
  ...
  avg_estimated_counterterrorism_abuse_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Détention Arbitraire & Torture", v: entity.arbitrary_detention_torture_score, c: "#dc2626" },
              { l: "Démantèlement Droits Civils", v: entity.civil_rights_dismantlement_score, c: "#b91c1c" },
              { l: "Ciblage des Minorités", v: entity.minorities_targeting_score, c: "#991b1b" },
              { l: "Absence Contrôle Judiciaire", v: entity.judicial_oversight_absence_score, c: "#7f1d1d" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Abus Antiterrorisme Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_counterterrorism_abuse_index}</span>
            </div>
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Intervention d&apos;urgence pour les détenus soumis à la torture et détention arbitraire sans recours judiciaire</p>
                <p>› Mobilisation des mécanismes internationaux anti-torture et des rapporteurs spéciaux onusiens</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Documentation systématique du ciblage des minorités et des abus sous couvert antiterroriste</p>
                <p>› Plaidoyer pour le rétablissement du contrôle judiciaire indépendant des opérations sécuritaires</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance des dérives législatives et soutien aux mécanismes nationaux de contrôle des droits</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur les évolutions normatives et prévention des dérives sécuritaires</p>
            )}
          </div>
        )}
      </div>
...
export default function CounterterrorismAbuseEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/counterterrorism-abuse-engine")
      .then(r => r.json())
      .then(d => { setData(d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données Counterterrorism Abuse Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse des abus antiterrorisme…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Score Moyen", v: `${(data.avg_composite ?? 0).toFixed(1)}`, c: "text-red-400" },
    { l: "Indice Abus Antiterrorisme Moyen", v: `${(data.avg_estimated_counterterrorism_abuse_index ?? 0).toFixed(2)}`, c: "text-red-300" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Abus Antiterrorisme</h1>
          <p className="text-slate-400 text-sm mt-1">Détention arbitraire, torture et démantèlement des droits civils sous prétexte sécuritaire</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {kpis.map(({ l, v, c }) => (
            <div key={l} className="bg-slate-900 border border-red-500/20 rounded-xl p-4 text-center">
        ...
        <div className="bg-slate-900 border border-red-500/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Abus Antiterrorisme Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("arbitrary_detention_torture_score")} label="Détention/Torture" color={ACCENT} />
            <GaugeRing value={avgScore("civil_rights_dismantlement_score")} label="Démantèlement Droits" color="#b91c1c" />
            <GaugeRing value={avgScore("minorities_targeting_score")} label="Ciblage Minorités" color="#991b1b" />
            <GaugeRing value={avgScore("judicial_oversight_absence_score")} label="Absence Contrôle" color="#7f1d1d" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-red-500/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns d&apos;Abus Sécuritaire</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#dc2626", "#b91c1c", "#991b1b", "#7f1d1d", "#450a0a"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_counterterrorism_abuse_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Counterterrorism Abuse Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
