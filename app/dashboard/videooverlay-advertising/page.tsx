...
const SIGNALS = [
  "Overlay consent disclosure",
  "Skip button compliance",
...
const ACTIONS = [
  "Enforce overlay consent",
  "Implement skip controls",
  "Limit attention tracking",
  "Audit overlay data",
  "Review content matching",
];
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque videooverlay advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_index.toFixed(2)} / 10`,
    `Conformité plateforme videooverlay : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 419 — Video Overlay Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Video Overlay Advertising · Wave 419</div>
          </div>
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            {ACTIONS.map((action, i) => (
              <div key={i} style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(180,83,9,0.15)" }}>
...
export default function VideooverlayAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/videooverlay-advertising")
      .then((r) => r.json())
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Video Overlay Advertising…
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Video Overlay Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#b4530999", margin: 0 }}>
          Caelum Partners · Wave 419 · Analyse publicitaire videooverlay · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Video Overlay Advertising Intelligence · Wave 419
      </div>
