...
interface HateCrimeEntity {
  id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  racial_bias_crime_rate_score: number;
  religious_motivated_violence_score: number;
  lgbtq_targeted_violence_score: number;
  accountability_prosecution_failure_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_hate_crime_index: number;
  last_updated: string;
}

interface HateCrimeSummary {
  ...
  data_sources: string[];
  avg_estimated_hate_crime_index: number;
}

interface HateCrimeData {
  entities: HateCrimeEntity[];
  summary: HateCrimeSummary;
}
...
function EntityCard({ entity, onClick }: { entity: HateCrimeEntity; onClick: (e: HateCrimeEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.racial_bias_crime_rate_score}</p>
          <p className="text-[10px] text-gray-500">Racial</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.religious_motivated_violence_score}</p>
          <p className="text-[10px] text-gray-500">Religieux</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.lgbtq_targeted_violence_score}</p>
          <p className="text-[10px] text-gray-500">LGBTQ+</p>
        </div>
      </div>
...
function DetailModal({ entity, onClose }: { entity: HateCrimeEntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Biais Racial Crime", value: `${entity.racial_bias_crime_rate_score}/100` },
                  { label: "Violence Religieuse", value: `${entity.religious_motivated_violence_score}/100` },
                  { label: "Violence LGBTQ+", value: `${entity.lgbtq_targeted_violence_score}/100` },
                  { label: "Échec Poursuites", value: `${entity.accountability_prosecution_failure_score}/100` },
                  { label: "Indice HC", value: `${entity.estimated_hate_crime_index}/10` },
              ...
              <div className="bg-white/4 rounded-lg p-4">
                ...
                <p className="text-[11px] text-gray-500 mt-1.5">
                  Formule : racial×0.30 + religieux×0.25 + lgbtq×0.25 + poursuites×0.20
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "contexte" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 mt-1"><span className="text-gray-300 font-medium">Indice HC :</span> {entity.estimated_hate_crime_index}/10</p>
              </div>
              <div className={`rounded-xl border p-4 ${cfg.bg} ${cfg.border}`}>
                <p className={`text-sm font-semibold ${cfg.color}`}>{entity.primary_pattern}</p>
                <p className="text-xs text-gray-400 mt-2">Renforcer les mécanismes de signalement, les formations anti-biais et les poursuites judiciaires systématiques.</p>
              </div>
            </div>
          )}
        </div>
...
export default function HateCrimeEnginePage() {
  const [data, setData] = useState<HateCrimeData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<HateCrimeEntity | null>(null);

  useEffect(() => {
    fetch("/api/hate-crime-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as HateCrimeData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Hate Crime Engine</h1>
            <p className="text-sm text-gray-400 mt-1">
              Crimes de Haine &amp; Violences Motivées par l&apos;Idéologie
            </p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Indice HC moyen" value={`${s.avg_estimated_hate_crime_index ?? "—"}/10`} sub="hate crime index" accent="text-red-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Crimes de Haine</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_hate_crime_index ?? 0} max={10} label="Indice HC" color={ACCENT} />
                </div>
              </div>
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.critical_alerts} entité{s.critical_alerts > 1 ? "s" : ""} en situation critique de crimes de haine
                  </p>
                  <p className="text-xs text-red-400/70 mt-0.5">
                    Intervention immédiate requise — renforcement des poursuites judiciaires urgent
                  </p>
                </div>
              </div>
            )}
