...
const ACCENT = "#059669";
const TITRE = "Protection Sociale & Droits à la Sécurité Sociale";
...
export default function Page() {
  ...
  useEffect(() => { fetch("/api/social-security-rights-engine").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
          ...
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 200px", display: "flex", flexDirection: "column", gap: 10, justifyContent: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#94a3b8" }}>Index SSR</span><strong style={{ color: ACCENT }}>{data.avg_estimated_social_security_rights_index?.toFixed(2)}</strong></div>
          </div>
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 280px" }}>
            ...
            {(data.critical_alerts ?? []).map((a: string, i: number) => (
              <div key={i} style={{ color: "#6ee7b7", fontSize: 12, padding: "4px 0", borderBottom: "1px solid #1e293b" }}>{a}</div>
            ))}
          </div>
        </div>
