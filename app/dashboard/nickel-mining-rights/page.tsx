...
interface Entity {
  ...
  labor_safety_violations_score: number;
  community_displacement_rights_deficit_score: number;
  environmental_health_impact_score: number;
  indigenous_land_rights_breach_score: number;
  estimated_nickel_mining_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_nickel_mining_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Violations Sécurité Travail", value: entity.labor_safety_violations_score, weight: "0.30" },
    { label: "Déficit Droits Déplacement Communautaire", value: entity.community_displacement_rights_deficit_score, weight: "0.25" },
    { label: "Impact Santé Environnementale", value: entity.environmental_health_impact_score, weight: "0.25" },
    { label: "Violation Droits Fonciers Autochtones", value: entity.indigenous_land_rights_breach_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_nickel_mining_rights_index === "number" ? entity.estimated_nickel_mining_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Mines de Nickel</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = Sécurité×0.30 + Déplacement×0.25 + Environnement×0.25 + Autochtones×0.20
                </div>
              </div>
...
export default function NickelMiningRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/nickel-mining-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits dans les Mines de Nickel…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_nickel_mining_rights_index ?? avg(allEntities.map(e => e.estimated_nickel_mining_rights_index));
  ...
  const avgSecurite = avg(allEntities.map(e => e.labor_safety_violations_score));
  const avgDeplacement = avg(allEntities.map(e => e.community_displacement_rights_deficit_score));
  const avgEnvironnement = avg(allEntities.map(e => e.environmental_health_impact_score));
  const avgAutochtones = avg(allEntities.map(e => e.indigenous_land_rights_breach_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Mines de Nickel", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits dans les Mines de Nickel</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Nickel Mining Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgSecurite} label="Sécurité Travail" />
          <GaugeRing value={avgDeplacement} label="Déplacement Communautaire" />
          <GaugeRing value={avgEnvironnement} label="Impact Environnemental" />
          <GaugeRing value={avgAutochtones} label="Droits Autochtones" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Mines Nickel: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_nickel_mining_rights_index === "number" ? e.estimated_nickel_mining_rights_index.toFixed(2) : "—"}</span>
            </div>
