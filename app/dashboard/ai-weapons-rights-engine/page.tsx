...
interface Entity {
  ...
  last_updated: string;
  autonomous_weapon_civilian_harm_score?: number;
  accountability_gap_lethal_ai_score?: number;
  arms_race_destabilization_score?: number;
  ban_treaty_absence_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "autonomous_weapon_civilian_harm_score", label: "Dommages civils armes autonomes" },
  { key: "accountability_gap_lethal_ai_score", label: "Lacune responsabilité IA létale" },
  { key: "arms_race_destabilization_score", label: "Déstabilisation course aux armements" },
  { key: "ban_treaty_absence_score", label: "Absence de traité d&apos;interdiction" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits liés aux armes autonomes à décision létale : dommages civils, lacunes de responsabilité, course aux armements et absence de traités d&apos;interdiction.</p>
              <p className="text-slate-400">Source : AI Weapons Rights Engine · Wave 253</p>
            </div>
          )}
        </div>
      </div>
...
export default function AIWeaponsRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/ai-weapons-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>AI Weapons Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Armes Autonomes · Responsabilité · Course aux Armements · Traités d&apos;Interdiction</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#1e293b40" } : {}}>
