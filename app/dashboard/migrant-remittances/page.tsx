...
interface SwarmData { engine: string; avg_composite: number; distribution: Record<string, number>; entities: Entity[]; estimated_migrant_remittances_index: number; }
...
export default function MigrantRemittancesPage() {
  const [data, setData] = useState<SwarmData | null>(null);
  const [loading, setLoading] = useState(true);

  async function load() {
    try {
      const res = await fetch("/api/migrantremittances");
  ...
  return (
    <div className="p-6 space-y-6 text-slate-100">
      <div>
        <h1 className="text-2xl font-bold text-white">Droits des Migrants — Remittances & Expulsions</h1>
        <p className="text-slate-400 text-sm mt-1">Transferts de fonds, déportations forcées et protection des travailleurs migrants</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        {data.estimated_migrant_remittances_index != null && (
          <p className="text-xs text-slate-500 mt-3">Indice migrants & remittances : <span className="text-indigo-300 font-semibold">{data.estimated_migrant_remittances_index}/10</span></p>
        )}
      </div>
    </div>
  );
