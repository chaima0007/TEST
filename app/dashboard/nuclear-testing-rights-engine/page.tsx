...
interface Entity {
  ...
  last_updated: string;
  radiation_exposure_civilian_score?: number;
  testing_zone_displacement_score?: number;
  long_term_health_impact_score?: number;
  reparation_denial_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "radiation_exposure_civilian_score", label: "Exposition aux radiations (civils)" },
  { key: "testing_zone_displacement_score", label: "Déplacements zones d&apos;essais" },
  { key: "long_term_health_impact_score", label: "Impact sanitaire long terme" },
  { key: "reparation_denial_score", label: "Refus de réparation" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits humains liés aux essais nucléaires : exposition aux radiations civiles, déplacements des zones d&apos;essais, impacts sanitaires long terme et refus de réparation.</p>
              <p className="text-slate-400">Source : Nuclear Testing Rights Engine · Wave 247</p>
            </div>
          )}
        </div>
      </div>
...
export default function NuclearTestingRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/nuclear-testing-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Nuclear Testing Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Exposition Radiations · Déplacements · Impact Sanitaire · Réparation</p>
        </div>
