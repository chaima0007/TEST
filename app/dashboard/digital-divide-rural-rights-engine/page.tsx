...
interface Entity {
  ...
  last_updated: string;
  rural_internet_access_gap_score?: number;
  digital_skills_exclusion_score?: number;
  public_service_digital_exclusion_score?: number;
  infrastructure_investment_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "rural_internet_access_gap_score", label: "Écart d&apos;accès internet rural" },
  { key: "digital_skills_exclusion_score", label: "Exclusion par compétences numériques" },
  { key: "public_service_digital_exclusion_score", label: "Exclusion des services publics numériques" },
  { key: "infrastructure_investment_gap_score", label: "Déficit d&apos;investissement en infrastructure" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la fracture numérique rurale : accès internet, exclusion par compétences numériques, services publics dématérialisés et déficits d&apos;investissement en infrastructure.</p>
              <p className="text-slate-400">Source : Digital Divide Rural Rights Engine · Wave 252</p>
            </div>
          )}
        </div>
      </div>
...
export default function DigitalDivideRuralRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/digital-divide-rural-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Digital Divide Rural Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Accès Internet · Compétences Numériques · Services Publics · Infrastructure</p>
        </div>
