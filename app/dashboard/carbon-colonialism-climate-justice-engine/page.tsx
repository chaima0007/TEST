...
interface Entity {
  ...
  carbon_offset_land_grab_score: number;
  climate_finance_inequity_score: number;
  green_energy_displacement_score: number;
  loss_damage_reparations_deficit_score: number;
  estimated_carbon_colonialism_climate_justice_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_carbon_colonialism_climate_justice_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Accaparement Terres via Crédits Carbone", value: entity.carbon_offset_land_grab_score, weight: "0.30" },
    { label: "Inéquité Financement Climatique", value: entity.climate_finance_inequity_score, weight: "0.25" },
    { label: "Déplacement par Énergie Verte", value: entity.green_energy_displacement_score, weight: "0.25" },
    { label: "Déficit Réparations Pertes & Dommages", value: entity.loss_damage_reparations_deficit_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_carbon_colonialism_climate_justice_index === "number" ? entity.estimated_carbon_colonialism_climate_justice_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Colonialisme Carbone</div>
                </div>
              </div>
...
export default function CarbonColonialismClimateJusticeEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/carbon-colonialism-climate-justice-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Colonialisme Carbone &amp; Justice Climatique…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_carbon_colonialism_climate_justice_index ?? avg(allEntities.map(e => e.estimated_carbon_colonialism_climate_justice_index));
  ...
  const avgLandGrab = avg(allEntities.map(e => e.carbon_offset_land_grab_score));
  const avgFinance = avg(allEntities.map(e => e.climate_finance_inequity_score));
  const avgDisplacement = avg(allEntities.map(e => e.green_energy_displacement_score));
  const avgLossDamage = avg(allEntities.map(e => e.loss_damage_reparations_deficit_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Colonialisme Carbone", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Colonialisme Carbone &amp; Justice Climatique</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Carbon Colonialism Climate Justice Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgLandGrab} label="Accaparement Carbone" />
          <GaugeRing value={avgFinance} label="Inéquité Financement" />
          <GaugeRing value={avgDisplacement} label="Déplacement Énergie Verte" />
          <GaugeRing value={avgLossDamage} label="Déficit Pertes & Dommages" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Colonialisme Carbone: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_carbon_colonialism_climate_justice_index === "number" ? e.estimated_carbon_colonialism_climate_justice_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
