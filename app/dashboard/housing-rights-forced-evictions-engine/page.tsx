...
export default function HousingRightsForcedEvictionsPage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#052e16";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", "modéré": "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/housing-rights-forced-evictions-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_housing_rights_forced_evictions_index"] as number) ?? 0;
