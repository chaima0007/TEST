...
export default function CulturalGenocideHeritageDestructionPage() {
  ...
  useEffect(() => {
    fetch("/api/cultural-genocide-heritage-destruction-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Génocide Culturel &amp; Destruction du Patrimoine</h1>
          <p className="text-slate-400 mt-1 text-sm">UNESCO · Crimes de Guerre · Identité Culturelle · Mémoire Collective</p>
        </div>
