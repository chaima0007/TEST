...
interface Entity {
  ...
  last_updated: string;
  development_exclusion_score?: number;
  resource_benefit_denial_score?: number;
  development_forced_displacement_score?: number;
  participation_rights_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "development_exclusion_score", label: "Exclusion des processus de développement" },
  { key: "resource_benefit_denial_score", label: "Refus du bénéfice des ressources naturelles" },
  { key: "development_forced_displacement_score", label: "Déplacement forcé lié au développement" },
  { key: "participation_rights_gap_score", label: "Lacunes dans les droits de participation" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse du droit au développement : exclusion des processus, refus des bénéfices des ressources naturelles, déplacements forcés liés au développement et lacunes dans les droits de participation.</p>
              <p className="text-slate-400">Source : Right to Development Rights Engine · Wave 250</p>
            </div>
          )}
        </div>
      </div>
...
export default function RightToDevelopmentRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-development-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Right to Development Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Exclusion Développement · Ressources Naturelles · Déplacements Forcés · Participation</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#065f4620" } : {}}>
