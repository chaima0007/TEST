...
interface Entity {
  ...
  last_updated: string;
  state_sanctioned_therapy_score?: number;
  religious_coercion_therapy_score?: number;
  minor_psychological_harm_score?: number;
  medical_complicity_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "state_sanctioned_therapy_score", label: "Thérapies sanctionnées par l&apos;État" },
  { key: "religious_coercion_therapy_score", label: "Coercition religieuse thérapeutique" },
  { key: "minor_psychological_harm_score", label: "Préjudice psychologique mineurs" },
  { key: "medical_complicity_score", label: "Complicité médicale" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des thérapies de conversion : pratiques sanctionnées par l&apos;État, coercition religieuse, préjudices psychologiques graves sur les mineurs et complicité du corps médical.</p>
              <p className="text-slate-400">Source : Conversion Therapy Rights Engine · Wave 260</p>
            </div>
          )}
        </div>
      </div>
...
export default function ConversionTherapyRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/conversion-therapy-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Conversion Therapy Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">État · Coercition Religieuse · Mineurs · Complicité Médicale</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#ec489920" } : {}}>
