...
interface FAEntity {
  ...
  composite_score: number; protest_criminalization_score: number;
  excessive_force_dispersal_score: number; organizer_persecution_score: number;
  emergency_law_abuse_score: number; risk_level: string; primary_pattern: string;
  key_signals: string[]; estimated_freedom_of_assembly_index: number; last_updated: string;
}
interface FASummary {
  ...
  confidence_score: number; data_sources: string[]; entities: FAEntity[];
  avg_estimated_freedom_of_assembly_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: FAEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "Aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Criminalisation Manifestation", v: entity.protest_criminalization_score, c: "#ef4444" },
              { l: "Force Dispersion", v: entity.excessive_force_dispersal_score, c: "#f97316" },
              { l: "Persécution Org.", v: entity.organizer_persecution_score, c: "#a855f7" },
              { l: "Abus Lois Urgence", v: entity.emergency_law_abuse_score, c: "#f59e0b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Liberté de Réunion</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_freedom_of_assembly_index}</span>
            </div>
          </div>
        )}
        {tab === "Signaux" && (
        ...
        {tab === "Contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Mécanismes d'urgence de protection des manifestants persécutés</p><p>› Pressions diplomatiques internationales pour la liberté de réunion</p><p>› Programmes d'aide juridique d'urgence pour organisateurs arrêtés</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des garanties légales pour les rassemblements pacifiques</p><p>› Réseaux de soutien aux organisateurs sous pression politique</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance accrue des restrictions aux rassemblements publics</p><p>› Développement de chartes de liberté d'expression et de réunion</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la vigilance sur la liberté de réunion</p><p>› Consolidation des mécanismes de protection des manifestants</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  repression_totale_reunions: "#ef4444",
  controle_manifestations: "#f97316",
  restrictions_progressives: "#f59e0b",
  tensions_assemblees: "#eab308",
  liberte_reunion_exemplaire: "#10b981",
};

export default function FreedomOfAssemblyEnginePage() {
  const [data, setData] = useState<FASummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<FAEntity | null>(null);

  useEffect(() => {
    fetch("/api/freedom-of-assembly-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données liberté de réunion"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la liberté de réunion en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof FAEntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Liberté de Réunion</h1>
          <p className="text-slate-400 text-sm mt-1">La criminalisation des rassemblements pacifiques comme baromètre de la dérive autoritaire — quand les foules deviennent des ennemis d'État</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Composite Moyen", v: `${data.avg_composite}%`, t: "text-orange-300" },
            ...
            { l: "Index Moyen Réunion", v: data.avg_estimated_freedom_of_assembly_index, t: "text-orange-400" },
            { l: "Confiance Analyse", v: `${Math.round((data.confidence_score ?? 0) * 100)}%`, t: "text-slate-300" },
          ].map(({ l, v, t }) => (
            <div key={l} className="bg-slate-900 border border-orange-500/20 rounded-xl p-4">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 rounded-xl p-6 border border-orange-500/20">
          ...
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("protest_criminalization_score")} label="Criminalisation Manifestation" color={ACCENT} />
            <GaugeRing value={avgScore("excessive_force_dispersal_score")} label="Force Dispersion" color="#f97316" />
            <GaugeRing value={avgScore("organizer_persecution_score")} label="Persécution Org." color="#a855f7" />
            <GaugeRing value={avgScore("emergency_law_abuse_score")} label="Abus Lois Urgence" color="#f59e0b" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-orange-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Répression des Réunions</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-orange-900/10 border border-orange-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Critiques — Liberté de Réunion</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              className={`bg-slate-900 rounded-xl p-4 border cursor-pointer transition-all hover:border-orange-500/50 ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_freedom_of_assembly_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Liberté de Réunion · {data.last_analysis}</p>
      </div>
    </div>
  );
}
