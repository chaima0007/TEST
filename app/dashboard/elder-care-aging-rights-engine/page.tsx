...
interface Entity {
  ...
  elder_abuse_neglect_institutionalization_severity_score: number;
  pension_social_protection_exclusion_scale_score: number;
  ageism_employment_healthcare_discrimination_score: number;
  dementia_care_autonomy_rights_deficit_gap_score: number;
  estimated_elder_care_aging_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_elder_care_aging_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Maltraitance & Institutionnalisation", value: entity.elder_abuse_neglect_institutionalization_severity_score, weight: "0.30" },
    { label: "Exclusion Pension & Protection Sociale", value: entity.pension_social_protection_exclusion_scale_score, weight: "0.25" },
    { label: "Âgisme Emploi & Santé", value: entity.ageism_employment_healthcare_discrimination_score, weight: "0.25" },
    { label: "Déficit Soins Démence & Autonomie", value: entity.dementia_care_autonomy_rights_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_elder_care_aging_rights_index === "number" ? entity.estimated_elder_care_aging_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Personnes Âgées</div>
                </div>
              </div>
...
export default function ElderCareAgingRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/elder-care-aging-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits des Personnes Âgées & Soins…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_elder_care_aging_rights_index ?? avg(allEntities.map(e => e.estimated_elder_care_aging_rights_index));
  ...
  const avgElderAbuse = avg(allEntities.map(e => e.elder_abuse_neglect_institutionalization_severity_score));
  const avgPension = avg(allEntities.map(e => e.pension_social_protection_exclusion_scale_score));
  const avgAgeism = avg(allEntities.map(e => e.ageism_employment_healthcare_discrimination_score));
  const avgDementia = avg(allEntities.map(e => e.dementia_care_autonomy_rights_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Personnes Âgées", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits des Personnes Âgées & Soins</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Elder Care & Aging Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgElderAbuse} label="Maltraitance" />
          <GaugeRing value={avgPension} label="Exclusion Pension" />
          <GaugeRing value={avgAgeism} label="Âgisme" />
          <GaugeRing value={avgDementia} label="Soins Démence" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Personnes Âgées: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_elder_care_aging_rights_index === "number" ? e.estimated_elder_care_aging_rights_index.toFixed(2) : "—"}</span>
            </div>
