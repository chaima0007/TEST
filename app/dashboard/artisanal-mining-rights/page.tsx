...
interface Entity {
  ...
  last_updated: string;
  labor_conditions_score?: number;
  environmental_damage_score?: number;
  legal_recognition_score?: number;
  revenue_sharing_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "labor_conditions_score", label: "Conditions de travail" },
  { key: "environmental_damage_score", label: "Dommages environnementaux" },
  { key: "legal_recognition_score", label: "Reconnaissance légale" },
  { key: "revenue_sharing_score", label: "Partage des revenus" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits dans les mines artisanales : conditions de travail, dommages environnementaux, reconnaissance légale et partage des revenus affectant les mineurs artisanaux et leurs communautés.</p>
              <p className="text-slate-400">Source : Artisanal Mining Rights Engine · Wave 267</p>
            </div>
          )}
        </div>
      </div>
...
export default function ArtisanalMiningRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/artisanal-mining-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits dans les Mines Artisanales</h1>
          <p className="text-slate-400 mt-1 text-sm">Conditions de Travail · Dommages Environnementaux · Reconnaissance Légale · Partage des Revenus</p>
        </div>
