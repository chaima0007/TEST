...
const ACCENT = "#1a0f00";
const INDEX_KEY = "estimated_cultural_heritage_rights_index";
...
const FALLBACK_ENTITIES = [
  { id: "CHR-001", name: "British Museum", country: "Royaume-Uni", sector: "8M objets issus de la colonisation — Marbres d&apos;Elgin, Rosette, Benin Bronzes refus systématique de restitution", composite_score: 88, risk_level: "critique", estimated_cultural_heritage_rights_index: 8.8 },
  { id: "CHR-002", name: "France/Louvre", country: "France", sector: "Acquisitions coloniales non restituées — Afrique subsaharienne, Maghreb, Océanie & rapport Savoy ignoré", composite_score: 86, risk_level: "critique", estimated_cultural_heritage_rights_index: 8.6 },
  { id: "CHR-003", name: "Smithsonian", country: "États-Unis", sector: "Collections amérindiennes & restes humains — NAGPRA partiellement appliqué, 100 000+ objets non rendus", composite_score: 84, risk_level: "critique", estimated_cultural_heritage_rights_index: 8.4 },
  { id: "CHR-004", name: "Belgium", country: "Belgique", sector: "Tervuren & trésors congolais — pillage systématique colonisation Leopold II, restitution très partielle", composite_score: 80, risk_level: "critique", estimated_cultural_heritage_rights_index: 8.0 },
  { id: "CHR-005", name: "Germany", country: "Allemagne", sector: "Bronzes du Bénin en cours de restitution — premier État européen à agir mais processus incomplet", composite_score: 53, risk_level: "élevé", estimated_cultural_heritage_rights_index: 5.3 },
  { id: "CHR-006", name: "Netherlands", country: "Pays-Bas", sector: "Politique de restitution adoptée 2023 — 478 objets restitués mais cadre juridique fragile", composite_score: 50, risk_level: "élevé", estimated_cultural_heritage_rights_index: 5.0 },
  { id: "CHR-007", name: "Australia", country: "Australie", sector: "Programme AIATSIS de rapatriement aborigène — progrès significatifs mais restes humains encore retenus", composite_score: 37, risk_level: "modéré", estimated_cultural_heritage_rights_index: 3.7 },
  { id: "CHR-008", name: "New Zealand/Te Papa", country: "Nouvelle-Zélande", sector: "Politique biculturelle Maori intégrée — restitution koiwi tangata (ossements) systématique & exemplaire", composite_score: 9, risk_level: "faible", estimated_cultural_heritage_rights_index: 0.9 },
];
...
const FALLBACK_DATA = {
  ...
  avg_estimated_cultural_heritage_rights_index: 6.09,
  confidence_score: 0.84,
  last_analysis: "2026-06-22",
  data_sources: ["unidroit_cultural_property", "unesco_convention_1970", "source_lab_provenance", "association_critical_heritage"],
  critical_alerts: ["British Museum: 8M_objets_colonisation_dont_marbles_elgin", "France/Louvre: acquisitions_coloniales_non_restituées"],
...
interface DashData {
  ...
  avg_estimated_cultural_heritage_rights_index?: number;
        ...
        <div style={{ padding: "20px 24px 24px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
            {[
              ...
              { label: "Index CHR", value: indexVal !== undefined ? `${indexVal}/10` : "—" },
          ...
          <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "12px 16px" }}>
            ...
            <p style={{ fontSize: 11, color: "#475569", margin: "6px 0 0" }}>
              Patrimoine culturel &amp; restitution coloniale
            </p>
          </div>
        </div>
      </div>
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/cultural-heritage-rights-engine")
      .then(r => r.json())
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020817", color: "white", padding: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#fb923c", margin: "0 0 4px" }}>
            Patrimoine Culturel &mdash; Restitution &amp; Droits
          </h1>
          <p style={{ fontSize: 13, color: "#64748b", margin: 0 }}>
            Cultural Heritage Rights Engine &middot; Wave 214
          </p>
        </div>
      ...
      {loading && (
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: 200 }}>
          <div style={{ width: 32, height: 32, border: "2px solid #fb923c", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
        </div>
      )}
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 28 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px", display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              <GaugeRing value={dash.avg_composite} stroke="#fb923c" />
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
              <p style={{ fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 6px" }}>Index CHR Moyen</p>
              <p style={{ fontSize: 28, fontWeight: 700, color: "#fb923c", margin: 0 }}>{dash.avg_estimated_cultural_heritage_rights_index ?? "—"}</p>
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
              <p style={{ fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 6px" }}>Entit&eacute;s</p>
              <p style={{ fontSize: 28, fontWeight: 700, color: "#fb923c", margin: 0 }}>{dash.total_entities}</p>
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
          ...
          {(dash.data_sources ?? []).length > 0 && (
            <div style={{ background: "rgba(26,15,0,0.3)", border: "1px solid rgba(251,146,60,0.15)", borderRadius: 12, padding: "14px 18px", marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {(dash.data_sources ?? []).map((src: string, i: number) => (
                  <span key={i} style={{ fontSize: 12, color: "#fb923c", background: "rgba(251,146,60,0.08)", border: "1px solid rgba(251,146,60,0.15)", borderRadius: 6, padding: "3px 10px" }}>
                    {src}
                  </span>
                ))}
              </div>
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, gap: 8 }}>
                    ...
                    <div style={{ flexShrink: 0 }}>
                      <GaugeRing value={e.composite_score} stroke={RISK_COLORS[e.risk_level] ?? "#fb923c"} />
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: cfg.color }}>{cfg.label}</span>
                    <span style={{ fontSize: 12, color: "#64748b" }}>
                      Index: <span style={{ fontWeight: 700, color: "#fb923c" }}>{indexVal ?? "—"}</span>
                    </span>
                  </div>
                </button>
              );
