...
interface ABREntity {
  ...
  employment_housing_algorithmic_exclusion_severity_score: number;
  predictive_policing_racial_bias_scale_score: number;
  judicial_algorithmic_sentencing_gap_score: number;
  algorithmic_transparency_right_explanation_absence_score: number;
  estimated_algorithmic_bias_rights_index: number;
  risk_level: string;
...
interface ABRData {
  ...
  avg_estimated_algorithmic_bias_rights_index: number;
...
const SUB_KEYS = [
  { key: "employment_housing_algorithmic_exclusion_severity_score", label: "Exclusion Emploi/Logement" },
  { key: "predictive_policing_racial_bias_scale_score",             label: "Biais Police Prédictive" },
  { key: "judicial_algorithmic_sentencing_gap_score",               label: "Écart Sentencing Judiciaire" },
  { key: "algorithmic_transparency_right_explanation_absence_score",label: "Absence Transparence Algo" },
];
...
function DetailModal({ entity, onClose, dataSources }: { entity: ABREntity; onClose: () => void; dataSources: string[] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={onClose}>
      <div className="bg-slate-900 border border-purple-500/30 rounded-2xl w-full max-w-lg mx-4 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex border-b border-slate-800">
          {(["Aperçu", "Métriques", "Sources"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`flex-1 py-2.5 text-sm font-medium transition-colors ${tab === t ? "border-b-2 text-purple-400" : "text-slate-500 hover:text-slate-300"}`}
          ...
          {tab === "Aperçu" && (
            <div className="space-y-4">
              ...
              <div className="bg-slate-800 rounded-lg p-3">
                <div className="text-xs text-slate-500 mb-1">Index Biais Algorithmiques &amp; Droits</div>
                <div className="text-lg font-bold" style={{ color: ACCENT }}>{entity.estimated_algorithmic_bias_rights_index} / 10</div>
              </div>
          ...
          {tab === "Métriques" && (
            <div className="space-y-3">
              {SUB_KEYS.map(({ key, label }, i) => {
                ...
                const colors = [ACCENT, "#c084fc", "#d8b4fe", "#e9d5ff"];
                return (
                  <div key={key}>
              ...
              <div className="pt-2 flex justify-between text-sm border-t border-slate-800">
                ...
                <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_algorithmic_bias_rights_index} / 10</span>
              </div>
            </div>
          )}
          ...
          {tab === "Sources" && (
            <div className="space-y-3">
              ...
              <div className="bg-slate-800 rounded-lg p-3">
                ...
                <div className="text-sm font-mono text-purple-300">{entity.primary_pattern}</div>
              </div>
            </div>
          )}
        </div>
...
export default function AlgorithmicBiasRightsPage() {
  const [data, setData] = useState<ABRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<ABREntity | null>(null);

  useEffect(() => {
    fetch("/api/algorithmic-bias-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données de biais algorithmiques…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl font-bold tracking-tight" style={{ color: ACCENT }}>Biais Algorithmiques &amp; Droits</h1>
          </div>
          <p className="text-slate-400 text-sm ml-6">Exclusion algorithmique, biais prédictifs et absence de transparence dans les systèmes décisionnels</p>
          <p className="text-slate-600 text-xs ml-6 mt-0.5">Caelum Partners · Algorithmic Bias Rights Engine · {data.last_analysis}</p>
        </div>

        {/* Critical Alerts */}
        {(data.critical_alerts?.length ?? 0) > 0 && (
          <div className="border border-purple-500/30 bg-purple-500/10 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-purple-400 mb-2">Alertes Critiques</h2>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen",        value: (data.avg_estimated_algorithmic_bias_rights_index ?? 0).toFixed(2) },
            ...
            <div key={k.label} className="bg-slate-900 border border-purple-500/20 rounded-xl p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-purple-500/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns de Biais Algorithmiques</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={[ACCENT, "#c084fc", "#d8b4fe", "#e9d5ff", "#f3e8ff"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              className={`bg-slate-900 border rounded-xl p-4 cursor-pointer hover:border-purple-500/50 transition-all ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-500">idx {entity.estimated_algorithmic_bias_rights_index}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Biais Algorithmiques &amp; Droits · {data.last_analysis}</p>
      </div>
    </div>
  );
}
