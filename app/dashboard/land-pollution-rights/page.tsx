...
interface Entity {
  ...
  last_updated: string;
  soil_contamination_score?: number;
  industrial_dumping_score?: number;
  remediation_rights_score?: number;
  toxic_exposure_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "soil_contamination_score", label: "Contamination des sols" },
  { key: "industrial_dumping_score", label: "Déversements industriels" },
  { key: "remediation_rights_score", label: "Droits à la dépollution" },
  { key: "toxic_exposure_score", label: "Exposition aux toxiques" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits contre la pollution des terres : contamination des sols, déversements industriels, droits à la dépollution et expositions aux toxiques affectant les communautés.</p>
              <p className="text-slate-400">Source : Land Pollution Rights Engine · Wave 264</p>
            </div>
          )}
        </div>
      </div>
...
export default function LandPollutionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/land-pollution-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits contre la Pollution des Terres</h1>
          <p className="text-slate-400 mt-1 text-sm">Contamination des Sols · Déversements Industriels · Dépollution · Exposition aux Toxiques</p>
        </div>
