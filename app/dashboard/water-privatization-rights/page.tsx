...
const SUB_SCORES = [
  { key: "access_control_score", label: "Contrôle d&apos;accès" },
  { key: "pricing_equity_score", label: "Équité tarifaire" },
  { key: "regulatory_capture_score", label: "Capture réglementaire" },
  { key: "community_impact_score", label: "Impact communautaire" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits contre la privatisation de l&apos;eau : contrôle d&apos;accès, équité tarifaire, capture réglementaire et impact sur les communautés affectant les populations vulnérables.</p>
              <p className="text-slate-400">Source : Water Privatization Rights Engine · Wave 267</p>
            </div>
          )}
        </div>
      </div>
...
export default function WaterPrivatizationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/water-privatization-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits contre la Privatisation de l&apos;Eau</h1>
          <p className="text-slate-400 mt-1 text-sm">Contrôle d&apos;Accès · Équité Tarifaire · Capture Réglementaire · Impact Communautaire</p>
        </div>
