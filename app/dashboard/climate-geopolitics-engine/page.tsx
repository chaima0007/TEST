...
interface CGEntity {
  ...
  composite_score: number; arctic_resources_militarization_score: number;
  climate_migration_weaponization_score: number; green_tech_monopoly_score: number;
  climate_sabotage_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_climate_geopolitics_index: number; last_updated: string;
}
interface CGSummary {
  ...
  entities: CGEntity[]; avg_estimated_climate_geopolitics_index: number;
}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Militarisation Ressources Arctiques", v: entity.arctic_resources_militarization_score, c: "#22c55e" },
              { l: "Weaponisation Migration Climatique", v: entity.climate_migration_weaponization_score, c: "#16a34a" },
              { l: "Monopole Tech Verte", v: entity.green_tech_monopoly_score, c: "#15803d" },
              { l: "Sabotage Climatique", v: entity.climate_sabotage_score, c: "#166534" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Géopolitique Climatique Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_climate_geopolitics_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Activation d'une surveillance renforcée des routes arctiques et coordination avec les membres du Conseil de l'Arctique</p><p>› Déploiement de contre-mesures face aux monopoles tech verts et renforcement des alliances de transition énergétique souveraine</p></>}
            {entity.risk_level === "élevé" && <><p>› Audit des dépendances en technologies vertes critiques et diversification des chaînes d'approvisionnement en composants énergétiques</p><p>› Surveillance des flux migratoires climatiques instrumentalisés et renforcement des cadres de protection juridique internationale</p></>}
            {entity.risk_level === "modéré" && <><p>› Suivi des revendications territoriales arctiques et analyse des doctrines d'exploitation des ressources en zones disputées</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille sur les accords climatiques et les dynamiques de transition énergétique géopolitique</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function ClimateGeopoliticsPage() {
  const [data, setData] = useState<CGSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<CGEntity | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/climate-geopolitics-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données géopolitique climatique"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse de la géopolitique climatique en cours…</div></div>;
  ...
  const avgScore = (k: keyof CGEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Climate Geopolitics Engine</h1>
          <p className="text-slate-400 text-sm">Militarisation arctique, monopoles tech verte et migrations climatiques comme armes · v{data.engine_version}</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { l: "Entités Analysées", v: data.total_entities, g: "from-slate-700 to-slate-600", t: "text-white" },
            { l: "Score Moyen", v: `${data.avg_composite}%`, g: "from-green-900 to-green-800", t: "text-green-300" },
            { l: "Index Géopolitique Climatique Moyen", v: data.avg_estimated_climate_geopolitics_index, g: "from-green-900 to-green-800", t: "text-green-300" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs de Géopolitique Climatique Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("arctic_resources_militarization_score")} label="Militarisation Arctique" color="#22c55e" />
            <GaugeRing value={avgScore("climate_migration_weaponization_score")} label="Migration Climatique Arm." color="#16a34a" />
            <GaugeRing value={avgScore("green_tech_monopoly_score")} label="Monopole Tech Verte" color="#15803d" />
            <GaugeRing value={avgScore("climate_sabotage_score")} label="Sabotage Climatique" color="#166534" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns Géopolitique Climatique</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={data.total_entities} color={["#22c55e", "#16a34a", "#15803d", "#166534", "#14532d"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        {data.critical_alerts.length > 0 && (
          <div className="rounded-xl p-4" style={{ backgroundColor: "#22c55e10", border: "1px solid #22c55e30" }}>
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Géopolitique Climatique Critiques</h2>
            <ul className="space-y-1">{data.critical_alerts.map((a, i) => <li key={i} className="text-sm" style={{ color: "#86efac" }}>› {a}</li>)}</ul>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              onMouseEnter={e => (e.currentTarget.style.borderColor = "#22c55e50")}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_climate_geopolitics_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Climate Geopolitics Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
