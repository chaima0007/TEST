...
interface Entity {
  ...
  internet_censorship_shutdown_score: number;
  surveillance_privacy_violation_score: number;
  digital_expression_suppression_score: number;
  algorithmic_discrimination_score: number;
  signals?: string[];
  context?: string;
}
...
interface ApiData {
  ...
  eleve_count: number;
  avg_estimated_digital_rights_index: number;
...
function DetailModal({ entity, onClose }: DetailModalProps) {
  ...
  const subScores = [
    { label: "Censure/Coupure", value: entity.internet_censorship_shutdown_score },
    { label: "Surveillance", value: entity.surveillance_privacy_violation_score },
    { label: "Expression Numérique", value: entity.digital_expression_suppression_score },
    { label: "Discrimination Algo.", value: entity.algorithmic_discrimination_score },
  ];
...
export default function DigitalRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/digital-rights-engine")
      .then((r) => r.json())
  ...
  const kpis = [
    ...
    { label: "Index Moyen", value: data?.avg_estimated_digital_rights_index ?? 0, fmt: (v: number) => v.toFixed(1) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>
          Digital Rights Engine
        </h1>
        <p className="text-slate-400 text-sm mt-1">
          Analyse de la censure numérique, surveillance et droits digitaux
        </p>
      </div>
      ...
      {!loading && data && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6 flex items-center gap-6">
          <GaugeRing value={data.avg_estimated_digital_rights_index ?? 0} color={ACCENT} />
          <div>
            <p className="text-slate-400 text-sm">Index Moyen Digital Rights</p>
            <p className="text-white text-3xl font-bold">{(data.avg_estimated_digital_rights_index ?? 0).toFixed(1)}</p>
          </div>
        </div>
      )}
