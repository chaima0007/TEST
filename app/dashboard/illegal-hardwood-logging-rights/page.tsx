      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: 24, maxWidth: 480, width: "90%", maxHeight: "80vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        ...
        {activeTab === "Analyse" && (
          <>
            ...
            {entity.estimated_illegal_hardwood_logging_index !== undefined && (
              <p style={{ color: "#64748b", fontSize: 14 }}>Indice estimé : <strong>{entity.estimated_illegal_hardwood_logging_index}</strong></p>
            )}
          </>
        )}
        {activeTab === "Entités" && (
...
export default function IllegalHardwoodLoggingRightsDashboard() {
  const [data, setData] = useState<any>(null);
  const [selected, setSelected] = useState<any>(null);

  useEffect(() => {
    fetch("/api/illegal-hardwood-logging-rights")
      .then(r => r.json())
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f8fafc", padding: 32 }}>
      <h1 style={{ fontSize: 28, fontWeight: 800, color: "#1e293b", marginBottom: 8 }}>
        Droits — Bois Précieux Illégaux
      </h1>
      <p style={{ color: "#64748b", marginBottom: 32 }}>Analyse CaelumSwarm™ — Exploitation illégale de bois précieux et déforestation</p>
