...
interface Entity {
  ...
  systemic_discrimination_score: number;
  racial_profiling_police_score: number;
  wealth_education_gap_score: number;
  hate_speech_ideology_score: number;
  signals?: string[];
  context?: string;
}
...
interface ApiData {
  ...
  eleve_count: number;
  avg_estimated_racial_justice_index: number;
...
function DetailModal({ entity, onClose }: DetailModalProps) {
  ...
  const subScores = [
    { label: "Discrim. Systémique", value: entity.systemic_discrimination_score },
    { label: "Profilage Racial", value: entity.racial_profiling_police_score },
    { label: "Inégalité Richesse/Éducation", value: entity.wealth_education_gap_score },
    { label: "Discours Haine", value: entity.hate_speech_ideology_score },
  ];
...
export default function RacialJusticeEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/racial-justice-engine")
      .then((r) => r.json())
  ...
  const kpis = [
    ...
    { label: "Index Moyen", value: data?.avg_estimated_racial_justice_index ?? 0, fmt: (v: number) => v.toFixed(1) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>
          Racial Justice Engine
        </h1>
        <p className="text-slate-400 text-sm mt-1">
          Analyse des dynamiques de discrimination systémique et justice raciale
        </p>
      </div>
      ...
      {!loading && data && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6 flex items-center gap-6">
          <GaugeRing value={data.avg_estimated_racial_justice_index ?? 0} color={ACCENT} />
          <div>
            <p className="text-slate-400 text-sm">Index Moyen Racial Justice</p>
            <p className="text-white text-3xl font-bold">{(data.avg_estimated_racial_justice_index ?? 0).toFixed(1)}</p>
          </div>
        </div>
      )}
