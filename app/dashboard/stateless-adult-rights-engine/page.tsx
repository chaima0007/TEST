...
interface Entity {
  ...
  last_updated: string;
  statelessness_documentation_denial_score?: number;
  civil_registration_exclusion_score?: number;
  employment_legal_barrier_score?: number;
  detention_stateless_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "statelessness_documentation_denial_score", label: "Refus de documentation pour les apatrides" },
  { key: "civil_registration_exclusion_score", label: "Exclusion de l&apos;enregistrement civil" },
  { key: "employment_legal_barrier_score", label: "Barrières légales à l&apos;emploi" },
  { key: "detention_stateless_score", label: "Détention des personnes apatrides" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits des adultes apatrides : refus de documentation, exclusion de l&apos;enregistrement civil, barrières légales à l&apos;emploi et détention arbitraire des personnes apatrides.</p>
              <p className="text-slate-400">Source : Stateless Adult Rights Engine · Wave 250</p>
            </div>
          )}
        </div>
      </div>
...
export default function StatelessAdultRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/stateless-adult-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Stateless Adult Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Documentation · Enregistrement Civil · Emploi · Détention</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#37415120" } : {}}>
