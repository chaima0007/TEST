...
interface Entity {
  ...
  last_updated: string;
  overwork_exploitation_score?: number;
  rest_time_denial_score?: number;
  vacation_rights_violation_score?: number;
  cultural_participation_exclusion_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "overwork_exploitation_score", label: "Exploitation par surmenage" },
  { key: "rest_time_denial_score", label: "Déni du temps de repos" },
  { key: "vacation_rights_violation_score", label: "Violations des droits aux vacances" },
  { key: "cultural_participation_exclusion_score", label: "Exclusion de la participation culturelle" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse du droit au repos et aux loisirs : exploitation par surmenage, déni du temps de repos, violations des droits aux vacances et exclusion de la participation culturelle.</p>
              <p className="text-slate-400">Source : Right to Leisure Rights Engine · Wave 256</p>
            </div>
          )}
        </div>
      </div>
...
export default function RightToLeisureRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-leisure-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Right to Leisure Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Surmenage · Temps de Repos · Droits aux Vacances · Participation Culturelle</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#6d28d920" } : {}}>
