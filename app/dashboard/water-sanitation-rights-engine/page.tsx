...
export default function WaterSanitationRightsPage() {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  useEffect(() => {
    fetch("/api/water-sanitation-rights-engine")
      .then((r) => r.json())
  ...
  const avgIndex = (data.avg_estimated_water_sanitation_rights_index as number) ?? 0;
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-6">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: CYAN }}>
            Droit à l&apos;Eau & à l&apos;Assainissement (WASH)
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            SDG 6 · WASH · WHO/UNICEF JMP · Right to Safe Drinking Water · UN Resolution 64/292
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Avg Composite", value: avgComposite.toFixed(2), unit: "/100" },
            { label: "Indice WASH", value: avgIndex.toFixed(2), unit: "/10" },
            ...
            <div key={label} className="rounded-xl bg-slate-900 border border-slate-800 p-4 text-center">
              ...
              <div className="text-2xl font-bold" style={{ color: CYAN }}>{value}<span className="text-sm text-slate-500 ml-1">{unit}</span></div>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 flex flex-col items-center">
            <div className="text-xs text-slate-500 uppercase tracking-widest mb-4">Score Global WASH</div>
            <div className="w-32 h-32"><GaugeRing value={avgComposite} color={CYAN} /></div>
            <div className="mt-3 text-3xl font-bold" style={{ color: CYAN }}>{avgComposite.toFixed(1)}</div>
          </div>
          ...
          <div className="rounded-xl bg-slate-900 border border-slate-800 overflow-hidden">
            ...
            <div className="divide-y divide-slate-800">
              {entities.map((e) => {
                ...
                const idx = e.estimated_water_sanitation_rights_index as number;
                return (
                  <div key={e.entity_id as string} className="px-6 py-3 flex items-center gap-4">
                    <div className="text-xs font-mono text-slate-500 w-8">{e.entity_id as string}</div>
        ...
        <div className="rounded-xl border p-5" style={{ borderColor: CYAN + "40", backgroundColor: CYAN + "10" }}>
          <div className="text-xs font-semibold mb-1" style={{ color: CYAN }}>SDG 6 — Accès Universel 2030</div>
          <p className="text-xs text-slate-300 leading-relaxed">
            2,2 milliards de personnes n&apos;ont pas accès à l&apos;eau potable gérée en toute sécurité.
            4,2 milliards manquent d&apos;assainissement de base. Le droit à l&apos;eau a été reconnu par l&apos;ONU en 2010
            (Résolution 64/292) mais reste non justiciable dans 80% des pays critiques.
          </p>
        </div>
      </div>
    </div>
