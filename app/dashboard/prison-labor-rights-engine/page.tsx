...
interface Entity {
  ...
  forced_prison_labor_coercion_severity_score: number;
  wage_theft_below_minimum_compensation_scale_score: number;
  unsafe_working_conditions_incarcerated_score: number;
  legal_protection_prisoner_worker_gap_score: number;
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_prison_labor_rights_index: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "forced_prison_labor_coercion_severity_score", label: "Coercition Travail Pénitentiaire" },
  { key: "wage_theft_below_minimum_compensation_scale_score", label: "Vol Salaire / Sous-Rémunération" },
  { key: "unsafe_working_conditions_incarcerated_score", label: "Conditions Travail Dangereuses" },
  { key: "legal_protection_prisoner_worker_gap_score", label: "Gap Protection Légale" },
];
        ...
        <div className="flex border-b border-slate-800">
          {["Aperçu", "Métriques", "Sources"].map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-slate-400/50 text-slate-300" : "text-slate-400 hover:text-white"}`}>
              {t}
            </button>
          ))}
        </div>
          ...
          {tab === 0 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Indice Travail Prison</span>
                <span className="font-semibold" style={{ color: ACCENT }}>{entity.estimated_prison_labor_rights_index}/10</span>
              </div>
              <div className="flex items-center justify-between text-sm">
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/prison-labor-rights-engine").then(r=>r.json()).then(d=>{ setData(d.payload??d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Prison Labor Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Coercition Travail · Vol Salaire · Conditions Dangereuses · Protection Légale</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice Travail Prison</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_prison_labor_rights_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        {filtered.map(e => {
          ...
          return (
            <button key={e.entity_id} onClick={() => setSelected(e)}
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold uppercase ${cfg.color}`}>{cfg.label}</span>
                <span className="text-xs text-slate-500">Indice: <span className="font-bold" style={{ color: ACCENT }}>{e.estimated_prison_labor_rights_index}</span></span>
              </div>
            </button>
          );
        })}
