...
const PATTERN_COLORS: Record<string, string> = {
  none: "#16a34a",
  homelessness_crisis_explosion: "#dc2626",
  financialization_speculation_trap: "#7c3aed",
  rental_market_collapse: "#059669",
  displacement_gentrification: "#b45309",
  social_housing_defunding: "#0891b2",
};
const SEVERITY_COLORS: Record<string, string> = {
  "marché_logement_sous_surveillance": "#16a34a",
  "tension_marché_immobilier_structurelle": "#d97706",
  "crise_accessibilité_immobilière_majeure": "#92400e",
  "crise_logement_systémique_critique": "#dc2626",
};
const ACTION_COLORS: Record<string, string> = {
  "veille_marché_logement_continue": "#16a34a",
  "renforcement_politiques_accessibilité_logement": "#d97706",
  "régulation_marché_immobilier_accélérée": "#92400e",
  "intervention_urgente_crise_logement_systémique": "#dc2626",
};
...
const SEV_BADGE: Record<string, string> = {
  "marché_logement_sous_surveillance": "bg-green-900 text-green-300",
  "tension_marché_immobilier_structurelle": "bg-amber-900 text-amber-300",
  "crise_accessibilité_immobilière_majeure": "bg-orange-950 text-orange-400",
  "crise_logement_systémique_critique": "bg-red-950 text-red-400",
};
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <span className="ml-2 text-slate-500 text-xs">
              {entity.market_type.replace(/_/g, " ")}
            </span>
          </div>
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ...
              ["Score Spéculation",    entity.speculation_score,    "#7c3aed"],
              ["Score Offre",          entity.supply_score,         "#059669"],
              ["Score Sans-Abrisme",   entity.homelessness_score,   "#dc2626"],
            ].map(([l, v, c]) => (
              <div
                key={String(l)}
            ...
            <div className="col-span-2 bg-slate-900 border border-orange-700/20 rounded-lg p-3">
              <div className="text-orange-500/60 text-xs mb-1">Score Composite Crise Logement</div>
          ...
          <div className="bg-slate-900 border border-orange-700/20 rounded-lg p-4 text-sm text-slate-200 leading-relaxed">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="bg-slate-800 rounded p-2">
                <div className="text-orange-500/50 text-xs mb-0.5">Ratio Prix / Revenu</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.price_to_income_ratio * 100)}%
                </div>
              </div>
              <div className="bg-slate-800 rounded p-2">
                <div className="text-orange-500/50 text-xs mb-0.5">Taux Charge Locative</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.rent_burden_rate * 100)}%
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-900 border border-orange-700/20 rounded-lg p-3">
              <div className="text-orange-500/60 text-xs mb-1">Type de Marché</div>
            ...
            <div className="bg-slate-900 border border-orange-700/20 rounded-lg p-3">
              ...
              <div className="text-red-400 font-medium capitalize">
                {entity.housing_pattern.replace(/_/g, " ")}
              </div>
            </div>
          </div>
        )}
...
export default function HousingCrisisDashboard() {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-orange-500 text-lg animate-pulse">
          Initialisation du Moteur Crise Logement...
        </div>
      </div>
    );
  }
  ...
  const filtered = entities.filter(
    e =>
      ...
      (patFilter === "all" || e.housing_pattern === patFilter)
  );

  const dists = [
    { title: "Niveau de Risque",          counts: summary.risk_distribution,     colors: RISK_COLORS     },
    { title: "Patron Logement",           counts: summary.pattern_distribution,  colors: PATTERN_COLORS  },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-orange-500">
          Crise Logement &amp; Accessibilité Immobilière — Module 422
        </h1>
        <p className="text-red-500/60 text-sm mt-1">
          Accessibilité · Spéculation · Offre · Sans-Abrisme — Caelum Partners — Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ["Total Marchés",                 summary.total,                                                             "text-orange-500"],
          ["Crise Logement Critique",       summary.critical,                                                          "text-red-400"],
          ["Crise Majeure",                 summary.high,                                                              "text-orange-700"],
          ["Composite Moyen",               `${summary.avg_composite.toFixed(1)}`,                                     "text-orange-400"],
          ["Index Accessibilité Logement",  `${summary.avg_estimated_housing_affordability_index.toFixed(2)}/10`,      "text-orange-500"],
          ["Accessibilité Moyenne",         `${avg_affordability.toFixed(1)}`,                                         "text-red-400"],
        ].map(([l, v, c]) => (
          <div
            key={String(l)}
      ...
      <div className="bg-slate-900 border border-orange-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          ...
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.speculation_score, 0) / (entities.length || 1)}
            label="Spéculation Moy."
            color="#7c3aed"
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.supply_score, 0) / (entities.length || 1)}
            label="Offre Moy."
            color="#059669"
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.homelessness_score, 0) / (entities.length || 1)}
            label="Sans-Abrisme Moy."
            color="#dc2626"
          />
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {[
          "all",
          "none",
          "homelessness_crisis_explosion",
          "financialization_speculation_trap",
          "rental_market_collapse",
          "displacement_gentrification",
          "social_housing_defunding",
        ].map(p => (
          <button
            key={p}
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div className="text-xs text-red-400/60 mb-2 capitalize">
              {e.housing_pattern.replace(/_/g, " ")}
            </div>
            <div className="text-xs text-orange-500/70 font-medium mb-2">
              Access.: {e.affordability_score.toFixed(1)} · Spéc.: {e.speculation_score.toFixed(1)}
            </div>
