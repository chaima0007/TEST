...
interface PLEntity {
  ...
  estimated_port_capture_index: number; last_updated: string;
}
interface PLSummary {
  ...
  entities: PLEntity[]; avg_estimated_port_capture_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: PLEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Acquisition Portuaire Chinoise", v: entity.chinese_port_acquisition_score, c: "#0891b2" },
              { l: "Contrôle Points d'Étranglement", v: entity.strategic_chokepoint_control_score, c: "#f97316" },
              { l: "Dépendance Logistique", v: entity.logistics_dependency_score, c: "#eab308" },
              { l: "Infrastructure Double Usage", v: entity.dual_use_infrastructure_score, c: "#dc2626" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Capture Portuaire Estimé</span>
              <span className="text-cyan-400 font-bold">{entity.estimated_port_capture_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Plan d'urgence de diversification des routes logistiques stratégiques</p><p>› Audit immédiat des concessions portuaires étrangères et renégociation des contrats critiques</p></>}
            {entity.risk_level === "élevé" && <><p>› Réduction des dépendances aux opérateurs portuaires à capitaux étrangers sensibles</p><p>› Renforcement des partenariats multilatéraux sur les infrastructures logistiques</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance des investissements étrangers dans les infrastructures portuaires</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille sur les dynamiques d'acquisition portuaire stratégique</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function PortLogisticsCaptureEnginePage() {
  const [data, setData] = useState<PLSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<PLEntity | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/port-logistics-capture-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données capture portuaire logistique"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse de la capture portuaire &amp; logistique mondiale…</div></div>;
  ...
  const avgScore = (k: keyof PLEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Port Logistics Capture Intelligence</h1>
          <p className="text-slate-400 text-sm">Capture portuaire &amp; dépendance logistique stratégique — Caelum Partners · v{data.engine_version}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Index Capture Portuaire Moy.", v: data.avg_estimated_port_capture_index, g: "from-cyan-900 to-cyan-800", t: "text-cyan-300" },
          ].map(({ l, v, g, t }) => (
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs de Capture Portuaire Logistique Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("chinese_port_acquisition_score")} label="Acquisition Portuaire Chinoise" color="#0891b2" />
            <GaugeRing value={avgScore("strategic_chokepoint_control_score")} label="Contrôle Points d'Étranglement" color="#f97316" />
            <GaugeRing value={avgScore("logistics_dependency_score")} label="Dépendance Logistique" color="#eab308" />
            <GaugeRing value={avgScore("dual_use_infrastructure_score")} label="Infrastructure Double Usage" color="#dc2626" />
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Distribution Risques Capture Portuaire</h2>
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Capture Portuaire Logistique</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={data.total_entities} color={["#0891b2", "#f97316", "#eab308", "#10b981", "#dc2626"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-cyan-900/10 border border-cyan-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-cyan-400 mb-2">Alertes Capture Portuaire Logistique Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="h-1.5 bg-slate-800 rounded-full mb-2">
                <div className="h-full rounded-full" style={{ width: `${entity.composite_score}%`, backgroundColor: "#0891b2" }} />
              </div>
              <div className="flex justify-between text-xs text-slate-500">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span>idx {entity.estimated_port_capture_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Port Logistics Capture Intelligence · {data.last_analysis}</p>
      </div>
    </div>
  );
}
