...
interface SVWEntity {
  ...
  key_signals: string[];
  estimated_sexual_violence_wartime_index: number;
  systematic_rape_targeting_score: number;
  state_actor_perpetration_score: number;
  survivor_access_justice_score: number;
  conflict_zone_impunity_score: number;
  last_updated: string;
}

interface SVWSummary {
  total_entities: number;
  avg_composite: number;
  critical_alerts: number;
  avg_estimated_sexual_violence_wartime_index: number;
  data_sources: string[];
...
function EntityCard({ entity, onClick }: { entity: SVWEntity; onClick: (e: SVWEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.systematic_rape_targeting_score ?? "—"}</p>
          <p className="text-[10px] text-gray-500">Viol Systématique</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.conflict_zone_impunity_score ?? "—"}</p>
          <p className="text-[10px] text-gray-500">Impunité Zone Conflit</p>
        </div>
      </div>
...
function DetailModal({ entity, onClose }: { entity: SVWEntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Viol Systématique", value: `${entity.systematic_rape_targeting_score ?? "—"}/100` },
                  { label: "Perpétration État", value: `${entity.state_actor_perpetration_score ?? "—"}/100` },
                  { label: "Accès Justice Survivants", value: `${entity.survivor_access_justice_score ?? "—"}/100` },
                  { label: "Impunité Zone Conflit", value: `${entity.conflict_zone_impunity_score ?? "—"}/100` },
                  { label: "Indice VSG", value: `${entity.estimated_sexual_violence_wartime_index ?? "—"}/10` },
          ...
          {tab === "contexte" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 mt-1"><span className="text-gray-300 font-medium">Indice VSG :</span> {entity.estimated_sexual_violence_wartime_index ?? "—"}/10</p>
...
export default function Page() {
  const [data, setData] = useState<SVWData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [filter, setFilter] = useState("tous");
  const [modal, setModal] = useState<SVWEntity | null>(null);
  const [tab, setTab] = useState("apercu");

  useEffect(() => {
    fetch("/api/sexual-violence-wartime-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as SVWData);
        setLoading(false);
      })
      .catch((e) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Sexual Violence Wartime Engine</h1>
            <p className="text-sm text-gray-400 mt-1">Violences Sexuelles en Temps de Conflit</p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Indice VSG moyen" value={`${s.avg_estimated_sexual_violence_wartime_index ?? "—"}/10`} sub="score normalisé" accent="text-rose-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                ...
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_sexual_violence_wartime_index ?? 0} max={10} label="Indice VSG" color="#f43f5e" />
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filtered.map((entity) => (
                <EntityCard key={entity.entity_id} entity={entity} onClick={setModal} />
              ))}
              {filtered.length === 0 && (
