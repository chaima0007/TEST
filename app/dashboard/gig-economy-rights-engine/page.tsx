...
interface Entity {
  ...
  algorithmic_wage_theft_control_severity_score: number;
  zero_hours_contract_precarity_scale_score: number;
  social_protection_gig_worker_exclusion_score: number;
  platform_misclassification_labor_rights_gap_score: number;
  estimated_gig_economy_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_gig_economy_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Vol Salaire Algorithmique", value: entity.algorithmic_wage_theft_control_severity_score, weight: "0.30" },
    { label: "Contrats Zéro Heure", value: entity.zero_hours_contract_precarity_scale_score, weight: "0.25" },
    { label: "Exclusion Protection Sociale", value: entity.social_protection_gig_worker_exclusion_score, weight: "0.25" },
    { label: "Déclassification Plateforme", value: entity.platform_misclassification_labor_rights_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_gig_economy_rights_index === "number" ? entity.estimated_gig_economy_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Économie Gig</div>
                </div>
              </div>
...
export default function GigEconomyRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/gig-economy-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Économie Gig…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_gig_economy_rights_index ?? avg(allEntities.map(e => e.estimated_gig_economy_rights_index));
  ...
  const avgWageTheft = avg(allEntities.map(e => e.algorithmic_wage_theft_control_severity_score));
  const avgZeroHours = avg(allEntities.map(e => e.zero_hours_contract_precarity_scale_score));
  const avgSocialProtection = avg(allEntities.map(e => e.social_protection_gig_worker_exclusion_score));
  const avgMisclassification = avg(allEntities.map(e => e.platform_misclassification_labor_rights_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Économie Gig", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Gig Economy Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Vol salarial algorithmique, précarité des contrats zéro heure et exclusion de la protection sociale — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgWageTheft} label="Vol Salaire Algorithmique" />
          <GaugeRing value={avgZeroHours} label="Contrats Zéro Heure" />
          <GaugeRing value={avgSocialProtection} label="Exclusion Protection Sociale" />
          <GaugeRing value={avgMisclassification} label="Déclassification Plateforme" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Économie Gig: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_gig_economy_rights_index === "number" ? e.estimated_gig_economy_rights_index.toFixed(2) : "—"}</span>
            </div>
