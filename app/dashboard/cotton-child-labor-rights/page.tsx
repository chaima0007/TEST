...
interface Entity {
  ...
  child_labor_exposure_score: number;
  forced_labor_score: number;
  wage_theft_score: number;
  hazardous_conditions_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_cotton_child_labor_index: number;
  last_updated: string;
  [key: string]: unknown;
}
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_cotton_child_labor_index: number;
}
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/cotton-child-labor-rights").then(r => r.json()).then(j => setData(j.payload ?? j)).catch(console.error);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Droits — Travail Enfant dans le Coton</h1>
        <p className="text-slate-400 mt-1">Convention OIT n°138 &amp; 182 · UNICEF · Protocole Harkin-Engel</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice Coton</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_cotton_child_labor_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            <div className="p-6">
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { key: "child_labor_exposure_score", label: "Exposition Travail Enfant" },
                      { key: "forced_labor_score", label: "Travail Forcé" },
                      { key: "wage_theft_score", label: "Vol de Salaire" },
                      { key: "hazardous_conditions_score", label: "Conditions Dangereuses" },
                    ].map(({ key, label }) => (
                      <div key={key}>
              ...
              {tab === 2 && (
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-400">Indice Coton</span>
                    <span className="text-slate-200">{selected.estimated_cotton_child_labor_index}</span>
                  </div>
                </div>
              )}
            </div>
