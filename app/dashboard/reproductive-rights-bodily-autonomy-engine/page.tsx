...
interface Entity {
  ...
  abortion_criminalization_ban_severity_score: number;
  forced_sterilization_contraception_coercion_scale_score: number;
  maternal_mortality_healthcare_denial_score: number;
  reproductive_autonomy_legal_protection_deficit_gap_score: number;
  estimated_reproductive_rights_bodily_autonomy_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_reproductive_rights_bodily_autonomy_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Criminalisation & Interdiction Avortement", value: entity.abortion_criminalization_ban_severity_score, weight: "0.30" },
    { label: "Stérilisation Forcée & Coercition", value: entity.forced_sterilization_contraception_coercion_scale_score, weight: "0.25" },
    { label: "Mortalité Maternelle & Déni Soins", value: entity.maternal_mortality_healthcare_denial_score, weight: "0.25" },
    { label: "Déficit Protection Juridique Autonomie", value: entity.reproductive_autonomy_legal_protection_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_reproductive_rights_bodily_autonomy_index === "number" ? entity.estimated_reproductive_rights_bodily_autonomy_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Reproductifs</div>
                </div>
              </div>
...
export default function ReproductiveRightsBodilyAutonomyEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/reproductive-rights-bodily-autonomy-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Reproductifs & Autonomie Corporelle…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_reproductive_rights_bodily_autonomy_index ?? avg(allEntities.map(e => e.estimated_reproductive_rights_bodily_autonomy_index));
  ...
  const avgAbortion = avg(allEntities.map(e => e.abortion_criminalization_ban_severity_score));
  const avgSterilization = avg(allEntities.map(e => e.forced_sterilization_contraception_coercion_scale_score));
  const avgMaternal = avg(allEntities.map(e => e.maternal_mortality_healthcare_denial_score));
  const avgAutonomy = avg(allEntities.map(e => e.reproductive_autonomy_legal_protection_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Reproductifs", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Reproductifs & Autonomie Corporelle</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Reproductive Rights Bodily Autonomy Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgAbortion} label="Criminalisation Avortement" />
          <GaugeRing value={avgSterilization} label="Stérilisation Forcée" />
          <GaugeRing value={avgMaternal} label="Mortalité Maternelle" />
          <GaugeRing value={avgAutonomy} label="Protection Juridique" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Reproductifs: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_reproductive_rights_bodily_autonomy_index === "number" ? e.estimated_reproductive_rights_bodily_autonomy_index.toFixed(2) : "—"}</span>
            </div>
