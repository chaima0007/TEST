...
export default function EthnicMinorityRightsDiscriminationPage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#1e1b4b";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", "modéré": "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/ethnic-minority-rights-discrimination-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_ethnic_minority_rights_discrimination_index"] as number) ?? 0;
