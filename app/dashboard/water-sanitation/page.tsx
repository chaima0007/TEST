...
interface SwarmData { engine: string; avg_composite: number; distribution: Record<string, number>; entities: Entity[]; estimated_water_sanitation_index: number; }
...
export default function WaterSanitationPage() {
  ...
  async function load() {
    try {
      const res = await fetch("/api/watersanitation");
  ...
  return (
    <div className="p-6 space-y-6 text-slate-100">
      <div>
        <h1 className="text-2xl font-bold text-white">Droit à l&apos;Eau et à l&apos;Assainissement</h1>
        <p className="text-slate-400 text-sm mt-1">Accès à l&apos;eau potable, hygiène et dignité humaine</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        {data.estimated_water_sanitation_index != null && (
          <p className="text-xs text-slate-500 mt-3">Indice eau & assainissement : <span className="text-indigo-300 font-semibold">{data.estimated_water_sanitation_index}/10</span></p>
        )}
      </div>
    </div>
  );
