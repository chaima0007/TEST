...
export default function SexualViolenceConflictAccountabilityPage() {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  useEffect(() => {
    fetch("/api/sexual-violence-conflict-accountability-engine")
      .then((r) => r.json())
  ...
  const avgIndex = (data.avg_estimated_crsv_accountability_index as number) ?? 0;
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-6">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: ROSE }}>
            Violences Sexuelles en Conflit — Responsabilité
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            CRSV · UNSC Res. 1820 · ICC Prosecution · Survivor Justice · Reparations
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Avg Composite", value: avgComposite.toFixed(2), unit: "/100" },
            { label: "Indice CRSV", value: avgIndex.toFixed(2), unit: "/10" },
            ...
            <div key={label} className="rounded-xl bg-slate-900 border border-slate-800 p-4 text-center">
              ...
              <div className="text-2xl font-bold" style={{ color: ROSE }}>{value}<span className="text-sm text-slate-500 ml-1">{unit}</span></div>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 flex flex-col items-center">
            <div className="text-xs text-slate-500 uppercase tracking-widest mb-4">Score Global CRSV</div>
            <div className="w-32 h-32"><GaugeRing value={avgComposite} color={ROSE} /></div>
            <div className="mt-3 text-3xl font-bold" style={{ color: ROSE }}>{avgComposite.toFixed(1)}</div>
          </div>
        ...
        <div className="rounded-xl border p-5 text-sm" style={{ borderColor: ROSE + "40", backgroundColor: ROSE + "10" }}>
          <div className="font-semibold mb-1" style={{ color: ROSE }}>Alerte CPI</div>
          <p className="text-slate-300 text-xs leading-relaxed">
            4 contextes critiques : Soudan/RSF, RDC/M23, Myanmar et Russie-Ukraine font l&apos;objet d&apos;enquêtes CPI actives pour crimes sexuels
            utilisés comme armes de guerre. L&apos;impunité systémique reste le principal obstacle à la prévention.
          </p>
        </div>
      </div>
    </div>
