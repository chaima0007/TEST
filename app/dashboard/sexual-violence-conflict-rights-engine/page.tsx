...
interface Entity {
  ...
  last_updated: string;
  wartime_rape_prevalence_score?: number;
  survivor_support_gap_score?: number;
  perpetrator_impunity_score?: number;
  justice_mechanism_failure_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "wartime_rape_prevalence_score", label: "Prévalence du viol en temps de guerre" },
  { key: "survivor_support_gap_score", label: "Lacunes de soutien aux survivant·e·s" },
  { key: "perpetrator_impunity_score", label: "Impunité des auteurs" },
  { key: "justice_mechanism_failure_score", label: "Échec des mécanismes de justice" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des violences sexuelles en contexte de conflit armé : prévalence du viol en temps de guerre, lacunes de soutien aux survivant·e·s, impunité des auteurs et échec des mécanismes de justice.</p>
              <p className="text-slate-400">Source : Sexual Violence Conflict Rights Engine · Wave 246</p>
            </div>
          )}
        </div>
      </div>
...
export default function SexualViolenceConflictRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/sexual-violence-conflict-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Sexual Violence Conflict Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Viol en Temps de Guerre · Soutien aux Survivant·e·s · Impunité · Mécanismes de Justice</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#9f123920" } : {}}>
