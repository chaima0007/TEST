...
interface Entity {
  ...
  climate_vulnerability_displacement_score: number;
  fossil_fuel_subsidy_harm_score: number;
  indigenous_land_rights_score: number;
  adaptation_finance_denial_score: number;
  signals?: string[];
  context?: string;
}
...
interface ApiData {
  ...
  eleve_count: number;
  avg_estimated_climate_justice_index: number;
...
function DetailModal({ entity, onClose }: DetailModalProps) {
  ...
  const subScores = [
    { label: "Vulnérabilité Climatique", value: entity.climate_vulnerability_displacement_score },
    { label: "Subventions Fossiles", value: entity.fossil_fuel_subsidy_harm_score },
    { label: "Droits Indigènes", value: entity.indigenous_land_rights_score },
    { label: "Financement Adaptation", value: entity.adaptation_finance_denial_score },
  ];
...
export default function ClimateJusticeEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/climate-justice-engine")
      .then((r) => r.json())
  ...
  const kpis = [
    ...
    { label: "Index Moyen", value: data?.avg_estimated_climate_justice_index ?? 0, fmt: (v: number) => v.toFixed(1) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>
          Climate Justice Engine
        </h1>
        <p className="text-slate-400 text-sm mt-1">
          Analyse de la vulnérabilité climatique, droits indigènes et justice environnementale
        </p>
      </div>
      ...
      {!loading && data && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6 flex items-center gap-6">
          <GaugeRing value={data.avg_estimated_climate_justice_index ?? 0} color={ACCENT} />
          <div>
            <p className="text-slate-400 text-sm">Index Moyen Climate Justice</p>
            <p className="text-white text-3xl font-bold">{(data.avg_estimated_climate_justice_index ?? 0).toFixed(1)}</p>
          </div>
        </div>
      )}
