...
interface Entity {
  ...
  supply_chain_abuse_score: number;
  impunity_legal_gap_score: number;
  environmental_harm_score: number;
  greenwash_disclosure_fraud_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_corporate_accountability_index: number;
  last_updated: string;
  [key: string]: unknown;
}
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_corporate_accountability_index: number;
}
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/corporate-accountability-engine").then(r => r.json()).then(j => setData(j.data ?? j)).catch(console.error);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Corporate Accountability Engine</h1>
        <p className="text-slate-400 mt-1">Principes Directeurs ONU · OCDE · Devoir de Vigilance</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice CA</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_corporate_accountability_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { key: "supply_chain_abuse_score", label: "Abus Chaîne Appro." },
                      { key: "impunity_legal_gap_score", label: "Lacune Impunité" },
                      { key: "environmental_harm_score", label: "Préjudice Env." },
                      { key: "greenwash_disclosure_fraud_score", label: "Greenwash/Fraude" },
                    ].map(({ key, label }) => (
                      <div key={key}>
