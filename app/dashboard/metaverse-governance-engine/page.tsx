...
type MGEEntity = {
  ...
  risk_level: string;
  metaverse_pattern: string;
  severity: string;
  recommended_action: string;
  signal: string;
  platform_concentration: number;
  regulatory_gap: number;
};
...
type MGESummary = {
  ...
  avg_estimated_metaverse_governance_index: number;
};
...
const PATTERN_COLORS: Record<string, string> = {
  none: "#16a34a",
  platform_monopoly_capture: "#dc2626",
  virtual_labor_exploitation: "#7c3aed",
  identity_data_colonialism: "#0891b2",
  addiction_by_design_crisis: "#b45309",
  regulatory_jurisdiction_vacuum: "#059669",
};
const SEVERITY_COLORS: Record<string, string> = {
  "surveillance_métavers_active": "#16a34a",
  "déséquilibre_pouvoir_numérique_structurel": "#d97706",
  "crise_économie_virtuelle_majeure": "#9333ea",
  "crise_gouvernance_métavers_systémique": "#dc2626",
};
const ACTION_COLORS: Record<string, string> = {
  "veille_métavers_continue": "#16a34a",
  "renforcement_gouvernance_numérique_mondiale": "#d97706",
  "réforme_accélérée_économie_virtuelle_exploitée": "#9333ea",
  "intervention_urgente_régulation_métavers_critique": "#dc2626",
};
...
const SEV_BADGE: Record<string, string> = {
  "surveillance_métavers_active": "bg-green-900 text-green-300",
  "déséquilibre_pouvoir_numérique_structurel": "bg-amber-900 text-amber-300",
  "crise_économie_virtuelle_majeure": "bg-violet-900 text-violet-300",
  "crise_gouvernance_métavers_systémique": "bg-red-950 text-red-400",
};

function DetailModal({ entity, onClose }: { entity: MGEEntity; onClose: () => void }) {
      ...
      >
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ["Score Monopole",      entity.monopoly_score,     "#dc2626"],
              ["Score Exploitation",  entity.exploitation_score, "#7c3aed"],
              ["Score Identité",      entity.identity_score,     "#0891b2"],
              ["Score Addiction",     entity.addiction_score,    "#b45309"],
            ].map(([l, v, c]) => (
              <div
                key={String(l)}
            ...
            <div className="col-span-2 bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-400/60 text-xs mb-1">Score Composite Gouvernance Métavers</div>
          ...
          <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-4 text-sm text-slate-200 leading-relaxed">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="bg-slate-800 rounded p-2">
                <div className="text-violet-400/50 text-xs mb-0.5">Concentration Plateforme</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.platform_concentration * 100)}%
                </div>
              </div>
              <div className="bg-slate-800 rounded p-2">
                <div className="text-violet-400/50 text-xs mb-0.5">Vide Réglementaire</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.regulatory_gap * 100)}%
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-400/60 text-xs mb-1">Type de Plateforme</div>
            ...
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              ...
              <div className="text-purple-400 font-medium capitalize">
                {entity.metaverse_pattern.replace(/_/g, " ")}
              </div>
            </div>
          </div>
        )}
...
export default function MetaverseGovernanceDashboard() {
  ...
  const [selected, setSelected] = useState<MGEEntity | null>(null);

  useEffect(() => {
    fetch("/api/metaverse-governance-engine")
      .then(r => r.json())
      .then(setData)
      .catch(console.error);
  }, []);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-violet-400 text-lg animate-pulse">
          Initialisation du Moteur Gouvernance Métavers...
        </div>
      </div>
    );
  }
  ...
  const filtered = entities.filter(
    e =>
      ...
      (patFilter === "all" || e.metaverse_pattern === patFilter)
  );

  const dists = [
    { title: "Niveau de Risque",        counts: summary.risk_distribution,     colors: RISK_COLORS     },
    { title: "Patron Métavers",         counts: summary.pattern_distribution,  colors: PATTERN_COLORS  },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-violet-400">
          Gouvernance Métavers &amp; Économie Monde Virtuel — Module 400
        </h1>
        <p className="text-purple-500/60 text-sm mt-1">
          Monopole · Exploitation · Identité · Addiction — Caelum Partners — Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ["Total Plateformes",              summary.total,                                                                    "text-violet-400"],
          ["Crise Gouvernance",              summary.critical,                                                                 "text-red-400"],
          ["Crise Majeure",                  summary.high,                                                                     "text-purple-400"],
          ["Composite Moyen",                `${summary.avg_composite.toFixed(1)}`,                                            "text-violet-300"],
          ["Index Gouvernance Métavers",     `${summary.avg_estimated_metaverse_governance_index.toFixed(2)}/10`,              "text-violet-400"],
          ["Sous Surveillance",              summary.low,                                                                      "text-green-400"],
        ].map(([l, v, c]) => (
          <div
            key={String(l)}
      ...
      <div className="bg-slate-900 border border-violet-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          ...
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.identity_score, 0) / (entities.length || 1)}
            label="Identité Moy."
            color="#0891b2"
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.addiction_score, 0) / (entities.length || 1)}
            label="Addiction Moy."
            color="#b45309"
          />
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {[
          ...
          "virtual_labor_exploitation",
          "identity_data_colonialism",
          "addiction_by_design_crisis",
          "regulatory_jurisdiction_vacuum",
        ].map(p => (
          <button
            key={p}
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div className="text-xs text-purple-400/60 mb-2 capitalize">
              {e.metaverse_pattern.replace(/_/g, " ")}
            </div>
            <div className="text-xs text-violet-400/70 font-medium mb-2">
              Monopole: {e.monopoly_score.toFixed(1)} · Exploit.: {e.exploitation_score.toFixed(1)}
            </div>
