...
interface Entity {
  ...
  mercury_poisoning_prevalence_health_impact_score: number;
  illegal_mining_enforcement_gap_score: number;
  community_rights_displacement_score: number;
  minamata_convention_compliance_deficit_score: number;
  estimated_artisanal_gold_mercury_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_artisanal_gold_mercury_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Prévalence Intoxication Mercure & Santé", value: entity.mercury_poisoning_prevalence_health_impact_score, weight: "0.30" },
    { label: "Déficit Contrôle Mines Illégales", value: entity.illegal_mining_enforcement_gap_score, weight: "0.25" },
    { label: "Droits Communautés & Déplacements", value: entity.community_rights_displacement_score, weight: "0.25" },
    { label: "Déficit Conformité Convention Minamata", value: entity.minamata_convention_compliance_deficit_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_artisanal_gold_mercury_index === "number" ? entity.estimated_artisanal_gold_mercury_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Mercure Orpaillage</div>
                </div>
              </div>
...
export default function ArtisanalGoldMercuryRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/artisanal-gold-mercury-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits — Mercure dans l&apos;Orpaillage…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_artisanal_gold_mercury_index ?? avg(allEntities.map(e => e.estimated_artisanal_gold_mercury_index));
  ...
  const avgMercury = avg(allEntities.map(e => e.mercury_poisoning_prevalence_health_impact_score));
  const avgIllegal = avg(allEntities.map(e => e.illegal_mining_enforcement_gap_score));
  const avgCommunity = avg(allEntities.map(e => e.community_rights_displacement_score));
  const avgMinamata = avg(allEntities.map(e => e.minamata_convention_compliance_deficit_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Mercure Orpaillage", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits — Mercure dans l&apos;Orpaillage</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Artisanal Gold Mercury Rights Engine · Wave 281 — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgMercury} label="Intoxication Mercure" />
          <GaugeRing value={avgIllegal} label="Mines Illégales" />
          <GaugeRing value={avgCommunity} label="Droits Communautés" />
          <GaugeRing value={avgMinamata} label="Déficit Minamata" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Mercure Orpaillage : <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_artisanal_gold_mercury_index === "number" ? e.estimated_artisanal_gold_mercury_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
