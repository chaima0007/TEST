...
interface Entity { entity: string; risk_level: string; avg_composite: number; estimated_pharma_access_index: number; }
...
export default function PharmaceuticalAccessPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/pharmaceuaccess");
  ...
  return (
    <div className="p-6 space-y-6 text-slate-100">
      <div>
        <h1 className="text-2xl font-bold text-white">Accès aux Médicaments Essentiels</h1>
        <p className="text-slate-400 text-sm mt-1">Monopoles pharmaceutiques, brevets abusifs et droits à la santé</p>
      </div>
        ...
        {data.entities.map((e) => (
          <div key={e.entity} className={`rounded-xl border p-4 ${RISK_COLORS[e.risk_level] ?? "text-slate-400 bg-slate-900 border-slate-800"}`}>
            <div className="w-20 h-20 mx-auto mb-3">
              <GaugeRing value={e.avg_composite} color={e.risk_level === "critique" ? "#ef4444" : e.risk_level === "élevé" ? "#f97316" : e.risk_level === "modéré" ? "#eab308" : "#22c55e"} />
            </div>
