...
interface Entity {
  ...
  last_updated: string;
  illicit_finance_human_rights_score?: number;
  asset_recovery_failure_score?: number;
  offshore_secrecy_score?: number;
  beneficial_ownership_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "illicit_finance_human_rights_score", label: "Financement illicite & droits humains" },
  { key: "asset_recovery_failure_score", label: "Échec de récupération d&apos;actifs" },
  { key: "offshore_secrecy_score", label: "Opacité des structures offshore" },
  { key: "beneficial_ownership_gap_score", label: "Lacunes de propriété effective" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des violations liées au blanchiment d&apos;argent et aux droits humains : financement illicite, échec de récupération d&apos;actifs, opacité offshore et lacunes de propriété effective.</p>
              <p className="text-slate-400">Source : Anti-Money Laundering Rights Engine · Wave 246</p>
            </div>
          )}
        </div>
      </div>
...
export default function AntiMoneyLaunderingRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/anti-money-laundering-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Anti-Money Laundering Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Financement Illicite · Récupération d&apos;Actifs · Opacité Offshore · Propriété Effective</p>
        </div>
