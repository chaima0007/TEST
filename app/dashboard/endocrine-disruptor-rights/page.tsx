...
interface Entity {
  ...
  last_updated: string;
  chemical_exposure_score?: number;
  hormonal_disruption_score?: number;
  regulatory_protection_score?: number;
...
const SUB_SCORES = [
  { key: "chemical_exposure_score", label: "Exposition chimique" },
  { key: "hormonal_disruption_score", label: "Perturbation hormonale" },
  { key: "regulatory_protection_score", label: "Protection réglementaire" },
  { key: "vulnerable_population_score", label: "Populations vulnérables" },
];
        ...
        <div className="flex border-b border-slate-800">
          {(["overview", "scores", "context"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === "overview" ? "Analyse" : t === "scores" ? "Entités" : "Méthodologie"}
            </button>
          ))}
        </div>
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits contre les perturbateurs endocriniens : exposition aux produits chimiques perturbateurs, dérèglements hormonaux, lacunes réglementaires et impacts sur les populations vulnérables (femmes enceintes, enfants).</p>
              <p className="text-slate-400">Source : Endocrine Disruptor Rights Engine · Wave 264</p>
            </div>
          )}
        </div>
      </div>
...
export default function EndocrineDisruptorRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/endocrine-disruptor-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits contre les Perturbateurs Endocriniens</h1>
          <p className="text-slate-400 mt-1 text-sm">Exposition Chimique · Perturbation Hormonale · Protection Réglementaire · Populations Vulnérables</p>
        </div>
