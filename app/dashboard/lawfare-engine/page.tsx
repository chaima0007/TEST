...
interface Entity {
  ...
  international_court_manipulation_score: number;
  sanctions_legal_weaponization_score: number;
  extraterritoriality_abuse_score: number;
  treaty_weaponization_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_lawfare_index: number;
  last_updated: string;
}
...
interface Data {
  ...
  avg_estimated_lawfare_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Manipulation Cours Internationales", v: entity.international_court_manipulation_score, c: "#6366f1" },
              { l: "Weaponisation Légale Sanctions", v: entity.sanctions_legal_weaponization_score, c: "#818cf8" },
              { l: "Abus Extraterritorialité", v: entity.extraterritoriality_abuse_score, c: "#4f46e5" },
              { l: "Weaponisation Traités", v: entity.treaty_weaponization_score, c: "#7c3aed" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Lawfare Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_lawfare_index}</span>
            </div>
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Activation immédiate des protocoles de contre-lawfare et mobilisation des équipes juridiques internationales</p>
                <p>› Saisine urgente des instances internationales compétentes et coordination multilatérale de riposte légale</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Renforcement de la veille juridique et préparation des dossiers de défense anticipée</p>
                <p>› Consultation des experts en droit international et cartographie des vecteurs d'attaque potentiels</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance accrue des procédures judiciaires internationales et analyse des précédents juridiques</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur les évolutions du droit international et les positions doctrinales des acteurs clés</p>
            )}
          </div>
        )}
      </div>
...
export default function LawfareEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/lawfare-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données Lawfare Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la weaponisation juridique…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Score Moyen", v: `${(data.avg_composite ?? 0).toFixed(1)}`, c: "text-indigo-400" },
    { l: "Index Lawfare Moyen", v: `${(data.avg_estimated_lawfare_index ?? 0).toFixed(2)}`, c: "text-indigo-300" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Lawfare Engine</h1>
          <p className="text-slate-400 text-sm mt-1">Weaponisation du droit international comme instrument de puissance géopolitique</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {kpis.map(({ l, v, c }) => (
            <div key={l} className="bg-slate-900 border border-indigo-500/20 rounded-xl p-4 text-center">
        ...
        <div className="bg-slate-900 border border-indigo-500/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Lawfare Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("international_court_manipulation_score")} label="Manipulation Cours Int." color={ACCENT} />
            <GaugeRing value={avgScore("sanctions_legal_weaponization_score")} label="Weaponisation Sanctions" color="#818cf8" />
            <GaugeRing value={avgScore("extraterritoriality_abuse_score")} label="Abus Extraterritorialité" color="#4f46e5" />
            <GaugeRing value={avgScore("treaty_weaponization_score")} label="Weaponisation Traités" color="#7c3aed" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-indigo-500/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns Lawfare</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#6366f1", "#818cf8", "#4f46e5", "#7c3aed", "#a78bfa"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_lawfare_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Lawfare Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
