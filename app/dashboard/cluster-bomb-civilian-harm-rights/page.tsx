...
interface Entity {
  ...
  risk_level: string;
  estimated_cluster_bomb_harm_index: number;
}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <h2 style={{ color: "#fff", fontSize: 20, fontWeight: 700 }}>Détails — Bombes à Sous-Munitions</h2>
        ...
        {tab === "analyse" && (
          <div style={{ color: "#cbd5e1", lineHeight: 1.7 }}>
            <p style={{ marginBottom: 12 }}>
              Les bombes à sous-munitions causent des dommages civils massifs et durables, souvent
              des décennies après les conflits armés. Les survivants et communautés affectées font
              face à des obstacles persistants pour accéder à la justice et à l&apos;indemnisation.
            </p>
            <p style={{ marginBottom: 12 }}>
              Score composite moyen : <strong style={{ color: "#fff" }}>{data.avg_composite.toFixed(2)}</strong> — niveau critique
              indiquant une protection insuffisante des droits des victimes civiles dans les zones touchées.
            </p>
            <p>
              La Convention d&apos;Oslo de 2008 interdit ces armes mais de nombreux États ne l&apos;ont pas
              ratifiée, laissant des millions de civils exposés à des restes explosifs de guerre.
            </p>
          </div>
        )}
                ...
                <tbody>
                  {data.entities.map((e) => (
                    <tr key={e.entity} style={{ borderBottom: "1px solid #0f172a" }}>
                      ...
                      <td style={{ color: "#f1f5f9", textAlign: "right", padding: "8px 4px" }}>{e.estimated_cluster_bomb_harm_index.toFixed(2)}</td>
        ...
        {tab === "methodologie" && (
          <div style={{ color: "#cbd5e1", lineHeight: 1.7 }}>
            <p style={{ marginBottom: 12 }}>
              <strong style={{ color: "#fff" }}>Moteur :</strong> cluster_bomb_civilian_harm_rights_engine
            </p>
            <p style={{ marginBottom: 12 }}>
              <strong style={{ color: "#fff" }}>Sous-scores :</strong> Contamination territoriale (×0.30) ·
              Accès à la justice (×0.25) · Indemnisation des victimes (×0.25) · Déminage effectif (×0.20)
            </p>
            <p style={{ marginBottom: 12 }}>
...
export default function ClusterBombCivilianHarmRightsDashboard() {
  ...
  useEffect(() => {
    fetch("/api/cluster-bomb-civilian-harm-rights")
      .then((r) => r.json())
      .then((d) => {
        setData(d.payload ?? d);
        setLoading(false);
      ...
      <div style={{ maxWidth: 1024, margin: "0 auto" }}>
        ...
        <div style={{ borderLeft: `4px solid ${ACCENT}`, paddingLeft: 24, marginBottom: 32 }}>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: "#fff", margin: 0 }}>
            Droits des Victimes de Bombes à Sous-Munitions
          </h1>
          <p style={{ color: "#94a3b8", marginTop: 8 }}>
            Droit Humanitaire · Victimes Civiles · Déminage · Convention d&apos;Oslo
          </p>
        </div>
        ...
        <div style={{ color: "#475569", fontSize: 12, textAlign: "right" }}>
          Moteur : cluster_bomb_civilian_harm_rights_engine · Wave 278 · Caelum Partners
        </div>
      </div>
    </div>
  );
