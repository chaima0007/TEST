...
interface Entity {
  ...
  corporate_human_rights_abuse_impunity_severity_score: number;
  supply_chain_forced_labor_exploitation_scale_score: number;
  environmental_corporate_destruction_impunity_score: number;
  remedy_access_corporate_victims_gap_score: number;
  estimated_corporate_accountability_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_corporate_accountability_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Impunité Violations Corp.", value: entity.corporate_human_rights_abuse_impunity_severity_score, weight: "0.30" },
    { label: "Travail Forcé Supply Chain", value: entity.supply_chain_forced_labor_exploitation_scale_score, weight: "0.25" },
    { label: "Destruction Environnementale", value: entity.environmental_corporate_destruction_impunity_score, weight: "0.25" },
    { label: "Accès aux Recours Victimes", value: entity.remedy_access_corporate_victims_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_corporate_accountability_rights_index === "number" ? entity.estimated_corporate_accountability_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Responsabilité Corp.</div>
                </div>
              </div>
...
export default function CorporateAccountabilityRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/corporate-accountability-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Responsabilité Corporative…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_corporate_accountability_rights_index ?? avg(allEntities.map(e => e.estimated_corporate_accountability_rights_index));
  ...
  const avgImpunity = avg(allEntities.map(e => e.corporate_human_rights_abuse_impunity_severity_score));
  const avgForcedLabor = avg(allEntities.map(e => e.supply_chain_forced_labor_exploitation_scale_score));
  const avgEnvDestruction = avg(allEntities.map(e => e.environmental_corporate_destruction_impunity_score));
  const avgRemedy = avg(allEntities.map(e => e.remedy_access_corporate_victims_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Responsabilité Corp.", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Corporate Accountability Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Impunité corporative, travail forcé supply chain et lacunes d&apos;accès aux recours — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgImpunity} label="Impunité Corp." />
          <GaugeRing value={avgForcedLabor} label="Travail Forcé SC" />
          <GaugeRing value={avgEnvDestruction} label="Destruction Env." />
          <GaugeRing value={avgRemedy} label="Accès Recours" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Responsabilité Corp.: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_corporate_accountability_rights_index === "number" ? e.estimated_corporate_accountability_rights_index.toFixed(2) : "—"}</span>
            </div>
