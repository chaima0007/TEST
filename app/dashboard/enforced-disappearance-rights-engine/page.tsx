...
export default function EnforcedDisappearanceRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/enforced-disappearance-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Disparitions Forcées &amp; Droits</h1>
          <p className="text-slate-400 mt-1 text-sm">CPED · Disparitions Politiques · Impunité · ICMP</p>
        </div>
