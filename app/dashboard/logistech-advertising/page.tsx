      ...
      <div style={{ background: "#0f172a", border: "1px solid #334155", borderRadius: 16, width: "100%", maxWidth: 640, maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        <div style={{ padding: 24, borderBottom: "1px solid #1e293b", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "#94a3b8", marginTop: 2 }}>Logistech Advertising Intelligence</p>
          </div>
        ...
        <div style={{ padding: 24 }}>
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <p style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.6 }}>
                Cet acteur opère dans le secteur de la publicité logistech. Son score composite reflète
                l&apos;exposition aux risques liés aux pratiques publicitaires des plateformes logistiques
                technologiques, notamment la transparence tarifaire et l&apos;impact sur les travailleurs.
              </p>
            </div>
          )}
          {tab === "metriques" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                { label: "Transparence tarifaire & offres", value: entity.composite_score * 0.30 },
                { label: "Conformité réglementaire transport", value: entity.composite_score * 0.25 },
                { label: "Pratiques marketing éthique", value: entity.composite_score * 0.25 },
                { label: "Impact travailleurs logistiques", value: entity.composite_score * 0.20 },
              ].map(m => (
          ...
          {tab === "sources" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {["FTC — Federal Trade Commission (Logistics Advertising)", "IRU — International Road Transport Union", "ETF — European Transport Workers&apos; Federation", "CLECAT — European Logistics Association Standards"].map(s => (
                <div key={s} style={{ background: "#1e293b", borderRadius: 8, padding: 12 }}>
                  <p style={{ fontSize: 12, color: "#94a3b8" }} dangerouslySetInnerHTML={{ __html: s }} />
                </div>
              ))}
            </div>
          )}
...
export default function LogistechAdvertisingDashboard() {
  const [data, setData] = useState<DashData | null>(null);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<Entity | null>(null);

  useEffect(() => {
    fetch("/api/logistech-advertising")
      .then(r => r.json())
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#020617", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ color: "#94a3b8", fontSize: 14 }}>Chargement Logistech Advertising…</p>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020617", color: "white", padding: "32px 16px" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto 32px" }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 800, color: "white", marginBottom: 4 }}>Logistech Advertising Intelligence</h1>
        <p style={{ fontSize: 14, color: "#94a3b8" }}>
          Surveillance des pratiques publicitaires dans le secteur logistech — transparence tarifaire, conformité et impact travail
        </p>
      </div>
