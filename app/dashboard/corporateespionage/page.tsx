...
export default function CorporateEspionageDashboard() {
  const [data, setData] = useState<{ entities?: { entity: string; composite: number; level: string }[]; avg_composite?: number } | null>(null);
  useEffect(() => { fetch("/api/corporateespionage").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Corporate Espionage Rights</h1>
