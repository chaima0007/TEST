...
interface Entity {
  ...
  risk_level: string
  estimated_land_grab_rights_index: number
}
...
export default function AgribusinessLandGrabCommunityRightsPage() {
  const [data, setData] = useState<DomainData | null>(null)

  useEffect(() => {
    fetch("/api/agribusiness-land-grab-community-rights-engine")
      .then((r) => r.json())
  ...
  if (!data) {
    return (
      <div style={{ background: "#0f172a", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: ACCENT, fontSize: 18 }}>Chargement agrobusiness et droits communautaires…</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ background: "#0f172a", minHeight: "100vh", padding: "32px 24px", fontFamily: "system-ui, sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "#f1f5f9" }}>
              Agrobusiness, Accaparement Terres &amp; Droits Communautaires
            </h1>
            <p style={{ margin: 0, color: "#94a3b8", fontSize: 14 }}>
              Wave 174 — Indice mondial des droits humains · Caelum Partners
            </p>
          </div>
        </div>
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", gap: 16 }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: ACCENT }}>{entity.estimated_land_grab_rights_index.toFixed(2)}</div>
                  <div style={{ fontSize: 11, color: "#64748b" }}>Indice terres</div>
                </div>
                <div>
      ...
      <div style={{ maxWidth: 1100, margin: "40px auto 0", textAlign: "center", color: "#334155", fontSize: 12 }}>
        Dernière mise à jour : {new Date(data.generated_at).toLocaleString("fr-FR")} · Caelum Partners Wave 174
      </div>
    </div>
  )
}
