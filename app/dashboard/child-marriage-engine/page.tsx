...
interface MarriageEntity {
  ...
  legal_protection_gap_score: number;
  education_gap_score: number;
  socioeconomic_pressure_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_marriage_index: number;
  last_updated: string;
}
...
interface MarriageSummary {
  ...
  data_sources: string[];
  entities: MarriageEntity[];
  avg_estimated_marriage_index: number;
}
...
function EntityCard({ entity, onClick }: { entity: MarriageEntity; onClick: (e: MarriageEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.prevalence_score}</p>
          <p className="text-[10px] text-gray-500">Prévalence</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.education_gap_score}</p>
          <p className="text-[10px] text-gray-500">Éducation</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.legal_protection_gap_score}</p>
...
function DetailModal({ entity, onClose }: { entity: MarriageEntity; onClose: () => void }) {
  ...
  const actions: Record<string, string> = {
    "Prévalence Mariage Infantile Critique": "Déployer des équipes d'intervention d'urgence et alerter l'UNICEF sous 24h.",
    "Défaillance Législative Grave": "Engager des réformes législatives d'urgence avec le soutien des Nations Unies.",
    "Exclusion Scolaire Féminine": "Lancer des programmes de scolarisation d'urgence et des bourses de maintien scolaire.",
    "Pression Socio-Économique Familiale": "Déployer des transferts monétaires conditionnels et renforcer les filets de sécurité sociale.",
    "Risque Norme Culturelle Persistante": "Engager les leaders communautaires dans des programmes de sensibilisation et de changement normatif.",
  };
          ...
          {tab === "scores" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Prévalence Mariage", value: `${entity.prevalence_score}/100` },
                  { label: "Défaillance Législative", value: `${entity.legal_protection_gap_score}/100` },
                  { label: "Écart Éducation", value: `${entity.education_gap_score}/100` },
                  { label: "Pression Socio-Éco", value: `${entity.socioeconomic_pressure_score}/100` },
                  { label: "Indice Mariage", value: `${entity.estimated_marriage_index}/10` },
              ...
              <div className="bg-white/4 rounded-lg p-4">
                ...
                <p className="text-[11px] text-gray-500 mt-1.5">
                  Formule : prévalence×0.30 + légal×0.25 + éducation×0.25 + socio-éco×0.20
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "actions" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-300 leading-relaxed">{actions[entity.primary_pattern] ?? "Renforcer la protection légale et les programmes d'éducation des filles."}</p>
              </div>
...
export default function ChildMarriagePage() {
  ...
  const [selected, setSelected] = useState<MarriageEntity | null>(null);

  useEffect(() => {
    fetch("/api/child-marriage-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as MarriageData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Intelligence Mariage Infantile</h1>
            <p className="text-sm text-gray-400 mt-1">
              Moteur d&apos;intelligence Swarm — surveillance et prévention du mariage précoce et forcé
            </p>
          </div>
          <span className="text-xs text-gray-500 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5">
            {total} régions analysées
          </span>
        </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              <KpiCard label="Total Régions" value={s.total_entities} sub="zones analysées" />
              ...
              <KpiCard label="Indice Mariage Infantile" value={`${s.avg_estimated_marriage_index}/10`} sub="score normalisé" accent="text-pink-400" />
              <KpiCard label="Confiance Analyse" value={`${s.confidence_score}%`} sub="fiabilité données" accent="text-indigo-400" />
              <KpiCard label="Sources Données" value={s.data_sources.length} sub="organismes ONU/ONG" accent="text-violet-400" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Protection Enfance</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_marriage_index} max={10} label="Indice Mariage" color="#ec4899" />
                </div>
              </div>
              ...
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                ...
                <div className="mt-5 pt-4 border-t border-white/8 grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-white/4 rounded-lg p-3">
                    ...
                    <p className="text-lg font-bold text-pink-400 mt-0.5">{s.engine_version}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              ...
              {filtered.length === 0 && (
                <div className="col-span-full text-center py-12 text-gray-400 text-sm">
                  Aucune région dans ce niveau de risque
                </div>
              )}
            </div>
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.critical_alerts} région{s.critical_alerts > 1 ? "s" : ""} en situation critique de mariage infantile
                  </p>
