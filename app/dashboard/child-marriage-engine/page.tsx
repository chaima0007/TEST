...
interface ChildMarriageEntity {
  entity_id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  prevalence_underage_unions_score: number;
  girls_education_dropout_score: number;
  health_maternal_mortality_score: number;
  legal_enforcement_gap_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_child_marriage_index: number;
  last_updated: string;
}

interface ChildMarriageSummary {
  ...
  data_sources: string[];
  avg_estimated_child_marriage_index: number;
}

interface ChildMarriageData {
  entities: ChildMarriageEntity[];
  summary: ChildMarriageSummary;
}
...
function EntityCard({ entity, onClick }: { entity: ChildMarriageEntity; onClick: (e: ChildMarriageEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.prevalence_underage_unions_score}</p>
          <p className="text-[10px] text-gray-500">Prévalence</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.girls_education_dropout_score}</p>
          <p className="text-[10px] text-gray-500">Éducation</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.legal_enforcement_gap_score}</p>
...
function DetailModal({ entity, onClose }: { entity: ChildMarriageEntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Prévalence Unions Mineures", value: `${entity.prevalence_underage_unions_score}/100` },
                  { label: "Décrochage Scolaire Filles", value: `${entity.girls_education_dropout_score}/100` },
                  { label: "Mortalité Maternelle", value: `${entity.health_maternal_mortality_score}/100` },
                  { label: "Lacune Application Légale", value: `${entity.legal_enforcement_gap_score}/100` },
                  { label: "Index Mariages Enfants", value: `${entity.estimated_child_marriage_index}/10` },
              ...
              <div className="bg-white/4 rounded-lg p-4">
                ...
                <p className="text-[11px] text-gray-500 mt-1.5">
                  Formule : prévalence×0.30 + éducation×0.25 + santé×0.25 + légal×0.20
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "contexte" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 mt-1"><span className="text-gray-300 font-medium">Index Mariages Enfants :</span> {entity.estimated_child_marriage_index}/10</p>
              </div>
              <div className={`rounded-xl border p-4 ${cfg.bg} ${cfg.border}`}>
                <p className={`text-sm font-semibold ${cfg.color}`}>{entity.primary_pattern}</p>
                <p className="text-xs text-gray-400 mt-2">Renforcer la protection légale des mineures, garantir l&apos;accès à l&apos;éducation des filles et réduire la mortalité maternelle liée aux grossesses précoces.</p>
              </div>
            </div>
          )}
        </div>
...
export default function ChildMarriageEnginePage() {
  const [data, setData] = useState<ChildMarriageData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<ChildMarriageEntity | null>(null);
  ...
  useEffect(() => {
    fetch("/api/child-marriage-engine")
      ...
      .then((json) => {
        ...
        setData(payload as ChildMarriageData);
        setLoading(false);
      })
  ...
  return (
    <div className="bg-slate-950 min-h-screen text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Mariages d&apos;Enfants</h1>
            <p className="text-sm text-gray-400 mt-1">
              Moteur de surveillance des unions précoces et forcées impliquant des mineurs
            </p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Index Mariages Enfants" value={`${s.avg_estimated_child_marriage_index ?? "—"}/10`} sub="child marriage index" accent="text-pink-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Mariage d&apos;Enfants</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_child_marriage_index ?? 0} max={10} label="Index Mariage" color={ACCENT} />
                </div>
              </div>
            ...
            {s.critical_alerts > 0 && (
              <div className="rounded-xl px-5 py-4 flex items-start gap-3" style={{ backgroundColor: "rgba(157,23,77,0.08)", border: "1px solid rgba(157,23,77,0.3)" }}>
                <div className="w-2 h-2 rounded-full mt-1.5 shrink-0 animate-pulse" style={{ backgroundColor: ACCENT }} />
                <div>
                  <p className="text-sm font-medium" style={{ color: ACCENT }}>
                    {s.critical_alerts} entité{s.critical_alerts > 1 ? "s" : ""} en situation critique de mariage d&apos;enfants
                  </p>
                  <p className="text-xs mt-0.5" style={{ color: `${ACCENT}b3` }}>
                    Intervention immédiate requise — protection de l&apos;enfance et des droits des mineures prioritaire
                  </p>
                </div>
              </div>
            )}
