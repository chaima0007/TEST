...
interface Entity {
  ...
  last_updated: string;
  marine_protection_score?: number;
  fishing_rights_score?: number;
...
const SUB_SCORES = [
  { key: "marine_protection_score", label: "Protection marine" },
  { key: "fishing_rights_score", label: "Droits de pêche" },
  { key: "pollution_score", label: "Pollution océanique" },
  { key: "biodiversity_score", label: "Biodiversité marine" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits océaniques : protection marine, droits de pêche, pollution océanique et biodiversité marine affectant les écosystèmes et communautés côtières.</p>
              <p className="text-slate-400">Source : Ocean Rights Engine · Wave 265</p>
            </div>
          )}
        </div>
      </div>
...
export default function OceanRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/ocean-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits Océaniques</h1>
          <p className="text-slate-400 mt-1 text-sm">Protection Marine · Droits de Pêche · Pollution Océanique · Biodiversité Marine</p>
        </div>
