...
export default function StatelessnessCitizenshipDeprivationPage() {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  useEffect(() => {
    fetch("/api/statelessness-citizenship-deprivation-engine")
      .then((r) => r.json())
  ...
  const avgIndex = (data.avg_estimated_statelessness_citizenship_deprivation_index as number) ?? 0;
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-6">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: VIOLET }}>
            Apatridie &amp; Privation de Nationalité
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            UNHCR Tendances Mondiales · Institut Apatridie · Convention 1954 · HRW 2022
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Avg Composite", value: avgComposite.toFixed(2), unit: "/100" },
            { label: "Indice Apatridie", value: avgIndex.toFixed(2), unit: "/10" },
            ...
            <div key={label} className="rounded-xl bg-slate-900 border border-slate-800 p-4 text-center">
              ...
              <div className="text-2xl font-bold" style={{ color: VIOLET }}>{value}<span className="text-sm text-slate-500 ml-1">{unit}</span></div>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 flex flex-col items-center">
            <div className="text-xs text-slate-500 uppercase tracking-widest mb-4">Score Global Apatridie</div>
            <div className="w-32 h-32"><GaugeRing value={avgComposite} color={VIOLET} /></div>
            <div className="mt-3 text-3xl font-bold" style={{ color: VIOLET }}>{avgComposite.toFixed(1)}</div>
          </div>
        ...
        {entities.length > 0 && (
          <div className="rounded-xl bg-slate-900 border border-slate-800 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-800">
              <div className="text-xs text-slate-500 uppercase tracking-widest">Populations analysées</div>
            </div>
            <div className="divide-y divide-slate-800">
              {entities.map((e) => {
                ...
                const idx = e.estimated_statelessness_citizenship_deprivation_index as number;
                return (
        ...
        <div className="rounded-xl border p-5" style={{ borderColor: VIOLET + "40", backgroundColor: VIOLET + "10" }}>
          <div className="text-xs font-semibold mb-1" style={{ color: VIOLET }}>UNHCR 2023 — Crise Mondiale Apatridie</div>
          <p className="text-xs text-slate-300 leading-relaxed">
            Plus de 4,4 millions d&apos;apatrides recensés dans 94 pays. Le chiffre réel est estimé à
            10–15 millions. Les Rohingyas représentent la plus grande population apatride au monde.
            L&apos;apatridie prive les individus de tout accès à l&apos;éducation, la santé et la justice.
          </p>
        </div>
      </div>
    </div>
