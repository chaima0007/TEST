...
interface MHREntity {
  ...
  forced_institutionalization_score: number;
  treatment_access_denial_score: number;
  legal_capacity_deprivation_score: number;
  stigma_discrimination_barrier_score: number;
  risk_level: string;
...
function DetailModal({ entity, onClose }: { entity: MHREntity; onClose: () => void }) {
      ...
      <div className="w-full max-w-lg rounded-xl border border-slate-700/50 bg-slate-900 p-6 shadow-2xl">
        ...
        {tab === 0 && (
          <div className="space-y-3">
            {[
              { label: "Institutionnalisation Forcée", value: entity.forced_institutionalization_score },
              { label: "Accès Traitement", value: entity.treatment_access_denial_score },
              { label: "Capacité Juridique", value: entity.legal_capacity_deprivation_score },
              { label: "Stigmatisation", value: entity.stigma_discrimination_barrier_score },
            ].map((s) => (
        ...
        {tab === 1 && (
          <div className="flex flex-col items-center gap-4 py-4">
            <GaugeRing value={entity.estimated_mental_health_rights_index} max={10} />
            <div className="text-center">
              <div className="text-3xl font-bold text-indigo-400">
                {typeof entity.estimated_mental_health_rights_index === "number"
                  ? entity.estimated_mental_health_rights_index.toFixed(1)
                  : entity.estimated_mental_health_rights_index}
              </div>
              <div className="text-sm text-slate-500">Index Santé Mentale</div>
            </div>
          </div>
        )}
        {tab === 2 && (
          <div className="rounded-lg border border-slate-700/40 bg-slate-800/50 p-4">
            <p className="text-sm leading-relaxed text-slate-400">
              Analyse des violations des droits en santé mentale pour {entity.name}. Indice global&nbsp;:&nbsp;
              <span className="text-slate-200 font-medium">
                {typeof entity.estimated_mental_health_rights_index === "number"
                  ? entity.estimated_mental_health_rights_index.toFixed(2)
                  : entity.estimated_mental_health_rights_index}
              </span>
...
export default function MentalHealthRightsEngine() {
  const [data, setData] = useState<ApiData>({});
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<MHREntity | null>(null);
  ...
  const filters = ["tous", "forced_institutionalization", "treatment_access_denial", "legal_capacity_deprivation", "stigma_discrimination_barrier"];
  const entities: MHREntity[] = (data.entities ?? []).filter(
    ...
    <div className="min-h-screen bg-slate-950 p-6 text-slate-100">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-100">Mental Health Rights Engine</h1>
            <p className="mt-1 text-sm text-slate-500">Surveillance des violations des droits en santé mentale — institutionnalisation forcée &amp; capacité juridique</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {(kpis.length > 0 ? kpis : [
            { label: "Institutionnalisation Forcée", value: "—" },
            { label: "Accès Traitement", value: "—" },
            { label: "Capacité Juridique", value: "—" },
            { label: "Stigmatisation", value: "—" },
            { label: "Index Santé Mentale Moyen", value: "—" },
            ...
            {entities.map((entity) => (
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="text-xs text-slate-500">Index Santé Mentale</span>
                ...
                <div className="mt-2 space-y-1">
                  {[
                    { key: "forced_institutionalization_score", label: "Institutionnalisation Forcée" },
                    { key: "treatment_access_denial_score", label: "Accès Traitement" },
                    { key: "legal_capacity_deprivation_score", label: "Capacité Juridique" },
                    { key: "stigma_discrimination_barrier_score", label: "Stigmatisation" },
                  ].map(({ key, label }) => (
