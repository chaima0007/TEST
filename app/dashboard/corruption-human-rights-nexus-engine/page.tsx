...
export default function CorruptionHumanRightsNexusPage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#78350f";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", "modéré": "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/corruption-human-rights-nexus-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_corruption_human_rights_nexus_index"] as number) ?? 0;
