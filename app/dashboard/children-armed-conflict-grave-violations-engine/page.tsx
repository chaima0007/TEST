...
export default function ChildrenArmedConflictGraveViolationsPage() {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  useEffect(() => {
    fetch("/api/children-armed-conflict-grave-violations-engine")
      .then((r) => r.json())
  ...
  const avgIndex = (data.avg_estimated_children_conflict_index as number) ?? 0;
  ...
  const violations = [
    { label: "Meurtre & mutilation", icon: "⚠" },
    { label: "Recrutement enfants soldats", icon: "⚠" },
    { label: "Violences sexuelles", icon: "⚠" },
    { label: "Enlèvements", icon: "⚠" },
    { label: "Attaques écoles & hôpitaux", icon: "⚠" },
    { label: "Déni d&apos;aide humanitaire", icon: "⚠" },
  ];
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-6">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: AMBER }}>
            Enfants en Conflits Armés — Violations Graves
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            UN MRM · 6 Violations Graves ONU · UNICEF · Child Soldiers International
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Avg Composite", value: avgComposite.toFixed(2), unit: "/100" },
            { label: "Indice Conflit", value: avgIndex.toFixed(2), unit: "/10" },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 flex flex-col items-center">
            <div className="text-xs text-slate-500 uppercase tracking-widest mb-4">Score Global Violations</div>
          ...
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-6">
            <div className="text-xs text-slate-500 uppercase tracking-widest mb-4">6 Violations Graves ONU</div>
            <div className="space-y-2">
              {violations.map((v) => (
                <div key={v.label} className="flex items-center gap-2 text-xs text-slate-400">
                  <span style={{ color: AMBER }}>{v.icon}</span>
        ...
        {entities.length > 0 && (
          <div className="rounded-xl bg-slate-900 border border-slate-800 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-800">
              <div className="text-xs text-slate-500 uppercase tracking-widest">Contextes de conflit analysés</div>
            </div>
            <div className="divide-y divide-slate-800">
              {entities.map((e) => {
                ...
                const idx = e.estimated_children_conflict_index as number;
                return (
