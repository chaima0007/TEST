...
interface Entity {
  ...
  refugee_protection_failure_score: number;
  pushback_refoulement_score: number;
  detention_conditions_score: number;
  integration_rights_denial_score: number;
  signals?: string[];
  context?: string;
}
...
interface ApiData {
  ...
  eleve_count: number;
  avg_estimated_refugee_rights_index: number;
...
function DetailModal({ entity, onClose }: DetailModalProps) {
  ...
  const subScores = [
    { label: "Défaillance Protection", value: entity.refugee_protection_failure_score },
    { label: "Push-back/Refoulement", value: entity.pushback_refoulement_score },
    { label: "Conditions Détention", value: entity.detention_conditions_score },
    { label: "Déni Intégration", value: entity.integration_rights_denial_score },
  ];
...
export default function RefugeeRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/refugee-rights-engine")
      .then((r) => r.json())
  ...
  const kpis = [
    ...
    { label: "Index Moyen", value: data?.avg_estimated_refugee_rights_index ?? 0, fmt: (v: number) => v.toFixed(1) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>
          Refugee Rights Engine
        </h1>
        <p className="text-slate-400 text-sm mt-1">
          Analyse des défaillances de protection des réfugiés, du refoulement et des conditions de détention
        </p>
      </div>
      ...
      {!loading && data && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6 flex items-center gap-6">
          <GaugeRing value={data.avg_estimated_refugee_rights_index ?? 0} color={ACCENT} />
          <div>
            <p className="text-slate-400 text-sm">Index Moyen Refugee Rights</p>
            <p className="text-white text-3xl font-bold">{(data.avg_estimated_refugee_rights_index ?? 0).toFixed(1)}</p>
          </div>
        </div>
      )}
