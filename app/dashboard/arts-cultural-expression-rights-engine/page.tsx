...
interface Entity {
  ...
  artist_censorship_imprisonment_severity_score: number;
  cultural_institution_state_capture_scale_score: number;
  online_creative_content_suppression_score: number;
  artistic_minority_voice_exclusion_deficit_gap_score: number;
  estimated_arts_cultural_expression_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_arts_cultural_expression_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Censure & Emprisonnement Artistes", value: entity.artist_censorship_imprisonment_severity_score, weight: "0.30" },
    { label: "Capture Étatique Institutions Culturelles", value: entity.cultural_institution_state_capture_scale_score, weight: "0.25" },
    { label: "Suppression Contenu Créatif En Ligne", value: entity.online_creative_content_suppression_score, weight: "0.25" },
    { label: "Déficit Voix Minorités Artistiques", value: entity.artistic_minority_voice_exclusion_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_arts_cultural_expression_rights_index === "number" ? entity.estimated_arts_cultural_expression_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Expression Artistique</div>
                </div>
              </div>
...
export default function ArtsCulturalExpressionRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/arts-cultural-expression-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Expression Artistique & Droits Culturels…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_arts_cultural_expression_rights_index ?? avg(allEntities.map(e => e.estimated_arts_cultural_expression_rights_index));
  ...
  const avgCensorship = avg(allEntities.map(e => e.artist_censorship_imprisonment_severity_score));
  const avgCapture = avg(allEntities.map(e => e.cultural_institution_state_capture_scale_score));
  const avgOnline = avg(allEntities.map(e => e.online_creative_content_suppression_score));
  const avgMinority = avg(allEntities.map(e => e.artistic_minority_voice_exclusion_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Expression Artistique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Expression Artistique & Droits Culturels</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Arts Cultural Expression Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgCensorship} label="Censure Artistes" />
          <GaugeRing value={avgCapture} label="Capture Institutions" />
          <GaugeRing value={avgOnline} label="Suppression En Ligne" />
          <GaugeRing value={avgMinority} label="Voix Minorités" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Expression Artistique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_arts_cultural_expression_rights_index === "number" ? e.estimated_arts_cultural_expression_rights_index.toFixed(2) : "—"}</span>
            </div>
