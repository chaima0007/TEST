...
interface AEVEntity {
  ...
  embargo_breach_frequency_score: number;
  state_complicity_level_score: number;
  civil_war_fueling_score: number;
  accountability_enforcement_failure_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_arms_embargo_violation_index: number;
  last_updated: string;
}

interface AEVSummary {
  ...
  data_sources: string[];
  entities: AEVEntity[];
  avg_estimated_arms_embargo_violation_index: number;
}
...
function EntityCard({ entity, onClick }: { entity: AEVEntity; onClick: (e: AEVEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.embargo_breach_frequency_score}</p>
          <p className="text-[10px] text-gray-500">Violations</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.state_complicity_level_score}</p>
          <p className="text-[10px] text-gray-500">Complicité État</p>
        </div>
      </div>
      <div className="flex items-center justify-between text-xs">
        <span className="text-gray-400">Score: <span className={`font-bold ${rc}`}>{entity.composite_score}</span></span>
        <span className="text-gray-500 truncate ml-2">AEV: {entity.estimated_arms_embargo_violation_index}/10</span>
      </div>
    </button>
  );
}

function DetailModal({ entity, onClose }: { entity: AEVEntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Fréquence Violations Embargo", value: `${entity.embargo_breach_frequency_score}/100` },
                  { label: "Complicité État", value: `${entity.state_complicity_level_score}/100` },
                  { label: "Alimentation Guerre Civile", value: `${entity.civil_war_fueling_score}/100` },
                  { label: "Échec Responsabilité", value: `${entity.accountability_enforcement_failure_score}/100` },
                  { label: "Indice AEV", value: `${entity.estimated_arms_embargo_violation_index}/10` },
            ...
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-300 leading-relaxed">
                  Renforcer les mécanismes de contrôle des exportations d&apos;armes et engager des sanctions ciblées contre les acteurs complices des violations d&apos;embargo.
                </p>
              </div>
...
export default function ArmsEmbargoViolationEnginePage() {
  const [data, setData] = useState<AEVData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<AEVEntity | null>(null);

  useEffect(() => {
    fetch("/api/arms-embargo-violation-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as AEVData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Arms Embargo Violation Engine</h1>
            <p className="text-sm text-gray-400 mt-1">Violations d&apos;Embargos sur les Armes &amp; Complicité Étatique</p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              <KpiCard label="Entités analysées" value={s.total_entities} sub="acteurs surveillés" />
              ...
              <KpiCard label="Indice AEV moyen" value={`${s.avg_estimated_arms_embargo_violation_index?.toFixed(1) ?? "—"}/10`} sub="violations embargo" accent="text-purple-400" />
              <KpiCard label="Sources de données" value={s.data_sources?.length ?? 0} sub="SIPRI/ONU/OSCE" accent="text-violet-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Violations Embargo Armes</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_arms_embargo_violation_index ?? 0} max={10} label="Indice AEV" color="#a855f7" />
                </div>
              </div>
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.critical_alerts} entité{s.critical_alerts > 1 ? "s" : ""} en violation critique d&apos;embargo sur les armes
                  </p>
                </div>
              </div>
            )}
