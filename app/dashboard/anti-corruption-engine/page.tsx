...
interface DashData { agent?: string; total_entities?: number; avg_composite?: number; confidence_score?: number; risk_distribution?: Record<string,number>; avg_estimated_anti_corruption_index?: number; entities?: Entity[]; [key: string]: unknown; }
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/anti-corruption-engine").then(r => r.json()).then(d => { setData(d); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Anti-Corruption</h1>
        <p className="text-slate-400 mt-1">Anti-Corruption Engine — Analyse des risques de corruption systémique et kleptocracie</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
          <p className="text-slate-400 text-sm">Indice Anti-Corruption Moyen</p>
          <p className="text-2xl font-bold mt-1" style={{ color: ACCENT }}>{loading ? "…" : data.avg_estimated_anti_corruption_index?.toFixed(2) ?? "—"}</p>
