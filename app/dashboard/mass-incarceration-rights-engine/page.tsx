...
interface Entity {
  ...
  last_updated: string;
  prison_overcrowding_score?: number;
  racial_incarceration_bias_score?: number;
  private_prison_rights_violation_score?: number;
  rehabilitation_absence_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "prison_overcrowding_score", label: "Surpopulation carcérale" },
  { key: "racial_incarceration_bias_score", label: "Biais racial d&apos;incarcération" },
  { key: "private_prison_rights_violation_score", label: "Violations dans les prisons privées" },
  { key: "rehabilitation_absence_score", label: "Absence de programmes de réhabilitation" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de l&apos;incarcération de masse : surpopulation carcérale, biais racial d&apos;incarcération, violations dans les prisons privées et absence de programmes de réhabilitation.</p>
              <p className="text-slate-400">Source : Mass Incarceration Rights Engine · Wave 251</p>
            </div>
          )}
        </div>
      </div>
...
export default function MassIncarcerationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/mass-incarceration-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Mass Incarceration Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Surpopulation · Biais Racial · Prisons Privées · Réhabilitation</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#1e3a5f20" } : {}}>
