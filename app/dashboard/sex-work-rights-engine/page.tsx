...
interface Entity {
  ...
  criminalization_rate_score: number;
  violence_impunity_score: number;
  legal_protection_absence_score: number;
  stigma_discrimination_score: number;
  risk_level: string;
...
interface Data {
  ...
  avg_estimated_sex_work_rights_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Taux de Criminalisation", v: entity.criminalization_rate_score, c: "#db2777" },
              { l: "Impunité de la Violence", v: entity.violence_impunity_score, c: "#be185d" },
              { l: "Absence de Protection Légale", v: entity.legal_protection_absence_score, c: "#9d174d" },
              { l: "Stigmatisation & Discrimination", v: entity.stigma_discrimination_score, c: "#831843" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Droits Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_sex_work_rights_index}</span>
            </div>
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Mobilisation urgente pour la dépénalisation et la protection légale des travailleur·euses du sexe exposé·es</p>
                <p>› Coordination avec les organisations de défense des droits pour contrer l&apos;impunité des violences</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Renforcement des mécanismes de signalement sécurisé et d&apos;accès à la justice</p>
                <p>› Plaidoyer accru pour des réformes législatives reconnaissant les droits fondamentaux</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance des évolutions législatives et appui aux programmes de réduction des stigmatisations</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur les pratiques discriminatoires et soutien aux initiatives communautaires</p>
            )}
          </div>
        )}
      </div>
...
export default function SexWorkRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/sex-work-rights-engine")
      .then(r => r.json())
      .then(d => { setData(d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données Sex Work Rights Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse des droits des travailleur·euses du sexe…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Score Moyen", v: `${(data.avg_composite ?? 0).toFixed(1)}`, c: "text-pink-400" },
    { l: "Indice Droits Moyen", v: `${(data.avg_estimated_sex_work_rights_index ?? 0).toFixed(2)}`, c: "text-pink-300" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits Travail Sexuel</h1>
          <p className="text-slate-400 text-sm mt-1">Criminalisation, impunité et protection légale des travailleur·euses du sexe</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {kpis.map(({ l, v, c }) => (
            <div key={l} className="bg-slate-900 border border-pink-500/20 rounded-xl p-4 text-center">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 border border-pink-500/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Droits Travail Sexuel Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("criminalization_rate_score")} label="Criminalisation" color={ACCENT} />
            <GaugeRing value={avgScore("violence_impunity_score")} label="Impunité Violence" color="#be185d" />
            <GaugeRing value={avgScore("legal_protection_absence_score")} label="Absence Protection" color="#9d174d" />
            <GaugeRing value={avgScore("stigma_discrimination_score")} label="Stigmatisation" color="#831843" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-pink-500/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns de Violation des Droits</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#db2777", "#be185d", "#9d174d", "#831843", "#500724"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              className={`bg-slate-900 rounded-xl p-4 border cursor-pointer transition-all hover:border-pink-500/50 ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_sex_work_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Sex Work Rights Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
