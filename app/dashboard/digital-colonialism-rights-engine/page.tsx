...
const ACCENT = "#1a0a2e";
const INDEX_KEY = "estimated_digital_colonialism_rights_index";
...
const FALLBACK_ENTITIES = [
  { id: "DCR-001", name: "Meta/Facebook", country: "USA/Global", sector: "Réseaux sociaux & extraction données pays du Sud", composite_score: 88, risk_level: "critique", estimated_digital_colonialism_rights_index: 8.8 },
  { id: "DCR-002", name: "Google", country: "USA/Global", sector: "Monopole moteur recherche & IA générative", composite_score: 87, risk_level: "critique", estimated_digital_colonialism_rights_index: 8.7 },
  { id: "DCR-003", name: "Amazon AWS", country: "USA/Global", sector: "Infrastructure cloud coloniale numérique", composite_score: 84, risk_level: "critique", estimated_digital_colonialism_rights_index: 8.4 },
  { id: "DCR-004", name: "Microsoft Azure", country: "USA/Global", sector: "Dépendance technologique États & institutions", composite_score: 81, risk_level: "critique", estimated_digital_colonialism_rights_index: 8.1 },
  { id: "DCR-005", name: "Alibaba", country: "Chine/Global", sector: "Expansion numérique Afrique & Asie du Sud", composite_score: 57, risk_level: "élevé", estimated_digital_colonialism_rights_index: 5.7 },
  { id: "DCR-006", name: "Huawei", country: "Chine/Global", sector: "Infrastructure 5G & surveillance États partenaires", composite_score: 54, risk_level: "élevé", estimated_digital_colonialism_rights_index: 5.4 },
  { id: "DCR-007", name: "EU Digital Markets Act", country: "Europe", sector: "Régulation marchés numériques — contraintes GAFAM", composite_score: 31, risk_level: "modéré", estimated_digital_colonialism_rights_index: 3.1 },
  { id: "DCR-008", name: "Fairdata Initiative", country: "Global", sector: "Souveraineté données pays en développement", composite_score: 6, risk_level: "faible", estimated_digital_colonialism_rights_index: 0.6 },
];
...
const FALLBACK_DATA = {
  ...
  avg_estimated_digital_colonialism_rights_index: 6.11,
  confidence_score: 0.85,
  last_analysis: "2026-06-22",
  data_sources: ["digital_rights_watch", "access_now_reports", "eff_surveillance", "eu_digital_markets_act"],
  critical_alerts: ["Meta: extraction_données_pays_du_sud", "Google: monopole_ia_générateur", "Amazon: infrastructure_coloniale_numérique"],
...
interface DashData {
  ...
  avg_estimated_digital_colonialism_rights_index?: number;
        ...
        <div style={{ padding: "20px 24px 24px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
            {[
              ...
              { label: "Index DCR", value: indexVal !== undefined ? `${indexVal}/10` : "—" },
          ...
          <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "12px 16px" }}>
            ...
            <p style={{ fontSize: 11, color: "#475569", margin: "6px 0 0" }}>
              Colonialisme numérique &amp; souveraineté des données
            </p>
          </div>
        </div>
      </div>
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/digital-colonialism-rights-engine")
      .then(r => r.json())
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020817", color: "white", padding: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#a855f7", margin: "0 0 4px" }}>
            Colonialisme Num&eacute;rique &mdash; Droits Souverainet&eacute;
          </h1>
          <p style={{ fontSize: 13, color: "#64748b", margin: 0 }}>
            Digital Colonialism Rights Engine &middot; Wave 211
          </p>
        </div>
      ...
      {loading && (
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: 200 }}>
          <div style={{ width: 32, height: 32, border: "2px solid #a855f7", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
        </div>
      )}
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 28 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px", display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              <GaugeRing value={dash.avg_composite} stroke="#a855f7" />
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
              <p style={{ fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 6px" }}>Index DCR Moyen</p>
              <p style={{ fontSize: 28, fontWeight: 700, color: "#a855f7", margin: 0 }}>{dash.avg_estimated_digital_colonialism_rights_index ?? "—"}</p>
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
              <p style={{ fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 6px" }}>Entit&eacute;s</p>
              <p style={{ fontSize: 28, fontWeight: 700, color: "#a855f7", margin: 0 }}>{dash.total_entities}</p>
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
          ...
          {(dash.data_sources ?? []).length > 0 && (
            <div style={{ background: "rgba(168,85,247,0.06)", border: "1px solid rgba(168,85,247,0.15)", borderRadius: 12, padding: "14px 18px", marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {(dash.data_sources ?? []).map((src: string, i: number) => (
                  <span key={i} style={{ fontSize: 12, color: "#c4b5fd", background: "rgba(168,85,247,0.1)", border: "1px solid rgba(168,85,247,0.2)", borderRadius: 6, padding: "3px 10px" }}>
                    {src}
                  </span>
                ))}
              </div>
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, gap: 8 }}>
                    ...
                    <div style={{ flexShrink: 0 }}>
                      <GaugeRing value={e.composite_score} stroke={RISK_COLORS[e.risk_level] ?? "#a855f7"} />
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: cfg.color }}>{cfg.label}</span>
                    <span style={{ fontSize: 12, color: "#64748b" }}>
                      Index: <span style={{ fontWeight: 700, color: "#a855f7" }}>{indexVal ?? "—"}</span>
                    </span>
                  </div>
                </button>
              );
