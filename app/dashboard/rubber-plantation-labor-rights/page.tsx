...
const ACCENT = "#059669";
const TITRE = "Droits — Plantations d'Hévéa";
...
function DetailModal({ entity, onClose, accent }: { entity: any; onClose: () => void; accent: string }) {
  const [tab, setTab] = useState<"Analyse" | "Contexte" | "Sources">("Analyse");
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }} onClick={onClose}>
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 24, maxWidth: 600, width: "90%", maxHeight: "80vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {(["Analyse", "Contexte", "Sources"] as const).map(t => (
...
export default function Page() {
  ...
  useEffect(() => { fetch("/api/rubber-plantation-labor-rights").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
  ...
  return (
    <div style={{ background: "#020617", minHeight: "100vh", color: "#f1f5f9", fontFamily: "system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <p style={{ color: "#94a3b8", marginBottom: 32 }}>CaelumSwarm™ · CSDDD 2024/1760 · Wave 286</p>
        <p style={{ color: "#64748b", fontSize: 14, marginBottom: 32 }}>Conditions de travail dans les plantations d&apos;hévéa &amp; droits des travailleurs</p>
        <div style={{ display: "flex", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
          ...
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 200px", display: "flex", flexDirection: "column", gap: 10, justifyContent: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#94a3b8" }}>Index RPLR</span><strong style={{ color: ACCENT }}>{data.avg_estimated_rubber_plantation_labor_rights_index?.toFixed(2)}</strong></div>
          </div>
