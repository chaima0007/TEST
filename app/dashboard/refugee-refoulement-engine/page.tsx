...
interface Entity {
  ...
  pushback_frequency_violence_score: number;
  arbitrary_detention_scale_score: number;
  non_refoulement_violation_score: number;
  asylum_system_failure_score: number;
  estimated_refugee_refoulement_index: number;
  risk_level: string;
...
const PATTERN_LABELS: Record<string, string> = {
  pushback_illegal_violent: "Pushback Illégal Violent",
  detention_arbitraire_migrants: "Détention Arbitraire",
  externalisation_controle_frontalier: "Externalisation Frontière",
  refoulement_zones_conflit: "Refoulement Zones Conflit",
  protection_refugies_exemplaire: "Protection Exemplaire",
};
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/refugee-refoulement-engine")
      .then(r => r.json())
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Refugee Refoulement Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Surveillance du refoulement des réfugiés et violations du droit d'asile</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    {[
                      { label: "Pushbacks Violence", value: selected.pushback_frequency_violence_score },
                      { label: "Détention Arbitraire", value: selected.arbitrary_detention_scale_score },
                      { label: "Violation Non-Refoulement", value: selected.non_refoulement_violation_score },
                      { label: "Échec Système Asile", value: selected.asylum_system_failure_score },
                    ].map(s => (
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Refugee Refoulement</div>
                    <div className="text-lg font-bold" style={{ color: ACCENT }}>{typeof selected.estimated_refugee_refoulement_index === "number" ? selected.estimated_refugee_refoulement_index.toFixed(1) : "—"}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
