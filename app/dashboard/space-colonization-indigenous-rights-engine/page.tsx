...
interface Entity {
  ...
  space_resource_extraction_colonialism_score: number;
  indigenous_celestial_heritage_erasure_score: number;
  military_space_dominance_score: number;
  international_governance_accountability_score: number;
  estimated_space_colonization_indigenous_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_space_colonization_indigenous_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Extraction Ressources Spatiales & Colonialisme", value: entity.space_resource_extraction_colonialism_score, weight: "0.30" },
    { label: "Effacement Héritage Céleste Autochtone", value: entity.indigenous_celestial_heritage_erasure_score, weight: "0.25" },
    { label: "Domination Militaire Spatiale", value: entity.military_space_dominance_score, weight: "0.25" },
    { label: "Déficit Gouvernance Internationale", value: entity.international_governance_accountability_score, weight: "0.20" },
  ];
  return (
    <div
          ...
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                ...
                <div style={{ background: "rgba(30,41,59,0.5)", borderRadius: "12px", padding: "16px", textAlign: "center" }}>
                  <div style={{ fontSize: "28px", fontWeight: "bold", color: ACCENT }}>{typeof entity.estimated_space_colonization_indigenous_rights_index === "number" ? entity.estimated_space_colonization_indigenous_rights_index.toFixed(2) : "—"}</div>
                  <div style={{ fontSize: "11px", color: "#94a3b8", marginTop: "4px" }}>Index Droits Autochtones Spatiaux</div>
                </div>
              </div>
...
export default function SpaceColonizationIndigenousRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/space-colonization-indigenous-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ background: "#020617", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: ACCENT, fontSize: "13px" }}>Initialisation Colonisation Spatiale &amp; Droits Autochtones…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_space_colonization_indigenous_rights_index ?? avg(allEntities.map(e => e.estimated_space_colonization_indigenous_rights_index));
  ...
  const avgExtraction = avg(allEntities.map(e => e.space_resource_extraction_colonialism_score));
  const avgHeritage = avg(allEntities.map(e => e.indigenous_celestial_heritage_erasure_score));
  const avgMilitary = avg(allEntities.map(e => e.military_space_dominance_score));
  const avgGovernance = avg(allEntities.map(e => e.international_governance_accountability_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Autochtones", value: avgIndex.toFixed(2) },
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "white", padding: "24px", display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "4px" }}>
          ...
          <h1 style={{ fontSize: "22px", fontWeight: "bold", letterSpacing: "-0.025em", margin: 0 }}>Colonisation Spatiale &amp; Droits Autochtones</h1>
        </div>
        <p style={{ color: "#94a3b8", fontSize: "13px", marginLeft: "24px" }}>
          Space Colonization Indigenous Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: "12px", padding: "20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "24px" }}>
          <GaugeRing value={avgExtraction} label="Extraction & Colonialisme" />
          <GaugeRing value={avgHeritage} label="Effacement Héritage Autochtone" />
          <GaugeRing value={avgMilitary} label="Domination Militaire" />
          <GaugeRing value={avgGovernance} label="Déficit Gouvernance" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div style={{ fontSize: "11px", color: "#64748b", marginTop: "8px" }}>
              Index Droits Autochtones Spatiaux: <span style={{ fontWeight: 500, color: ACCENT }}>{typeof e.estimated_space_colonization_indigenous_rights_index === "number" ? e.estimated_space_colonization_indigenous_rights_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
