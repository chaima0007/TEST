...
interface Entity {
  ...
  involuntary_psychiatric_hospitalization_severity_score: number;
  electroconvulsive_chemical_restraint_scale_score: number;
  mental_health_criminal_justice_diversion_failure_score: number;
  community_mental_health_service_absence_deficit_gap_score: number;
  estimated_mental_health_involuntary_treatment_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_mental_health_involuntary_treatment_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Hospitalisation Psychiatrique Forcée", value: entity.involuntary_psychiatric_hospitalization_severity_score, weight: "0.30" },
    { label: "Contention Chimique & Électroconvulsive", value: entity.electroconvulsive_chemical_restraint_scale_score, weight: "0.25" },
    { label: "Échec Déjudiciarisation Santé Mentale", value: entity.mental_health_criminal_justice_diversion_failure_score, weight: "0.25" },
    { label: "Déficit Services Santé Mentale Communautaires", value: entity.community_mental_health_service_absence_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_mental_health_involuntary_treatment_index === "number" ? entity.estimated_mental_health_involuntary_treatment_index.toFixed(2) : "—"}</div>
...
export default function MentalHealthInvoluntaryTreatmentEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/mental-health-involuntary-treatment-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Santé Mentale & Traitement Involontaire…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_mental_health_involuntary_treatment_index ?? avg(allEntities.map(e => e.estimated_mental_health_involuntary_treatment_index));
  ...
  const avgHospitalization = avg(allEntities.map(e => e.involuntary_psychiatric_hospitalization_severity_score));
  const avgRestraint = avg(allEntities.map(e => e.electroconvulsive_chemical_restraint_scale_score));
  const avgDiversion = avg(allEntities.map(e => e.mental_health_criminal_justice_diversion_failure_score));
  const avgCommunity = avg(allEntities.map(e => e.community_mental_health_service_absence_deficit_gap_score));
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Santé Mentale & Traitement Involontaire</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Mental Health Involuntary Treatment Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgHospitalization} label="Hospitalisation Forcée" />
          <GaugeRing value={avgRestraint} label="Contention Chimique" />
          <GaugeRing value={avgDiversion} label="Échec Déjudiciarisation" />
          <GaugeRing value={avgCommunity} label="Services Communautaires" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Santé Mentale: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_mental_health_involuntary_treatment_index === "number" ? e.estimated_mental_health_involuntary_treatment_index.toFixed(2) : "—"}</span>
            </div>
