...
interface Entity {
  ...
  criminalization_imprisonment_score: number;
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/human-rights-defenders-engine").then(r => r.json()).then(j => setData(j.data ?? j)).catch(console.error);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Human Rights Defenders Engine</h1>
        <p className="text-slate-400 mt-1">Convention ICCPR · Rapporteur ONU · Mécanisme UPR</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice HRD</p>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { key: "killings_disappearances_score", label: "Meurtres/Disp." },
                      { key: "criminalization_imprisonment_score", label: "Criminalisation" },
                      { key: "surveillance_intimidation_score", label: "Surveillance/Intimid." },
                      { key: "legal_protection_absence_score", label: "Absence Protection" },
                    ].map(({ key, label }) => (
                      <div key={key}>
