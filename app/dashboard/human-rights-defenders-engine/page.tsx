...
interface Entity {
  ...
  defender_killing_criminalization_severity_score: number;
  surveillance_harassment_intimidation_scale_score: number;
  legal_framework_protection_absence_score: number;
  civic_space_shrinking_repression_score: number;
  estimated_human_rights_defenders_index: number;
  risk_level: string;
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_human_rights_defenders_index: number;
  data_sources: string[];
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "defender_killing_criminalization_severity_score", label: "Assassinats/Criminalisation" },
  { key: "surveillance_harassment_intimidation_scale_score", label: "Surveillance/Harcèlement" },
  { key: "legal_framework_protection_absence_score", label: "Protection Juridique" },
  { key: "civic_space_shrinking_repression_score", label: "Espace Civique" },
];
        ...
        <div className="flex border-b border-slate-800">
          {["Aperçu", "Métriques", "Sources"].map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 text-red-400" : "text-slate-400 hover:text-white"}`}
          ...
          {tab === 0 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Index Défenseurs</span>
                <span className="font-semibold" style={{ color: ACCENT }}>{entity.estimated_human_rights_defenders_index}/10</span>
              </div>
              <div className="flex items-center justify-between text-sm">
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/human-rights-defenders-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Human Rights Defenders Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Assassinats/Criminalisation · Surveillance · Protection Juridique · Espace Civique</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Index Défenseurs</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_human_rights_defenders_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        {filtered.map(e => {
          ...
          return (
            <button key={e.entity_id} onClick={() => setSelected(e)}
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold uppercase ${cfg.color}`}>{cfg.label}</span>
                <span className="text-xs text-slate-500">Index: <span className="font-bold" style={{ color: ACCENT }}>{e.estimated_human_rights_defenders_index}</span></span>
              </div>
            </button>
          );
        })}
