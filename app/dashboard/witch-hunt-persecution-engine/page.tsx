...
interface WHEntity {
  ...
  accused_killings_violence_scale_score: number;
  legal_protection_absence_score: number;
  stigma_social_exclusion_severity_score: number;
  children_women_targeting_pattern_score: number;
  estimated_witch_hunt_persecution_index: number;
  risk_level: string;
...
interface WHData {
  ...
  data_sources: string[];
  entities: WHEntity[];
  avg_estimated_witch_hunt_persecution_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function WitchHuntPersecutionPage() {
  const [data, setData] = useState<WHData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<WHEntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/witch-hunt-persecution-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données de persécution…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "accused_killings_violence_scale_score", label: "Meurtres Rituels" },
    { key: "legal_protection_absence_score", label: "Protection Légale" },
    { key: "stigma_social_exclusion_severity_score", label: "Stigmatisation" },
    { key: "children_women_targeting_pattern_score", label: "Ciblage Femmes/Enfants" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Chasse aux Sorcières &amp; Persécution</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Witch Hunt Persecution Engine</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_witch_hunt_persecution_index ?? "—" },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Persécution</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#f59e0b", "#f97316", "#eab308", "#ef4444", "#10b981"];
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-500">idx {entity.estimated_witch_hunt_persecution_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Chasse aux Sorcières &amp; Persécution · {data.last_analysis}</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Persécution</div>
                    <div className="text-lg font-bold text-amber-400">{selected.estimated_witch_hunt_persecution_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
