...
type Entity = {
  ...
  risk_level: string
  estimated_lgbtqi_persecution_index: number
}
...
export default function LgbtqiRightsCriminalizationDashboard() {
  const [data, setData] = useState<DomainData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch("/api/lgbtqi-rights-criminalization-engine")
      .then(r => r.json())
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2" style={{ color: ACCENT }}>
            Criminalisation des Droits LGBTQI+
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-slate-900 rounded-xl p-6">
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              Ce tableau de bord mesure la criminalisation légale et la persécution des personnes LGBTQI+,
              analysant les lois pénales, l&apos;impunité des violences et les avancées législatives mondiales.
            </p>
          </div>
        </div>
          ...
          {data.entities.map((entity) => (
            <div key={entity.entity_id} className="bg-slate-900 rounded-xl p-5 border border-slate-800">
              ...
              <div className="flex items-center justify-between text-xs text-slate-500">
                ...
                <span>Index persécution : <span className="font-mono" style={{ color: ACCENT }}>{entity.estimated_lgbtqi_persecution_index.toFixed(2)}</span></span>
              </div>
            </div>
          ))}
        </div>
