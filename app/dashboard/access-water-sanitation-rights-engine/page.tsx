...
export default function AccessWaterSanitationRightsDashboard() {
  const [data, setData] = useState<DomainData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch("/api/access-water-sanitation-rights-engine")
      .then(r => r.json())
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2" style={{ color: ACCENT }}>
            Droits d&apos;Accès à l&apos;Eau &amp; à l&apos;Assainissement
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-slate-900 rounded-xl p-6">
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              Ce tableau de bord évalue l&apos;accès à l&apos;eau potable et aux infrastructures d&apos;assainissement,
              analysant les crises hydriques, la privatisation et les violations du droit humain à l&apos;eau.
            </p>
          </div>
        </div>
          ...
          {data.entities.map((entity) => (
            <div key={entity.entity_id} className="bg-slate-900 rounded-xl p-5 border border-slate-800">
              ...
              <div className="flex items-center justify-between text-xs text-slate-500">
                ...
                <span>Index eau : <span className="font-mono" style={{ color: ACCENT }}>{entity.estimated_water_rights_index.toFixed(2)}</span></span>
              </div>
            </div>
          ))}
        </div>
