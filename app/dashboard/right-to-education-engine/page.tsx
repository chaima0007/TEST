...
interface EDUEntity {
  ...
  school_exclusion_rate_score: number;
  gender_education_barrier_score: number;
  quality_infrastructure_failure_score: number;
  conflict_education_destruction_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  avg_estimated_right_to_education_index: number;
  last_updated: string;
}
...
function EntityCard({ entity, onClick }: { entity: EDUEntity; onClick: (e: EDUEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.school_exclusion_rate_score}</p>
          <p className="text-[10px] text-gray-500">Exclusion Scolaire</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.gender_education_barrier_score}</p>
          <p className="text-[10px] text-gray-500">Barrière Genre</p>
        </div>
      </div>
      <div className="flex items-center justify-between text-xs">
        <span className="text-gray-400">Score: <span className={`font-bold ${rc}`}>{entity.composite_score}</span></span>
        <span className="text-gray-500 truncate ml-2">Édu: {entity.avg_estimated_right_to_education_index}/10</span>
      </div>
    </button>
  );
}

function DetailModal({ entity, onClose }: { entity: EDUEntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Exclusion Scolaire", value: `${entity.school_exclusion_rate_score}/100` },
                  { label: "Barrière Genre", value: `${entity.gender_education_barrier_score}/100` },
                  { label: "Qualité Infrastructure", value: `${entity.quality_infrastructure_failure_score}/100` },
                  { label: "Destruction Conflit", value: `${entity.conflict_education_destruction_score}/100` },
                  { label: "Indice Éducation", value: `${entity.avg_estimated_right_to_education_index}/10` },
            ...
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-300 leading-relaxed">
                  Garantir l'accès universel à une éducation de qualité, éliminer les barrières systémiques liées au genre, au conflit et aux inégalités d'infrastructure scolaire.
                </p>
              </div>
...
export default function RightToEducationEnginePage() {
  const [data, setData] = useState<EDUData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<EDUEntity | null>(null);

  useEffect(() => {
    fetch("/api/right-to-education-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as EDUData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Right to Education Engine</h1>
            <p className="text-sm text-gray-400 mt-1">Droit à l'Éducation &amp; Barrières Systémiques</p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Index Moyen Éducation" value={`${s.avg_estimated_right_to_education_index?.toFixed(1) ?? "—"}/10`} sub="droit éducation" accent="text-sky-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Droit à l'Éducation</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_right_to_education_index ?? 0} max={10} label="Index Édu." color="#0369a1" />
                </div>
              </div>
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.critical_alerts} entité{s.critical_alerts > 1 ? "s" : ""} en situation critique de violation du droit à l'éducation
                  </p>
                </div>
              </div>
            )}
