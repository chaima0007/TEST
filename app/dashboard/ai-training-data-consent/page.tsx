...
export default function AiTrainingDataConsentDashboard() {
  const [data, setData] = useState<{ entities?: { entity: string; composite: number; level: string }[]; avg_composite?: number } | null>(null);
  useEffect(() => { fetch("/api/aitrainingdataconsent").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-2">AI Training Data Consent &amp; Rights</h1>
      <p className="text-gray-500 text-sm mb-6">Droits sur les données utilisées pour entraîner les systèmes d&apos;intelligence artificielle</p>
