...
interface Entity {
  ...
  algorithmic_discrimination_deployment_score: number;
  due_process_algorithmic_denial_score: number;
  transparency_explainability_deficit_score: number;
  regulatory_accountability_framework_score: number;
  estimated_algorithmic_justice_bias_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_algorithmic_justice_bias_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déploiement Discriminatoire", value: entity.algorithmic_discrimination_deployment_score, weight: "0.30" },
    { label: "Déni Due Process Algorithmique", value: entity.due_process_algorithmic_denial_score, weight: "0.25" },
    { label: "Déficit Transparence & Explicabilité", value: entity.transparency_explainability_deficit_score, weight: "0.25" },
    { label: "Cadre Régulation & Responsabilité", value: entity.regulatory_accountability_framework_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                ...
                <div style={{ background: "rgba(30,41,59,0.5)", borderRadius: "12px", padding: "16px", textAlign: "center" }}>
                  <div style={{ fontSize: "30px", fontWeight: "bold", color: ACCENT }}>{typeof entity.estimated_algorithmic_justice_bias_index === "number" ? entity.estimated_algorithmic_justice_bias_index.toFixed(2) : "—"}</div>
                  <div style={{ fontSize: "12px", color: "#94a3b8", marginTop: "4px" }}>Index Justice Algorithmique</div>
                </div>
              </div>
              <div style={{ borderRadius: "8px", padding: "12px", border: `1px solid`, ...(RB[entity.risk_level] ? {} : { borderColor: "#334155", background: "rgba(30,41,59,0.3)" }) }} className={RB[entity.risk_level] ?? ""}>
...
export default function AlgorithmicJusticeBiasRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/algorithmic-justice-bias-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ background: "#020617", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontSize: "14px", color: ACCENT, animation: "pulse 2s infinite" }}>Initialisation Justice Algorithmique &amp; Biais IA…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_algorithmic_justice_bias_index ?? avg(allEntities.map(e => e.estimated_algorithmic_justice_bias_index));
  ...
  const avgDiscrimination = avg(allEntities.map(e => e.algorithmic_discrimination_deployment_score));
  const avgDueProcess = avg(allEntities.map(e => e.due_process_algorithmic_denial_score));
  const avgTransparency = avg(allEntities.map(e => e.transparency_explainability_deficit_score));
  const avgRegulatory = avg(allEntities.map(e => e.regulatory_accountability_framework_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Justice Algo.", value: avgIndex.toFixed(2) },
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "white", padding: "24px", display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "4px" }}>
          ...
          <h1 style={{ fontSize: "24px", fontWeight: "bold", letterSpacing: "-0.025em" }}>Justice Algorithmique &amp; Biais IA</h1>
        </div>
        <p style={{ color: "#94a3b8", fontSize: "14px", marginLeft: "24px" }}>
          Algorithmic Justice &amp; Bias Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: "12px", padding: "20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: "24px" }}>
          <GaugeRing value={avgDiscrimination} label="Discrimination Algo." />
          <GaugeRing value={avgDueProcess} label="Déni Due Process" />
          <GaugeRing value={avgTransparency} label="Déficit Transparence" />
          <GaugeRing value={avgRegulatory} label="Cadre Régulation" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div style={{ fontSize: "12px", color: "#64748b", marginTop: "8px" }}>
              Index Justice Algo.: <span style={{ fontWeight: 500, color: ACCENT }}>{typeof e.estimated_algorithmic_justice_bias_index === "number" ? e.estimated_algorithmic_justice_bias_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
