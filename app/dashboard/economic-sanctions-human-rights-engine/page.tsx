...
interface Entity {
  ...
  civilian_suffering_scale_score: number;
  medicine_food_access_denial_score: number;
  democratic_governance_erosion_score: number;
  sanctions_impunity_perpetrators_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_economic_sanctions_human_rights_index: number;
  last_updated: string;
}
...
interface Data {
  ...
  avg_estimated_economic_sanctions_human_rights_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Échelle Souffrance Civile", v: entity.civilian_suffering_scale_score, c: "#b45309" },
              { l: "Déni Accès Médicaments / Nourriture", v: entity.medicine_food_access_denial_score, c: "#d97706" },
              { l: "Érosion Gouvernance Démocratique", v: entity.democratic_governance_erosion_score, c: "#f59e0b" },
              { l: "Impunité des Auteurs de Sanctions", v: entity.sanctions_impunity_perpetrators_score, c: "#fbbf24" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Sanctions Économiques & Droits Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_economic_sanctions_human_rights_index}</span>
            </div>
        ...
        {tab === "contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Intervention d&apos;urgence pour les populations civiles privées d&apos;accès aux médicaments et à la nourriture par les sanctions</p>
                <p>› Mobilisation des mécanismes onusiens pour évaluer les impacts humanitaires et demander des exemptions sanitaires</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Documentation systématique des souffrances civiles et de l&apos;érosion démocratique liées aux régimes de sanctions</p>
                <p>› Plaidoyer pour la révision des sanctions ciblées afin de minimiser l&apos;impact sur les populations vulnérables</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance des effets secondaires des sanctions sur les droits fondamentaux et soutien aux mécanismes d&apos;exemption humanitaire</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur l&apos;évolution des régimes de sanctions et prévention des dérives humanitaires</p>
            )}
          </div>
        )}
      </div>
...
export default function EconomicSanctionsHumanRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/economic-sanctions-human-rights-engine")
      .then(r => r.json())
      .then(d => { setData(d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données Economic Sanctions Human Rights Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse des sanctions économiques & droits humains…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Score Moyen", v: `${(data.avg_composite ?? 0).toFixed(1)}`, c: "text-amber-400" },
    { l: "Indice Sanctions Moyen", v: `${(data.avg_estimated_economic_sanctions_human_rights_index ?? 0).toFixed(2)}`, c: "text-amber-300" },
    { l: "Critique", v: dist.critique ?? 0, c: "text-red-400" },
    { l: "Index Sanctions", v: dist["élevé"] ?? 0, c: "text-orange-400" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Sanctions Économiques & Droits</h1>
          <p className="text-slate-400 text-sm mt-1">Impact humanitaire des sanctions économiques sur les droits fondamentaux et la gouvernance démocratique</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {kpis.map(({ l, v, c }) => (
            <div key={l} className="bg-slate-900 border border-amber-500/20 rounded-xl p-4 text-center">
        ...
        <div className="bg-slate-900 border border-amber-500/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Sanctions Économiques & Droits Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("civilian_suffering_scale_score")} label="Souffrance Civile" color={ACCENT} />
            <GaugeRing value={avgScore("medicine_food_access_denial_score")} label="Déni Accès Essentiels" color="#d97706" />
            <GaugeRing value={avgScore("democratic_governance_erosion_score")} label="Érosion Démocratie" color="#f59e0b" />
            <GaugeRing value={avgScore("sanctions_impunity_perpetrators_score")} label="Impunité Auteurs" color="#fbbf24" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-amber-500/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns d&apos;Impact des Sanctions</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#b45309", "#d97706", "#f59e0b", "#fbbf24", "#fcd34d"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_economic_sanctions_human_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Economic Sanctions Human Rights Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
