...
const ACCENT = "#0a1628";
const INDEX_KEY = "estimated_climate_refugees_rights_index";
...
const FALLBACK_ENTITIES = [
  { id: "CRR-001", name: "Bangladesh", country: "Asie du Sud", sector: "150M personnes menacées — montée des eaux, cyclones & déplacements côtiers massifs", composite_score: 90, risk_level: "critique", estimated_climate_refugees_rights_index: 9.0 },
  { id: "CRR-002", name: "Tuvalu", country: "Océanie", sector: "Submersion totale imminente — aucun cadre juridique international de réfugié climatique", composite_score: 86, risk_level: "critique", estimated_climate_refugees_rights_index: 8.6 },
  { id: "CRR-003", name: "Somalia", country: "Afrique de l&apos;Est", sector: "Sécheresses récurrentes, conflits & déplacements internes 3,8M — convergence crises", composite_score: 82, risk_level: "critique", estimated_climate_refugees_rights_index: 8.2 },
  { id: "CRR-004", name: "Sudan", country: "Afrique du Nord-Est", sector: "Inondations 2024 catastrophiques, Darfour & réfugiés climatiques sans statut légal", composite_score: 78, risk_level: "critique", estimated_climate_refugees_rights_index: 7.8 },
  { id: "CRR-005", name: "Mozambique", country: "Afrique Australe", sector: "Cyclones Idai/Kenneth — reconstruction insuffisante & déplacés sans protection durable", composite_score: 57, risk_level: "élevé", estimated_climate_refugees_rights_index: 5.7 },
  { id: "CRR-006", name: "India/Sundarbans", country: "Asie du Sud", sector: "Delta du Gange sous eau — 4M personnes déplacées sans plan de relogement national", composite_score: 54, risk_level: "élevé", estimated_climate_refugees_rights_index: 5.4 },
  { id: "CRR-007", name: "Indonesia", country: "Asie du Sud-Est", sector: "Jakarta sous eau, transfert capitale & déplacements côtiers sans compensation adéquate", composite_score: 35, risk_level: "modéré", estimated_climate_refugees_rights_index: 3.5 },
  { id: "CRR-008", name: "New Zealand", country: "Océanie", sector: "Refugee visa climatique Pacifique — cadre légal progressiste mais insuffisant à l&apos;échelle", composite_score: 11, risk_level: "faible", estimated_climate_refugees_rights_index: 1.1 },
];
...
const FALLBACK_DATA = {
  ...
  avg_estimated_climate_refugees_rights_index: 6.16,
  confidence_score: 0.81,
  last_analysis: "2026-06-22",
  data_sources: ["unhcr_climate_displacement", "internal_displacement_monitoring", "ipcc_reports", "climate_refugee_watch"],
  critical_alerts: ["Tuvalu: submersion_territoire_national", "Bangladesh: delta_bengale_critique", "Sudan: convergence_conflit_climat"],
...
interface DashData {
  ...
  avg_estimated_climate_refugees_rights_index?: number;
        ...
        <div style={{ padding: "20px 24px 24px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
            {[
              ...
              { label: "Index CRR", value: indexVal !== undefined ? `${indexVal}/10` : "—" },
          ...
          <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "12px 16px" }}>
            ...
            <p style={{ fontSize: 11, color: "#475569", margin: "6px 0 0" }}>
              Droits &amp; protection des r&eacute;fugi&eacute;s climatiques
            </p>
          </div>
        </div>
      </div>
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/climate-refugees-rights-engine")
      .then(r => r.json())
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020817", color: "white", padding: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#93c5fd", margin: "0 0 4px" }}>
            R&eacute;fugi&eacute;s Climatiques &mdash; Droits &amp; Protection
          </h1>
          <p style={{ fontSize: 13, color: "#64748b", margin: 0 }}>
            Climate Refugees Rights Engine &middot; Wave 212
          </p>
        </div>
      ...
      {loading && (
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: 200 }}>
          <div style={{ width: 32, height: 32, border: "2px solid #93c5fd", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
        </div>
      )}
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 28 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px", display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              <GaugeRing value={dash.avg_composite} stroke="#93c5fd" />
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
              <p style={{ fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 6px" }}>Index CRR Moyen</p>
              <p style={{ fontSize: 28, fontWeight: 700, color: "#93c5fd", margin: 0 }}>{dash.avg_estimated_climate_refugees_rights_index ?? "—"}</p>
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
              <p style={{ fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 6px" }}>Entit&eacute;s</p>
              <p style={{ fontSize: 28, fontWeight: 700, color: "#93c5fd", margin: 0 }}>{dash.total_entities}</p>
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
          ...
          {(dash.data_sources ?? []).length > 0 && (
            <div style={{ background: "rgba(147,197,253,0.05)", border: "1px solid rgba(147,197,253,0.15)", borderRadius: 12, padding: "14px 18px", marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {(dash.data_sources ?? []).map((src: string, i: number) => (
                  <span key={i} style={{ fontSize: 12, color: "#93c5fd", background: "rgba(147,197,253,0.08)", border: "1px solid rgba(147,197,253,0.15)", borderRadius: 6, padding: "3px 10px" }}>
                    {src}
                  </span>
                ))}
              </div>
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, gap: 8 }}>
                    ...
                    <div style={{ flexShrink: 0 }}>
                      <GaugeRing value={e.composite_score} stroke={RISK_COLORS[e.risk_level] ?? "#93c5fd"} />
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: cfg.color }}>{cfg.label}</span>
                    <span style={{ fontSize: 12, color: "#64748b" }}>
                      Index: <span style={{ fontWeight: 700, color: "#93c5fd" }}>{indexVal ?? "—"}</span>
                    </span>
                  </div>
                </button>
              );
