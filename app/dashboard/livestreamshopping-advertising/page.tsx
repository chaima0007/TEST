...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque publicitaire livestream shopping : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_index.toFixed(2)} / 10`,
    `Conformité campagne livestream shopping : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 394 — Livestream Shopping Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: `${COLOR}99`, marginTop: 2 }}>Livestream Shopping Advertising · Wave 394</div>
          </div>
        ...
        {tab === "signaux" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              `Score composite ${entity.composite_score.toFixed(1)} — ${entity.risk_level === "critique" ? "surveillance publicitaire livestream shopping urgente" : entity.risk_level === "élevé" ? "monitoring renforcé recommandé" : "suivi standard actif"}`,
              `Indice Caelum Livestream Shopping Advertising : ${entity.estimated_index.toFixed(2)} / 10`,
              `Campagne livestream shopping en ${entity.risk_level === "faible" ? "zone verte — performance nominale" : "zone d'alerte — optimisation requise"}`,
            ].map((sig, i) => (
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(239,68,68,0.15)" }}>
              ...
              <div>{entity.risk_level === "critique" ? "Audit immédiat des campagnes livestream shopping — conformité réglementaire urgente" : entity.risk_level === "élevé" ? "Révision des stratégies publicitaires livestream shopping — renforcement compliance" : entity.risk_level === "modéré" ? "Optimisation continue des métriques publicitaires livestream shopping" : "Maintien des bonnes pratiques — veille sectorielle livestream shopping"}</div>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(239,68,68,0.15)" }}>
...
export default function LivestreamshoppingAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/livestreamshopping-advertising")
      .then((r) => r.json())
      .then((d) => { setData(d.payload ?? d); setLoading(false); })
      .catch(() => { setError("Erreur de chargement des données Livestream Shopping Advertising."); setLoading(false); });
  }, []);

  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Livestream Shopping Advertising&hellip;
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Livestream Shopping Advertising
          </h1>
        </div>
        <p style={{ fontSize: 13, color: `${COLOR}99`, margin: 0 }}>
          Droits de l&apos;enfant — Secteur shopping en direct · Caelum Partners · Wave 394 · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
      ...
      <div style={{ background: "#0f172a", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 14, padding: 20, marginBottom: 24 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: "#93c5fd99", textTransform: "uppercase", letterSpacing: 1, marginBottom: 16 }}>
          Métriques Globales Livestream Shopping
        </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Livestream Shopping Advertising Intelligence · Wave 394
      </div>
