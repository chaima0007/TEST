...
interface Entity {
  ...
  athlete_exploitation_unpaid_labor_severity_score: number;
  doping_corruption_cover_up_scale_score: number;
  sports_sexual_abuse_coach_impunity_score: number;
  athlete_free_speech_political_expression_deficit_gap_score: number;
  estimated_sports_athlete_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_sports_athlete_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Exploitation & Travail Non Payé", value: entity.athlete_exploitation_unpaid_labor_severity_score, weight: "0.30" },
    { label: "Dopage & Corruption Dissimulée", value: entity.doping_corruption_cover_up_scale_score, weight: "0.25" },
    { label: "Abus Sexuels Sport & Impunité", value: entity.sports_sexual_abuse_coach_impunity_score, weight: "0.25" },
    { label: "Déficit Liberté Expression Athlètes", value: entity.athlete_free_speech_political_expression_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_sports_athlete_rights_index === "number" ? entity.estimated_sports_athlete_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Athlètes</div>
                </div>
              </div>
...
export default function SportsAthleteRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/sports-athlete-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits des Athlètes & Sport…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_sports_athlete_rights_index ?? avg(allEntities.map(e => e.estimated_sports_athlete_rights_index));
  ...
  const avgExploitation = avg(allEntities.map(e => e.athlete_exploitation_unpaid_labor_severity_score));
  const avgDoping = avg(allEntities.map(e => e.doping_corruption_cover_up_scale_score));
  const avgSexualAbuse = avg(allEntities.map(e => e.sports_sexual_abuse_coach_impunity_score));
  const avgFreeSpeech = avg(allEntities.map(e => e.athlete_free_speech_political_expression_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Athlètes", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits des Athlètes & Sport</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Sports Athlete Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgExploitation} label="Exploitation & Travail Non Payé" />
          <GaugeRing value={avgDoping} label="Dopage & Corruption Dissimulée" />
          <GaugeRing value={avgSexualAbuse} label="Abus Sexuels Sport & Impunité" />
          <GaugeRing value={avgFreeSpeech} label="Déficit Liberté Expression" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Athlètes: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_sports_athlete_rights_index === "number" ? e.estimated_sports_athlete_rights_index.toFixed(2) : "—"}</span>
            </div>
