...
interface Entity {
  ...
  antiterrorism_law_political_misuse_severity_score: number;
  secret_detention_torture_rendition_scale_score: number;
  mass_surveillance_privacy_violation_score: number;
  fair_trial_rights_suspension_terrorism_deficit_gap_score: number;
  estimated_counter_terrorism_rights_violations_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_counter_terrorism_rights_violations_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Usage Politique Lois Antiterroristes", value: entity.antiterrorism_law_political_misuse_severity_score, weight: "0.30" },
    { label: "Détention Secrète, Torture & Restitution", value: entity.secret_detention_torture_rendition_scale_score, weight: "0.25" },
    { label: "Surveillance Masse & Violation Vie Privée", value: entity.mass_surveillance_privacy_violation_score, weight: "0.25" },
    { label: "Déficit Procès Équitable Terrorisme", value: entity.fair_trial_rights_suspension_terrorism_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_counter_terrorism_rights_violations_index === "number" ? entity.estimated_counter_terrorism_rights_violations_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Antiterrorisme & Droits</div>
                </div>
              </div>
...
export default function CounterTerrorismRightsViolationsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/counter-terrorism-rights-violations-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Antiterrorisme & Violations des Droits…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_counter_terrorism_rights_violations_index ?? avg(allEntities.map(e => e.estimated_counter_terrorism_rights_violations_index));
  ...
  const avgLawMisuse = avg(allEntities.map(e => e.antiterrorism_law_political_misuse_severity_score));
  const avgDetention = avg(allEntities.map(e => e.secret_detention_torture_rendition_scale_score));
  const avgSurveillance = avg(allEntities.map(e => e.mass_surveillance_privacy_violation_score));
  const avgFairTrial = avg(allEntities.map(e => e.fair_trial_rights_suspension_terrorism_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Antiterrorisme & Droits", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Antiterrorisme & Violations des Droits</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Counter Terrorism Rights Violations Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgLawMisuse} label="Usage Politique Lois" />
          <GaugeRing value={avgDetention} label="Détention Secrète & Torture" />
          <GaugeRing value={avgSurveillance} label="Surveillance de Masse" />
          <GaugeRing value={avgFairTrial} label="Déficit Procès Équitable" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Antiterrorisme & Droits: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_counter_terrorism_rights_violations_index === "number" ? e.estimated_counter_terrorism_rights_violations_index.toFixed(2) : "—"}</span>
            </div>
