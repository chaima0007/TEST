...
interface AFSEntity {
  ...
  severity: string;
  estimated_academic_freedom_suppression_index: number;
...
function EntityCard({ entity }: { entity: AFSEntity }) {
  const idx = entity.estimated_academic_freedom_suppression_index;
  return (
    <div
      style={{
        background: "#ffffff",
...
function SummaryBadges({ entities, avg }: { entities: AFSEntity[]; avg: number }) {
...
export default function AcademicFreedomSuppressionPage() {
  const [data, setData] = useState<AFSData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/academic-freedom-suppression-engine")
      .then((r) => r.json())
