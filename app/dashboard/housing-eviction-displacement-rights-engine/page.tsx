...
const ACCENT = "#0369a1";
const SLUG = "housing-eviction-displacement-rights-engine";
...
function DetailModal({ entity, onClose }: { entity: Record<string, unknown>; onClose: () => void }) {
  ...
  const SUB_SCORES = [
    "forced_eviction_land_grabbing_severity_score",
    "homelessness_inadequate_housing_scale_score",
    "discriminatory_housing_segregation_score",
    "housing_affordability_social_protection_deficit_gap_score",
  ];
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {tab === "sources" && (
            <p className="text-slate-400 text-sm">Sources: UN Special Rapporteur on Adequate Housing, UN-Habitat Global Housing Policy, COHRE Centre on Housing Rights and Evictions.</p>
          )}
        </div>
      </div>
    </div>
...
export default function HousingEvictionDisplacementRightsPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white">Logement &amp; Expulsions Forcées</h1>
          <p className="text-slate-400 mt-1">Droit au logement, expulsions forcées, sans-abrisme et déplacement urbain</p>
        </div>
