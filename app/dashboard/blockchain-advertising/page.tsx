...
const API_URL = "/api/blockchain-advertising";
const TITLE = "Blockchain Advertising Intelligence";
const COLOR = "#f59e0b";

type Entity = { entity?: string; name?: string; composite_score: number; risk_level: string };
...
const CERTS_TEMPLATE: Omit<Cert, "status" | "year">[] = [
  { code: "ISO-27001", label: "ISO 27001", issuer: "ISO" },
  { code: "GDPR-ADS", label: "GDPR Advertising Compliance", issuer: "Union Européenne" },
  { code: "IAB-BLOCKCHAIN", label: "IAB Blockchain Standards", issuer: "IAB Tech Lab" },
  { code: "W3C-DID", label: "W3C DID Specification", issuer: "W3C" },
  { code: "CCPA-ADTECH", label: "CCPA AdTech Compliance", issuer: "California AG" },
];
...
function DetailModal({ entity, color, onClose }: { entity: Entity; color: string; onClose: () => void }) {
  ...
  const label = entity.entity ?? entity.name ?? "";
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
      <div style={{ background: "#fff", borderRadius: 12, padding: 24, width: 520, maxWidth: "90vw", maxHeight: "80vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
          <h2 style={{ fontWeight: "bold", fontSize: 18 }}>{label}</h2>
        ...
        {tab === "indicateurs" && <p>Conformité blockchain advertising — les plateformes publicitaires décentralisées doivent intégrer des mécanismes de transparence on-chain vérifiables : ce contrôle est soumis aux obligations de traçabilité des dépenses publicitaires et aux standards IAB Tech Lab pour la publicité blockchain.</p>}
        {tab === "recommandations" && <p>Intégrer des smart contracts d&apos;audit automatisé pour les impressions publicitaires. Documenter les flux de tokens dans un registre immuable conforme aux standards GDPR et effectuer des audits réguliers d&apos;impact sur la vie privée des utilisateurs selon le cadre W3C DID.</p>}
        {tab === "certifications" && (
          <div>
            <p style={{ fontSize: 12, color: "#6b7280", marginBottom: 12 }}>Attestations de conformité — {label}</p>
...
export default function Page() {
  ...
  const distribution = data.distribution ?? data.entities.reduce((acc, e) => { acc[e.risk_level] = (acc[e.risk_level] ?? 0) + 1; return acc; }, {} as Record<string, number>);
  return (
    <div style={{ padding: 32, fontFamily: "sans-serif", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16 }}>
        {data.entities.map((e, i) => {
          const label = e.entity ?? e.name ?? "";
          return (
            <div key={label || i} onClick={() => setSelected(e)}
