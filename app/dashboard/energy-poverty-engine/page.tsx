...
type EPEEntity = {
  ...
  signal: string;
  energy_access_gap: number;
  rural_electrification_gap: number;
};
...
type EPESummary = {
  ...
  avg_estimated_energy_justice_index: number;
};
...
const PATTERN_COLORS: Record<string, string> = {
  none: "#16a34a",
  energy_access_collapse: "#dc2626",
  just_transition_failure: "#7c3aed",
  green_inequality_trap: "#059669",
  colonial_energy_debt_crisis: "#b45309",
  energy_affordability_crisis_pattern: "#0891b2",
};
const SEVERITY_COLORS: Record<string, string> = {
  "accès_énergie_sous_surveillance": "#16a34a",
  "inégalité_énergétique_structurelle": "#d97706",
  "crise_justice_climatique_majeure": "#92400e",
  "crise_pauvreté_énergétique_systémique": "#dc2626",
};
const ACTION_COLORS: Record<string, string> = {
  "veille_accès_énergie_continue": "#16a34a",
  "renforcement_politiques_justice_énergétique": "#d97706",
  "transition_juste_accélérée_communautés_vulnérables": "#92400e",
  "intervention_urgente_accès_énergie_critique": "#dc2626",
};
...
const SEV_BADGE: Record<string, string> = {
  "accès_énergie_sous_surveillance": "bg-green-900 text-green-300",
  "inégalité_énergétique_structurelle": "bg-amber-900 text-amber-300",
  "crise_justice_climatique_majeure": "bg-amber-950 text-amber-500",
  "crise_pauvreté_énergétique_systémique": "bg-red-950 text-red-400",
};

function DetailModal({ entity, onClose }: { entity: EPEEntity; onClose: () => void }) {
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <span className="ml-2 text-slate-500 text-xs">
              {entity.energy_sector.replace(/_/g, " ")}
            </span>
          </div>
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ...
              ["Score Justice",    entity.justice_score,    "#16a34a"],
              ["Score Transition", entity.transition_score, "#059669"],
            ...
            <div className="col-span-2 bg-slate-900 border border-amber-700/20 rounded-lg p-3">
              <div className="text-amber-500/60 text-xs mb-1">Score Composite Pauvreté Énergétique</div>
          ...
          <div className="bg-slate-900 border border-amber-700/20 rounded-lg p-4 text-sm text-slate-200 leading-relaxed">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="bg-slate-800 rounded p-2">
                <div className="text-amber-500/50 text-xs mb-0.5">Accès Énergie Gap</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.energy_access_gap * 100)}%
                </div>
              </div>
              <div className="bg-slate-800 rounded p-2">
                <div className="text-amber-500/50 text-xs mb-0.5">Gap Électrification Rurale</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.rural_electrification_gap * 100)}%
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-900 border border-amber-700/20 rounded-lg p-3">
              <div className="text-amber-500/60 text-xs mb-1">Secteur Énergétique</div>
            ...
            <div className="bg-slate-900 border border-amber-700/20 rounded-lg p-3">
              ...
              <div className="text-green-400 font-medium capitalize">
                {entity.energy_pattern.replace(/_/g, " ")}
              </div>
            </div>
          </div>
        )}
...
export default function EnergyPovertyDashboard() {
  ...
  const [selected, setSelected] = useState<EPEEntity | null>(null);

  useEffect(() => {
    fetch("/api/energy-poverty-engine")
      .then(r => r.json())
      .then(setData)
      .catch(console.error);
  }, []);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-amber-500 text-lg animate-pulse">
          Initialisation du Moteur Pauvreté Énergétique...
        </div>
      </div>
    );
  }
  ...
  const filtered = entities.filter(
    e =>
      ...
      (patFilter === "all" || e.energy_pattern === patFilter)
  );

  const dists = [
    { title: "Niveau de Risque",          counts: summary.risk_distribution,     colors: RISK_COLORS     },
    { title: "Patron Énergétique",        counts: summary.pattern_distribution,  colors: PATTERN_COLORS  },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-amber-500">
          Pauvreté Énergétique &amp; Justice Climatique — Module 379
        </h1>
        <p className="text-green-500/60 text-sm mt-1">
          Accès · Justice · Transition · Systémique — Caelum Partners — Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ["Total Communautés",          summary.total,                                                         "text-amber-500"],
          ["Crise Accès Énergie",        summary.critical,                                                      "text-red-400"],
          ["Crise Majeure",              summary.high,                                                          "text-amber-700"],
          ["Composite Moyen",            `${summary.avg_composite.toFixed(1)}`,                                 "text-green-500"],
          ["Index Justice Énergétique",  `${summary.avg_estimated_energy_justice_index.toFixed(2)}/10`,         "text-amber-500"],
          ["Accès Moyen",                `${avg_access.toFixed(1)}`,                                            "text-green-500"],
        ].map(([l, v, c]) => (
          <div
            key={String(l)}
      ...
      <div className="bg-slate-900 border border-amber-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          ...
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.justice_score, 0) / (entities.length || 1)}
            label="Justice Moy."
            color="#16a34a"
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.transition_score, 0) / (entities.length || 1)}
            label="Transition Moy."
            color="#059669"
          />
          <GaugeRing
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {[
          "all",
          "none",
          "energy_access_collapse",
          "just_transition_failure",
          "green_inequality_trap",
          "colonial_energy_debt_crisis",
          "energy_affordability_crisis_pattern",
        ].map(p => (
          <button
            key={p}
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div className="text-xs text-green-500/60 mb-2 capitalize">
              {e.energy_pattern.replace(/_/g, " ")}
            </div>
            <div className="text-xs text-amber-500/70 font-medium mb-2">
              Accès: {e.access_score.toFixed(1)} · Justice: {e.justice_score.toFixed(1)}
            </div>
