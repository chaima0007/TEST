...
export default function PressFreedomJournalistSafetyPage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#0f3460";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", modéré: "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/press-freedom-journalist-safety-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_press_freedom_journalist_safety_index"] as number) ?? 0;
