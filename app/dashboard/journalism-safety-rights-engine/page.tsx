...
interface Entity {
  ...
  journalist_killings_score: number;
  arbitrary_detention_press_score: number;
  censorship_suppression_score: number;
  surveillance_intimidation_score: number;
  estimated_journalism_safety_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_journalism_safety_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Assassinats de Journalistes", value: entity.journalist_killings_score, weight: "0.30" },
    { label: "Détention Arbitraire Presse", value: entity.arbitrary_detention_press_score, weight: "0.25" },
    { label: "Censure & Suppression", value: entity.censorship_suppression_score, weight: "0.25" },
    { label: "Surveillance & Intimidation", value: entity.surveillance_intimidation_score, weight: "0.20" },
  ];
          ...
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: 16, textAlign: "center" }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: ACCENT }}>{typeof entity.estimated_journalism_safety_rights_index === "number" ? entity.estimated_journalism_safety_rights_index.toFixed(2) : "—"}</div>
                  <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>Index Sécurité Presse</div>
                </div>
              </div>
...
export default function JournalismSafetyRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/journalism-safety-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ background: "#020617", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: ACCENT, fontSize: 14 }}>Initialisation Sécurité des Journalistes &amp; Liberté de Presse…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_journalism_safety_rights_index ?? avg(allEntities.map(e => e.estimated_journalism_safety_rights_index));
  ...
  const avgKillings = avg(allEntities.map(e => e.journalist_killings_score));
  const avgDetention = avg(allEntities.map(e => e.arbitrary_detention_press_score));
  const avgCensorship = avg(allEntities.map(e => e.censorship_suppression_score));
  const avgSurveillance = avg(allEntities.map(e => e.surveillance_intimidation_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Sécurité Presse", value: avgIndex.toFixed(2) },
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "#f8fafc", padding: 24, display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.5px" }}>Sécurité des Journalistes &amp; Liberté de Presse</h1>
        </div>
        <p style={{ fontSize: 13, color: "#64748b", marginLeft: 16 }}>
          Journalism Safety Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: 24 }}>
          <GaugeRing value={avgKillings} label="Assassinats Journalistes" />
          <GaugeRing value={avgDetention} label="Détention Arbitraire" />
          <GaugeRing value={avgCensorship} label="Censure & Suppression" />
          <GaugeRing value={avgSurveillance} label="Surveillance & Intimidation" />
        </div>
      </div>
          ...
          return (
            <div key={e.entity_id} onClick={() => setSelected(e)}
              ...
              <div style={{ fontSize: 11, color: "#64748b" }}>
                Index: <span style={{ fontWeight: 600, color: ACCENT }}>{typeof e.estimated_journalism_safety_rights_index === "number" ? e.estimated_journalism_safety_rights_index.toFixed(2) : "—"}</span>
              </div>
            </div>
          );
        })}
