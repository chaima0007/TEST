...
interface SEEntity {
  ...
  composite_score: number; informal_sector_dominance_score: number;
  tax_evasion_systemic_score: number; corruption_lubricant_score: number;
  regulatory_arbitrage_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_shadow_index: number; last_updated: string;
}
interface SESummary {
  ...
  entities: SEEntity[]; avg_estimated_shadow_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: SEEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Informalité", v: entity.informal_sector_dominance_score, c: "#94a3b8" },
              { l: "Évasion Fiscale", v: entity.tax_evasion_systemic_score, c: "#64748b" },
              { l: "Corruption Lubrifiant", v: entity.corruption_lubricant_score, c: "#475569" },
              { l: "Arbitrage Réglementaire", v: entity.regulatory_arbitrage_score, c: "#334155" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Économie Grise Caelum</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_shadow_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Réforme fiscale radicale pour l'inclusion de l'économie informelle</p><p>› Programme anti-corruption systémique avec sanctions renforcées</p><p>› Dialogue social pour formaliser les activités informelles</p></>}
            {entity.risk_level === "élevé" && <><p>› Simplification administrative pour réduire l'arbitrage réglementaire</p><p>› Renforcement des capacités de collecte fiscale</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance des zones grises économiques</p><p>› Incitations à la formalisation des entreprises</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la vigilance sur les flux informels</p><p>› Consolidation des institutions fiscales</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  economie_parallele_totale: "#ef4444",
  evasion_systemique: "#f97316",
  dualisme_economique: "#94a3b8",
  tensions_formelles: "#eab308",
  economie_formelle: "#10b981",
};

export default function ShadowEconomyEnginePage() {
  const [data, setData] = useState<SESummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<SEEntity | null>(null);

  useEffect(() => {
    fetch("/api/shadow-economy-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données économie de l'ombre"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de l'économie de l'ombre en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof SEEntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Économie de l'Ombre</h1>
          <p className="text-slate-400 text-sm mt-1">L'informalité comme mesure du divorce entre l'État et ses citoyens</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Composite Moyen", v: `${data.avg_composite}%`, t: "text-slate-300" },
            { l: "Alertes Critiques", v: data.critical_alerts.length, t: "text-red-300" },
            { l: "Indice Économie Grise", v: data.avg_estimated_shadow_index, t: "text-slate-400" },
            { l: "Sources", v: data.data_sources?.length ?? 0, t: "text-orange-300" },
            { l: "Confiance Analyse", v: `${Math.round((data.confidence_score ?? 0) * 100)}%`, t: "text-slate-500" },
          ].map(({ l, v, t }) => (
            <div key={l} className="bg-slate-900 border border-slate-600/30 rounded-xl p-4">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-600/30">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Dimensions de l'Économie Souterraine — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("informal_sector_dominance_score")} label="Informalité" color={ACCENT} />
            <GaugeRing value={avgScore("tax_evasion_systemic_score")} label="Évasion Fiscale" color="#64748b" />
            <GaugeRing value={avgScore("corruption_lubricant_score")} label="Corruption Lubrifiant" color="#475569" />
            <GaugeRing value={avgScore("regulatory_arbitrage_score")} label="Arbitrage Régl." color="#334155" />
          </div>
        </div>

        {/* Distribution Bars */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-600/30">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-600/30">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns d'Économie Souterraine</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-slate-800/50 border border-slate-600/30 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Économie de l'Ombre Critiques</h2>
            <ul className="space-y-1">{data.critical_alerts.map((a, i) => <li key={i} className="text-sm text-slate-300">› {a}</li>)}</ul>
          </div>
        )}
        ...
        <div className="flex gap-2 flex-wrap">
          {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              style={filter === f ? { backgroundColor: ACCENT, borderColor: ACCENT, color: "#0f172a" } : {}}>
              {f}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              className={`bg-slate-900 rounded-xl p-4 border cursor-pointer transition-all hover:border-slate-400/50 ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_shadow_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Économie de l'Ombre · {data.last_analysis}</p>
      </div>
    </div>
  );
}
