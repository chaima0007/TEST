...
interface Entity {
  ...
  racial_profiling_police_brutality_severity_score: number;
  systemic_economic_housing_discrimination_scale_score: number;
  hate_crime_racial_violence_impunity_score: number;
  anti_racism_policy_reparation_deficit_gap_score: number;
  estimated_racial_discrimination_systemic_racism_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_racial_discrimination_systemic_racism_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Profilage Racial & Brutalité Policière", value: entity.racial_profiling_police_brutality_severity_score, weight: "0.30" },
    { label: "Discrimination Économique & Logement Systémique", value: entity.systemic_economic_housing_discrimination_scale_score, weight: "0.25" },
    { label: "Crimes Haineux & Impunité Violences Raciales", value: entity.hate_crime_racial_violence_impunity_score, weight: "0.25" },
    { label: "Déficit Politiques Anti-Racisme & Réparations", value: entity.anti_racism_policy_reparation_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_racial_discrimination_systemic_racism_index === "number" ? entity.estimated_racial_discrimination_systemic_racism_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Racisme Systémique</div>
                </div>
              </div>
...
export default function RacialDiscriminationSystemicRacismEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/racial-discrimination-systemic-racism-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Discrimination Raciale & Racisme Systémique…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_racial_discrimination_systemic_racism_index ?? avg(allEntities.map(e => e.estimated_racial_discrimination_systemic_racism_index));
  ...
  const avgRacialProfiling = avg(allEntities.map(e => e.racial_profiling_police_brutality_severity_score));
  const avgEconomic = avg(allEntities.map(e => e.systemic_economic_housing_discrimination_scale_score));
  const avgHateCrime = avg(allEntities.map(e => e.hate_crime_racial_violence_impunity_score));
  const avgAntiRacism = avg(allEntities.map(e => e.anti_racism_policy_reparation_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Racisme Systémique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Discrimination Raciale & Racisme Systémique</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Racial Discrimination Systemic Racism Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgRacialProfiling} label="Profilage Racial" />
          <GaugeRing value={avgEconomic} label="Discrimination Économique" />
          <GaugeRing value={avgHateCrime} label="Crimes Haineux" />
          <GaugeRing value={avgAntiRacism} label="Politiques Anti-Racisme" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Racisme Systémique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_racial_discrimination_systemic_racism_index === "number" ? e.estimated_racial_discrimination_systemic_racism_index.toFixed(2) : "—"}</span>
            </div>
