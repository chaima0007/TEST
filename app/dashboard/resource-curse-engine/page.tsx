...
interface RCEntity {
  ...
  composite_score: number; resource_dependency_score: number;
  institutional_degradation_score: number; dutch_disease_score: number;
  elite_capture_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_curse_index: number; last_updated: string;
}
interface RCSummary {
  ...
  entities: RCEntity[]; avg_estimated_curse_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: RCEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Dépendance Ressources", v: entity.resource_dependency_score, c: "#f59e0b" },
              { l: "Dégradation Institutionnelle", v: entity.institutional_degradation_score, c: "#f97316" },
              { l: "Syndrome Hollandais", v: entity.dutch_disease_score, c: "#ef4444" },
              { l: "Capture Élites", v: entity.elite_capture_score, c: "#a855f7" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Malédiction Caelum</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_curse_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Plan national de diversification économique d'urgence</p><p>› Réforme institutionnelle radicale pour briser la capture des élites</p><p>› Fonds souverain de transformation post-rente</p></>}
            {entity.risk_level === "élevé" && <><p>› Politique industrielle de diversification prioritaire</p><p>› Renforcement des institutions et de la gouvernance des ressources</p></>}
            {entity.risk_level === "modéré" && <><p>› Investissement dans les secteurs hors ressources naturelles</p><p>› Surveillance des symptômes du syndrome hollandais</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la vigilance sur la dépendance aux ressources</p><p>› Consolidation des institutions économiques diversifiées</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  malediction_ressources_totale: "#ef4444",
  dependance_rentiere_critique: "#f97316",
  syndrome_hollandais: "#f59e0b",
  rente_moderee: "#eab308",
  diversification_reussie: "#10b981",
};

export default function ResourceCurseEnginePage() {
  const [data, setData] = useState<RCSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<RCEntity | null>(null);

  useEffect(() => {
    fetch("/api/resource-curse-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données malédiction des ressources"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la malédiction des ressources en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof RCEntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Malédiction des Ressources</h1>
          <p className="text-slate-400 text-sm mt-1">Le paradoxe de l'abondance — richesse naturelle et sous-développement institutionnel</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Composite Moyen", v: `${data.avg_composite}%`, t: "text-amber-300" },
            { l: "Alertes Critiques", v: data.critical_alerts.length, t: "text-red-300" },
            { l: "Indice Malédiction Moyen", v: data.avg_estimated_curse_index, t: "text-amber-400" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-amber-500/20">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Dimensions de la Malédiction — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("resource_dependency_score")} label="Dépendance Ressources" color={ACCENT} />
            <GaugeRing value={avgScore("institutional_degradation_score")} label="Dégradation Instit." color="#f97316" />
            <GaugeRing value={avgScore("dutch_disease_score")} label="Syndrome Hollandais" color="#ef4444" />
            <GaugeRing value={avgScore("elite_capture_score")} label="Capture Élites" color="#a855f7" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-amber-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Malédiction des Ressources</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-amber-900/10 border border-amber-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Malédiction Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_curse_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Malédiction des Ressources · {data.last_analysis}</p>
      </div>
    </div>
  );
}
