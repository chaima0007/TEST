...
interface Entity {
  ...
  last_updated: string;
  ecosystem_destruction_score?: number;
  community_consent_score?: number;
  biodiversity_loss_score?: number;
  profit_sharing_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "ecosystem_destruction_score", label: "Destruction des écosystèmes" },
  { key: "community_consent_score", label: "Consentement communautaire" },
  { key: "biodiversity_loss_score", label: "Perte de biodiversité" },
  { key: "profit_sharing_score", label: "Partage des bénéfices" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits sur les mines des fonds marins : destruction des écosystèmes abyssaux, consentement des communautés côtières, perte de biodiversité marine et partage équitable des bénéfices d&apos;extraction.</p>
              <p className="text-slate-400">Source : Deep Sea Mining Rights Engine · Wave 270</p>
            </div>
          )}
        </div>
      </div>
...
export default function DeepSeaMiningRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/deep-sea-mining-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits sur les Mines des Fonds Marins</h1>
          <p className="text-slate-400 mt-1 text-sm">Destruction des Écosystèmes · Consentement Communautaire · Biodiversité Marine · Partage des Bénéfices</p>
        </div>
