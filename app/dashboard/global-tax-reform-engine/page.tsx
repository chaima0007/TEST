...
interface TaxEntity {
  ...
  tax_haven_exposure_score: number;
  profit_shifting_score: number;
  treaty_abuse_score: number;
  regulatory_arbitrage_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_tax_index: number;
  last_updated: string;
}
...
interface TaxSummary {
  ...
  data_sources: string[];
  entities: TaxEntity[];
  avg_estimated_tax_index: number;
}
...
const PATTERNS = [
  { name: "Évasion Fiscale Agressive", severity: "critique", signal: "tax_haven_exposure_score > 70", action: "Signalement au Groupe BEPS de l'OCDE et audit fiscal forcé", test: (e: TaxEntity) => e.tax_haven_exposure_score > 70 },
  { name: "Transfert de Bénéfices", severity: "critique", signal: "profit_shifting_score > 65", action: "Application du taux minimum global de 15% OCDE/G20", test: (e: TaxEntity) => e.profit_shifting_score > 65 },
  { name: "Abus de Conventions Fiscales", severity: "élevé", signal: "treaty_abuse_score > 60", action: "Renégociation des traités fiscaux bilatéraux", test: (e: TaxEntity) => e.treaty_abuse_score > 60 },
  { name: "Arbitrage Réglementaire", severity: "élevé", signal: "regulatory_arbitrage_score > 55", action: "Harmonisation des règles CFC et substance économique", test: (e: TaxEntity) => e.regulatory_arbitrage_score > 55 },
  { name: "Opacité Fiscale", severity: "modéré", signal: "composite_score > 40", action: "Reporting pays par pays public obligatoire", test: (e: TaxEntity) => e.composite_score > 40 },
];
...
function EntityCard({ entity, onClick }: { entity: TaxEntity; onClick: (e: TaxEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.tax_haven_exposure_score}</p>
          <p className="text-[10px] text-gray-500">Paradis Fiscal</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.profit_shifting_score}</p>
          <p className="text-[10px] text-gray-500">Transfert Bénéf.</p>
        </div>
      </div>
...
function DetailModal({ entity, onClose }: { entity: TaxEntity; onClose: () => void }) {
        ...
        <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
          {tab === "scores" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Exposition Paradis Fiscaux", value: `${entity.tax_haven_exposure_score}/100` },
                  { label: "Transfert de Bénéfices", value: `${entity.profit_shifting_score}/100` },
                  { label: "Abus Conventions", value: `${entity.treaty_abuse_score}/100` },
                  { label: "Arbitrage Réglementaire", value: `${entity.regulatory_arbitrage_score}/100` },
                  { label: "Index Fiscal", value: `${entity.estimated_tax_index}/10` },
              ...
              <div className="bg-white/4 rounded-lg p-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm text-gray-300">Score Composite Fiscal</span>
                ...
                <p className="text-[11px] text-gray-500 mt-1.5">Formule : paradis×0.30 + transfert×0.25 + conventions×0.25 + arbitrage×0.20</p>
              </div>
              <div className="space-y-2">
                <p className="text-xs text-gray-400 font-medium">Signaux Détectés</p>
          ...
          {tab === "signaux" && (
            <div className="space-y-3">
              {triggered.length === 0 ? (
                <div className="text-center py-8 text-gray-400 text-sm">Aucun risque d&apos;évasion fiscale détecté</div>
              ) : triggered.map((p) => (
            ...
            <div className="space-y-3">
              {triggered.length === 0 ? (
                <div className="bg-emerald-500/10 border border-emerald-500/25 rounded-xl p-4">
                  <p className="text-sm text-emerald-400 font-medium">Conformité Fiscale Vérifiée</p>
                  <p className="text-xs text-emerald-400/70 mt-1">Maintenir les pratiques de reporting transparent et d&apos;alignement sur les standards OCDE.</p>
                </div>
              ) : triggered.map((p, i) => (
              ...
              <div className="bg-slate-800/60 border border-white/8 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 mt-1"><span className="text-gray-300 font-medium">Index Fiscal :</span> {entity.estimated_tax_index}/10</p>
              </div>
            </div>
          )}
        </div>
...
export default function GlobalTaxReformPage() {
  const [data, setData] = useState<TaxData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("all");
  const [selected, setSelected] = useState<TaxEntity | null>(null);

  useEffect(() => {
    fetch("/api/global-tax-reform-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as TaxData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Réforme Fiscale Mondiale</h1>
            <p className="text-sm text-gray-400 mt-1">Moteur d&apos;intelligence Swarm — analyse des risques d&apos;évasion fiscale et d&apos;optimisation agressive des multinationales</p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              <KpiCard label="Total Entités" value={s.total_entities} sub="multinationales analysées" />
              ...
              <KpiCard label="Entités Critiques" value={s.risk_distribution["critique"] ?? 0} sub="évasion avérée" accent="text-red-400" />
              <KpiCard label="Index Fiscal Moyen" value={`${s.avg_estimated_tax_index}/10`} sub="indice de risque" accent="text-indigo-400" />
              <KpiCard label="Confiance Analyse" value={`${s.confidence_score}%`} sub="fiabilité des données" accent="text-emerald-400" />
              <KpiCard label="Alertes Critiques" value={s.critical_alerts.length} sub="interventions OCDE" accent="text-red-400" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Fiscaux Clés</h2>
                <div className="grid grid-cols-4 gap-4">
                  <GaugeRing value={s.avg_composite} max={100} label="Score Global" color="#f87171" />
                  <GaugeRing value={s.avg_estimated_tax_index} max={10} label="Index Fiscal" color="#818cf8" />
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.risk_distribution["critique"]} entité{(s.risk_distribution["critique"] ?? 0) > 1 ? "s" : ""} en situation critique d&apos;évasion fiscale
                  </p>
                  <p className="text-xs text-red-400/70 mt-0.5">
                    Entreprise la plus à risque : <span className="font-medium">{s.top_risk_entities[0]}</span>
                  </p>
                </div>
              </div>
            )}
