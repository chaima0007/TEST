...
export default function ForcedDisappearanceExtrajudicialKillingPage() {
  ...
  useEffect(() => {
    fetch("/api/forced-disappearance-extrajudicial-killing-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_forced_disappearance_extrajudicial_killing_index"] as number) ?? 0;
