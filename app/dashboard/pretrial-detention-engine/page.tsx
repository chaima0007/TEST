...
interface PDEntity {
  ...
  detention_duration_excess_score: number;
  inhumane_conditions_score: number;
  legal_access_denial_score: number;
  presumption_innocence_violation_score: number;
  risk_level: string;
  primary_pattern: string;
  estimated_pretrial_detention_index: number;
  last_updated: string;
  [key: string]: unknown;
}
...
interface PDSummary {
  ...
  avg_estimated_pretrial_detention_index: number;
  data_sources: string[];
  entities: PDEntity[];
}
...
function DetailModal({ entity, onClose }: { entity: PDEntity; onClose: () => void }) {
  const [tab, setTab] = useState(0);
  const tabs = ["Scores", "Indicateurs", "Contexte juridique"];
  return (
      ...
      <div className="w-full max-w-lg rounded-xl border border-slate-700/50 bg-slate-900 p-6 shadow-2xl">
        ...
        {tab === 0 && (
          <div className="space-y-3">
            {[
              { label: "Detention Duration Excess", value: entity.detention_duration_excess_score },
              { label: "Inhumane Conditions", value: entity.inhumane_conditions_score },
              { label: "Legal Access Denial", value: entity.legal_access_denial_score },
              { label: "Presumption of Innocence Violation", value: entity.presumption_innocence_violation_score },
            ].map((s) => (
        ...
        {tab === 1 && (
          <div className="flex flex-col items-center gap-4 py-4">
            <GaugeRing value={entity.estimated_pretrial_detention_index} max={10} />
            <div className="text-center">
              <div className="text-3xl font-bold text-amber-400">
                {typeof entity.estimated_pretrial_detention_index === "number"
                  ? entity.estimated_pretrial_detention_index.toFixed(1)
                  : entity.estimated_pretrial_detention_index}
              </div>
              <div className="text-sm text-slate-500">Pretrial Detention Index</div>
            </div>
          </div>
        )}
        {tab === 2 && (
          <div className="rounded-lg border border-slate-700/40 bg-slate-800/50 p-4">
            <p className="text-sm leading-relaxed text-slate-400">
              Analyse du contexte juridique de la détention préventive pour {entity.name}. Indice global&nbsp;:&nbsp;
              <span className="text-slate-200 font-medium">
                {typeof entity.estimated_pretrial_detention_index === "number"
                  ? entity.estimated_pretrial_detention_index.toFixed(2)
                  : entity.estimated_pretrial_detention_index}
              </span>
...
export default function PretrialDetentionEngine() {
  const [data, setData] = useState<ApiData>({});
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<PDEntity | null>(null);

  useEffect(() => {
    fetch("/api/pretrial-detention-engine")
      .then((r) => r.json())
  ...
  const entities: PDEntity[] = (data.entities ?? []).filter(
    ...
    <div className="min-h-screen bg-slate-950 p-6 text-slate-100">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-100">Pretrial Detention Engine</h1>
            <p className="mt-1 text-sm text-slate-500">Monitoring des abus de détention préventive</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {(kpis.length > 0 ? kpis : [
            { label: "Detention Duration Excess", value: "—" },
            { label: "Inhumane Conditions", value: "—" },
            { label: "Legal Access Denial", value: "—" },
            { label: "Presumption Innocence Violation", value: "—" },
            { label: "Pretrial Detention Index Moyen", value: "—" },
            ...
            {entities.map((entity) => (
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="text-xs text-slate-500">PD Index</span>
                ...
                <div className="mt-2 space-y-1">
                  {[
                    { key: "detention_duration_excess_score", label: "Duration" },
                    { key: "inhumane_conditions_score", label: "Conditions" },
                    { key: "legal_access_denial_score", label: "Legal Access" },
                    { key: "presumption_innocence_violation_score", label: "Innocence" },
                  ].map(({ key, label }) => (
