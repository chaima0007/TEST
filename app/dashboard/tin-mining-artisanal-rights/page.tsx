...
interface Entity {
  ...
  estimated_tin_mining_artisanal_rights_index: number;
  [key: string]: unknown;
}
...
interface DashData {
  ...
  avg_estimated_tin_mining_artisanal_rights_index?: number;
...
export default function TinMiningArtisanalRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/tin-mining-artisanal-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ background: "#020617", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: ACCENT, fontSize: 14 }}>Initialisation Droits des Mineurs d&apos;Étain…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_tin_mining_artisanal_rights_index ?? avg(allEntities.map(e => e.estimated_tin_mining_artisanal_rights_index));
  ...
  const kpis = [
    ...
    { label: "Index Étain", value: avgIndex.toFixed(2) },
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "#fff", padding: 24, display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em" }}>Droits des Mineurs d&apos;Étain</h1>
        </div>
        <p style={{ color: "#94a3b8", fontSize: 14, marginLeft: 16 }}>
          Tin Mining Artisanal Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
        ...
        <div style={{ display: "flex", gap: 32, flexWrap: "wrap", justifyContent: "space-around" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 12, color: "#94a3b8" }}>Index Étain (×10)</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div style={{ fontSize: 11, color: "#64748b", marginTop: 8 }}>
              Index Étain: <span style={{ fontWeight: 600, color: ACCENT }}>{typeof e.estimated_tin_mining_artisanal_rights_index === "number" ? e.estimated_tin_mining_artisanal_rights_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
