...
interface Entity {
  ...
  country: string;
  sector: string;
  defender_killing_impunity_severity_score: number;
  criminalization_slapp_lawsuit_scale_score: number;
  corporate_state_collusion_resource_conflict_score: number;
  indigenous_land_defender_protection_deficit_score: number;
  estimated_environmental_defenders_persecution_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_environmental_defenders_persecution_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Assassinats & Impunité", value: entity.defender_killing_impunity_severity_score, weight: "0.30" },
    { label: "Criminalisation SLAPP", value: entity.criminalization_slapp_lawsuit_scale_score, weight: "0.25" },
    { label: "Collusion Corpo-Étatique", value: entity.corporate_state_collusion_resource_conflict_score, weight: "0.25" },
    { label: "Déficit Protection Autochtones", value: entity.indigenous_land_defender_protection_deficit_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.defender_killing_impunity_severity_score === "number" ? entity.defender_killing_impunity_severity_score.toFixed(1) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Assassinats Impunité</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_environmental_defenders_persecution_index === "number" ? entity.estimated_environmental_defenders_persecution_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Défenseurs Environnementaux</div>
                </div>
              </div>
...
export default function EnvironmentalDefendersPerseculionEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/environmental-defenders-persecution-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation D&eacute;fenseurs Environnementaux Pers&eacute;cut&eacute;s…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.defender_killing_impunity_severity_score));
  const avgIndex = data?.avg_estimated_environmental_defenders_persecution_index ?? avg(allEntities.map(e => e.estimated_environmental_defenders_persecution_index));
  ...
  const avgKilling = avg(allEntities.map(e => e.defender_killing_impunity_severity_score));
  const avgSLAPP = avg(allEntities.map(e => e.criminalization_slapp_lawsuit_scale_score));
  const avgCollusion = avg(allEntities.map(e => e.corporate_state_collusion_resource_conflict_score));
  const avgIndigenous = avg(allEntities.map(e => e.indigenous_land_defender_protection_deficit_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Défenseurs Environnementaux", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">D&eacute;fenseurs Environnementaux Pers&eacute;cut&eacute;s</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Environmental Defenders Persecution Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgKilling} label="Assassinats & Impunité" />
          <GaugeRing value={avgSLAPP} label="Criminalisation SLAPP" />
          <GaugeRing value={avgCollusion} label="Collusion Corpo-Étatique" />
          <GaugeRing value={avgIndigenous} label="Déficit Protection Autochtones" />
        </div>
      </div>
          ...
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.defender_killing_impunity_severity_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.defender_killing_impunity_severity_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Défenseurs Environnementaux: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_environmental_defenders_persecution_index === "number" ? e.estimated_environmental_defenders_persecution_index.toFixed(2) : "—"}</span>
            </div>
