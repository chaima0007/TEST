...
interface Entity {
  ...
  estimated_racial_discrimination_rights_index: number;
  [key: string]: unknown;
}
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/racial-discrimination-rights-engine").then(r => r.json()).then(d => setData(d.payload ?? d)).catch(console.error);
  }, []);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020617", color: "#f1f5f9", padding: "24px" }}>
      <div style={{ marginBottom: "32px" }}>
        <h1 style={{ fontSize: "30px", fontWeight: 700, color: ACCENT_LIGHT, margin: 0 }}>Racial Discrimination Rights Engine</h1>
        <p style={{ color: "#94a3b8", marginTop: "4px" }}>Racisme Systémique · Apartheid · Discrimination Policière · Droits Minorities</p>
      </div>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "24px", flexWrap: "wrap" }}>
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{
            ...
            background: filter === f ? "#450a0a" : "transparent",
            border: filter === f ? "1px solid #ef4444" : "1px solid #334155",
            ...
            <div style={{ padding: "24px" }}>
              {tab === 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px" }}>
                    <span style={{ color: "#94a3b8" }}>Indice RDR</span>
                    <span style={{ color: ACCENT_LIGHT, fontWeight: 600 }}>{selected.estimated_racial_discrimination_rights_index}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px" }}>
              ...
              {tab === 1 && (
                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", marginBottom: "4px" }}>
                      <span style={{ color: "#94a3b8" }}>Indice Discrimination Raciale</span>
                      <span style={{ color: "#f1f5f9" }}>{Number(selected.estimated_racial_discrimination_rights_index) * 10}/100</span>
                    </div>
                    <div style={{ height: "6px", background: "#1e293b", borderRadius: "3px", overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${Number(selected.estimated_racial_discrimination_rights_index) * 10}%`, background: RC[selected.risk_level], borderRadius: "3px" }} />
                    </div>
                  </div>
                </div>
              )}
