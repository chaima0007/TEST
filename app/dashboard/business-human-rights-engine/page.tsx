...
interface Entity {
  ...
  remedy_access_denial_score: number;
  regulatory_framework_gap_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_business_human_rights_index: number;
  last_updated: string;
}
...
interface Data {
  ...
  avg_estimated_business_human_rights_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "aperçu" && (
          <div className="space-y-3">
            {([
              { l: "Abus Chaîne d’Approvisionnement", v: entity.supply_chain_abuse_scale_score, c: "#1e40af" },
              { l: "Impunité Corporative", v: entity.corporate_impunity_score, c: "#1d4ed8" },
              { l: "Déni d’Accès aux Recours", v: entity.remedy_access_denial_score, c: "#2563eb" },
              { l: "Lacunes du Cadre Réglementaire", v: entity.regulatory_framework_gap_score, c: "#3b82f6" },
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Entreprises DR Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_business_human_rights_index}</span>
            </div>
        ...
        {tab === "contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Intervention d&apos;urgence pour les travailleurs victimes d&apos;abus dans les chaînes d&apos;approvisionnement sans recours disponible</p>
                <p>› Mobilisation des mécanismes de diligence raisonnable et des points de contact nationaux de l&apos;OCDE</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Documentation systématique des violations des droits humains par les entreprises et des obstacles aux recours</p>
                <p>› Plaidoyer pour l&apos;adoption de législations contraignantes sur la diligence raisonnable en matière de droits humains</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance des pratiques d&apos;entreprise et soutien aux mécanismes de plainte accessibles aux travailleurs</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur les évolutions réglementaires et prévention des lacunes dans les cadres de responsabilité</p>
            )}
          </div>
        )}
      </div>
...
export default function BusinessHumanRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/business-human-rights-engine")
      .then(r => r.json())
      .then(d => { setData(d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données Business Human Rights Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse des droits humains en entreprise…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Score Moyen", v: `${(data.avg_composite ?? 0).toFixed(1)}`, c: "text-blue-400" },
    { l: "Index Entreprises DR", v: `${(data.avg_estimated_business_human_rights_index ?? 0).toFixed(2)}`, c: "text-blue-300" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Entreprises &amp; Droits Humains</h1>
          <p className="text-slate-400 text-sm mt-1">Abus en chaîne d&apos;approvisionnement, impunité corporative et déni d&apos;accès aux recours</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {kpis.map(({ l, v, c }) => (
            <div key={l} className="bg-slate-900 border border-blue-800/20 rounded-xl p-4 text-center">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 border border-blue-800/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Entreprises &amp; Droits Humains Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("supply_chain_abuse_scale_score")} label="Abus Chaîne Appro." color={ACCENT} />
            <GaugeRing value={avgScore("corporate_impunity_score")} label="Impunité Corporative" color="#1d4ed8" />
            <GaugeRing value={avgScore("remedy_access_denial_score")} label="Déni de Recours" color="#2563eb" />
            <GaugeRing value={avgScore("regulatory_framework_gap_score")} label="Lacunes Réglementation" color="#3b82f6" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-blue-800/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns d&apos;Abus Corporatif</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#1e40af", "#1d4ed8", "#2563eb", "#3b82f6", "#60a5fa"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              className={`bg-slate-900 rounded-xl p-4 border cursor-pointer transition-all hover:border-blue-800/50 ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_business_human_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Business Human Rights Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
