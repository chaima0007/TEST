      ...
      <div style={{ background:"#0f172a",border:"1px solid #1e293b",borderRadius:12,padding:24,maxWidth:600,width:"90%",maxHeight:"80vh",overflowY:"auto" }}
        ...
        {tab==="apercu" && (
          <div style={{ color:"#cbd5e1",fontSize:14,lineHeight:1.7 }}>
            <div>Score composite : <strong style={{ color:accent }}>{entity?.composite_score}</strong></div>
            <div>Niveau : <strong style={{ color:accent }}>{entity?.risk_level}</strong></div>
            <div>Pays : {entity?.country}</div>
            <div>Pattern : {entity?.primary_pattern}</div>
          </div>
        )}
        {tab==="metriques" && (
          <div>
            {entity && Object.entries(entity)
              ...
              .map(([k,v]) => (
                <div key={k} style={{ display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid #1e293b",color:"#cbd5e1",fontSize:13 }}>
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/prison-conditions-rights-engine")
      .then(r => r.json())
  ...
  return (
    <div style={{ background:"#020617",minHeight:"100vh",color:"#f1f5f9",fontFamily:"system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth:1200,margin:"0 auto",padding:"32px 24px" }}>
        <h1 style={{ fontSize:28,fontWeight:700,color:ACCENT,marginBottom:8 }}>Conditions de Détention, Surpopulation &amp; Droits des Détenus</h1>
        <p style={{ color:"#94a3b8",marginBottom:32 }}>CaelumSwarm™ · CSDDD 2024/1760 · Wave 224</p>
        <div style={{ display:"flex",gap:16,marginBottom:32,flexWrap:"wrap" }}>
          <div style={{ background:"#0f172a",borderRadius:8,padding:20,flex:"1 1 160px",textAlign:"center" }}>
            <GaugeRing value={data.avg_composite} color={ACCENT} />
            <div style={{ color:"#94a3b8",fontSize:12,marginTop:8 }}>Score moyen</div>
            <div style={{ color:ACCENT,fontWeight:700,fontSize:18 }}>{data.avg_composite?.toFixed(2)}</div>
          </div>
          <div style={{ background:"#0f172a",borderRadius:8,padding:20,flex:"1 1 200px",display:"flex",flexDirection:"column",gap:12,justifyContent:"center" }}>
            <div style={{ display:"flex",justifyContent:"space-between" }}>
              <span style={{ color:"#94a3b8" }}>Entités</span><strong style={{ color:ACCENT }}>{data.total_entities}</strong>
            </div>
            <div style={{ display:"flex",justifyContent:"space-between" }}>
              <span style={{ color:"#94a3b8" }}>Critiques</span><strong style={{ color:"#ef4444" }}>{data.risk_distribution?.critique ?? 0}</strong>
            </div>
            <div style={{ display:"flex",justifyContent:"space-between" }}>
              <span style={{ color:"#94a3b8" }}>Confiance</span><strong style={{ color:"#22c55e" }}>{((data.confidence_score ?? 0.85)*100).toFixed(0)}%</strong>
            </div>
          </div>
        </div>
