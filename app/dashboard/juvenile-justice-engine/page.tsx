...
interface Entity {
  ...
  child_detention_scale_score: number;
  rehabilitation_failure_score: number;
  age_criminal_responsibility_score: number;
  torture_juvenile_detention_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_juvenile_justice_index: number;
  last_updated: string;
}
...
interface Data {
  ...
  avg_estimated_juvenile_justice_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Détention des Mineurs", v: entity.child_detention_scale_score, c: "#7c2d12" },
              { l: "Échec de Réhabilitation", v: entity.rehabilitation_failure_score, c: "#92400e" },
              { l: "Âge de Responsabilité Pénale", v: entity.age_criminal_responsibility_score, c: "#a16207" },
              { l: "Torture en Détention Juvénile", v: entity.torture_juvenile_detention_score, c: "#78350f" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Justice Juvénile Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_juvenile_justice_index}</span>
            </div>
        ...
        {tab === "contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Intervention d&apos;urgence pour les mineurs détenus dans des conditions de torture et sans accès à la réhabilitation</p>
                <p>› Mobilisation des mécanismes onusiens de protection de l&apos;enfance et des comités des droits de l&apos;enfant</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Documentation systématique des conditions de détention juvénile et des pratiques d&apos;abus</p>
                <p>› Plaidoyer pour le relèvement de l&apos;âge de responsabilité pénale et l&apos;accès à la justice réparatrice</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance des réformes législatives et soutien aux programmes de réhabilitation des mineurs</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur les évolutions normatives et prévention des régressions dans la justice juvénile</p>
            )}
          </div>
        )}
      </div>
...
export default function JuvenileJusticeEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/juvenile-justice-engine")
      .then(r => r.json())
      .then(d => { setData(d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données Justice Juvénile Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la justice juvénile…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Score Moyen", v: `${(data.avg_composite ?? 0).toFixed(1)}`, c: "text-orange-400" },
    { l: "Index Justice Juvénile", v: `${(data.avg_estimated_juvenile_justice_index ?? 0).toFixed(2)}`, c: "text-orange-300" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Justice Juvénile</h1>
          <p className="text-slate-400 text-sm mt-1">Détention des mineurs, échec de réhabilitation et torture en milieu juvénile</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {kpis.map(({ l, v, c }) => (
            <div key={l} className="bg-slate-900 border border-orange-900/20 rounded-xl p-4 text-center">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 border border-orange-900/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Justice Juvénile Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("child_detention_scale_score")} label="Détention Mineurs" color={ACCENT} />
            <GaugeRing value={avgScore("rehabilitation_failure_score")} label="Échec Réhabilitation" color="#92400e" />
            <GaugeRing value={avgScore("age_criminal_responsibility_score")} label="Âge Responsabilité" color="#a16207" />
            <GaugeRing value={avgScore("torture_juvenile_detention_score")} label="Torture Juvénile" color="#78350f" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-orange-900/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns de Justice Juvénile</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#7c2d12", "#92400e", "#a16207", "#78350f", "#431407"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              className={`bg-slate-900 rounded-xl p-4 border cursor-pointer transition-all hover:border-orange-900/50 ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_juvenile_justice_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Juvenile Justice Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
