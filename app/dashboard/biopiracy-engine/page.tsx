...
interface BiopirateEntity {
  entity_id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  genetic_appropriation_score: number;
  traditional_knowledge_score: number;
  patent_abuse_score: number;
  commercial_extraction_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_biopiracy_index: number;
  last_updated: string;
}

interface BiopirateySummary {
  ...
  data_sources: string[];
  entities: BiopirateEntity[];
  avg_estimated_biopiracy_index: number;
}

interface BiopirateData {
  entities: BiopirateEntity[];
  summary: BiopirateySummary;
}
...
function EntityCard({ entity, onClick }: { entity: BiopirateEntity; onClick: (e: BiopirateEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.genetic_appropriation_score}</p>
          <p className="text-[10px] text-gray-500">Génétique</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.traditional_knowledge_score}</p>
          <p className="text-[10px] text-gray-500">Savoirs</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.patent_abuse_score}</p>
          <p className="text-[10px] text-gray-500">Brevets</p>
        </div>
      </div>
      <div className="flex items-center justify-between text-xs">
        <span className="text-gray-400">Score: <span className={`font-bold ${cfg.color}`}>{entity.composite_score}</span></span>
        <span className="text-gray-500 truncate ml-2">{entity.primary_pattern.slice(0, 25)}…</span>
      </div>
    </button>
  );
}
...
function DetailModal({ entity, onClose }: { entity: BiopirateEntity; onClose: () => void }) {
  ...
  const actions: Record<string, string> = {
    "Appropriation de Ressource Génétique": "Lancer une procédure d'urgence auprès de l'OMPI et notifier les autorités douanières des pays concernés.",
    "Violation de Savoir Traditionnel": "Engager des experts juridiques autochtones et déposer plainte auprès du mécanisme CBD-Nagoya.",
    "Brevet Illégitime sur Biodiversité": "Contester le brevet devant l'OEB et initier une campagne de sensibilisation internationale.",
    "Extraction Commerciale Non-Autorisée": "Bloquer les exportations et exiger un partage équitable des avantages (APA).",
    "Surveillance Protocole de Nagoya": "Renforcer les audits de conformité APA et former les douanes locales à la détection.",
  };
          ...
          {tab === "scores" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Appropriation Génétique", value: `${entity.genetic_appropriation_score}/100` },
                  { label: "Savoir Traditionnel", value: `${entity.traditional_knowledge_score}/100` },
                  { label: "Abus Brevets", value: `${entity.patent_abuse_score}/100` },
                  { label: "Extraction Commerciale", value: `${entity.commercial_extraction_score}/100` },
                  { label: "Indice Biopiraterie", value: `${entity.estimated_biopiracy_index}/10` },
              ...
              <div className="bg-white/4 rounded-lg p-4">
                ...
                <p className="text-[11px] text-gray-500 mt-1.5">
                  Formule : génétique×0.30 + savoir×0.25 + brevet×0.25 + extraction×0.20
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "actions" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-300 leading-relaxed">{actions[entity.primary_pattern] ?? "Surveiller la conformité au Protocole de Nagoya et renforcer les contrôles APA."}</p>
              </div>
...
export default function BiopirateEnginePage() {
  const [data, setData] = useState<BiopirateData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<BiopirateEntity | null>(null);

  useEffect(() => {
    fetch("/api/biopiracy-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as BiopirateData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Intelligence Biopiraterie</h1>
            <p className="text-sm text-gray-400 mt-1">
              Moteur d&apos;intelligence Swarm — appropriation illégale de ressources génétiques et savoirs autochtones
            </p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              <KpiCard label="Total Entités" value={s.total_entities} sub="cas analysés" />
              ...
              <KpiCard label="Alertes Critiques" value={s.critical_alerts} sub="violation grave" accent="text-red-400" />
              <KpiCard label="Indice Biopiraterie" value={`${s.avg_estimated_biopiracy_index}/10`} sub="score normalisé" accent="text-emerald-400" />
              <KpiCard label="Confiance Analyse" value={`${s.confidence_score}%`} sub="fiabilité données" accent="text-indigo-400" />
              <KpiCard label="Sources Données" value={s.data_sources.length} sub="organismes internationaux" accent="text-teal-400" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Biopiraterie</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_biopiracy_index} max={10} label="Indice Biopir." color="#10b981" />
                </div>
              </div>
              ...
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                ...
                <div className="mt-5 pt-4 border-t border-white/8 grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-white/4 rounded-lg p-3">
                    ...
                    <p className="text-lg font-bold text-emerald-400 mt-0.5">{s.engine_version}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.critical_alerts} cas de biopiraterie critique détecté{s.critical_alerts > 1 ? "s" : ""}
                  </p>
                  <p className="text-xs text-red-400/70 mt-0.5">
                    Cas le plus grave : <span className="font-medium">{s.top_risk_entities[0]}</span>
                  </p>
                </div>
              </div>
            )}
