...
export default function MigrantWorkersRightsKafalaPage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#5c3317";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", "modéré": "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/migrant-workers-rights-kafala-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_migrant_workers_rights_kafala_index"] as number) ?? 0;
