...
interface Entity {
  ...
  official_denial_obstruction_score: number;
  victim_silencing_score: number;
  archive_destruction_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_right_to_truth_index: number;
  last_updated: string;
}
...
interface Data {
  ...
  avg_estimated_right_to_truth_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Absence de Commission Vérité", v: entity.truth_commission_absence_score, c: "#d97706" },
              { l: "Déni Officiel & Obstruction", v: entity.official_denial_obstruction_score, c: "#b45309" },
              { l: "Réduction au Silence des Victimes", v: entity.victim_silencing_score, c: "#92400e" },
              { l: "Destruction des Archives", v: entity.archive_destruction_score, c: "#78350f" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Droit à la Vérité Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_right_to_truth_index}</span>
            </div>
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Saisine urgente des mécanismes internationaux pour préserver les archives et protéger les témoins</p>
                <p>› Pression diplomatique maximale pour l&apos;établissement de processus de vérité et justice</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Documentation et préservation d&apos;urgence des preuves avant destruction par les autorités</p>
                <p>› Soutien aux victimes et témoins exposés aux représailles pour leur témoignage</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance des obstructions officielles et appui aux initiatives de mémoire collective</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur les tentatives de réécriture historique et soutien aux commissions vérité</p>
            )}
          </div>
        )}
      </div>
...
export default function RightToTruthEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-truth-engine")
      .then(r => r.json())
      .then(d => { setData(d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données Right to Truth Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse du droit à la vérité…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Indice Droit à la Vérité Moyen", v: `${(data.avg_estimated_right_to_truth_index ?? 0).toFixed(2)}`, c: "text-amber-300" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droit à la Vérité</h1>
          <p className="text-slate-400 text-sm mt-1">Déni officiel, obstruction mémorielle et réduction au silence des victimes</p>
        ...
        <div className="bg-slate-900 border border-amber-500/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Droit à la Vérité Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("truth_commission_absence_score")} label="Absence Commission" color={ACCENT} />
            <GaugeRing value={avgScore("official_denial_obstruction_score")} label="Déni Officiel" color="#b45309" />
            <GaugeRing value={avgScore("victim_silencing_score")} label="Réduction au Silence" color="#92400e" />
            <GaugeRing value={avgScore("archive_destruction_score")} label="Destruction Archives" color="#78350f" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-amber-500/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns d&apos;Obstruction Mémorielle</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#d97706", "#b45309", "#92400e", "#78350f", "#451a03"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_right_to_truth_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Right to Truth Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
