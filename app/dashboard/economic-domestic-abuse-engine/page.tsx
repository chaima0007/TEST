...
interface EDAEntity {
  ...
  financial_control_coercion_severity_score: number;
  legal_economic_protection_absence_score: number;
  asset_debt_exploitation_scale_score: number;
  economic_recovery_support_gap_score: number;
  estimated_economic_domestic_abuse_index: number;
  risk_level: string;
...
interface EDAData {
  ...
  data_sources: string[];
  entities: EDAEntity[];
  avg_estimated_economic_domestic_abuse_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function EconomicDomesticAbusePage() {
  const [data, setData] = useState<EDAData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<EDAEntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/economic-domestic-abuse-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données d&apos;abus économique…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "financial_control_coercion_severity_score", label: "Contrôle Financier" },
    { key: "legal_economic_protection_absence_score", label: "Protection Légale Absente" },
    { key: "asset_debt_exploitation_scale_score", label: "Exploitation Actifs/Dettes" },
    { key: "economic_recovery_support_gap_score", label: "Lacunes Soutien Rétablissement" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Abus Économique Conjugal</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Contrôle financier &amp; violence économique dans les relations intimes</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_economic_domestic_abuse_index ?? "—" },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Contrôle Économique</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#fb923c", "#f97316", "#eab308", "#ef4444", "#10b981"];
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-500">idx {entity.estimated_economic_domestic_abuse_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Abus Économique Conjugal · {data.last_analysis}</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Abus Économique</div>
                    <div className="text-lg font-bold text-orange-400">{selected.estimated_economic_domestic_abuse_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
