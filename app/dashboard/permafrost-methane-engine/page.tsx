...
type PMEEntity = {
  ...
  region: string;
  thaw_score: number;
  methane_score: number;
  feedback_score: number;
  governance_score: number;
  composite_score: number;
  risk_level: string;
  permafrost_pattern: string;
  severity: string;
  recommended_action: string;
  signal: string;
  permafrost_degradation_rate: number;
  methane_flux_intensity: number;
};
...
type PMESummary = {
  ...
  avg_estimated_permafrost_risk_index: number;
};
...
const PATTERN_COLORS: Record<string, string> = {
  none: "#16a34a",
  abrupt_thaw_collapse: "#dc2626",
  subsea_methane_eruption: "#7c3aed",
  tipping_point_cascade: "#b45309",
  infrastructure_sinkhole_crisis: "#0891b2",
  indigenous_territory_loss: "#059669",
};
const SEVERITY_COLORS: Record<string, string> = {
  "pergélisol_sous_surveillance": "#16a34a",
  "dégradation_pergélisol_structurelle": "#0891b2",
  "crise_climatique_arctique_majeure": "#1d4ed8",
  "crise_pergélisol_méthane_systémique": "#dc2626",
};
const ACTION_COLORS: Record<string, string> = {
  "veille_pergélisol_méthane_continue": "#16a34a",
  "renforcement_monitoring_pergélisol_régional": "#0891b2",
  "surveillance_renforcée_émissions_arctiques": "#1d4ed8",
  "intervention_urgente_pergélisol_méthane_critique": "#dc2626",
};
...
const SEV_BADGE: Record<string, string> = {
  "pergélisol_sous_surveillance": "bg-green-900 text-green-300",
  "dégradation_pergélisol_structurelle": "bg-cyan-900 text-cyan-300",
  "crise_climatique_arctique_majeure": "bg-blue-950 text-blue-400",
  "crise_pergélisol_méthane_systémique": "bg-red-950 text-red-400",
};
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <span className="ml-2 text-slate-500 text-xs">
              {entity.ecosystem_type.replace(/_/g, " ")}
            </span>
          </div>
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ["Score Dégel",      entity.thaw_score,      "#0891b2"],
              ["Score Méthane",    entity.methane_score,   "#7c3aed"],
              ["Score Rétroaction", entity.feedback_score, "#b45309"],
              ["Score Gouvernance", entity.governance_score, "#dc2626"],
            ].map(([l, v, c]) => (
              <div
                key={String(l)}
            ...
            <div className="col-span-2 bg-slate-900 border border-cyan-700/20 rounded-lg p-3">
              <div className="text-cyan-500/60 text-xs mb-1">Score Composite Pergélisol Méthane</div>
          ...
          <div className="bg-slate-900 border border-cyan-700/20 rounded-lg p-4 text-sm text-slate-200 leading-relaxed">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="bg-slate-800 rounded p-2">
                <div className="text-cyan-500/50 text-xs mb-0.5">Taux Dégradation Pergélisol</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.permafrost_degradation_rate * 100)}%
                </div>
              </div>
              <div className="bg-slate-800 rounded p-2">
                <div className="text-cyan-500/50 text-xs mb-0.5">Intensité Flux Méthane</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.methane_flux_intensity * 100)}%
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-900 border border-cyan-700/20 rounded-lg p-3">
              <div className="text-cyan-500/60 text-xs mb-1">Type d&apos;Écosystème</div>
            ...
            <div className="bg-slate-900 border border-cyan-700/20 rounded-lg p-3">
              ...
              <div className="text-cyan-400 font-medium capitalize">
                {entity.permafrost_pattern.replace(/_/g, " ")}
              </div>
            </div>
          </div>
        )}
...
export default function PermafrostMethaneDashboard() {
  ...
  useEffect(() => {
    fetch("/api/permafrost-methane-engine")
      .then(r => r.json())
      .then(setData)
      .catch(console.error);
  }, []);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-cyan-500 text-lg animate-pulse">
          Initialisation du Moteur Pergélisol & Méthane Arctique...
        </div>
      </div>
    );
  }

  const { entities, summary, avg_thaw } = data;
  const filtered = entities.filter(
    e =>
      (riskFilter === "all" || e.risk_level === riskFilter) &&
      (patFilter === "all" || e.permafrost_pattern === patFilter)
  );

  const dists = [
    { title: "Niveau de Risque",       counts: summary.risk_distribution,     colors: RISK_COLORS     },
    { title: "Patron Pergélisol",      counts: summary.pattern_distribution,  colors: PATTERN_COLORS  },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-cyan-500">
          Pergélisol &amp; Méthane Arctique — Module 433
        </h1>
        <p className="text-blue-500/60 text-sm mt-1">
          Dégel · Méthane · Rétroaction · Gouvernance — Caelum Partners — Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ["Total Écosystèmes",           summary.total,                                                              "text-cyan-500"],
          ["Crise Pergélisol",            summary.critical,                                                           "text-red-400"],
          ["Crise Arctique Majeure",      summary.high,                                                               "text-blue-400"],
          ["Composite Moyen",             `${summary.avg_composite.toFixed(1)}`,                                      "text-cyan-500"],
          ["Index Risque Pergélisol",     `${summary.avg_estimated_permafrost_risk_index.toFixed(2)}/10`,             "text-cyan-500"],
          ["Score Dégel Moyen",           `${avg_thaw.toFixed(1)}`,                                                   "text-blue-400"],
        ].map(([l, v, c]) => (
          <div
            key={String(l)}
      ...
      <div className="bg-slate-900 border border-cyan-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          <GaugeRing
            value={entities.reduce((s, e) => s + e.thaw_score, 0) / (entities.length || 1)}
            label="Dégel Moy."
            color="#0891b2"
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.methane_score, 0) / (entities.length || 1)}
            label="Méthane Moy."
            color="#7c3aed"
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.feedback_score, 0) / (entities.length || 1)}
            label="Rétroaction Moy."
            color="#b45309"
          />
          <GaugeRing
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {[
          "all",
          "none",
          "abrupt_thaw_collapse",
          "subsea_methane_eruption",
          "tipping_point_cascade",
          "infrastructure_sinkhole_crisis",
          "indigenous_territory_loss",
        ].map(p => (
          <button
            key={p}
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div className="text-xs text-cyan-500/60 mb-2 capitalize">
              {e.permafrost_pattern.replace(/_/g, " ")}
            </div>
            <div className="text-xs text-blue-400/70 font-medium mb-2">
              Dégel: {e.thaw_score.toFixed(1)} · Méthane: {e.methane_score.toFixed(1)}
            </div>
