...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/gender-based-violence-engine").then(r => r.json()).then(d => { setData(d); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Violence de Genre</h1>
        <p className="text-slate-400 mt-1">Gender Based Violence Engine — Analyse des risques de violence basée sur le genre</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
          <p className="text-slate-400 text-sm">Indice VBG Moyen</p>
          <p className="text-2xl font-bold mt-1" style={{ color: ACCENT }}>{loading ? "…" : data.avg_estimated_gender_based_violence_index?.toFixed(2) ?? "—"}</p>
