...
interface Entity {
  ...
  last_updated: string;
  climate_forced_migration_score?: number;
  sea_level_displacement_score?: number;
  drought_land_loss_score?: number;
  climate_refugee_protection_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "climate_forced_migration_score", label: "Migration forcée climatique" },
  { key: "sea_level_displacement_score", label: "Déplacement montée des eaux" },
  { key: "drought_land_loss_score", label: "Perte de terres par sécheresse" },
  { key: "climate_refugee_protection_gap_score", label: "Lacune protection réfugiés climatiques" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des déplacements climatiques et des violations des droits des populations déplacées. Les dimensions analysées couvrent la migration forcée par le climat, la montée des eaux, la perte de terres par sécheresse et les lacunes de protection des réfugiés climatiques.</p>
              <p className="text-slate-400">Source : Climate Displacement Rights Engine · Wave 242</p>
            </div>
          )}
        </div>
      </div>
...
export default function ClimateDisplacementRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/climate-displacement-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Climate Displacement Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Migration Climatique · Montée des Eaux · Sécheresse · Protection Réfugiés</p>
        </div>
