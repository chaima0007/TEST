...
interface IPREntity {
  ...
  severity: string;
  estimated_indigenous_peoples_rights_index: number;
...
const SEVERITY_STYLES: Record<string, { bg: string; color: string; label: string }> = {
  ...
  "élevé":  { bg: "#fef3c7", color: "#92400e", label: "Élevé" },
...
function EntityCard({ entity }: { entity: IPREntity }) {
  const idx = entity.estimated_indigenous_peoples_rights_index;
  return (
    <div
      style={{
        background: "#ffffff",
...
function SummaryBadges({ entities, avg }: { entities: IPREntity[]; avg: number }) {
...
export default function IndigenousPeoplesRightsPage() {
  const [data, setData] = useState<IPRData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/indigenous-peoples-rights-engine")
      .then((r) => r.json())
