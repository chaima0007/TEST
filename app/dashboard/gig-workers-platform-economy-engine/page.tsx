...
interface GWPEntity {
  ...
  severity: string;
  estimated_gig_workers_platform_economy_index: number;
...
const SEVERITY_STYLES: Record<string, { bg: string; color: string; label: string }> = {
  ...
  "élevé":  { bg: "#eff6ff", color: "#1d4ed8", label: "Élevé" },
  "modéré": { bg: "#f0f9ff", color: "#0369a1", label: "Modéré" },
...
function EntityCard({ entity }: { entity: GWPEntity }) {
  const idx = entity.estimated_gig_workers_platform_economy_index;
  return (
    <div
      style={{
        background: "#ffffff",
...
function SummaryBadges({ entities, avg }: { entities: GWPEntity[]; avg: number }) {
...
export default function GigWorkersPlatformEconomyPage() {
  const [data, setData] = useState<GWPData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/gig-workers-platform-economy-engine")
      .then((r) => r.json())
    ...
    <div style={{ padding: "24px 24px 48px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      >
        <div>
          ...
          >
            Économie Plateformes &amp; Travailleurs
          </h1>
