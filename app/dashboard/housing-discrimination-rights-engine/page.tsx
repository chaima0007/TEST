...
interface Entity {
  ...
  last_updated: string;
  racial_housing_discrimination_score?: number;
  eviction_abuse_score?: number;
  segregation_persistence_score?: number;
  housing_policy_bias_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "racial_housing_discrimination_score", label: "Discrimination raciale au logement" },
  { key: "eviction_abuse_score", label: "Abus d&apos;expulsion" },
  { key: "segregation_persistence_score", label: "Persistance de la ségrégation" },
  { key: "housing_policy_bias_score", label: "Biais des politiques de logement" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la discrimination dans le logement et des violations des droits liées à la ségrégation résidentielle, aux expulsions abusives et aux biais des politiques de logement.</p>
              <p className="text-slate-400">Source : Housing Discrimination Rights Engine · Wave 244</p>
            </div>
          )}
        </div>
      </div>
...
export default function HousingDiscriminationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/housing-discrimination-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Housing Discrimination Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Discrimination Raciale · Expulsions · Ségrégation · Politiques Logement</p>
        </div>
