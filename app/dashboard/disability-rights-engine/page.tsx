...
interface DREntity {
  ...
  composite_score: number; institutionalization_forced_score: number;
  employment_discrimination_score: number; accessibility_barrier_score: number;
  legal_capacity_denial_score: number; risk_level: string; primary_pattern: string;
  key_signals: string[]; estimated_disability_rights_index: number; last_updated: string;
}
interface DRSummary {
  ...
  confidence_score: number; data_sources: string[]; entities: DREntity[];
...
function DetailModal({ entity, onClose }: { entity: DREntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "Aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Institutionnalisation", v: entity.institutionalization_forced_score, c: "#ef4444" },
              { l: "Discrimination Emploi", v: entity.employment_discrimination_score, c: "#f97316" },
              { l: "Barrière Accessibilité", v: entity.accessibility_barrier_score, c: "#a855f7" },
              { l: "Capacité Juridique", v: entity.legal_capacity_denial_score, c: "#f59e0b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Droits Handicap</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_disability_rights_index}</span>
            </div>
          </div>
        )}
        {tab === "Signaux" && (
        ...
        {tab === "Contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Mécanismes d'urgence de protection des personnes handicapées institutionnalisées de force</p><p>› Pressions internationales pour l'application de la CDPH</p><p>› Programmes d'aide juridique pour les personnes privées de capacité juridique</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des lois anti-discrimination pour les personnes handicapées</p><p>› Réseaux de soutien à l'inclusion professionnelle et sociale</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance accrue des barrières à l'accessibilité</p><p>› Développement de plans d'action inclusifs conformes à la CDPH</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la vigilance sur les droits des personnes handicapées</p><p>› Consolidation des mécanismes de participation et d'autonomie</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  exclusion_systematique_handicap: "#ef4444",
  discrimination_institutionnelle: "#f97316",
  barrieres_accessibilite: "#f59e0b",
  tensions_inclusives: "#eab308",
  inclusion_exemplaire: "#10b981",
};

export default function DisabilityRightsEnginePage() {
  const [data, setData] = useState<DRSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<DREntity | null>(null);

  useEffect(() => {
    fetch("/api/disability-rights-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données droits handicap"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse des droits des personnes handicapées en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof DREntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits des Personnes Handicapées</h1>
          <p className="text-slate-400 text-sm mt-1">L'exclusion des personnes handicapées comme révélateur des fractures systémiques — quand l'accessibilité devient privilège et l'autonomie exception</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Index Moyen Handicap", v: data.avg_estimated_disability_rights_index, t: "text-cyan-400" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-cyan-500/20">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Dimensions de l'Exclusion — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("institutionalization_forced_score")} label="Institutionnalisation" color={ACCENT} />
            <GaugeRing value={avgScore("employment_discrimination_score")} label="Discrimination Emploi" color="#f97316" />
            <GaugeRing value={avgScore("accessibility_barrier_score")} label="Barrière Accessibilité" color="#a855f7" />
            <GaugeRing value={avgScore("legal_capacity_denial_score")} label="Capacité Juridique" color="#f59e0b" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-cyan-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns d'Exclusion du Handicap</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-cyan-900/10 border border-cyan-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Critiques — Droits Handicap</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_disability_rights_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droits Handicap · {data.last_analysis}</p>
      </div>
    </div>
  );
}
