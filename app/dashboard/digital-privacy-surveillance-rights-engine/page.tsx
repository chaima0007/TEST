...
interface Entity {
  ...
  mass_surveillance_biometric_dragnet_severity_score: number;
  internet_shutdown_censorship_scale_score: number;
  spyware_targeted_surveillance_journalist_score: number;
  data_protection_gdpr_enforcement_deficit_gap_score: number;
  estimated_digital_privacy_surveillance_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_digital_privacy_surveillance_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Surveillance de Masse & Biométrie", value: entity.mass_surveillance_biometric_dragnet_severity_score, weight: "0.30" },
    { label: "Coupures Internet & Censure", value: entity.internet_shutdown_censorship_scale_score, weight: "0.25" },
    { label: "Spyware & Surveillance Ciblée", value: entity.spyware_targeted_surveillance_journalist_score, weight: "0.25" },
    { label: "Déficit Protection Données", value: entity.data_protection_gdpr_enforcement_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_digital_privacy_surveillance_rights_index === "number" ? entity.estimated_digital_privacy_surveillance_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Vie Privée Numérique</div>
                </div>
              </div>
...
export default function DigitalPrivacySurveillanceRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/digital-privacy-surveillance-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Vie Privée Numérique…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_digital_privacy_surveillance_rights_index ?? avg(allEntities.map(e => e.estimated_digital_privacy_surveillance_rights_index));
  ...
  const avgSurveillanceMasse = avg(allEntities.map(e => e.mass_surveillance_biometric_dragnet_severity_score));
  const avgCoupuresInternet = avg(allEntities.map(e => e.internet_shutdown_censorship_scale_score));
  const avgSpyware = avg(allEntities.map(e => e.spyware_targeted_surveillance_journalist_score));
  const avgDeficitDonnees = avg(allEntities.map(e => e.data_protection_gdpr_enforcement_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Vie Privée Numérique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Vie Privée Numérique & Surveillance</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Digital Privacy & Surveillance Rights Engine — Surveillance de masse, censure internet et protection des données — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgSurveillanceMasse} label="Surveillance Masse" />
          <GaugeRing value={avgCoupuresInternet} label="Coupures Internet" />
          <GaugeRing value={avgSpyware} label="Spyware Ciblé" />
          <GaugeRing value={avgDeficitDonnees} label="Déficit Données" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Vie Privée Numérique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_digital_privacy_surveillance_rights_index === "number" ? e.estimated_digital_privacy_surveillance_rights_index.toFixed(2) : "—"}</span>
            </div>
