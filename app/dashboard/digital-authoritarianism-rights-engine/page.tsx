...
interface Entity {
  ...
  last_updated: string;
  internet_shutdown_score?: number;
  social_media_control_score?: number;
  digital_repression_score?: number;
  vpn_ban_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "internet_shutdown_score", label: "Coupures internet" },
  { key: "social_media_control_score", label: "Contrôle des réseaux sociaux" },
  { key: "digital_repression_score", label: "Répression numérique" },
  { key: "vpn_ban_score", label: "Interdiction de VPN" },
];
...
export default function DigitalAuthoritarianismRightsDashboard() {
  ...
  useEffect(() => {
    fetch("/api/digital-authoritarianism-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Digital Authoritarianism Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Coupures Internet · Contrôle Réseaux Sociaux · Répression Numérique · Interdiction VPN</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-[#1e3a5f]/40 text-[#3b6cb7]"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "rgba(30,58,95,0.25)" } : {}}>
