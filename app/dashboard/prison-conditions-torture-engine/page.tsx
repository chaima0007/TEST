...
interface Entity {
  ...
  estimated_prison_rights_index: number;
  risk_level: string;
  [key: string]: unknown;
}
...
export default function PrisonConditionsTortureDashboard() {
  const [data, setData] = useState<DashData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");

  useEffect(() => {
    fetch("/api/prison-conditions-torture-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Conditions Carcérales &amp; Torture…</div>
      </div>
    );
  }
  ...
  const avgIndex = avg(allEntities.map(e => e.estimated_prison_rights_index));
  ...
  const kpis = [
    ...
    { label: "Index Carcéral", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Conditions Carcérales &amp; Torture</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Prison Conditions &amp; Torture Engine — Analyse des violations dans les systèmes pénitentiaires mondiaux — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <GaugeRing value={avgIndex * 10} label="Index Carcéral ×10" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Carcéral: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_prison_rights_index === "number" ? e.estimated_prison_rights_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
