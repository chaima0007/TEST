...
export default function ChildSoldiersArmedRecruitmentDashboard() {
  const [data, setData] = useState<DomainData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch("/api/child-soldiers-armed-recruitment-engine")
      .then(r => r.json())
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm font-medium uppercase tracking-widest" style={{ color: ACCENT }}>
              Wave 170 — Enfants Soldats
            </span>
          </div>
          <h1 className="text-3xl font-bold text-white mb-1">
            Recrutement Armé d&apos;Enfants Soldats
          </h1>
          <p className="text-slate-400 text-sm">
            Monitoring &amp; évaluation du recrutement d&apos;enfants dans les conflits armés à l&apos;échelle mondiale
          </p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="mt-auto pt-2 border-t border-slate-800 flex justify-between items-center">
                  <span className="text-xs text-slate-500">Index Enfants Soldats</span>
                  <span className="text-sm font-bold" style={{ color: ACCENT }}>
                    {entity.estimated_child_soldiers_index.toFixed(2)}
                  </span>
                </div>
              </div>
            )
