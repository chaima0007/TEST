...
interface ConflictMineralEntity {
  id: string
  name: string
  composite_score: number
  risk_level: string
  estimated_conflict_mineral_index: number
}
interface ConflictMineralData {
  entities: ConflictMineralEntity[]
...
function EntityCard({ entity, onClick }: { entity: ConflictMineralEntity; onClick: (e: ConflictMineralEntity) => void }) {
  ...
  return (
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        ...
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.estimated_conflict_mineral_index}/10</p>
          <p className="text-[10px] text-gray-500">Indice minerais</p>
        </div>
      </div>
...
function DetailModal({ entity, onClose }: { entity: ConflictMineralEntity; onClose: () => void }) {
  const cfg = RISK_CONFIG[entity.risk_level] ?? RISK_CONFIG.faible
  return (
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: "Indice Minerais Conflit", value: `${entity.estimated_conflict_mineral_index}/10` },
          ...
          <div className="bg-white/4 rounded-lg p-4">
            ...
            <p className="text-[11px] text-gray-500 mt-1.5">
              Minerais finançant des conflits armés &amp; violations graves des droits humains
            </p>
          </div>
        </div>
      </div>
...
export default function ConflictMineralResourcesDashboard() {
  const [data, setData] = useState<ConflictMineralData | null>(null)
  const [loading, setLoading] = useState(true)
  const [filter, setFilter] = useState("tous")
  const [selected, setSelected] = useState<ConflictMineralEntity | null>(null)

  useEffect(() => {
    fetch("/api/conflict-mineral-resources-engine")
      .then((r) => r.json())
      .then((d) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Minerais de Conflit &amp; Ressources</h1>
            <p className="text-sm text-gray-400 mt-1">
              Exploitation minière finançant des conflits armés &amp; violations des droits humains
            </p>
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Minerais de Conflit</h2>
            ...
            {critiques > 0 && (
              <div className="rounded-xl px-5 py-4 flex items-start gap-3" style={{ backgroundColor: "rgba(249,115,22,0.08)", border: "1px solid rgba(249,115,22,0.2)" }}>
                <div className="w-2 h-2 rounded-full mt-1.5 shrink-0 animate-pulse" style={{ backgroundColor: ACCENT }} />
                <div>
                  <p className="text-sm font-medium" style={{ color: ACCENT }}>
                    {critiques} entité{critiques > 1 ? "s" : ""} en situation critique de minerais finançant des conflits
                  </p>
                  <p className="text-xs mt-0.5" style={{ color: `${ACCENT}b3` }}>
                    Intervention immédiate requise — traçabilité et certification des chaînes d&apos;approvisionnement prioritaires
                  </p>
                </div>
              </div>
            )}
