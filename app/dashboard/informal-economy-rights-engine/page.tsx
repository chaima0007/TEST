...
interface Entity {
  ...
  social_protection_exclusion_scale_score: number;
  legal_status_vulnerability_severity_score: number;
  workplace_safety_absence_pattern_score: number;
  income_instability_poverty_trap_score: number;
  risk_level: string;
...
const SUB_SCORES = [
  { key: "social_protection_exclusion_scale_score", label: "Exclusion Protection Sociale" },
  { key: "legal_status_vulnerability_severity_score", label: "Vulnérabilité Statut Légal" },
  { key: "workplace_safety_absence_pattern_score", label: "Sécurité Travail Absente" },
  { key: "income_instability_poverty_trap_score", label: "Piège Précarité Revenus" },
];
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/informal-economy-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-teal-400">Droits de l&apos;Économie Informelle</h1>
        <p className="text-slate-400 mt-1">Informal Economy Rights Engine</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold mt-1 text-teal-400">{data.total_entities}</p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold mt-1 text-teal-400">{data.avg_estimated_informal_economy_rights_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Confidence</p>
          <p className="text-3xl font-bold mt-1 text-teal-400">{Math.round((data.confidence_score ?? 0) * 100)}%</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f ? "border-transparent text-white bg-teal-500" : "border-slate-700 text-slate-400 hover:border-slate-500"}`}>
        ...
        {filtered.map(e => (
          <button key={e.entity_id} onClick={() => { setSelected(e); setTab(0); }}
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <span className="text-lg font-bold text-teal-400">{e.composite_score}</span>
            </div>
          </button>
        ))}
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Vue générale", "Sous-scores", "Alertes"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === i ? "text-white border-b-2 border-teal-400" : "text-slate-400"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-2xl font-bold text-teal-400">{selected.composite_score}/100</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Index économie informelle</span>
                    <span className="text-slate-200">{selected.estimated_informal_economy_rights_index}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  {SUB_SCORES.map(({ key, label }) => (
                    <div key={key}>
                      ...
                      <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                        <div className="h-full rounded-full bg-teal-400" style={{ width: `${selected[key] as number}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
