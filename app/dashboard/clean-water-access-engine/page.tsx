...
interface WaterEntity {
  ...
  infrastructure_failure_score: number;
  access_exclusion_score: number;
  risk_level: string;
...
function EntityCard({ entity, onClick }: { entity: WaterEntity; onClick: (e: WaterEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.water_stress_score}</p>
          <p className="text-[10px] text-gray-500">Stress</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.contamination_score}</p>
          <p className="text-[10px] text-gray-500">Contamin.</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.access_exclusion_score}</p>
          <p className="text-[10px] text-gray-500">Accès</p>
        </div>
      </div>
...
function DetailModal({ entity, onClose }: { entity: WaterEntity; onClose: () => void }) {
  ...
  const actions: Record<string, string> = {
    "Stress Hydrique Extrême": "Déployer des unités de purification d'urgence et activer les réserves stratégiques sous 48h.",
    "Contamination Critique de Source": "Isoler les sources contaminées et fournir une eau potable alternative immédiatement.",
    "Défaillance Infrastructurelle Majeure": "Engager des équipes de réhabilitation prioritaire et solliciter le financement de la Banque Mondiale.",
    "Exclusion d'Accès Populations Vulnérables": "Lancer des programmes d'accès équitable avec ciblage des ménages les plus pauvres.",
    "Risque Gouvernance Eau": "Renforcer les institutions de gestion des bassins versants et améliorer la tarification sociale.",
  };
          ...
          {tab === "scores" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Stress Hydrique", value: `${entity.water_stress_score}/100` },
                  { label: "Contamination", value: `${entity.contamination_score}/100` },
                  { label: "Défaillance Infra", value: `${entity.infrastructure_failure_score}/100` },
                  { label: "Exclusion Accès", value: `${entity.access_exclusion_score}/100` },
                  { label: "Indice Eau", value: `${entity.estimated_water_index}/10` },
              ...
              <div className="bg-white/4 rounded-lg p-4">
                ...
                <p className="text-[11px] text-gray-500 mt-1.5">
                  Formule : stress×0.30 + contamination×0.25 + infra×0.25 + accès×0.20
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "actions" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-300 leading-relaxed">{actions[entity.primary_pattern] ?? "Surveiller la qualité de l'eau et renforcer les infrastructures d'assainissement."}</p>
              </div>
...
export default function CleanWaterPage() {
  ...
  const [selected, setSelected] = useState<WaterEntity | null>(null);

  useEffect(() => {
    fetch("/api/clean-water-access-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as WaterData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Accès à l&apos;Eau Potable</h1>
            <p className="text-sm text-gray-400 mt-1">
              Moteur d&apos;intelligence Swarm — surveillance stress hydrique et accès à l&apos;eau propre
            </p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Indice Eau Propre" value={`${s.avg_estimated_water_index}/10`} sub="score normalisé" accent="text-blue-400" />
              <KpiCard label="Confiance Analyse" value={`${s.confidence_score}%`} sub="fiabilité données" accent="text-indigo-400" />
              <KpiCard label="Sources Données" value={s.data_sources.length} sub="ONU/OMS/FAO" accent="text-sky-400" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Hydriques</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_water_index} max={10} label="Indice Eau" color="#3b82f6" />
                </div>
              </div>
              ...
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                ...
                <div className="mt-5 pt-4 border-t border-white/8 grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-white/4 rounded-lg p-3">
                    ...
                    <p className="text-lg font-bold text-blue-400 mt-0.5">{s.engine_version}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.critical_alerts} zone{s.critical_alerts > 1 ? "s" : ""} en crise hydrique critique
                  </p>
                  <p className="text-xs text-red-400/70 mt-0.5">
                    Zone la plus critique : <span className="font-medium">{s.top_risk_entities[0]}</span>
                  </p>
                </div>
              </div>
            )}
