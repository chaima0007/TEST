...
interface OVEntity {
  ...
  physical_obstetric_abuse_severity_score: number;
  informed_consent_violation_scale_score: number;
  legal_accountability_gap_score: number;
  institutional_denial_minimization_pattern_score: number;
  estimated_obstetric_violence_rights_index: number;
  risk_level: string;
...
interface OVData {
  ...
  data_sources: string[];
  entities: OVEntity[];
  avg_estimated_obstetric_violence_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function ObstetricViolenceRightsPage() {
  const [data, setData] = useState<OVData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<OVEntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/obstetric-violence-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données de violence obstétricale…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "physical_obstetric_abuse_severity_score", label: "Abus Physique Obstétrical" },
    { key: "informed_consent_violation_scale_score", label: "Violation Consentement" },
    { key: "legal_accountability_gap_score", label: "Lacune Responsabilité Légale" },
    { key: "institutional_denial_minimization_pattern_score", label: "Déni Institutionnel" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Violence Obstétricale &amp; Droits</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Maltraitance maternité, consentement &amp; accountability médicale</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_obstetric_violence_rights_index ?? "—" },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Violence</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#f87171", "#f97316", "#eab308", "#ef4444", "#10b981"];
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-500">idx {entity.estimated_obstetric_violence_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Violence Obstétricale &amp; Droits · {data.last_analysis}</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Violence Obstétricale</div>
                    <div className="text-lg font-bold text-red-400">{selected.estimated_obstetric_violence_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
