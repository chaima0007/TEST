...
type SCEEntity = {
  ...
  signal: string;
  supply_visibility: number;
  forced_labor_risk: number;
};
...
type SCESummary = {
  ...
  avg_estimated_esg_transparency_index: number;
};
...
const PATTERN_COLORS: Record<string, string> = {
  none: "#16a34a",
  forced_labor_opacity: "#dc2626",
  greenwashing_deception: "#7c3aed",
  conflict_mineral_complicity: "#b45309",
  carbon_laundering_scheme: "#0891b2",
  supplier_audit_capture: "#be185d",
};
const SEVERITY_COLORS: Record<string, string> = {
  "transparence_esg_sous_surveillance": "#16a34a",
  "non_conformité_esg_structurelle": "#d97706",
  "risque_esg_majeur_détecté": "#92400e",
  "crise_opacité_chaîne_approvisionnement_critique": "#dc2626",
};
const ACTION_COLORS: Record<string, string> = {
  "veille_transparence_chaîne_approvisionnement": "#16a34a",
  "mise_en_conformité_esg_progressive": "#d97706",
  "renforcement_contrôle_fournisseurs_tier2_tier3": "#92400e",
  "audit_urgence_chaîne_approvisionnement_intégrale": "#dc2626",
};
...
const SEV_BADGE: Record<string, string> = {
  "transparence_esg_sous_surveillance": "bg-green-900 text-green-300",
  "non_conformité_esg_structurelle": "bg-amber-900 text-amber-300",
  "risque_esg_majeur_détecté": "bg-amber-950 text-amber-500",
  "crise_opacité_chaîne_approvisionnement_critique": "bg-red-950 text-red-400",
};
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <span className="ml-2 text-slate-500 text-xs">
              {entity.industry_type.replace(/_/g, " ")}
            </span>
          </div>
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ["Score Visibilité",      entity.visibility_score,    "#d97706"],
              ["Score Travail",         entity.labor_score,         "#dc2626"],
              ["Score Environnement",   entity.environmental_score, "#059669"],
            ...
            <div className="col-span-2 bg-slate-900 border border-green-700/20 rounded-lg p-3">
              <div className="text-green-500/60 text-xs mb-1">Score Composite ESG</div>
          ...
          <div className="bg-slate-900 border border-green-700/20 rounded-lg p-4 text-sm text-slate-200 leading-relaxed">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="bg-slate-800 rounded p-2">
                <div className="text-green-500/50 text-xs mb-0.5">Visibilité Chaîne</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.supply_visibility * 100)}%
                </div>
              </div>
              <div className="bg-slate-800 rounded p-2">
                <div className="text-green-500/50 text-xs mb-0.5">Risque Travail Forcé</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.forced_labor_risk * 100)}%
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-900 border border-green-700/20 rounded-lg p-3">
              <div className="text-green-500/60 text-xs mb-1">Type d&apos;Industrie</div>
              <div className="text-white font-medium capitalize">
                {entity.industry_type.replace(/_/g, " ")}
              </div>
            </div>
            <div className="bg-slate-900 border border-green-700/20 rounded-lg p-3">
              <div className="text-green-500/60 text-xs mb-1">Patron ESG Détecté</div>
              <div className="text-green-400 font-medium capitalize">
                {entity.esg_pattern.replace(/_/g, " ")}
              </div>
            </div>
          </div>
        )}
...
export default function SupplyChainESGDashboard() {
  ...
  useEffect(() => {
    fetch("/api/supply-chain-esg-engine")
      .then(r => r.json())
      .then(setData)
      .catch(console.error);
  }, []);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-green-500 text-lg animate-pulse">
          Initialisation du Moteur ESG Chaîne d&apos;Approvisionnement...
        </div>
      </div>
    );
  }
  ...
  const filtered = entities.filter(
    e =>
      ...
      (patFilter === "all" || e.esg_pattern === patFilter)
  );

  const dists = [
    { title: "Niveau de Risque ESG",       counts: summary.risk_distribution,     colors: RISK_COLORS     },
    { title: "Patron ESG Détecté",          counts: summary.pattern_distribution,  colors: PATTERN_COLORS  },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-green-500">
          Transparence Chaîne d&apos;Approvisionnement ESG — Module 397
        </h1>
        <p className="text-amber-500/60 text-sm mt-1">
          Visibilité · Travail · Environnement · Gouvernance — Caelum Partners — Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ["Total Entités",                 summary.total,                                                                    "text-green-500"],
          ["Crise ESG Critique",            summary.critical,                                                                 "text-red-400"],
          ["Risque Majeur",                 summary.high,                                                                     "text-amber-700"],
          ["Composite Moyen",               `${summary.avg_composite.toFixed(1)}`,                                            "text-amber-500"],
          ["Index Transparence ESG",        `${summary.avg_estimated_esg_transparency_index.toFixed(2)}/10`,                  "text-green-500"],
      ...
      <div className="bg-slate-900 border border-green-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          <GaugeRing
            value={entities.reduce((s, e) => s + e.visibility_score, 0) / (entities.length || 1)}
            label="Visibilité Moy."
            color="#d97706"
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.labor_score, 0) / (entities.length || 1)}
            label="Travail Moy."
            color="#dc2626"
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.environmental_score, 0) / (entities.length || 1)}
            label="Environnement Moy."
            color="#059669"
          />
          <GaugeRing
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {[
          "all",
          "none",
          "forced_labor_opacity",
          "greenwashing_deception",
          "conflict_mineral_complicity",
          "carbon_laundering_scheme",
          "supplier_audit_capture",
        ].map(p => (
          <button
            key={p}
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div className="text-xs text-green-500/60 mb-2 capitalize">
              {e.esg_pattern.replace(/_/g, " ")}
            </div>
            <div className="text-xs text-amber-500/70 font-medium mb-2">
              Vis: {e.visibility_score.toFixed(1)} · Trav: {e.labor_score.toFixed(1)}
            </div>
