...
interface AREntity {
  ...
  ritual_killing_body_parts_trade_score: number;
  legal_protection_enforcement_gap_score: number;
  discrimination_exclusion_severity_score: number;
  institutional_abandonment_scale_score: number;
  estimated_albinism_rights_index: number;
  risk_level: string;
...
interface ARData {
  ...
  data_sources: string[];
  entities: AREntity[];
  avg_estimated_albinism_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function AlbinismRightsPage() {
  const [data, setData] = useState<ARData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<AREntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/albinism-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données sur les droits des personnes albinos…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "ritual_killing_body_parts_trade_score", label: "Meurtres Rituels Organes" },
    { key: "legal_protection_enforcement_gap_score", label: "Lacune Protection Légale" },
    { key: "discrimination_exclusion_severity_score", label: "Discrimination & Exclusion" },
    { key: "institutional_abandonment_scale_score", label: "Abandon Institutionnel" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits des Personnes Albinos</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Albinism Rights Engine</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_albinism_rights_index ?? "—" },
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#8b5cf6" />
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            ...
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#8b5cf6", "#f97316", "#eab308", "#ef4444", "#10b981"];
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#8b5cf6" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_albinism_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droits des Personnes Albinos · {data.last_analysis}</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#8b5cf6" />
                    <div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Droits Albinos</div>
                    <div className="text-lg font-bold text-violet-400">{selected.estimated_albinism_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
