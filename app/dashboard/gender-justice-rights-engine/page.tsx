...
interface Entity {
  ...
  last_updated: string;
  gender_based_violence_impunity_score?: number;
  femicide_rate_score?: number;
  legal_gender_discrimination_score?: number;
  gender_justice_access_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "gender_based_violence_impunity_score", label: "Impunité violences basées sur le genre" },
  { key: "femicide_rate_score", label: "Taux de fémicide" },
  { key: "legal_gender_discrimination_score", label: "Discrimination juridique de genre" },
  { key: "gender_justice_access_gap_score", label: "Lacune d&apos;accès à la justice de genre" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits à la justice de genre : impunité des violences basées sur le genre, taux de fémicide, discriminations juridiques et lacunes d&apos;accès à la justice pour les femmes.</p>
              <p className="text-slate-400">Source : Gender Justice Rights Engine · Wave 255</p>
            </div>
          )}
        </div>
      </div>
...
export default function GenderJusticeRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/gender-justice-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Gender Justice Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Violences de Genre · Fémicide · Discrimination Juridique · Accès à la Justice</p>
        </div>
