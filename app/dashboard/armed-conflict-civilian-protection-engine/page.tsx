...
interface Entity {
  ...
  civilian_targeting_indiscriminate_attack_severity_score: number;
  siege_starvation_collective_punishment_scale_score: number;
  hospital_school_infrastructure_attack_score: number;
  humanitarian_access_aid_blockage_deficit_gap_score: number;
  estimated_armed_conflict_civilian_protection_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_armed_conflict_civilian_protection_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Ciblage Civils & Attaques Indiscriminées", value: entity.civilian_targeting_indiscriminate_attack_severity_score, weight: "0.30" },
    { label: "Siège, Famine & Punition Collective", value: entity.siege_starvation_collective_punishment_scale_score, weight: "0.25" },
    { label: "Attaques Hôpitaux, Écoles & Infrastructure", value: entity.hospital_school_infrastructure_attack_score, weight: "0.25" },
    { label: "Déficit Accès Humanitaire & Blocage Aide", value: entity.humanitarian_access_aid_blockage_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_armed_conflict_civilian_protection_index === "number" ? entity.estimated_armed_conflict_civilian_protection_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Protection Civils</div>
                </div>
              </div>
...
export default function ArmedConflictCivilianProtectionEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/armed-conflict-civilian-protection-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Protection des Civils en Conflit Armé…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_armed_conflict_civilian_protection_index ?? avg(allEntities.map(e => e.estimated_armed_conflict_civilian_protection_index));
  ...
  const avgCivilianTargeting = avg(allEntities.map(e => e.civilian_targeting_indiscriminate_attack_severity_score));
  const avgSiege = avg(allEntities.map(e => e.siege_starvation_collective_punishment_scale_score));
  const avgHospital = avg(allEntities.map(e => e.hospital_school_infrastructure_attack_score));
  const avgHumanitarian = avg(allEntities.map(e => e.humanitarian_access_aid_blockage_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Protection Civils", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Protection des Civils en Conflit Armé</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Armed Conflict Civilian Protection Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgCivilianTargeting} label="Ciblage Civils" />
          <GaugeRing value={avgSiege} label="Siège & Famine" />
          <GaugeRing value={avgHospital} label="Attaques Infrastructure" />
          <GaugeRing value={avgHumanitarian} label="Accès Humanitaire" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Protection Civils: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_armed_conflict_civilian_protection_index === "number" ? e.estimated_armed_conflict_civilian_protection_index.toFixed(2) : "—"}</span>
            </div>
