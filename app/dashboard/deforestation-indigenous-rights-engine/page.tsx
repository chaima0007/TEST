...
interface DIREntity {
  ...
  forest_cover_loss_rate_score: number;
  indigenous_land_dispossession_score: number;
  land_defender_killings_score: number;
  legal_protection_absence_score: number;
  estimated_deforestation_indigenous_rights_index: number;
  risk_level: string;
...
interface DIRData {
  ...
  data_sources: string[];
  entities: DIREntity[];
  avg_estimated_deforestation_indigenous_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function DeforestationIndigenousRightsPage() {
  const [data, setData] = useState<DIRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<DIREntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/deforestation-indigenous-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-emerald-400 animate-pulse">Chargement des données déforestation…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "forest_cover_loss_rate_score", label: "Perte Couvert Forestier" },
    { key: "indigenous_land_dispossession_score", label: "Dépossession Terres" },
    { key: "land_defender_killings_score", label: "Défenseurs Tués" },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: ACCENT, boxShadow: `0 0 6px ${ACCENT}` }} />
          <h1 className="text-2xl font-bold tracking-tight">Déforestation &amp; Droits des Peuples Autochtones</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Amazonie · Borneo · Congo · Défenseurs terres tués · Global Witness</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_deforestation_indigenous_rights_index ?? "—" },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Déforestation</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#34d399", "#10b981", "#059669", "#6ee7b7", "#a7f3d0"];
        ...
        <div className="flex gap-2 flex-wrap">
          {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-full text-sm border transition-all capitalize ${filter === f ? "bg-emerald-600 border-emerald-500 text-white" : "bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-500"}`}>
              {f}
            </button>
          ))}
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-500">idx {entity.estimated_deforestation_indigenous_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Déforestation &amp; Droits Autochtones · {data.last_analysis}</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Déforestation &amp; Droits Autochtones</div>
                    <div className="text-lg font-bold text-emerald-400">{selected.estimated_deforestation_indigenous_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
