...
const COLOR = "#0369a1";
const DOMAIN = "crossdeviceadvertising-advertising";
const INDEX_KEY = "estimated_crossdeviceadvertising_index";
const TITLE = "Cross-Device Advertising";

const SIGNALS = [
  "Device graph consent",
  "Identity resolution limits",
  "Cross-device tracking opt-out",
  "Probabilistic matching controls",
  "Deterministic ID compliance",
];

const ACTIONS = [
  "Enforce device graph consent",
  "Limit identity resolution",
  "Strengthen cross-device opt-out",
  "Audit probabilistic matching",
  "Review deterministic ID use",
];
...
function deriveCerts(entity: Entity): string[] {
  const idx = (entity[INDEX_KEY] as number) ?? 0;
  return [
    `Score composite validé : ${entity.composite_score.toFixed(2)} / 100`,
    `Niveau de risque cross-device advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${idx.toFixed(2)} / 10`,
    `Conformité cross-device : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 416 — Cross-Device Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>{TITLE} · Wave 416</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: idx * 10, color: COLOR },
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            {ACTIONS.map((action, i) => (
              <div key={i} style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: `1px solid ${COLOR}26` }}>
            ...
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: `1px solid ${COLOR}26` }}>
              ...
              <div style={{ color: COLOR, fontWeight: 700, fontSize: 18 }}>{idx.toFixed(2)} / 10</div>
            </div>
          </div>
        )}
      </div>
...
export default function CrossdeviceadvertisingAdvertisingDashboard() {
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + ((e[INDEX_KEY] as number) ?? 0), 0) / entities.length
    : 0;
    ...
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 13, color: `${COLOR}99`, margin: 0 }}>
          Caelum Partners · Wave 416 · Analyse publicitaire cross-device · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
                ...
                <span style={{ color: COLOR, fontWeight: 600 }}>{idx.toFixed(2)}</span>
              </div>
            </div>
          );
        })}
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · {TITLE} Intelligence · Wave 416
      </div>
