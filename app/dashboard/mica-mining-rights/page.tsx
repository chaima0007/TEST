...
interface Entity {
  ...
  child_labor_mica_extraction_severity_score: number;
  hazardous_dust_lung_disease_exposure_score: number;
  supply_chain_opacity_accountability_deficit_score: number;
  community_displacement_mining_concession_score: number;
  estimated_mica_mining_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_mica_mining_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Travail des Enfants dans l&apos;Extraction du Mica", value: entity.child_labor_mica_extraction_severity_score, weight: "0.30" },
    { label: "Exposition Poussières & Maladies Pulmonaires", value: entity.hazardous_dust_lung_disease_exposure_score, weight: "0.25" },
    { label: "Opacité Chaîne d&apos;Approvisionnement", value: entity.supply_chain_opacity_accountability_deficit_score, weight: "0.25" },
    { label: "Déplacement Communautaire & Concessions", value: entity.community_displacement_mining_concession_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_mica_mining_rights_index === "number" ? entity.estimated_mica_mining_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Extraction Mica</div>
                </div>
              </div>
              ...
              {subScores.map((s) => (
                <div key={s.label} className="bg-slate-800/50 rounded-lg p-3">
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="text-sm text-slate-300" dangerouslySetInnerHTML={{ __html: s.label }} />
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-sm text-slate-300 space-y-1">
                  <div>Travail des Enfants Mica × 0.30</div>
                  <div>Exposition Poussières Pulmonaires × 0.25</div>
                  <div>Opacité Chaîne Approvisionnement × 0.25</div>
                  <div>Déplacement Communautaire × 0.20</div>
                </div>
              </div>
...
export default function MicaMiningRightsPage() {
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits dans l&apos;Extraction du Mica…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_mica_mining_rights_index ?? avg(allEntities.map(e => e.estimated_mica_mining_rights_index));
  ...
  const avgChildLabor = avg(allEntities.map(e => e.child_labor_mica_extraction_severity_score));
  const avgDust = avg(allEntities.map(e => e.hazardous_dust_lung_disease_exposure_score));
  const avgOpacity = avg(allEntities.map(e => e.supply_chain_opacity_accountability_deficit_score));
  const avgDisplacement = avg(allEntities.map(e => e.community_displacement_mining_concession_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Extraction Mica", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits dans l&apos;Extraction du Mica</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Mica Mining Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgChildLabor} label="Travail Enfants Mica" />
          <GaugeRing value={avgDust} label="Exposition Poussières" />
          <GaugeRing value={avgOpacity} label="Opacité Chaîne" />
          <GaugeRing value={avgDisplacement} label="Déplacement Communautaire" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Extraction Mica: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_mica_mining_rights_index === "number" ? e.estimated_mica_mining_rights_index.toFixed(2) : "—"}</span>
            </div>
