...
interface Entity {
  ...
  statelessness_documentation_denial_severity_score: number;
  arbitrary_detention_deportation_stateless_scale_score: number;
  birth_registration_nationality_access_gap_score: number;
  stateless_children_education_rights_deficit_gap_score: number;
  estimated_stateless_persons_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_stateless_persons_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déni Documentation & Apatridie", value: entity.statelessness_documentation_denial_severity_score, weight: "0.30" },
    { label: "Détention Arbitraire & Expulsion", value: entity.arbitrary_detention_deportation_stateless_scale_score, weight: "0.25" },
    { label: "Enregistrement Naissance & Nationalité", value: entity.birth_registration_nationality_access_gap_score, weight: "0.25" },
    { label: "Déficit Éducation Enfants Apatrides", value: entity.stateless_children_education_rights_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_stateless_persons_rights_index === "number" ? entity.estimated_stateless_persons_rights_index.toFixed(2) : "—"}</div>
...
export default function StatelessPersonsRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/stateless-persons-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Apatridie & Droits des Personnes Apatrides…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_stateless_persons_rights_index ?? avg(allEntities.map(e => e.estimated_stateless_persons_rights_index));
  ...
  const avgDocDenial = avg(allEntities.map(e => e.statelessness_documentation_denial_severity_score));
  const avgDetention = avg(allEntities.map(e => e.arbitrary_detention_deportation_stateless_scale_score));
  const avgBirthReg = avg(allEntities.map(e => e.birth_registration_nationality_access_gap_score));
  const avgChildrenEdu = avg(allEntities.map(e => e.stateless_children_education_rights_deficit_gap_score));
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Apatridie & Droits des Personnes Apatrides</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Stateless Persons Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgDocDenial} label="Déni Documentation" />
          <GaugeRing value={avgDetention} label="Détention Arbitraire" />
          <GaugeRing value={avgBirthReg} label="Enregistrement Naissance" />
          <GaugeRing value={avgChildrenEdu} label="Éducation Enfants" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Apatridie: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_stateless_persons_rights_index === "number" ? e.estimated_stateless_persons_rights_index.toFixed(2) : "—"}</span>
            </div>
