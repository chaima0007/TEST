...
interface Entity {
  ...
  extractive_industry_territory_invasion_severity_score: number;
  fpic_violation_forced_displacement_scale_score: number;
  indigenous_environmental_defender_killing_score: number;
  legal_title_recognition_land_rights_deficit_gap_score: number;
  estimated_indigenous_land_rights_extraction_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_indigenous_land_rights_extraction_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Invasion Territoire par Industries Extractives", value: entity.extractive_industry_territory_invasion_severity_score, weight: "0.30" },
    { label: "Violation CPLI & Déplacement Forcé", value: entity.fpic_violation_forced_displacement_scale_score, weight: "0.25" },
    { label: "Meurtres Défenseurs Autochtones Env.", value: entity.indigenous_environmental_defender_killing_score, weight: "0.25" },
    { label: "Déficit Reconnaissance Titres Fonciers", value: entity.legal_title_recognition_land_rights_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_indigenous_land_rights_extraction_index === "number" ? entity.estimated_indigenous_land_rights_extraction_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Fonciers Autochtones</div>
                </div>
              </div>
...
export default function IndigenousLandRightsExtractionEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/indigenous-land-rights-extraction-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Fonciers Autochtones & Extraction…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_indigenous_land_rights_extraction_index ?? avg(allEntities.map(e => e.estimated_indigenous_land_rights_extraction_index));
  ...
  const avgExtractiveInvasion = avg(allEntities.map(e => e.extractive_industry_territory_invasion_severity_score));
  const avgFpicViolation = avg(allEntities.map(e => e.fpic_violation_forced_displacement_scale_score));
  const avgDefenderKilling = avg(allEntities.map(e => e.indigenous_environmental_defender_killing_score));
  const avgLegalTitle = avg(allEntities.map(e => e.legal_title_recognition_land_rights_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Fonciers Autochtones", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Fonciers Autochtones & Extraction</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Indigenous Land Rights Extraction Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgExtractiveInvasion} label="Invasion Extractive" />
          <GaugeRing value={avgFpicViolation} label="Violation CPLI" />
          <GaugeRing value={avgDefenderKilling} label="Meurtres Défenseurs" />
          <GaugeRing value={avgLegalTitle} label="Déficit Titres Fonciers" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Fonciers Autochtones: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_indigenous_land_rights_extraction_index === "number" ? e.estimated_indigenous_land_rights_extraction_index.toFixed(2) : "—"}</span>
            </div>
