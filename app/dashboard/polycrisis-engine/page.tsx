...
interface PCEntity {
  ...
  composite_score: number; crisis_density_score: number;
  cascade_velocity_score: number; institutional_resilience_gap_score: number;
  global_contagion_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_polycrisis_index: number; last_updated: string;
}
interface PCSummary {
  ...
  entities: PCEntity[]; avg_estimated_polycrisis_index: number;
}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Densité de Crises", v: entity.crisis_density_score, c: "#ef4444" },
              { l: "Vélocité de Cascade", v: entity.cascade_velocity_score, c: "#f97316" },
              { l: "Gap Résilience Inst.", v: entity.institutional_resilience_gap_score, c: "#eab308" },
              { l: "Contagion Globale", v: entity.global_contagion_score, c: "#8b5cf6" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Polycrise</span>
              <span className="text-cyan-400 font-bold">{entity.estimated_polycrisis_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Activation cellule crise inter-ministérielle immédiate</p><p>› Coordination ONU d'urgence — protocole OCHA</p><p>› Révision plans de résilience systémique sous 48h</p></>}
            {entity.risk_level === "élevé" && <><p>› Déploiement ressources préventives dans zones convergentes</p><p>› Renforcement capacités d'absorption régionales</p></>}
            {entity.risk_level === "modéré" && <><p>› Renforcement de la redondance institutionnelle</p><p>› Mise à jour des scenarii de risque polycrise</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille polycrise continue</p><p>› Partage des meilleures pratiques de résilience</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function PolycrisesPage() {
  ...
  useEffect(() => {
    fetch("/api/polycrisis-engine")
      .then(r => r.json())
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données polycrise"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse polycrise globale…</div></div>;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Polycrise Mondiale — Intelligence de Cascade</h1>
          <p className="text-slate-400 text-sm">Convergence des crises systémiques mondiales · v{data.engine_version}</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { l: "Régions Analysées", v: data.total_entities, g: "from-slate-700 to-slate-600", t: "text-white" },
            { l: "Score Moyen", v: `${data.avg_composite}%`, g: "from-cyan-900 to-cyan-800", t: "text-cyan-300" },
            { l: "Alertes Critiques", v: data.critical_alerts.length, g: "from-red-900 to-red-800", t: "text-red-300" },
            { l: "Index Polycrise", v: data.avg_estimated_polycrisis_index, g: "from-purple-900 to-purple-800", t: "text-purple-300" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Indicateurs Polycrise Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("crisis_density_score")} label="Densité Crises" color="#ef4444" />
            <GaugeRing value={avgScore("cascade_velocity_score")} label="Vélocité Cascade" color="#f97316" />
            <GaugeRing value={avgScore("institutional_resilience_gap_score")} label="Gap Résilience" color="#eab308" />
            <GaugeRing value={avgScore("global_contagion_score")} label="Contagion Globale" color="#8b5cf6" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Cascade</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-red-900/10 border border-red-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-red-400 mb-2">Alertes Polycrise Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_polycrisis_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Polycrise Mondiale · {data.last_analysis}</p>
      </div>
    </div>
  );
}
