...
interface Entity {
  ...
  risk_level: string
  estimated_modern_slavery_index: number
}
...
export default function ModernSlaveryForcedLaborPage() {
  const [data, setData] = useState<DomainData | null>(null)

  useEffect(() => {
    fetch("/api/modern-slavery-forced-labor-contemporary-engine")
      .then((r) => r.json())
    ...
    <div style={{ background: "#0f172a", minHeight: "100vh", padding: "32px 24px", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "#f1f5f9" }}>Esclavage Moderne &amp; Travail Forcé Contemporain</h1>
          ...
          <div key={entity.id} style={{ background: "#1e293b", borderRadius: 12, padding: 22, display: "flex", gap: 20, alignItems: "flex-start", borderLeft: `4px solid ${ACCENT}` }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", gap: 16 }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: ACCENT }}>{entity.estimated_modern_slavery_index.toFixed(2)}</div>
                  <div style={{ fontSize: 11, color: "#64748b" }}>Indice esclavage</div>
                </div>
                <div>
