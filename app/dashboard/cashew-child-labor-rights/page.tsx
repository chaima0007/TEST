...
interface EntityResult {
  ...
  risk_level: string;
  estimated_cashew_child_labor_index: number;
}
...
export default function CashewChildLaborDashboard() {
  const [data, setData] = useState<EngineData | null>(null);
  const [selected, setSelected] = useState<EntityResult | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/cashew-child-labor-rights")
      .then(r => r.json())
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 32, fontFamily: "sans-serif" }}>
      <h1 style={{ fontSize: 28, fontWeight: 800, marginBottom: 8 }}>Droits Enfants Travail Anacarde</h1>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 32, maxWidth: 480, width: "90%", maxHeight: "80vh", overflow: "auto" }}
            ...
            {tab === 1 && (
              <div>
                ...
                <p>Index estimé : {(selected as Record<string, unknown>)["estimated_cashew_child_labor_index"] as number ?? "N/A"}</p>
              </div>
            )}
            {tab === 2 && (
              <div>
