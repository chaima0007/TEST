...
interface Entity {
  ...
  last_updated: string;
  open_access_denial_score?: number;
  research_censorship_score?: number;
  technology_transfer_block_score?: number;
  scientist_persecution_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "open_access_denial_score", label: "Refus d&apos;accès ouvert" },
  { key: "research_censorship_score", label: "Censure de la recherche" },
  { key: "technology_transfer_block_score", label: "Blocage des transferts technologiques" },
  { key: "scientist_persecution_score", label: "Persécution des scientifiques" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse du droit à la science : refus d&apos;accès ouvert, censure de la recherche, blocage des transferts technologiques et persécution des scientifiques.</p>
              <p className="text-slate-400">Source : Right to Science Rights Engine · Wave 248</p>
            </div>
          )}
        </div>
      </div>
...
export default function RightToScienceRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-science-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Right to Science Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Accès Ouvert · Censure · Transferts Technologiques · Persécution</p>
        </div>
