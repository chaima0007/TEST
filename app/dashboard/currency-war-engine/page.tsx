...
interface CWEntity {
  ...
  composite_score: number; dedollarization_strategy_score: number;
  currency_manipulation_score: number; cbdc_geopolitical_score: number;
  gold_reserve_weaponization_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_currency_war_index: number; last_updated: string;
}
interface CWSummary {
  ...
  entities: CWEntity[]; avg_estimated_currency_war_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: CWEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Stratégie de Dédollarisation", v: entity.dedollarization_strategy_score, c: "#eab308" },
              { l: "Manipulation Monétaire", v: entity.currency_manipulation_score, c: "#ca8a04" },
              { l: "Géopolitique CBDC", v: entity.cbdc_geopolitical_score, c: "#a16207" },
              { l: "Weaponisation Réserves Or", v: entity.gold_reserve_weaponization_score, c: "#854d0e" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Guerre des Devises Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_currency_war_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Surveillance immédiate des flux de dédollarisation et coordination avec banques centrales alliées</p><p>› Activation des mécanismes de contre-mesures aux CBDC adverses et protection des réserves stratégiques</p></>}
            {entity.risk_level === "élevé" && <><p>› Analyse approfondie des stratégies de manipulation des taux de change et des accords de swap bilatéraux</p><p>› Renforcement de la résilience du système de paiement international face aux alternatives au SWIFT</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance des accumulations de réserves d'or et des accords de règlement en devises locales</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille sur les politiques monétaires et les initiatives de numérisation des devises</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function CurrencyWarPage() {
  const [data, setData] = useState<CWSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<CWEntity | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/currency-war-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données guerre des devises"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse des dynamiques de guerre des devises…</div></div>;
  ...
  const avgScore = (k: keyof CWEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Currency War Intelligence Engine</h1>
          <p className="text-slate-400 text-sm">Cartographie des dynamiques de guerre des devises et dédollarisation · v{data.engine_version}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Sources de Données", v: data.data_sources.length, g: "from-yellow-900 to-yellow-800", t: "text-yellow-300" },
            { l: "Confiance Analyse", v: `${Math.round(data.confidence_score * 100)}%`, g: "from-slate-800 to-slate-700", t: "text-slate-300" },
            { l: "Index Guerre Devises Moyen", v: data.avg_estimated_currency_war_index, g: "from-yellow-900 to-yellow-800", t: "text-yellow-300" },
          ].map(({ l, v, g, t }) => (
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs de Guerre des Devises Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("dedollarization_strategy_score")} label="Dédollarisation" color="#eab308" />
            <GaugeRing value={avgScore("currency_manipulation_score")} label="Manipulation Monétaire" color="#ca8a04" />
            <GaugeRing value={avgScore("cbdc_geopolitical_score")} label="Géopolitique CBDC" color="#a16207" />
            <GaugeRing value={avgScore("gold_reserve_weaponization_score")} label="Weaponisation Or" color="#854d0e" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Guerre des Devises</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={data.total_entities} color={["#eab308", "#ca8a04", "#a16207", "#854d0e", "#713f12"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        {data.critical_alerts.length > 0 && (
          <div className="rounded-xl p-4" style={{ backgroundColor: "#eab30810", border: "1px solid #eab30830" }}>
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Guerre des Devises Critiques</h2>
            <ul className="space-y-1">{data.critical_alerts.map((a, i) => <li key={i} className="text-sm" style={{ color: "#fde047" }}>› {a}</li>)}</ul>
          </div>
        )}
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_currency_war_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Currency War Intelligence Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
