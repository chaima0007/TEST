...
interface Entity {
  ...
  last_updated: string;
  historical_genocide_denial_score?: number;
  reparation_refusal_score?: number;
  cultural_erasure_legacy_score?: number;
  memory_law_suppression_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "historical_genocide_denial_score", label: "Déni du génocide historique" },
  { key: "reparation_refusal_score", label: "Refus de réparations" },
  { key: "cultural_erasure_legacy_score", label: "Héritage d&apos;effacement culturel" },
  { key: "memory_law_suppression_score", label: "Suppression des lois mémorielles" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits liés au génocide colonial : déni historique, refus de réparations, effacement culturel et suppression des lois mémorielles.</p>
              <p className="text-slate-400">Source : Colonial Genocide Rights Engine · Wave 252</p>
            </div>
          )}
        </div>
      </div>
...
export default function ColonialGenocideRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/colonial-genocide-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Colonial Genocide Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Déni Historique · Réparations · Effacement Culturel · Lois Mémorielles</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#7c2d1220" } : {}}>
