...
const ACCENT = "#7c3aed";
const TITRE = "Droits — Travail Enfant dans la Vanille";
...
export default function Page() {
  ...
  useEffect(() => { fetch("/api/vanilla-child-labor-rights").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
  ...
  return (
    <div style={{ background: "#020617", minHeight: "100vh", color: "#f1f5f9", fontFamily: "system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <p style={{ color: "#64748b", fontSize: 14, marginBottom: 32 }}>Travail des enfants dans la filière vanille &amp; lacunes de protection</p>
        <div style={{ display: "flex", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
          ...
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 200px", display: "flex", flexDirection: "column", gap: 10, justifyContent: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#94a3b8" }}>Index VCLR</span><strong style={{ color: ACCENT }}>{data.avg_estimated_vanilla_child_labor_rights_index?.toFixed(2)}</strong></div>
          </div>
