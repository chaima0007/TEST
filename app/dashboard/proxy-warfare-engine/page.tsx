...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/proxy-warfare-engine").then(r => r.json()).then(j => setData(j.payload ?? j));
  }, []);
  ...
  const PATTERN_LABELS: Record<string, string> = {
    reseau_mandataire_global: "Réseau Mandataire Global",
    transfert_armes_deniable: "Transfert Armes Niable",
    mandataire_crimes_civils: "Mandataire Crimes Civils",
    impunite_commanditaire: "Impunité Commanditaire",
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Proxy Warfare Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Surveillance des guerres par procuration et responsabilité des commanditaires</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    {["proxy_network_depth_score", "deniability_arms_transfer_score", "proxy_civilian_harm_score", "sponsor_impunity_score"].map(key => (
