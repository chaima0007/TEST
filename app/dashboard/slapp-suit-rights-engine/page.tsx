...
interface Entity {
  ...
  last_updated: string;
  corporate_litigation_abuse_score?: number;
  journalist_silencing_score?: number;
  activist_legal_harassment_score?: number;
  strategic_lawsuit_chilling_effect_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "corporate_litigation_abuse_score", label: "Abus de litige corporate" },
  { key: "journalist_silencing_score", label: "Réduction au silence journalistes" },
  { key: "activist_legal_harassment_score", label: "Harcèlement judiciaire militants" },
  { key: "strategic_lawsuit_chilling_effect_score", label: "Effet dissuasif poursuites stratégiques" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des poursuites SLAPP : abus de litiges corporatifs pour réduire au silence les journalistes, harcèlement judiciaire des militants et effets dissuasifs des poursuites stratégiques anti-participation.</p>
              <p className="text-slate-400">Source : SLAPP Suit Rights Engine · Wave 260</p>
            </div>
          )}
        </div>
      </div>
...
export default function SlappSuitRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/slapp-suit-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>SLAPP Suit Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Litige Abusif · Journalistes · Militants · Effet Dissuasif</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#f59e0b20" } : {}}>
