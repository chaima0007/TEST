...
interface Entity {
  ...
  involuntary_hospitalization_severity_score: number;
  crpd_compliance_deficit_score: number;
  deinstitutionalization_gap_score: number;
  informed_consent_violations_score: number;
  estimated_mental_health_forced_treatment_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_mental_health_forced_treatment_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Hospitalisation Involontaire", value: entity.involuntary_hospitalization_severity_score, weight: "0.30" },
    { label: "Déficit Conformité CRPD", value: entity.crpd_compliance_deficit_score, weight: "0.25" },
    { label: "Écart Désinstitutionnalisation", value: entity.deinstitutionalization_gap_score, weight: "0.25" },
    { label: "Violations Consentement Éclairé", value: entity.informed_consent_violations_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_mental_health_forced_treatment_rights_index === "number" ? entity.estimated_mental_health_forced_treatment_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Traitement Forcé</div>
                </div>
              </div>
...
export default function MentalHealthForcedTreatmentRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/mental-health-forced-treatment-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Santé Mentale &amp; Traitement Forcé…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_mental_health_forced_treatment_rights_index
    ?? (Object.entries(data ?? {}).find(([k]) => k.startsWith("avg_estimated_"))?.[1] as number)
    ?? avg(allEntities.map(e => e.estimated_mental_health_forced_treatment_rights_index));
  ...
  const avgHospitalization = avg(allEntities.map(e => e.involuntary_hospitalization_severity_score));
  const avgCrpd = avg(allEntities.map(e => e.crpd_compliance_deficit_score));
  const avgDeinstitut = avg(allEntities.map(e => e.deinstitutionalization_gap_score));
  const avgConsent = avg(allEntities.map(e => e.informed_consent_violations_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Traitement Forcé", value: typeof avgIndex === "number" ? avgIndex.toFixed(2) : "—" },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Santé Mentale &amp; Traitement Psychiatrique Forcé</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Hospitalisation Involontaire · CRPD · Désinstitutionnalisation · Consentement — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgHospitalization} label="Hospitalisation Involontaire" />
          <GaugeRing value={avgCrpd} label="Déficit CRPD" />
          <GaugeRing value={avgDeinstitut} label="Désinstitutionnalisation" />
          <GaugeRing value={avgConsent} label="Consentement Éclairé" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Traitement Forcé: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_mental_health_forced_treatment_rights_index === "number" ? e.estimated_mental_health_forced_treatment_rights_index.toFixed(2) : "—"}</span>
            </div>
