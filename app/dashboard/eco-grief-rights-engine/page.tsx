...
interface EGREntity {
  ...
  climate_displacement_trauma_severity_score: number;
  ecological_loss_grief_recognition_gap_score: number;
  mental_health_climate_support_absence_score: number;
  indigenous_land_loss_cultural_trauma_score: number;
  estimated_eco_grief_rights_index: number;
  risk_level: string;
...
interface EGRData {
  ...
  data_sources: string[];
  entities: EGREntity[];
  avg_estimated_eco_grief_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function EcoGriefRightsPage() {
  const [data, setData] = useState<EGRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<EGREntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/eco-grief-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données d&apos;éco-deuil…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "climate_displacement_trauma_severity_score", label: "Trauma Déplacement Climatique" },
    { key: "ecological_loss_grief_recognition_gap_score", label: "Reconnaissance Deuil Écologique" },
    { key: "mental_health_climate_support_absence_score", label: "Soutien Santé Mentale Absent" },
    { key: "indigenous_land_loss_cultural_trauma_score", label: "Trauma Culturel Peuples Autochtones" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droit au Deuil Écologique</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Éco-anxiété, solastalgie &amp; trauma climatique</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_eco_grief_rights_index ?? "—" },
            { label: "Confiance", value: `${Math.round((data.confidence_score ?? 0) * 100)}%` },
            { label: "Dernière Analyse", value: data.last_analysis ?? "—" },
            { label: "Version", value: data.engine_version ? `v${data.engine_version}` : "—" },
          ].map(k => (
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#4ade80" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Trauma Climatique</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#4ade80", "#f97316", "#eab308", "#ef4444", "#10b981"];
        ...
        {(data.critical_alerts?.length ?? 0) > 0 && (
          <div className="border border-green-500/30 bg-green-500/10 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-green-400 mb-2">Alertes Critiques</h2>
        ...
        <div className="flex gap-2 flex-wrap">
          {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-full text-sm border transition-all capitalize ${filter === f ? "bg-green-500 border-green-400 text-white" : "bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-500"}`}>
              {f}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              className={`bg-slate-900 border rounded-xl p-4 cursor-pointer hover:border-green-500/50 transition-all ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#4ade80" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_eco_grief_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droit au Deuil Écologique · {data.last_analysis}</p>
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Aperçu", "Métriques", "Sources"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`px-6 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-green-500 text-green-400" : "text-slate-500 hover:text-slate-300"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#4ade80" />
                    <div>
                      <div className="text-2xl font-bold text-green-400">{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Éco-Deuil</div>
                    <div className="text-lg font-bold text-green-400">{selected.estimated_eco_grief_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
