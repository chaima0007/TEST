...
interface Entity {
  ...
  genome_editing_embryo_ethics_severity_score: number;
  genetic_database_state_surveillance_scale_score: number;
  insurance_employment_genetic_discrimination_score: number;
  informed_consent_biobank_research_deficit_gap_score: number;
  estimated_bioethics_genetic_privacy_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_bioethics_genetic_privacy_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Édition Génome & Éthique Embryonnaire", value: entity.genome_editing_embryo_ethics_severity_score, weight: "0.30" },
    { label: "Base de Données Génétiques & Surveillance État", value: entity.genetic_database_state_surveillance_scale_score, weight: "0.25" },
    { label: "Discrimination Génétique Assurance & Emploi", value: entity.insurance_employment_genetic_discrimination_score, weight: "0.25" },
    { label: "Déficit Consentement Éclairé & Biobanques", value: entity.informed_consent_biobank_research_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_bioethics_genetic_privacy_rights_index === "number" ? entity.estimated_bioethics_genetic_privacy_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Bioéthique & Vie Privée Génétique</div>
                </div>
              </div>
...
export default function BioethicsGeneticPrivacyRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/bioethics-genetic-privacy-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Bioéthique & Droits à la Vie Privée Génétique…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_bioethics_genetic_privacy_rights_index ?? avg(allEntities.map(e => e.estimated_bioethics_genetic_privacy_rights_index));
  ...
  const avgGenomeEditing = avg(allEntities.map(e => e.genome_editing_embryo_ethics_severity_score));
  const avgGeneticDatabase = avg(allEntities.map(e => e.genetic_database_state_surveillance_scale_score));
  const avgGeneticDiscrimination = avg(allEntities.map(e => e.insurance_employment_genetic_discrimination_score));
  const avgInformedConsent = avg(allEntities.map(e => e.informed_consent_biobank_research_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Bioéthique Génétique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Bioéthique & Droits à la Vie Privée Génétique</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Bioethics Genetic Privacy Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgGenomeEditing} label="Édition Génome" />
          <GaugeRing value={avgGeneticDatabase} label="Base Données Génétiques" />
          <GaugeRing value={avgGeneticDiscrimination} label="Discrimination Génétique" />
          <GaugeRing value={avgInformedConsent} label="Consentement Éclairé" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Bioéthique Génétique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_bioethics_genetic_privacy_rights_index === "number" ? e.estimated_bioethics_genetic_privacy_rights_index.toFixed(2) : "—"}</span>
            </div>
