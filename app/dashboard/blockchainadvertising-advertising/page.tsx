...
type Entity = {
  ...
  risk_level: string;
  estimated_blockchainadvertising_index: number;
};
...
const SIGNALS = [
  "Smart contract ad consent",
  "On-chain transparency",
  "Immutable ad records",
  "Gas fee disclosure",
  "Chain data privacy",
];

const ACTIONS = [
  "Enforce smart contract consent",
  "Publish on-chain ad records",
  "Require immutable disclosures",
  "Disclose gas fees",
  "Protect chain data privacy",
];
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque blockchain advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_blockchainadvertising_index.toFixed(2)} / 10`,
    `Conformité blockchain advertising : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 424 — Blockchain Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Blockchain Advertising · Wave 424</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: entity.estimated_blockchainadvertising_index * 10, color: COLOR },
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {ACTIONS.map((action, i) => (
              <div key={i} style={{ background: "#1e293b", borderRadius: 8, padding: 12, fontSize: 13, color: "#cbd5e1", border: "1px solid rgba(29,78,216,0.15)" }}>
                {action}
              </div>
            ))}
          </div>
...
export default function BlockchainadvertisingAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/blockchainadvertising-advertising")
      .then((r) => r.json())
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Blockchain Advertising…
      </div>
    );
  }
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + e.estimated_blockchainadvertising_index, 0) / entities.length
    : 0;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Blockchain Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#1d4ed899", margin: 0 }}>
          Caelum Partners · Wave 424 · Analyse publicité blockchain · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
        ...
        {filtered.map((entity, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
              ...
              <span style={{ color: COLOR, fontWeight: 600 }}>{entity.estimated_blockchainadvertising_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Blockchain Advertising Intelligence · Wave 424
      </div>
