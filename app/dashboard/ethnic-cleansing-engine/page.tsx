...
type ECEntity = {
  ...
  forced_displacement_scale_score: number;
  ethnic_massacre_documentation_score: number;
  demographic_engineering_state_score: number;
  ethnic_cleansing_impunity_score: number;
  estimated_ethnic_cleansing_index: number;
  risk_level: string;
          ...
          {tab === "scores" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>
                    {entity.estimated_ethnic_cleansing_index.toFixed(1)}
                  </div>
                  <div className="text-xs text-slate-400 mt-1">Index Nettoyage Ethnique</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Déplacements Forcés", value: entity.forced_displacement_scale_score },
                  { label: "Massacres Documentés", value: entity.ethnic_massacre_documentation_score },
                  { label: "Ingénierie Démographique", value: entity.demographic_engineering_state_score },
                  { label: "Impunité Nettoyage Ethnique", value: entity.ethnic_cleansing_impunity_score },
                ].map((s) => (
...
export default function EthnicCleansingEnginePage() {
  const [entities, setEntities] = useState<ECEntity[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    fetch("/api/ethnic-cleansing-engine")
      .then((r) => r.json())
      .then((json: ApiResponse) => {
        const rows = json?.data ?? (Array.isArray(json) ? json : []);
        setEntities(rows as ECEntity[]);
        setLoading(false);
      })
      .catch(() => {
  ...
  const avgDisplacement = avg(entities.map((e) => e.forced_displacement_scale_score));
  const avgMassacre = avg(entities.map((e) => e.ethnic_massacre_documentation_score));
  const avgDemographic = avg(entities.map((e) => e.demographic_engineering_state_score));
  const avgImpunity = avg(entities.map((e) => e.ethnic_cleansing_impunity_score));
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-white">Nettoyage Ethnique Engine</h1>
        </div>
        <p className="text-slate-400 ml-5">
          Épuration Démographique &amp; Génocide Territorial — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 rounded-2xl p-6 mb-8" style={{ border: `1px solid ${ACCENT}1a` }}>
        ...
        <div className="flex flex-wrap justify-around gap-6">
          <GaugeRing value={avgDisplacement} label="Déplacements Forcés" color={ACCENT} />
          <GaugeRing value={avgMassacre} label="Massacres Documentés" color="#7c3aed" />
          <GaugeRing value={avgDemographic} label="Ingénierie Démograph." color="#0891b2" />
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs text-slate-500">Index ECE</div>
                </div>
              </div>
              <div
