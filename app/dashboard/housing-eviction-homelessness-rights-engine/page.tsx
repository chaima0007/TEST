...
interface Entity {
  ...
  forced_eviction_displacement_severity_score: number;
  homelessness_criminalization_scale_score: number;
  land_tenure_insecurity_indigenous_score: number;
  affordable_housing_right_enforcement_deficit_gap_score: number;
  estimated_housing_eviction_homelessness_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_housing_eviction_homelessness_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Expulsions Forcées & Déplacement", value: entity.forced_eviction_displacement_severity_score, weight: "0.30" },
    { label: "Criminalisation Sans-Abri", value: entity.homelessness_criminalization_scale_score, weight: "0.25" },
    { label: "Insécurité Foncière Autochtones", value: entity.land_tenure_insecurity_indigenous_score, weight: "0.25" },
    { label: "Déficit Logement Abordable", value: entity.affordable_housing_right_enforcement_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_housing_eviction_homelessness_rights_index === "number" ? entity.estimated_housing_eviction_homelessness_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droit au Logement</div>
                </div>
              </div>
...
export default function HousingEvictionHomelessnessRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/housing-eviction-homelessness-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droit au Logement…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_housing_eviction_homelessness_rights_index ?? avg(allEntities.map(e => e.estimated_housing_eviction_homelessness_rights_index));
  ...
  const avgExpulsionsForcees = avg(allEntities.map(e => e.forced_eviction_displacement_severity_score));
  const avgCriminalisationSansAbri = avg(allEntities.map(e => e.homelessness_criminalization_scale_score));
  const avgInsecuriteFonciere = avg(allEntities.map(e => e.land_tenure_insecurity_indigenous_score));
  const avgDeficitLogement = avg(allEntities.map(e => e.affordable_housing_right_enforcement_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droit au Logement", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droit au Logement & Expulsions Forcées</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Housing, Eviction & Homelessness Rights Engine — Expulsions forcées, criminalisation des sans-abri et insécurité foncière autochtone — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgExpulsionsForcees} label="Expulsions Forcées" />
          <GaugeRing value={avgCriminalisationSansAbri} label="Criminalisation" />
          <GaugeRing value={avgInsecuriteFonciere} label="Insécurité Foncière" />
          <GaugeRing value={avgDeficitLogement} label="Déficit Logement" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droit au Logement: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_housing_eviction_homelessness_rights_index === "number" ? e.estimated_housing_eviction_homelessness_rights_index.toFixed(2) : "—"}</span>
            </div>
