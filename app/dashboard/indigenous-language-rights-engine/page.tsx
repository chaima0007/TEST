...
interface Entity {
  ...
  last_updated: string;
  language_extinction_rate_score?: number;
  education_language_suppression_score?: number;
  official_language_exclusion_score?: number;
  cultural_transmission_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "language_extinction_rate_score", label: "Taux d&apos;extinction linguistique" },
  { key: "education_language_suppression_score", label: "Suppression éducative de la langue" },
  { key: "official_language_exclusion_score", label: "Exclusion linguistique officielle" },
  { key: "cultural_transmission_gap_score", label: "Déficit de transmission culturelle" },
];
          ...
          {tab === "context" && (
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-slate-200 font-medium">{entity.confidence_score as number | undefined ?? "—"}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Identifiant</span>
                <span className="text-slate-200 font-mono text-xs">{entity.id}</span>
              </div>
            </div>
          )}
        </div>
...
export default function IndigenousLanguageRightsDashboard() {
  ...
  useEffect(() => {
    fetch("/api/indigenous-language-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Indigenous Language Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Extinction Linguistique · Suppression Éducative · Exclusion Officielle · Transmission Culturelle</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-[#166534]/40 text-[#22a05a]"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "rgba(22,101,52,0.15)" } : {}}>
