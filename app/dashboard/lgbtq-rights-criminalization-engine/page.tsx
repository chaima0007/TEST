...
function DetailModal({ entity, onClose }: { entity: Record<string, unknown>; onClose: () => void }) {
  ...
  const SUB_SCORES = [
    "state_criminalization_imprisonment_severity_score",
    "violence_persecution_impunity_scale_score",
    "legal_identity_recognition_denial_score",
    "anti_lgbtq_legislation_rollback_deficit_gap_score",
  ];
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {tab === "sources" && (
            <p className="text-slate-400 text-sm">Sources: ILGA World state-sponsored homophobia report, Human Rights Watch LGBTQ+ persecution data, UN Independent Expert on sexual orientation and gender identity.</p>
          )}
        </div>
      </div>
    </div>
...
export default function LgbtqRightsCriminalizationPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white">Droits LGBTQ+ &amp; Criminalisation</h1>
          <p className="text-slate-400 mt-1">Criminalisation LGBTQ+, droits à l&apos;identité et violence d&apos;État</p>
        </div>
