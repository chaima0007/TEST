...
type Entity = {
  ...
  risk_level: string;
  estimated_emailmarketing_index: number;
};
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque email marketing advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_emailmarketing_index.toFixed(2)} / 10`,
    `Conformité fournisseurs ESP : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 403 — Email Marketing Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Email Marketing Advertising · Wave 403</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: entity.estimated_emailmarketing_index * 10, color: COLOR },
        ...
        {tab === "signaux" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              `Score composite ${entity.composite_score.toFixed(1)} — ${entity.risk_level === "critique" ? "surveillance email urgente" : entity.risk_level === "élevé" ? "monitoring renforcé recommandé" : "suivi standard actif"}`,
              `Indice Caelum Email Marketing Advertising : ${entity.estimated_emailmarketing_index.toFixed(2)} / 10`,
              `Analyse conformité CSDDD pour la chaîne d&apos;approvisionnement email marketing. Risques liés aux conditions de travail dans les centres de données et les équipes de rédaction partenaires.`,
            ].map((sig, i) => (
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(99,102,241,0.15)" }}>
              ...
              <div>{entity.risk_level === "critique" ? "Auditer fournisseurs ESP partenaires" : entity.risk_level === "élevé" ? "Vérifier conformité RGPD données" : entity.risk_level === "modéré" ? "Mettre en place politique éthique email" : "Certifier ISO 26000"}</div>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(99,102,241,0.15)" }}>
              <div style={{ fontSize: 11, color: "#93c5fd", marginBottom: 4 }}>Index Caelum</div>
              <div style={{ color: COLOR, fontWeight: 700, fontSize: 18 }}>{entity.estimated_emailmarketing_index.toFixed(2)} / 10</div>
            </div>
          </div>
        )}
      </div>
...
export default function EmailmarketingAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/emailmarketing-advertising")
      .then((r) => r.json())
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Email Marketing Advertising…
      </div>
    );
  }
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + e.estimated_emailmarketing_index, 0) / entities.length
    : 0;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Email Marketing Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#6366f199", margin: 0 }}>
          Caelum Partners · Wave 403 · Analyse conformité email marketing · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
        ...
        {filtered.map((entity, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
              ...
              <span style={{ color: COLOR, fontWeight: 600 }}>{entity.estimated_emailmarketing_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Email Marketing Advertising Intelligence · Wave 403
      </div>
