...
interface Entity {
  ...
  last_updated: string;
  nonconsensual_image_sharing_score?: number;
  cyberstalking_severity_score?: number;
  online_misogyny_platform_tolerance_score?: number;
  doxxing_harassment_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "nonconsensual_image_sharing_score", label: "Partage images non-consenti" },
  { key: "cyberstalking_severity_score", label: "Sévérité cyberharcèlement" },
  { key: "online_misogyny_platform_tolerance_score", label: "Tolérance misogynie en ligne" },
  { key: "doxxing_harassment_score", label: "Harcèlement doxxing" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la violence en ligne contre les femmes : partage non-consenti d&apos;images intimes, cyberharcèlement, tolérance des plateformes à la misogynie et harcèlement par doxxing.</p>
              <p className="text-slate-400">Source : Online Violence Against Women Rights Engine · Wave 263</p>
            </div>
          )}
        </div>
      </div>
...
export default function OnlineViolenceWomenRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/online-violence-women-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Online Violence Against Women Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Images Non-Consenties · Cyberharcèlement · Misogynie · Doxxing</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-pink-900/40 text-pink-300"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "#db277720" } : {}}>
