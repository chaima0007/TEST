...
interface LURFAEntity {
  entity_id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  union_busting_repression_severity_score: number;
  collective_bargaining_prohibition_scale_score: number;
  strike_right_criminalization_score: number;
  labor_organizer_persecution_impunity_score: number;
  estimated_labor_union_rights_freedom_association_index: number;
  risk_level: string;
...
interface LURFAData {
  ...
  data_sources: string[];
  entities: LURFAEntity[];
  avg_estimated_labor_union_rights_freedom_association_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function LaborUnionRightsFreedomAssociationEnginePage() {
  const [data, setData] = useState<LURFAData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<LURFAEntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/labor-union-rights-freedom-association-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données syndicats & liberté d&apos;association…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "union_busting_repression_severity_score", label: "Répression Syndicale" },
    { key: "collective_bargaining_prohibition_scale_score", label: "Interdiction Négociation Collective" },
    { key: "strike_right_criminalization_score", label: "Criminalisation Droit de Grève" },
    { key: "labor_organizer_persecution_impunity_score", label: "Persécution Organisateurs" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Syndicats & Liberté d&apos;Association</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Répression Syndicale · Interdiction Négociation Collective · Criminalisation Droit de Grève · Persécution Organisateurs
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Syndicats", value: data.avg_estimated_labor_union_rights_freedom_association_index ?? "—" },
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke={ACCENT} />
                <span className="text-xs text-slate-400 text-center max-w-[120px]">{label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Répression</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#0369a1", "#f97316", "#eab308", "#ef4444", "#10b981"];
        ...
        {(data.critical_alerts?.length ?? 0) > 0 && (
          <div className="border border-blue-700/30 bg-blue-700/10 rounded-xl p-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-full text-sm border transition-all capitalize ${filter === f ? "text-white border-transparent" : "bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-500"}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              className={`bg-slate-900 border rounded-xl p-4 cursor-pointer hover:border-blue-700/50 transition-all ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              <div className="flex justify-between items-start mb-3">
                <div className="flex-1 min-w-0 pr-2">
                  <p className="text-xs font-mono text-slate-500">{entity.entity_id}</p>
                  <p className="text-sm font-semibold text-white line-clamp-2">{entity.name}</p>
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-500">idx {entity.estimated_labor_union_rights_freedom_association_index}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Syndicats & Liberté d&apos;Association · {data.last_analysis}</p>
      </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setSelected(null)}>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex border-b border-slate-800">
              {["Aperçu", "Métriques", "Signaux"].map((t, i) => (
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-2xl font-bold" style={{ color: ACCENT }}>{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Syndicats & Liberté d&apos;Association</div>
                    <div className="text-lg font-bold" style={{ color: ACCENT }}>{selected.estimated_labor_union_rights_freedom_association_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
                    ...
                    return (
                      <div key={key}>
                        ...
                        <div className="h-2 bg-slate-800 rounded-full">
                          <div className="h-full rounded-full" style={{ width: `${val}%`, backgroundColor: ACCENT }} />
                        </div>
                      </div>
                    );
                  })}
                ...
                <ul className="space-y-2">
                  {(selected.key_signals ?? []).map((s, i) => (
                    <li key={i} className="flex gap-2 text-sm text-slate-300">
                      <span className="shrink-0 mt-0.5" style={{ color: ACCENT }}>&#9658;</span>{s}
                    </li>
                  ))}
                </ul>
              )}
