...
interface Entity {
  ...
  risk_level: string
  estimated_food_rights_index: number
}
...
export default function FoodRightAgribusinessHungerPage() {
  const [data, setData] = useState<DomainData | null>(null)

  useEffect(() => {
    fetch("/api/food-right-agribusiness-hunger-engine")
      .then((r) => r.json())
  ...
  return (
    <div style={{ background: "#0f172a", minHeight: "100vh", padding: "32px 24px", fontFamily: "system-ui, sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ background: ACCENT, borderRadius: 12, padding: 12, flexShrink: 0 }}>
            <svg width={28} height={28} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth={2}>
              ...
              <path d="M7 14c0 2.8 2.2 5 5 5" />
              <path d="M17 10c0-2.8-2.2-5-5-5" />
            </svg>
          </div>
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "#f1f5f9" }}>Droit à l&apos;Alimentation &amp; Agro-industrie</h1>
            <p style={{ margin: 0, color: "#94a3b8", fontSize: 14 }}>Wave 179 — Indice mondial des droits humains · Caelum Partners</p>
          </div>
        </div>
          ...
          <div key={entity.id} style={{ background: "#1e293b", borderRadius: 12, padding: 22, display: "flex", gap: 20, alignItems: "flex-start", borderLeft: `4px solid ${ACCENT}` }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", gap: 16 }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: ACCENT }}>{entity.estimated_food_rights_index.toFixed(2)}</div>
                  <div style={{ fontSize: 11, color: "#64748b" }}>Indice droit aliment.</div>
                </div>
                <div>
      ...
      <div style={{ maxWidth: 1100, margin: "40px auto 0", textAlign: "center", color: "#334155", fontSize: 12 }}>
        Dernière mise à jour : {new Date(data.generated_at).toLocaleString("fr-FR")} · Caelum Partners Wave 179
      </div>
    </div>
  )
}
