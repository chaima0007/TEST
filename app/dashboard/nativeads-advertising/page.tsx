...
const COLOR = "#f97316";
const DOMAIN = "nativeads-advertising";
const TITLE = "Native Ads Advertising";
const WAVE = 402;
...
type Entity = {
  ...
  risk_level: string;
  estimated_nativeads_index: number;
};
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque native ads advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_nativeads_index.toFixed(2)} / 10`,
    `Conformité plateforme native ads : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave ${WAVE} — Native Ads Advertising Intelligence`,
  ];
}
      ...
      >
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: entity.estimated_nativeads_index * 10, color: COLOR },
        ...
        {tab === "signaux" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              `Score composite ${entity.composite_score.toFixed(1)} — ${entity.risk_level === "critique" ? "surveillance native ads urgente" : entity.risk_level === "élevé" ? "monitoring renforcé recommandé" : "suivi standard actif"}`,
              `Indice Caelum Native Ads Advertising : ${entity.estimated_nativeads_index.toFixed(2)} / 10`,
              "Analyse conformité CSDDD pour la chaîne d'approvisionnement native advertising. Risques liés aux conditions de travail des équipes éditoriales et des studios créatifs partenaires.",
            ].map((sig, i) => (
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: `1px solid ${COLOR}26` }}>
              ...
              <div>{entity.risk_level === "critique" ? "Audit immédiat des agences native ads — conformité réglementaire urgente" : entity.risk_level === "élevé" ? "Révision des stratégies native ads — renforcement compliance" : entity.risk_level === "modéré" ? "Optimisation continue des métriques native ads advertising" : "Maintien des bonnes pratiques — veille sectorielle"}</div>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: `1px solid ${COLOR}26` }}>
              <div style={{ fontSize: 11, color: "#93c5fd", marginBottom: 4 }}>Actions Recommandées</div>
              <ul style={{ margin: 0, paddingLeft: 16, display: "flex", flexDirection: "column", gap: 4 }}>
                <li>Auditer agences éditoriales partenaires</li>
                <li>Vérifier conditions studios créatifs</li>
                <li>Mettre en place code conduite native ads</li>
                <li>Certifier SA8000</li>
              </ul>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: `1px solid ${COLOR}26` }}>
              <div style={{ fontSize: 11, color: "#93c5fd", marginBottom: 4 }}>Index Caelum</div>
              <div style={{ color: COLOR, fontWeight: 700, fontSize: 18 }}>{entity.estimated_nativeads_index.toFixed(2)} / 10</div>
            </div>
          </div>
        )}
      </div>
...
export default function NativeAdsAdvertisingDashboard() {
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + e.estimated_nativeads_index, 0) / entities.length
    : 0;
    ...
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 13, color: `${COLOR}99`, margin: 0 }}>
          Caelum Partners · Wave {WAVE} · Analyse native ads advertising · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
        ...
        {filtered.map((entity, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
              ...
              <span style={{ color: COLOR, fontWeight: 600 }}>{entity.estimated_nativeads_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
