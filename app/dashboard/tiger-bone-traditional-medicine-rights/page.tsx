...
function DetailModal({ entity, onClose }: { entity: any; onClose: () => void }) {
  ...
  const tabs = ["Analyse", "Entités", "Méthodologie"];
  return (
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {tabs.map(tab => (
            <span key={tab} onClick={() => setActiveTab(tab)}
              style={{ padding: "4px 12px", borderRadius: 20, background: activeTab === tab ? "#1e293b" : "#f1f5f9", color: activeTab === tab ? "#fff" : "#1e293b", fontSize: 13, cursor: "pointer" }}>{tab}</span>
          ))}
        </div>
        {activeTab === "Analyse" && (
          <>
            ...
            {entity.estimated_tiger_bone_medicine_index !== undefined && (
              <p style={{ color: "#64748b", fontSize: 14 }}>Indice estimé : <strong>{entity.estimated_tiger_bone_medicine_index}</strong></p>
            )}
          </>
        )}
        {activeTab === "Entités" && (
          <p style={{ color: "#64748b", fontSize: 14 }}>Entité : <strong>{entity.entity}</strong></p>
        )}
        {activeTab === "Méthodologie" && (
          <p style={{ color: "#64748b", fontSize: 14 }}>Poids : sub1×0.30 + sub2×0.25 + sub3×0.25 + sub4×0.20. Seuils : critique ≥60, élevé ≥40, modéré ≥20, faible &lt;20.</p>
        )}
...
export default function TigerBoneTraditionalMedicineRightsDashboard() {
  const [data, setData] = useState<any>(null);
  const [selected, setSelected] = useState<any>(null);

  useEffect(() => {
    fetch("/api/tiger-bone-traditional-medicine-rights")
      .then(r => r.json())
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f8fafc", padding: 32 }}>
      <h1 style={{ fontSize: 28, fontWeight: 800, color: "#1e293b", marginBottom: 8 }}>
        Droits — Os de Tigre et Médecine Traditionnelle
      </h1>
      <p style={{ color: "#64748b", marginBottom: 32 }}>Analyse CaelumSwarm™ — Trafic d&apos;os de tigre et médecine traditionnelle</p>
        ...
        {entities.map((e: any) => (
          <div key={e.entity} onClick={() => setSelected(e)}
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ fontSize: 12, color: e.risk_level === "critique" ? "#dc2626" : e.risk_level === "élevé" ? "#f97316" : e.risk_level === "modéré" ? "#eab308" : "#22c55e" }}>{e.risk_level}</div>
            </div>
          </div>
        ))}
      </div>
