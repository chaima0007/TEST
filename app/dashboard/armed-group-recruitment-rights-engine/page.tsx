...
interface Entity {
  ...
  last_updated: string;
  forced_recruitment_score?: number;
  economic_coercion_recruitment_score?: number;
  youth_targeting_score?: number;
...
const SUB_SCORES = [
  { key: "forced_recruitment_score", label: "Recrutement forcé par groupes armés" },
  { key: "economic_coercion_recruitment_score", label: "Coercition économique au recrutement" },
  { key: "youth_targeting_score", label: "Ciblage des jeunes et enfants soldats" },
  { key: "international_law_violation_score", label: "Violations du droit international humanitaire" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits face au recrutement par groupes armés : recrutement forcé, coercition économique, ciblage des jeunes et violations du droit international humanitaire.</p>
              <p className="text-slate-400">Source : Armed Group Recruitment Rights Engine · Wave 250</p>
            </div>
          )}
        </div>
      </div>
...
export default function ArmedGroupRecruitmentRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/armed-group-recruitment-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Armed Group Recruitment Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Recrutement Forcé · Coercition Économique · Ciblage Jeunes · Droit International</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#7f1d1d20" } : {}}>
