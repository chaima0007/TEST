...
const API_URL = "/api/hospitality-advertising";
const TITLE = "Publicité Hospitalité — Conformité & Droits";
const COLOR = "#b45309";
  ...
  return (
    <svg width={88} height={88} viewBox="0 0 88 88">
      ...
      <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central"
        fontSize={16} fontWeight="bold" fill={color}>{Math.round(pct)}</text>
    </svg>
  );
}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
      <div style={{ background: "#fff", borderRadius: 12, padding: 24, width: 520, maxWidth: "90vw", maxHeight: "80vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
          <h2 style={{ fontWeight: "bold", fontSize: 18 }}>{entity.name}</h2>
        ...
        {tab === "apercu" && (
          <div>
            ...
            <p style={{ marginBottom: 8 }}>Indice estimé : <strong>{entity.estimated_index}</strong></p>
            <p>Risque : <strong style={{ color }}>{entity.risk_level}</strong></p>
          </div>
        )}
        {tab === "indicateurs" && (
          <p>Conformité CSDDD 2024/1760 — les plateformes de publicité dans le secteur hospitalier doivent respecter des engagements de transparence sur les pratiques de ciblage et la chaîne d&apos;approvisionnement touristique. Les obligations de diligence raisonnable de l&apos;ILO-C182 s&apos;appliquent aux travailleurs du secteur hôtelier et à leurs conditions d&apos;emploi.</p>
        )}
        {tab === "recommandations" && (
          <p>Mettre en oeuvre des politiques de publicité responsable dans le secteur hospitalier conformes à la CSDDD 2024/1760. Documenter les engagements en matière de droits du travail dans les chaînes hôtelières partenaires et effectuer des audits réguliers selon le cadre OIT C182 et les standards internationaux du tourisme durable.</p>
        )}
        {tab === "certifications" && (
          <div>
            <p style={{ fontSize: 12, color: "#6b7280", marginBottom: 12 }}>Attestations de conformité — {entity.name}</p>
...
export default function Page() {
  ...
  const distribution = data.entities.reduce((acc: Record<string, number>, e) => {
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
        {Object.entries(distribution).map(([level, count]) => (
          <div key={level} style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 8, padding: "12px 20px", minWidth: 120, textAlign: "center" }}>
            <div style={{ fontSize: 24, fontWeight: "bold", color: riskColors[level] ?? "#374151" }}>{count}</div>
