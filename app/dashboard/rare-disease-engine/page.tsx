...
type RDEEntity = {
  ...
  risk_level: string;
  rare_disease_pattern: string;
  severity: string;
  recommended_action: string;
  signal: string;
  orphan_drug_price_index: number;
  diagnostic_delay_years: number;
};
...
type RDESummary = {
  ...
  avg_estimated_rare_disease_access_index: number;
};
    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="flex h-3 rounded overflow-hidden gap-px">
        {Object.entries(counts).map(([k, v]) => (
          <div
            key={k}
            style={{ width: `${(v / total) * 100}%`, background: colors[k] || "#5b21b6" }}
            title={`${k}: ${v}`}
          />
        ))}
      </div>
...
const PATTERN_COLORS: Record<string, string> = {
  none: "#16a34a",
  orphan_drug_pricing_crisis: "#dc2626",
  diagnostic_odyssey_barrier: "#7c3aed",
  research_funding_desert: "#b45309",
  regulatory_pathway_blockade: "#0891b2",
  patient_advocacy_capture: "#be185d",
};
const SEVERITY_COLORS: Record<string, string> = {
  "surveillance_maladies_rares_active": "#16a34a",
  "inégalité_accès_thérapeutique_structurelle": "#d97706",
  "crise_recherche_maladies_rares_majeure": "#7c3aed",
  "crise_accès_médicaments_orphelins_systémique": "#dc2626",
};
const ACTION_COLORS: Record<string, string> = {
  "veille_maladies_rares_continue": "#16a34a",
  "amélioration_politiques_accès_thérapeutique": "#d97706",
  "renforcement_parcours_diagnostic_et_recherche": "#7c3aed",
  "intervention_urgente_accès_médicaments_orphelins": "#dc2626",
};
const RISK_BADGE: Record<string, string> = {
  low: "bg-green-900 text-green-300",
  moderate: "bg-amber-900 text-amber-300",
  high: "bg-violet-950 text-violet-300",
  critical: "bg-red-950 text-red-400",
};
const SEV_BADGE: Record<string, string> = {
  "surveillance_maladies_rares_active": "bg-green-900 text-green-300",
  "inégalité_accès_thérapeutique_structurelle": "bg-amber-900 text-amber-300",
  "crise_recherche_maladies_rares_majeure": "bg-violet-950 text-violet-300",
  "crise_accès_médicaments_orphelins_systémique": "bg-red-950 text-red-400",
};

function DetailModal({ entity, onClose }: { entity: RDEEntity; onClose: () => void }) {
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <span className="ml-2 text-slate-500 text-xs">
              {entity.disease_category.replace(/_/g, " ")}
            </span>
          </div>
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ["Score Accès",          entity.access_score,        "#7c3aed"],
              ["Score Recherche",      entity.research_score,      "#0891b2"],
              ["Score Accessibilité",  entity.affordability_score, "#d97706"],
              ["Score Réglementaire",  entity.regulatory_score,    "#dc2626"],
            ].map(([l, v, c]) => (
              <div
                key={String(l)}
            ...
            <div className="col-span-2 bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-400/60 text-xs mb-1">Score Composite Maladies Rares</div>
          ...
          <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-4 text-sm text-slate-200 leading-relaxed">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="bg-slate-800 rounded p-2">
                <div className="text-violet-400/50 text-xs mb-0.5">Prix Médicament Orphelin</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.orphan_drug_price_index * 100)}%
                </div>
              </div>
              <div className="bg-slate-800 rounded p-2">
                <div className="text-violet-400/50 text-xs mb-0.5">Délai Diagnostic</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.diagnostic_delay_years * 100)}%
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-400/60 text-xs mb-1">Catégorie de Maladie</div>
            ...
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              ...
              <div className="text-purple-400 font-medium capitalize">
                {entity.rare_disease_pattern.replace(/_/g, " ")}
              </div>
            </div>
          </div>
        )}
...
export default function RareDiseaseDashboard() {
  ...
  const [selected, setSelected] = useState<RDEEntity | null>(null);

  useEffect(() => {
    fetch("/api/rare-disease-engine")
      .then(r => r.json())
      .then(setData)
      .catch(console.error);
  }, []);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-violet-400 text-lg animate-pulse">
          Initialisation du Moteur Maladies Rares...
        </div>
      </div>
    );
  }

  const { entities, summary, avg_access } = data;
  const filtered = entities.filter(
    e =>
      (riskFilter === "all" || e.risk_level === riskFilter) &&
      (patFilter === "all" || e.rare_disease_pattern === patFilter)
  );
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-violet-400">
          Maladies Rares &amp; Médicaments Orphelins — Module 435
        </h1>
        <p className="text-purple-400/60 text-sm mt-1">
          Accès · Recherche · Accessibilité · Réglementaire — Caelum Partners — Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          ["Crise Accès Orphelins",            summary.critical,                                                                  "text-red-400"],
          ["Crise Majeure Recherche",          summary.high,                                                                      "text-violet-500"],
          ["Composite Moyen",                  `${summary.avg_composite.toFixed(1)}`,                                             "text-purple-400"],
          ["Index Accès Maladies Rares",       `${summary.avg_estimated_rare_disease_access_index.toFixed(2)}/10`,                "text-violet-400"],
          ["Accès Moyen",                      `${avg_access.toFixed(1)}`,                                                       "text-purple-400"],
        ].map(([l, v, c]) => (
          <div
            key={String(l)}
      ...
      <div className="bg-slate-900 border border-violet-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          ...
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.research_score, 0) / (entities.length || 1)}
            label="Recherche Moy."
            color="#0891b2"
          />
          <GaugeRing
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {[
          "all",
          "none",
          "orphan_drug_pricing_crisis",
          "diagnostic_odyssey_barrier",
          "research_funding_desert",
          "regulatory_pathway_blockade",
          "patient_advocacy_capture",
        ].map(p => (
          <button
            key={p}
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div className="text-xs text-slate-500 mb-2 capitalize">
              {e.disease_category.replace(/_/g, " ")}
            </div>
            ...
            <div className="text-xs text-purple-400/60 mb-2 capitalize">
              {e.rare_disease_pattern.replace(/_/g, " ")}
            </div>
            <div className="text-xs text-violet-400/70 font-medium mb-2">
              Accès: {e.access_score.toFixed(1)} · Recherche: {e.research_score.toFixed(1)}
            </div>
