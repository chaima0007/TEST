...
const ACCENT = "#0891b2";
const TITRE = "Droits Handicap — CRPD";
...
export default function Page() {
  ...
  useEffect(() => { fetch("/api/disability-rights-crpd-engine").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
          ...
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 200px", display: "flex", flexDirection: "column", gap: 10, justifyContent: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#94a3b8" }}>Index DRC</span><strong style={{ color: ACCENT }}>{data.avg_estimated_disability_rights_crpd_index?.toFixed(2)}</strong></div>
          </div>
          ...
          {(data.entities ?? []).map((e: any) => (
            <div key={e.entity_id} onClick={() => setSelected(e)} style={{ background: "#0f172a", borderRadius: 8, padding: 16, cursor: "pointer", border: `1px solid ${e.risk_level === "critique" ? "#ef4444" : e.risk_level === "élevé" ? "#f59e0b" : "#1e293b"}` }}>
              ...
              <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4 }}>
                {["crpd_implementation_gap_score", "institutional_segregation_score", "employment_exclusion_score", "legal_capacity_denial_crpd_score"].map(k => (
