...
export default function CorporateImpunityAccountabilityPage() {
  const [data, setData] = useState<DashData | null>(null)
  const [loading, setLoading] = useState(true)
  const [filter, setFilter] = useState("tous")

  useEffect(() => {
    fetch("/api/corporate-impunity-accountability-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false) }).catch(() => { setLoading(false) })
  }, [])

  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Impunité Corporative &amp; Responsabilité…</div>
      </div>
    )
  }
  ...
  const avgComposite = data?.avg_composite ?? 57.61
  ...
  const avgIndex = allEntities.length ? allEntities.reduce((a, e) => a + e.estimated_corporate_impunity_index, 0) / allEntities.length : 0
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Impunité Corporative &amp; Responsabilité</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Corporate Impunity &amp; Accountability Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          ...
          { label: "Index Impunité Moyen", value: avgIndex.toFixed(2) },
        ...
        {filtered.map(e => (
          <div key={e.id}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Impunité: <span className="font-medium" style={{ color: ACCENT }}>{e.estimated_corporate_impunity_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
