...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/migrant-domestic-workers-engine").then(r => r.json()).then(d => { setData(d); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Travailleurs Domestiques</h1>
        <p className="text-slate-400 mt-1">Migrant Domestic Workers Engine — Kafala, exploitation et 67M travailleurs invisibles</p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800"><p className="text-slate-400 text-sm">Indice Domestiques Moyen</p><p className="text-2xl font-bold mt-1" style={{color:ACCENT}}>{loading?"…":(data as any).avg_estimated_migrant_domestic_workers_index?.toFixed(2) ?? "—"}</p><p className="text-slate-500 text-xs mt-1">/10</p></div>
