...
interface Entity {
  ...
  biometric_mass_surveillance_score: number;
  surveillance_export_score: number;
  internet_shutdown_censorship_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_ai_surveillance_index: number;
  last_updated: string;
}
...
interface Data {
  ...
  avg_estimated_ai_surveillance_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Score Crédit Social", v: entity.social_credit_score, c: "#ef4444" },
              { l: "Surveillance Biométrique de Masse", v: entity.biometric_mass_surveillance_score, c: "#dc2626" },
              { l: "Export du Modèle Surveillance", v: entity.surveillance_export_score, c: "#b91c1c" },
              { l: "Coupures Internet & Censure", v: entity.internet_shutdown_censorship_score, c: "#991b1b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Surveillance IA Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_ai_surveillance_index}</span>
            </div>
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Activation des mécanismes de contre-surveillance et protection des dissidents exposés au régime autoritaire</p>
                <p>› Coordination multilatérale urgente pour bloquer l'export des technologies de surveillance autocratique</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Renforcement des protections numériques pour les populations cibles et revue des partenariats technologiques</p>
                <p>› Cartographie approfondie des vecteurs d'exportation du modèle autoritaire de surveillance IA</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance accrue des déploiements de systèmes biométriques et des coupures réseau documentées</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur les évolutions législatives et technologiques de l'appareil de surveillance</p>
            )}
          </div>
        )}
      </div>
...
export default function AISurveillanceAutocracyEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/ai-surveillance-autocracy-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données AI Surveillance Autocracy Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la surveillance autoritaire IA…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Index Surveillance IA Moyen", v: `${(data.avg_estimated_ai_surveillance_index ?? 0).toFixed(2)}`, c: "text-red-300" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>AI Surveillance Autocracy Engine</h1>
          <p className="text-slate-400 text-sm mt-1">Surveillance IA de masse et exportation du modèle autoritaire</p>
        ...
        <div className="bg-slate-900 border border-red-500/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Surveillance IA Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("social_credit_score")} label="Crédit Social" color={ACCENT} />
            <GaugeRing value={avgScore("biometric_mass_surveillance_score")} label="Biométrie Masse" color="#dc2626" />
            <GaugeRing value={avgScore("surveillance_export_score")} label="Export Surveillance" color="#b91c1c" />
            <GaugeRing value={avgScore("internet_shutdown_censorship_score")} label="Censure Internet" color="#991b1b" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-red-500/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns Surveillance Autocratique</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#ef4444", "#dc2626", "#b91c1c", "#991b1b", "#7f1d1d"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_ai_surveillance_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · AI Surveillance Autocracy Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
