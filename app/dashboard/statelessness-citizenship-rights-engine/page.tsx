...
interface Entity {
  ...
  country: string;
  sector: string;
  stateless_population_scale_severity_score: number;
  citizenship_deprivation_arbitrariness_score: number;
  documentation_access_birth_registration_deficit_score: number;
  stateless_rights_protection_mechanism_gap_score: number;
  estimated_statelessness_citizenship_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_statelessness_citizenship_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Population Apatride Échelle &amp; Sévérité", value: entity.stateless_population_scale_severity_score, weight: "0.30" },
    { label: "Privation Nationalité Arbitraire", value: entity.citizenship_deprivation_arbitrariness_score, weight: "0.25" },
    { label: "Déficit Enregistrement Civil", value: entity.documentation_access_birth_registration_deficit_score, weight: "0.25" },
    { label: "Absence Mécanisme Protection", value: entity.stateless_rights_protection_mechanism_gap_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{entity.stateless_population_scale_severity_score.toFixed(1)}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Population Apatride</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_statelessness_citizenship_rights_index === "number" ? entity.estimated_statelessness_citizenship_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Apatridie</div>
                </div>
              </div>
...
export default function StatelessnessCitizenshipRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/statelessness-citizenship-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Apatridie &amp; Droits à la Nationalité…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.stateless_population_scale_severity_score));
  const avgIndex = data?.avg_estimated_statelessness_citizenship_rights_index ?? avg(allEntities.map(e => e.estimated_statelessness_citizenship_rights_index));
  ...
  const avgStateless = avg(allEntities.map(e => e.stateless_population_scale_severity_score));
  const avgDeprivation = avg(allEntities.map(e => e.citizenship_deprivation_arbitrariness_score));
  const avgDocumentation = avg(allEntities.map(e => e.documentation_access_birth_registration_deficit_score));
  const avgMechanism = avg(allEntities.map(e => e.stateless_rights_protection_mechanism_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Apatridie", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Apatridie &amp; Droits à la Nationalité</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Statelessness &amp; Citizenship Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgStateless} label="Population Apatride Échelle" />
          <GaugeRing value={avgDeprivation} label="Privation Nationalité Arbitraire" />
          <GaugeRing value={avgDocumentation} label="Déficit Enregistrement Civil" />
          <GaugeRing value={avgMechanism} label="Absence Mécanisme Protection" />
        </div>
      </div>
          ...
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.stateless_population_scale_severity_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.stateless_population_scale_severity_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Apatridie: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_statelessness_citizenship_rights_index === "number" ? e.estimated_statelessness_citizenship_rights_index.toFixed(2) : "—"}</span>
            </div>
