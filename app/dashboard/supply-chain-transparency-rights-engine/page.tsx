...
interface Entity {
  ...
  last_updated: string;
  corporate_opacity_score?: number;
  due_diligence_absence_score?: number;
  audit_washing_score?: number;
  remedy_access_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "corporate_opacity_score", label: "Opacité corporative" },
  { key: "due_diligence_absence_score", label: "Absence de diligence raisonnable" },
  { key: "audit_washing_score", label: "Audit washing" },
  { key: "remedy_access_gap_score", label: "Lacune d&apos;accès aux recours" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la transparence des chaînes d&apos;approvisionnement : opacité corporative, absence de diligence raisonnable, audit washing et lacunes d&apos;accès aux recours pour les victimes.</p>
              <p className="text-slate-400">Source : Supply Chain Transparency Rights Engine · Wave 253</p>
            </div>
          )}
        </div>
      </div>
...
export default function SupplyChainTransparencyRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/supply-chain-transparency-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Supply Chain Transparency Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Opacité Corporative · Diligence Raisonnable · Audit Washing · Accès aux Recours</p>
        </div>
