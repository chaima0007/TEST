...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque salestech advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_index.toFixed(2)} / 10`,
    `Conformité plateforme salestech : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 384 — Salestech Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Salestech Advertising · Wave 384</div>
          </div>
        ...
        {tab === "signaux" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              `Score composite ${entity.composite_score.toFixed(1)} — ${entity.risk_level === "critique" ? "surveillance salestech urgente" : entity.risk_level === "élevé" ? "monitoring renforcé recommandé" : "suivi standard actif"}`,
              `Indice Caelum Salestech Advertising : ${entity.estimated_index.toFixed(2)} / 10`,
              `Plateforme salestech en ${entity.risk_level === "faible" ? "zone verte — performance nominale" : "zone d&apos;alerte — optimisation requise"}`,
            ].map((sig, i) => (
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(22,163,74,0.15)" }}>
              ...
              <div>{entity.risk_level === "critique" ? "Audit immédiat des plateformes salestech — conformité réglementaire urgente" : entity.risk_level === "élevé" ? "Révision des stratégies salestech — renforcement compliance" : entity.risk_level === "modéré" ? "Optimisation continue des métriques salestech advertising" : "Maintien des bonnes pratiques — veille sectorielle"}</div>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(22,163,74,0.15)" }}>
...
export default function SalestechAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/salestech-advertising")
      .then((r) => r.json())
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Salestech Advertising…
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Salestech Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#16a34a99", margin: 0 }}>
          Caelum Partners · Wave 384 · Analyse publicitaire salestech · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Salestech Advertising Intelligence · Wave 384
      </div>
