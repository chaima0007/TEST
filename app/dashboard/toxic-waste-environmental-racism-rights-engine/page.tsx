...
interface Entity {
  ...
  proximite_dechets_toxiques_communautes_marginalisees_score: number;
  impunite_entreprises_defaillance_reglementaire_score: number;
  impacts_sanitaires_acces_soins_populations_exposees_score: number;
  capacite_recours_juridique_representation_victimes_score: number;
  estimated_toxic_waste_environmental_racism_rights_index: number;
  risk_level: string;
...
const SUB_SCORES = [
  { key: "proximite_dechets_toxiques_communautes_marginalisees_score", label: "Proximité Déchets Toxiques" },
  { key: "impunite_entreprises_defaillance_reglementaire_score", label: "Impunité Entreprises" },
  { key: "impacts_sanitaires_acces_soins_populations_exposees_score", label: "Impacts Sanitaires" },
  { key: "capacite_recours_juridique_representation_victimes_score", label: "Recours Juridique Victimes" },
];
        ...
        <div className="flex border-b border-slate-800">
          {["Aperçu", "Métriques"].map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 text-green-400" : "text-slate-400 hover:text-white"}`}
          ...
          {tab === 0 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Indice Racisme Environnemental</span>
                <span className="font-semibold" style={{ color: ACCENT_LIGHT }}>{entity.estimated_toxic_waste_environmental_racism_rights_index}/10</span>
              </div>
              <div className="flex items-center justify-between text-sm">
...
export default function ToxicWasteEnvironmentalRacismRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/toxic-waste-environmental-racism-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT_LIGHT }}>Racisme Environnemental &amp; Déchets Toxiques</h1>
          <p className="text-slate-400 mt-1 text-sm">Pollution · Communautés Marginalisées · Justice Environnementale · UNSR Toxics</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice TWR</p>
        ...
        {filtered.map(e => {
          ...
          return (
            <button key={e.id} onClick={() => setSelected(e)}
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold uppercase ${cfg.color}`}>{cfg.label}</span>
                <span className="text-xs text-slate-500">Indice: <span className="font-bold" style={{ color: ACCENT_LIGHT }}>{e.estimated_toxic_waste_environmental_racism_rights_index}</span></span>
              </div>
            </button>
          );
        })}
