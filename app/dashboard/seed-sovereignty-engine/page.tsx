...
interface SSEntity {
  ...
  composite_score: number; corporate_seed_control_score: number;
  genetic_erosion_score: number; farmer_seed_rights_violation_score: number;
  gmo_dependency_score: number; risk_level: string; primary_pattern: string;
  key_signals: string[]; estimated_seed_risk_index: number; last_updated: string;
}
interface SSSummary {
  ...
  confidence_score: number; data_sources: string[]; entities: SSEntity[];
  avg_estimated_seed_risk_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: SSEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Contrôle Corporatif", v: entity.corporate_seed_control_score, c: "#ef4444" },
              { l: "Érosion Génétique", v: entity.genetic_erosion_score, c: "#f97316" },
              { l: "Droits Paysans Violés", v: entity.farmer_seed_rights_violation_score, c: "#a855f7" },
              { l: "Dépendance OGM", v: entity.gmo_dependency_score, c: "#f59e0b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Risque Semencier</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_seed_risk_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Plan d'urgence pour la souveraineté semencière nationale</p><p>› Création de banques de semences publiques indépendantes</p><p>› Interdiction des brevets sur les semences paysannes</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des droits des agriculteurs sur les semences</p><p>› Soutien aux variétés locales et programmes de préservation génétique</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance accrue des pratiques corporatives semencières</p><p>› Développement de partenariats avec des réseaux de graines paysannes</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la vigilance sur la diversité semencière</p><p>› Consolidation des systèmes de semences communautaires</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  dependance_semenciere_totale: "#ef4444",
  erosion_genetique_critique: "#f97316",
  oligopole_semencier: "#f59e0b",
  pression_biodiversite: "#eab308",
  souverainete_preservee: "#10b981",
};

export default function SeedSovereigntyEnginePage() {
  const [data, setData] = useState<SSSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<SSEntity | null>(null);

  useEffect(() => {
    fetch("/api/seed-sovereignty-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données souveraineté semencière"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la souveraineté semencière en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof SSEntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Souveraineté Semencière</h1>
          <p className="text-slate-400 text-sm mt-1">Le contrôle des semences comme arme géopolitique — qui contrôle les semences contrôle l'alimentation</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Composite Moyen", v: `${data.avg_composite}%`, t: "text-green-300" },
            { l: "Alertes Critiques", v: data.critical_alerts.length, t: "text-red-300" },
            { l: "Indice Risque Semencier", v: data.avg_estimated_seed_risk_index, t: "text-green-400" },
            { l: "Sources", v: data.data_sources?.length ?? 0, t: "text-emerald-300" },
            { l: "Confiance Analyse", v: `${Math.round((data.confidence_score ?? 0) * 100)}%`, t: "text-slate-300" },
          ].map(({ l, v, t }) => (
            <div key={l} className="bg-slate-900 border border-green-500/20 rounded-xl p-4">
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-green-500/20">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Dimensions Semencières — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("corporate_seed_control_score")} label="Contrôle Corporatif" color={ACCENT} />
            <GaugeRing value={avgScore("genetic_erosion_score")} label="Érosion Génétique" color="#f97316" />
            <GaugeRing value={avgScore("farmer_seed_rights_violation_score")} label="Droits Paysans Violés" color="#a855f7" />
            <GaugeRing value={avgScore("gmo_dependency_score")} label="Dépendance OGM" color="#f59e0b" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-green-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Contrôle Semencier</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-green-900/10 border border-green-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Semencières Critiques</h2>
            <ul className="space-y-1">{data.critical_alerts.map((a, i) => <li key={i} className="text-sm text-green-300">› {a}</li>)}</ul>
          </div>
        )}
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_seed_risk_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Souveraineté Semencière · {data.last_analysis}</p>
      </div>
    </div>
  );
}
