...
interface Entity {
  ...
  country: string;
  sector: string;
  sharia_apostasy_lgbtq_death_penalty_score: number;
  conversion_therapy_state_sanctioned_scale_score: number;
  religious_institution_political_power_rights_rollback_score: number;
  lgbtq_asylum_protection_deficit_score: number;
  estimated_religious_law_lgbtq_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_religious_law_lgbtq_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Peine Mort Droit Religieux", value: entity.sharia_apostasy_lgbtq_death_penalty_score, weight: "0.30" },
    { label: "Thérapies Conversion Étatiques", value: entity.conversion_therapy_state_sanctioned_scale_score, weight: "0.25" },
    { label: "Pouvoir Religieux Rétrogression", value: entity.religious_institution_political_power_rights_rollback_score, weight: "0.25" },
    { label: "Déficit Asile LGBTQ+", value: entity.lgbtq_asylum_protection_deficit_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.sharia_apostasy_lgbtq_death_penalty_score === "number" ? entity.sharia_apostasy_lgbtq_death_penalty_score.toFixed(1) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Peine Mort Religieuse</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_religious_law_lgbtq_rights_index === "number" ? entity.estimated_religious_law_lgbtq_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits LGBTQ+ Religieux</div>
                </div>
              </div>
...
export default function ReligiousLawLgbtqRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/religious-law-lgbtq-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droit Religieux &amp; Droits LGBTQ+…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.sharia_apostasy_lgbtq_death_penalty_score));
  const avgIndex = data?.avg_estimated_religious_law_lgbtq_rights_index ?? avg(allEntities.map(e => e.estimated_religious_law_lgbtq_rights_index));
  ...
  const avgSharia = avg(allEntities.map(e => e.sharia_apostasy_lgbtq_death_penalty_score));
  const avgConversion = avg(allEntities.map(e => e.conversion_therapy_state_sanctioned_scale_score));
  const avgReligious = avg(allEntities.map(e => e.religious_institution_political_power_rights_rollback_score));
  const avgAsylum = avg(allEntities.map(e => e.lgbtq_asylum_protection_deficit_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits LGBTQ+ Religieux", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droit Religieux &amp; Droits LGBTQ+</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Religious Law &amp; LGBTQ+ Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgSharia} label="Peine Mort Droit Religieux" />
          <GaugeRing value={avgConversion} label="Thérapies Conversion Étatiques" />
          <GaugeRing value={avgReligious} label="Pouvoir Religieux Rétrogression" />
          <GaugeRing value={avgAsylum} label="Déficit Asile LGBTQ+" />
        </div>
      </div>
          ...
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.sharia_apostasy_lgbtq_death_penalty_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.sharia_apostasy_lgbtq_death_penalty_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Droits LGBTQ+ Religieux: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_religious_law_lgbtq_rights_index === "number" ? e.estimated_religious_law_lgbtq_rights_index.toFixed(2) : "—"}</span>
            </div>
