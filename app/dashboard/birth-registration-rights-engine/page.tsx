...
interface Entity {
  ...
  unregistered_births_score: number;
  stateless_children_risk_score: number;
  documentation_access_barrier_score: number;
  civil_registry_collapse_score: number;
  estimated_birth_registration_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_birth_registration_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Naissances Non Enregistrées", value: entity.unregistered_births_score, weight: "0.30" },
    { label: "Risque Apatridie Enfants", value: entity.stateless_children_risk_score, weight: "0.25" },
    { label: "Barrières Documentation", value: entity.documentation_access_barrier_score, weight: "0.25" },
    { label: "Effondrement Registre Civil", value: entity.civil_registry_collapse_score, weight: "0.20" },
  ];
          ...
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: 16, textAlign: "center" }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: ACCENT }}>{typeof entity.estimated_birth_registration_rights_index === "number" ? entity.estimated_birth_registration_rights_index.toFixed(2) : "—"}</div>
                  <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>Index Enregistrement</div>
                </div>
              </div>
...
export default function BirthRegistrationRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/birth-registration-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ background: "#020617", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: ACCENT, fontSize: 14 }}>Initialisation Enregistrement des Naissances &amp; Droits Civils…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_birth_registration_rights_index ?? avg(allEntities.map(e => e.estimated_birth_registration_rights_index));
  ...
  const avgUnregistered = avg(allEntities.map(e => e.unregistered_births_score));
  const avgStateless = avg(allEntities.map(e => e.stateless_children_risk_score));
  const avgDocBarrier = avg(allEntities.map(e => e.documentation_access_barrier_score));
  const avgRegistryCollapse = avg(allEntities.map(e => e.civil_registry_collapse_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Enregistrement", value: avgIndex.toFixed(2) },
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "#f8fafc", padding: 24, display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.5px" }}>Enregistrement des Naissances &amp; Droits Civils</h1>
        </div>
        <p style={{ fontSize: 13, color: "#64748b", marginLeft: 16 }}>
          Birth Registration Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: 24 }}>
          <GaugeRing value={avgUnregistered} label="Naissances Non Enregistrées" />
          <GaugeRing value={avgStateless} label="Risque Apatridie" />
          <GaugeRing value={avgDocBarrier} label="Barrières Documentation" />
          <GaugeRing value={avgRegistryCollapse} label="Effondrement Registre" />
        </div>
      </div>
          ...
          return (
            <div key={e.entity_id} onClick={() => setSelected(e)}
              ...
              <div style={{ fontSize: 11, color: "#64748b" }}>
                Index: <span style={{ fontWeight: 600, color: ACCENT }}>{typeof e.estimated_birth_registration_rights_index === "number" ? e.estimated_birth_registration_rights_index.toFixed(2) : "—"}</span>
              </div>
            </div>
          );
        })}
