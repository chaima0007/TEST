...
interface Entity {
  ...
  risk_level: string
  estimated_mine_contamination_index: number
  [key: string]: unknown
}
...
export default function AntiPersonnelMinesClusterMunitionsPage() {
  const [data, setData] = useState<DashData | null>(null)
  const [loading, setLoading] = useState(true)
  const [filter, setFilter] = useState("tous")

  useEffect(() => {
    fetch("/api/anti-personnel-mines-cluster-munitions-engine")
      .then(r => r.json())
  ...
  const avgIndex = avg(allEntities.map(e => e.estimated_mine_contamination_index))
  ...
  const kpis = [
    ...
    { label: "Indice contamination", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400 text-sm ml-6">
          Contamination par mines et engins non explosés — violations droits humains
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <GaugeRing value={avgIndex * 10} label="Indice contamination (×10)" />
        ...
        {filtered.map(e => (
          <div key={e.entity_id}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Indice contamination: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_mine_contamination_index === "number" ? e.estimated_mine_contamination_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
