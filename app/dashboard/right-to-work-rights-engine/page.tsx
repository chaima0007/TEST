...
const ACCENT = "#0d9488";
const TITRE = "Droit au Travail Décent";
...
export default function Page() {
  ...
  useEffect(() => { fetch("/api/right-to-work-rights-engine").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
  ...
  return (
    <div style={{ background: "#020617", minHeight: "100vh", color: "#f1f5f9", fontFamily: "system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <p style={{ color: "#94a3b8", marginBottom: 32 }}>CaelumSwarm™ · CSDDD 2024/1760 · Wave 240</p>
        <div style={{ display: "flex", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
          ...
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 200px", display: "flex", flexDirection: "column", gap: 10, justifyContent: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#94a3b8" }}>Index RTW</span><strong style={{ color: ACCENT }}>{data.avg_estimated_right_to_work_rights_index?.toFixed(2)}</strong></div>
          </div>
          ...
          {(data.entities ?? []).map((e: any) => (
            <div key={e.entity_id} onClick={() => setSelected(e)} style={{ background: "#0f172a", borderRadius: 8, padding: 16, cursor: "pointer", border: `1px solid ${e.risk_level === "critique" ? "#ef4444" : e.risk_level === "élevé" ? "#f59e0b" : "#1e293b"}` }}>
              ...
              <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4 }}>
                {["decent_work_denial_score", "forced_labor_conditions_score", "union_access_denial_score", "precarious_employment_score"].map(k => (
