...
interface Entity {
  ...
  country: string;
  sector: string;
  land_grabbing_smallholder_displacement_score: number;
  corporate_seed_monopoly_farmer_dependency_score: number;
  food_import_dependency_price_shock_vulnerability_score: number;
  traditional_farming_knowledge_erosion_score: number;
  estimated_food_sovereignty_agricultural_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_food_sovereignty_agricultural_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Accaparement Terres", value: entity.land_grabbing_smallholder_displacement_score, weight: "0.30" },
    { label: "Monopole Semences Corporatif", value: entity.corporate_seed_monopoly_farmer_dependency_score, weight: "0.25" },
    { label: "Dépendance Alimentaire", value: entity.food_import_dependency_price_shock_vulnerability_score, weight: "0.25" },
    { label: "Érosion Savoirs Agricoles", value: entity.traditional_farming_knowledge_erosion_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.land_grabbing_smallholder_displacement_score === "number" ? entity.land_grabbing_smallholder_displacement_score.toFixed(1) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Accaparement Terres</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_food_sovereignty_agricultural_rights_index === "number" ? entity.estimated_food_sovereignty_agricultural_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Souveraineté Alimentaire</div>
                </div>
              </div>
...
export default function FoodSovereigntyAgriculturalRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/food-sovereignty-agricultural-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Souveraineté Alimentaire &amp; Droits Agricoles…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.land_grabbing_smallholder_displacement_score));
  const avgIndex = data?.avg_estimated_food_sovereignty_agricultural_rights_index ?? avg(allEntities.map(e => e.estimated_food_sovereignty_agricultural_rights_index));
  ...
  const avgLand = avg(allEntities.map(e => e.land_grabbing_smallholder_displacement_score));
  const avgSeed = avg(allEntities.map(e => e.corporate_seed_monopoly_farmer_dependency_score));
  const avgImport = avg(allEntities.map(e => e.food_import_dependency_price_shock_vulnerability_score));
  const avgTraditional = avg(allEntities.map(e => e.traditional_farming_knowledge_erosion_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Souveraineté Alimentaire", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Souveraineté Alimentaire &amp; Droits Agricoles</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Food Sovereignty &amp; Agricultural Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgLand} label="Accaparement Terres" />
          <GaugeRing value={avgSeed} label="Monopole Semences Corporatif" />
          <GaugeRing value={avgImport} label="Dépendance Alimentaire" />
          <GaugeRing value={avgTraditional} label="Érosion Savoirs Agricoles" />
        </div>
      </div>
          ...
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.land_grabbing_smallholder_displacement_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.land_grabbing_smallholder_displacement_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Souveraineté Alimentaire: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_food_sovereignty_agricultural_rights_index === "number" ? e.estimated_food_sovereignty_agricultural_rights_index.toFixed(2) : "—"}</span>
            </div>
