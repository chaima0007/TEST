...
interface Entity {
  ...
  war_crimes_severity_documentation_score: number;
  ihl_violations_civilian_harm_score: number;
  impunity_gap_icc_referral_score: number;
  accountability_reparation_mechanism_score: number;
  estimated_war_crimes_accountability_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_war_crimes_accountability_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Sévérité & Documentation Crimes de Guerre", value: entity.war_crimes_severity_documentation_score, weight: "0.30" },
    { label: "Violations DIH & Dommages Civils", value: entity.ihl_violations_civilian_harm_score, weight: "0.25" },
    { label: "Déficit Impunité & Renvoi CPI", value: entity.impunity_gap_icc_referral_score, weight: "0.25" },
    { label: "Mécanismes Responsabilité & Réparations", value: entity.accountability_reparation_mechanism_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT_DISPLAY }}>{typeof entity.estimated_war_crimes_accountability_index === "number" ? entity.estimated_war_crimes_accountability_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Crimes de Guerre</div>
                </div>
              </div>
...
export default function WarCrimesAccountabilityPage() {
  ...
  useEffect(() => {
    fetch("/api/war-crimes-accountability-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT_DISPLAY }}>Initialisation Crimes de Guerre & Responsabilité…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_war_crimes_accountability_index ?? avg(allEntities.map(e => e.estimated_war_crimes_accountability_index));
  ...
  const avgCrimes = avg(allEntities.map(e => e.war_crimes_severity_documentation_score));
  const avgDIH = avg(allEntities.map(e => e.ihl_violations_civilian_harm_score));
  const avgImpunite = avg(allEntities.map(e => e.impunity_gap_icc_referral_score));
  const avgReparations = avg(allEntities.map(e => e.accountability_reparation_mechanism_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Crimes de Guerre", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Crimes de Guerre & Responsabilité</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          War Crimes Accountability Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgCrimes} label="Sévérité Crimes" />
          <GaugeRing value={avgDIH} label="Violations DIH" />
          <GaugeRing value={avgImpunite} label="Impunité & CPI" />
          <GaugeRing value={avgReparations} label="Réparations" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Crimes de Guerre: <span className="font-medium" style={{ color: ACCENT_DISPLAY }}>{typeof e.estimated_war_crimes_accountability_index === "number" ? e.estimated_war_crimes_accountability_index.toFixed(2) : "—"}</span>
            </div>
