...
interface Entity { entity: string; risk_level: string; avg_composite: number; estimated_carceral_capitalism_index: number; }
...
export default function CarceralCapitalismPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/carceralcapitalism");
  ...
  return (
    <div className="p-6 space-y-6 text-slate-100">
      <div>
        <h1 className="text-2xl font-bold text-white">Capitalisme Carcéral</h1>
        <p className="text-slate-400 text-sm mt-1">Prisons privées, profit sur la détention et droits des incarcérés</p>
      </div>
