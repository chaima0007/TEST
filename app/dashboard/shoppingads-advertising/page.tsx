...
type Entity = {
  ...
  risk_level: string;
  estimated_shoppingads_index: number;
};
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque shopping ads advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_shoppingads_index.toFixed(2)} / 10`,
    `Conformité plateforme shopping ads : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 405 — Shopping Ads Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Shopping Ads Advertising · Wave 405</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: entity.estimated_shoppingads_index * 10, color: COLOR },
        ...
        {tab === "signaux" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              `Score composite ${entity.composite_score.toFixed(1)} — ${entity.risk_level === "critique" ? "surveillance shopping ads urgente" : entity.risk_level === "élevé" ? "monitoring renforcé recommandé" : "suivi standard actif"}`,
              `Indice Caelum Shopping Ads Advertising : ${entity.estimated_shoppingads_index.toFixed(2)} / 10`,
              "Analyse conformité CSDDD pour la chaîne d&apos;approvisionnement shopping ads. Risques liés aux conditions de travail chez les fournisseurs de produits référencés et les prestataires logistiques.",
            ].map((sig, i) => (
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(22,163,74,0.15)" }}>
              ...
              <div>{entity.risk_level === "critique" ? "Auditer marchands partenaires plateforme — conformité réglementaire urgente" : entity.risk_level === "élevé" ? "Vérifier conditions fournisseurs produits — renforcement compliance" : entity.risk_level === "modéré" ? "Code conduite shopping ads — optimisation continue" : "Certifier SA8000 — maintien bonnes pratiques"}</div>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(22,163,74,0.15)" }}>
              <div style={{ fontSize: 11, color: "#93c5fd", marginBottom: 4 }}>Index Caelum</div>
              <div style={{ color: COLOR, fontWeight: 700, fontSize: 18 }}>{entity.estimated_shoppingads_index.toFixed(2)} / 10</div>
            </div>
          </div>
        )}
      </div>
...
export default function ShoppingAdsAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/shoppingads-advertising")
      .then((r) => r.json())
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Shopping Ads Advertising…
      </div>
    );
  }
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + e.estimated_shoppingads_index, 0) / entities.length
    : 0;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Shopping Ads Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#16a34a99", margin: 0 }}>
          Caelum Partners · Wave 405 · Analyse publicitaire shopping ads · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
        ...
        {filtered.map((entity, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
              ...
              <span style={{ color: COLOR, fontWeight: 600 }}>{entity.estimated_shoppingads_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Shopping Ads Advertising Intelligence · Wave 405
      </div>
