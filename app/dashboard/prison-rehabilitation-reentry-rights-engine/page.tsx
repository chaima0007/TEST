...
interface Entity {
  ...
  prison_overcrowding_inhumane_conditions_severity_score: number;
  rehabilitation_education_program_absence_scale_score: number;
  solitary_confinement_prolonged_use_score: number;
  post_release_reintegration_support_deficit_gap_score: number;
  estimated_prison_rehabilitation_reentry_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_prison_rehabilitation_reentry_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Surpopulation & Conditions Inhumaines", value: entity.prison_overcrowding_inhumane_conditions_severity_score, weight: "0.30" },
    { label: "Absence Programmes Réhabilitation", value: entity.rehabilitation_education_program_absence_scale_score, weight: "0.25" },
    { label: "Isolement Prolongé", value: entity.solitary_confinement_prolonged_use_score, weight: "0.25" },
    { label: "Déficit Soutien Post-Libération", value: entity.post_release_reintegration_support_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_prison_rehabilitation_reentry_rights_index === "number" ? entity.estimated_prison_rehabilitation_reentry_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Réhabilitation Carcérale</div>
                </div>
              </div>
...
export default function PrisonRehabilitationReentryRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/prison-rehabilitation-reentry-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Réhabilitation Carcérale & Réinsertion…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_prison_rehabilitation_reentry_rights_index ?? avg(allEntities.map(e => e.estimated_prison_rehabilitation_reentry_rights_index));
  ...
  const avgOvercrowding = avg(allEntities.map(e => e.prison_overcrowding_inhumane_conditions_severity_score));
  const avgRehabilitation = avg(allEntities.map(e => e.rehabilitation_education_program_absence_scale_score));
  const avgSolitary = avg(allEntities.map(e => e.solitary_confinement_prolonged_use_score));
  const avgPostRelease = avg(allEntities.map(e => e.post_release_reintegration_support_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Réhabilitation", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Réhabilitation Carcérale & Réinsertion</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Prison Rehabilitation & Reentry Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgOvercrowding} label="Surpopulation & Conditions Inhumaines" />
          <GaugeRing value={avgRehabilitation} label="Absence Programmes Réhabilitation" />
          <GaugeRing value={avgSolitary} label="Isolement Prolongé" />
          <GaugeRing value={avgPostRelease} label="Déficit Soutien Post-Libération" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Réhabilitation: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_prison_rehabilitation_reentry_rights_index === "number" ? e.estimated_prison_rehabilitation_reentry_rights_index.toFixed(2) : "—"}</span>
            </div>
