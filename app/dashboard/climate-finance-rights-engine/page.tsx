...
interface Entity {
  ...
  last_updated: string;
  loss_damage_funding_gap_score?: number;
  adaptation_fund_exclusion_score?: number;
  green_conditionality_harm_score?: number;
  climate_finance_transparency_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "loss_damage_funding_gap_score", label: "Lacune de financement pertes et dommages" },
  { key: "adaptation_fund_exclusion_score", label: "Exclusion du fonds d&apos;adaptation" },
  { key: "green_conditionality_harm_score", label: "Préjudice des conditionnalités vertes" },
  { key: "climate_finance_transparency_gap_score", label: "Manque de transparence finance climat" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits à la finance climatique : lacunes de financement pertes et dommages, exclusion des fonds d&apos;adaptation, conditionnalités vertes préjudiciables et manque de transparence dans la finance climat.</p>
              <p className="text-slate-400">Source : Climate Finance Rights Engine · Wave 254</p>
            </div>
          )}
        </div>
      </div>
...
export default function ClimateFinanceRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/climate-finance-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Climate Finance Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Pertes & Dommages · Fonds d&apos;Adaptation · Conditionnalités Vertes · Transparence</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-green-900/40 text-green-300"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "#16653420" } : {}}>
