...
type CSEntity = {
  ...
  forced_child_recruitment_score: number;
  child_combat_deployment_score: number;
  state_armed_group_complicity_score: number;
  child_soldier_impunity_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_child_soldiers_index: number;
  last_updated: string;
};
...
function GaugeRing({ value, label, color }: { value: number; label: string; color: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span className="text-xs text-[#f59e0b]/70 text-center">{label}</span>
    </div>
  );
}
...
function DetailModal({ entity, onClose }: { entity: CSEntity; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="bg-slate-900 border border-[#f59e0b]/20 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl"
        ...
        <div className="p-6">
          {tab === "scores" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold text-[#f59e0b]">
                    {entity.estimated_child_soldiers_index.toFixed(1)}
                  </div>
                  <div className="text-xs text-slate-400 mt-1">Index Enfants Soldats</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Recrutement Forcé", value: entity.forced_child_recruitment_score },
                  { label: "Déploiement Combat", value: entity.child_combat_deployment_score },
                  { label: "Complicité Étatique", value: entity.state_armed_group_complicity_score },
                  { label: "Impunité Recruteurs", value: entity.child_soldier_impunity_score },
                ].map((s) => (
          ...
          {tab === "actions" && (
            <div className="space-y-3">
              {[
                "Signaler aux mécanismes onusiens de protection de l'enfance",
                "Documenter les cas de recrutement pour les juridictions internationales",
                "Coordonner avec les ONG de démobilisation et réintégration",
                "Alerter les ambassades et organismes régionaux compétents",
                "Activer les protocoles d'urgence pour enfants en zones de conflit",
              ].map((action, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 bg-slate-800/50 rounded-lg p-3 text-sm text-slate-300"
                >
                  <span className="w-6 h-6 rounded-full bg-[#f59e0b]/20 text-[#f59e0b] flex items-center justify-center text-xs font-bold flex-shrink-0">
                    {i + 1}
                  </span>
                  {action}
                </div>
...
export default function ChildSoldiersEnginePage() {
  const [entities, setEntities] = useState<CSEntity[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<CSEntity | null>(null);

  useEffect(() => {
    setLoading(true);
    fetch("/api/child-soldiers-engine")
      .then((r) => r.json())
      .then((json: ApiResponse) => {
        const rows = json?.data ?? (Array.isArray(json) ? json : []);
        setEntities(rows as CSEntity[]);
        setLoading(false);
      })
      .catch(() => {
  ...
  const avgIndex = avg(entities.map((e) => e.estimated_child_soldiers_index));
  ...
  const avgForcedRecruitment = avg(entities.map((e) => e.forced_child_recruitment_score));
  const avgCombatDeployment = avg(entities.map((e) => e.child_combat_deployment_score));
  const avgStateComplicity = avg(entities.map((e) => e.state_armed_group_complicity_score));
  const avgImpunity = avg(entities.map((e) => e.child_soldier_impunity_score));
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-white">Child Soldiers Engine</h1>
        </div>
        <p className="text-slate-400 ml-5">
          Recrutement forcé d&apos;enfants soldats, militarisation des mineurs et criminalité de guerre — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        {[
          ...
          { label: "Index Enfants Soldats Moyen", value: avgIndex.toFixed(1), suffix: "" },
          ...
          <div
            key={kpi.label}
            className="bg-slate-900 border border-[#f59e0b]/10 rounded-xl p-4 text-center"
          >
      ...
      {/* Gauge Rings */}
      <div className="bg-slate-900 border border-[#f59e0b]/10 rounded-2xl p-6 mb-8">
        <h2 className="text-sm font-semibold text-slate-300 mb-6">
          Scores Moyens par Dimension
        </h2>
        <div className="flex flex-wrap justify-around gap-6">
          <GaugeRing value={avgForcedRecruitment} label="Recrutement Forcé" color="#f59e0b" />
          <GaugeRing value={avgCombatDeployment} label="Déploiement Combat" color="#ef4444" />
          <GaugeRing value={avgStateComplicity} label="Complicité Étatique" color="#8b5cf6" />
          <GaugeRing value={avgImpunity} label="Impunité Recruteurs" color="#06b6d4" />
        </div>
      </div>
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs text-slate-500">Index CS</div>
                </div>
              </div>
              <div
