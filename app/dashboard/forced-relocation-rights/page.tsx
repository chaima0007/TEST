...
interface Entity {
  ...
  compensation_rights_score?: number;
  resettlement_quality_score?: number;
  legal_protection_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "displacement_risk_score", label: "Risque de déplacement" },
  { key: "compensation_rights_score", label: "Droits à la compensation" },
  { key: "resettlement_quality_score", label: "Qualité de réinstallation" },
  { key: "legal_protection_score", label: "Protection juridique" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits contre les déplacements forcés : risque de déplacement, droits à la compensation, qualité de réinstallation et protection juridique affectant les populations vulnérables et déplacées.</p>
              <p className="text-slate-400">Source : Forced Relocation Rights Engine · Wave 266</p>
            </div>
          )}
        </div>
      </div>
...
export default function ForcedRelocationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/forced-relocation-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits contre les Déplacements Forcés</h1>
          <p className="text-slate-400 mt-1 text-sm">Risque de Déplacement · Compensation · Réinstallation · Protection Juridique</p>
        </div>
