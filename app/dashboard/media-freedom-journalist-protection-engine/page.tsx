...
export default function MediaFreedomJournalistProtectionPage() {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  useEffect(() => {
    fetch("/api/media-freedom-journalist-protection-engine")
      .then((r) => r.json())
  ...
  const avgIndex = (data.avg_estimated_media_freedom_journalist_protection_index as number) ?? 0;
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-6">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: GREEN }}>
            Liberté de la Presse — Protection des Journalistes
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            RSF World Press Freedom · CPJ · IFJ · SLAPP Laws · Digital Surveillance Press
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Avg Composite", value: avgComposite.toFixed(2), unit: "/100" },
            { label: "Indice Presse", value: avgIndex.toFixed(2), unit: "/10" },
            ...
            <div key={label} className="rounded-xl bg-slate-900 border border-slate-800 p-4 text-center">
              ...
              <div className="text-2xl font-bold" style={{ color: GREEN }}>{value}<span className="text-sm text-slate-500 ml-1">{unit}</span></div>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 flex flex-col items-center">
            <div className="text-xs text-slate-500 uppercase tracking-widest mb-4">Score Global Liberté Presse</div>
            <div className="w-32 h-32"><GaugeRing value={avgComposite} color={GREEN} /></div>
            <div className="mt-3 text-3xl font-bold" style={{ color: GREEN }}>{avgComposite.toFixed(1)}</div>
          </div>
          ...
          <div className="rounded-xl bg-slate-900 border border-slate-800 overflow-hidden">
            ...
            <div className="divide-y divide-slate-800">
              {entities.map((e) => {
                ...
                const idx = e.estimated_media_freedom_journalist_protection_index as number;
                return (
        ...
        <div className="rounded-xl border p-5" style={{ borderColor: GREEN + "40", backgroundColor: GREEN + "10" }}>
          <div className="text-xs font-semibold mb-1" style={{ color: GREEN }}>RSF 2023 — Alerte Mondiale</div>
          <p className="text-xs text-slate-300 leading-relaxed">
            67 journalistes tués en 2023. 521 emprisonnés. L&apos;Érythrée détient des journalistes depuis 2001 sans procès.
            La Corée du Nord n&apos;autorise aucun média indépendant depuis 1948.
          </p>
        </div>
      </div>
    </div>
