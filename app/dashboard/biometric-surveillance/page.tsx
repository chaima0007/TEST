...
interface SwarmData { engine: string; avg_composite: number; distribution: Record<string, number>; entities: Entity[]; estimated_biometric_surveillance_index: number; }
...
export default function BiometricSurveillancePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/biometricsurveillance");
  ...
  return (
    <div className="p-6 space-y-6 text-slate-100">
      <div>
        <h1 className="text-2xl font-bold text-white">Surveillance Biométrique &amp; Génétique</h1>
        <p className="text-slate-400 text-sm mt-1">Collecte non consentie, profilage ethnique et droits à la vie privée</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        {data.estimated_biometric_surveillance_index != null && (
          <p className="text-xs text-slate-500 mt-3">Indice surveillance biométrique : <span className="text-indigo-300 font-semibold">{data.estimated_biometric_surveillance_index}/10</span></p>
        )}
      </div>
    </div>
  );
