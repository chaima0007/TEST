...
export default function ArmsTradeCivilianHarmPage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#7c0a02";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", "modéré": "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/arms-trade-civilian-harm-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_arms_trade_civilian_harm_index"] as number) ?? 0;
