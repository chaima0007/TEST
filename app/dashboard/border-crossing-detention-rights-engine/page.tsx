...
interface Entity {
  ...
  border_violence_pushback_death_severity_score: number;
  immigration_detention_conditions_duration_scale_score: number;
  family_separation_unaccompanied_minors_score: number;
  stateless_person_documentation_legal_aid_deficit_gap_score: number;
  estimated_border_crossing_detention_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_border_crossing_detention_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Violence Frontalière & Refoulements Mortels", value: entity.border_violence_pushback_death_severity_score, weight: "0.30" },
    { label: "Conditions & Durée Détention Immigration", value: entity.immigration_detention_conditions_duration_scale_score, weight: "0.25" },
    { label: "Séparation Familiale & Mineurs Non Accompagnés", value: entity.family_separation_unaccompanied_minors_score, weight: "0.25" },
    { label: "Déficit Documentation Apatrides & Aide Juridique", value: entity.stateless_person_documentation_legal_aid_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_border_crossing_detention_rights_index === "number" ? entity.estimated_border_crossing_detention_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Frontières</div>
                </div>
              </div>
...
export default function BorderCrossingDetentionRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/border-crossing-detention-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits aux Frontières & Détention…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_border_crossing_detention_rights_index ?? avg(allEntities.map(e => e.estimated_border_crossing_detention_rights_index));
  ...
  const avgBorderViolence = avg(allEntities.map(e => e.border_violence_pushback_death_severity_score));
  const avgDetention = avg(allEntities.map(e => e.immigration_detention_conditions_duration_scale_score));
  const avgFamilySep = avg(allEntities.map(e => e.family_separation_unaccompanied_minors_score));
  const avgStateless = avg(allEntities.map(e => e.stateless_person_documentation_legal_aid_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Frontières", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits aux Frontières & Détention</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Border Crossing Detention Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgBorderViolence} label="Violence Frontalière" />
          <GaugeRing value={avgDetention} label="Détention Immigration" />
          <GaugeRing value={avgFamilySep} label="Séparation Familiale" />
          <GaugeRing value={avgStateless} label="Apatrides & Aide Juridique" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Frontières: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_border_crossing_detention_rights_index === "number" ? e.estimated_border_crossing_detention_rights_index.toFixed(2) : "—"}</span>
            </div>
