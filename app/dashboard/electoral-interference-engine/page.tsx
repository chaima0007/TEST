...
function DetailModal({ entity, onClose }: { entity: EIEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Plan national d'urgence pour la protection de l'intégrité électorale</p><p>› Contre-mesures immédiates contre les opérations d'ingérence étrangère</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des infrastructures électorales contre les cyberattaques</p><p>› Lutte contre la désinformation et le financement électoral illicite</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance accrue des campagnes de désinformation numériques</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille sur les opérations d'influence étrangères</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function ElectoralInterferencePage() {
  const [data, setData] = useState<EISummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<EIEntity | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/electoral-interference-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données ingérence électorale"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse de l'ingérence électorale mondiale…</div></div>;
  ...
  const avgScore = (k: keyof EIEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Distribution Risques d'Ingérence</h2>
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns d'Ingérence Électorale</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={data.total_entities} color={["#ef4444", "#f97316", "#eab308", "#10b981", "#3b82f6"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-blue-900/10 border border-blue-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-blue-400 mb-2">Alertes Ingérence Électorale Critiques</h2>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Ingérence Électorale Étrangère · {data.last_analysis}</p>
      </div>
    </div>
  );
}
