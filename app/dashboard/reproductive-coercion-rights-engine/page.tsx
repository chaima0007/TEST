...
interface Entity {
  ...
  last_updated: string;
  forced_contraception_score?: number;
  partner_birth_control_sabotage_score?: number;
  forced_pregnancy_score?: number;
  coerced_abortion_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "forced_contraception_score", label: "Contraception forcée" },
  { key: "partner_birth_control_sabotage_score", label: "Sabotage contraception partenaire" },
  { key: "forced_pregnancy_score", label: "Grossesse forcée" },
  { key: "coerced_abortion_score", label: "Avortement sous coercition" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la coercition reproductive : contraception forcée, sabotage de la contraception par le partenaire, grossesses imposées et avortements sous coercition.</p>
              <p className="text-slate-400">Source : Reproductive Coercion Rights Engine · Wave 261</p>
            </div>
          )}
        </div>
      </div>
...
export default function ReproductiveCoercionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/reproductive-coercion-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Reproductive Coercion Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Contraception Forcée · Sabotage · Grossesse Imposée · Avortement Coercitif</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#be185d20" } : {}}>
