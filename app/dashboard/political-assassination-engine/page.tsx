...
type PAEntity = {
  ...
  state_sponsored_assassination_score: number;
  cross_border_targeting_score: number;
  journalist_activist_elimination_score: number;
  assassination_impunity_score: number;
  estimated_political_assassination_index: number;
  risk_level: string;
...
function DetailModal({ entity, onClose }: { entity: PAEntity; onClose: () => void }) {
          ...
          {tab === "scores" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>
                    {entity.estimated_political_assassination_index.toFixed(1)}
                  </div>
                  <div className="text-xs text-slate-400 mt-1">Index Assassinats Politiques</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Assassinats Commandités par l'État", value: entity.state_sponsored_assassination_score },
                  { label: "Ciblage Transfrontalier", value: entity.cross_border_targeting_score },
                  { label: "Élimination Journalistes & Militants", value: entity.journalist_activist_elimination_score },
                  { label: "Impunité Assassinats", value: entity.assassination_impunity_score },
                ].map((s) => (
...
export default function PoliticalAssassinationEnginePage() {
  const [entities, setEntities] = useState<PAEntity[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<PAEntity | null>(null);

  useEffect(() => {
    setLoading(true);
    fetch("/api/political-assassination-engine")
      .then((r) => r.json())
      .then((json: ApiResponse) => {
        const rows = json?.data ?? (Array.isArray(json) ? json : []);
        setEntities(rows as PAEntity[]);
        setLoading(false);
      })
      .catch(() => {
  ...
  const avgStateSponsored = avg(entities.map((e) => e.state_sponsored_assassination_score));
  const avgCrossBorder = avg(entities.map((e) => e.cross_border_targeting_score));
  const avgJournalist = avg(entities.map((e) => e.journalist_activist_elimination_score));
  const avgImpunity = avg(entities.map((e) => e.assassination_impunity_score));
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-white">Assassinats Politiques Engine</h1>
        </div>
        <p className="text-slate-400 ml-5">
          Éliminations Ciblées &amp; Terreur Transnationale — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 rounded-2xl p-6 mb-8" style={{ border: `1px solid ${ACCENT}1a` }}>
        ...
        <div className="flex flex-wrap justify-around gap-6">
          <GaugeRing value={avgStateSponsored} label="Assassinats État" color={ACCENT} />
          <GaugeRing value={avgCrossBorder} label="Ciblage Transfrontalier" color="#7c3aed" />
          <GaugeRing value={avgJournalist} label="Élimination Journalistes" color="#0891b2" />
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs text-slate-500">Index PAE</div>
                </div>
              </div>
              <div
