...
const ACCENT = "#1e3a5f";
const DOMAIN_SLUG = "ai-algorithmic-bias-rights-engine";

const RC: Record<string, string> = { critique: "text-red-400", "élevé": "text-orange-400", modéré: "text-yellow-400", faible: "text-emerald-400" };

const FALLBACK_ENTITIES = [
  { id: "AAB-001", name: "Amazon / Rekognition", country: "USA", sector: "Facial Recognition", composite_score: 85.80, risk_level: "critique", primary_pattern: "Racial Bias in Recognition", key_signals: ["Misidentification of minorities", "Police contract concerns", "Internal bias reports suppressed"], last_updated: "2026-06-22" },
  { id: "AAB-002", name: "Meta / ad targeting", country: "USA", sector: "Digital Advertising", composite_score: 83.55, risk_level: "critique", primary_pattern: "Discriminatory Ad Targeting", key_signals: ["Housing ads discrimination", "Employment ad exclusion", "EEOC violations"], last_updated: "2026-06-22" },
  { id: "AAB-003", name: "COMPAS", country: "USA", sector: "Criminal Justice", composite_score: 80.55, risk_level: "critique", primary_pattern: "Recidivism Score Bias", key_signals: ["Racial disparity in scores", "Due process concerns", "Lack of transparency"], last_updated: "2026-06-22" },
  { id: "AAB-004", name: "HireVue", country: "USA", sector: "Recruitment AI", composite_score: 74.80, risk_level: "critique", primary_pattern: "Hiring Discrimination", key_signals: ["Facial analysis bias", "Disability discrimination", "Regulatory scrutiny"], last_updated: "2026-06-22" },
  { id: "AAB-005", name: "Google / Search", country: "USA", sector: "Search Engine", composite_score: 58.35, risk_level: "élevé", primary_pattern: "Search Result Bias", key_signals: ["Stereotyped image results", "Filter bubble effects", "Advertiser bias"], last_updated: "2026-06-22" },
  { id: "AAB-006", name: "Apple / credit card", country: "USA", sector: "Fintech", composite_score: 52.35, risk_level: "élevé", primary_pattern: "Credit Scoring Discrimination", key_signals: ["Gender credit limit disparity", "Opaque algorithm", "Regulatory investigation"], last_updated: "2026-06-22" },
  { id: "AAB-007", name: "IBM / facial recognition", country: "USA", sector: "Enterprise AI", composite_score: 35.40, risk_level: "modéré", primary_pattern: "Technology Withdrawal", key_signals: ["Voluntary market exit", "Research on bias ongoing", "Partial accountability"], last_updated: "2026-06-22" },
  { id: "AAB-008", name: "Partnership on AI", country: "International", sector: "AI Governance", composite_score: 13.60, risk_level: "faible", primary_pattern: "Governance Framework", key_signals: ["Bias mitigation guidelines", "Multi-stakeholder engagement", "Transparency standards"], last_updated: "2026-06-22" },
];
...
export default function AIAlgorithmicBiasRightsPage() {
  const [data, setData] = useState<AABData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<AABEntity | null>(null);
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#020617", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: "#60a5fa", animation: "pulse 2s infinite" }}>Chargement des données biais algorithmiques…</div>
    </div>
  );
  if (!data) return (
    <div style={{ minHeight: "100vh", background: "#020617", display: "flex", alignItems: "center", justifyContent: "center" }}>
  ...
  const entities: AABEntity[] = data.entities ?? FALLBACK_ENTITIES;
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "white", padding: "24px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto 32px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          <div style={{ width: 12, height: 12, borderRadius: "50%", background: accentDisplay }} />
          <h1 style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.025em", margin: 0 }}>Biais IA &amp; Discrimination</h1>
        </div>
        <p style={{ color: "#94a3b8", fontSize: 14, marginLeft: 24 }}>AI Algorithmic Bias Rights Engine · Discrimination algorithmique et droits fondamentaux</p>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 24 }}>
          {[
            ...
            <div key={k.label} style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 16 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 700, color: accentDisplay }}>{k.value}</div>
            </div>
          ))}
        </div>
          ...
          {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{
                ...
                borderColor: filter === f ? accentDisplay : "#334155",
                background: filter === f ? accentDisplay : "#1e293b",
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                ...
                <GaugeRing value={entity.composite_score} stroke={accentDisplay} />
              </div>
        ...
        <p style={{ fontSize: 12, color: "#475569", textAlign: "center" }}>Caelum Partners · Biais IA &amp; Discrimination · {data.last_analysis ?? "2026-06-22"}</p>
      </div>
              ...
              {["Vue générale", "Signaux", "Pattern"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  style={{
                    ...
                    borderBottom: tab === i ? `2px solid ${accentDisplay}` : "2px solid transparent",
                    color: tab === i ? accentDisplay : "#64748b"
                  }}>
                  {t}
                </button>
              ))}
              ...
              {tab === 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    <GaugeRing value={selected.composite_score} stroke={accentDisplay} />
                    <div>
                      <div style={{ fontSize: 28, fontWeight: 700, color: accentDisplay }}>{selected.composite_score.toFixed(1)}</div>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 8 }}>
                  {(selected.key_signals ?? []).map((s, i) => (
                    <li key={i} style={{ display: "flex", gap: 8, fontSize: 14, color: "#cbd5e1" }}>
                      <span style={{ color: accentDisplay, flexShrink: 0, marginTop: 2 }}>▸</span>{s}
                    </li>
                  ))}
                </ul>
              )}
