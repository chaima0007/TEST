...
interface Entity {
  ...
  language_prohibition_forced_assimilation_severity_score: number;
  minority_language_education_media_suppression_scale_score: number;
  cultural_heritage_destruction_appropriation_score: number;
  language_revitalization_legal_recognition_deficit_gap_score: number;
  estimated_minority_language_cultural_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_minority_language_cultural_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Interdiction Langue & Assimilation Forcée", value: entity.language_prohibition_forced_assimilation_severity_score, weight: "0.30" },
    { label: "Suppression Éducation & Médias Langue Minoritaire", value: entity.minority_language_education_media_suppression_scale_score, weight: "0.25" },
    { label: "Destruction & Appropriation Patrimoine Culturel", value: entity.cultural_heritage_destruction_appropriation_score, weight: "0.25" },
    { label: "Déficit Revitalisation & Reconnaissance Juridique", value: entity.language_revitalization_legal_recognition_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_minority_language_cultural_rights_index === "number" ? entity.estimated_minority_language_cultural_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Linguistiques & Culturels</div>
                </div>
              </div>
...
export default function MinorityLanguageCulturalRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/minority-language-cultural-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Linguistiques & Culturels des Minorités…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_minority_language_cultural_rights_index ?? avg(allEntities.map(e => e.estimated_minority_language_cultural_rights_index));
  ...
  const avgLanguageProhibition = avg(allEntities.map(e => e.language_prohibition_forced_assimilation_severity_score));
  const avgLanguageEducation = avg(allEntities.map(e => e.minority_language_education_media_suppression_scale_score));
  const avgCulturalHeritage = avg(allEntities.map(e => e.cultural_heritage_destruction_appropriation_score));
  const avgLanguageRevitalization = avg(allEntities.map(e => e.language_revitalization_legal_recognition_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Linguistiques", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Linguistiques & Culturels des Minorités</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Minority Language Cultural Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgLanguageProhibition} label="Interdiction Langue" />
          <GaugeRing value={avgLanguageEducation} label="Éducation & Médias" />
          <GaugeRing value={avgCulturalHeritage} label="Patrimoine Culturel" />
          <GaugeRing value={avgLanguageRevitalization} label="Revitalisation Langue" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Linguistiques: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_minority_language_cultural_rights_index === "number" ? e.estimated_minority_language_cultural_rights_index.toFixed(2) : "—"}</span>
            </div>
