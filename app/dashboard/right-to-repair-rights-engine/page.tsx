...
interface Entity {
  ...
  last_updated: string;
  manufacturer_repair_restriction_score?: number;
  software_lock_exploitation_score?: number;
  independent_repairer_exclusion_score?: number;
  planned_obsolescence_harm_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "manufacturer_repair_restriction_score", label: "Restrictions réparation fabricants" },
  { key: "software_lock_exploitation_score", label: "Exploitation verrous logiciels" },
  { key: "independent_repairer_exclusion_score", label: "Exclusion réparateurs indépendants" },
  { key: "planned_obsolescence_harm_score", label: "Préjudice obsolescence programmée" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse du droit à la réparation : restrictions imposées par les fabricants, exploitation des verrous logiciels, exclusion des réparateurs indépendants et préjudices de l&apos;obsolescence programmée.</p>
              <p className="text-slate-400">Source : Right to Repair Rights Engine · Wave 259</p>
            </div>
          )}
        </div>
      </div>
...
export default function RightToRepairRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-repair-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Right to Repair Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Restrictions Fabricants · Verrous Logiciels · Réparateurs · Obsolescence</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#16a34a20" } : {}}>
