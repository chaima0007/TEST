...
interface Entity {
  ...
  grocery_density_score?: number;
  nutrition_rights_score?: number;
...
const SUB_SCORES = [
  { key: "food_access_score", label: "Accès alimentaire" },
  { key: "grocery_density_score", label: "Densité d&apos;épiceries" },
  { key: "nutrition_rights_score", label: "Droits à la nutrition" },
  { key: "community_support_score", label: "Soutien communautaire" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits aux déserts alimentaires : accès alimentaire, densité d&apos;épiceries, droits à la nutrition et soutien communautaire affectant les populations vulnérables.</p>
              <p className="text-slate-400">Source : Food Desert Rights Engine · Wave 265</p>
            </div>
          )}
        </div>
      </div>
...
export default function FoodDesertRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/food-desert-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits aux Déserts Alimentaires</h1>
          <p className="text-slate-400 mt-1 text-sm">Accès Alimentaire · Densité d&apos;Épiceries · Nutrition · Soutien Communautaire</p>
        </div>
