...
interface SEEntity {
  ...
  composite_score: number; shadow_fleet_oil_score: number;
  crypto_sanctions_bypass_score: number; shell_company_network_score: number;
  correspondent_banking_evasion_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_sanctions_evasion_index: number; last_updated: string;
}
interface SESummary {
  ...
  entities: SEEntity[]; avg_estimated_sanctions_evasion_index: number;
}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Flotte Fantôme Pétrolière", v: entity.shadow_fleet_oil_score, c: "#06b6d4" },
              { l: "Contournement Crypto Sanctions", v: entity.crypto_sanctions_bypass_score, c: "#0891b2" },
              { l: "Réseau Sociétés Écrans", v: entity.shell_company_network_score, c: "#0e7490" },
              { l: "Évasion Banques Correspondantes", v: entity.correspondent_banking_evasion_score, c: "#155e75" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Évasion Sanctions Estimé</span>
              <span className="text-cyan-400 font-bold">{entity.estimated_sanctions_evasion_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Blocage immédiat des flux financiers identifiés et saisie des actifs des sociétés écrans</p><p>› Coordination OFAC/UE/OFSI pour désignations synchronisées et gel des comptes correspondants</p></>}
            {entity.risk_level === "élevé" && <><p>› Activation des procédures KYC renforcées auprès des banques correspondantes impliquées</p><p>› Traçabilité approfondie des transactions crypto vers les portefeuilles identifiés</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance accrue des mouvements de la flotte fantôme et des transferts pétroliers suspects</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille sur les structures corporate et les bénéficiaires effectifs</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function SanctionsEvasionPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/sanctions-evasion-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données évasion sanctions"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse des réseaux d'évasion de sanctions…</div></div>;
  ...
  const avgScore = (k: keyof SEEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Sanctions Evasion Intelligence Engine</h1>
          <p className="text-slate-400 text-sm">Cartographie des réseaux d'évasion de sanctions et flux financiers illicites · v{data.engine_version}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Sources de Données", v: data.data_sources.length, g: "from-cyan-900 to-cyan-800", t: "text-cyan-300" },
            { l: "Confiance Analyse", v: `${Math.round(data.confidence_score * 100)}%`, g: "from-slate-800 to-slate-700", t: "text-slate-300" },
            { l: "Index Évasion Moyen", v: data.avg_estimated_sanctions_evasion_index, g: "from-cyan-900 to-cyan-800", t: "text-cyan-300" },
          ].map(({ l, v, g, t }) => (
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs d'Évasion de Sanctions Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("shadow_fleet_oil_score")} label="Flotte Fantôme" color="#06b6d4" />
            <GaugeRing value={avgScore("crypto_sanctions_bypass_score")} label="Contournement Crypto" color="#0891b2" />
            <GaugeRing value={avgScore("shell_company_network_score")} label="Sociétés Écrans" color="#0e7490" />
            <GaugeRing value={avgScore("correspondent_banking_evasion_score")} label="Banques Correspondantes" color="#155e75" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns d'Évasion de Sanctions</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={data.total_entities} color={["#06b6d4", "#0891b2", "#0e7490", "#155e75", "#164e63"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-cyan-900/10 border border-cyan-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-cyan-400 mb-2">Alertes Évasion Sanctions Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_sanctions_evasion_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Sanctions Evasion Intelligence Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
