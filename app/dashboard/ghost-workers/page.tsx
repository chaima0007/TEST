...
interface Entity { entity: string; risk_level: string; avg_composite: number; estimated_ghost_worker_index: number; }
...
export default function GhostWorkersPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/ghostworkers");
  ...
  return (
    <div className="p-6 space-y-6 text-slate-100">
      <div>
        <h1 className="text-2xl font-bold text-white">Travailleurs Fantômes des Plateformes IA</h1>
        <p className="text-slate-400 text-sm mt-1">Labélisation IA, modération de contenu et micro-tâches invisibles</p>
      </div>
