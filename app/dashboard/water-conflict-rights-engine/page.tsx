...
interface Entity {
  ...
  last_updated: string;
  transboundary_water_conflict_score?: number;
  weaponization_water_score?: number;
  dam_displacement_score?: number;
  water_rights_enforcement_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "transboundary_water_conflict_score", label: "Conflits hydriques transfrontaliers" },
  { key: "weaponization_water_score", label: "Militarisation de l&apos;eau" },
  { key: "dam_displacement_score", label: "Déplacements liés aux barrages" },
  { key: "water_rights_enforcement_gap_score", label: "Lacune d&apos;application des droits à l&apos;eau" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits liés aux conflits hydriques : tensions transfrontalières, militarisation de l&apos;eau, déplacements causés par les barrages et lacunes d&apos;application des droits à l&apos;eau.</p>
              <p className="text-slate-400">Source : Water Conflict Rights Engine · Wave 253</p>
            </div>
          )}
        </div>
      </div>
...
export default function WaterConflictRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/water-conflict-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Water Conflict Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Conflits Transfrontaliers · Militarisation · Barrages · Droits à l&apos;Eau</p>
        </div>
