...
interface Entity {
  ...
  statelessness_legal_vulnerability_severity_score: number;
  documentation_citizenship_denial_scale_score: number;
  stateless_detention_expulsion_risk_score: number;
  reduced_social_rights_access_barrier_score: number;
...
interface DashData {
  ...
  avg_estimated_statelessness_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Vulnérabilité Juridique", value: entity.statelessness_legal_vulnerability_severity_score, weight: "0.30" },
    { label: "Déni Documentation/Citoyenneté", value: entity.documentation_citizenship_denial_scale_score, weight: "0.25" },
    { label: "Risque Détention/Expulsion", value: entity.stateless_detention_expulsion_risk_score, weight: "0.25" },
    { label: "Barrière Droits Sociaux", value: entity.reduced_social_rights_access_barrier_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_statelessness_rights_index === "number" ? entity.estimated_statelessness_rights_index.toFixed(2) : "—"}</div>
...
export default function StatelessnessRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/statelessness-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Apatrides…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_statelessness_rights_index ?? avg(allEntities.map(e => e.estimated_statelessness_rights_index));
  ...
  const avgLegalVuln = avg(allEntities.map(e => e.statelessness_legal_vulnerability_severity_score));
  const avgDocDenial = avg(allEntities.map(e => e.documentation_citizenship_denial_scale_score));
  const avgDetention = avg(allEntities.map(e => e.stateless_detention_expulsion_risk_score));
  const avgSocialRights = avg(allEntities.map(e => e.reduced_social_rights_access_barrier_score));
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Statelessness Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Apatridie, déni de citoyenneté et vulnérabilité juridique des sans-État — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgLegalVuln} label="Vulnérabilité Juridique" />
          <GaugeRing value={avgDocDenial} label="Déni Citoyenneté" />
          <GaugeRing value={avgDetention} label="Détention/Expulsion" />
          <GaugeRing value={avgSocialRights} label="Droits Sociaux" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Apatridie: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_statelessness_rights_index === "number" ? e.estimated_statelessness_rights_index.toFixed(2) : "—"}</span>
            </div>
