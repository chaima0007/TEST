...
interface Entity {
  ...
  statelessness_documentation_deprivation_severity_score: number;
  birth_registration_absence_exclusion_scale_score: number;
  nationality_acquisition_barrier_score: number;
  social_service_access_stateless_gap_score: number;
...
const SUB_SCORES = [
  { key: "statelessness_documentation_deprivation_severity_score", label: "Déprivation Documentation" },
  { key: "birth_registration_absence_exclusion_scale_score", label: "Absence Enregistrement Naissance" },
  { key: "nationality_acquisition_barrier_score", label: "Barrière Acquisition Nationalité" },
  { key: "social_service_access_stateless_gap_score", label: "Accès Services Sociaux" },
];
        ...
        <div className="flex border-b border-slate-800">
          {["Aperçu", "Métriques", "Sources"].map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-indigo-500/50 text-indigo-400" : "text-slate-400 hover:text-white"}`}>
              {t}
            </button>
          ))}
        </div>
          ...
          {tab === 0 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Indice Apatridie</span>
                <span className="font-semibold" style={{ color: ACCENT }}>{entity.estimated_statelessness_rights_index}/10</span>
              </div>
              <div className="flex items-center justify-between text-sm">
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/statelessness-rights-engine").then(r=>r.json()).then(d=>{ setData(d.payload??d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Statelessness Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Déprivation Documentation · Enregistrement Naissance · Nationalité · Services Sociaux</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice Apatridie</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_statelessness_rights_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        {filtered.map(e => {
          ...
          return (
            <button key={e.entity_id} onClick={() => setSelected(e)}
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold uppercase ${cfg.color}`}>{cfg.label}</span>
                <span className="text-xs text-slate-500">Indice: <span className="font-bold" style={{ color: ACCENT }}>{e.estimated_statelessness_rights_index}</span></span>
              </div>
            </button>
          );
        })}
