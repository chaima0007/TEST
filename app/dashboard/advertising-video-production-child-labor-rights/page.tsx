...
const API_URL = "/api/advertising-video-production-child-labor-rights";
const TITLE = "Droits Enfants Production Vidéo Publicitaire";
const COLOR = "#7c2d12";
...
type Entity = {
  ...
  risk_level: string;
  estimated_advertising_video_production_child_labor_rights_index: number;
};
type ApiData = {
  entities: Entity[];
      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: 24, width: 480, maxWidth: "90vw" }}>
        ...
        {tab === "apercu" && (
          <div>
            ...
            <p>Niveau de risque : <strong style={{ color }}>{entity.risk_level}</strong></p>
          </div>
        )}
        {tab === "indicateurs" && (
          <p>Index estimé : <strong>{entity.estimated_advertising_video_production_child_labor_rights_index}</strong></p>
        )}
        {tab === "recommandations" && (
          <p>Renforcer la surveillance des chaînes d&apos;approvisionnement et appliquer les normes CSDDD 2024/1760.</p>
        )}
      </div>
    </div>
  );
          ...
          <div key={e.entity} onClick={() => setSelected(e)}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 9999,
                background: (riskColors[e.risk_level] ?? "#374151") + "20",
                color: riskColors[e.risk_level] ?? "#374151" }}>{e.risk_level}</span>
            </div>
