...
interface Entity {
  ...
  toxic_pollution_environmental_health_severity_score: number;
  indigenous_environmental_destruction_scale_score: number;
  environmental_defender_criminalisation_score: number;
  state_corporate_environmental_impunity_gap_score: number;
  estimated_environmental_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_environmental_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Pollution Toxique", value: entity.toxic_pollution_environmental_health_severity_score, weight: "0.30" },
    { label: "Destruction Autochtone", value: entity.indigenous_environmental_destruction_scale_score, weight: "0.25" },
    { label: "Criminalisation Défenseurs", value: entity.environmental_defender_criminalisation_score, weight: "0.25" },
    { label: "Impunité État/Entreprise", value: entity.state_corporate_environmental_impunity_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_environmental_rights_index === "number" ? entity.estimated_environmental_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Environnement</div>
                </div>
              </div>
...
export default function EnvironmentalRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/environmental-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Environnementaux…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_environmental_rights_index ?? avg(allEntities.map(e => e.estimated_environmental_rights_index));
  ...
  const avgToxicPollution = avg(allEntities.map(e => e.toxic_pollution_environmental_health_severity_score));
  const avgIndigenous = avg(allEntities.map(e => e.indigenous_environmental_destruction_scale_score));
  const avgDefenders = avg(allEntities.map(e => e.environmental_defender_criminalisation_score));
  const avgImpunity = avg(allEntities.map(e => e.state_corporate_environmental_impunity_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Environnement", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Environmental Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Pollution toxique, destruction environnementale autochtone et criminalisation des défenseurs — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgToxicPollution} label="Pollution Toxique" />
          <GaugeRing value={avgIndigenous} label="Destruction Autochtone" />
          <GaugeRing value={avgDefenders} label="Criminalisation Défenseurs" />
          <GaugeRing value={avgImpunity} label="Impunité État/Entreprise" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Environnement: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_environmental_rights_index === "number" ? e.estimated_environmental_rights_index.toFixed(2) : "—"}</span>
            </div>
