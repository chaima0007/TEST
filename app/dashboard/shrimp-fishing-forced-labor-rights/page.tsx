...
interface Entity {
  ...
  forced_labor_trafficking_prevalence_score: number;
  debt_bondage_deceptive_recruitment_score: number;
  flag_state_impunity_enforcement_deficit_score: number;
  corporate_supply_chain_complicity_score: number;
  estimated_shrimp_fishing_forced_labor_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_shrimp_fishing_forced_labor_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Prévalence Travail Forcé & Traite", value: entity.forced_labor_trafficking_prevalence_score, weight: "0.30" },
    { label: "Servitude par Dettes & Recrutement Frauduleux", value: entity.debt_bondage_deceptive_recruitment_score, weight: "0.25" },
    { label: "Impunité État Pavillon & Déficit Application", value: entity.flag_state_impunity_enforcement_deficit_score, weight: "0.25" },
    { label: "Complicité Corporative Chaîne Approvisionnement", value: entity.corporate_supply_chain_complicity_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_shrimp_fishing_forced_labor_index === "number" ? entity.estimated_shrimp_fishing_forced_labor_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Esclavage Maritime</div>
                </div>
              </div>
...
export default function ShrimpFishingForcedLaborRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/shrimp-fishing-forced-labor-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits — Esclavage Maritime Crevettiers…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_shrimp_fishing_forced_labor_index ?? avg(allEntities.map(e => e.estimated_shrimp_fishing_forced_labor_index));
  ...
  const avgForcedLabor = avg(allEntities.map(e => e.forced_labor_trafficking_prevalence_score));
  const avgDebt = avg(allEntities.map(e => e.debt_bondage_deceptive_recruitment_score));
  const avgFlag = avg(allEntities.map(e => e.flag_state_impunity_enforcement_deficit_score));
  const avgCorporate = avg(allEntities.map(e => e.corporate_supply_chain_complicity_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Esclavage Maritime", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits — Esclavage Maritime Crevettiers</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Shrimp Fishing Forced Labor Rights Engine · Wave 281 — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgForcedLabor} label="Travail Forcé & Traite" />
          <GaugeRing value={avgDebt} label="Servitude par Dettes" />
          <GaugeRing value={avgFlag} label="Impunité État Pavillon" />
          <GaugeRing value={avgCorporate} label="Complicité Corporative" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Esclavage Maritime : <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_shrimp_fishing_forced_labor_index === "number" ? e.estimated_shrimp_fishing_forced_labor_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
