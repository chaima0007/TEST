...
interface Entity {
  ...
  last_updated: string;
  artisanal_miner_exploitation_score?: number;
  child_cobalt_labor_score?: number;
  toxic_exposure_mining_score?: number;
...
const SUB_SCORES = [
  { key: "artisanal_miner_exploitation_score", label: "Exploitation mineurs artisanaux" },
  { key: "child_cobalt_labor_score", label: "Travail enfants mines cobalt" },
  { key: "toxic_exposure_mining_score", label: "Exposition toxique minière" },
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits dans les mines de cobalt : exploitation des mineurs artisanaux, travail des enfants dans les mines de RDC, exposition toxique et opacité des chaînes d&apos;approvisionnement tech.</p>
              <p className="text-slate-400">Source : Cobalt Mining Rights Engine · Wave 258</p>
            </div>
          )}
        </div>
      </div>
...
export default function CobaltMiningRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/cobalt-mining-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Cobalt Mining Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Mineurs Artisanaux · Travail Enfants · Toxicité · Chaîne d&apos;Approvisionnement</p>
        </div>
