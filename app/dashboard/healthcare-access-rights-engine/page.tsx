...
interface Entity {
  ...
  estimated_healthcare_access_rights_index: number;
  [key: string]: unknown;
}
        ...
        <div style={{ padding: 24 }}>
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ color: "#94a3b8", fontSize: 14 }}>Indice Accès Santé Estimé</span>
                <span style={{ fontSize: 20, fontWeight: 700, color: ACCENT_LIGHT }}>{typeof entity.estimated_healthcare_access_rights_index === "number" ? entity.estimated_healthcare_access_rights_index.toFixed(2) : "—"}</span>
              </div>
          ...
          {tab === "metriques" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                ...
                { label: "Indice HAR Estimé", value: (entity.estimated_healthcare_access_rights_index as number) * 10 },
              ].map(s => (
                <div key={s.label}>
          ...
          {tab === "sources" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {["who_universal_health_coverage_index_2024", "world_bank_health_expenditure_2024", "unicef_maternal_mortality_2024", "msf_access_medicines_report"].map(s => (
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/healthcare-access-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#020617", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: ACCENT_LIGHT, fontSize: 14 }}>Initialisation Healthcare Access Rights Engine…</div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "#f1f5f9", padding: 24 }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 800, color: ACCENT_LIGHT, margin: 0 }}>Healthcare Access Rights Engine</h1>
        </div>
        <p style={{ color: "#64748b", fontSize: 14, margin: 0, paddingLeft: 16 }}>OMS · Banque Mondiale · UNICEF · MSF · Couverture Santé Universelle</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 16, marginBottom: 32 }}>
        {[
          ...
          { label: "Indice HAR", value: String(data?.estimated_healthcare_access_rights_index ?? 6.00) },
        ...
        <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <GaugeRing value={data?.avg_composite ?? 60.01} />
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap" }}>
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: "6px 16px", borderRadius: 999, fontSize: 13, fontWeight: 500, cursor: "pointer", border: `1px solid ${filter === f ? ACCENT_LIGHT : "#334155"}`, background: filter === f ? "rgba(45,212,191,0.1)" : "transparent", color: filter === f ? ACCENT_LIGHT : "#94a3b8" }}>
          ...
          return (
            <button key={e.id} onClick={() => setSelected(e)}
              ...
              <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>
                Indice HAR: <span style={{ color: ACCENT_LIGHT, fontWeight: 600 }}>{typeof e.estimated_healthcare_access_rights_index === "number" ? e.estimated_healthcare_access_rights_index.toFixed(2) : "—"}</span>
              </div>
            </button>
          );
        })}
