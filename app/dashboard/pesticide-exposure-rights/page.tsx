...
interface Entity {
  ...
  community_awareness_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  ...
  { key: "health_monitoring_score", label: "Suivi sanitaire" },
  { key: "community_awareness_score", label: "Sensibilisation communautaire" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits contre l&apos;exposition aux pesticides : exposition chimique, protection réglementaire, suivi sanitaire et sensibilisation communautaire affectant les travailleurs agricoles et populations rurales.</p>
              <p className="text-slate-400">Source : Pesticide Exposure Rights Engine · Wave 268</p>
            </div>
          )}
        </div>
      </div>
...
export default function PesticideExposureRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/pesticide-exposure-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits contre l&apos;Exposition aux Pesticides</h1>
          <p className="text-slate-400 mt-1 text-sm">Exposition Chimique · Protection Réglementaire · Suivi Sanitaire · Sensibilisation Communautaire</p>
        </div>
