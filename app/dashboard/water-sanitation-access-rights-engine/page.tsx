...
interface Entity {
  ...
  safe_water_access_denial_severity_score: number;
  sanitation_infrastructure_absence_scale_score: number;
  water_privatization_commodification_score: number;
  climate_water_stress_conflict_deficit_gap_score: number;
  estimated_water_sanitation_access_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_water_sanitation_access_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déni d'Accès Eau Potable", value: entity.safe_water_access_denial_severity_score, weight: "0.30" },
    { label: "Absence Infrastructure Assainissement", value: entity.sanitation_infrastructure_absence_scale_score, weight: "0.25" },
    { label: "Privatisation & Marchandisation Eau", value: entity.water_privatization_commodification_score, weight: "0.25" },
    { label: "Déficit Eau Stress Climatique & Conflit", value: entity.climate_water_stress_conflict_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_water_sanitation_access_rights_index === "number" ? entity.estimated_water_sanitation_access_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Eau & Assainissement</div>
                </div>
              </div>
...
export default function WaterSanitationAccessRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/water-sanitation-access-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Accès à l&apos;Eau & Assainissement…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_water_sanitation_access_rights_index ?? avg(allEntities.map(e => e.estimated_water_sanitation_access_rights_index));
  ...
  const avgSafeWater = avg(allEntities.map(e => e.safe_water_access_denial_severity_score));
  const avgSanitation = avg(allEntities.map(e => e.sanitation_infrastructure_absence_scale_score));
  const avgPrivatization = avg(allEntities.map(e => e.water_privatization_commodification_score));
  const avgClimateStress = avg(allEntities.map(e => e.climate_water_stress_conflict_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Eau & Assainissement", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Accès à l&apos;Eau & Assainissement</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Water Sanitation Access Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgSafeWater} label="Eau Potable" />
          <GaugeRing value={avgSanitation} label="Assainissement" />
          <GaugeRing value={avgPrivatization} label="Privatisation Eau" />
          <GaugeRing value={avgClimateStress} label="Stress Climatique" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Eau & Assainissement: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_water_sanitation_access_rights_index === "number" ? e.estimated_water_sanitation_access_rights_index.toFixed(2) : "—"}</span>
            </div>
