...
interface AFEntity {
  ...
  composite_score: number; political_censorship_score: number;
  researcher_persecution_score: number; brain_drain_severity_score: number;
  institutional_autonomy_loss_score: number; risk_level: string; primary_pattern: string;
  key_signals: string[]; estimated_repression_index: number; last_updated: string;
}
interface AFSummary {
  ...
  confidence_score: number; data_sources: string[]; entities: AFEntity[];
  avg_estimated_repression_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: AFEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Censure Politique", v: entity.political_censorship_score, c: "#ef4444" },
              { l: "Persécution Chercheurs", v: entity.researcher_persecution_score, c: "#f97316" },
              { l: "Fuite des Cerveaux", v: entity.brain_drain_severity_score, c: "#a855f7" },
              { l: "Perte Autonomie Instit.", v: entity.institutional_autonomy_loss_score, c: "#f59e0b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Répression Académique</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_repression_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Mécanismes d'urgence de protection des chercheurs persécutés</p><p>› Pressions diplomatiques internationales pour la liberté académique</p><p>› Programmes d'accueil d'urgence pour chercheurs exilés</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des garanties institutionnelles d'indépendance universitaire</p><p>› Réseaux de soutien aux chercheurs sous pression politique</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance accrue des ingérences politiques dans la recherche</p><p>› Développement de chartes d'indépendance académique</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la vigilance sur la liberté académique</p><p>› Consolidation des mécanismes de gouvernance universitaire autonome</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  repression_academique_totale: "#ef4444",
  controle_ideologique_universitaire: "#f97316",
  pression_systemique: "#f59e0b",
  tensions_academiques: "#eab308",
  liberte_academique_exemplaire: "#10b981",
};

export default function AcademicFreedomEnginePage() {
  const [data, setData] = useState<AFSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<AFEntity | null>(null);

  useEffect(() => {
    fetch("/api/academic-freedom-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données liberté académique"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la liberté académique en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof AFEntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Liberté Académique</h1>
          <p className="text-slate-400 text-sm mt-1">La répression des universités comme précurseur de l'autoritarisme — les régimes qui brûlent les livres brûlent ensuite les gens</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Composite Moyen", v: `${data.avg_composite}%`, t: "text-indigo-300" },
            { l: "Alertes Critiques", v: data.critical_alerts.length, t: "text-red-300" },
            { l: "Indice Répression Académique", v: data.avg_estimated_repression_index, t: "text-indigo-400" },
            { l: "Sources", v: data.data_sources?.length ?? 0, t: "text-violet-300" },
            { l: "Confiance Analyse", v: `${Math.round((data.confidence_score ?? 0) * 100)}%`, t: "text-slate-300" },
          ].map(({ l, v, t }) => (
            <div key={l} className="bg-slate-900 border border-indigo-500/20 rounded-xl p-4">
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-indigo-500/20">
          ...
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("political_censorship_score")} label="Censure Politique" color={ACCENT} />
            <GaugeRing value={avgScore("researcher_persecution_score")} label="Persécution Chercheurs" color="#f97316" />
            <GaugeRing value={avgScore("brain_drain_severity_score")} label="Fuite des Cerveaux" color="#a855f7" />
            <GaugeRing value={avgScore("institutional_autonomy_loss_score")} label="Perte Autonomie Instit." color="#f59e0b" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-indigo-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Répression Académique</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-indigo-900/10 border border-indigo-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Académiques Critiques</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              className={`bg-slate-900 rounded-xl p-4 border cursor-pointer transition-all hover:border-indigo-500/50 ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_repression_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Liberté Académique · {data.last_analysis}</p>
      </div>
    </div>
  );
}
