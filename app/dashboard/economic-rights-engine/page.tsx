...
interface ApiData {
  ...
  eleve_count: number;
  avg_estimated_economic_rights_index: number;
...
function DetailModal({ entity, onClose }: DetailModalProps) {
  ...
  const subScores = [
    { label: "Déni Travail Décent", value: entity.decent_work_denial_score },
    { label: "Vol de Salaires", value: entity.wage_theft_exploitation_score },
    { label: "Suppression Syndicale", value: entity.union_suppression_score },
    { label: "Absence Plancher Social", value: entity.social_floor_absence_score },
  ];
...
export default function EconomicRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/economic-rights-engine")
      .then((r) => r.json())
  ...
  const kpis = [
    ...
    { label: "Index Moyen", value: data?.avg_estimated_economic_rights_index ?? 0, fmt: (v: number) => v.toFixed(1) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>
          Economic Rights Engine
        </h1>
        <p className="text-slate-400 text-sm mt-1">
          Analyse des violations des droits économiques, du travail décent et de la protection sociale
        </p>
      </div>
      ...
      {!loading && data && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6 flex items-center gap-6">
          <GaugeRing value={data.avg_estimated_economic_rights_index ?? 0} color={ACCENT} />
          <div>
            <p className="text-slate-400 text-sm">Index Moyen Economic Rights</p>
            <p className="text-white text-3xl font-bold">{(data.avg_estimated_economic_rights_index ?? 0).toFixed(1)}</p>
          </div>
        </div>
      )}
