...
interface Entity {
  ...
  country: string;
  sector: string;
  internet_shutdown_frequency_severity_score: number;
  social_media_platform_blocking_score: number;
  vpn_encryption_criminalization_score: number;
  digital_expression_persecution_score: number;
  estimated_internet_shutdown_digital_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_internet_shutdown_digital_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Coupures Internet Fréquence &amp; Sévérité", value: entity.internet_shutdown_frequency_severity_score, weight: "0.30" },
    { label: "Blocage Plateformes Sociales", value: entity.social_media_platform_blocking_score, weight: "0.25" },
    { label: "Criminalisation VPN &amp; Chiffrement", value: entity.vpn_encryption_criminalization_score, weight: "0.25" },
    { label: "Persécution Expression Numérique", value: entity.digital_expression_persecution_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{entity.internet_shutdown_frequency_severity_score.toFixed(1)}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Coupures Internet</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_internet_shutdown_digital_rights_index === "number" ? entity.estimated_internet_shutdown_digital_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Numériques</div>
                </div>
              </div>
...
export default function InternetShutdownDigitalRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/internet-shutdown-digital-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Coupures Internet &amp; Droits Numériques…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.internet_shutdown_frequency_severity_score));
  const avgIndex = data?.avg_estimated_internet_shutdown_digital_rights_index ?? avg(allEntities.map(e => e.estimated_internet_shutdown_digital_rights_index));
  ...
  const avgShutdown = avg(allEntities.map(e => e.internet_shutdown_frequency_severity_score));
  const avgBlocking = avg(allEntities.map(e => e.social_media_platform_blocking_score));
  const avgVPN = avg(allEntities.map(e => e.vpn_encryption_criminalization_score));
  const avgExpression = avg(allEntities.map(e => e.digital_expression_persecution_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Numériques", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Coupures Internet &amp; Droits Numériques</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Internet Shutdown &amp; Digital Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgShutdown} label="Coupures Internet Fréquence" />
          <GaugeRing value={avgBlocking} label="Blocage Plateformes Sociales" />
          <GaugeRing value={avgVPN} label="Criminalisation VPN" />
          <GaugeRing value={avgExpression} label="Persecution Expression Numérique" />
        </div>
      </div>
          ...
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.internet_shutdown_frequency_severity_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.internet_shutdown_frequency_severity_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Numériques: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_internet_shutdown_digital_rights_index === "number" ? e.estimated_internet_shutdown_digital_rights_index.toFixed(2) : "—"}</span>
            </div>
