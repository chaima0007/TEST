...
type BSEEntity = {
  ...
  risk_level: string;
  biosurveillance_pattern: string;
  severity: string;
  recommended_action: string;
  signal: string;
  outbreak_detection_delay: number;
  genomic_sequencing_gap: number;
};
...
type BSESummary = {
  ...
  avg_estimated_biosurveillance_index: number;
};
...
const PAT_COLORS: Record<string, string> = {
  none: "#10b981",
  outbreak_detection_failure: "#ef4444",
  data_sharing_political_suppression: "#f97316",
  zoonotic_surveillance_gap: "#a855f7",
  genomic_surveillance_collapse: "#06b6d4",
  cross_border_surveillance_failure: "#1d4ed8",
};
const SEV_COLORS: Record<string, string> = {
  biosurveillance_sous_contrôle: "#10b981",
  "défaillance_biosurveillance_structurelle": "#f59e0b",
  "crise_alerte_précoce_épidémique_majeure": "#3b82f6",
  "effondrement_biosurveillance_systémique": "#ef4444",
};
const ACTION_COLORS: Record<string, string> = {
  veille_biosurveillance_continue: "#10b981",
  "amélioration_surveillance_épidémique_systémique": "#06b6d4",
  renforcement_alerte_précoce_urgence: "#3b82f6",
  intervention_biosurveillance_urgence_mondiale: "#ef4444",
};
...
const SEV_BADGE: Record<string, string> = {
  biosurveillance_sous_contrôle: "bg-emerald-900 text-emerald-300",
  "défaillance_biosurveillance_structurelle": "bg-amber-900 text-amber-300",
  "crise_alerte_précoce_épidémique_majeure": "bg-blue-900 text-blue-300",
  "effondrement_biosurveillance_systémique": "bg-red-950 text-red-400",
};

function DetailModal({ entity, onClose }: { entity: BSEEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-950 border border-teal-700/30 rounded-xl w-full max-w-lg p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <span className="ml-2 text-slate-500 text-xs">{entity.surveillance_system.replace(/_/g, " ")}</span>
          </div>
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ["Score Détection",   entity.detection_score,   "#ef4444"],
              ["Score Réponse",     entity.response_score,    "#f97316"],
            ...
            <div className="col-span-2 bg-slate-900 border border-teal-700/20 rounded-lg p-3">
              <div className="text-teal-300/60 text-xs mb-1">Score Composite Biosurveillance</div>
          ...
          <div className="bg-slate-900 border border-teal-700/20 rounded-lg p-4 text-sm text-slate-200 leading-relaxed">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="bg-slate-800 rounded p-2">
                <div className="text-teal-300/50 text-xs mb-0.5">Délai Détection Épidémie</div>
                <div className="text-white text-sm font-medium">{Math.round(entity.outbreak_detection_delay * 100)}%</div>
              </div>
              <div className="bg-slate-800 rounded p-2">
                <div className="text-teal-300/50 text-xs mb-0.5">Lacune Séquençage Génomique</div>
                <div className="text-white text-sm font-medium">{Math.round(entity.genomic_sequencing_gap * 100)}%</div>
              </div>
            </div>
          </div>
        )}
        ...
        {tab === "action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-900 border border-teal-700/20 rounded-lg p-3">
              <div className="text-teal-300/60 text-xs mb-1">Système de Surveillance</div>
              <div className="text-white font-medium">{entity.surveillance_system.replace(/_/g, " ")}</div>
            </div>
            <div className="bg-slate-900 border border-teal-700/20 rounded-lg p-3">
              <div className="text-teal-300/60 text-xs mb-1">Patron Biosurveillance Détecté</div>
              <div className="text-teal-300 font-medium">{entity.biosurveillance_pattern.replace(/_/g, " ")}</div>
            </div>
          </div>
        )}
      </div>
...
export default function BiosurveillanceDashboard() {
  const [data, setData]             = useState<{ entities: BSEEntity[]; summary: BSESummary } | null>(null);
  const [riskFilter, setRiskFilter] = useState<string>("all");
  const [patFilter, setPatFilter]   = useState<string>("all");
  const [selected, setSelected]     = useState<BSEEntity | null>(null);

  useEffect(() => {
    fetch("/api/biosurveillance-engine")
      .then(r => r.json())
      .then(setData)
      .catch(console.error);
  }, []);
  ...
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-teal-400 text-lg animate-pulse">Initialisation du Moteur Biosurveillance & Alerte Précoce...</div>
    </div>
  );
  ...
  const filtered = entities.filter(e =>
    ...
    (patFilter === "all" || e.biosurveillance_pattern === patFilter)
  );

  const avgDetection = entities.reduce((s, e) => s + e.detection_score, 0) / (entities.length || 1);

  const dists = [
    { title: "Niveau de Risque Biosurveillance", counts: summary.risk_distribution,     colors: RISK_COLORS   },
    { title: "Patron Biosurveillance Détecté",   counts: summary.pattern_distribution,  colors: PAT_COLORS    },
    { title: "Sévérité Biosurveillance",         counts: summary.severity_distribution, colors: SEV_COLORS    },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-teal-400">Biosurveillance &amp; Alerte Précoce Épidémique — Module 391</h1>
        <p className="text-teal-300/50 text-sm mt-1">
          Détection Épidémique · Réponse Transfrontalière · Gouvernance OMS · Défaillances Systémiques One Health
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          ["Détection Échouée",     summary.critical,                                                 "text-red-400"],
          ["Crise Majeure",         summary.high,                                                     "text-amber-400"],
          ["Composite Moyen",       `${summary.avg_composite.toFixed(1)}`,                            "text-teal-300"],
          ["Index Biosurveillance", `${summary.avg_estimated_biosurveillance_index.toFixed(2)}/10`,   "text-amber-400"],
          ["Détection Moyenne",     `${Math.round(avgDetection * 10) / 10}`,                          "text-teal-300"],
        ].map(([l, v, c]) => (
      ...
      <div className="bg-slate-900 border border-teal-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          <GaugeRing
            value={entities.reduce((s, e) => s + e.detection_score, 0) / (entities.length || 1)}
            label="Détection Moy."
            color="#ef4444"
          />
          <GaugeRing
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {["all", "none", "outbreak_detection_failure", "data_sharing_political_suppression", "zoonotic_surveillance_gap", "genomic_surveillance_collapse", "cross_border_surveillance_failure"].map(p => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mb-2 capitalize">{e.surveillance_system.replace(/_/g, " ")}</div>
            ...
            <div className="text-xs text-teal-400/60 mb-2 capitalize">{e.biosurveillance_pattern.replace(/_/g, " ")}</div>
            <div className="text-xs text-teal-400 font-medium mb-2">
              Dét: {e.detection_score.toFixed(1)} · Rép: {e.response_score.toFixed(1)}
            </div>
