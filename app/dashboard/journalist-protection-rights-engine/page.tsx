...
interface Entity {
  ...
  last_updated: string;
  journalist_killing_score?: number;
  source_protection_failure_score?: number;
  strategic_lawsuit_abuse_score?: number;
  press_accreditation_denial_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "journalist_killing_score", label: "Meurtre de journalistes" },
  { key: "source_protection_failure_score", label: "Échec de protection des sources" },
  { key: "strategic_lawsuit_abuse_score", label: "Abus de poursuites stratégiques (SLAPP)" },
  { key: "press_accreditation_denial_score", label: "Refus d&apos;accréditation de presse" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des violations de la protection des journalistes : meurtres et violences, échec de la protection des sources, abus de poursuites stratégiques (SLAPP) et refus d&apos;accréditation de presse.</p>
              <p className="text-slate-400">Source : Journalist Protection Rights Engine · Wave 245</p>
            </div>
          )}
        </div>
      </div>
...
export default function JournalistProtectionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/journalist-protection-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Journalist Protection Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Meurtres de Journalistes · Protection des Sources · Poursuites SLAPP · Accréditation de Presse</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#0369a120" } : {}}>
