...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/cultural-property-looting-engine").then(r => r.json()).then(j => setData(j.payload ?? j));
  }, []);
  ...
  const PATTERN_LABELS: Record<string, string> = {
    destruction_patrimoine_systematique: "Destruction Patrimoine",
    trafic_antiquites_illicite: "Trafic Antiquités",
    saisie_musees_archives: "Saisie Musées Archives",
    refus_restitution: "Refus Restitution",
  };
  const SCORE_KEYS = [
    "systematic_heritage_destruction_score",
    "illicit_antiquities_trafficking_score",
    "museum_archive_seizure_score",
    "restitution_refusal_impunity_score",
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Cultural Property Looting Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Surveillance du pillage du patrimoine culturel et trafic d&apos;antiquités</p>
      </div>
