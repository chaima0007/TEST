...
interface Entity {
  ...
  retaliation_prosecution_score: number;
  state_surveillance_chilling_score: number;
  media_source_protection_failure_score: number;
  risk_level: string;
...
interface Data {
  ...
  avg_estimated_whistleblower_protection_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Absence Cadre Juridique", v: entity.legal_framework_absence_score, c: "#0369a1" },
              { l: "Poursuites Représailles", v: entity.retaliation_prosecution_score, c: "#0284c7" },
              { l: "Surveillance État / Effet Dissuasif", v: entity.state_surveillance_chilling_score, c: "#0ea5e9" },
              { l: "Échec Protection Sources Médias", v: entity.media_source_protection_failure_score, c: "#38bdf8" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Protection Lanceurs d&apos;Alerte Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_whistleblower_protection_index}</span>
            </div>
        ...
        {tab === "contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Intervention d&apos;urgence pour les lanceurs d&apos;alerte exposés à des poursuites judiciaires et à la surveillance étatique</p>
                <p>› Mobilisation des mécanismes internationaux de protection des sources et des défenseurs des droits</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Documentation systématique des représailles et des défaillances du cadre juridique de protection</p>
                <p>› Plaidoyer pour le renforcement des lois de protection des lanceurs d&apos;alerte et des sources journalistiques</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance des évolutions législatives et soutien aux organisations de protection des lanceurs d&apos;alerte</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur les pratiques de surveillance et prévention des dérives régressives</p>
            )}
          </div>
        )}
      </div>
...
export default function WhistleblowerProtectionEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/whistleblower-protection-engine")
      .then(r => r.json())
      .then(d => { setData(d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données Whistleblower Protection Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la protection des lanceurs d&apos;alerte…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Score Moyen", v: `${(data.avg_composite ?? 0).toFixed(1)}`, c: "text-sky-400" },
    { l: "Indice Protection Moyen", v: `${(data.avg_estimated_whistleblower_protection_index ?? 0).toFixed(2)}`, c: "text-sky-300" },
    { l: "Critique", v: dist.critique ?? 0, c: "text-red-400" },
    { l: "Index Protection", v: dist["élevé"] ?? 0, c: "text-orange-400" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Protection Lanceurs d&apos;Alerte</h1>
          <p className="text-slate-400 text-sm mt-1">Absence de cadre juridique, représailles et surveillance étatique contre les lanceurs d&apos;alerte</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {kpis.map(({ l, v, c }) => (
            <div key={l} className="bg-slate-900 border border-sky-500/20 rounded-xl p-4 text-center">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 border border-sky-500/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Protection Lanceurs d&apos;Alerte Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("legal_framework_absence_score")} label="Absence Cadre Juridique" color={ACCENT} />
            <GaugeRing value={avgScore("retaliation_prosecution_score")} label="Poursuites Représailles" color="#0284c7" />
            <GaugeRing value={avgScore("state_surveillance_chilling_score")} label="Surveillance / Dissuasion" color="#0ea5e9" />
            <GaugeRing value={avgScore("media_source_protection_failure_score")} label="Échec Protection Sources" color="#38bdf8" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-sky-500/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns de Suppression des Lanceurs d&apos;Alerte</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#0369a1", "#0284c7", "#0ea5e9", "#38bdf8", "#7dd3fc"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_whistleblower_protection_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Whistleblower Protection Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
