...
interface Entity {
  ...
  famine_starvation_weaponization_severity_score: number;
  land_grab_smallholder_displacement_scale_score: number;
  seed_patent_corporate_monopoly_score: number;
  food_access_indigenous_sovereignty_deficit_gap_score: number;
  estimated_food_sovereignty_famine_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_food_sovereignty_famine_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Famine Weaponisée & Blocus", value: entity.famine_starvation_weaponization_severity_score, weight: "0.30" },
    { label: "Accaparement Terres & Expulsion", value: entity.land_grab_smallholder_displacement_scale_score, weight: "0.25" },
    { label: "Monopole Semencier & Brevets", value: entity.seed_patent_corporate_monopoly_score, weight: "0.25" },
    { label: "Déficit Souveraineté Alimentaire", value: entity.food_access_indigenous_sovereignty_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_food_sovereignty_famine_rights_index === "number" ? entity.estimated_food_sovereignty_famine_rights_index.toFixed(2) : "—"}</div>
...
export default function FoodSovereigntyFamineRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/food-sovereignty-famine-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Souveraineté Alimentaire & Droit à la Nourriture…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_food_sovereignty_famine_rights_index ?? avg(allEntities.map(e => e.estimated_food_sovereignty_famine_rights_index));
  ...
  const avgFamine = avg(allEntities.map(e => e.famine_starvation_weaponization_severity_score));
  const avgLandGrab = avg(allEntities.map(e => e.land_grab_smallholder_displacement_scale_score));
  const avgSeedMonopoly = avg(allEntities.map(e => e.seed_patent_corporate_monopoly_score));
  const avgFoodAccess = avg(allEntities.map(e => e.food_access_indigenous_sovereignty_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Souveraineté Alim.", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Souveraineté Alimentaire & Droit à la Nourriture</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Food Sovereignty & Famine Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgFamine} label="Famine Weaponisée" />
          <GaugeRing value={avgLandGrab} label="Accaparement Terres" />
          <GaugeRing value={avgSeedMonopoly} label="Monopole Semencier" />
          <GaugeRing value={avgFoodAccess} label="Souveraineté Alim." />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Souveraineté Alim.: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_food_sovereignty_famine_rights_index === "number" ? e.estimated_food_sovereignty_famine_rights_index.toFixed(2) : "—"}</span>
            </div>
