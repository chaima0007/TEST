...
interface Entity {
  ...
  forced_confession_torture_evidence_severity_score: number;
  secret_trial_mass_prosecution_scale_score: number;
  legal_representation_denial_obstruction_score: number;
  presumption_innocence_pretrial_violation_gap_score: number;
  estimated_fair_trial_due_process_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_fair_trial_due_process_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Aveux Forcés/Torture", value: entity.forced_confession_torture_evidence_severity_score, weight: "0.30" },
    { label: "Procès Secrets/Masse", value: entity.secret_trial_mass_prosecution_scale_score, weight: "0.25" },
    { label: "Déni Représentation", value: entity.legal_representation_denial_obstruction_score, weight: "0.25" },
    { label: "Présomption Innocence", value: entity.presumption_innocence_pretrial_violation_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_fair_trial_due_process_index === "number" ? entity.estimated_fair_trial_due_process_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Procès Équitable</div>
                </div>
              </div>
...
export default function FairTrialDueProcessEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/fair-trial-due-process-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Procès Équitable…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_fair_trial_due_process_index ?? avg(allEntities.map(e => e.estimated_fair_trial_due_process_index));
  ...
  const avgAveuxForces = avg(allEntities.map(e => e.forced_confession_torture_evidence_severity_score));
  const avgProcesSec = avg(allEntities.map(e => e.secret_trial_mass_prosecution_scale_score));
  const avgRepresentation = avg(allEntities.map(e => e.legal_representation_denial_obstruction_score));
  const avgPresomption = avg(allEntities.map(e => e.presumption_innocence_pretrial_violation_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Procès Équitable", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Fair Trial Due Process Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Aveux forcés, procès secrets et déni de représentation légale — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgAveuxForces} label="Aveux Forcés" />
          <GaugeRing value={avgProcesSec} label="Procès Secrets" />
          <GaugeRing value={avgRepresentation} label="Représentation Légale" />
          <GaugeRing value={avgPresomption} label="Présomption Innoc." />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Procès Équitable: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_fair_trial_due_process_index === "number" ? e.estimated_fair_trial_due_process_index.toFixed(2) : "—"}</span>
            </div>
