...
export default function ClimateFinanceLossDamageRightsPage() {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  useEffect(() => {
    fetch("/api/climate-finance-loss-damage-rights-engine")
      .then((r) => r.json())
  ...
  const avgIndex = (data.avg_estimated_climate_finance_loss_damage_index as number) ?? 0;
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-6">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: TEAL }}>
            Financement Climatique — Pertes &amp; Préjudices
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            Loss &amp; Damage · UNFCCC COP28 Fund · V20 Vulnerable Nations · Climate Justice
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Avg Composite", value: avgComposite.toFixed(2), unit: "/100" },
            { label: "Indice L&D", value: avgIndex.toFixed(2), unit: "/10" },
            ...
            <div key={label} className="rounded-xl bg-slate-900 border border-slate-800 p-4 text-center">
              ...
              <div className="text-2xl font-bold" style={{ color: TEAL }}>{value}<span className="text-sm text-slate-500 ml-1">{unit}</span></div>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 flex flex-col items-center">
            <div className="text-xs text-slate-500 uppercase tracking-widest mb-4">Score Global L&amp;D</div>
            <div className="w-32 h-32"><GaugeRing value={avgComposite} color={TEAL} /></div>
            <div className="mt-3 text-3xl font-bold" style={{ color: TEAL }}>{avgComposite.toFixed(1)}</div>
          </div>
        ...
        {entities.length > 0 && (
          <div className="rounded-xl bg-slate-900 border border-slate-800 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-800">
              <div className="text-xs text-slate-500 uppercase tracking-widest">Nations vulnérables analysées</div>
            </div>
            <div className="divide-y divide-slate-800">
              {entities.map((e) => {
                ...
                const idx = e.estimated_climate_finance_loss_damage_index as number;
                return (
        ...
        <div className="rounded-xl border p-5 text-sm" style={{ borderColor: TEAL + "40", backgroundColor: TEAL + "10" }}>
          <div className="font-semibold mb-1" style={{ color: TEAL }}>Note Droits</div>
          <p className="text-slate-300 text-xs leading-relaxed">
            Les nations critiques (Bangladesh, Pakistan, Mozambique, Tuvalu) subissent des pertes climatiques dépassant 5% de leur PIB annuel.
            Le Fonds COP28 de 700M USD reste insuffisant face aux 400 milliards estimés nécessaires chaque année d&apos;ici 2030.
          </p>
        </div>
      </div>
    </div>
