...
interface Entity {
  ...
  last_updated: string;
  gig_worker_misclassification_score?: number;
  algorithm_wage_manipulation_score?: number;
  social_protection_exclusion_score?: number;
  collective_bargaining_denial_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "gig_worker_misclassification_score", label: "Mauvaise classification des travailleurs de plateforme" },
  { key: "algorithm_wage_manipulation_score", label: "Manipulation algorithmique des salaires" },
  { key: "social_protection_exclusion_score", label: "Exclusion de la protection sociale" },
  { key: "collective_bargaining_denial_score", label: "Déni de négociation collective" },
];

function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  const [tab, setTab] = useState<"overview" | "scores" | "context">("overview");
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto"
        ...
        <div className="flex border-b border-slate-800">
          {(["overview", "scores", "context"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`flex-1 py-2.5 text-sm font-medium capitalize transition-colors ${tab === t ? "text-white border-b-2" : "text-slate-400 hover:text-slate-200"}`}
              style={tab === t ? { borderColor: ACCENT } : {}}>
              {t === "overview" ? "Vue d&apos;ensemble" : t === "scores" ? "Sous-scores" : "Contexte"}
            </button>
          ))}
        </div>
        <div className="p-6 space-y-4">
          ...
          {tab === "scores" && (
            <div className="space-y-3">
              {SUB_SCORES.map(({ key, label }) => {
                const val = entity[key] as number | undefined;
                return (
                  <div key={key}>
                    ...
                    {val !== undefined && (
                      <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                        <div className="h-full rounded-full" style={{ width: `${Math.min(val, 100)}%`, backgroundColor: ACCENT }} />
                      </div>
                    )}
                  </div>
                );
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits des travailleurs de plateforme : mauvaise classification des travailleurs, manipulation algorithmique des salaires, exclusion de la protection sociale et déni de négociation collective.</p>
              <p className="text-slate-400">Source : Platform Workers Rights Engine · Wave 249</p>
            </div>
          )}
        </div>
      </div>
...
export default function PlatformWorkersRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/platform-workers-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  const indexValue = Object.entries(data).find(([k]) => k.startsWith("avg_estimated_"))?.[1] as number | undefined;

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Platform Workers Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Classification · Manipulation Algorithmique · Protection Sociale · Négociation Collective</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{indexValue ?? "—"}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Alertes</p>
          <p className="text-3xl font-bold mt-1 text-red-400">{Array.isArray(data.critical_alerts) ? data.critical_alerts.length : (data.critical_alerts ?? 0)}</p>
        </div>
      </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-blue-900/40 text-blue-300"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "#1e40af20" } : {}}>
        ...
        {filtered.map(e => {
          ...
          return (
            <button key={e.id} onClick={() => setSelected(e)}
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold uppercase ${cfg.color}`}>{cfg.label}</span>
                <span className="text-xs text-slate-500">{e.primary_pattern?.replace(/_/g, " ")}</span>
              </div>
            </button>
          );
        })}
