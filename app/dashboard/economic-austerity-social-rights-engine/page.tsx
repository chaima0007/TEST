...
interface Entity {
  ...
  healthcare_education_cuts_severity_score: number;
  pension_social_security_dismantlement_scale_score: number;
  imf_structural_adjustment_conditionality_score: number;
  progressive_taxation_wealth_redistribution_deficit_gap_score: number;
  estimated_economic_austerity_social_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_economic_austerity_social_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Coupes Santé & Éducation", value: entity.healthcare_education_cuts_severity_score, weight: "0.30" },
    { label: "Démantèlement Retraites & Sécurité Sociale", value: entity.pension_social_security_dismantlement_scale_score, weight: "0.25" },
    { label: "Conditionnalités FMI & Ajustement Structurel", value: entity.imf_structural_adjustment_conditionality_score, weight: "0.25" },
    { label: "Déficit Fiscalité Progressive & Redistribution", value: entity.progressive_taxation_wealth_redistribution_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_economic_austerity_social_rights_index === "number" ? entity.estimated_economic_austerity_social_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Austérité & Droits Sociaux</div>
                </div>
              </div>
...
export default function EconomicAusteritySocialRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/economic-austerity-social-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Austérité Économique & Droits Sociaux…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_economic_austerity_social_rights_index ?? avg(allEntities.map(e => e.estimated_economic_austerity_social_rights_index));
  ...
  const avgHealthcare = avg(allEntities.map(e => e.healthcare_education_cuts_severity_score));
  const avgPension = avg(allEntities.map(e => e.pension_social_security_dismantlement_scale_score));
  const avgImf = avg(allEntities.map(e => e.imf_structural_adjustment_conditionality_score));
  const avgTaxation = avg(allEntities.map(e => e.progressive_taxation_wealth_redistribution_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Austérité & Droits Sociaux", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Austérité Économique & Droits Sociaux</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Economic Austerity Social Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgHealthcare} label="Coupes Santé & Éducation" />
          <GaugeRing value={avgPension} label="Retraites & Séc. Sociale" />
          <GaugeRing value={avgImf} label="Conditionnalités FMI" />
          <GaugeRing value={avgTaxation} label="Fiscalité Progressive" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Austérité & Droits Sociaux: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_economic_austerity_social_rights_index === "number" ? e.estimated_economic_austerity_social_rights_index.toFixed(2) : "—"}</span>
            </div>
