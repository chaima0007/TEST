...
interface DBEntity {
  ...
  bonded_labor_prevalence_score: number;
  debt_coercion_mechanism_score: number;
  legal_protection_failure_score: number;
  intergenerational_transmission_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_debt_bondage_index: number;
  last_updated: string;
}
...
interface DBSummary {
  ...
  data_sources: string[];
  entities: DBEntity[];
  avg_estimated_debt_bondage_index: number;
}
...
function EntityCard({ entity, onClick }: { entity: DBEntity; onClick: (e: DBEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.bonded_labor_prevalence_score}</p>
          <p className="text-[10px] text-gray-500">Travail Lié</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.debt_coercion_mechanism_score}</p>
      ...
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="text-gray-500 truncate ml-2">DB: {entity.estimated_debt_bondage_index}/10</span>
      </div>
    </button>
  );
}

function DetailModal({ entity, onClose }: { entity: DBEntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Travail Lié Prévalence", value: `${entity.bonded_labor_prevalence_score}/100` },
                  { label: "Mécanisme Coercition Dette", value: `${entity.debt_coercion_mechanism_score}/100` },
                  { label: "Défaillance Protection Légale", value: `${entity.legal_protection_failure_score}/100` },
                  { label: "Transmission Intergénérationnelle", value: `${entity.intergenerational_transmission_score}/100` },
                  { label: "Indice DB", value: `${entity.estimated_debt_bondage_index}/10` },
            ...
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-300 leading-relaxed">
                  Renforcer les protections légales contre l&apos;esclavage par endettement et soutenir les mécanismes de libération des travailleurs liés.
                </p>
              </div>
...
export default function DebtBondageEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/debt-bondage-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as DBData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Debt Bondage Engine</h1>
            <p className="text-sm text-gray-400 mt-1">Esclavage par Endettement &amp; Servitude pour Dettes</p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Indice DB moyen" value={`${s.avg_estimated_debt_bondage_index?.toFixed(1) ?? "—"}/10`} sub="servitude pour dettes" accent="text-amber-500" />
              <KpiCard label="Sources de données" value={s.data_sources?.length ?? 0} sub="organismes OIT/ONU" accent="text-violet-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Esclavage par Endettement</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_debt_bondage_index ?? 0} max={10} label="Indice DB" color="#f59e0b" />
                </div>
              </div>
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.critical_alerts} entité{s.critical_alerts > 1 ? "s" : ""} en situation critique d&apos;esclavage par endettement
                  </p>
                </div>
              </div>
            )}
