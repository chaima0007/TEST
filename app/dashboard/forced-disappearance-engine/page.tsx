...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/forced-disappearance-engine").then(r => r.json()).then(j => setData(j.payload ?? j));
  }, []);
  ...
  const PATTERN_LABELS: Record<string, string> = {
    disparition_forcee_etatique: "Disparition Forcée Étatique",
    reseau_detention_secrete: "Réseau Détention Secrète",
    deni_famille_information: "Déni Famille Information",
    impunite_perpetrateurs_disparition: "Impunité Perpétrateurs",
  };
  const SCORE_KEYS = [
    "state_enforced_disappearance_score",
    "secret_detention_network_score",
    "family_notification_denial_score",
    "disappearance_impunity_score",
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Forced Disappearance Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Surveillance des disparitions forcées et détentions secrètes</p>
      </div>
