...
export default function LgbtqRightsViolenceCriminalizationPage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#7e22ce";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", modéré: "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/lgbtq-rights-violence-criminalization-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_lgbtq_rights_violence_criminalization_index"] as number) ?? 0;
