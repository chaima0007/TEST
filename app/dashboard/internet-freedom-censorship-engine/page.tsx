...
interface Entity {
  ...
  censorship_blocking_severity_score: number;
  surveillance_data_control_score: number;
  shutdown_infrastructure_disruption_score: number;
  press_digital_rights_suppression_score: number;
  estimated_internet_freedom_censorship_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_internet_freedom_censorship_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Censure & Blocage Sévérité", value: entity.censorship_blocking_severity_score, weight: "0.30" },
    { label: "Surveillance & Contrôle des Données", value: entity.surveillance_data_control_score, weight: "0.25" },
    { label: "Coupures & Perturbations Infra", value: entity.shutdown_infrastructure_disruption_score, weight: "0.25" },
    { label: "Suppression Droits Numériques & Presse", value: entity.press_digital_rights_suppression_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT_DISPLAY }}>{typeof entity.estimated_internet_freedom_censorship_index === "number" ? entity.estimated_internet_freedom_censorship_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Liberté Internet</div>
                </div>
              </div>
...
export default function InternetFreedomCensorshipPage() {
  ...
  useEffect(() => {
    fetch("/api/internet-freedom-censorship-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT_DISPLAY }}>Initialisation Liberté Internet & Censure…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_internet_freedom_censorship_index ?? avg(allEntities.map(e => e.estimated_internet_freedom_censorship_index));
  ...
  const avgCensure = avg(allEntities.map(e => e.censorship_blocking_severity_score));
  const avgSurveillance = avg(allEntities.map(e => e.surveillance_data_control_score));
  const avgShutdown = avg(allEntities.map(e => e.shutdown_infrastructure_disruption_score));
  const avgPresse = avg(allEntities.map(e => e.press_digital_rights_suppression_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Liberté Internet", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Liberté Internet & Censure Numérique</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Internet Freedom Censorship Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgCensure} label="Censure & Blocage" />
          <GaugeRing value={avgSurveillance} label="Surveillance" />
          <GaugeRing value={avgShutdown} label="Coupures Internet" />
          <GaugeRing value={avgPresse} label="Droits Numériques" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Liberté Internet: <span className="font-medium" style={{ color: ACCENT_DISPLAY }}>{typeof e.estimated_internet_freedom_censorship_index === "number" ? e.estimated_internet_freedom_censorship_index.toFixed(2) : "—"}</span>
            </div>
