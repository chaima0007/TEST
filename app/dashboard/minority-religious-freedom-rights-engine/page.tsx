...
interface Entity {
  ...
  religious_persecution_imprisonment_severity_score: number;
  place_of_worship_destruction_ban_scale_score: number;
  blasphemy_apostasy_law_prosecution_score: number;
  religious_minority_civil_rights_exclusion_gap_score: number;
  estimated_minority_religious_freedom_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_minority_religious_freedom_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Persécution & Emprisonnement Religieux", value: entity.religious_persecution_imprisonment_severity_score, weight: "0.30" },
    { label: "Destruction Lieux de Culte", value: entity.place_of_worship_destruction_ban_scale_score, weight: "0.25" },
    { label: "Poursuites Blasphème & Apostasie", value: entity.blasphemy_apostasy_law_prosecution_score, weight: "0.25" },
    { label: "Exclusion Droits Civils Minorités", value: entity.religious_minority_civil_rights_exclusion_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_minority_religious_freedom_rights_index === "number" ? entity.estimated_minority_religious_freedom_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Liberté Religieuse</div>
                </div>
              </div>
...
export default function MinorityReligiousFreedomRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/minority-religious-freedom-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Liberté Religieuse des Minorités…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_minority_religious_freedom_rights_index ?? avg(allEntities.map(e => e.estimated_minority_religious_freedom_rights_index));
  ...
  const avgPersecution = avg(allEntities.map(e => e.religious_persecution_imprisonment_severity_score));
  const avgWorship = avg(allEntities.map(e => e.place_of_worship_destruction_ban_scale_score));
  const avgBlasphemy = avg(allEntities.map(e => e.blasphemy_apostasy_law_prosecution_score));
  const avgCivilRights = avg(allEntities.map(e => e.religious_minority_civil_rights_exclusion_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Liberté Religieuse", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Liberté Religieuse des Minorités</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Minority Religious Freedom Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgPersecution} label="Persécution Religieuse" />
          <GaugeRing value={avgWorship} label="Lieux de Culte" />
          <GaugeRing value={avgBlasphemy} label="Blasphème & Apostasie" />
          <GaugeRing value={avgCivilRights} label="Droits Civils Minorités" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Liberté Religieuse: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_minority_religious_freedom_rights_index === "number" ? e.estimated_minority_religious_freedom_rights_index.toFixed(2) : "—"}</span>
            </div>
