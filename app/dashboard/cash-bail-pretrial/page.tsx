...
export default function CashBailPretrialDashboard() {
  const [data, setData] = useState<{ entities?: { entity: string; composite: number; level: string }[]; avg_composite?: number } | null>(null);
  useEffect(() => { fetch("/api/cashbailpretrial").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-2">Cash Bail &amp; Pretrial Justice Reform</h1>
      <p className="text-gray-500 text-sm mb-6">Réforme du système de caution monétaire et droits à la liberté préventive</p>
