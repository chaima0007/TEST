...
interface Entity {
  ...
  neural_data_extraction_commercialization_score: number;
  cognitive_manipulation_advertising_score: number;
  mental_privacy_surveillance_score: number;
  neurorights_regulatory_framework_score: number;
  estimated_neurotech_cognitive_liberty_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_neurotech_cognitive_liberty_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Extraction & Commercialisation Données Neurales", value: entity.neural_data_extraction_commercialization_score, weight: "0.30" },
    { label: "Manipulation Cognitive Publicitaire", value: entity.cognitive_manipulation_advertising_score, weight: "0.25" },
    { label: "Surveillance Vie Mentale Privée", value: entity.mental_privacy_surveillance_score, weight: "0.25" },
    { label: "Déficit Cadre Réglementaire Neurodroits", value: entity.neurorights_regulatory_framework_score, weight: "0.20" },
  ];
  return (
    <div
          ...
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                ...
                <div style={{ background: "rgba(30,41,59,0.5)", borderRadius: "12px", padding: "16px", textAlign: "center" }}>
                  <div style={{ fontSize: "28px", fontWeight: "bold", color: ACCENT }}>{typeof entity.estimated_neurotech_cognitive_liberty_index === "number" ? entity.estimated_neurotech_cognitive_liberty_index.toFixed(2) : "—"}</div>
                  <div style={{ fontSize: "11px", color: "#94a3b8", marginTop: "4px" }}>Index Liberté Cognitive</div>
                </div>
              </div>
...
export default function NeurotechCognitiveLibertyRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/neurotech-cognitive-liberty-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ background: "#020617", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: ACCENT, fontSize: "13px" }}>Initialisation Neurotechnologie &amp; Liberté Cognitive…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_neurotech_cognitive_liberty_index ?? avg(allEntities.map(e => e.estimated_neurotech_cognitive_liberty_index));
  ...
  const avgNeural = avg(allEntities.map(e => e.neural_data_extraction_commercialization_score));
  const avgCognitive = avg(allEntities.map(e => e.cognitive_manipulation_advertising_score));
  const avgMental = avg(allEntities.map(e => e.mental_privacy_surveillance_score));
  const avgRegulatory = avg(allEntities.map(e => e.neurorights_regulatory_framework_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Liberté Cognitive", value: avgIndex.toFixed(2) },
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "white", padding: "24px", display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "4px" }}>
          ...
          <h1 style={{ fontSize: "22px", fontWeight: "bold", letterSpacing: "-0.025em", margin: 0 }}>Neurotechnologie &amp; Liberté Cognitive</h1>
        </div>
        <p style={{ color: "#94a3b8", fontSize: "13px", marginLeft: "24px" }}>
          Neurotech Cognitive Liberty Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: "12px", padding: "20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "24px" }}>
          <GaugeRing value={avgNeural} label="Extraction Données Neurales" />
          <GaugeRing value={avgCognitive} label="Manipulation Cognitive" />
          <GaugeRing value={avgMental} label="Surveillance Vie Mentale" />
          <GaugeRing value={avgRegulatory} label="Déficit Régulation Neurodroits" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div style={{ fontSize: "11px", color: "#64748b", marginTop: "8px" }}>
              Index Liberté Cognitive: <span style={{ fontWeight: 500, color: ACCENT }}>{typeof e.estimated_neurotech_cognitive_liberty_index === "number" ? e.estimated_neurotech_cognitive_liberty_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
