...
interface Entity {
  ...
  indigenous_data_extraction_commercialization_severity_score: number;
  biometric_genomic_collection_without_consent_scale_score: number;
  government_surveillance_indigenous_communities_score: number;
  ocap_fpic_data_governance_exclusion_gap_score: number;
  estimated_indigenous_data_sovereignty_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_indigenous_data_sovereignty_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Extraction Commerciale", value: entity.indigenous_data_extraction_commercialization_severity_score, weight: "0.30" },
    { label: "Biométrie Sans Consentement", value: entity.biometric_genomic_collection_without_consent_scale_score, weight: "0.25" },
    { label: "Surveillance Communautés", value: entity.government_surveillance_indigenous_communities_score, weight: "0.25" },
    { label: "Exclusion Gouvernance OCAP", value: entity.ocap_fpic_data_governance_exclusion_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_indigenous_data_sovereignty_index === "number" ? entity.estimated_indigenous_data_sovereignty_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Souveraineté Données</div>
                </div>
              </div>
...
export default function IndigenousDataSovereigntyEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/indigenous-data-sovereignty-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Souveraineté Données Autochtones…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_indigenous_data_sovereignty_index ?? avg(allEntities.map(e => e.estimated_indigenous_data_sovereignty_index));
  ...
  const avgExtractionData = avg(allEntities.map(e => e.indigenous_data_extraction_commercialization_severity_score));
  const avgBiometrie = avg(allEntities.map(e => e.biometric_genomic_collection_without_consent_scale_score));
  const avgSurveillance = avg(allEntities.map(e => e.government_surveillance_indigenous_communities_score));
  const avgExclusionOCAP = avg(allEntities.map(e => e.ocap_fpic_data_governance_exclusion_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Souveraineté Données", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Indigenous Data Sovereignty Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Extraction de données autochtones, biométrie sans consentement et surveillance communautaire — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgExtractionData} label="Extraction Données" />
          <GaugeRing value={avgBiometrie} label="Biométrie SC" />
          <GaugeRing value={avgSurveillance} label="Surveillance" />
          <GaugeRing value={avgExclusionOCAP} label="Exclusion OCAP" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Souveraineté Données: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_indigenous_data_sovereignty_index === "number" ? e.estimated_indigenous_data_sovereignty_index.toFixed(2) : "—"}</span>
            </div>
