...
interface Entity {
  ...
  autonomous_lethal_decision_risk_score: number;
  ihl_compliance_deficit_score: number;
  accountability_gap_aws_score: number;
  ban_treaty_resistance_score: number;
  estimated_autonomous_weapons_lethal_robotics_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_autonomous_weapons_lethal_robotics_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Risque Décision Létale Autonome", value: entity.autonomous_lethal_decision_risk_score, weight: "0.30" },
    { label: "Déficit Conformité DIH", value: entity.ihl_compliance_deficit_score, weight: "0.25" },
    { label: "Lacune Redevabilité SALA", value: entity.accountability_gap_aws_score, weight: "0.25" },
    { label: "Résistance Traité d&apos;Interdiction", value: entity.ban_treaty_resistance_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_autonomous_weapons_lethal_robotics_rights_index === "number" ? entity.estimated_autonomous_weapons_lethal_robotics_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Armes Autonomes</div>
                </div>
              </div>
...
export default function AutonomousWeaponsLethalRoboticsRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/autonomous-weapons-lethal-robotics-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Armes Autonomes &amp; Robots Tueurs…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_autonomous_weapons_lethal_robotics_rights_index
    ?? (Object.entries(data ?? {}).find(([k]) => k.startsWith("avg_estimated_"))?.[1] as number)
    ?? avg(allEntities.map(e => e.estimated_autonomous_weapons_lethal_robotics_rights_index));
  ...
  const avgLethalDecision = avg(allEntities.map(e => e.autonomous_lethal_decision_risk_score));
  const avgIhl = avg(allEntities.map(e => e.ihl_compliance_deficit_score));
  const avgAccountability = avg(allEntities.map(e => e.accountability_gap_aws_score));
  const avgBan = avg(allEntities.map(e => e.ban_treaty_resistance_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Armes Autonomes", value: typeof avgIndex === "number" ? avgIndex.toFixed(2) : "—" },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Armes Autonomes &amp; Robots Tueurs — Droits Humains</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Killer Robots · LAWS · Responsabilité IA · Droit International Humanitaire — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgLethalDecision} label="Décision Létale Autonome" />
          <GaugeRing value={avgIhl} label="Déficit DIH" />
          <GaugeRing value={avgAccountability} label="Redevabilité SALA" />
          <GaugeRing value={avgBan} label="Résistance Traité" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Armes Autonomes: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_autonomous_weapons_lethal_robotics_rights_index === "number" ? e.estimated_autonomous_weapons_lethal_robotics_rights_index.toFixed(2) : "—"}</span>
            </div>
