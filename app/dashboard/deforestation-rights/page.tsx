...
interface Entity {
  ...
  illegal_logging_forest_clearing_severity_score: number;
  indigenous_land_displacement_deforestation_score: number;
  carbon_credit_greenwashing_impunity_score: number;
  forest_defender_criminalization_violence_score: number;
  estimated_deforestation_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_deforestation_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déforestation Illégale & Défrichage", value: entity.illegal_logging_forest_clearing_severity_score, weight: "0.30" },
    { label: "Déplacement Autochtone lié à la Déforestation", value: entity.indigenous_land_displacement_deforestation_score, weight: "0.25" },
    { label: "Greenwashing Crédits Carbone & Impunité", value: entity.carbon_credit_greenwashing_impunity_score, weight: "0.25" },
    { label: "Criminalisation & Violence contre Défenseurs", value: entity.forest_defender_criminalization_violence_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_deforestation_rights_index === "number" ? entity.estimated_deforestation_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Déforestation</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-sm text-slate-300 space-y-1">
                  <div>Déforestation Illégale × 0.30</div>
                  <div>Déplacement Autochtone × 0.25</div>
                  <div>Greenwashing Carbone × 0.25</div>
                  <div>Criminalisation Défenseurs × 0.20</div>
                </div>
              </div>
...
export default function DeforestationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/deforestation-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits contre la Déforestation…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_deforestation_rights_index ?? avg(allEntities.map(e => e.estimated_deforestation_rights_index));
  ...
  const avgIllegalLogging = avg(allEntities.map(e => e.illegal_logging_forest_clearing_severity_score));
  const avgIndigenousDisplacement = avg(allEntities.map(e => e.indigenous_land_displacement_deforestation_score));
  const avgGreenwashing = avg(allEntities.map(e => e.carbon_credit_greenwashing_impunity_score));
  const avgDefenderCriminalization = avg(allEntities.map(e => e.forest_defender_criminalization_violence_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Déforestation", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits contre la Déforestation</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Deforestation Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgIllegalLogging} label="Déforestation Illégale" />
          <GaugeRing value={avgIndigenousDisplacement} label="Déplacement Autochtone" />
          <GaugeRing value={avgGreenwashing} label="Greenwashing Carbone" />
          <GaugeRing value={avgDefenderCriminalization} label="Criminalisation Défenseurs" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Déforestation: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_deforestation_rights_index === "number" ? e.estimated_deforestation_rights_index.toFixed(2) : "—"}</span>
            </div>
