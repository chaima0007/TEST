...
interface Entity {
  ...
  last_updated: string;
  integration_program_absence_score?: number;
  language_barrier_exclusion_score?: number;
  employment_discrimination_refugee_score?: number;
  housing_segregation_refugee_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "integration_program_absence_score", label: "Absence de programmes d&apos;intégration" },
  { key: "language_barrier_exclusion_score", label: "Exclusion par barrière linguistique" },
  { key: "employment_discrimination_refugee_score", label: "Discrimination à l&apos;emploi des réfugiés" },
  { key: "housing_segregation_refugee_score", label: "Ségrégation résidentielle des réfugiés" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits d&apos;intégration des réfugiés : absence de programmes d&apos;intégration, barrières linguistiques, discrimination à l&apos;emploi et ségrégation résidentielle.</p>
              <p className="text-slate-400">Source : Refugee Integration Rights Engine · Wave 252</p>
            </div>
          )}
        </div>
      </div>
...
export default function RefugeeIntegrationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/refugee-integration-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Refugee Integration Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Programmes d&apos;Intégration · Barrière Linguistique · Emploi · Logement</p>
        </div>
