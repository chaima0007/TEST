...
interface Entity {
  ...
  water_access_rights_deficit_score: number;
  aquifer_depletion_community_rights_score: number;
  irrigation_monopoly_rights_score: number;
  smallholder_water_exclusion_score: number;
  estimated_avocado_water_theft_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_avocado_water_theft_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déficit Droits Accès Eau", value: entity.water_access_rights_deficit_score, weight: "0.30" },
    { label: "Droits Communautaires Aquifères", value: entity.aquifer_depletion_community_rights_score, weight: "0.25" },
    { label: "Droits Monopole Irrigation", value: entity.irrigation_monopoly_rights_score, weight: "0.25" },
    { label: "Exclusion Eau Petits Agriculteurs", value: entity.smallholder_water_exclusion_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_avocado_water_theft_index === "number" ? entity.estimated_avocado_water_theft_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Avocats-Vol d&apos;Eau</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = AccèsEau×0.30 + Aquifères×0.25 + Irrigation×0.25 + Exclusion×0.20
                </div>
              </div>
...
export default function AvocadoWaterTheftRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/avocado-water-theft-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits — Avocats et Vol d&apos;Eau…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_avocado_water_theft_index ?? avg(allEntities.map(e => e.estimated_avocado_water_theft_index));
  ...
  const avgAccesEau = avg(allEntities.map(e => e.water_access_rights_deficit_score));
  const avgAquiferes = avg(allEntities.map(e => e.aquifer_depletion_community_rights_score));
  const avgIrrigation = avg(allEntities.map(e => e.irrigation_monopoly_rights_score));
  const avgExclusion = avg(allEntities.map(e => e.smallholder_water_exclusion_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Avocats-Eau", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits — Avocats et Vol d&apos;Eau</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Avocado Water Theft Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgAccesEau} label="Accès Eau" />
          <GaugeRing value={avgAquiferes} label="Aquifères Communautaires" />
          <GaugeRing value={avgIrrigation} label="Monopole Irrigation" />
          <GaugeRing value={avgExclusion} label="Exclusion Petits Agriculteurs" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Avocats-Eau: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_avocado_water_theft_index === "number" ? e.estimated_avocado_water_theft_index.toFixed(2) : "—"}</span>
            </div>
