...
interface WTBREntity {
  entity_id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  illegal_wildlife_trade_volume_score: number;
  species_extinction_risk_score: number;
  enforcement_capacity_gap_score: number;
  transnational_crime_network_score: number;
  estimated_wildlife_trafficking_biodiversity_rights_index: number;
  risk_level: string;
...
interface WTBRData {
  ...
  data_sources: string[];
  entities: WTBREntity[];
  avg_estimated_wildlife_trafficking_biodiversity_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function WildlifeTraffickingBiodiversityRightsPage() {
  const [data, setData] = useState<WTBRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<WTBREntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/wildlife-trafficking-biodiversity-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-cyan-400 animate-pulse">Chargement des données trafic faune…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "illegal_wildlife_trade_volume_score", label: "Volume Trafic Illégal" },
    { key: "species_extinction_risk_score", label: "Risque Extinction Espèces" },
    { key: "enforcement_capacity_gap_score", label: "Déficit Application Loi" },
    { key: "transnational_crime_network_score", label: "Réseaux Crime Transnational" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: ACCENT, boxShadow: `0 0 6px #38bdf8` }} />
          <h1 className="text-2xl font-bold tracking-tight">Trafic Faune Sauvage &amp; Droits Biodiversité</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">CITES · UNODC · WWF · 4ème crime organisé mondial</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_wildlife_trafficking_biodiversity_rights_index ?? "—" },
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#38bdf8" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Trafic</h2>
            <div className="space-y-2">
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#38bdf8" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_wildlife_trafficking_biodiversity_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Trafic Faune Sauvage &amp; Droits Biodiversité · {data.last_analysis}</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#38bdf8" />
                    <div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Trafic Faune &amp; Droits Biodiversité</div>
                    <div className="text-lg font-bold text-cyan-400">{selected.estimated_wildlife_trafficking_biodiversity_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
