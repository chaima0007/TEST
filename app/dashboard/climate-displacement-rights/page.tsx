...
interface Entity {
  ...
  climate_induced_displacement_scale_severity_score: number;
  statelessness_legal_protection_gap_score: number;
  adaptation_finance_access_inequity_score: number;
  loss_damage_reparation_accountability_score: number;
  estimated_climate_displacement_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_climate_displacement_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déplacements Induits par le Climat", value: entity.climate_induced_displacement_scale_severity_score, weight: "0.30" },
    { label: "Apatridie & Lacunes Protection Juridique", value: entity.statelessness_legal_protection_gap_score, weight: "0.25" },
    { label: "Iniquité Accès Finance Adaptation", value: entity.adaptation_finance_access_inequity_score, weight: "0.25" },
    { label: "Pertes & Dommages — Responsabilité", value: entity.loss_damage_reparation_accountability_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_climate_displacement_rights_index === "number" ? entity.estimated_climate_displacement_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Déplacés Climatiques</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-sm text-slate-300 space-y-1">
                  <div>Déplacements Induits Climat × 0.30</div>
                  <div>Apatridie & Protection Juridique × 0.25</div>
                  <div>Finance Adaptation Inéquitable × 0.25</div>
                  <div>Pertes & Dommages Responsabilité × 0.20</div>
                </div>
              </div>
...
export default function ClimateDisplacementRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/climate-displacement-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits des Déplacés Climatiques…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_climate_displacement_rights_index ?? avg(allEntities.map(e => e.estimated_climate_displacement_rights_index));
  ...
  const avgDisplacement = avg(allEntities.map(e => e.climate_induced_displacement_scale_severity_score));
  const avgStatelessness = avg(allEntities.map(e => e.statelessness_legal_protection_gap_score));
  const avgFinance = avg(allEntities.map(e => e.adaptation_finance_access_inequity_score));
  const avgLossDamage = avg(allEntities.map(e => e.loss_damage_reparation_accountability_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Déplacés Climatiques", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits des Déplacés Climatiques</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Climate Displacement Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgDisplacement} label="Déplacements Climatiques" />
          <GaugeRing value={avgStatelessness} label="Apatridie & Protection" />
          <GaugeRing value={avgFinance} label="Finance Adaptation" />
          <GaugeRing value={avgLossDamage} label="Pertes & Dommages" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Déplacés Climatiques: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_climate_displacement_rights_index === "number" ? e.estimated_climate_displacement_rights_index.toFixed(2) : "—"}</span>
            </div>
