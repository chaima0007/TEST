...
const ACCENT = "#b45309"
const DOMAIN = "statelessness-document-rights-engine"

type Entity = {
  id: string; name: string; composite_score: number; risk_level: string
  stateless_population_scale_score: number
  documentation_access_denial_score: number
  legal_protection_framework_gap_score: number
  political_will_resolution_deficit_score: number
  estimated_statelessness_document_rights_index: number
}
...
export default function StatelessnessDocumentRightsDashboard() {
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "#f1f5f9", fontFamily: "system-ui, sans-serif", padding: "32px 24px" }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <span style={{ color: "#94a3b8", fontSize: 13, letterSpacing: "0.08em", textTransform: "uppercase" }}>Apatridie & Droits Documentaires — Wave 193</span>
        </div>
        <h1 style={{ fontSize: 28, fontWeight: 700, color: "#f8fafc", margin: 0 }}>Apatridie &amp; Acc&egrave;s aux Documents</h1>
        <p style={{ color: "#94a3b8", fontSize: 14, marginTop: 6 }}>Analyse des risques pour les droits civils fondamentaux des populations apatrides — {data.entities.length} entités</p>
      </div>
          ...
          <div key={entity.id} onClick={() => setSelected(entity)} style={{ background: "#1e293b", borderRadius: 12, padding: 20, cursor: "pointer", border: "1px solid #334155" }} onMouseEnter={e => (e.currentTarget.style.borderColor = ACCENT)} onMouseLeave={e => (e.currentTarget.style.borderColor = "#334155")}>
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {[["Population Apatride", entity.stateless_population_scale_score], ["Accès Documents", entity.documentation_access_denial_score], ["Gap Légal", entity.legal_protection_framework_gap_score], ["Déficit Volonté", entity.political_will_resolution_deficit_score]].map(([label, val]) => (
            ...
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #334155", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 11, color: "#64748b" }}>Index Apatridie</span>
              <span style={{ fontSize: 16, fontWeight: 700, color: ACCENT }}>{entity.estimated_statelessness_document_rights_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
      <div style={{ textAlign: "center", color: "#475569", fontSize: 12, paddingTop: 16, borderTop: "1px solid #1e293b" }}>Caelum Partners — Wave 193 · Statelessness Document Rights Engine · {new Date(data.generated_at).toLocaleDateString("fr-FR")}</div>
      {selected && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setSelected(null)}>
          <div style={{ background: "#1e293b", borderRadius: 16, padding: 32, maxWidth: 480, width: "90%", border: "1px solid #475569" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
              {[["Population Apatride", selected.stateless_population_scale_score], ["Déni Accès Documents", selected.documentation_access_denial_score], ["Gap Cadre Légal", selected.legal_protection_framework_gap_score], ["Déficit Volonté Politique", selected.political_will_resolution_deficit_score]].map(([label, val]) => (
            ...
            <div style={{ background: "#0f172a", borderRadius: 8, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 13, color: "#94a3b8" }}>Index Apatridie</span>
              <span style={{ fontSize: 20, fontWeight: 700, color: ACCENT }}>{selected.estimated_statelessness_document_rights_index.toFixed(2)}</span>
            </div>
          </div>
        </div>
      )}
