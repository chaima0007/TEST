...
interface Entity {
  ...
  extrajudicial_killing_state_sanctioned_severity_score: number;
  death_penalty_wrongful_execution_scale_score: number;
  enforced_disappearance_targeted_assassination_score: number;
  accountability_impunity_state_violence_deficit_gap_score: number;
  estimated_death_penalty_extrajudicial_killings_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_death_penalty_extrajudicial_killings_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Meurtres Extrajudiciaires Étatiques", value: entity.extrajudicial_killing_state_sanctioned_severity_score, weight: "0.30" },
    { label: "Peine Mort & Exécutions Injustes", value: entity.death_penalty_wrongful_execution_scale_score, weight: "0.25" },
    { label: "Disparitions Forcées & Assassinats", value: entity.enforced_disappearance_targeted_assassination_score, weight: "0.25" },
    { label: "Impunité Violence Étatique", value: entity.accountability_impunity_state_violence_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_death_penalty_extrajudicial_killings_index === "number" ? entity.estimated_death_penalty_extrajudicial_killings_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Peine de Mort</div>
                </div>
              </div>
...
export default function DeathPenaltyExtrajudicialKillingsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/death-penalty-extrajudicial-killings-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Peine de Mort & Exécutions Extrajudiciaires…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_death_penalty_extrajudicial_killings_index ?? avg(allEntities.map(e => e.estimated_death_penalty_extrajudicial_killings_index));
  ...
  const avgExtrajudicial = avg(allEntities.map(e => e.extrajudicial_killing_state_sanctioned_severity_score));
  const avgDeathPenalty = avg(allEntities.map(e => e.death_penalty_wrongful_execution_scale_score));
  const avgDisparitions = avg(allEntities.map(e => e.enforced_disappearance_targeted_assassination_score));
  const avgImpunite = avg(allEntities.map(e => e.accountability_impunity_state_violence_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Peine de Mort", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Peine de Mort & Exécutions Extrajudiciaires</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Death Penalty & Extrajudicial Killings Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgExtrajudicial} label="Meurtres Extrajudiciaires" />
          <GaugeRing value={avgDeathPenalty} label="Peine de Mort" />
          <GaugeRing value={avgDisparitions} label="Disparitions Forcées" />
          <GaugeRing value={avgImpunite} label="Impunité Étatique" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Peine de Mort: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_death_penalty_extrajudicial_killings_index === "number" ? e.estimated_death_penalty_extrajudicial_killings_index.toFixed(2) : "—"}</span>
            </div>
