...
interface Entity {
  ...
  social_protection_exclusion_severity_score: number;
  labor_rights_enforcement_gap_score: number;
  income_insecurity_poverty_trap_scale_score: number;
  formalization_barrier_discrimination_score: number;
  estimated_informal_economy_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_informal_economy_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Sévérité Exclusion Protection Sociale", value: entity.social_protection_exclusion_severity_score, weight: "0.30" },
    { label: "Lacune Application Droits du Travail", value: entity.labor_rights_enforcement_gap_score, weight: "0.25" },
    { label: "Échelle Insécurité Revenus & Piège Pauvreté", value: entity.income_insecurity_poverty_trap_scale_score, weight: "0.25" },
    { label: "Barrière Formalisation & Discrimination", value: entity.formalization_barrier_discrimination_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_informal_economy_rights_index === "number" ? entity.estimated_informal_economy_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Économie Informelle</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = ProtectionSociale×0.30 + DroitsTravail×0.25 + Revenus×0.25 + Formalisation×0.20
                </div>
              </div>
...
export default function InformalEconomyRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/informal-economy-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits dans l&apos;Économie Informelle…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_informal_economy_rights_index ?? avg(allEntities.map(e => e.estimated_informal_economy_rights_index));
  ...
  const avgSocial = avg(allEntities.map(e => e.social_protection_exclusion_severity_score));
  const avgLabor = avg(allEntities.map(e => e.labor_rights_enforcement_gap_score));
  const avgIncome = avg(allEntities.map(e => e.income_insecurity_poverty_trap_scale_score));
  const avgFormalization = avg(allEntities.map(e => e.formalization_barrier_discrimination_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Économie Informelle", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits dans l&apos;Économie Informelle</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Informal Economy Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgSocial} label="Protection Sociale" />
          <GaugeRing value={avgLabor} label="Droits du Travail" />
          <GaugeRing value={avgIncome} label="Insécurité Revenus" />
          <GaugeRing value={avgFormalization} label="Barrière Formalisation" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Économie Informelle: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_informal_economy_rights_index === "number" ? e.estimated_informal_economy_rights_index.toFixed(2) : "—"}</span>
            </div>
