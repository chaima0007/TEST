...
interface Entity {
  ...
  caste_based_violence_impunity_severity_score: number;
  economic_exclusion_manual_scavenging_scale_score: number;
  educational_discrimination_access_barriers_score: number;
  legal_protection_enforcement_deficit_gap_score: number;
  estimated_caste_discrimination_dalit_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_caste_discrimination_dalit_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Violences Basées sur la Caste & Impunité", value: entity.caste_based_violence_impunity_severity_score, weight: "0.30" },
    { label: "Exclusion Économique & Ébouage Manuel", value: entity.economic_exclusion_manual_scavenging_scale_score, weight: "0.25" },
    { label: "Discrimination Éducative & Barrières d&apos;Accès", value: entity.educational_discrimination_access_barriers_score, weight: "0.25" },
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_caste_discrimination_dalit_rights_index === "number" ? entity.estimated_caste_discrimination_dalit_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Discrimination Caste</div>
                </div>
              </div>
...
export default function CasteDiscriminationDalitRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/caste-discrimination-dalit-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Discrimination de Caste &amp; Droits Dalit…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_caste_discrimination_dalit_rights_index ?? avg(allEntities.map(e => e.estimated_caste_discrimination_dalit_rights_index));
  ...
  const avgViolence = avg(allEntities.map(e => e.caste_based_violence_impunity_severity_score));
  const avgEconomic = avg(allEntities.map(e => e.economic_exclusion_manual_scavenging_scale_score));
  const avgEducation = avg(allEntities.map(e => e.educational_discrimination_access_barriers_score));
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Discrimination de Caste &amp; Droits Dalit</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          1.3 milliard de personnes affectées · Inde · Népal · Pakistan · Bangladesh
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgViolence} label="Violences & Impunité" />
          <GaugeRing value={avgEconomic} label="Exclusion Économique" />
          <GaugeRing value={avgEducation} label="Discrimination Éducative" />
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Discrimination Caste: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_caste_discrimination_dalit_rights_index === "number" ? e.estimated_caste_discrimination_dalit_rights_index.toFixed(2) : "—"}</span>
            </div>
