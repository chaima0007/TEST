...
interface Entity {
  ...
  detention_enfants_preventive_longue_duree_mixte_adultes_score: number;
  manque_alternatives_detention_absence_tribunaux_specialises_score: number;
  conditions_detention_violence_abus_education_sante_score: number;
  acces_representation_juridique_recours_effectifs_mineurs_score: number;
  estimated_juvenile_justice_child_detention_rights_index: number;
  risk_level: string;
...
const SUB_SCORES = [
  { key: "detention_enfants_preventive_longue_duree_mixte_adultes_score", label: "Détention Préventive & Mixité Adultes" },
  { key: "manque_alternatives_detention_absence_tribunaux_specialises_score", label: "Absence Alternatives & Tribunaux Spécialisés" },
  { key: "conditions_detention_violence_abus_education_sante_score", label: "Conditions Détention & Abus" },
  { key: "acces_representation_juridique_recours_effectifs_mineurs_score", label: "Accès Représentation Juridique" },
];
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto"
        ...
        <div className="p-6 border-b border-slate-800 flex justify-between items-start">
          <div>
            ...
            <p className="text-sm text-slate-400">{entity.country}</p>
          </div>
          <button onClick={onClose} className="text-slate-500 hover:text-white text-xl leading-none">✕</button>
        </div>
        <div className="flex border-b border-slate-800">
          {["Aperçu", "Métriques"].map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 text-blue-400" : "text-slate-400 hover:text-white"}`}
              style={tab === i ? { borderColor: ACCENT_LIGHT } : {}}>
              {t}
            </button>
          ))}
        </div>
          ...
          {tab === 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-bold" style={{ color: ACCENT_LIGHT }}>{entity.composite_score}/100</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Indice Justice Juvénile</span>
                <span className="font-semibold" style={{ color: ACCENT_LIGHT }}>{entity.estimated_juvenile_justice_child_detention_rights_index}/10</span>
              </div>
              <div className="flex items-center justify-between text-sm">
              ...
              {SUB_SCORES.map(({ key, label }) => (
                <div key={key}>
                  ...
                  <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${entity[key] as number}%`, backgroundColor: ACCENT_LIGHT }} />
                  </div>
                </div>
              ))}
            </div>
...
export default function JuvenileJusticeChildDetentionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/juvenile-justice-child-detention-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="w-8 h-8 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: ACCENT_LIGHT, borderTopColor: "transparent" }} />
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT_LIGHT }}>Justice Juvénile &amp; Détention d&apos;Enfants</h1>
          <p className="text-slate-400 mt-1 text-sm">Droits CRC · Réhabilitation · Criminalisation Jeunesse · Alternatives Détention</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT_LIGHT }}>{data.total_entities}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col items-center">
          ...
          <GaugeRing value={data.avg_composite} stroke={ACCENT_LIGHT} />
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice JJD</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT_LIGHT }}>{avgIndex ?? "—"}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Confiance</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT_LIGHT }}>{data.confidence_score ?? "—"}</p>
        </div>
      </div>
            ...
            <button key={e.entity_id} onClick={() => setSelected(e)}
              ...
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="shrink-0 ml-3">
                  <GaugeRing value={e.composite_score} stroke={ACCENT_LIGHT} />
                </div>
              </div>
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold uppercase ${cfg.color}`}>{cfg.label}</span>
                <span className="text-xs text-slate-500">Indice: <span className="font-bold" style={{ color: ACCENT_LIGHT }}>{e.estimated_juvenile_justice_child_detention_rights_index}</span></span>
              </div>
            </button>
          );
        })}
