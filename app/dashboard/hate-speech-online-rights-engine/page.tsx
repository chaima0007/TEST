...
interface Entity {
  ...
  last_updated: string;
  online_incitement_violence_score?: number;
  platform_moderation_failure_score?: number;
  minority_targeting_score?: number;
...
const SUB_SCORES = [
  { key: "online_incitement_violence_score", label: "Incitation en ligne à la violence" },
  { key: "platform_moderation_failure_score", label: "Échec de modération des plateformes" },
  { key: "minority_targeting_score", label: "Ciblage des minorités" },
  { key: "legal_accountability_gap_score", label: "Lacunes de responsabilité légale" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des discours haineux en ligne : incitation à la violence, échec de modération des plateformes, ciblage des minorités et lacunes de responsabilité légale.</p>
              <p className="text-slate-400">Source : Hate Speech Online Rights Engine · Wave 251</p>
            </div>
          )}
        </div>
      </div>
...
export default function HateSpeechOnlineRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/hate-speech-online-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Hate Speech Online Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Incitation Violence · Modération Plateformes · Ciblage Minorités · Responsabilité Légale</p>
        </div>
