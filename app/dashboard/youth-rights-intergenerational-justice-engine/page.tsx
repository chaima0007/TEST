...
interface Entity {
  ...
  youth_employment_education_rights_deprivation_score: number;
  child_soldier_exploitation_intergenerational_harm_score: number;
  intergenerational_poverty_inequality_transmission_score: number;
  state_protection_future_generations_deficit_score: number;
  estimated_youth_rights_intergenerational_justice_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_youth_rights_intergenerational_justice_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Privation Emploi & Éducation Jeunes", value: entity.youth_employment_education_rights_deprivation_score, weight: "0.30" },
    { label: "Exploitation Enfants Soldats", value: entity.child_soldier_exploitation_intergenerational_harm_score, weight: "0.25" },
    { label: "Transmission Pauvreté Intergénérationnelle", value: entity.intergenerational_poverty_inequality_transmission_score, weight: "0.25" },
    { label: "Déficit Protection Générations Futures", value: entity.state_protection_future_generations_deficit_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_youth_rights_intergenerational_justice_index === "number" ? entity.estimated_youth_rights_intergenerational_justice_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Justice Intergénérationnelle</div>
                </div>
              </div>
...
export default function YouthRightsIntergenerationalJusticeEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/youth-rights-intergenerational-justice-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits des Jeunes &amp; Justice Intergénérationnelle…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_youth_rights_intergenerational_justice_index ?? avg(allEntities.map(e => e.estimated_youth_rights_intergenerational_justice_index));
  ...
  const avgYouthEmployment = avg(allEntities.map(e => e.youth_employment_education_rights_deprivation_score));
  const avgChildSoldier = avg(allEntities.map(e => e.child_soldier_exploitation_intergenerational_harm_score));
  const avgPoverty = avg(allEntities.map(e => e.intergenerational_poverty_inequality_transmission_score));
  const avgStateProtection = avg(allEntities.map(e => e.state_protection_future_generations_deficit_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Justice Intergén.", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits des Jeunes &amp; Justice Intergénérationnelle</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Youth Rights &amp; Intergenerational Justice Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgYouthEmployment} label="Privation Emploi/Éduc." />
          <GaugeRing value={avgChildSoldier} label="Enfants Soldats" />
          <GaugeRing value={avgPoverty} label="Pauvreté Intergén." />
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Justice Intergén.: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_youth_rights_intergenerational_justice_index === "number" ? e.estimated_youth_rights_intergenerational_justice_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
