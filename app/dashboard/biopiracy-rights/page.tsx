...
interface Entity {
  ...
  last_updated: string;
  genetic_resource_score?: number;
  patent_protection_score?: number;
  indigenous_knowledge_score?: number;
  benefit_sharing_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "genetic_resource_score", label: "Ressources génétiques" },
  { key: "patent_protection_score", label: "Protection des brevets" },
  { key: "indigenous_knowledge_score", label: "Savoirs autochtones" },
  { key: "benefit_sharing_score", label: "Partage des avantages" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits contre la biopiraterie : ressources génétiques, protection des brevets, savoirs autochtones et partage des avantages affectant les communautés locales et autochtones.</p>
              <p className="text-slate-400">Source : Biopiracy Rights Engine · Wave 266</p>
            </div>
          )}
        </div>
      </div>
...
export default function BiopiacyRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/biopiracy-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits contre la Biopiraterie</h1>
          <p className="text-slate-400 mt-1 text-sm">Ressources Génétiques · Protection des Brevets · Savoirs Autochtones · Partage des Avantages</p>
        </div>
