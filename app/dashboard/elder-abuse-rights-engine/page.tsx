...
interface EAREntity {
  ...
  institutional_abuse_prevalence_score: number;
  financial_exploitation_scale_score: number;
  legal_protection_framework_gap_score: number;
  autonomy_dignity_violation_score: number;
  risk_level: string;
  primary_pattern: string;
  estimated_elder_abuse_rights_index: number;
  last_updated: string;
  [key: string]: unknown;
}

interface EARSummary {
  agent: string;
  domain: string;
  ...
  avg_estimated_elder_abuse_rights_index: number;
  data_sources: string[];
  entities: EAREntity[];
}
...
function DetailModal({ entity, onClose }: { entity: EAREntity; onClose: () => void }) {
  const [tab, setTab] = useState(0);
  const tabs = ["Scores", "Indicateurs", "Contexte maltraitance"];
  return (
      ...
      <div className="w-full max-w-lg rounded-xl border border-slate-700/50 bg-slate-900 p-6 shadow-2xl">
        ...
        {tab === 0 && (
          <div className="space-y-3">
            {[
              { label: "Abus Institutionnel", value: entity.institutional_abuse_prevalence_score },
              { label: "Exploitation Financière", value: entity.financial_exploitation_scale_score },
              { label: "Gap Protection Légale", value: entity.legal_protection_framework_gap_score },
              { label: "Autonomie & Dignité", value: entity.autonomy_dignity_violation_score },
            ].map((s) => (
        ...
        {tab === 1 && (
          <div className="flex flex-col items-center gap-4 py-4">
            <GaugeRing value={entity.estimated_elder_abuse_rights_index} max={10} />
            <div className="text-center">
              <div className="text-3xl font-bold text-purple-400">
                {typeof entity.estimated_elder_abuse_rights_index === "number"
                  ? entity.estimated_elder_abuse_rights_index.toFixed(1)
                  : entity.estimated_elder_abuse_rights_index}
              </div>
              <div className="text-sm text-slate-500">Index Maltraitance Aînés</div>
            </div>
          </div>
        )}
        {tab === 2 && (
          <div className="rounded-lg border border-slate-700/40 bg-slate-800/50 p-4">
            <p className="text-sm leading-relaxed text-slate-400">
              Analyse du contexte de maltraitance des aînés pour {entity.name}. Indice global&nbsp;:&nbsp;
              <span className="text-slate-200 font-medium">
                {typeof entity.estimated_elder_abuse_rights_index === "number"
                  ? entity.estimated_elder_abuse_rights_index.toFixed(2)
                  : entity.estimated_elder_abuse_rights_index}
              </span>
...
export default function ElderAbuseRightsEngine() {
  const [data, setData] = useState<ApiData>({});
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<EAREntity | null>(null);

  useEffect(() => {
    fetch("/api/elder-abuse-rights-engine")
      .then((r) => r.json())
  ...
  const entities: EAREntity[] = (data.entities ?? []).filter(
    ...
    <div className="min-h-screen bg-slate-950 p-6 text-slate-100">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-100">Elder Abuse Rights Engine</h1>
            <p className="mt-1 text-sm text-slate-500">Surveillance de la maltraitance des aînés — abus institutionnels, exploitation financière &amp; autonomie</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {(kpis.length > 0 ? kpis : [
            { label: "Abus Institutionnel", value: "—" },
            { label: "Exploitation Financière", value: "—" },
            { label: "Gap Protection Légale", value: "—" },
            { label: "Autonomie & Dignité", value: "—" },
            { label: "Index Maltraitance", value: "—" },
            ...
            {entities.map((entity) => (
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="text-xs text-slate-500">Maltraitance Index</span>
                ...
                <div className="mt-2 space-y-1">
                  {[
                    { key: "institutional_abuse_prevalence_score", label: "Abus Institutionnel" },
                    { key: "financial_exploitation_scale_score", label: "Exploitation Financière" },
                    { key: "legal_protection_framework_gap_score", label: "Gap Protection Légale" },
                    { key: "autonomy_dignity_violation_score", label: "Autonomie & Dignité" },
                  ].map(({ key, label }) => (
