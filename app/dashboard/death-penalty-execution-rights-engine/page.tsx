...
export default function DeathPenaltyExecutionRightsPage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#27141c";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", "modéré": "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/death-penalty-execution-rights-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_death_penalty_execution_rights_index"] as number) ?? 0;
