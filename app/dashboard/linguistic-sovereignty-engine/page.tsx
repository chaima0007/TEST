...
interface LSEntity {
  ...
  composite_score: number; speakers_collapse_score: number;
  intergenerational_transmission_failure_score: number;
  digital_language_exclusion_score: number;
  colonial_language_dominance_score: number;
  risk_level: string; primary_pattern: string; key_signals: string[];
  estimated_linguistic_risk_index: number; last_updated: string;
}
interface LSSummary {
  ...
  confidence_score: number; data_sources: string[]; entities: LSEntity[];
  avg_estimated_linguistic_risk_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: LSEntity; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-slate-900 border border-purple-900/40 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Effondrement Locuteurs", v: entity.speakers_collapse_score, c: "#a855f7" },
              { l: "Échec Transmission", v: entity.intergenerational_transmission_failure_score, c: "#c084fc" },
              { l: "Exclusion Numérique", v: entity.digital_language_exclusion_score, c: "#d8b4fe" },
              { l: "Domination Coloniale", v: entity.colonial_language_dominance_score, c: "#7c3aed" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Risque Linguistique Caelum</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_linguistic_risk_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <p>› Programme d&apos;urgence de revitalisation — documentation et transmission inter-générationnelle</p>}
            {entity.risk_level === "élevé" && <p>› Politiques d&apos;immersion linguistique et protection des espaces de transmission</p>}
            {entity.risk_level === "modéré" && <p>› Politiques de soutien à la vitalité linguistique et reconnaissance officielle</p>}
            {entity.risk_level === "faible" && <p>› Maintien des politiques de vitalité et documentation proactive</p>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  extinction_linguistique: "#ef4444",
  domination_coloniale_linguistique: "#f97316",
  erosion_acceleree: "#eab308",
  pression_linguistique: "#f59e0b",
  vitalite_preservee: "#10b981",
};

export default function LinguisticSovereigntyEnginePage() {
  const [data, setData] = useState<LSSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<LSEntity | null>(null);

  useEffect(() => {
    fetch("/api/linguistic-sovereignty-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données souveraineté linguistique"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la souveraineté linguistique en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof LSEntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Souveraineté Linguistique</h1>
          <p className="text-slate-400 text-sm mt-1">La disparition des langues comme perte géopolitique — la domination linguistique est la forme la plus durable du colonialisme</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Composite Moyen", v: `${data.avg_composite}%`, t: "text-purple-300" },
            { l: "Alertes Critiques", v: data.critical_alerts.length, t: "text-red-400" },
            { l: "Indice Risque Linguistique", v: data.avg_estimated_linguistic_risk_index, t: "text-purple-400" },
            { l: "Sources", v: data.data_sources?.length ?? 0, t: "text-purple-300" },
            { l: "Confiance Analyse", v: `${Math.round((data.confidence_score ?? 0) * 100)}%`, t: "text-slate-300" },
          ].map(({ l, v, t }) => (
            <div key={l} className="bg-slate-900 border border-purple-500/20 rounded-xl p-4">
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-purple-500/20">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs de Risque Linguistique — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("speakers_collapse_score")} label="Effondrement Locuteurs" color={ACCENT} />
            <GaugeRing value={avgScore("intergenerational_transmission_failure_score")} label="Échec Transmission" color="#c084fc" />
            <GaugeRing value={avgScore("digital_language_exclusion_score")} label="Exclusion Numérique" color="#d8b4fe" />
            <GaugeRing value={avgScore("colonial_language_dominance_score")} label="Domination Coloniale" color="#7c3aed" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-purple-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Risque Linguistique</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-purple-900/10 border border-purple-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Souveraineté Linguistique Critiques</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              className={`bg-slate-900 rounded-xl p-4 border cursor-pointer transition-all hover:border-purple-500/50 ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_linguistic_risk_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Souveraineté Linguistique · {data.last_analysis}</p>
      </div>
    </div>
  );
}
