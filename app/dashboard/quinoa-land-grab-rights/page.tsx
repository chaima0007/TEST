...
interface Entity {
  ...
  indigenous_land_dispossession_score: number;
  export_monoculture_rights_deficit_score: number;
  andean_community_displacement_score: number;
  food_sovereignty_erosion_score: number;
  estimated_quinoa_land_grab_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_quinoa_land_grab_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Dépossession Terres Indigènes", value: entity.indigenous_land_dispossession_score, weight: "0.30" },
    { label: "Déficit Droits Monoculture Export", value: entity.export_monoculture_rights_deficit_score, weight: "0.25" },
    { label: "Déplacement Communautés Andines", value: entity.andean_community_displacement_score, weight: "0.25" },
    { label: "Érosion Souveraineté Alimentaire", value: entity.food_sovereignty_erosion_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_quinoa_land_grab_index === "number" ? entity.estimated_quinoa_land_grab_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Quinoa-Accaparement</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = Dépossession×0.30 + Monoculture×0.25 + Déplacement×0.25 + Souveraineté×0.20
                </div>
              </div>
...
export default function QuinoaLandGrabRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/quinoa-land-grab-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits — Quinoa et Accaparement Terres Andines…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_quinoa_land_grab_index ?? avg(allEntities.map(e => e.estimated_quinoa_land_grab_index));
  ...
  const avgDepossession = avg(allEntities.map(e => e.indigenous_land_dispossession_score));
  const avgMonoculture = avg(allEntities.map(e => e.export_monoculture_rights_deficit_score));
  const avgDeplacement = avg(allEntities.map(e => e.andean_community_displacement_score));
  const avgSouverainete = avg(allEntities.map(e => e.food_sovereignty_erosion_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Quinoa-Accaparement", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits — Quinoa et Accaparement Terres Andines</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Quinoa Land Grab Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgDepossession} label="Dépossession Terres" />
          <GaugeRing value={avgMonoculture} label="Monoculture Export" />
          <GaugeRing value={avgDeplacement} label="Déplacement Andin" />
          <GaugeRing value={avgSouverainete} label="Souveraineté Alimentaire" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Quinoa-Accaparement: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_quinoa_land_grab_index === "number" ? e.estimated_quinoa_land_grab_index.toFixed(2) : "—"}</span>
            </div>
