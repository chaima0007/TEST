...
interface Entity {
  ...
  reparations_denial_score: number;
  perpetrator_impunity_score: number;
  memory_historical_denial_score: number;
  risk_level: string;
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_transitional_justice_index: number;
}
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/transitional-justice-engine").then(r => r.json()).then(j => setData(j.data ?? j)).catch(console.error);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Transitional Justice Engine</h1>
        <p className="text-slate-400 mt-1">CPI · Commissions Vérité · Réparations Post-Conflit</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_transitional_justice_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { key: "truth_commission_absence_score", label: "Absence Commission Vérité" },
                      { key: "reparations_denial_score", label: "Déni Réparations" },
                      { key: "perpetrator_impunity_score", label: "Impunité Auteurs" },
                      { key: "memory_historical_denial_score", label: "Déni Mémoire" },
                    ].map(({ key, label }) => (
                      <div key={key}>
