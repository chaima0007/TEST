...
interface Entity {
  ...
  curriculum_obstruction_score: number;
  civil_society_repression_score: number;
  access_denial_marginalized_score: number;
  international_framework_gap_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_human_rights_education_index: number;
  last_updated: string;
}
...
interface Data {
  ...
  avg_estimated_human_rights_education_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Obstruction des Programmes", v: entity.curriculum_obstruction_score, c: "#0e7490" },
              { l: "Répression Société Civile", v: entity.civil_society_repression_score, c: "#0c6478" },
              { l: "Déni d'Accès aux Marginalisés", v: entity.access_denial_marginalized_score, c: "#0a5163" },
              { l: "Écart Cadres Internationaux", v: entity.international_framework_gap_score, c: "#083d4a" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Éducation DR Estimé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_human_rights_education_index}</span>
            </div>
        ...
        {tab === "contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Intervention d&apos;urgence pour protéger les éducateurs et organisations de la société civile ciblés pour leur travail sur les droits humains</p>
                <p>› Mobilisation des mécanismes onusiens pour l&apos;éducation et des rapporteurs spéciaux sur la liberté d&apos;expression et d&apos;enseignement</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Documentation systématique des obstructions aux programmes d&apos;éducation aux droits humains et des restrictions imposées aux ONG</p>
                <p>› Plaidoyer pour l&apos;alignement des curricula nationaux sur les standards de l&apos;ONU et les résolutions du Conseil des droits de l&apos;homme</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance des restrictions à l&apos;éducation aux droits et soutien aux acteurs locaux de la société civile engagés dans la sensibilisation</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur l&apos;accès à l&apos;éducation aux droits humains et prévention des régressions normatives</p>
            )}
          </div>
        )}
      </div>
...
export default function HumanRightsEducationEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/human-rights-education-engine")
      .then(r => r.json())
      .then(d => { setData(d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données Human Rights Education Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de l&apos;éducation aux droits humains…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Index Éducation DR Moyen", v: `${(data.avg_estimated_human_rights_education_index ?? 0).toFixed(2)}`, c: "text-cyan-300" },
    { l: "Critique", v: dist.critique ?? 0, c: "text-red-400" },
    { l: "Index Éducation DR", v: dist["élevé"] ?? 0, c: "text-orange-400" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Éducation Droits Humains</h1>
          <p className="text-slate-400 text-sm mt-1">Obstruction des programmes, répression de la société civile et écarts avec les cadres internationaux</p>
        ...
        <div className="bg-slate-900 border border-cyan-500/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Éducation Droits Humains Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("curriculum_obstruction_score")} label="Obstruction Programmes" color={ACCENT} />
            <GaugeRing value={avgScore("civil_society_repression_score")} label="Répression Société Civile" color="#0c6478" />
            <GaugeRing value={avgScore("access_denial_marginalized_score")} label="Déni Accès Marginalisés" color="#0a5163" />
            <GaugeRing value={avgScore("international_framework_gap_score")} label="Écart Cadres Intl." color="#083d4a" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-cyan-500/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns d&apos;Obstruction Éducative</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#0e7490", "#0c6478", "#0a5163", "#083d4a", "#062d37"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_human_rights_education_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Human Rights Education Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
