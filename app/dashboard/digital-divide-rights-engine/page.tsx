...
interface Entity {
  ...
  last_updated: string;
  internet_access_gap_score?: number;
  device_affordability_exclusion_score?: number;
  digital_literacy_barrier_score?: number;
  rural_connectivity_deprivation_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "internet_access_gap_score", label: "Écart accès internet" },
  { key: "device_affordability_exclusion_score", label: "Exclusion coût appareils" },
  { key: "digital_literacy_barrier_score", label: "Barrière alphabétisation numérique" },
  { key: "rural_connectivity_deprivation_score", label: "Privation connectivité rurale" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la fracture numérique : écarts d&apos;accès à internet, exclusion due au coût des appareils, barrières d&apos;alphabétisation numérique et privation de connectivité dans les zones rurales.</p>
              <p className="text-slate-400">Source : Digital Divide Rights Engine · Wave 263</p>
            </div>
          )}
        </div>
      </div>
...
export default function DigitalDivideRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/digital-divide-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Digital Divide Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Accès Internet · Appareils · Alphabétisation · Zones Rurales</p>
        </div>
