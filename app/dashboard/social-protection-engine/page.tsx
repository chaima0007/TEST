...
interface SPEntity {
  ...
  composite_score: number; social_security_coverage_gap_score: number;
  pension_system_collapse_score: number; child_poverty_welfare_failure_score: number;
  informal_worker_exclusion_score: number; risk_level: string; primary_pattern: string;
  key_signals: string[]; avg_estimated_social_protection_index: number; last_updated: string;
}
interface SPSummary {
  ...
  confidence_score: number; data_sources: string[]; entities: SPEntity[];
  avg_estimated_social_protection_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: SPEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "Aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Lacune Séc. Sociale", v: entity.social_security_coverage_gap_score, c: "#ef4444" },
              { l: "Effond. Pension", v: entity.pension_system_collapse_score, c: "#f97316" },
              { l: "Pauvreté Enfants", v: entity.child_poverty_welfare_failure_score, c: "#a855f7" },
              { l: "Exclusion Informels", v: entity.informal_worker_exclusion_score, c: "#f59e0b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Protection Sociale</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.avg_estimated_social_protection_index}</span>
            </div>
          </div>
        )}
        {tab === "Signaux" && (
        ...
        {tab === "Contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Déploiement d'urgence de filets de protection sociale</p><p>› Mobilisation internationale pour les systèmes de retraite en crise</p><p>› Intervention d'urgence contre la pauvreté infantile extrême</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des programmes de sécurité sociale</p><p>› Surveillance des lacunes de couverture des travailleurs informels</p></>}
            {entity.risk_level === "modéré" && <><p>› Développement de politiques de protection sociale inclusives</p><p>› Renforcement de la résilience des systèmes de pension</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien des standards minimaux de protection sociale</p><p>› Consolidation des programmes d'aide à l'enfance</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  crise_protection_aigue: "#ef4444",
  inegalites_couverture_sociale: "#f97316",
  fragilite_systemique: "#f59e0b",
  tensions_protection: "#eab308",
  protection_exemplaire: "#10b981",
};

export default function SocialProtectionEnginePage() {
  const [data, setData] = useState<SPSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<SPEntity | null>(null);

  useEffect(() => {
    fetch("/api/social-protection-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données protection sociale"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse de la protection sociale en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof SPEntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Social Protection Engine</h1>
          <p className="text-slate-400 text-sm mt-1">La protection sociale comme droit fondamental — quand les filets de sécurité se déchirent, les plus vulnérables tombent dans le vide</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Moyen", v: `${data.avg_composite}%`, t: "text-violet-300" },
            ...
            { l: "Index Moyen Protection", v: data.avg_estimated_social_protection_index, t: "text-violet-400" },
            { l: "Confiance", v: `${Math.round((data.confidence_score ?? 0) * 100)}%`, t: "text-slate-300" },
          ].map(({ l, v, t }) => (
            <div key={l} className="bg-slate-900 border border-violet-500/20 rounded-xl p-4">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 rounded-xl p-6 border border-violet-500/20">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Dimensions des Défaillances de Protection Sociale — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("social_security_coverage_gap_score")} label="Lacune Séc. Sociale" color={ACCENT} />
            <GaugeRing value={avgScore("pension_system_collapse_score")} label="Effond. Pension" color="#f97316" />
            <GaugeRing value={avgScore("child_poverty_welfare_failure_score")} label="Pauvreté Enfants" color="#a855f7" />
            <GaugeRing value={avgScore("informal_worker_exclusion_score")} label="Exclusion Informels" color="#f59e0b" />
          </div>
        </div>

        {/* Distribution Bars */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-violet-500/20">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-violet-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Défaillance Sociale</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-violet-900/10 border border-violet-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Critiques — Protection Sociale</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.avg_estimated_social_protection_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Social Protection Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
