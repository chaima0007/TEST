...
interface Entity {
  ...
  estimated_environmental_rights_defenders_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_environmental_rights_defenders_index?: number;
...
export default function EnvironmentalRightsDefendersPage() {
  ...
  useEffect(() => {
    fetch("/api/environmental-rights-defenders-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT_DISPLAY }}>Initialisation Défenseurs Droits Environnementaux…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_environmental_rights_defenders_index ?? avg(allEntities.map(e => e.estimated_environmental_rights_defenders_index));
  ...
  const kpis = [
    ...
    { label: "Index Défenseurs Env.", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-slate-800 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-400 mt-0.5">{selected.country}</p>
                <span className={`text-xs font-semibold uppercase mt-1 inline-block ${RC[selected.risk_level] ?? "text-slate-400"}`}>{selected.risk_level}</span>
              </div>
              <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-white transition-colors text-2xl leading-none">&times;</button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT_DISPLAY }}>{selected.composite_score.toFixed(1)}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Composite</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT_DISPLAY }}>{typeof selected.estimated_environmental_rights_defenders_index === "number" ? selected.estimated_environmental_rights_defenders_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Défenseurs Env.</div>
                </div>
              </div>
              <div className={`rounded-lg p-3 border ${RB[selected.risk_level] ?? "border-slate-700 bg-slate-800/30"}`}>
                <span className={`text-sm font-semibold capitalize ${RC[selected.risk_level] ?? "text-slate-300"}`}>
                  Niveau de risque : {selected.risk_level}
                </span>
              </div>
              <div className="bg-slate-800/50 rounded-xl p-4">
                <div className="text-xs text-slate-400 mb-2">Pattern Principal</div>
                <div className="text-sm font-medium" style={{ color: ACCENT_DISPLAY }}>{selected.primary_pattern}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Défenseurs Droits Environnementaux</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Environmental Rights Defenders Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgComposite} label="Score Moyen" />
          <GaugeRing value={avgIndex * 10} label="Index Défenseurs" />
          <GaugeRing value={countCritique * 12.5} label="Taux Critique" />
          <GaugeRing value={typeof confidence === "string" ? parseFloat(confidence) || 88 : 88} label="Confiance" />
        </div>
      </div>
      ...
      {(data?.critical_alerts ?? []).length > 0 && (
        <div className="bg-green-950/30 border border-green-500/30 rounded-xl p-4">
          <h3 className="text-green-400 text-xs uppercase tracking-wide mb-2 font-semibold">Alertes Critiques</h3>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Défenseurs Env.: <span className="font-medium" style={{ color: ACCENT_DISPLAY }}>{typeof e.estimated_environmental_rights_defenders_index === "number" ? e.estimated_environmental_rights_defenders_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
