...
interface Entity {
  ...
  last_updated: string;
  armed_conflict_persistence_score?: number;
  militarization_civilian_harm_score?: number;
  peace_negotiation_exclusion_score?: number;
  disarmament_failure_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "armed_conflict_persistence_score", label: "Persistance des conflits armés" },
  { key: "militarization_civilian_harm_score", label: "Préjudices civils de la militarisation" },
  { key: "peace_negotiation_exclusion_score", label: "Exclusion des négociations de paix" },
  { key: "disarmament_failure_score", label: "Échec du désarmement" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse du droit à la paix : persistance des conflits armés, préjudices civils de la militarisation, exclusion des négociations de paix et échecs du désarmement.</p>
              <p className="text-slate-400">Source : Right to Peace Rights Engine · Wave 254</p>
            </div>
          )}
        </div>
      </div>
...
export default function RightToPeaceRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-peace-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Right to Peace Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Conflits Armés · Militarisation · Négociations de Paix · Désarmement</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#0c4a6e20" } : {}}>
