...
interface Entity {
  ...
  genetic_data_exploitation_corporate_score: number;
  discrimination_employment_insurance_score: number;
  consent_privacy_genomic_database_score: number;
  regulatory_framework_protection_score: number;
  estimated_biotech_genetic_discrimination_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_biotech_genetic_discrimination_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Exploitation Données Génétiques Corporatives", value: entity.genetic_data_exploitation_corporate_score, weight: "0.30" },
    { label: "Discrimination Emploi & Assurance", value: entity.discrimination_employment_insurance_score, weight: "0.25" },
    { label: "Consentement & Vie Privée Bases Génomiques", value: entity.consent_privacy_genomic_database_score, weight: "0.25" },
    { label: "Cadre Réglementaire Protection", value: entity.regulatory_framework_protection_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_biotech_genetic_discrimination_index === "number" ? entity.estimated_biotech_genetic_discrimination_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Discrimination Génétique</div>
                </div>
              </div>
...
export default function BiotechGeneticDiscriminationRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/biotech-genetic-discrimination-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Discrimination Génétique &amp; Biotechnologie…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_biotech_genetic_discrimination_index ?? avg(allEntities.map(e => e.estimated_biotech_genetic_discrimination_index));
  ...
  const avgGeneticExploitation = avg(allEntities.map(e => e.genetic_data_exploitation_corporate_score));
  const avgDiscrimination = avg(allEntities.map(e => e.discrimination_employment_insurance_score));
  const avgConsent = avg(allEntities.map(e => e.consent_privacy_genomic_database_score));
  const avgRegulatory = avg(allEntities.map(e => e.regulatory_framework_protection_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Discrimination Gén.", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Discrimination Génétique &amp; Biotechnologie</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Biotech Genetic Discrimination Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgGeneticExploitation} label="Exploitation Génétique Corp." />
          <GaugeRing value={avgDiscrimination} label="Discrimination Emploi/Assurance" />
          <GaugeRing value={avgConsent} label="Consentement & Vie Privée" />
          <GaugeRing value={avgRegulatory} label="Cadre Réglementaire" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Discrimination Gén.: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_biotech_genetic_discrimination_index === "number" ? e.estimated_biotech_genetic_discrimination_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
