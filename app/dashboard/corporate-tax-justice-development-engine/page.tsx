...
interface Entity {
  ...
  estimated_tax_justice_index: number;
  risk_level: string;
  [key: string]: unknown;
}
...
interface DashData {
  ...
  avg_estimated_tax_justice_index?: number;
...
export default function CorporateTaxJusticeDevelopmentDashboard() {
  const [raw, setRaw] = useState<DashData | null>(null);
  const [selected, setSelected] = useState<Entity | null>(null);
  const [riskFilter, setRiskFilter] = useState<string>("all");

  useEffect(() => {
    fetch("/api/corporate-tax-justice-development-engine")
      .then((r) => r.json())
  ...
  if (!raw) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-orange-400 text-lg animate-pulse">Initialisation du Moteur Justice Fiscale Corporative &amp; Développement...</div>
    </div>
  );
  ...
  const avgIndex = raw.avg_estimated_tax_justice_index ?? (entities.reduce((s, e) => s + e.estimated_tax_justice_index, 0) / n);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 p-6 space-y-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setSelected(null)}>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                <div className="text-3xl font-bold" style={{ color: ACCENT }}>{selected.estimated_tax_justice_index.toFixed(2)}</div>
                <div className="text-xs text-slate-400 mt-1">Index Justice Fiscale /10</div>
              </div>
            </div>
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Justice Fiscale Corporative &amp; Développement</h1>
        <p className="text-slate-400 text-sm mt-1">
          Évasion Fiscale · Paradis Fiscaux · BEPS · Impact Développement — Intelligence CTJ — Caelum Partners
        </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          ["Composite Moyen", avgComposite.toFixed(1), "text-orange-400"],
          ["Index Justice Fiscale Moyen", avgIndex.toFixed(2) + "/10", "text-orange-300"],
          ["Risque Critique", dist["critique"] ?? 0, "text-red-400"],
        ].map(([l, v, c]) => (
          <div key={String(l)} className="bg-slate-900 border border-orange-900/30 rounded-xl p-3 text-center">
      ...
      {/* Gauge Rings */}
      <div className="bg-slate-900 border border-orange-900/30 rounded-xl p-5">
      ...
      <div className="flex flex-wrap gap-2">
        {["all", "critique", "élevé", "modéré", "faible"].map((r) => (
          <button key={r} onClick={() => setRiskFilter(r)}
            ...
            style={riskFilter === r ? { background: ACCENT, borderColor: ACCENT } : { borderColor: "#7c2d12" }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map((e) => (
          <div key={e.id} onClick={() => setSelected(e)}
            className={`bg-slate-900 border rounded-xl p-4 cursor-pointer hover:border-orange-500/50 transition-colors ${RB[e.risk_level] ?? "border-slate-800"}`}>
            ...
            <div className="flex items-end justify-between">
              ...
              <div className="text-right">
                <div className="text-lg font-bold text-orange-300">{e.estimated_tax_justice_index.toFixed(2)}</div>
