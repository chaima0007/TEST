...
interface Entity {
  ...
  last_updated: string;
  sodomy_law_enforcement_score?: number;
  lgbtq_hate_crime_impunity_score?: number;
  same_sex_relationship_ban_score?: number;
  death_penalty_lgbtq_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "sodomy_law_enforcement_score", label: "Application lois sodomie" },
  { key: "lgbtq_hate_crime_impunity_score", label: "Impunité crimes haine LGBTQ" },
  { key: "same_sex_relationship_ban_score", label: "Interdiction relations même sexe" },
  { key: "death_penalty_lgbtq_score", label: "Peine de mort orientation sexuelle" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la criminalisation LGBTQ : application des lois de sodomie, impunité pour les crimes haineux, interdictions des relations et recours à la peine de mort pour orientation sexuelle.</p>
              <p className="text-slate-400">Source : LGBTQ Criminalization Rights Engine · Wave 260</p>
            </div>
          )}
        </div>
      </div>
...
export default function LgbtqCriminalizationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/lgbtq-criminalization-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>LGBTQ Criminalization Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Lois Sodomie · Crimes Haine · Interdictions · Peine de Mort</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#a855f720" } : {}}>
