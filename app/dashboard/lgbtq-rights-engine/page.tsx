...
interface LGBTQEntity {
  entity_id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  criminalization_severity_score: number;
  violence_impunity_score: number;
  legal_recognition_denial_score: number;
  forced_conversion_repression_score: number;
  risk_level: string;
...
interface LGBTQSummary {
  ...
  data_sources: string[];
  entities: LGBTQEntity[];
  avg_estimated_lgbtq_rights_index: number;
}

interface LGBTQData {
  entities: LGBTQEntity[];
  summary: LGBTQSummary;
}
...
function EntityCard({ entity, onClick }: { entity: LGBTQEntity; onClick: (e: LGBTQEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.criminalization_severity_score}</p>
          <p className="text-[10px] text-gray-500">Criminalisation</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.violence_impunity_score}</p>
          <p className="text-[10px] text-gray-500">Violence/Impunité</p>
        </div>
      </div>
      <div className="flex items-center justify-between text-xs">
        <span className="text-gray-400">Score: <span className={`font-bold ${rc}`}>{entity.composite_score}</span></span>
        <span className="text-gray-500 truncate ml-2">LGBTQ+: {entity.avg_estimated_lgbtq_rights_index}/10</span>
      </div>
    </button>
  );
}

function DetailModal({ entity, onClose }: { entity: LGBTQEntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Criminalisation", value: `${entity.criminalization_severity_score}/100` },
                  { label: "Violence/Impunité", value: `${entity.violence_impunity_score}/100` },
                  { label: "Déni Reconnaissance", value: `${entity.legal_recognition_denial_score}/100` },
                  { label: "Conversion Forcée", value: `${entity.forced_conversion_repression_score}/100` },
                  { label: "Indice LGBTQ+", value: `${entity.avg_estimated_lgbtq_rights_index}/10` },
            ...
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-300 leading-relaxed">
                  Renforcer les protections légales et sociales pour les personnes LGBTQ+, lutter contre la criminalisation et promouvoir la reconnaissance juridique des droits fondamentaux.
                </p>
              </div>
...
export default function LGBTQRightsEnginePage() {
  const [data, setData] = useState<LGBTQData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<LGBTQEntity | null>(null);

  useEffect(() => {
    fetch("/api/lgbtq-rights-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as LGBTQData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">LGBTQ+ Rights Engine</h1>
            <p className="text-sm text-gray-400 mt-1">Droits LGBTQ+ &amp; Criminalisation Systémique</p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              <KpiCard label="Total Entités" value={s.total_entities} sub="entités surveillées" />
              ...
              <KpiCard label="Critiques" value={s.critical_alerts} sub="intervention urgente" accent="text-red-400" />
              <KpiCard label="Élevés" value={s.risk_distribution["élevé"] ?? 0} sub="risque élevé" accent="text-orange-400" />
              <KpiCard label="Index Moyen LGBTQ+" value={`${s.avg_estimated_lgbtq_rights_index?.toFixed(1) ?? "—"}/10`} sub="droits LGBTQ+" accent="text-violet-400" />
              <KpiCard label="Confiance" value={s.engine_version ?? "—"} sub="version moteur" accent="text-indigo-400" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Droits LGBTQ+</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_lgbtq_rights_index ?? 0} max={10} label="Index LGBTQ+" color="#7c3aed" />
                </div>
              </div>
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.critical_alerts} entité{s.critical_alerts > 1 ? "s" : ""} en situation critique de violation des droits LGBTQ+
                  </p>
                </div>
              </div>
            )}
