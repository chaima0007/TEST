...
interface Entity {
  ...
  legal_aid_absence_score: number;
  judicial_corruption_score: number;
  impunity_accountability_score: number;
  discrimination_access_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_access_to_justice_index: number;
  last_updated: string;
  [key: string]: unknown;
}
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_access_to_justice_index: number;
}
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/access-to-justice-engine").then(r => r.json()).then(j => setData(j.data ?? j)).catch(console.error);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Access to Justice Engine</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice ATJ</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_access_to_justice_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { key: "legal_aid_absence_score", label: "Absence Aide Jur." },
                      { key: "judicial_corruption_score", label: "Corruption Jud." },
                      { key: "impunity_accountability_score", label: "Impunité" },
                      { key: "discrimination_access_score", label: "Discrimination Accès" },
                    ].map(({ key, label }) => (
                      <div key={key}>
