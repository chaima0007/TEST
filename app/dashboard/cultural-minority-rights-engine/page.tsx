...
interface Entity {
  ...
  cultural_assimilation_forced_erasure_severity_score: number;
  language_suppression_minority_discrimination_scale_score: number;
  cultural_heritage_destruction_desecration_score: number;
  minority_cultural_participation_exclusion_gap_score: number;
  estimated_cultural_minority_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_cultural_minority_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Assimilation Forcée", value: entity.cultural_assimilation_forced_erasure_severity_score, weight: "0.30" },
    { label: "Suppression Linguistique", value: entity.language_suppression_minority_discrimination_scale_score, weight: "0.25" },
    { label: "Destruction Patrimoine", value: entity.cultural_heritage_destruction_desecration_score, weight: "0.25" },
    { label: "Exclusion Participation", value: entity.minority_cultural_participation_exclusion_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_cultural_minority_rights_index === "number" ? entity.estimated_cultural_minority_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Culturels</div>
                </div>
              </div>
...
export default function CulturalMinorityRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/cultural-minority-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Minorités Culturelles…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_cultural_minority_rights_index ?? avg(allEntities.map(e => e.estimated_cultural_minority_rights_index));
  ...
  const avgAssimilation = avg(allEntities.map(e => e.cultural_assimilation_forced_erasure_severity_score));
  const avgLanguage = avg(allEntities.map(e => e.language_suppression_minority_discrimination_scale_score));
  const avgHeritage = avg(allEntities.map(e => e.cultural_heritage_destruction_desecration_score));
  const avgExclusion = avg(allEntities.map(e => e.minority_cultural_participation_exclusion_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Culturels", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Cultural Minority Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Assimilation forcée, suppression linguistique et destruction du patrimoine culturel — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgAssimilation} label="Assimilation Forcée" />
          <GaugeRing value={avgLanguage} label="Suppression Langue" />
          <GaugeRing value={avgHeritage} label="Destruction Patrim." />
          <GaugeRing value={avgExclusion} label="Exclusion Cult." />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Culturels: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_cultural_minority_rights_index === "number" ? e.estimated_cultural_minority_rights_index.toFixed(2) : "—"}</span>
            </div>
