...
interface Entity { entity: string; risk_level: string; avg_composite: number; estimated_immigration_detention_index: number; }
...
export default function ImmigrationDetentionPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/immigrationdetention");
  ...
  return (
    <div className="p-6 space-y-6 text-slate-100">
      <div>
        <h1 className="text-2xl font-bold text-white">Détention des Migrants pour Profit</h1>
        <p className="text-slate-400 text-sm mt-1">Centres de rétention privés, ICE et droits des demandeurs d&apos;asile</p>
      </div>
