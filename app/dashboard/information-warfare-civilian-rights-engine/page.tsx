...
interface Entity {
  ...
  estimated_information_warfare_civilian_rights_index: number;
}
...
interface Payload {
  ...
  avg_estimated_information_warfare_civilian_rights_index: number;
...
export default function InformationWarfareCivilianRightsPage() {
  const [data, setData] = useState<Payload | null>(null);
  const [error, setError] = useState(false);

  useEffect(() => {
    fetch("/api/information-warfare-civilian-rights-engine")
      .then((r) => r.json())
    ...
    <main className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="rounded-2xl p-6 text-white" style={{ backgroundColor: ACCENT }}>
          <h1 className="text-2xl font-bold">Guerre de l&apos;Information &amp; Droits des Civils</h1>
          <p className="text-sm mt-1 opacity-80">Désinformation · EU DisinfoLab · Freedom House · Stanford IO</p>
          <div className="mt-4 flex gap-8 text-sm">
            ...
            <div>
              <div className="opacity-60">Index Info-Guerre</div>
              <div className="text-xl font-semibold">{data.avg_estimated_information_warfare_civilian_rights_index}</div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data.entities.map((e, i) => (
              <div key={i} className="bg-white rounded-xl p-4 shadow-sm flex items-center gap-4">
                <GaugeRing
                  value={e.estimated_information_warfare_civilian_rights_index}
