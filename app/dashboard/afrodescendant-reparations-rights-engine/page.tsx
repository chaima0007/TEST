...
interface Entity {
  ...
  structural_racism_systemic_exclusion_severity_score: number;
  reparations_policy_implementation_deficit_scale_score: number;
  police_violence_criminal_justice_bias_score: number;
  political_representation_recognition_gap_score: number;
  estimated_afrodescendant_reparations_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_afrodescendant_reparations_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Racisme Structurel & Exclusion Systémique", value: entity.structural_racism_systemic_exclusion_severity_score, weight: "0.30" },
    { label: "Déficit Politique Réparations & Application", value: entity.reparations_policy_implementation_deficit_scale_score, weight: "0.25" },
    { label: "Violences Policières & Biais Justice Pénale", value: entity.police_violence_criminal_justice_bias_score, weight: "0.25" },
    { label: "Représentation Politique & Écart Reconnaissance", value: entity.political_representation_recognition_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_afrodescendant_reparations_rights_index === "number" ? entity.estimated_afrodescendant_reparations_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Réparations Afrodescendants</div>
                </div>
              </div>
...
export default function AfrodescendantReparationsRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/afrodescendant-reparations-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Réparations &amp; Droits des Afrodescendants…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_afrodescendant_reparations_rights_index ?? avg(allEntities.map(e => e.estimated_afrodescendant_reparations_rights_index));
  ...
  const avgStructural = avg(allEntities.map(e => e.structural_racism_systemic_exclusion_severity_score));
  const avgReparations = avg(allEntities.map(e => e.reparations_policy_implementation_deficit_scale_score));
  const avgPolice = avg(allEntities.map(e => e.police_violence_criminal_justice_bias_score));
  const avgPolitical = avg(allEntities.map(e => e.political_representation_recognition_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Réparations Afrodescendants", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Réparations &amp; Droits des Afrodescendants</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Déclaration Durban · CARICOM · Justice raciale · 200M+ personnes concernées
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgStructural} label="Racisme Structurel" />
          <GaugeRing value={avgReparations} label="Politique Réparations" />
          <GaugeRing value={avgPolice} label="Violences Policières" />
          <GaugeRing value={avgPolitical} label="Représentation Politique" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Réparations Afrodescendants: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_afrodescendant_reparations_rights_index === "number" ? e.estimated_afrodescendant_reparations_rights_index.toFixed(2) : "—"}</span>
            </div>
