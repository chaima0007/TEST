...
interface Entity {
  ...
  last_updated: string;
  illegal_dumping_score?: number;
  health_impact_score?: number;
  remediation_score?: number;
  community_resistance_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "illegal_dumping_score", label: "Déversements illégaux" },
  { key: "health_impact_score", label: "Impact sur la santé" },
  { key: "remediation_score", label: "Droits à la dépollution" },
  { key: "community_resistance_score", label: "Résistance communautaire" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits contre les déchets toxiques : déversements illégaux, impact sur la santé, droits à la dépollution et résistance communautaire face aux sites contaminés.</p>
              <p className="text-slate-400">Source : Toxic Waste Dumping Rights Engine · Wave 265</p>
            </div>
          )}
        </div>
      </div>
...
export default function ToxicWasteDumpingRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/toxic-waste-dumping-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits contre les Déchets Toxiques</h1>
          <p className="text-slate-400 mt-1 text-sm">Déversements Illégaux · Impact Sanitaire · Dépollution · Résistance Communautaire</p>
        </div>
