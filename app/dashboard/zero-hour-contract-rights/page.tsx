...
interface Entity {
  ...
  scheduling_instability_rights_deficit_score: number;
  income_volatility_protection_gap_score: number;
  social_security_exclusion_score: number;
  collective_representation_denial_score: number;
  estimated_zero_hour_contract_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_zero_hour_contract_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déficit Droits Instabilité Horaire", value: entity.scheduling_instability_rights_deficit_score, weight: "0.30" },
    { label: "Lacune Protection Volatilité Revenus", value: entity.income_volatility_protection_gap_score, weight: "0.25" },
    { label: "Exclusion Sécurité Sociale", value: entity.social_security_exclusion_score, weight: "0.25" },
    { label: "Déni Représentation Collective", value: entity.collective_representation_denial_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_zero_hour_contract_rights_index === "number" ? entity.estimated_zero_hour_contract_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Zéro Heure</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = Instabilité×0.30 + Volatilité×0.25 + Exclusion×0.25 + Représentation×0.20
                </div>
              </div>
...
export default function ZeroHourContractRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/zero-hour-contract-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits des Travailleurs Zéro Heure…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_zero_hour_contract_rights_index ?? avg(allEntities.map(e => e.estimated_zero_hour_contract_rights_index));
  ...
  const avgInstabilite = avg(allEntities.map(e => e.scheduling_instability_rights_deficit_score));
  const avgVolatilite = avg(allEntities.map(e => e.income_volatility_protection_gap_score));
  const avgExclusion = avg(allEntities.map(e => e.social_security_exclusion_score));
  const avgRepresentation = avg(allEntities.map(e => e.collective_representation_denial_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Zéro Heure", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits des Travailleurs Zéro Heure</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Zero Hour Contract Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgInstabilite} label="Instabilité Horaire" />
          <GaugeRing value={avgVolatilite} label="Volatilité Revenus" />
          <GaugeRing value={avgExclusion} label="Exclusion Sociale" />
          <GaugeRing value={avgRepresentation} label="Représentation Collective" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Zéro Heure: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_zero_hour_contract_rights_index === "number" ? e.estimated_zero_hour_contract_rights_index.toFixed(2) : "—"}</span>
            </div>
