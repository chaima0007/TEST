...
const API_URL = "/api/ginger-harvesting-child-labor-rights";
const TITLE = "Droits Enfants Récolte Gingembre";
const COLOR = "#ea580c";

type Entity = { entity: string; composite_score: number; risk_level: string; estimated_ginger_harvesting_child_labor_rights_index: number };
      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: 24, width: 480, maxHeight: "80vh", overflowY: "auto" }}>
        ...
        {tab === "apercu" && (
          <div>
            ...
            <p>Indice estimé : <strong>{entity.estimated_ginger_harvesting_child_labor_rights_index}</strong></p>
          </div>
        )}
        {tab === "indicateurs" && (
          <div>
            <p style={{ color: "#6b7280", fontSize: 14 }}>Indicateurs clés pour {entity.entity} dans le domaine du gingembre.</p>
            <div style={{ marginTop: 12, padding: 12, background: "#f9fafb", borderRadius: 8 }}>
              <p>Score composite : {entity.composite_score}/100</p>
              <p>Indice domaine : {entity.estimated_ginger_harvesting_child_labor_rights_index}/10</p>
            </div>
          </div>
        )}
        {tab === "recommandations" && (
          <div>
            <ul style={{ listStyle: "disc", paddingLeft: 20, color: "#374151", fontSize: 14 }}>
              <li style={{ marginBottom: 8 }}>Renforcer la supervision des chaînes d&apos;approvisionnement en gingembre</li>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
        {data.entities.map(e => (
          <div key={e.entity} onClick={() => setSelected(e)} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 20, cursor: "pointer" }}>
            ...
            <p style={{ fontSize: 12, color: "#6b7280" }}>Indice : {e.estimated_ginger_harvesting_child_labor_rights_index}/10</p>
          </div>
        ))}
      </div>
