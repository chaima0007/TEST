...
interface EntityResult {
  ...
  risk_level: string;
  estimated_shea_butter_labor_index: number;
}
...
export default function SheaButterLaborDashboard() {
  const [data, setData] = useState<EngineData | null>(null);
  const [selected, setSelected] = useState<EntityResult | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/shea-butter-labor-rights")
      .then(r => r.json())
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 32, fontFamily: "sans-serif" }}>
      <h1 style={{ fontSize: 28, fontWeight: 800, marginBottom: 8 }}>Droits Travailleurs Beurre de Karité</h1>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 32, maxWidth: 480, width: "90%", maxHeight: "80vh", overflow: "auto" }}
            ...
            {tab === 0 && (
              <div>
                ...
                <p style={{ marginTop: 12 }}>Niveau de risque : <strong style={{ color: riskColor(selected.risk_level) }}>{selected.risk_level}</strong></p>
                <p>Score composite : <strong>{selected.composite_score}</strong></p>
              </div>
            )}
            {tab === 1 && (
              <div>
                <h3 style={{ fontWeight: 700 }}>Indicateurs clés</h3>
                <p>Score composite : {selected.composite_score}/100</p>
                <p>Niveau : {selected.risk_level}</p>
                <p>Index estimé : {(selected as Record<string, unknown>)["estimated_shea_butter_labor_index"] as number ?? "N/A"}</p>
              </div>
            )}
            {tab === 2 && (
              <div>
                <h3 style={{ fontWeight: 700 }}>Recommandations CSDDD</h3>
                <p>Conformité EU 2024/1760 requise. Audit de diligence raisonnable obligatoire pour ce domaine à risque {selected.risk_level}.</p>
              </div>
            )}
