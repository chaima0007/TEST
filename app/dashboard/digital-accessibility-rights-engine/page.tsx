...
const ACCENT = "#f97316";
const TITRE = "Accessibilité Numérique";
...
export default function Page() {
  ...
  useEffect(() => { fetch("/api/digital-accessibility-rights-engine").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
  ...
  return (
    <div style={{ background: "#020617", minHeight: "100vh", color: "#f1f5f9", fontFamily: "system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <p style={{ color: "#94a3b8", marginBottom: 32 }}>CaelumSwarm™ · CSDDD 2024/1760 · Wave 236</p>
        <div style={{ display: "flex", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
          ...
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 200px", display: "flex", flexDirection: "column", gap: 10, justifyContent: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#94a3b8" }}>Index DAR</span><strong style={{ color: ACCENT }}>{data.avg_estimated_digital_accessibility_rights_index?.toFixed(2)}</strong></div>
          </div>
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 280px" }}>
            ...
            {(data.critical_alerts ?? []).map((a: string, i: number) => (
              <div key={i} style={{ color: "#fca5a5", fontSize: 12, padding: "4px 0", borderBottom: "1px solid #1e293b" }}>{a}</div>
            ))}
          </div>
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 280px" }}>
            <div style={{ color: "#64748b", fontSize: 11, marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 }}>Métriques clés</div>
            {[
              { label: "Lacune conformité WCAG", key: "wcag_compliance_gap" },
              { label: "Exclusion tech. assistive", key: "assistive_technology_exclusion" },
              { label: "Exclusion services publics", key: "public_service_digital_exclusion" },
              { label: "Barrière emploi numérique", key: "employment_digital_barrier" },
            ].map(m => (
              <div key={m.key} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid #1e293b" }}>
                <span style={{ color: "#94a3b8", fontSize: 12 }}>{m.label}</span>
                <strong style={{ color: ACCENT, fontSize: 12 }}>{m.key}</strong>
              </div>
            ))}
          </div>
        </div>
