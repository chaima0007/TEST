...
interface WREntity {
  ...
  composite_score: number; safe_water_access_denial_score: number;
  sanitation_deprivation_score: number; water_privatization_exclusion_score: number;
  pollution_contamination_score: number; risk_level: string; primary_pattern: string;
  key_signals: string[]; avg_estimated_water_rights_index: number; last_updated: string;
}
interface WRSummary {
  ...
  confidence_score: number; data_sources: string[]; entities: WREntity[];
  avg_estimated_water_rights_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: WREntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "Aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Déni Eau Potable", v: entity.safe_water_access_denial_score, c: "#ef4444" },
              { l: "Privation Assainissement", v: entity.sanitation_deprivation_score, c: "#f97316" },
              { l: "Privatisation Eau", v: entity.water_privatization_exclusion_score, c: "#a855f7" },
              { l: "Contamination/Pollution", v: entity.pollution_contamination_score, c: "#f59e0b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Droits à l'Eau</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.avg_estimated_water_rights_index}</span>
            </div>
          </div>
        )}
        {tab === "Signaux" && (
        ...
        {tab === "Contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Mécanismes d'urgence humanitaire pour l'accès à l'eau potable</p><p>› Mobilisation internationale pour la fourniture d'eau sûre</p><p>› Réponse d'urgence aux crises d'assainissement</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des programmes d'accès universel à l'eau</p><p>› Surveillance des processus de privatisation de l'eau</p></>}
            {entity.risk_level === "modéré" && <><p>› Développement de politiques d'eau inclusives</p><p>› Renforcement de la résilience des infrastructures hydriques</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien des standards minimaux d'accès à l'eau</p><p>› Consolidation des programmes de gestion durable de l'eau</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  crise_eau_aigue: "#ef4444",
  inegalites_acces_eau: "#f97316",
  fragilite_hydrique: "#f59e0b",
  tensions_eau: "#eab308",
  gestion_eau_exemplaire: "#10b981",
};

export default function WaterRightsEnginePage() {
  const [data, setData] = useState<WRSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<WREntity | null>(null);

  useEffect(() => {
    fetch("/api/water-rights-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données droits à l'eau"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse des droits à l'eau en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof WREntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Water Rights Engine</h1>
          <p className="text-slate-400 text-sm mt-1">L'eau comme droit fondamental — quand l'accès à l'eau potable devient privilège, c'est la vie humaine elle-même qui est menacée</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Moyen", v: `${data.avg_composite}%`, t: "text-sky-300" },
            ...
            { l: "Index Moyen Eau", v: data.avg_estimated_water_rights_index, t: "text-sky-400" },
            { l: "Confiance", v: `${Math.round((data.confidence_score ?? 0) * 100)}%`, t: "text-slate-300" },
          ].map(({ l, v, t }) => (
            <div key={l} className="bg-slate-900 border border-sky-500/20 rounded-xl p-4">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 rounded-xl p-6 border border-sky-500/20">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Dimensions des Violations Hydriques — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("safe_water_access_denial_score")} label="Déni Eau Potable" color={ACCENT} />
            <GaugeRing value={avgScore("sanitation_deprivation_score")} label="Privation Assainissement" color="#f97316" />
            <GaugeRing value={avgScore("water_privatization_exclusion_score")} label="Privatisation Eau" color="#a855f7" />
            <GaugeRing value={avgScore("pollution_contamination_score")} label="Contamination/Pollution" color="#f59e0b" />
          </div>
        </div>

        {/* Distribution Bars */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-sky-500/20">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-sky-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Crise Hydrique</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-sky-900/10 border border-sky-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Critiques — Droits à l'Eau</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              className={`bg-slate-900 rounded-xl p-4 border cursor-pointer transition-all hover:border-sky-500/50 ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.avg_estimated_water_rights_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Water Rights Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
