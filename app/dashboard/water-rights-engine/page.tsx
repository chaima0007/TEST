...
interface Entity {
  ...
  access_denial_scale_score: number;
  privatization_commodification_score: number;
  pollution_industrial_contamination_score: number;
  transboundary_conflict_governance_score: number;
  risk_level: string;
  primary_pattern: string;
  estimated_water_rights_index: number;
}
      ...
      <div className="bg-slate-900 border border-cyan-500/30 rounded-xl w-full max-w-2xl p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "apercu" && (
          <div className="space-y-3">
            ...
            <div className="flex justify-between text-sm"><span className="text-slate-400">Indice droits à l&apos;eau</span><span className="text-cyan-400 font-semibold">{entity.estimated_water_rights_index?.toFixed(2)}</span></div>
        ...
        {tab === "signaux" && (
          <div className="space-y-3">
            <div className="flex justify-between text-sm"><span className="text-slate-400">Déni d&apos;accès</span><span className="text-white">{entity.access_denial_scale_score?.toFixed(2)}</span></div>
            <div className="flex justify-between text-sm"><span className="text-slate-400">Privatisation / marchandisation</span><span className="text-white">{entity.privatization_commodification_score?.toFixed(2)}</span></div>
            <div className="flex justify-between text-sm"><span className="text-slate-400">Pollution industrielle</span><span className="text-white">{entity.pollution_industrial_contamination_score?.toFixed(2)}</span></div>
            <div className="flex justify-between text-sm"><span className="text-slate-400">Conflits transfrontaliers</span><span className="text-white">{entity.transboundary_conflict_governance_score?.toFixed(2)}</span></div>
          </div>
        )}
        {tab === "contexte" && (
          <div className="space-y-3">
...
export default function WaterRightsEngine() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-cyan-400">Water Rights Engine</h1>
          <p className="text-slate-400 mt-1">Analyse des droits à l&apos;eau et indicateurs de conflits hydriques</p>
        </div>
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              ...
              <div className="bg-slate-900 border border-cyan-500/30 rounded-xl p-5">
                ...
                <div className="flex items-center gap-3">
                  <GaugeRing value={data.confidence_score} max={100} color="text-cyan-400" />
              ...
              <div className="bg-slate-900 border border-cyan-500/30 rounded-xl p-5">
                ...
                <p className="text-lg font-bold text-cyan-400 truncate">{data.top_risk_entities?.[0]?.name ?? "—"}</p>
                <p className="text-slate-400 text-xs truncate">{data.top_risk_entities?.[0]?.country ?? ""}</p>
              </div>
              <div className="bg-slate-900 border border-cyan-500/30 rounded-xl p-5">
                <p className="text-slate-400 text-xs uppercase tracking-wider mb-2">Indice droits eau moy.</p>
                <div className="flex items-center gap-3">
                  <GaugeRing value={data.avg_estimated_water_rights_index} max={100} color="text-cyan-400" />
                  <p className="text-3xl font-bold text-white">{data.avg_estimated_water_rights_index?.toFixed(1)}</p>
                </div>
              </div>
            </div>
