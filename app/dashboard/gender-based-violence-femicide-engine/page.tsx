...
interface Entity {
  ...
  femicide_domestic_violence_impunity_severity_score: number;
  rape_sexual_violence_prosecution_gap_scale_score: number;
  honor_killing_forced_marriage_prevalence_score: number;
  gbv_legal_protection_enforcement_deficit_gap_score: number;
  estimated_gender_based_violence_femicide_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_gender_based_violence_femicide_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Féminicide & Impunité", value: entity.femicide_domestic_violence_impunity_severity_score, weight: "0.30" },
    { label: "Poursuites Violences Sexuelles", value: entity.rape_sexual_violence_prosecution_gap_scale_score, weight: "0.25" },
    { label: "Crimes d'Honneur & Mariage Forcé", value: entity.honor_killing_forced_marriage_prevalence_score, weight: "0.25" },
    { label: "Déficit Protection Légale", value: entity.gbv_legal_protection_enforcement_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_gender_based_violence_femicide_index === "number" ? entity.estimated_gender_based_violence_femicide_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index VBG & Féminicide</div>
                </div>
              </div>
...
export default function GenderBasedViolenceFemicideEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/gender-based-violence-femicide-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Violences Basées sur le Genre & Féminicide…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_gender_based_violence_femicide_index ?? avg(allEntities.map(e => e.estimated_gender_based_violence_femicide_index));
  ...
  const avgFemicide = avg(allEntities.map(e => e.femicide_domestic_violence_impunity_severity_score));
  const avgRape = avg(allEntities.map(e => e.rape_sexual_violence_prosecution_gap_scale_score));
  const avgHonor = avg(allEntities.map(e => e.honor_killing_forced_marriage_prevalence_score));
  const avgProtection = avg(allEntities.map(e => e.gbv_legal_protection_enforcement_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index VBG & Féminicide", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Violences Basées sur le Genre & Féminicide</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Gender-Based Violence & Femicide Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgFemicide} label="Féminicide & Impunité" />
          <GaugeRing value={avgRape} label="Poursuites Violences" />
          <GaugeRing value={avgHonor} label="Crimes d&apos;Honneur" />
          <GaugeRing value={avgProtection} label="Déficit Protection" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index VBG & Féminicide: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_gender_based_violence_femicide_index === "number" ? e.estimated_gender_based_violence_femicide_index.toFixed(2) : "—"}</span>
            </div>
