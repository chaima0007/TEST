...
interface Entity {
  ...
  strike_prohibition_criminalization_severity_score: number;
  essential_services_overclassification_scope_scale_score: number;
  union_busting_retaliation_workers_score: number;
  precarious_workers_strike_exclusion_gap_score: number;
  estimated_right_to_strike_labor_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_right_to_strike_labor_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Criminalisation Grève", value: entity.strike_prohibition_criminalization_severity_score, weight: "0.30" },
    { label: "Services Essentiels", value: entity.essential_services_overclassification_scope_scale_score, weight: "0.25" },
    { label: "Répression Syndicale", value: entity.union_busting_retaliation_workers_score, weight: "0.25" },
    { label: "Travailleurs Précaires", value: entity.precarious_workers_strike_exclusion_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_right_to_strike_labor_index === "number" ? entity.estimated_right_to_strike_labor_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droit de Grève</div>
                </div>
              </div>
...
export default function RightToStrikeLaborEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-strike-labor-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droit de Grève…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_right_to_strike_labor_index ?? avg(allEntities.map(e => e.estimated_right_to_strike_labor_index));
  ...
  const avgStrikeProhibition = avg(allEntities.map(e => e.strike_prohibition_criminalization_severity_score));
  const avgEssentialServices = avg(allEntities.map(e => e.essential_services_overclassification_scope_scale_score));
  const avgUnionBusting = avg(allEntities.map(e => e.union_busting_retaliation_workers_score));
  const avgPrecarious = avg(allEntities.map(e => e.precarious_workers_strike_exclusion_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droit de Grève", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Right to Strike Labor Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Criminalisation des grèves, répression syndicale et exclusion des travailleurs précaires — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgStrikeProhibition} label="Criminalisation" />
          <GaugeRing value={avgEssentialServices} label="Services Essentiels" />
          <GaugeRing value={avgUnionBusting} label="Répression Syndicale" />
          <GaugeRing value={avgPrecarious} label="Travailleurs Précaires" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droit de Grève: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_right_to_strike_labor_index === "number" ? e.estimated_right_to_strike_labor_index.toFixed(2) : "—"}</span>
            </div>
