...
interface Entity {
  ...
  algorithmic_management_rights_deficit_score: number;
  social_protection_access_gap_score: number;
  collective_bargaining_exclusion_score: number;
  platform_accountability_transparency_score: number;
  estimated_platform_worker_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_platform_worker_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déficit Droits Gestion Algorithmique", value: entity.algorithmic_management_rights_deficit_score, weight: "0.30" },
    { label: "Lacune Accès Protection Sociale", value: entity.social_protection_access_gap_score, weight: "0.25" },
    { label: "Exclusion Négociation Collective", value: entity.collective_bargaining_exclusion_score, weight: "0.25" },
    { label: "Responsabilité & Transparence Plateformes", value: entity.platform_accountability_transparency_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_platform_worker_rights_index === "number" ? entity.estimated_platform_worker_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Travailleurs Plateformes</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = Algorithmique×0.30 + Protection×0.25 + Négociation×0.25 + Responsabilité×0.20
                </div>
              </div>
...
export default function PlatformWorkerRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/platform-worker-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits des Travailleurs de Plateformes…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_platform_worker_rights_index ?? avg(allEntities.map(e => e.estimated_platform_worker_rights_index));
  ...
  const avgAlgorithmique = avg(allEntities.map(e => e.algorithmic_management_rights_deficit_score));
  const avgProtection = avg(allEntities.map(e => e.social_protection_access_gap_score));
  const avgNegociation = avg(allEntities.map(e => e.collective_bargaining_exclusion_score));
  const avgResponsabilite = avg(allEntities.map(e => e.platform_accountability_transparency_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Travailleurs Plateformes", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits des Travailleurs de Plateformes</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Platform Worker Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgAlgorithmique} label="Gestion Algorithmique" />
          <GaugeRing value={avgProtection} label="Protection Sociale" />
          <GaugeRing value={avgNegociation} label="Négociation Collective" />
          <GaugeRing value={avgResponsabilite} label="Responsabilité Plateforme" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Travailleurs Plateformes: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_platform_worker_rights_index === "number" ? e.estimated_platform_worker_rights_index.toFixed(2) : "—"}</span>
            </div>
