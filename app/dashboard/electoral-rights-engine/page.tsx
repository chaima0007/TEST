...
interface Entity {
  ...
  voting_suppression_score: number;
  election_fraud_manipulation_score: number;
  opposition_persecution_score: number;
  democratic_regression_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_electoral_rights_index: number;
  last_updated: string;
  [key: string]: unknown;
}
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_electoral_rights_index: number;
}
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/electoral-rights-engine").then(r => r.json()).then(j => setData(j.data ?? j)).catch(console.error);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Electoral Rights Engine</h1>
        <p className="text-slate-400 mt-1">PIDCP Art.25 · OSCE · Intégrité Électorale</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_electoral_rights_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { key: "voting_suppression_score", label: "Suppression Vote" },
                      { key: "election_fraud_manipulation_score", label: "Fraude Électorale" },
                      { key: "opposition_persecution_score", label: "Persécution Opposition" },
                      { key: "democratic_regression_score", label: "Régression Démocratique" },
                    ].map(({ key, label }) => (
                      <div key={key}>
