...
interface RDEntity {
  ...
  biometric_data_coercion_scale_score: number;
  digital_exclusion_service_denial_severity_score: number;
  surveillance_control_refugee_population_score: number;
  data_sharing_persecution_risk_gap_score: number;
  estimated_refugee_digital_rights_index: number;
  risk_level: string;
...
interface RDData {
  ...
  data_sources: string[];
  entities: RDEntity[];
  avg_estimated_refugee_digital_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function RefugeeDigitalRightsPage() {
  const [data, setData] = useState<RDData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<RDEntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/refugee-digital-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données de droits numériques…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "biometric_data_coercion_scale_score", label: "Coercition Biométrique" },
    { key: "digital_exclusion_service_denial_severity_score", label: "Exclusion Numérique" },
    { key: "surveillance_control_refugee_population_score", label: "Surveillance & Contrôle" },
    { key: "data_sharing_persecution_risk_gap_score", label: "Risque Partage Données" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Numériques des Réfugiés</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Biométrie forcée, surveillance &amp; exclusion numérique des déplacés</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_refugee_digital_rights_index ?? "—" },
            { label: "Confidence", value: `${Math.round((data.confidence_score ?? 0) * 100)}%` },
          ].map(k => (
            <div key={k.label} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              <div className="text-xs text-slate-500 mb-1">{k.label}</div>
              <div className="text-xl font-bold text-sky-400">{k.value}</div>
            </div>
          ))}
        </div>
        ...
        {(data.critical_alerts?.length ?? 0) > 0 && (
          <div className="border border-sky-500/30 bg-sky-500/10 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-sky-400 mb-2">Alertes Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-500">idx {entity.estimated_refugee_digital_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droits Numériques des Réfugiés · {data.last_analysis}</p>
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Aperçu", "Métriques", "Sources"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`px-6 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-sky-500 text-sky-400" : "text-slate-500 hover:text-slate-300"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-sky-400">{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Droits Numériques Réfugiés</div>
                    <div className="text-lg font-bold text-sky-400">{selected.estimated_refugee_digital_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
                    ...
                    return (
                      <div key={key}>
                        ...
                        <div className="h-2 bg-slate-800 rounded-full">
                          <div className="h-full rounded-full bg-sky-500" style={{ width: `${val}%` }} />
                        </div>
                      </div>
                    );
                  })}
                ...
                <ul className="space-y-2">
                  {(selected.key_signals ?? []).map((s, i) => (
                    <li key={i} className="flex gap-2 text-sm text-slate-300">
                      <span className="text-sky-400 shrink-0 mt-0.5">▸</span>{s}
                    </li>
                  ))}
                </ul>
              )}
