      ...
      <div style={{ background:"#0f172a",border:"1px solid #1e293b",borderRadius:12,padding:24,maxWidth:600,width:"90%",maxHeight:"80vh",overflowY:"auto" }}
        ...
        {tab==="metriques" && (
          <div>
            {entity && Object.entries(entity)
              ...
              .map(([k,v]) => (
                <div key={k} style={{ display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #1e293b",color:"#cbd5e1",fontSize:13 }}>
        ...
        {tab==="sources" && (
          <div style={{ color:"#94a3b8",fontSize:13 }}>
            <p>Pattern : {entity?.primary_pattern}</p>
            <p>Pays/Région : {entity?.country}</p>
          </div>
        )}
      </div>
    </div>
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/climate-refugee-rights-engine")
      .then(r => r.json())
  ...
  return (
    <div style={{ background:"#020617",minHeight:"100vh",color:"#f1f5f9",fontFamily:"system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth:1200,margin:"0 auto",padding:"32px 24px" }}>
        <h1 style={{ fontSize:28,fontWeight:700,color:ACCENT,marginBottom:8 }}>Déplacés Climatiques, Îles Submergées &amp; Droits Émergents</h1>
        <p style={{ color:"#94a3b8",marginBottom:32 }}>CaelumSwarm™ · CSDDD 2024/1760 · Wave 223</p>
        <div style={{ display:"flex",gap:16,marginBottom:32,flexWrap:"wrap" }}>
          ...
          <div style={{ background:"#0f172a",borderRadius:8,padding:20,flex:"1 1 160px",display:"flex",flexDirection:"column",justifyContent:"center",gap:12 }}>
            <div style={{ display:"flex",justifyContent:"space-between" }}>
              <span style={{ color:"#94a3b8" }}>Entités</span>
              <strong style={{ color:ACCENT }}>{data.total_entities}</strong>
            </div>
            <div style={{ display:"flex",justifyContent:"space-between" }}>
              <span style={{ color:"#94a3b8" }}>Critiques</span>
              <strong style={{ color:"#ef4444" }}>{data.risk_distribution?.critique ?? 0}</strong>
            </div>
          </div>
        </div>
