...
interface Entity {
  ...
  colonial_genocide_crime_denial_severity_score: number;
  cultural_patrimony_restitution_refusal_scale_score: number;
  reparations_financial_compensation_denial_score: number;
  indigenous_colonial_trauma_acknowledgment_deficit_gap_score: number;
  estimated_colonial_reparations_memory_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_colonial_reparations_memory_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déni Génocides & Crimes Coloniaux", value: entity.colonial_genocide_crime_denial_severity_score, weight: "0.30" },
    { label: "Refus Restitution Patrimoine", value: entity.cultural_patrimony_restitution_refusal_scale_score, weight: "0.25" },
    { label: "Déni Réparations Financières", value: entity.reparations_financial_compensation_denial_score, weight: "0.25" },
    { label: "Déficit Reconnaissance Trauma Colonial", value: entity.indigenous_colonial_trauma_acknowledgment_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_colonial_reparations_memory_rights_index === "number" ? entity.estimated_colonial_reparations_memory_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Réparations Coloniales</div>
                </div>
              </div>
...
export default function ColonialReparationsMemoryRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/colonial-reparations-memory-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Réparations Coloniales & Droits à la Mémoire…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_colonial_reparations_memory_rights_index ?? avg(allEntities.map(e => e.estimated_colonial_reparations_memory_rights_index));
  ...
  const avgGenociDenial = avg(allEntities.map(e => e.colonial_genocide_crime_denial_severity_score));
  const avgPatrimony = avg(allEntities.map(e => e.cultural_patrimony_restitution_refusal_scale_score));
  const avgReparations = avg(allEntities.map(e => e.reparations_financial_compensation_denial_score));
  const avgTrauma = avg(allEntities.map(e => e.indigenous_colonial_trauma_acknowledgment_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Réparations Coloniales", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Réparations Coloniales & Droits à la Mémoire</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Colonial Reparations & Memory Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgGenociDenial} label="Déni Génocides" />
          <GaugeRing value={avgPatrimony} label="Restitution Patrimoine" />
          <GaugeRing value={avgReparations} label="Réparations Financières" />
          <GaugeRing value={avgTrauma} label="Trauma Colonial" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Réparations Coloniales: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_colonial_reparations_memory_rights_index === "number" ? e.estimated_colonial_reparations_memory_rights_index.toFixed(2) : "—"}</span>
            </div>
