...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque publicitaire wealthtech : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_index.toFixed(2)} / 10`,
    `Conformité campagne wealthtech : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 385 — Wealthtech Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Wealthtech Advertising · Wave 385</div>
          </div>
        ...
        {tab === "signaux" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              ...
              `Indice Caelum Wealthtech Advertising : ${entity.estimated_index.toFixed(2)} / 10`,
              `Campagne wealthtech en ${entity.risk_level === "faible" ? "zone verte — performance nominale" : "zone d&apos;alerte — optimisation requise"}`,
            ].map((sig, i) => (
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(21,128,61,0.15)" }}>
              ...
              <div>{entity.risk_level === "critique" ? "Audit immédiat des campagnes wealthtech — conformité réglementaire urgente" : entity.risk_level === "élevé" ? "Révision des stratégies publicitaires — renforcement compliance" : entity.risk_level === "modéré" ? "Optimisation continue des métriques publicitaires wealthtech" : "Maintien des bonnes pratiques — veille sectorielle"}</div>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(21,128,61,0.15)" }}>
...
export default function WealthtechAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/wealthtech-advertising")
      .then((r) => r.json())
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Wealthtech Advertising…
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Wealthtech Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#15803d99", margin: 0 }}>
          Caelum Partners · Wave 385 · Analyse publicitaire wealthtech · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Wealthtech Advertising Intelligence · Wave 385
      </div>
