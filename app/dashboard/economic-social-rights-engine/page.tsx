...
interface Entity {
  ...
  estimated_economic_social_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_economic_social_rights_index?: number;
...
export default function EconomicSocialRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/economic-social-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT_DISPLAY }}>Initialisation Droits Économiques &amp; Sociaux…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_economic_social_rights_index ?? avg(allEntities.map(e => e.estimated_economic_social_rights_index));
  ...
  const kpis = [
    ...
    { label: "Index Droits Sociaux", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT_DISPLAY }}>{typeof selected.estimated_economic_social_rights_index === "number" ? selected.estimated_economic_social_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Sociaux</div>
                </div>
              </div>
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Économiques &amp; Sociaux</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Economic Social Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <GaugeRing value={avgIndex * 10} label="Index Droits Sociaux" />
          <GaugeRing value={countCritique * 12.5} label="Taux Critique" />
          <GaugeRing value={typeof confidence === "string" ? parseFloat(confidence) || 87 : 87} label="Confiance" />
        </div>
      </div>
      ...
      {(data?.critical_alerts ?? []).length > 0 && (
        <div className="bg-sky-950/30 border border-sky-500/30 rounded-xl p-4">
          <h3 className="text-sky-400 text-xs uppercase tracking-wide mb-2 font-semibold">Alertes Critiques</h3>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Sociaux: <span className="font-medium" style={{ color: ACCENT_DISPLAY }}>{typeof e.estimated_economic_social_rights_index === "number" ? e.estimated_economic_social_rights_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
