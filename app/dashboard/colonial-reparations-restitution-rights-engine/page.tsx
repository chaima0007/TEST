...
export default function ColonialReparationsRestitutionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/colonial-reparations-restitution-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); }).catch(() => setLoading(false));
  }, []);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Réparations Coloniales &amp; Restitution</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Héritage Colonial · Biens Pillés · CARICOM · Réparations Structurelles</p>
      </div>
