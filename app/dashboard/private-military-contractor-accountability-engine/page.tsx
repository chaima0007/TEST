...
interface Entity {
  ...
  estimated_private_military_contractor_accountability_index: number;
}
...
interface Payload {
  ...
  avg_estimated_private_military_contractor_accountability_index: number;
...
export default function PrivateMilitaryContractorAccountabilityPage() {
  const [data, setData] = useState<Payload | null>(null);
  const [error, setError] = useState(false);

  useEffect(() => {
    fetch("/api/private-military-contractor-accountability-engine")
      .then((r) => r.json())
    ...
    <main className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="rounded-2xl p-6 text-white" style={{ backgroundColor: ACCENT }}>
          <h1 className="text-2xl font-bold">Responsabilité Contractants Militaires Privés</h1>
          <p className="text-sm mt-1 opacity-80">Groupe Wagner · Document de Montreux · CICR · Mercenaires ONU</p>
          <div className="mt-4 flex gap-8 text-sm">
            ...
            <div>
              <div className="opacity-60">Index PMC</div>
              <div className="text-xl font-semibold">{data.avg_estimated_private_military_contractor_accountability_index}</div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data.entities.map((e, i) => (
              <div key={i} className="bg-white rounded-xl p-4 shadow-sm flex items-center gap-4">
                <GaugeRing
                  value={e.estimated_private_military_contractor_accountability_index}
