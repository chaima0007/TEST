...
interface Entity {
  ...
  wage_theft_labor_exploitation_severity_score: number;
  legal_protection_access_rights_deficit_score: number;
  recruitment_trafficking_abuse_scale_score: number;
  housing_dependency_freedom_restriction_score: number;
  estimated_migrant_domestic_worker_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_migrant_domestic_worker_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Sévérité Vol de Salaire & Exploitation Travail", value: entity.wage_theft_labor_exploitation_severity_score, weight: "0.30" },
    { label: "Déficit Protection Légale & Accès aux Droits", value: entity.legal_protection_access_rights_deficit_score, weight: "0.25" },
    { label: "Échelle Recrutement Abusif & Traite", value: entity.recruitment_trafficking_abuse_scale_score, weight: "0.25" },
    { label: "Restriction Liberté par Dépendance Logement", value: entity.housing_dependency_freedom_restriction_score, weight: "0.20" },
  ];
  return (
        ...
        <div className="p-6">
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_migrant_domestic_worker_rights_index === "number" ? entity.estimated_migrant_domestic_worker_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Travailleurs Domestiques</div>
                </div>
              </div>
          ...
          {tab === "methodologie" && (
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                <div className="text-xs text-slate-400 mb-2">Formule de Calcul</div>
                <div className="text-xs text-slate-300 font-mono">
                  Score = VolSalaire×0.30 + ProtectionLégale×0.25 + Recrutement×0.25 + Logement×0.20
                </div>
              </div>
...
export default function MigrantDomesticWorkerRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/migrant-domestic-worker-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits des Travailleurs Domestiques Migrants…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_migrant_domestic_worker_rights_index ?? avg(allEntities.map(e => e.estimated_migrant_domestic_worker_rights_index));
  ...
  const avgWage = avg(allEntities.map(e => e.wage_theft_labor_exploitation_severity_score));
  const avgLegal = avg(allEntities.map(e => e.legal_protection_access_rights_deficit_score));
  const avgRecruitment = avg(allEntities.map(e => e.recruitment_trafficking_abuse_scale_score));
  const avgHousing = avg(allEntities.map(e => e.housing_dependency_freedom_restriction_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Travailleurs Domestiques", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits des Travailleurs Domestiques Migrants</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Migrant Domestic Worker Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgWage} label="Vol de Salaire" />
          <GaugeRing value={avgLegal} label="Protection Légale" />
          <GaugeRing value={avgRecruitment} label="Recrutement Abusif" />
          <GaugeRing value={avgHousing} label="Dépendance Logement" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Travailleurs Domestiques: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_migrant_domestic_worker_rights_index === "number" ? e.estimated_migrant_domestic_worker_rights_index.toFixed(2) : "—"}</span>
            </div>
