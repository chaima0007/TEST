...
interface Entity {
  ...
  internet_shutdown_content_blocking_severity_score: number;
  social_media_political_censorship_scale_score: number;
  algorithmic_manipulation_disinformation_state_score: number;
  platform_transparency_accountability_deficit_gap_score: number;
  estimated_online_censorship_platform_governance_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_online_censorship_platform_governance_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Coupures Internet & Blocage Contenu", value: entity.internet_shutdown_content_blocking_severity_score, weight: "0.30" },
    { label: "Censure Politique Réseaux Sociaux", value: entity.social_media_political_censorship_scale_score, weight: "0.25" },
    { label: "Manipulation Algorithmique & Désinformation État", value: entity.algorithmic_manipulation_disinformation_state_score, weight: "0.25" },
    { label: "Déficit Transparence & Responsabilité Plateformes", value: entity.platform_transparency_accountability_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_online_censorship_platform_governance_index === "number" ? entity.estimated_online_censorship_platform_governance_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Censure En Ligne</div>
                </div>
              </div>
...
export default function OnlineCensorshipPlatformGovernanceEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/online-censorship-platform-governance-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Censure En Ligne & Gouvernance des Plateformes…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_online_censorship_platform_governance_index ?? avg(allEntities.map(e => e.estimated_online_censorship_platform_governance_index));
  ...
  const avgShutdown = avg(allEntities.map(e => e.internet_shutdown_content_blocking_severity_score));
  const avgSocialMedia = avg(allEntities.map(e => e.social_media_political_censorship_scale_score));
  const avgAlgorithmic = avg(allEntities.map(e => e.algorithmic_manipulation_disinformation_state_score));
  const avgTransparency = avg(allEntities.map(e => e.platform_transparency_accountability_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Censure En Ligne", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Censure En Ligne & Gouvernance des Plateformes</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Online Censorship Platform Governance Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <GaugeRing value={avgSocialMedia} label="Censure Réseaux Sociaux" />
          <GaugeRing value={avgAlgorithmic} label="Manipulation Algo." />
          <GaugeRing value={avgTransparency} label="Transparence Plateformes" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Censure En Ligne: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_online_censorship_platform_governance_index === "number" ? e.estimated_online_censorship_platform_governance_index.toFixed(2) : "—"}</span>
            </div>
