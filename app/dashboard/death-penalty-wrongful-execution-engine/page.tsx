...
export default function DeathPenaltyWrongfulExecutionPage() {
  const [data, setData] = useState<DomainData | null>(null)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    fetch("/api/death-penalty-wrongful-execution-engine")
      .then((r) => r.json())
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "#f1f5f9", padding: "2rem" }}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.5rem" }}>
          ...
          <span style={{ fontSize: 12, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 1 }}>Wave 186 · Droits Humains</span>
        </div>
        <h1 style={{ fontSize: "1.75rem", fontWeight: 700, color: "#f1f5f9", margin: 0 }}>
          Peine de Mort & Exécutions Erronées
        </h1>
        <p style={{ color: "#64748b", marginTop: "0.25rem", fontSize: 14 }}>
          Analyse mondiale des pratiques d&apos;exécution, condamnations à mort d&apos;innocents et abolition de la peine capitale
        </p>
      </div>

      {/* Stats */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "1rem", marginBottom: "2rem" }}>
        <div style={{ background: "#1e293b", borderRadius: 12, padding: "1.25rem", borderLeft: "4px solid #6b7280" }}>
          <p style={{ fontSize: 12, color: "#94a3b8", margin: 0, textTransform: "uppercase", letterSpacing: 0.5 }}>Score Moyen</p>
          <p style={{ fontSize: "2rem", fontWeight: 700, color: "#9ca3af", margin: "0.25rem 0 0" }}>{data.avg_composite.toFixed(1)}</p>
        </div>
          ...
          <div key={entity.id} style={{ background: "#1e293b", borderRadius: 12, padding: "1.25rem", display: "flex", alignItems: "center", gap: "1.25rem", border: "1px solid #334155" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ margin: "0.3rem 0 0", fontSize: 12, color: "#64748b" }}>
                Index peine de mort : <span style={{ color: "#9ca3af", fontWeight: 600 }}>{entity.estimated_death_penalty_index.toFixed(2)}</span> / 10
              </p>
            </div>
          </div>
        ))}
      ...
      <div style={{ marginTop: "2rem", paddingTop: "1rem", borderTop: "1px solid #1e293b", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 12, color: "#334155" }}>Wave 186 · Caelum Partners Swarm Intelligence</span>
