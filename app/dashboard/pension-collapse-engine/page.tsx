...
// Pension Collapse & Social Security System Failure — Caelum Partners
...
interface PCSummary {
  ...
  data_sources: string[];
  entities: PCEntity[];
  avg_estimated_pension_index: number;
}
...
const PAT_COLORS: Record<string, string> = {
  pension_insolvency_crisis:     "#ef4444",
  demographic_collapse:          "#f97316",
  investment_portfolio_meltdown: "#a855f7",
  reform_paralysis:              "#3b82f6",
  generational_burden_spiral:    "#f59e0b",
  none:                          "#10b981",
};
      ...
      <div className="bg-slate-900 border border-amber-700/30 rounded-xl w-full max-w-lg p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <span className="ml-2 text-amber-400 text-xs">{entity.country}</span>
            <span className="ml-2 text-stone-400 text-xs">{entity.sector}</span>
          </div>
        ...
        <div className="flex gap-2 mb-4">
          {(["scores", "signaux", "actions"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-3 py-1 rounded text-xs font-medium transition-colors ${tab === t ? "bg-amber-800 text-white" : "bg-slate-800 text-stone-400 hover:text-white"}`}>
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {([
              ["Déficit Financement (×0.30)", entity.score1, "#f59e0b"],
              ["Pression Démographique (×0.25)", entity.score2, "#ef4444"],
              ["Risque Investissement (×0.25)",  entity.score3, "#a855f7"],
              ["Gap Réforme Politique (×0.20)",  entity.score4, "#3b82f6"],
            ] as [string, number, string][]).map(([l, v, c]) => (
              <div key={l} className="bg-slate-800 border border-amber-700/20 rounded-lg p-3">
                <div className="text-stone-400 text-xs mb-1">{l}</div>
            ...
            <div className="col-span-2 bg-slate-800 border border-amber-700/20 rounded-lg p-3">
        ...
        {tab === "signaux" && (
          <div className="bg-slate-800 border border-amber-700/20 rounded-lg p-4 text-sm text-slate-200 leading-relaxed">
            {entity.key_signals}
            <div className="mt-3 flex gap-2 flex-wrap">
              <span className={`px-2 py-0.5 rounded text-xs font-medium ${meta?.badge || "bg-slate-700 text-stone-300"}`}>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-stone-400">
              <div>Index Retraite: <span className="text-amber-300">{entity.estimated_pension_index.toFixed(2)}/10</span></div>
              <div>Dernière Analyse: <span className="text-orange-300">{entity.last_updated}</span></div>
            </div>
          </div>
        )}
        ...
        {tab === "actions" && (
          <div className="space-y-3 text-sm">
            <div className="bg-slate-800 border border-amber-700/20 rounded-lg p-3">
              <div className="text-stone-400 text-xs mb-1">Niveau de Risque</div>
              <div className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${meta?.badge || "bg-slate-700 text-stone-300"}`}>
                {meta?.label || entity.risk_level}
              </div>
            </div>
            <div className="bg-slate-800 border border-amber-700/20 rounded-lg p-3">
              <div className="text-stone-400 text-xs mb-1">Patron Primaire Détecté</div>
              <div className="text-white font-medium capitalize">{entity.primary_pattern.replace(/_/g, " ")}</div>
            </div>
            <div className="bg-slate-800 border border-amber-700/20 rounded-lg p-3">
              <div className="text-stone-400 text-xs mb-1">Index Crise Retraite</div>
              <div className="font-bold text-xl" style={{ color: ACCENT }}>{entity.estimated_pension_index.toFixed(2)} <span className="text-xs text-stone-400">/ 10</span></div>
            </div>
          </div>
        )}
      </div>
...
export default function PensionCollapseDashboard() {
  const [data, setData] = useState<PCSummary | null>(null);
  const [riskFilter, setRisk] = useState<string>("tous");
  const [selected, setSelected] = useState<PCEntity | null>(null);
  ...
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="animate-pulse" style={{ color: ACCENT }}>Initialisation Moteur Effondrement Retraites — Caelum Partners...</div>
    </div>
  );
  ...
  const dists = [
    ...
    { title: "Patron Crise Retraites", counts: data.pattern_distribution, colors: PAT_COLORS },
    ...
    <div className="min-h-screen bg-slate-950 text-stone-300 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>
          Effondrement Retraites &amp; Sécurité Sociale
        </h1>
        <p className="text-stone-400 text-sm mt-1">
          Déficit Financement · Pression Démographique · Risque Investissement · Réforme Politique — Caelum Partners
        </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {([
          ["Total Systèmes",      data.total_entities,                                 "text-stone-300"],
          ["Critique",            critCount,                                            "text-red-400"],
          ["Élevé",               elevCount,                                            "text-orange-400"],
          ["Composite Moyen",     data.avg_composite.toFixed(1),                       "text-amber-300"],
          ["Index Retraite Moy.", data.avg_estimated_pension_index.toFixed(2) + "/10", "text-orange-300"],
          ["Confiance Moteur",    (data.confidence_score * 100).toFixed(0) + "%",      "text-amber-400"],
        ] as [string, string | number, string][]).map(([l, v, c]) => (
          <div key={String(l)} className="bg-slate-900 border border-amber-800/30 rounded-xl p-3 text-center">
      ...
      <div className="bg-slate-900 border border-amber-800/30 rounded-xl p-5">
        ...
        <div className="grid grid-cols-4 gap-4">
          <GaugeRing value={avgS1} label="Déficit Financement (×0.30)"   color="#f59e0b" />
          <GaugeRing value={avgS2} label="Pression Démogr. (×0.25)"      color="#ef4444" />
          <GaugeRing value={avgS3} label="Risque Investis. (×0.25)"      color="#a855f7" />
          <GaugeRing value={avgS4} label="Gap Réforme Pol. (×0.20)"      color="#3b82f6" />
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-amber-800/30 rounded-xl p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="flex flex-col gap-1">
          <span className="text-xs text-stone-400 font-medium">Sources de Données</span>
          {data.data_sources.map(s => (
            <span key={s} className="text-xs text-stone-500">• {s}</span>
          ))}
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-xs text-stone-400 font-medium">Alertes Critiques</span>
          ...
          <button key={r} onClick={() => setRisk(r)}
            className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
              riskFilter === r
                ...
                : "bg-slate-900 border-amber-800/30 text-stone-400 hover:text-white"
            }`}>
        ...
        {filtered.map(e => {
          ...
          return (
            <div key={e.entity_id} onClick={() => setSelected(e)}
              className="bg-slate-900 border border-slate-800 rounded-xl p-4 cursor-pointer hover:border-amber-700/50 transition-colors">
              <div className="flex items-center justify-between mb-1">
                <span className="font-bold text-white">{e.name}</span>
                <span className="text-xs text-stone-400">{e.country}</span>
              </div>
              <div className="text-xs text-amber-400 mb-2">{e.sector}</div>
              ...
              <div className="text-xs text-stone-500 mb-2 capitalize">{e.primary_pattern.replace(/_/g, " ")}</div>
              <div className="text-xs text-amber-400 truncate">{e.key_signals}</div>
            </div>
          );
        })}
      </div>
