...
const SUB_SCORES = [
  { key: "traceability_score", label: "Traçabilité" },
  { key: "due_diligence_score", label: "Diligence raisonnable" },
  { key: "community_impact_score", label: "Impact communautaire" },
  { key: "supply_chain_score", label: "Chaîne d&apos;approvisionnement" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits sur les minérais de conflit : traçabilité, diligence raisonnable, impact communautaire et chaîne d&apos;approvisionnement affectant les populations des zones d&apos;extraction.</p>
              <p className="text-slate-400">Source : Conflict Mineral Rights Engine · Wave 266</p>
            </div>
          )}
        </div>
      </div>
...
export default function ConflictMineralRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/conflict-mineral-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits sur les Minérais de Conflit</h1>
          <p className="text-slate-400 mt-1 text-sm">Traçabilité · Diligence Raisonnable · Impact Communautaire · Chaîne d&apos;Approvisionnement</p>
        </div>
