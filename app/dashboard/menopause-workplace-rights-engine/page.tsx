...
interface MWEntity {
  ...
  workplace_symptom_accommodation_denial_score: number;
  age_gender_intersectional_discrimination_scale_score: number;
  medical_recognition_menopause_disability_gap_score: number;
  career_penalty_forced_exit_pattern_score: number;
  estimated_menopause_workplace_rights_index: number;
  risk_level: string;
...
interface MWData {
  ...
  data_sources: string[];
  entities: MWEntity[];
  avg_estimated_menopause_workplace_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function MenopauseWorkplaceRightsPage() {
  const [data, setData] = useState<MWData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<MWEntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/menopause-workplace-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données ménopause &amp; travail…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "workplace_symptom_accommodation_denial_score", label: "Refus Aménagement" },
    { key: "age_gender_intersectional_discrimination_scale_score", label: "Discrimination Intersect." },
    { key: "medical_recognition_menopause_disability_gap_score", label: "Reconnaissance Médicale" },
    { key: "career_penalty_forced_exit_pattern_score", label: "Pénalité Carrière" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Ménopause &amp; Droits au Travail</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Discrimination intersectionnelle âge-genre en milieu professionnel</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_menopause_workplace_rights_index ?? "—" },
            ...
            <div key={k.label} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              <div className="text-xl font-bold text-pink-400">{k.value}</div>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#f472b6" />
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            ...
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#f472b6", "#f97316", "#eab308", "#ef4444", "#10b981"];
        ...
        {(data.critical_alerts?.length ?? 0) > 0 && (
          <div className="border border-pink-500/30 bg-pink-500/10 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-pink-400 mb-2">Alertes Critiques</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              className={`bg-slate-900 border rounded-xl p-4 cursor-pointer hover:border-pink-500/50 transition-all ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#f472b6" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_menopause_workplace_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Ménopause &amp; Droits au Travail · {data.last_analysis}</p>
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Aperçu", "Métriques", "Sources"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`px-6 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-pink-500 text-pink-400" : "text-slate-500 hover:text-slate-300"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#f472b6" />
                    <div>
                      <div className="text-2xl font-bold text-pink-400">{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Ménopause &amp; Travail</div>
                    <div className="text-lg font-bold text-pink-400">{selected.estimated_menopause_workplace_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
                    ...
                    return (
                      <div key={key}>
                        ...
                        <div className="h-2 bg-slate-800 rounded-full">
                          <div className="h-full rounded-full bg-pink-500" style={{ width: `${val}%` }} />
                        </div>
                      </div>
                    );
                  })}
                ...
                <ul className="space-y-2">
                  {(selected.key_signals ?? []).map((s, i) => (
                    <li key={i} className="flex gap-2 text-sm text-slate-300">
                      <span className="text-pink-400 shrink-0 mt-0.5">▸</span>{s}
                    </li>
                  ))}
                </ul>
              )}
