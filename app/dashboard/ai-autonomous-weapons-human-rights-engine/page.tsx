...
interface Entity {
  ...
  estimated_ai_weapons_index: number;
  risk_level: string;
  [key: string]: unknown;
}
...
export default function AIAutonomousWeaponsHumanRightsDashboard() {
  const [data, setData] = useState<DashData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");

  useEffect(() => {
    fetch("/api/ai-autonomous-weapons-human-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation IA &amp; Armes Autonomes…</div>
      </div>
    );
  }
  ...
  const avgIndex = avg(allEntities.map(e => e.estimated_ai_weapons_index));
  ...
  const kpis = [
    ...
    { label: "Index IA Armes", value: avgIndex.toFixed(2) },
    { label: "Critique", value: countCritique },
    { label: "Élevé", value: countEleve },
    { label: "Confiance", value: typeof data?.confidence_score === "number" ? `${(data.confidence_score * 100).toFixed(0)}%` : "—" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">IA &amp; Armes Autonomes — Droits Humains</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          AI &amp; Autonomous Weapons Human Rights Engine — Systèmes létaux autonomes et impacts sur les droits humains — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <h2 className="text-sm font-semibold text-slate-400 mb-4">Distribution des Scores Composites</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgComposite} label="Score Moyen Global" />
          <GaugeRing value={allEntities.filter(e => e.risk_level === "critique").length > 0 ? avg(allEntities.filter(e => e.risk_level === "critique").map(e => e.composite_score)) : 0} label="Moy. Critique" />
          <GaugeRing value={allEntities.filter(e => e.risk_level === "élevé").length > 0 ? avg(allEntities.filter(e => e.risk_level === "élevé").map(e => e.composite_score)) : 0} label="Moy. Élevé" />
          <GaugeRing value={avgIndex * 10} label="Index IA Armes ×10" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map(e => (
          <div key={e.id}
            className={`border rounded-xl p-4 transition-transform hover:scale-[1.01] ${RB[e.risk_level] ?? "border-slate-700 bg-slate-900"}`}>
            <div className="flex justify-between items-start mb-2">
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-sm leading-tight">{e.name}</div>
                {e.country && <div className="text-xs text-slate-400 mt-1">{e.country}</div>}
              </div>
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index IA Armes: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_ai_weapons_index === "number" ? e.estimated_ai_weapons_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
