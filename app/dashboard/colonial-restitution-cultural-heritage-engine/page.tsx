...
interface Entity {
  ...
  looted_objects_retention_score: number
  restitution_refusal_delay_score: number
  colonial_accountability_deficit_score: number
  indigenous_community_rights_score: number
  estimated_restitution_index: number
  risk_level: string
...
interface DomainData {
  ...
  avg_estimated_restitution_index?: number
  data_sources?: string[]
}
    ...
    <div onClick={onClick} style={{ background: "#0f172a", border: `1px solid ${RC[entity.risk_level]}33`, borderRadius: 12, padding: 16, cursor: "pointer" }}>
      ...
      <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
        <div style={{ background: "#1e293b", borderRadius: 6, padding: "4px 8px", fontSize: 11, color: "#94a3b8" }}>
          Index: <span style={{ color: ACCENT, fontWeight: 700 }}>{entity.estimated_restitution_index.toFixed(2)}</span>
        </div>
        <div style={{ background: "#1e293b", borderRadius: 6, padding: "4px 8px", fontSize: 11, color: "#94a3b8" }}>
...
function Modal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Rétention Objets Pillés", value: entity.looted_objects_retention_score, weight: "0.30" },
    { label: "Refus/Délai Restitution", value: entity.restitution_refusal_delay_score, weight: "0.25" },
    { label: "Déficit Responsabilité Coloniale", value: entity.colonial_accountability_deficit_score, weight: "0.25" },
    { label: "Droits Communautés Autochtones", value: entity.indigenous_community_rights_score, weight: "0.20" },
  ]
  return (
          ...
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div style={{ background: "#1e293b", borderRadius: 12, padding: 16, textAlign: "center" }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: ACCENT }}>{entity.estimated_restitution_index.toFixed(2)}</div>
                  <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>Index Restitution</div>
                </div>
              </div>
...
export default function ColonialRestitutionCulturalHeritagePage() {
  ...
  useEffect(() => {
    fetch("/api/colonial-restitution-cultural-heritage-engine")
      .then(r => r.json())
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 16px" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
            ...
            <div>
              <h1 style={{ fontSize: 28, fontWeight: 800, color: "#f1f5f9", margin: 0 }}>Restitution Coloniale & Patrimoine Culturel</h1>
              <p style={{ fontSize: 14, color: "#64748b", margin: 0, marginTop: 4 }}>Wave 184 — Suivi des restitutions d&apos;objets coloniaux et droits des communautés autochtones</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ borderTop: "1px solid #1e293b", paddingTop: 16, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          <span style={{ fontSize: 12, color: "#475569" }}>Caelum Partners — Wave 184 — Restitution Coloniale & Patrimoine Culturel</span>
