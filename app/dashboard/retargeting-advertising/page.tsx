...
type Entity = {
  ...
  risk_level: string;
  estimated_retargeting_index: number;
};
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque retargeting advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_retargeting_index.toFixed(2)} / 10`,
    `Conformité RGPD retargeting : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 404 — Retargeting Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Retargeting Advertising · Wave 404</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: entity.estimated_retargeting_index * 10, color: COLOR },
        ...
        {tab === "signaux" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              `Score composite ${entity.composite_score.toFixed(1)} — ${entity.risk_level === "critique" ? "surveillance retargeting urgente" : entity.risk_level === "élevé" ? "monitoring renforcé recommandé" : "suivi standard actif"}`,
              `Indice Caelum Retargeting Advertising : ${entity.estimated_retargeting_index.toFixed(2)} / 10`,
              `Analyse conformité CSDDD pour la chaîne d&apos;approvisionnement retargeting. Risques liés aux conditions de travail dans les agences de traitement de données et les centres de données.`,
            ].map((sig, i) => (
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(239,68,68,0.15)" }}>
              ...
              <div>{entity.risk_level === "critique" ? "Auditer agences données partenaires" : entity.risk_level === "élevé" ? "Vérifier conformité RGPD retargeting" : entity.risk_level === "modéré" ? "Mettre en place code conduite données audience" : "Certifier ISO 26000"}</div>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(239,68,68,0.15)" }}>
              <div style={{ fontSize: 11, color: "#93c5fd", marginBottom: 4 }}>Index Caelum</div>
              <div style={{ color: COLOR, fontWeight: 700, fontSize: 18 }}>{entity.estimated_retargeting_index.toFixed(2)} / 10</div>
            </div>
          </div>
        )}
      </div>
...
export default function RetargetingAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/retargeting-advertising")
      .then((r) => r.json())
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Retargeting Advertising…
      </div>
    );
  }
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + e.estimated_retargeting_index, 0) / entities.length
    : 0;
    ...
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Retargeting Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#ef444499", margin: 0 }}>
          Caelum Partners · Wave 404 · Analyse conformité retargeting · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
          {data.mode === "fallback" && <span style={{ marginLeft: 8, color: "#ef4444", fontSize: 11 }}>[mode dégradé]</span>}
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 32 }}>
        {[
          ...
          { label: "Composite Moyen", value: avgComposite.toFixed(1), color: "#ef4444" },
        ...
        {filtered.map((entity, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
              ...
              <span style={{ color: COLOR, fontWeight: 600 }}>{entity.estimated_retargeting_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Retargeting Advertising Intelligence · Wave 404
      </div>
