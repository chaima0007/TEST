...
export default function ContentModerationTraumaDashboard() {
  const [data, setData] = useState<{ entities?: { entity: string; composite: number; level: string }[]; avg_composite?: number } | null>(null);
  useEffect(() => { fetch("/api/contentmoderationtrauma").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-2">Content Moderation Worker Trauma Rights</h1>
      <p className="text-gray-500 text-sm mb-6">Protection des travailleurs exposés à des contenus traumatisants pour modérer les plateformes</p>
