...
type Entity = {
  ...
  risk_level: string;
  estimated_ugcadvertising_index: number;
};
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque UGC advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_ugcadvertising_index.toFixed(2)} / 10`,
    `Conformité plateforme UGC : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 420 — UGC Advertising Intelligence`,
  ];
}

const SIGNALS = [
  "Creator consent management",
  "UGC rights clearance",
  "User data in ads",
  "Brand safety in UGC",
...
const ACTIONS = [
  "Enforce creator consent",
  "Audit UGC rights",
  "Protect user data in ads",
  "Deploy UGC brand safety",
  "Disclose creator compensation",
];
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>UGC Advertising · Wave 420</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: entity.estimated_ugcadvertising_index * 10, color: COLOR },
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            {ACTIONS.map((action, i) => (
              <div key={i} style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(5,150,105,0.15)" }}>
...
export default function UgcadvertisingAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/ugcadvertising-advertising")
      .then((r) => r.json())
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + e.estimated_ugcadvertising_index, 0) / entities.length
    : 0;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            UGC Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#05966999", margin: 0 }}>
          Caelum Partners · Wave 420 · Analyse publicitaire UGC · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
        ...
        {filtered.map((entity, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
              ...
              <span style={{ color: COLOR, fontWeight: 600 }}>{entity.estimated_ugcadvertising_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · UGC Advertising Intelligence · Wave 420
      </div>
