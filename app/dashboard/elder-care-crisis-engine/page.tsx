...
type ECEntity = {
  ...
  caregiver_shortage_score: number;
  facility_quality_gap: number;
  affordability_crisis: number;
  social_isolation_index: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_eldercare_index: number;
  last_updated: string;
};
...
type ECSummary = {
  ...
  data_sources: string[];
  entities: ECEntity[];
  avg_estimated_eldercare_index: number;
};
...
const PAT_COLORS: Record<string, string> = {
  "Soins Séniors Satisfaisants": "#10b981",
  "Crise Soins Personnes Âgées": "#f59e0b",
  "Épidémie d'Isolement Sénior": "#a855f7",
  "Inaccessibilité Financière Soins": "#f97316",
  "Maltraitance Institutionnelle": "#06b6d4",
  "Désert de Soins Gériatriques": "#ef4444",
};
      ...
      <div className="bg-slate-950 border border-violet-700/30 rounded-xl w-full max-w-lg p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ["Pénurie Soignants", entity.caregiver_shortage_score, "#ef4444"],
              ["Qualité Établissements", entity.facility_quality_gap, "#f97316"],
              ["Crise Accessibilité", entity.affordability_crisis, "#a855f7"],
              ["Isolement Social", entity.social_isolation_index, "#06b6d4"],
            ].map(([l, v, c]) => (
            ...
            <div className="col-span-2 bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-300/60 text-xs mb-1">Score Composite Crise Soins Séniors</div>
        ...
        {tab === "signaux" && (
          <div className="space-y-3">
            <div className="flex gap-2 flex-wrap mb-2">
              ...
              <span className="px-2 py-0.5 rounded text-xs font-medium bg-slate-800 text-violet-300">{entity.primary_pattern}</span>
            </div>
            {entity.key_signals.map((sig, i) => (
              <div key={i} className="bg-slate-900 border border-violet-700/20 rounded-lg p-3 text-sm text-slate-200">{sig}</div>
            ))}
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-300/50 text-xs mb-0.5">Index Crise Soins Estimé</div>
              <div className="text-white text-sm font-medium">{entity.estimated_eldercare_index.toFixed(2)} / 10</div>
            </div>
          </div>
        )}
        ...
        {tab === "actions" && (
          <div className="space-y-3 text-sm">
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-300/60 text-xs mb-1">Pattern Détecté</div>
              <div className="text-white font-medium">{entity.primary_pattern}</div>
            </div>
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-300/60 text-xs mb-1">Secteur</div>
              <div className="text-white font-medium">{entity.sector}</div>
            </div>
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-300/60 text-xs mb-1">Dernière Analyse</div>
              <div className="text-violet-300 font-medium">{entity.last_updated}</div>
            </div>
          </div>
        )}
      </div>
...
export default function ElderCareCrisisDashboard() {
  const [data, setData] = useState<ECSummary | null>(null);
  const [riskFilter, setRiskFilter] = useState<string>("all");
  const [patFilter, setPatFilter] = useState<string>("all");
  const [selected, setSelected] = useState<ECEntity | null>(null);

  useEffect(() => {
    fetch("/api/elder-care-crisis-engine")
      .then(r => r.json())
      .then(setData)
      .catch(console.error);
  }, []);
  ...
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-violet-400 text-lg animate-pulse">Initialisation du Moteur Crise Soins Séniors...</div>
    </div>
  );

  const entities: ECEntity[] = data.entities || [];
  const filtered = entities.filter(e =>
    (riskFilter === "all" || e.risk_level === riskFilter) &&
    (patFilter === "all" || e.primary_pattern === patFilter)
  );

  const avgCaregiver = entities.reduce((s, e) => s + e.caregiver_shortage_score, 0) / (entities.length || 1);
  const avgFacility = entities.reduce((s, e) => s + e.facility_quality_gap, 0) / (entities.length || 1);
  const avgAfford = entities.reduce((s, e) => s + e.affordability_crisis, 0) / (entities.length || 1);
  const avgIsolation = entities.reduce((s, e) => s + e.social_isolation_index, 0) / (entities.length || 1);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-violet-400">Crise Soins Personnes Âgées — Module Intelligence</h1>
        <p className="text-violet-300/50 text-sm mt-1">
          Pénurie Soignants · Qualité Établissements · Accessibilité Financière · Isolement Social
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ["Total Entités", data.total_entities, "text-violet-400"],
          ["Alertes Critiques", data.critical_alerts, "text-red-400"],
          ["Niveau Élevé", (data.risk_distribution?.["élevé"] ?? 0), "text-blue-400"],
          ["Score Composite Moy.", `${data.avg_composite.toFixed(1)}`, "text-violet-300"],
          ["Index Soins Séniors", `${data.avg_estimated_eldercare_index.toFixed(2)}/10`, "text-amber-400"],
      ...
      <div className="bg-slate-900 border border-violet-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          <GaugeRing value={avgCaregiver} label="Pénurie Soignants" color="#ef4444" />
          <GaugeRing value={avgFacility} label="Qualité Établissements" color="#f97316" />
          <GaugeRing value={avgAfford} label="Crise Accessibilité" color="#a855f7" />
          <GaugeRing value={avgIsolation} label="Isolement Social" color="#06b6d4" />
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-violet-700/30 rounded-xl p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <DistBar title="Sources de Données" counts={{ "OCDE": 1, "OMS": 1, "HelpAge": 1 }} colors={{ "OCDE": "#06b6d4", "OMS": "#a855f7", "HelpAge": "#f59e0b" }} />
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {["all", "faible", "modéré", "élevé", "critique"].map(r => (
          <button key={r} onClick={() => setRiskFilter(r)}
            className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${riskFilter === r ? "bg-violet-900 border-violet-700 text-white" : "bg-slate-900 border-violet-700/30 text-violet-400/70 hover:text-white"}`}>
            {r}
          </button>
        ))}
        ...
        {["all", ...patterns].map(p => (
          <button key={p} onClick={() => setPatFilter(p)}
            className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${patFilter === p ? "bg-amber-950 border-amber-700 text-white" : "bg-slate-900 border-violet-700/30 text-violet-400/70 hover:text-white"}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-violet-400/60 mb-2">{e.primary_pattern}</div>
            <div className="text-xs text-violet-400 font-medium mb-1">
              Soignants: {e.caregiver_shortage_score} · Qualité: {e.facility_quality_gap}
            </div>
