...
interface Entity {
  ...
  last_updated: string;
  supply_chain_forced_labor_score?: number;
  corporate_due_diligence_gap_score?: number;
  debt_bondage_score?: number;
  repatriation_failure_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "supply_chain_forced_labor_score", label: "Travail forcé dans les chaînes d&apos;approvisionnement" },
  { key: "corporate_due_diligence_gap_score", label: "Lacune de diligence raisonnable des entreprises" },
  { key: "debt_bondage_score", label: "Servitude pour dettes" },
  { key: "repatriation_failure_score", label: "Échec du rapatriement" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse du travail forcé dans les chaînes d&apos;approvisionnement mondiales, des lacunes en matière de diligence raisonnable des entreprises, de la servitude pour dettes et des échecs de rapatriement des travailleurs migrants.</p>
              <p className="text-slate-400">Source : Forced Labor Supply Chain Rights Engine · Wave 244</p>
            </div>
          )}
        </div>
      </div>
...
export default function ForcedLaborSupplyChainRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/forced-labor-supply-chain-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Forced Labor Supply Chain Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Travail Forcé · Diligence Entreprises · Servitude Dettes · Rapatriement</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#1d4ed820" } : {}}>
