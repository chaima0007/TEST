...
type PAEEntity = {
  ...
  risk_level: string;
  pharma_pattern: string;
  severity: string;
  recommended_action: string;
  signal: string;
  drug_price_index: number;
  counterfeiting_risk: number;
};
...
type PAESummary = {
  ...
  avg_estimated_pharma_access_index: number;
};
    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="flex h-3 rounded overflow-hidden gap-px">
        {Object.entries(counts).map(([k, v]) => (
          <div
            key={k}
            style={{ width: `${(v / total) * 100}%`, background: colors[k] || "#0f766e" }}
            title={`${k}: ${v}`}
          />
        ))}
      </div>
      <div className="flex flex-wrap gap-x-3 gap-y-0.5">
        {Object.entries(counts).map(([k, v]) => (
          <span key={k} className="text-xs text-teal-500/60">
            <span style={{ color: colors[k] || "#0d9488" }}>■</span> {k.replace(/_/g, " ")} {v}
          </span>
        ))}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  none: "#16a34a",
  life_saving_drug_unaffordability: "#dc2626",
  patent_monopoly_abuse: "#7c3aed",
  generic_market_suppression: "#0f766e",
  clinical_trial_exclusion: "#b45309",
  counterfeit_drug_proliferation: "#0891b2",
};
const SEVERITY_COLORS: Record<string, string> = {
  "accès_pharmaceutique_sous_surveillance": "#16a34a",
  "inégalité_accès_médicaments_structurelle": "#0891b2",
  "crise_prix_pharmaceutiques_majeure": "#0d9488",
  "crise_accès_médicaments_systémique": "#dc2626",
};
const ACTION_COLORS: Record<string, string> = {
  "veille_marché_pharmaceutique_continue": "#16a34a",
  "renforcement_politiques_accès_génériques": "#0891b2",
  "négociation_prix_licences_obligatoires_accélérée": "#0d9488",
  "intervention_urgente_accès_médicaments_essentiels": "#dc2626",
};
const RISK_BADGE: Record<string, string> = {
  low: "bg-green-900 text-green-300",
  moderate: "bg-blue-900 text-blue-300",
  high: "bg-teal-900 text-teal-300",
  critical: "bg-red-950 text-red-400",
};
const SEV_BADGE: Record<string, string> = {
  "accès_pharmaceutique_sous_surveillance": "bg-green-900 text-green-300",
  "inégalité_accès_médicaments_structurelle": "bg-blue-900 text-blue-300",
  "crise_prix_pharmaceutiques_majeure": "bg-teal-900 text-teal-300",
  "crise_accès_médicaments_systémique": "bg-red-950 text-red-400",
};

function DetailModal({ entity, onClose }: { entity: PAEEntity; onClose: () => void }) {
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <span className="ml-2 text-slate-500 text-xs">
              {entity.therapeutic_area.replace(/_/g, " ")}
            </span>
          </div>
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ["Score Accessibilité",  entity.affordability_score, "#0891b2"],
              ["Score Monopole",       entity.monopoly_score,      "#7c3aed"],
              ["Score Disponibilité",  entity.availability_score,  "#0d9488"],
              ["Score Innovation",     entity.innovation_score,    "#d97706"],
            ].map(([l, v, c]) => (
              <div
                key={String(l)}
            ...
            <div className="col-span-2 bg-slate-900 border border-teal-700/20 rounded-lg p-3">
              <div className="text-teal-500/60 text-xs mb-1">Score Composite Accès Pharmaceutique</div>
          ...
          <div className="bg-slate-900 border border-teal-700/20 rounded-lg p-4 text-sm text-slate-200 leading-relaxed">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="bg-slate-800 rounded p-2">
                <div className="text-teal-500/50 text-xs mb-0.5">Indice Prix Médicaments</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.drug_price_index * 100)}%
                </div>
              </div>
              <div className="bg-slate-800 rounded p-2">
                <div className="text-teal-500/50 text-xs mb-0.5">Risque Contrefaçon</div>
                <div className="text-white text-sm font-medium">
                  {Math.round(entity.counterfeiting_risk * 100)}%
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-900 border border-teal-700/20 rounded-lg p-3">
              <div className="text-teal-500/60 text-xs mb-1">Aire Thérapeutique</div>
            ...
            <div className="bg-slate-900 border border-teal-700/20 rounded-lg p-3">
              ...
              <div className="text-teal-400 font-medium capitalize">
                {entity.pharma_pattern.replace(/_/g, " ")}
              </div>
            </div>
          </div>
        )}
...
export default function PharmaceuticalAccessDashboard() {
  ...
  const [selected, setSelected] = useState<PAEEntity | null>(null);

  useEffect(() => {
    fetch("/api/pharmaceutical-access-engine")
      .then(r => r.json())
      .then(setData)
      .catch(console.error);
  }, []);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-teal-400 text-lg animate-pulse">
          Initialisation du Moteur Accès Médicaments...
        </div>
      </div>
    );
  }

  const { entities, summary, avg_affordability } = data;
  const filtered = entities.filter(
    e =>
      (riskFilter === "all" || e.risk_level === riskFilter) &&
      (patFilter === "all" || e.pharma_pattern === patFilter)
  );

  const dists = [
    { title: "Niveau de Risque",         counts: summary.risk_distribution,     colors: RISK_COLORS     },
    { title: "Patron Pharmaceutique",    counts: summary.pattern_distribution,  colors: PATTERN_COLORS  },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-teal-400">
          Accès Médicaments &amp; Prix Pharmaceutiques — Module 411
        </h1>
        <p className="text-blue-400/60 text-sm mt-1">
          Accessibilité · Monopole · Disponibilité · Innovation — Caelum Partners — Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ["Total Entités",              summary.total,                                                          "text-teal-400"],
          ["Crise Accès Médicaments",    summary.critical,                                                       "text-red-400"],
          ["Crise Prix Majeure",         summary.high,                                                           "text-teal-600"],
          ["Composite Moyen",            `${summary.avg_composite.toFixed(1)}`,                                  "text-blue-400"],
          ["Index Accès Pharma",         `${summary.avg_estimated_pharma_access_index.toFixed(2)}/10`,           "text-teal-400"],
          ["Accessibilité Moy.",         `${avg_affordability.toFixed(1)}`,                                      "text-blue-400"],
        ].map(([l, v, c]) => (
          <div
            key={String(l)}
      ...
      <div className="bg-slate-900 border border-teal-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          ...
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.availability_score, 0) / (entities.length || 1)}
            label="Disponibilité Moy."
            color="#0d9488"
          />
          <GaugeRing
            value={entities.reduce((s, e) => s + e.innovation_score, 0) / (entities.length || 1)}
            label="Innovation Moy."
            color="#d97706"
          />
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {[
          "all",
          "none",
          "life_saving_drug_unaffordability",
          "patent_monopoly_abuse",
          "generic_market_suppression",
          "clinical_trial_exclusion",
          "counterfeit_drug_proliferation",
        ].map(p => (
          <button
            key={p}
        ...
        {filtered.map(e => (
          ...
          >
            ...
            <div className="text-xs text-slate-500 mb-2 capitalize">
              {e.therapeutic_area.replace(/_/g, " ")}
            </div>
            ...
            <div className="text-xs text-teal-400/60 mb-2 capitalize">
              {e.pharma_pattern.replace(/_/g, " ")}
            </div>
            <div className="text-xs text-blue-400/70 font-medium mb-2">
              Accès: {e.affordability_score.toFixed(1)} · Monopole: {e.monopoly_score.toFixed(1)}
            </div>
