...
interface Entity {
  ...
  last_updated: string;
  social_safety_net_gap_score?: number;
  unemployment_benefit_denial_score?: number;
  pension_rights_violation_score?: number;
  healthcare_social_protection_gap_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "social_safety_net_gap_score", label: "Lacunes du filet de sécurité sociale" },
  { key: "unemployment_benefit_denial_score", label: "Refus d&apos;allocations chômage" },
  { key: "pension_rights_violation_score", label: "Violations des droits à la retraite" },
  { key: "healthcare_social_protection_gap_score", label: "Déficits de protection santé" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits à la protection sociale : lacunes du filet de sécurité sociale, refus d&apos;allocations chômage, violations des droits à la retraite et déficits de protection santé.</p>
              <p className="text-slate-400">Source : Social Protection Rights Engine · Wave 256</p>
            </div>
          )}
        </div>
      </div>
...
export default function SocialProtectionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/social-protection-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Social Protection Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Filet de Sécurité · Chômage · Retraite · Protection Santé</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#0891b220" } : {}}>
