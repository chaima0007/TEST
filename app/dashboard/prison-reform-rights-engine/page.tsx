...
interface Entity {
  ...
  last_updated: string;
  inhumane_prison_conditions_score?: number;
  solitary_overuse_score?: number;
  prison_corruption_score?: number;
  reintegration_failure_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "inhumane_prison_conditions_score", label: "Conditions carcérales inhumaines" },
  { key: "solitary_overuse_score", label: "Abus de l&apos;isolement" },
  { key: "prison_corruption_score", label: "Corruption pénitentiaire" },
  { key: "reintegration_failure_score", label: "Échecs de réintégration" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la réforme pénitentiaire : conditions carcérales inhumaines, abus de l&apos;isolement, corruption pénitentiaire et échecs de réintégration.</p>
              <p className="text-slate-400">Source : Prison Reform Rights Engine · Wave 256</p>
            </div>
          )}
        </div>
      </div>
...
export default function PrisonReformRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/prison-reform-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Prison Reform Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Conditions Carcérales · Isolement · Corruption · Réintégration</p>
        </div>
