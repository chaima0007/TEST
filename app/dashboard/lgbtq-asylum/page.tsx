...
export default function LgbtqAsylumDashboard() {
  const [data, setData] = useState<{ entities?: { entity: string; composite: number; level: string }[]; avg_composite?: number } | null>(null);
  useEffect(() => { fetch("/api/lgbtqasylum").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-2">LGBTQ+ Asylum &amp; Refugee Rights</h1>
      <p className="text-gray-500 text-sm mb-6">Protection internationale des réfugiés LGBTQ+ et droits d&apos;asile</p>
