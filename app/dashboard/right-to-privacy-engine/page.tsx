...
interface Entity {
  ...
  mass_surveillance_score: number;
  data_exploitation_score: number;
  legal_protection_absence_score: number;
  digital_surveillance_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_right_to_privacy_index: number;
  last_updated: string;
  [key: string]: unknown;
}
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_right_to_privacy_index: number;
}
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/right-to-privacy-engine").then(r => r.json()).then(j => setData(j.data ?? j)).catch(console.error);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Right to Privacy Engine</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice RTP</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_right_to_privacy_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { key: "mass_surveillance_score", label: "Surveillance Masse" },
                      { key: "data_exploitation_score", label: "Exploitation Données" },
                      { key: "legal_protection_absence_score", label: "Absence Protection" },
                      { key: "digital_surveillance_score", label: "Surveillance Num." },
                    ].map(({ key, label }) => (
                      <div key={key}>
