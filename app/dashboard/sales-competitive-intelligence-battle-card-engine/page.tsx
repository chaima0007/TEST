...
type Rep = {
  ...
  preparedness_score: number; execution_score: number;
  intelligence_score: number; positioning_score: number;
...
type Summary = {
  ...
  avg_preparedness_score: number; avg_execution_score: number;
  avg_intelligence_score: number; avg_positioning_score: number;
...
const PAT_COLORS  = { none:"#10b981", unprepared_seller:"#ef4444", feature_fighter:"#f97316", price_surrenderer:"#f59e0b", late_discovery:"#a855f7", single_thread_loser:"#3b82f6" };
const SEV_COLORS  = { dominant:"#10b981", competing:"#f59e0b", struggling:"#f97316", losing:"#ef4444" };
const ACT_COLORS  = { no_action:"#10b981", competitive_awareness_coaching:"#f59e0b", battle_card_refresh_coaching:"#f97316", value_differentiation_coaching:"#a855f7", multi_thread_coaching:"#3b82f6", competitive_deal_review:"#ef4444", executive_competitive_escalation:"#dc2626" };
...
const SEV_BADGE   = { dominant:"bg-emerald-900 text-emerald-300", competing:"bg-amber-900 text-amber-300", struggling:"bg-orange-900 text-orange-300", losing:"bg-red-900 text-red-300" };
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-lg p-6 shadow-2xl" onClick={e=>e.stopPropagation()}>
        ...
        {tab==="scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ["Preparedness", rep.preparedness_score,  "#f97316"],
              ["Execution",    rep.execution_score,     "#ef4444"],
              ["Intelligence", rep.intelligence_score,  "#a855f7"],
              ["Positioning",  rep.positioning_score,   "#3b82f6"],
            ].map(([l,v,c])=>(
        ...
        {tab==="action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="flex gap-2">
              {rep.has_competitive_gap          && <span className="px-2 py-1 rounded bg-red-900 text-red-300 text-xs font-medium">⚔️ GAP</span>}
              {rep.requires_competitive_coaching && <span className="px-2 py-1 rounded bg-rose-900 text-rose-300 text-xs font-medium">🎯 COACH</span>}
            </div>
          </div>
        )}
      </div>
...
export default function CompetitiveIntelligenceDashboard() {
  ...
  useEffect(()=>{
    fetch("/api/sales-competitive-intelligence-battle-card-engine")
      .then(r=>r.json()).then(setData).catch(console.error);
  },[]);

  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-rose-400 text-lg animate-pulse">Loading Competitive Intelligence...</div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-white">Competitive Intelligence & Battle Card Engine</h1>
        <p className="text-slate-400 text-sm mt-1">Preparedness · Execution · Intelligence · Positioning</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ["Reps",           summary.total,                                                                  "text-rose-400"],
          ["Avg Composite",  summary.avg_competitive_composite,                                              "text-red-400"],
          ...
          ["Avg Win Rate",   `${Math.round((1 - summary.avg_execution_score/100) * 100)}%`,                  "text-rose-400"],
        ].map(([l,v,c])=>(
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          <Gauge value={summary.avg_preparedness_score}  label="Preparedness"  color="#f97316"/>
          <Gauge value={summary.avg_execution_score}     label="Execution"     color="#ef4444"/>
          <Gauge value={summary.avg_intelligence_score}  label="Intelligence"  color="#a855f7"/>
          <Gauge value={summary.avg_positioning_score}   label="Positioning"   color="#3b82f6"/>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {["all","unprepared_seller","feature_fighter","price_surrenderer","late_discovery","single_thread_loser"].map(p=>(
          <button key={p} onClick={()=>setPat(p)}
            className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${patFilter===p?"bg-red-800 border-red-700 text-white":"bg-slate-900 border-slate-700 text-slate-400 hover:text-white"}`}>
            {p.replace(/_/g," ")}
          </button>
        ))}
      </div>
        ...
        {filtered.map(r=>(
          <div key={r.rep_id} onClick={()=>setSelected(r)}
            ...
            <div className="flex gap-1 flex-wrap">
              {r.has_competitive_gap          && <span className="px-1.5 py-0.5 rounded bg-red-900 text-red-300 text-xs">⚔️ GAP</span>}
              {r.requires_competitive_coaching && <span className="px-1.5 py-0.5 rounded bg-rose-900 text-rose-300 text-xs">🎯 COACH</span>}
            </div>
          </div>
        ))}
      </div>
