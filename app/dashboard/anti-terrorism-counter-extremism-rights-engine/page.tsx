...
export default function AntiTerrorismCounterExtremismRightsPage() {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  useEffect(() => {
    fetch("/api/anti-terrorism-counter-extremism-rights-engine")
      .then((r) => r.json())
  ...
  const avgIndex = (data.avg_estimated_anti_terrorism_counter_extremism_rights_index as number) ?? 0;
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-6">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: AMBER }}>
            Antiterrorisme &amp; Droits Fondamentaux
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            UN SR Droits Humains · Amnesty International · HRW Lois Sécuritaires · CLUE Surveillance
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Avg Composite", value: avgComposite.toFixed(2), unit: "/100" },
            { label: "Indice Anti-Terror", value: avgIndex.toFixed(2), unit: "/10" },
            ...
            <div key={label} className="rounded-xl bg-slate-900 border border-slate-800 p-4 text-center">
              ...
              <div className="text-2xl font-bold" style={{ color: AMBER }}>{value}<span className="text-sm text-slate-500 ml-1">{unit}</span></div>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 flex flex-col items-center">
            <div className="text-xs text-slate-500 uppercase tracking-widest mb-4">Score Global Droits Anti-Terror</div>
            <div className="w-32 h-32"><GaugeRing value={avgComposite} color={AMBER} /></div>
            <div className="mt-3 text-3xl font-bold" style={{ color: AMBER }}>{avgComposite.toFixed(1)}</div>
          </div>
        ...
        {entities.length > 0 && (
          <div className="rounded-xl bg-slate-900 border border-slate-800 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-800">
              <div className="text-xs text-slate-500 uppercase tracking-widest">Pays analysés</div>
            </div>
            <div className="divide-y divide-slate-800">
              {entities.map((e) => {
                ...
                const idx = e.estimated_anti_terrorism_counter_extremism_rights_index as number;
                return (
        ...
        <div className="rounded-xl border p-5" style={{ borderColor: AMBER + "40", backgroundColor: AMBER + "10" }}>
          <div className="text-xs font-semibold mb-1" style={{ color: AMBER }}>ONU 2023 — Alerte Droits &amp; Sécurité</div>
          <p className="text-xs text-slate-300 leading-relaxed">
            Le Rapporteur spécial ONU alerte : 144 pays ont adopté des lois antiterroristes criminalisant
            la dissidence pacifique. Les définitions du &quot;terrorisme&quot; englobent régulièrement journalistes,
            défenseurs droits humains et minorités religieuses.
          </p>
        </div>
      </div>
    </div>
