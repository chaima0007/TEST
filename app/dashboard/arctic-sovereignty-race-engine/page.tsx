...
interface SovereigntyEntity {
  id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  territorial_claim_score: number;
  military_buildup_score: number;
  resource_extraction_score: number;
  maritime_route_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_sovereignty_index: number;
  last_updated: string;
}

interface SovereigntySummary {
  ...
  avg_estimated_sovereignty_index: number;
}
...
function EntityCard({ entity, onClick }: { entity: SovereigntyEntity; onClick: (e: SovereigntyEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.territorial_claim_score}</p>
          <p className="text-[10px] text-gray-500">Revendication</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.military_buildup_score}</p>
          <p className="text-[10px] text-gray-500">Militaire</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.maritime_route_score}</p>
          <p className="text-[10px] text-gray-500">Maritime</p>
        </div>
      </div>
...
function DetailModal({ entity, onClose }: { entity: SovereigntyEntity; onClose: () => void }) {
  ...
  const actions: Record<string, string> = {
    "Militarisation Arctique": "Déployer une surveillance satellite renforcée et alerter les partenaires OTAN sous 24h.",
    "Contestation de Route Maritime": "Initier des consultations diplomatiques d'urgence et renforcer la présence navale.",
    "Extraction Illégale de Ressources": "Saisir la Cour Internationale de Justice et bloquer les licences d'exploitation.",
    "Revendication Territoriale Escaladante": "Renforcer la présence diplomatique et documenter les violations pour le droit international.",
    "Tension de Gouvernance Arctique": "Engager des médiateurs du Conseil Arctique et proposer des zones tampons partagées.",
  };
          ...
          {tab === "scores" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Revendication Territoriale", value: `${entity.territorial_claim_score}/100` },
                  { label: "Militarisation", value: `${entity.military_buildup_score}/100` },
                  { label: "Extraction Ressources", value: `${entity.resource_extraction_score}/100` },
                  { label: "Route Maritime", value: `${entity.maritime_route_score}/100` },
                  { label: "Indice Souveraineté", value: `${entity.estimated_sovereignty_index}/10` },
              ...
              <div className="bg-white/4 rounded-lg p-4">
                ...
                <p className="text-[11px] text-gray-500 mt-1.5">
                  Formule : territorial×0.30 + militaire×0.25 + extraction×0.25 + maritime×0.20
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "actions" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-300 leading-relaxed">{actions[entity.primary_pattern] ?? "Surveiller l'évolution et renforcer la coopération arctique multilatérale."}</p>
              </div>
              <div className="bg-slate-800/60 border border-white/8 rounded-xl p-4">
                <p className="text-xs text-gray-400">
                  <span className="text-gray-300 font-medium">Pays concerné :</span> {entity.country}
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  <span className="text-gray-300 font-medium">Secteur :</span> {entity.sector}
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  <span className="text-gray-300 font-medium">Dernière analyse :</span> {entity.last_updated}
                </p>
              </div>
            </div>
          )}
...
export default function ArcticSovereigntyPage() {
  const [data, setData] = useState<SovereigntyData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<SovereigntyEntity | null>(null);

  useEffect(() => {
    fetch("/api/arctic-sovereignty-race-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as SovereigntyData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Course à la Souveraineté Arctique</h1>
            <p className="text-sm text-gray-400 mt-1">
              Moteur d&apos;intelligence Swarm — analyse des revendications territoriales et conflits arctiques
            </p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              <KpiCard label="Total Entités" value={s.total_entities} sub="zones analysées" />
              ...
              <KpiCard label="Alertes Critiques" value={s.critical_alerts} sub="intervention urgente" accent="text-red-400" />
              <KpiCard label="Indice Souveraineté" value={`${s.avg_estimated_sovereignty_index}/10`} sub="score normalisé" accent="text-cyan-400" />
              <KpiCard label="Confiance Analyse" value={`${s.confidence_score}%`} sub="fiabilité données" accent="text-indigo-400" />
              <KpiCard label="Entités Élevé/Critique" value={s.risk_distribution["critique"] + (s.risk_distribution["élevé"] ?? 0)}
                sub="risque significatif" accent="text-orange-400" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Géopolitiques</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_sovereignty_index} max={10} label="Indice Souv." color="#22d3ee" />
                </div>
              </div>
              ...
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                ...
                <div className="mt-5 pt-4 border-t border-white/8 grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-white/4 rounded-lg p-3">
                    ...
                    <p className="text-lg font-bold text-cyan-400 mt-0.5">{s.engine_version}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.critical_alerts} zone{s.critical_alerts > 1 ? "s" : ""} en situation critique
                  </p>
                  <p className="text-xs text-red-400/70 mt-0.5">
                    Risque le plus élevé : <span className="font-medium">{s.top_risk_entities[0]}</span>
                  </p>
                </div>
              </div>
            )}
