...
interface Entity {
  ...
  retaliation_prosecution_espionage_severity_score: number;
  source_journalist_surveillance_exposure_scale_score: number;
  whistleblower_legal_protection_absence_score: number;
  public_interest_disclosure_mechanism_deficit_gap_score: number;
  estimated_whistleblower_leaker_protection_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_whistleblower_leaker_protection_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Représailles & Poursuites Espionnage", value: entity.retaliation_prosecution_espionage_severity_score, weight: "0.30" },
    { label: "Surveillance Sources Journalistes", value: entity.source_journalist_surveillance_exposure_scale_score, weight: "0.25" },
    { label: "Absence Protection Juridique Lanceurs Alerte", value: entity.whistleblower_legal_protection_absence_score, weight: "0.25" },
    { label: "Déficit Mécanismes Divulgation Intérêt Public", value: entity.public_interest_disclosure_mechanism_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_whistleblower_leaker_protection_index === "number" ? entity.estimated_whistleblower_leaker_protection_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Lanceurs d&apos;Alerte</div>
                </div>
              </div>
...
export default function WhistleblowerLeakerProtectionEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/whistleblower-leaker-protection-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Lanceurs d&apos;Alerte & Protection des Sources…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_whistleblower_leaker_protection_index ?? avg(allEntities.map(e => e.estimated_whistleblower_leaker_protection_index));
  ...
  const avgRetaliation = avg(allEntities.map(e => e.retaliation_prosecution_espionage_severity_score));
  const avgSurveillance = avg(allEntities.map(e => e.source_journalist_surveillance_exposure_scale_score));
  const avgLegalProtection = avg(allEntities.map(e => e.whistleblower_legal_protection_absence_score));
  const avgDisclosure = avg(allEntities.map(e => e.public_interest_disclosure_mechanism_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Lanceurs Alerte", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Lanceurs d&apos;Alerte & Protection des Sources</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Whistleblower Leaker Protection Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgRetaliation} label="Représailles & Poursuites" />
          <GaugeRing value={avgSurveillance} label="Surveillance Sources" />
          <GaugeRing value={avgLegalProtection} label="Protection Juridique" />
          <GaugeRing value={avgDisclosure} label="Mécanismes Divulgation" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Lanceurs Alerte: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_whistleblower_leaker_protection_index === "number" ? e.estimated_whistleblower_leaker_protection_index.toFixed(2) : "—"}</span>
            </div>
