...
interface Entity {
  ...
  last_updated: string;
  active_mine_contamination_score?: number;
  victim_assistance_gap_score?: number;
  clearance_funding_gap_score?: number;
  mine_ban_non_compliance_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "active_mine_contamination_score", label: "Contamination active par mines terrestres" },
  { key: "victim_assistance_gap_score", label: "Lacunes d&apos;assistance aux victimes" },
  { key: "clearance_funding_gap_score", label: "Manque de financement pour le déminage" },
  { key: "mine_ban_non_compliance_score", label: "Non-conformité au traité d&apos;interdiction des mines" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits des victimes de mines terrestres : contamination active par mines, lacunes d&apos;assistance aux victimes, manque de financement pour le déminage et non-conformité au traité d&apos;interdiction des mines.</p>
              <p className="text-slate-400">Source : Land Mine Victims Rights Engine · Wave 249</p>
            </div>
          )}
        </div>
      </div>
...
export default function LandMineVictimsRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/land-mine-victims-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Land Mine Victims Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Contamination Active · Assistance Victimes · Financement Déminage · Conformité Traité</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#92400e20" } : {}}>
