...
interface DashData {
  ...
  avg_estimated_aml_rights_index?: number;
...
export default function AntiMoneyLaunderingHumanRightsDashboard() {
  const [raw, setRaw] = useState<DashData | null>(null);
  const [selected, setSelected] = useState<Entity | null>(null);
  const [riskFilter, setRiskFilter] = useState<string>("all");

  useEffect(() => {
    fetch("/api/anti-money-laundering-human-rights-engine")
      .then((r) => r.json())
  ...
  if (!raw) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-amber-400 text-lg animate-pulse">Initialisation du Moteur Blanchiment Argent &amp; Droits Humains...</div>
    </div>
  );
  ...
  const avgIndex = raw.avg_estimated_aml_rights_index ?? (entities.reduce((s, e) => s + e.estimated_aml_rights_index, 0) / n);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 p-6 space-y-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setSelected(null)}>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                <div className="text-3xl font-bold" style={{ color: ACCENT }}>{selected.estimated_aml_rights_index.toFixed(2)}</div>
                <div className="text-xs text-slate-400 mt-1">Index AML Droits /10</div>
              </div>
            </div>
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Blanchiment Argent &amp; Droits Humains</h1>
        <p className="text-slate-400 text-sm mt-1">
          Détection · Conformité FATF · Flux Illicites · Impact Droits Humains — Intelligence AML — Caelum Partners
        </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          ["Index AML Moyen", avgIndex.toFixed(2) + "/10", "text-yellow-400"],
      ...
      <div className="flex flex-wrap gap-2">
        {["all", "critique", "élevé", "modéré", "faible"].map((r) => (
          <button key={r} onClick={() => setRiskFilter(r)}
            ...
            style={riskFilter === r ? { background: ACCENT, borderColor: ACCENT } : { borderColor: "#92400e" }}>
          ...
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="flex items-end justify-between">
              ...
              <div className="text-right">
                <div className="text-lg font-bold text-yellow-400">{e.estimated_aml_rights_index.toFixed(2)}</div>
