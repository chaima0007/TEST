...
export default function AlgorithmicWageTheftDashboard() {
  const [data, setData] = useState<{ entities?: { entity: string; composite: number; level: string }[]; avg_composite?: number } | null>(null);
  useEffect(() => { fetch("/api/algorithmicwagetheft").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-2">Algorithmic Wage Theft &amp; Gig Economy Rights</h1>
      <p className="text-gray-500 text-sm mb-6">Vol de salaire algorithmique et droits des travailleurs de plateforme</p>
