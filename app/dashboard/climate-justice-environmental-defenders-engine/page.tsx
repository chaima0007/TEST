...
interface Entity {
  ...
  environmental_defender_killing_criminalization_severity_score: number;
  climate_loss_damage_vulnerable_population_scale_score: number;
  fossil_fuel_corporate_impunity_score: number;
  climate_litigation_access_justice_deficit_gap_score: number;
  estimated_climate_justice_environmental_defenders_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_climate_justice_environmental_defenders_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Meurtres & Criminalisation Défenseurs", value: entity.environmental_defender_killing_criminalization_severity_score, weight: "0.30" },
    { label: "Pertes & Dommages Climatiques", value: entity.climate_loss_damage_vulnerable_population_scale_score, weight: "0.25" },
    { label: "Impunité Entreprises Fossiles", value: entity.fossil_fuel_corporate_impunity_score, weight: "0.25" },
    { label: "Déficit Accès Justice Climatique", value: entity.climate_litigation_access_justice_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_climate_justice_environmental_defenders_index === "number" ? entity.estimated_climate_justice_environmental_defenders_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Justice Climatique</div>
                </div>
              </div>
...
export default function ClimateJusticeEnvironmentalDefendersEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/climate-justice-environmental-defenders-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Justice Climatique & Défenseurs Environnement…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_climate_justice_environmental_defenders_index ?? avg(allEntities.map(e => e.estimated_climate_justice_environmental_defenders_index));
  ...
  const avgDefenderKilling = avg(allEntities.map(e => e.environmental_defender_killing_criminalization_severity_score));
  const avgClimateLoss = avg(allEntities.map(e => e.climate_loss_damage_vulnerable_population_scale_score));
  const avgFossilImpunity = avg(allEntities.map(e => e.fossil_fuel_corporate_impunity_score));
  const avgLitigation = avg(allEntities.map(e => e.climate_litigation_access_justice_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Justice Climatique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Justice Climatique & Défenseurs Environnement</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Climate Justice & Environmental Defenders Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgDefenderKilling} label="Meurtres & Criminalisation" />
          <GaugeRing value={avgClimateLoss} label="Pertes & Dommages" />
          <GaugeRing value={avgFossilImpunity} label="Impunité Fossiles" />
          <GaugeRing value={avgLitigation} label="Déficit Justice Climatique" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Justice Climatique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_climate_justice_environmental_defenders_index === "number" ? e.estimated_climate_justice_environmental_defenders_index.toFixed(2) : "—"}</span>
            </div>
