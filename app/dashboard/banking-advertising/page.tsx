      ...
      <div style={{ background: "#0f172a", border: "1px solid #334155", borderRadius: 16, width: "100%", maxWidth: 640, maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        <div style={{ padding: 24, borderBottom: "1px solid #1e293b", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "#94a3b8", marginTop: 2 }}>Banking Advertising Intelligence</p>
          </div>
        ...
        <div style={{ padding: 24 }}>
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <p style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.6 }}>
                Cet acteur opère dans le secteur de la publicité bancaire. Son score composite reflète
                l&apos;exposition aux risques réglementaires, éthiques et d&apos;impact sur les droits humains
                dans les pratiques publicitaires et marketing financier.
              </p>
            </div>
          )}
          {tab === "metriques" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                { label: "Transparence publicitaire", value: entity.composite_score * 0.30 },
                { label: "Conformité réglementaire", value: entity.composite_score * 0.25 },
                { label: "Pratiques de ciblage éthique", value: entity.composite_score * 0.25 },
                { label: "Impact consommateurs vulnérables", value: entity.composite_score * 0.20 },
              ].map(m => (
          ...
          {tab === "sources" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {["CFPB — Consumer Financial Protection Bureau", "FCA UK Advertising Standards", "ACPR France — Publicité Financière", "Autorité des Marchés Financiers (AMF)"].map(s => (
...
export default function BankingAdvertisingDashboard() {
  const [data, setData] = useState<DashData | null>(null);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<Entity | null>(null);

  useEffect(() => {
    fetch("/api/banking-advertising")
      .then(r => r.json())
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#020617", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ color: "#94a3b8", fontSize: 14 }}>Chargement Banking Advertising…</p>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020617", color: "white", padding: "32px 16px" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto 32px" }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 800, color: "white", marginBottom: 4 }}>Banking Advertising Intelligence</h1>
        <p style={{ fontSize: 14, color: "#94a3b8" }}>
          Surveillance des pratiques publicitaires dans le secteur bancaire — transparence, conformité et impact éthique
        </p>
      </div>
