...
export default function MentalHealthDetentionPage() {
  const [data, setData] = useState<DomainData | null>(null)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    fetch("/api/mental-health-detention-psychiatric-rights-engine")
      .then((r) => r.json())
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "#f1f5f9", padding: "2rem" }}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        <h1 style={{ fontSize: "1.75rem", fontWeight: 700, color: "#f1f5f9", margin: 0 }}>
          Détention Psychiatrique & Droits en Santé Mentale
        </h1>
        <p style={{ color: "#64748b", marginTop: "0.25rem", fontSize: 14 }}>
          Analyse mondiale des internements forcés, psychiatrie punitive et violations des droits des patients psychiatriques
        </p>
      </div>
          ...
          <div key={entity.id} style={{ background: "#1e293b", borderRadius: 12, padding: "1.25rem", display: "flex", alignItems: "center", gap: "1.25rem", border: "1px solid #334155" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ margin: "0.3rem 0 0", fontSize: 12, color: "#64748b" }}>
                Index droits santé mentale : <span style={{ color: ACCENT, fontWeight: 600 }}>{entity.estimated_mental_health_rights_index.toFixed(2)}</span> / 10
              </p>
            </div>
          </div>
        ))}
