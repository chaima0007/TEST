...
interface Entity {
  ...
  last_updated: string;
  border_violence_score?: number;
  detention_migration_rights_score?: number;
  family_separation_score?: number;
  asylum_procedure_failure_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "border_violence_score", label: "Violence aux frontières" },
  { key: "detention_migration_rights_score", label: "Droits en détention migratoire" },
  { key: "family_separation_score", label: "Séparation familiale" },
  { key: "asylum_procedure_failure_score", label: "Défaillance des procédures d&apos;asile" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la gouvernance migratoire : violences aux frontières, violations des droits en détention, séparations familiales et défaillances des procédures d&apos;asile affectant les populations migrantes.</p>
              <p className="text-slate-400">Source : Migration Governance Rights Engine · Wave 255</p>
            </div>
          )}
        </div>
      </div>
...
export default function MigrationGovernanceRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/migration-governance-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Migration Governance Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Violence Frontières · Détention · Séparation Familiale · Procédures d&apos;Asile</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-sky-900/40 text-sky-300"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "#0284c720" } : {}}>
