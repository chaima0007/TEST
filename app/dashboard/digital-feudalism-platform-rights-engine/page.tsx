...
interface Entity {
  ...
  platform_monopoly_dependency_score: number;
  algorithmic_wage_theft_score: number;
  data_extraction_labor_rights_score: number;
  regulatory_capture_accountability_score: number;
  estimated_digital_feudalism_platform_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_digital_feudalism_platform_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Dépendance Monopole Plateforme", value: entity.platform_monopoly_dependency_score, weight: "0.30" },
    { label: "Vol Salarial Algorithmique", value: entity.algorithmic_wage_theft_score, weight: "0.25" },
    { label: "Extraction Données & Droits du Travail", value: entity.data_extraction_labor_rights_score, weight: "0.25" },
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_digital_feudalism_platform_rights_index === "number" ? entity.estimated_digital_feudalism_platform_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Féodalisme Numérique</div>
                </div>
              </div>
...
export default function DigitalFeudalismPlatformRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/digital-feudalism-platform-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Féodalisme Numérique &amp; Droits des Plateformes…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_digital_feudalism_platform_rights_index ?? avg(allEntities.map(e => e.estimated_digital_feudalism_platform_rights_index));
  ...
  const avgMonopoly = avg(allEntities.map(e => e.platform_monopoly_dependency_score));
  const avgWageTheft = avg(allEntities.map(e => e.algorithmic_wage_theft_score));
  const avgDataExtraction = avg(allEntities.map(e => e.data_extraction_labor_rights_score));
  const avgRegCapture = avg(allEntities.map(e => e.regulatory_capture_accountability_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Féodalisme Num.", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Féodalisme Numérique &amp; Droits des Plateformes</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Digital Feudalism Platform Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgMonopoly} label="Monopole Plateforme" />
          <GaugeRing value={avgWageTheft} label="Vol Salarial Algo." />
          <GaugeRing value={avgDataExtraction} label="Extraction Données" />
          <GaugeRing value={avgRegCapture} label="Capture Régulatoire" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Féodalisme Num.: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_digital_feudalism_platform_rights_index === "number" ? e.estimated_digital_feudalism_platform_rights_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
