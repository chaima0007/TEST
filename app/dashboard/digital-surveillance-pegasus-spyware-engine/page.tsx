...
interface Entity {
  ...
  risk_level: string
  estimated_surveillance_rights_index: number
}
...
export default function DigitalSurveillancePegasusPage() {
  const [data, setData] = useState<DomainData | null>(null)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    fetch("/api/digital-surveillance-pegasus-spyware-engine")
      .then((r) => r.json())
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "#f1f5f9", padding: "2rem" }}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        <h1 style={{ fontSize: "1.75rem", fontWeight: 700, color: "#f1f5f9", margin: 0 }}>
          Surveillance Numérique & Spyware Pegasus
        </h1>
        <p style={{ color: "#64748b", marginTop: "0.25rem", fontSize: 14 }}>
          Analyse mondiale de la surveillance numérique, utilisation du spyware Pegasus et violations des droits à la vie privée
        </p>
      </div>
          ...
          <div key={entity.id} style={{ background: "#1e293b", borderRadius: 12, padding: "1.25rem", display: "flex", alignItems: "center", gap: "1.25rem", border: "1px solid #334155" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ margin: "0.3rem 0 0", fontSize: 12, color: "#64748b" }}>
                Index surveillance : <span style={{ color: ACCENT, fontWeight: 600 }}>{entity.estimated_surveillance_rights_index.toFixed(2)}</span> / 10
              </p>
            </div>
          </div>
        ))}
