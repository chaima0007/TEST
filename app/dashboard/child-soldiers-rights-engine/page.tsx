...
interface Entity {
  ...
  forced_recruitment_underage_severity_score: number;
  demobilization_reintegration_failure_scale_score: number;
  sexual_violence_child_combatant_score: number;
  accountability_commander_liability_gap_score: number;
  estimated_child_soldiers_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_child_soldiers_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Recrutement Forcé", value: entity.forced_recruitment_underage_severity_score, weight: "0.30" },
    { label: "DDR Échec", value: entity.demobilization_reintegration_failure_scale_score, weight: "0.25" },
    { label: "Violence Sexuelle", value: entity.sexual_violence_child_combatant_score, weight: "0.25" },
    { label: "Responsabilité Commandants", value: entity.accountability_commander_liability_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_child_soldiers_rights_index === "number" ? entity.estimated_child_soldiers_rights_index.toFixed(2) : "—"}</div>
...
export default function ChildSoldiersRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/child-soldiers-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Enfants Soldats…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_child_soldiers_rights_index ?? avg(allEntities.map(e => e.estimated_child_soldiers_rights_index));
  ...
  const avgForcedRecruitment = avg(allEntities.map(e => e.forced_recruitment_underage_severity_score));
  const avgDDR = avg(allEntities.map(e => e.demobilization_reintegration_failure_scale_score));
  const avgSexualViolence = avg(allEntities.map(e => e.sexual_violence_child_combatant_score));
  const avgAccountability = avg(allEntities.map(e => e.accountability_commander_liability_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Enfants Soldats", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Child Soldiers Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Recrutement forcé de mineurs, échec DDR et violence sexuelle dans les conflits armés — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgForcedRecruitment} label="Recrutement Forcé" />
          <GaugeRing value={avgDDR} label="DDR Échec" />
          <GaugeRing value={avgSexualViolence} label="Violence Sexuelle" />
          <GaugeRing value={avgAccountability} label="Responsabilité Commandants" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Enfants Soldats: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_child_soldiers_rights_index === "number" ? e.estimated_child_soldiers_rights_index.toFixed(2) : "—"}</span>
            </div>
