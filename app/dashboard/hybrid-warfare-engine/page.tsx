...
interface HWEntity {
  ...
  composite_score: number; cyber_intensity_score: number;
  proxy_activity_score: number; info_operations_score: number;
  subthreshold_coercion_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_hybrid_index: number; last_updated: string;
}
interface HWSummary {
  ...
  entities: HWEntity[]; avg_estimated_hybrid_index: number;
}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Intensité Cyber", v: entity.cyber_intensity_score, c: "#ef4444" },
              { l: "Activité Proxies", v: entity.proxy_activity_score, c: "#f97316" },
              { l: "Opérations Info", v: entity.info_operations_score, c: "#eab308" },
              { l: "Coercition Sous-Seuil", v: entity.subthreshold_coercion_score, c: "#8b5cf6" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Hybride Caelum</span>
              <span className="text-violet-400 font-bold">{entity.estimated_hybrid_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
...
export default function HybridWarfarePage() {
  ...
  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse des opérations de guerre hybride mondiales…</div></div>;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Hybrid Warfare Engine — Zone Grise & Opérations Multi-Domaines</h1>
          <p className="text-slate-400 text-sm">Détection des offensives hybrides : cyber + proxy + info + coercition sous-seuil · v{data.engine_version}</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { l: "Zones Analysées", v: data.total_entities, g: "from-slate-700 to-slate-600", t: "text-white" },
            { l: "Score Hybride Moyen", v: `${data.avg_composite}%`, g: "from-violet-900 to-violet-800", t: "text-violet-300" },
            { l: "Alertes Critiques", v: data.critical_alerts.length, g: "from-red-900 to-red-800", t: "text-red-300" },
            { l: "Index Hybride", v: data.avg_estimated_hybrid_index, g: "from-purple-900 to-purple-800", t: "text-purple-300" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs Hybrides Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("cyber_intensity_score")} label="Intensité Cyber" color="#ef4444" />
            <GaugeRing value={avgScore("proxy_activity_score")} label="Activité Proxies" color="#f97316" />
            <GaugeRing value={avgScore("info_operations_score")} label="Ops Informationnelles" color="#eab308" />
            <GaugeRing value={avgScore("subthreshold_coercion_score")} label="Coercition Sous-Seuil" color="#8b5cf6" />
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Distribution Risques Hybrides</h2>
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns d'Opérations Hybrides</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-violet-900/10 border border-violet-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-violet-400 mb-2">Alertes Hybrides Critiques — Offensives Multi-Domaines</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_hybrid_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Guerre Hybride & Zone Grise · {data.last_analysis}</p>
      </div>
    </div>
  );
}
