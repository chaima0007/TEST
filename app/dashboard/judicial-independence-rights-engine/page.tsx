...
interface Entity {
  ...
  executive_judiciary_capture_severity_score: number;
  judicial_appointment_politicization_scale_score: number;
  judge_persecution_harassment_risk_score: number;
  court_packing_restructuring_abuse_gap_score: number;
  estimated_judicial_independence_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_judicial_independence_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Capture Exécutive", value: entity.executive_judiciary_capture_severity_score, weight: "0.30" },
    { label: "Nominations Politisées", value: entity.judicial_appointment_politicization_scale_score, weight: "0.25" },
    { label: "Persécution Juges", value: entity.judge_persecution_harassment_risk_score, weight: "0.25" },
    { label: "Restructuration Abusive", value: entity.court_packing_restructuring_abuse_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_judicial_independence_rights_index === "number" ? entity.estimated_judicial_independence_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Indépendance Judiciaire</div>
                </div>
              </div>
...
export default function JudicialIndependenceRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/judicial-independence-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Indépendance Judiciaire…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_judicial_independence_rights_index ?? avg(allEntities.map(e => e.estimated_judicial_independence_rights_index));
  ...
  const avgCaptureExecutive = avg(allEntities.map(e => e.executive_judiciary_capture_severity_score));
  const avgNominationsPolitisees = avg(allEntities.map(e => e.judicial_appointment_politicization_scale_score));
  const avgPersecutionJuges = avg(allEntities.map(e => e.judge_persecution_harassment_risk_score));
  const avgRestructuration = avg(allEntities.map(e => e.court_packing_restructuring_abuse_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Indépendance Judiciaire", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Judicial Independence Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Capture du pouvoir judiciaire, politisation des nominations et persécution des juges — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgCaptureExecutive} label="Capture Exécutive" />
          <GaugeRing value={avgNominationsPolitisees} label="Nominations Politi." />
          <GaugeRing value={avgPersecutionJuges} label="Persécution Juges" />
          <GaugeRing value={avgRestructuration} label="Restructuration" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Indépendance Judiciaire: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_judicial_independence_rights_index === "number" ? e.estimated_judicial_independence_rights_index.toFixed(2) : "—"}</span>
            </div>
