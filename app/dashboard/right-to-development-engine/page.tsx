...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-development-engine").then(r => r.json()).then(d => { setData(d); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Droit au Développement</h1>
        <p className="text-slate-400 mt-1">Right to Development Engine — Analyse des inégalités mondiales, pièges de la dette et exclusion du développement</p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800"><p className="text-slate-400 text-sm">Indice Développement Moyen</p><p className="text-2xl font-bold mt-1" style={{ color: ACCENT }}>{loading ? "…" : (data as any).avg_estimated_right_to_development_index?.toFixed(2) ?? "—"}</p><p className="text-slate-500 text-xs mt-1">/10</p></div>
