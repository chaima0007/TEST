...
interface Entity {
  ...
  last_updated: string;
  communal_land_seizure_score?: number;
  agribusiness_displacement_score?: number;
  land_registry_exclusion_score?: number;
  customary_rights_denial_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "communal_land_seizure_score", label: "Saisie de terres communautaires" },
  { key: "agribusiness_displacement_score", label: "Déplacement par l&apos;agrobusiness" },
  { key: "land_registry_exclusion_score", label: "Exclusion du registre foncier" },
  { key: "customary_rights_denial_score", label: "Déni des droits coutumiers" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits fonciers communautaires : saisies de terres, déplacements par l&apos;agrobusiness, exclusions des registres fonciers et déni des droits coutumiers des communautés autochtones et locales.</p>
              <p className="text-slate-400">Source : Community Land Rights Engine · Wave 255</p>
            </div>
          )}
        </div>
      </div>
...
export default function CommunityLandRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/community-land-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Community Land Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Saisies Foncières · Agrobusiness · Registre Foncier · Droits Coutumiers</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#15803d20" } : {}}>
