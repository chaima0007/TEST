...
interface Entity {
  ...
  last_updated: string;
  structural_discrimination_score?: number;
  racial_profiling_score?: number;
  economic_racial_exclusion_score?: number;
  hate_crime_impunity_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "structural_discrimination_score", label: "Discrimination structurelle" },
  { key: "racial_profiling_score", label: "Profilage racial" },
  { key: "economic_racial_exclusion_score", label: "Exclusion économique raciale" },
  { key: "hate_crime_impunity_score", label: "Impunité crimes de haine" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse du racisme systémique et des violations des droits fondamentaux. Les dimensions couvertes incluent la discrimination structurelle, le profilage racial, l&apos;exclusion économique raciale et l&apos;impunité face aux crimes de haine.</p>
              <p className="text-slate-400">Source : Racism Systemic Rights Engine · Wave 242</p>
            </div>
          )}
        </div>
      </div>
...
export default function RacismSystemicRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/racism-systemic-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Racism Systemic Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Discrimination Structurelle · Profilage Racial · Exclusion Économique · Crimes de Haine</p>
        </div>
