...
type Entity = {
  ...
  risk_level: string;
  estimated_mealkit_index: number;
};
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque publicitaire kits repas : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_mealkit_index.toFixed(2)} / 10`,
    `Conformité campagne Meal Kit : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 398 — Meal Kit Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: `${COLOR}99`, marginTop: 2 }}>Meal Kit Advertising · Wave 398</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: entity.estimated_mealkit_index * 10, color: COLOR },
        ...
        {tab === "signaux" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              `Score composite ${entity.composite_score.toFixed(1)} — ${entity.risk_level === "critique" ? "surveillance publicitaire kits repas urgente" : entity.risk_level === "élevé" ? "monitoring renforcé recommandé" : "suivi standard actif"}`,
              `Indice Caelum Meal Kit Advertising : ${entity.estimated_mealkit_index.toFixed(2)} / 10`,
              `Campagne kits repas en ${entity.risk_level === "faible" ? "zone verte — performance nominale" : "zone d'alerte — optimisation requise"}`,
            ].map((sig, i) => (
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(22,163,74,0.15)" }}>
              ...
              <div>{entity.risk_level === "critique" ? "Audit immédiat des campagnes kits repas — conformité réglementaire urgente" : entity.risk_level === "élevé" ? "Révision des stratégies publicitaires meal kit — renforcement compliance" : entity.risk_level === "modéré" ? "Optimisation continue des métriques publicitaires kits repas" : "Maintien des bonnes pratiques — veille sectorielle kits repas"}</div>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(22,163,74,0.15)" }}>
              <div style={{ fontSize: 11, color: "#93c5fd", marginBottom: 4 }}>Index Caelum</div>
              <div style={{ color: COLOR, fontWeight: 700, fontSize: 18 }}>{entity.estimated_mealkit_index.toFixed(2)} / 10</div>
            </div>
          </div>
        )}
      </div>
...
export default function MealkitAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/mealkit-advertising")
      .then((r) => r.json())
      .then((d) => { setData(d.payload ?? d); setLoading(false); })
      .catch(() => { setError("Erreur de chargement des données Meal Kit Advertising."); setLoading(false); });
  }, []);

  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Meal Kit Advertising&hellip;
      </div>
    );
  }
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + e.estimated_mealkit_index, 0) / entities.length
    : 0;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Meal Kit Advertising
          </h1>
        </div>
        <p style={{ fontSize: 13, color: `${COLOR}99`, margin: 0 }}>
          Droits de l&apos;enfant — Secteur kits repas · Caelum Partners · Wave 398 · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
        ...
        {filtered.map((entity, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
              ...
              <span style={{ color: COLOR, fontWeight: 600 }}>{entity.estimated_mealkit_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Meal Kit Advertising Intelligence · Wave 398
      </div>
