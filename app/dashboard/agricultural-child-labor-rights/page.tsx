...
interface Entity {
  ...
  hazardous_child_labor_exposure_score: number;
  education_access_denial_rights_score: number;
  child_labor_enforcement_gap_score: number;
  family_poverty_structural_drivers_score: number;
  estimated_agricultural_child_labor_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_agricultural_child_labor_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Exposition Travail Dangereux Enfants", value: entity.hazardous_child_labor_exposure_score, weight: "0.30" },
    { label: "Déni Droits Accès Éducation", value: entity.education_access_denial_rights_score, weight: "0.25" },
    { label: "Lacune Application Législation Travail Enfant", value: entity.child_labor_enforcement_gap_score, weight: "0.25" },
    { label: "Facteurs Structurels Pauvreté Familiale", value: entity.family_poverty_structural_drivers_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_agricultural_child_labor_rights_index === "number" ? entity.estimated_agricultural_child_labor_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Travail Enfant Agricole</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = Exposition×0.30 + Éducation×0.25 + Législation×0.25 + Pauvreté×0.20
                </div>
              </div>
...
export default function AgriculturalChildLaborRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/agricultural-child-labor-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits contre le Travail Enfant Agricole…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_agricultural_child_labor_rights_index ?? avg(allEntities.map(e => e.estimated_agricultural_child_labor_rights_index));
  ...
  const avgExposition = avg(allEntities.map(e => e.hazardous_child_labor_exposure_score));
  const avgEducation = avg(allEntities.map(e => e.education_access_denial_rights_score));
  const avgLegislation = avg(allEntities.map(e => e.child_labor_enforcement_gap_score));
  const avgPauvrete = avg(allEntities.map(e => e.family_poverty_structural_drivers_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Travail Enfant Agricole", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits contre le Travail Enfant Agricole</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Agricultural Child Labor Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgExposition} label="Exposition Dangereuse" />
          <GaugeRing value={avgEducation} label="Accès Éducation" />
          <GaugeRing value={avgLegislation} label="Application Loi" />
          <GaugeRing value={avgPauvrete} label="Pauvreté Structurelle" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Travail Enfant Agricole: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_agricultural_child_labor_rights_index === "number" ? e.estimated_agricultural_child_labor_rights_index.toFixed(2) : "—"}</span>
            </div>
