...
interface RHEntity {
  ...
  key_signals: string[]; avg_estimated_right_to_health_index: number; last_updated: string;
}
interface RHSummary {
  ...
  confidence_score: number; data_sources: string[]; entities: RHEntity[];
  avg_estimated_right_to_health_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: RHEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "Aperçu" && (
          <div className="space-y-3">
            {[
              { l: "Déni Accès Soins", v: entity.healthcare_access_denial_score, c: "#ef4444" },
              { l: "Barrière Prix Médicaments", v: entity.medicine_affordability_barrier_score, c: "#f97316" },
              { l: "Mortalité Mat./Inf.", v: entity.maternal_infant_mortality_score, c: "#a855f7" },
              { l: "Effondrement Système", v: entity.health_system_collapse_score, c: "#f59e0b" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Droit à la Santé</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.avg_estimated_right_to_health_index}</span>
            </div>
          </div>
        )}
        {tab === "Signaux" && (
        ...
        {tab === "Contexte" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Mécanismes d'urgence humanitaire pour l'accès aux soins essentiels</p><p>› Mobilisation internationale pour la fourniture de médicaments vitaux</p><p>› Soutien d'urgence aux systèmes de santé en effondrement</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des programmes d'accès universel aux soins</p><p>› Surveillance des barrières financières aux médicaments essentiels</p></>}
            {entity.risk_level === "modéré" && <><p>› Développement de politiques de santé inclusives</p><p>› Renforcement de la résilience du système de santé</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien des standards minimaux d'accès aux soins</p><p>› Consolidation des programmes de santé préventive</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  crise_sanitaire_aigue: "#ef4444",
  inegalites_acces_soins: "#f97316",
  fragilite_systemique: "#f59e0b",
  tensions_sante: "#eab308",
  sante_exemplaire: "#10b981",
};

export default function RightToHealthEnginePage() {
  const [data, setData] = useState<RHSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<RHEntity | null>(null);

  useEffect(() => {
    fetch("/api/right-to-health-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données droit à la santé"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse du droit à la santé en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof RHEntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Right to Health Engine</h1>
          <p className="text-slate-400 text-sm mt-1">La santé comme droit universel — quand l'accès aux soins devient privilège, c'est l'humanité entière qui est en danger</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Moyen", v: `${data.avg_composite}%`, t: "text-teal-300" },
            ...
            { l: "Index Moyen Santé", v: data.avg_estimated_right_to_health_index, t: "text-teal-400" },
            { l: "Confiance", v: `${Math.round((data.confidence_score ?? 0) * 100)}%`, t: "text-slate-300" },
          ].map(({ l, v, t }) => (
            <div key={l} className="bg-slate-900 border border-teal-500/20 rounded-xl p-4">
        ...
        {/* Gauge Rings */}
        <div className="bg-slate-900 rounded-xl p-6 border border-teal-500/20">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Dimensions des Violations Sanitaires — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("healthcare_access_denial_score")} label="Déni Accès Soins" color={ACCENT} />
            <GaugeRing value={avgScore("medicine_affordability_barrier_score")} label="Barrière Prix Médicaments" color="#f97316" />
            <GaugeRing value={avgScore("maternal_infant_mortality_score")} label="Mortalité Mat./Inf." color="#a855f7" />
            <GaugeRing value={avgScore("health_system_collapse_score")} label="Effondrement Système" color="#f59e0b" />
          </div>
        </div>

        {/* Distribution Bars */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-teal-500/20">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-teal-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Crise Sanitaire</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-teal-900/10 border border-teal-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Critiques — Droit à la Santé</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.avg_estimated_right_to_health_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Right to Health Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
