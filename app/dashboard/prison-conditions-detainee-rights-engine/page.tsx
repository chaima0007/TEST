...
export default function PrisonConditionsDetaineeRightsPage() {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  useEffect(() => {
    fetch("/api/prison-conditions-detainee-rights-engine")
      .then((r) => r.json())
  ...
  const avgIndex = (data.avg_estimated_prison_conditions_detainee_rights_index as number) ?? 0;
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-6">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: RED }}>
            Conditions Carcérales — Droits des Détenus
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            Prison Overcrowding · Torture · UN SPT · Mandela Rules · Penal Reform International
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Avg Composite", value: avgComposite.toFixed(2), unit: "/100" },
            { label: "Indice Carcéral", value: avgIndex.toFixed(2), unit: "/10" },
            ...
            <div key={label} className="rounded-xl bg-slate-900 border border-slate-800 p-4 text-center">
              ...
              <div className="text-2xl font-bold" style={{ color: RED }}>{value}<span className="text-sm text-slate-500 ml-1">{unit}</span></div>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 flex flex-col items-center">
            <div className="text-xs text-slate-500 uppercase tracking-widest mb-4">Score Global Conditions Carcérales</div>
            <div className="w-32 h-32"><GaugeRing value={avgComposite} color={RED} /></div>
            <div className="mt-3 text-3xl font-bold" style={{ color: RED }}>{avgComposite.toFixed(1)}</div>
          </div>
          ...
          <div className="rounded-xl bg-slate-900 border border-slate-800 overflow-hidden">
            ...
            <div className="divide-y divide-slate-800">
              {entities.map((e) => {
                ...
                const idx = e.estimated_prison_conditions_detainee_rights_index as number;
                return (
