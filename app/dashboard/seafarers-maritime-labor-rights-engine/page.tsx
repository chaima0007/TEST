...
interface Entity {
  ...
  ship_abandonment_wage_theft_severity_score: number;
  flag_state_labor_standard_evasion_scale_score: number;
  unsafe_working_conditions_fatality_risk_score: number;
  repatriation_recruitment_fee_exploitation_gap_score: number;
  estimated_seafarers_maritime_labor_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_seafarers_maritime_labor_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Abandon & Vol Salaires", value: entity.ship_abandonment_wage_theft_severity_score, weight: "0.30" },
    { label: "Pavillon Complaisance", value: entity.flag_state_labor_standard_evasion_scale_score, weight: "0.25" },
    { label: "Conditions Mortelles", value: entity.unsafe_working_conditions_fatality_risk_score, weight: "0.25" },
    { label: "Frais Recrutement", value: entity.repatriation_recruitment_fee_exploitation_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_seafarers_maritime_labor_rights_index === "number" ? entity.estimated_seafarers_maritime_labor_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Marins</div>
                </div>
              </div>
...
export default function SeafarersMaritimeLaborRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/seafarers-maritime-labor-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Marins & Maritime…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_seafarers_maritime_labor_rights_index ?? avg(allEntities.map(e => e.estimated_seafarers_maritime_labor_rights_index));
  ...
  const avgAbandonment = avg(allEntities.map(e => e.ship_abandonment_wage_theft_severity_score));
  const avgFlagState = avg(allEntities.map(e => e.flag_state_labor_standard_evasion_scale_score));
  const avgUnsafe = avg(allEntities.map(e => e.unsafe_working_conditions_fatality_risk_score));
  const avgRepatriation = avg(allEntities.map(e => e.repatriation_recruitment_fee_exploitation_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Marins", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Seafarers Maritime Labor Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Abandon en mer, pavillon de complaisance et exploitation des marins — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgAbandonment} label="Abandon/Salaires" />
          <GaugeRing value={avgFlagState} label="Pavillon Compli." />
          <GaugeRing value={avgUnsafe} label="Conditions Risque" />
          <GaugeRing value={avgRepatriation} label="Frais Recrutement" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Marins: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_seafarers_maritime_labor_rights_index === "number" ? e.estimated_seafarers_maritime_labor_rights_index.toFixed(2) : "—"}</span>
            </div>
