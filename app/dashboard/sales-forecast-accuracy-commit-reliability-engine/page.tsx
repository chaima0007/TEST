...
type Rep = {
  ...
  accuracy_score: number; discipline_score: number;
  timing_score: number; reliability_score: number;
  forecast_composite: number; has_forecast_gap: boolean;
  requires_manager_review: boolean; estimated_forecast_error_usd: number;
...
type Summary = {
  ...
  forecast_gap_count: number; manager_review_count: number;
  avg_accuracy_score: number; avg_discipline_score: number;
  avg_timing_score: number; avg_reliability_score: number;
...
const PAT_COLORS  = { none:"#10b981", chronic_overcommit:"#ef4444", sandbagger:"#3b82f6", commit_drifter:"#f59e0b", late_push_abuser:"#f97316", category_manipulator:"#a855f7" };
const SEV_COLORS  = { accurate:"#10b981", drifting:"#f59e0b", unreliable:"#f97316", blind_spot:"#ef4444" };
const ACT_COLORS  = { no_action:"#10b981", forecast_monitoring:"#f59e0b", commit_cadence_coaching:"#f97316", pipeline_review_increase:"#a855f7", forecast_hygiene_training:"#3b82f6", manager_forecast_alignment:"#06b6d4", deal_by_deal_review:"#f59e0b", forecast_process_reset:"#dc2626", executive_forecast_audit:"#7f1d1d" };
...
const SEV_BADGE   = { accurate:"bg-emerald-900 text-emerald-300", drifting:"bg-amber-900 text-amber-300", unreliable:"bg-orange-900 text-orange-300", blind_spot:"bg-red-900 text-red-300" };
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-lg p-6 shadow-2xl" onClick={e=>e.stopPropagation()}>
        ...
        {tab==="scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ...
              ["Timing",      rep.timing_score,      "#f97316"],
              ["Reliability", rep.reliability_score, "#3b82f6"],
            ].map(([l,v,c])=>(
        ...
        {tab==="action" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="bg-slate-800 rounded-lg p-3">
              <div className="text-slate-400 text-xs mb-1">Forecast Error Estimate</div>
              <div className="text-white font-bold">${rep.estimated_forecast_error_usd.toLocaleString()}</div>
            </div>
            <div className="flex gap-2">
              {rep.has_forecast_gap        && <span className="px-2 py-1 rounded bg-orange-900 text-orange-300 text-xs font-medium">FORECAST GAP</span>}
              {rep.requires_manager_review && <span className="px-2 py-1 rounded bg-cyan-900 text-cyan-300 text-xs font-medium">MGR REVIEW</span>}
            </div>
          </div>
        )}
      </div>
...
export default function ForecastDashboard() {
  ...
  useEffect(()=>{
    fetch("/api/sales-forecast-accuracy-commit-reliability-engine")
      .then(r=>r.json()).then(setData).catch(console.error);
  },[]);

  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-cyan-400 text-lg animate-pulse">Loading Forecast Intelligence...</div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-white">Forecast Accuracy & Commit Reliability Intelligence</h1>
        <p className="text-slate-400 text-sm mt-1">Accuracy · Discipline · Timing · Reliability</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ["Reps",           summary.total,                                                                                  "text-cyan-400"],
          ["Avg Composite",  summary.avg_forecast_composite,                                                                "text-teal-400"],
          ["Forecast Gaps",  summary.forecast_gap_count,                                                                    "text-orange-400"],
          ["Mgr Reviews",    summary.manager_review_count,                                                                  "text-amber-400"],
          ["Forecast Error", `$${(summary.total_estimated_forecast_error_usd/1000).toFixed(0)}K`,                          "text-red-400"],
          ["Avg Accuracy",   `${Math.round(100 - summary.avg_accuracy_score)}%`,                                           "text-cyan-400"],
        ].map(([l,v,c])=>(
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          ...
          <Gauge value={summary.avg_timing_score}      label="Timing"      color="#f97316"/>
          <Gauge value={summary.avg_reliability_score} label="Reliability" color="#3b82f6"/>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {["all","chronic_overcommit","sandbagger","commit_drifter","late_push_abuser","category_manipulator"].map(p=>(
        ...
        {filtered.map(r=>(
          <div key={r.rep_id} onClick={()=>setSelected(r)}
            ...
            <div className="flex gap-1 flex-wrap">
              {r.has_forecast_gap        && <span className="px-1.5 py-0.5 rounded bg-orange-900 text-orange-300 text-xs">FCST GAP</span>}
              {r.requires_manager_review && <span className="px-1.5 py-0.5 rounded bg-cyan-900 text-cyan-300 text-xs">MGR</span>}
            </div>
          </div>
        ))}
      </div>
