...
interface AREntity {
  ...
  composite_score: number; arms_race_acceleration_score: number;
  alignment_research_deficit_score: number; governance_lag_score: number;
  catastrophic_deployment_risk_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_alignment_risk_index: number; last_updated: string;
}
interface ARSummary {
  ...
  entities: AREntity[]; avg_estimated_alignment_risk_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: AREntity; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-slate-900 border border-red-900/40 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Accélération Course IA", v: entity.arms_race_acceleration_score, c: "#ef4444" },
              { l: "Déficit Recherche Align.", v: entity.alignment_research_deficit_score, c: "#f97316" },
              { l: "Retard Gouvernance", v: entity.governance_lag_score, c: "#eab308" },
              { l: "Risque Déploiement Cat.", v: entity.catastrophic_deployment_risk_score, c: "#dc2626" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Risque Alignement Caelum</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_alignment_risk_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Moratoire immédiat sur les déploiements IA à risque catastrophique</p><p>› Coopération internationale d'urgence sur la sécurité IA</p><p>› Investissement massif en recherche sur l'alignement IA</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des protocoles de test de sécurité IA</p><p>› Mise en place d'une gouvernance IA internationale contraignante</p></>}
            {entity.risk_level === "modéré" && <><p>› Accélération de la recherche sur l'interprétabilité IA</p><p>› Dialogue multi-acteurs sur les standards de sécurité IA</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la vigilance sur la course aux armements IA</p><p>› Consolidation des pratiques d'IA responsable</p></>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  course_catastrophique: "#ef4444",
  desalignement_systemique: "#f97316",
  risque_emergeant: "#eab308",
  vigilance_requise: "#f59e0b",
  precaution_exemplaire: "#10b981",
};

export default function AIAlignmentRiskEnginePage() {
  const [data, setData] = useState<ARSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<AREntity | null>(null);

  useEffect(() => {
    fetch("/api/ai-alignment-risk-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données risque d'alignement IA"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse du risque d'alignement IA en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof AREntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Risque d'Alignement IA</h1>
          <p className="text-slate-400 text-sm mt-1">La course aux armements IA — vitesse contre sécurité dans la géopolitique de l'alignement</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Score Composite Moyen", v: `${data.avg_composite}%`, t: "text-red-300" },
            { l: "Alertes Critiques", v: data.critical_alerts.length, t: "text-red-400" },
            { l: "Indice Risque Alignement", v: data.avg_estimated_alignment_risk_index, t: "text-red-400" },
            ...
            <div key={l} className="bg-slate-900 border border-red-500/20 rounded-xl p-4">
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-red-500/20">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs de Risque Alignement IA — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("arms_race_acceleration_score")} label="Accélération Course IA" color={ACCENT} />
            <GaugeRing value={avgScore("alignment_research_deficit_score")} label="Déficit Recherche Align." color="#f97316" />
            <GaugeRing value={avgScore("governance_lag_score")} label="Retard Gouvernance" color="#eab308" />
            <GaugeRing value={avgScore("catastrophic_deployment_risk_score")} label="Risque Déploiement Cat." color="#dc2626" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-red-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Risque Alignement IA</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-red-900/10 border border-red-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Risque Alignement IA Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_alignment_risk_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Risque d'Alignement IA · {data.last_analysis}</p>
      </div>
    </div>
  );
}
