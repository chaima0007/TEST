...
interface Entity {
  ...
  state_perpetrated_disappearance_severity_score: number;
  impunity_accountability_absence_scale_score: number;
  family_right_to_truth_obstruction_score: number;
  legal_framework_prevention_gap_score: number;
  estimated_enforced_disappearances_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_enforced_disappearances_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Disparitions État", value: entity.state_perpetrated_disappearance_severity_score, weight: "0.30" },
    { label: "Impunité", value: entity.impunity_accountability_absence_scale_score, weight: "0.25" },
    { label: "Droit à la Vérité", value: entity.family_right_to_truth_obstruction_score, weight: "0.25" },
    { label: "Cadre Légal", value: entity.legal_framework_prevention_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_enforced_disappearances_index === "number" ? entity.estimated_enforced_disappearances_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Disparitions</div>
                </div>
              </div>
...
export default function EnforcedDisappearancesEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/enforced-disappearances-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Disparitions Forcées…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_enforced_disappearances_index ?? avg(allEntities.map(e => e.estimated_enforced_disappearances_index));
  ...
  const avgStateDisappearances = avg(allEntities.map(e => e.state_perpetrated_disappearance_severity_score));
  const avgImpunity = avg(allEntities.map(e => e.impunity_accountability_absence_scale_score));
  const avgTruth = avg(allEntities.map(e => e.family_right_to_truth_obstruction_score));
  const avgLegal = avg(allEntities.map(e => e.legal_framework_prevention_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Disparitions", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Enforced Disappearances Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Disparitions perpétrées par l&apos;État, impunité et obstruction au droit à la vérité des familles — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgStateDisappearances} label="Disparitions État" />
          <GaugeRing value={avgImpunity} label="Impunité" />
          <GaugeRing value={avgTruth} label="Droit à la Vérité" />
          <GaugeRing value={avgLegal} label="Cadre Légal" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Disparitions: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_enforced_disappearances_index === "number" ? e.estimated_enforced_disappearances_index.toFixed(2) : "—"}</span>
            </div>
