...
interface Entity {
  ...
  estimated_seed_patents_food_sovereignty_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_seed_patents_food_sovereignty_index?: number;
...
export default function SeedPatentsFoodSovereigntyEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/seed-patents-food-sovereignty-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ background: "#020617", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "#7c3aed", fontSize: "14px" }}>Initialisation Brevets Semences &amp; Souverainet&eacute; Alimentaire…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_seed_patents_food_sovereignty_index ?? avg(allEntities.map(e => e.estimated_seed_patents_food_sovereignty_index));
  ...
  const kpis = [
    ...
    { label: "Index Brevets Semences", value: avgIndex.toFixed(2) },
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "white", padding: "24px", display: "flex", flexDirection: "column", gap: "24px" }}>
            ...
            <div style={{ padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                <div style={{ background: "rgba(30,41,59,0.5)", borderRadius: "12px", padding: "16px", textAlign: "center" }}>
                  <div style={{ fontSize: "28px", fontWeight: "bold", color: accentDisplay }}>{selected.composite_score.toFixed(1)}</div>
                  <div style={{ fontSize: "11px", color: "#94a3b8", marginTop: "4px" }}>Score Composite</div>
                </div>
                <div style={{ background: "rgba(30,41,59,0.5)", borderRadius: "12px", padding: "16px", textAlign: "center" }}>
                  <div style={{ fontSize: "28px", fontWeight: "bold", color: accentDisplay }}>{selected.estimated_seed_patents_food_sovereignty_index.toFixed(2)}</div>
                  <div style={{ fontSize: "11px", color: "#94a3b8", marginTop: "4px" }}>Index Brevets Semences</div>
                </div>
              </div>
              <div style={{ background: "rgba(30,41,59,0.5)", borderRadius: "12px", padding: "16px" }}>
                ...
                <div style={{ fontSize: "14px", fontWeight: 500, color: accentDisplay }}>{selected.primary_pattern}</div>
              </div>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "4px" }}>
          <div style={{ width: "12px", height: "32px", borderRadius: "9999px", background: accentDisplay }} />
          <h1 style={{ fontSize: "22px", fontWeight: "bold", letterSpacing: "-0.025em", margin: 0 }}>Brevets Semences &amp; Souverainet&eacute; Alimentaire</h1>
        </div>
        <p style={{ color: "#94a3b8", fontSize: "13px", marginLeft: "24px" }}>
          Seed Patents Food Sovereignty Engine — Caelum Partners · Wave 206
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px" }}>
        {kpis.map(k => (
          <div key={k.label} style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: "12px", padding: "16px" }}>
            ...
            <div style={{ fontSize: "20px", fontWeight: "bold", color: accentDisplay }}>{k.value}</div>
          </div>
        ))}
      </div>

      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: "12px", padding: "20px" }}>
        <h2 style={{ fontSize: "13px", fontWeight: 600, color: "#94a3b8", marginBottom: "16px" }}>Score Global — Brevets Semences</h2>
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
        {filters.map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: "6px 16px", borderRadius: "9999px", fontSize: "13px", fontWeight: filter === f ? "bold" : "normal", border: "none", cursor: "pointer", background: filter === f ? accentDisplay : "#1e293b", color: filter === f ? "white" : "#94a3b8", transition: "all 0.15s" }}>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div style={{ height: "6px", background: "#1e293b", borderRadius: "9999px", overflow: "hidden", marginTop: "8px" }}>
              <div style={{ height: "100%", borderRadius: "9999px", width: `${Math.min(e.composite_score, 100)}%`, background: accentDisplay, transition: "all 0.3s" }} />
            </div>
            <div style={{ fontSize: "11px", color: "#64748b", marginTop: "8px" }}>
              Index: <span style={{ fontWeight: 500, color: accentDisplay }}>{e.estimated_seed_patents_food_sovereignty_index.toFixed(2)}</span>
            </div>
