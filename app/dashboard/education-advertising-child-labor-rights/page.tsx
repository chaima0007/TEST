...
export default function EducationAdvertisingChildLaborRightsPage() {
  const [data, setData] = useState<ApiData | null>(null);
  const [selected, setSelected] = useState<Entity | null>(null);
  const [tab, setTab] = useState<"apercu" | "indicateurs" | "recommandations" | "certifications">("apercu");

  useEffect(() => {
    fetch("/api/education-advertising-child-labor-rights")
      .then(r => r.json())
  ...
  return (
    <div style={{ padding: 32, fontFamily: "sans-serif", maxWidth: 1100, margin: "0 auto" }}>
      <h1 style={{ fontSize: 26, fontWeight: "bold", color: COLOR, marginBottom: 4 }}>Publicité Éducation &amp; Formation</h1>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
            {(["apercu", "indicateurs", "recommandations", "certifications"] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid #e5e7eb", cursor: "pointer", fontSize: 12, fontWeight: tab === t ? 700 : 400, background: tab === t ? COLOR : "#fff", color: tab === t ? "#fff" : "#374151" }}>
