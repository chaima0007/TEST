...
interface HSIEntity {
  ...
  incitement_violence_scale_score: number;
  platform_amplification_failure_score: number;
  legal_accountability_gap_score: number;
  minority_targeting_pattern_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_hate_speech_incitement_index: number;
  last_updated: string;
  [key: string]: unknown;
}

interface HSISummary {
  total_entities: number;
  avg_composite: number;
  avg_estimated_hate_speech_incitement_index: number;
...
function DetailModal({ entity, onClose }: { entity: HSIEntity; onClose: () => void }) {
  ...
  const scores = [
    { label: "Incitation Violence", key: "incitement_violence_scale_score" },
    { label: "Amplification Plateforme", key: "platform_amplification_failure_score" },
    { label: "Gap Légal", key: "legal_accountability_gap_score" },
    { label: "Ciblage Minorités", key: "minority_targeting_pattern_score" },
  ];
  return (
    <div
    ...
    >
      <div
        className="bg-slate-900 border border-orange-500/20 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl"
        ...
        <div className="p-6">
          {tab === 0 && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>
                    {entity.estimated_hate_speech_incitement_index.toFixed(1)}
                  </div>
                  <div className="text-xs text-slate-400 mt-1">Index Discours Haineux</div>
                </div>
              </div>
          ...
          {tab === 1 && (
            <div className="flex flex-col items-center gap-4 py-4">
              <GaugeRing value={entity.estimated_hate_speech_incitement_index} max={10} />
              <div className="text-center">
                <div className="text-3xl font-bold" style={{ color: ACCENT }}>
                  {entity.estimated_hate_speech_incitement_index.toFixed(2)}
                </div>
                <div className="text-sm text-slate-500 mt-1">Index Discours Haineux</div>
              </div>
            ...
            <div className="space-y-3">
              <div className="rounded-lg border border-slate-700/40 bg-slate-800/50 p-4">
                <p className="text-sm leading-relaxed text-slate-400">
                  Analyse du contexte de discours haineux et d&apos;incitation pour{" "}
...
export default function HateSpeechIncitementEnginePage() {
  const [data, setData] = useState<HSIData>({});
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<HSIEntity | null>(null);

  useEffect(() => {
    fetch("/api/hate-speech-incitement-engine")
      .then((r) => r.json())
      .then((d) => { setData(d.payload ?? d); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);

  const summary = (data as { summary?: HSISummary }).summary;
  const rawEntities: HSIEntity[] = (data as { entities?: HSIEntity[] }).entities ?? summary?.entities ?? [];

  const PATTERN_KEYS = [
    "incitement_violence_scale",
    "platform_amplification_failure",
    "legal_accountability_gap",
    "minority_targeting_pattern",
  ];
  ...
  const avgIndex = avg(rawEntities.map((e) => e.estimated_hate_speech_incitement_index));
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h1 className="text-2xl font-bold tracking-tight">Hate Speech &amp; Incitement Engine</h1>
            </div>
            <p className="text-slate-400 text-sm ml-6">
              Surveillance des discours haineux et incitation &agrave; la violence sur les plateformes numériques
            </p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {kpis.map((kpi) => (
            ...
            >
              ...
              <div className="mt-2 text-2xl font-bold text-orange-400">{kpi.value}</div>
            </div>
          ))}
        </div>
            ...
            {entities.map((entity) => (
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="text-xs text-slate-500">Index Discours Haineux</span>
                ...
                <div className="mt-2 space-y-1">
                  {[
                    { key: "incitement_violence_scale_score", label: "Incitation Violence" },
                    { key: "platform_amplification_failure_score", label: "Amplification Plateforme" },
                    { key: "legal_accountability_gap_score", label: "Gap Légal" },
                    { key: "minority_targeting_pattern_score", label: "Ciblage Minorités" },
                  ].map(({ key, label }) => (
