...
interface Entity {
  ...
  estimated_climate_loss_damage_index: number;
  economic_loss_scale_score: number;
  cultural_territorial_loss_score: number;
  compensation_mechanism_absence_score: number;
  historical_emitter_impunity_score: number;
...
interface EngineResult {
  ...
  avg_estimated_climate_loss_damage_index: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Pertes Économiques", value: entity.economic_loss_scale_score },
    { label: "Pertes Culturelles & Territoriales", value: entity.cultural_territorial_loss_score },
    { label: "Absence Mécanisme Compensation", value: entity.compensation_mechanism_absence_score },
    { label: "Impunité Émetteurs Historiques", value: entity.historical_emitter_impunity_score },
  ];
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <div className="w-20 h-20 shrink-0">
                  <GaugeRing value={entity.estimated_climate_loss_damage_index ?? entity.composite_score} />
                </div>
                <div>
                  <p className="text-slate-400 text-sm">Index Pertes Climatiques</p>
                  <p className="text-white text-2xl font-bold">
                    {(entity.estimated_climate_loss_damage_index ?? entity.composite_score).toFixed(1)}
                  </p>
...
export default function ClimateLossDamageEnginePage() {
  const [data, setData] = useState<EngineResult | null>(null);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<Entity | null>(null);

  useEffect(() => {
    fetch("/api/climate-loss-damage-engine")
      .then((r) => r.json())
  ...
  if (!data) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="text-slate-400 animate-pulse">Chargement du module Pertes &amp; Dommages Climatiques…</div>
      </div>
    );
  }
  ...
  const kpis = [
    ...
    { label: "Index Pertes Climatiques", value: data.avg_estimated_climate_loss_damage_index?.toFixed(2) ?? "—" },
    ...
    <div className="bg-slate-950 min-h-screen text-slate-100 p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Pertes &amp; Dommages Climatiques</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Pertes économiques · Pertes culturelles &amp; territoriales · Compensation · Impunité émetteurs — Caelum Partners
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6 flex items-center gap-6">
        ...
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1">
            Index Pertes Climatiques: {(data.avg_estimated_climate_loss_damage_index ?? 0).toFixed(2)}
          </p>
        </div>
      </div>
          ...
          {filtered.map((entity) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-2">
                <span className="text-slate-400 text-xs">Index Pertes Climatiques</span>
              ...
              <div className="mt-2 grid grid-cols-2 gap-1 text-xs text-slate-500">
                <span>Écon: {(entity.economic_loss_scale_score ?? 0).toFixed(1)}</span>
                <span>Culturel: {(entity.cultural_territorial_loss_score ?? 0).toFixed(1)}</span>
                <span>Compensation: {(entity.compensation_mechanism_absence_score ?? 0).toFixed(1)}</span>
                <span>Impunité: {(entity.historical_emitter_impunity_score ?? 0).toFixed(1)}</span>
              </div>
            </button>
          ))}
        </div>
