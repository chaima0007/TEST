...
interface Entity {
  ...
  last_updated: string;
  pension_scam_exploitation_score?: number;
  caregiver_financial_fraud_score?: number;
  estate_theft_coercion_score?: number;
  elder_financial_isolation_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "pension_scam_exploitation_score", label: "Escroquerie pension retraite" },
  { key: "caregiver_financial_fraud_score", label: "Fraude financière aidants" },
  { key: "estate_theft_coercion_score", label: "Vol succession coercitif" },
  { key: "elder_financial_isolation_score", label: "Isolation financière aînés" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de l&apos;abus financier des aînés : escroqueries aux pensions, fraudes des aidants, vol coercitif de succession et isolation financière des personnes âgées.</p>
              <p className="text-slate-400">Source : Elder Financial Abuse Rights Engine · Wave 263</p>
            </div>
          )}
        </div>
      </div>
...
export default function ElderFinancialAbuseRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/elder-financial-abuse-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Elder Financial Abuse Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Pensions · Aidants Frauduleux · Successions · Isolation</p>
        </div>
