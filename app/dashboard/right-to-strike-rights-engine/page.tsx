...
interface Entity {
  ...
  last_updated: string;
  strike_ban_score?: number;
  essential_services_abuse_score?: number;
  strike_retaliation_score?: number;
  collective_action_restriction_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "strike_ban_score", label: "Interdiction de grève" },
  { key: "essential_services_abuse_score", label: "Abus de services essentiels" },
  { key: "strike_retaliation_score", label: "Représailles contre les grévistes" },
  { key: "collective_action_restriction_score", label: "Restriction de l&apos;action collective" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des violations du droit de grève : interdictions légales, abus de la notion de services essentiels, représailles contre les grévistes et restrictions à l&apos;action collective.</p>
              <p className="text-slate-400">Source : Right to Strike Rights Engine · Wave 245</p>
            </div>
          )}
        </div>
      </div>
...
export default function RightToStrikeRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/right-to-strike-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Right to Strike Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Interdiction de Grève · Abus Services Essentiels · Représailles · Restriction Action Collective</p>
        </div>
