...
interface Entity {
  ...
  last_updated: string;
  organ_trade_prevalence_score?: number;
  transplant_tourism_score?: number;
  donor_coercion_score?: number;
...
const SUB_SCORES = [
  { key: "organ_trade_prevalence_score", label: "Prévalence du trafic d&apos;organes" },
  { key: "transplant_tourism_score", label: "Tourisme de transplantation" },
  { key: "donor_coercion_score", label: "Coercition des donneurs" },
  { key: "legal_enforcement_gap_score", label: "Lacune d&apos;application juridique" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des violations des droits liées au trafic d&apos;organes : commerce illicite, tourisme de transplantation, coercition des donneurs et lacunes d&apos;application du cadre juridique de protection.</p>
              <p className="text-slate-400">Source : Organ Trafficking Rights Engine · Wave 245</p>
            </div>
          )}
        </div>
      </div>
...
export default function OrganTraffickingRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/organ-trafficking-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Organ Trafficking Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Commerce d&apos;Organes · Tourisme de Transplantation · Coercition des Donneurs · Application Juridique</p>
        </div>
