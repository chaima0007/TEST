...
interface Entity {
  ...
  toxic_exposure_community_health_rights_score: number;
  land_displacement_indigenous_rights_score: number;
  labor_exploitation_safety_deficit_score: number;
  environmental_accountability_governance_gap_score: number;
  estimated_rare_earth_mining_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_rare_earth_mining_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Droits Santé Communauté & Exposition Toxique", value: entity.toxic_exposure_community_health_rights_score, weight: "0.30" },
    { label: "Droits Autochtones & Déplacement Foncier", value: entity.land_displacement_indigenous_rights_score, weight: "0.25" },
    { label: "Exploitation Travail & Déficit Sécurité", value: entity.labor_exploitation_safety_deficit_score, weight: "0.25" },
    { label: "Lacune Gouvernance & Responsabilité Environnementale", value: entity.environmental_accountability_governance_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_rare_earth_mining_rights_index === "number" ? entity.estimated_rare_earth_mining_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Mines Terres Rares</div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-300 font-mono">
                  Score = Exposition×0.30 + Déplacement×0.25 + Exploitation×0.25 + Gouvernance×0.20
                </div>
              </div>
...
export default function RareEarthMiningRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/rare-earth-mining-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits dans les Mines de Terres Rares…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_rare_earth_mining_rights_index ?? avg(allEntities.map(e => e.estimated_rare_earth_mining_rights_index));
  ...
  const avgExposition = avg(allEntities.map(e => e.toxic_exposure_community_health_rights_score));
  const avgDeplacement = avg(allEntities.map(e => e.land_displacement_indigenous_rights_score));
  const avgExploitation = avg(allEntities.map(e => e.labor_exploitation_safety_deficit_score));
  const avgGouvernance = avg(allEntities.map(e => e.environmental_accountability_governance_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Mines Terres Rares", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits dans les Mines de Terres Rares</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Rare Earth Mining Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgExposition} label="Exposition Toxique" />
          <GaugeRing value={avgDeplacement} label="Déplacement Autochtone" />
          <GaugeRing value={avgExploitation} label="Exploitation Travail" />
          <GaugeRing value={avgGouvernance} label="Gouvernance Environnementale" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Mines Terres Rares: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_rare_earth_mining_rights_index === "number" ? e.estimated_rare_earth_mining_rights_index.toFixed(2) : "—"}</span>
            </div>
