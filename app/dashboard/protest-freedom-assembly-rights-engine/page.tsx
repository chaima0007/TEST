...
export default function ProtestFreedomAssemblyRightsPage() {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  useEffect(() => {
    fetch("/api/protest-freedom-assembly-rights-engine")
      .then((r) => r.json())
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-6">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: ACCENT }}>
            Liberté de Réunion &amp; Répression des Manifestations
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            Criminalisation protestation, arrestations massives &amp; impunité forces sécurité
          </p>
        </div>
