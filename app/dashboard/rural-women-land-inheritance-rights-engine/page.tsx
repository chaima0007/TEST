...
interface Entity {
  ...
  land_title_inheritance_denial_severity_score: number;
  customary_law_gender_discrimination_scale_score: number;
  agricultural_credit_market_access_barriers_score: number;
  legal_protection_enforcement_deficit_gap_score: number;
  estimated_rural_women_land_inheritance_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_rural_women_land_inheritance_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déni Titre Foncier & Héritage", value: entity.land_title_inheritance_denial_severity_score, weight: "0.30" },
    { label: "Droit Coutumier & Discrimination Genre", value: entity.customary_law_gender_discrimination_scale_score, weight: "0.25" },
    { label: "Accès Crédit Agricole & Marché", value: entity.agricultural_credit_market_access_barriers_score, weight: "0.25" },
    { label: "Déficit Protection Juridique & Application", value: entity.legal_protection_enforcement_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_rural_women_land_inheritance_rights_index === "number" ? entity.estimated_rural_women_land_inheritance_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Fonciers Femmes</div>
                </div>
              </div>
...
export default function RuralWomenLandInheritanceRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/rural-women-land-inheritance-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Fonciers &amp; Héritage — Femmes Rurales…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_rural_women_land_inheritance_rights_index ?? avg(allEntities.map(e => e.estimated_rural_women_land_inheritance_rights_index));
  ...
  const avgLandTitle = avg(allEntities.map(e => e.land_title_inheritance_denial_severity_score));
  const avgCustomary = avg(allEntities.map(e => e.customary_law_gender_discrimination_scale_score));
  const avgAgricultural = avg(allEntities.map(e => e.agricultural_credit_market_access_barriers_score));
  const avgLegal = avg(allEntities.map(e => e.legal_protection_enforcement_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Fonciers Femmes", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Fonciers &amp; Héritage — Femmes Rurales</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          FAO · ONU Femmes · Protocole africain · Accès terre 500M femmes rurales
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgLandTitle} label="Déni Titre Foncier" />
          <GaugeRing value={avgCustomary} label="Droit Coutumier" />
          <GaugeRing value={avgAgricultural} label="Accès Crédit Agricole" />
          <GaugeRing value={avgLegal} label="Protection Juridique" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Fonciers Femmes: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_rural_women_land_inheritance_rights_index === "number" ? e.estimated_rural_women_land_inheritance_rights_index.toFixed(2) : "—"}</span>
            </div>
