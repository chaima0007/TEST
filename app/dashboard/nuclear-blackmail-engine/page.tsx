...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/nuclear-blackmail-engine").then(r => r.json()).then(j => setData(j.payload ?? j));
  }, []);
  ...
  const PATTERN_LABELS: Record<string, string> = {
    menace_nucleaire_coercitive: "Menace Nucléaire Coercitive",
    proliferation_exportation_clandestine: "Prolifération Clandestine",
    non_conformite_traites_nucleaires: "Non-Conformité TNP",
    destabilisation_dissuasion: "Déstabilisation Dissuasion",
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Nuclear Blackmail Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Surveillance du chantage nucléaire, prolifération et déstabilisation stratégique</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    {["coercive_nuclear_threat_score", "proliferation_export_risk_score", "treaty_noncompliance_violation_score", "deterrence_destabilization_score"].map(key => (
