...
interface NDEntity {
  ...
  composite_score: number; nuclear_arsenal_modernization_score: number;
  second_strike_capability_score: number; nuclear_doctrine_escalation_score: number;
  npt_violation_risk_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_nuclear_deterrence_index: number; last_updated: string;
}
interface NDSummary {
  ...
  entities: NDEntity[]; avg_estimated_nuclear_deterrence_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: NDEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Modernisation Arsenal Nucléaire", v: entity.nuclear_arsenal_modernization_score, c: "#f59e0b" },
              { l: "Capacité Second Strike", v: entity.second_strike_capability_score, c: "#d97706" },
              { l: "Escalade Doctrine Nucléaire", v: entity.nuclear_doctrine_escalation_score, c: "#b45309" },
              { l: "Risque Violation TNP", v: entity.npt_violation_risk_score, c: "#92400e" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Dissuasion Nucléaire Estimé</span>
              <span className="text-amber-400 font-bold">{entity.estimated_nuclear_deterrence_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Saisine d'urgence de l'AIEA et activation des mécanismes de vérification TNP</p><p>› Coordination immédiate avec les puissances P5 pour sanctions ciblées et gel des programmes</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement de la surveillance satellitaire des sites nucléaires suspects</p><p>› Activation des canaux diplomatiques bilatéraux pour désescalade stratégique</p></>}
            {entity.risk_level === "modéré" && <><p>› Surveillance accrue des transferts de technologie à double usage</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien de la veille sur les doctrines nucléaires et exercices stratégiques</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function NuclearDeterrencePage() {
  const [data, setData] = useState<NDSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<NDEntity | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/nuclear-deterrence-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données dissuasion nucléaire"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse des arsenaux nucléaires mondiaux…</div></div>;
  ...
  const avgScore = (k: keyof NDEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Nuclear Deterrence Intelligence Engine</h1>
          <p className="text-slate-400 text-sm">Cartographie des arsenaux nucléaires et doctrines de dissuasion stratégique · v{data.engine_version}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Sources de Données", v: data.data_sources.length, g: "from-amber-900 to-amber-800", t: "text-amber-300" },
            { l: "Confiance Analyse", v: `${Math.round(data.confidence_score * 100)}%`, g: "from-slate-800 to-slate-700", t: "text-slate-300" },
            { l: "Index Dissuasion Moyen", v: data.avg_estimated_nuclear_deterrence_index, g: "from-amber-900 to-amber-800", t: "text-amber-300" },
          ].map(({ l, v, g, t }) => (
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs de Dissuasion Nucléaire Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("nuclear_arsenal_modernization_score")} label="Modernisation Arsenal" color="#f59e0b" />
            <GaugeRing value={avgScore("second_strike_capability_score")} label="Second Strike" color="#d97706" />
            <GaugeRing value={avgScore("nuclear_doctrine_escalation_score")} label="Escalade Doctrine" color="#b45309" />
            <GaugeRing value={avgScore("npt_violation_risk_score")} label="Violation TNP" color="#92400e" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Dissuasion Nucléaire</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-amber-900/10 border border-amber-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-amber-400 mb-2">Alertes Nucléaires Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_nuclear_deterrence_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Nuclear Deterrence Intelligence Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
