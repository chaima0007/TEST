...
interface NPREntity {
  ...
  forced_sedentarization_displacement_scale_score: number;
  legal_recognition_land_rights_gap_score: number;
  discrimination_service_access_denial_score: number;
  cultural_identity_assimilation_pressure_score: number;
  estimated_nomadic_peoples_rights_index: number;
  risk_level: string;
...
interface NPRData {
  ...
  data_sources: string[];
  entities: NPREntity[];
  avg_estimated_nomadic_peoples_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function NomadicPeoplesRightsPage() {
  const [data, setData] = useState<NPRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<NPREntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/nomadic-peoples-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-orange-400 animate-pulse">Chargement des données peuples nomades…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "forced_sedentarization_displacement_scale_score", label: "Sédentarisation Forcée" },
    { key: "legal_recognition_land_rights_gap_score", label: "Reconnaissance Droits Fonciers" },
    { key: "discrimination_service_access_denial_score", label: "Discrimination & Exclusion" },
    { key: "cultural_identity_assimilation_pressure_score", label: "Pression Assimilation Culturelle" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits des Peuples Nomades</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Nomadic Peoples Rights Engine</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_nomadic_peoples_rights_index ?? "—" },
            { label: "Confidence", value: `${Math.round((data.confidence_score ?? 0) * 100)}%` },
          ].map(k => (
            <div key={k.label} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              <div className="text-xs text-slate-500 mb-1">{k.label}</div>
              <div className="text-xl font-bold text-orange-400">{k.value}</div>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#fb923c" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Violation</h2>
            <div className="space-y-2">
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#fb923c" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_nomadic_peoples_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droits des Peuples Nomades · {data.last_analysis}</p>
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Vue générale", "Sous-scores", "Alertes"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`px-6 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-orange-500 text-orange-400" : "text-slate-500 hover:text-slate-300"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#fb923c" />
                    <div>
                      <div className="text-2xl font-bold text-orange-400">{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Droits Peuples Nomades</div>
                    <div className="text-lg font-bold text-orange-400">{selected.estimated_nomadic_peoples_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
