...
interface Entity {
  ...
  estimated_gender_pay_gap_rights_index: number;
  [key: string]: unknown;
}
        ...
        <div style={{ padding: 24 }}>
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ color: "#94a3b8", fontSize: 14 }}>Indice Écart Salarial Estimé</span>
                <span style={{ fontSize: 20, fontWeight: 700, color: ACCENT_LIGHT }}>{typeof entity.estimated_gender_pay_gap_rights_index === "number" ? entity.estimated_gender_pay_gap_rights_index.toFixed(2) : "—"}</span>
              </div>
          ...
          {tab === "metriques" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                ...
                { label: "Indice GPG Droits Estimé", value: (entity.estimated_gender_pay_gap_rights_index as number) * 10 },
              ].map(s => (
                <div key={s.label}>
          ...
          {tab === "sources" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {["ilo_gender_pay_gap_report_2024", "wef_global_gender_gap_index_2024", "oxfam_womens_economic_rights_2024", "eu_gender_equality_index_2024"].map(s => (
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/gender-pay-gap-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#020617", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: ACCENT_LIGHT, fontSize: 14 }}>Initialisation Gender Pay Gap Rights Engine…</div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "#f1f5f9", padding: 24 }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 800, color: ACCENT_LIGHT, margin: 0 }}>Gender Pay Gap Rights Engine</h1>
        </div>
        <p style={{ color: "#64748b", fontSize: 14, margin: 0, paddingLeft: 16 }}>OIT · WEF · Oxfam · Égalité Salariale · Droits Économiques Femmes</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 16, marginBottom: 32 }}>
        {[
          ...
          { label: "Indice GPG", value: String(data?.estimated_gender_pay_gap_rights_index ?? 6.13) },
          { label: "Confiance", value: `${Math.round((data?.confidence_score ?? 0.86) * 100)}%` },
        ].map(k => (
        ...
        <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <GaugeRing value={data?.avg_composite ?? 61.33} />
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap" }}>
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: "6px 16px", borderRadius: 999, fontSize: 13, fontWeight: 500, cursor: "pointer", border: `1px solid ${filter === f ? ACCENT_LIGHT : "#334155"}`, background: filter === f ? "rgba(192,132,252,0.1)" : "transparent", color: filter === f ? ACCENT_LIGHT : "#94a3b8" }}>
          ...
          return (
            <button key={e.id} onClick={() => setSelected(e)}
              ...
              <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>
                Indice GPG: <span style={{ color: ACCENT_LIGHT, fontWeight: 600 }}>{typeof e.estimated_gender_pay_gap_rights_index === "number" ? e.estimated_gender_pay_gap_rights_index.toFixed(2) : "—"}</span>
              </div>
            </button>
          );
        })}
