...
interface Entity {
  ...
  safe_water_access_deprivation_severity_score: number;
  water_privatisation_corporate_capture_scale_score: number;
  indigenous_water_rights_violation_score: number;
  sanitation_hygiene_exclusion_gap_score: number;
  estimated_water_rights_access_index: number;
  risk_level: string;
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_water_rights_access_index: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "safe_water_access_deprivation_severity_score", label: "Déprivation Accès Eau Sûre" },
  { key: "water_privatisation_corporate_capture_scale_score", label: "Privatisation & Capture Corporate" },
  { key: "indigenous_water_rights_violation_score", label: "Violations Droits Eau Autochtones" },
  { key: "sanitation_hygiene_exclusion_gap_score", label: "Gap Assainissement & Hygiène" },
];
        ...
        <div className="flex border-b border-slate-800">
          {["Aperçu", "Métriques", "Sources"].map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-cyan-600/50 text-cyan-600" : "text-slate-400 hover:text-white"}`}>
              {t}
            </button>
          ))}
        </div>
          ...
          {tab === 0 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Indice Droits Accès à l&apos;Eau</span>
                <span className="font-semibold" style={{ color: ACCENT }}>{entity.estimated_water_rights_access_index}/10</span>
              </div>
              <div className="flex items-center justify-between text-sm">
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/water-rights-access-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Water Rights Access Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Accès Eau Sûre · Privatisation · Droits Autochtones · Assainissement</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice WRA</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_water_rights_access_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "bg-cyan-600/10 border-cyan-600/30 text-cyan-600"
        ...
        {filtered.map(e => {
          ...
          return (
            <button key={e.entity_id} onClick={() => setSelected(e)}
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold uppercase ${cfg.color}`}>{cfg.label}</span>
                <span className="text-xs text-slate-500">Indice: <span className="font-bold" style={{ color: ACCENT }}>{e.estimated_water_rights_access_index}</span></span>
              </div>
            </button>
          );
        })}
