...
interface Entity {
  ...
  last_updated: string;
  brain_data_commodification_score?: number;
  neurotech_coercion_score?: number;
  cognitive_autonomy_violation_score?: number;
  nonconsensual_brain_scanning_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "brain_data_commodification_score", label: "Marchandisation données cérébrales" },
  { key: "neurotech_coercion_score", label: "Coercition neurotechnologique" },
  { key: "cognitive_autonomy_violation_score", label: "Violation autonomie cognitive" },
  { key: "nonconsensual_brain_scanning_score", label: "Scan cérébral non-consenti" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de la neuroéthique : marchandisation des données cérébrales, coercition neurotechnologique, violations de l&apos;autonomie cognitive et scans cérébraux non-consentis.</p>
              <p className="text-slate-400">Source : Neuroethics Rights Engine · Wave 259</p>
            </div>
          )}
        </div>
      </div>
...
export default function NeuroethicsRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/neuroethics-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Neuroethics Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Données Cérébrales · Neurotechnologie · Autonomie Cognitive · Consentement</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#8b5cf620" } : {}}>
