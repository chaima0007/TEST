...
interface Entity {
  ...
  labor_sex_exploitation_scale_score: number;
  border_recruitment_deception_score: number;
  victim_identification_failure_score: number;
  prosecution_conviction_gap_score: number;
  estimated_human_trafficking_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_human_trafficking_index: number;
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/human-trafficking-engine")
      .then(r => r.json())
  ...
  if (!data) return (
    <div className="bg-slate-950 min-h-screen flex items-center justify-center">
      <div className="text-orange-900 animate-pulse">Initialisation du Moteur Traite des Êtres Humains…</div>
    </div>
  );
  ...
  const confidence = typeof data.confidence_score === "number"
    ...
    : "—";
  const avgIndex = typeof data.avg_estimated_human_trafficking_index === "number"
    ? data.avg_estimated_human_trafficking_index.toFixed(2)
    : "—";
  ...
  const kpis = [
    ...
    { label: "Index Traite", value: avgIndex },
    ...
    <div className="bg-slate-950 min-h-screen text-slate-100 p-6 space-y-6">
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Traite des Êtres Humains</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Exploitation par le travail et le sexe, recrutement frauduleux, défaillances d&apos;identification et lacunes de poursuites
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {kpis.map(k => (
          <div key={k.label} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            <div className="text-xl font-bold" style={{ color: "#c2410c" }}>{k.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {(() => {
            ...
            return [
              { label: "Exploitation Travail & Sexe", key: "labor_sex_exploitation_scale_score" as keyof Entity },
              { label: "Recrutement Frontalier", key: "border_recruitment_deception_score" as keyof Entity },
              { label: "Échec Identification Victimes", key: "victim_identification_failure_score" as keyof Entity },
              { label: "Lacune Poursuites", key: "prosecution_conviction_gap_score" as keyof Entity },
            ].map(s => (
        ...
        {entities.map(e => (
          <div key={e.entity_id} onClick={() => { setSelected(e); setTab(0); }}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Traite: <span className="font-medium" style={{ color: "#c2410c" }}>{e.estimated_human_trafficking_index}</span>
            </div>
          </div>
        ))}
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Aperçu", "Signaux", "Contexte"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  ...
                  style={tab === i ? { borderColor: ACCENT, color: "#c2410c" } : {}}>
                  {t}
                </button>
              ))}
            </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="ml-auto text-right">
                      <div className="text-lg font-bold" style={{ color: "#c2410c" }}>{selected.estimated_human_trafficking_index}</div>
                      <div className="text-xs text-slate-500">Index Traite</div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { label: "Exploitation Travail & Sexe", value: selected.labor_sex_exploitation_scale_score },
                      { label: "Recrutement Frontalier", value: selected.border_recruitment_deception_score },
                      { label: "Échec Identification Victimes", value: selected.victim_identification_failure_score },
                      { label: "Lacune Poursuites", value: selected.prosecution_conviction_gap_score },
                    ].map(s => (
                ...
                <ul className="space-y-3">
                  {(selected.key_signals ?? []).map((s, i) => (
                    <li key={i} className="flex gap-2 text-sm text-slate-300">
                      <span style={{ color: "#c2410c" }} className="shrink-0 mt-0.5">▸</span>
                      {s}
                    </li>
                  ))}
                </ul>
