...
type RPEntity = {
  ...
  state_religious_violence_score: number;
  blasphemy_apostasy_criminalization_score: number;
  minority_institutional_exclusion_score: number;
  religious_persecution_impunity_score: number;
          ...
          {tab === "scores" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>
                    {entity.estimated_religious_persecution_index.toFixed(1)}
                  </div>
                  <div className="text-xs text-slate-400 mt-1">Index Persécution Religieuse</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Violence Religieuse d'État", value: entity.state_religious_violence_score },
                  { label: "Criminalisation Blasphème & Apostasie", value: entity.blasphemy_apostasy_criminalization_score },
                  { label: "Exclusion Institutionnelle Minorités", value: entity.minority_institutional_exclusion_score },
                  { label: "Impunité Persécution Religieuse", value: entity.religious_persecution_impunity_score },
                ].map((s) => (
...
export default function ReligiousPersecutionEnginePage() {
  const [entities, setEntities] = useState<RPEntity[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    fetch("/api/religious-persecution-engine")
      .then((r) => r.json())
      .then((json: ApiResponse) => {
        const rows = json?.data ?? (Array.isArray(json) ? json : []);
        setEntities(rows as RPEntity[]);
        setLoading(false);
      })
      .catch(() => {
  ...
  const avgStateViolence = avg(entities.map((e) => e.state_religious_violence_score));
  const avgBlasphemy = avg(entities.map((e) => e.blasphemy_apostasy_criminalization_score));
  const avgExclusion = avg(entities.map((e) => e.minority_institutional_exclusion_score));
  const avgImpunity = avg(entities.map((e) => e.religious_persecution_impunity_score));
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-white">Persécution Religieuse Engine</h1>
        </div>
        <p className="text-slate-400 ml-5">
          Minorités Confessionnelles &amp; Lois Blasphème Létales — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 rounded-2xl p-6 mb-8" style={{ border: `1px solid ${ACCENT}1a` }}>
        ...
        <div className="flex flex-wrap justify-around gap-6">
          <GaugeRing value={avgStateViolence} label="Violence Religieuse État" color={ACCENT} />
          <GaugeRing value={avgBlasphemy} label="Blasphème & Apostasie" color="#7c3aed" />
          <GaugeRing value={avgExclusion} label="Exclusion Minorités" color="#0891b2" />
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs text-slate-500">Index RPE</div>
                </div>
              </div>
              <div
