...
interface PVEntity {
  ...
  composite_score: number; succession_crisis_score: number;
  institutional_vacuum_score: number;
  proxy_war_attraction_score: number;
  regional_destabilization_score: number;
  risk_level: string; primary_pattern: string; key_signals: string[];
  estimated_vacuum_index: number; last_updated: string;
}
interface PVSummary {
  ...
  confidence_score: number; data_sources: string[]; entities: PVEntity[];
  avg_estimated_vacuum_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: PVEntity; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-slate-900 border border-orange-900/40 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Crise de Succession", v: entity.succession_crisis_score, c: "#f97316" },
              { l: "Vide Institutionnel", v: entity.institutional_vacuum_score, c: "#fb923c" },
              { l: "Attrait Guerre Proxy", v: entity.proxy_war_attraction_score, c: "#fdba74" },
              { l: "Déstabilisation Régionale", v: entity.regional_destabilization_score, c: "#ea580c" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Indice Vide de Pouvoir Caelum</span>
              <span className="font-bold" style={{ color: ACCENT }}>{entity.estimated_vacuum_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <p>› Engagement diplomatique d&apos;urgence — médiation internationale et stabilisation</p>}
            {entity.risk_level === "élevé" && <p>› Diplomatie préventive et cadres régionaux de sécurité collective</p>}
            {entity.risk_level === "modéré" && <p>› Accompagnement de la transition et renforcement des institutions</p>}
            {entity.risk_level === "faible" && <p>› Maintien des institutions démocratiques et mécanismes de transfert pacifique</p>}
          </div>
        )}
      </div>
    </div>
...
const PATTERN_COLORS: Record<string, string> = {
  vide_catastrophique: "#ef4444",
  guerre_succession: "#f97316",
  competition_hegemonique: "#eab308",
  transition_fragile: "#f59e0b",
  succession_institutionnalisee: "#10b981",
};

export default function PowerVacuumEnginePage() {
  const [data, setData] = useState<PVSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<PVEntity | null>(null);

  useEffect(() => {
    fetch("/api/power-vacuum-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données vides de pouvoir géopolitiques"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse des vides de pouvoir géopolitiques en cours…</div>
    </div>
  );
  ...
  const avgScore = (k: keyof PVEntity) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Vides de Pouvoir Géopolitiques</h1>
          <p className="text-slate-400 text-sm mt-1">Crises de succession et vides hégémoniques — les incubateurs des conflits par procuration de la prochaine décennie</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Indice Vide de Pouvoir", v: data.avg_estimated_vacuum_index, t: "text-orange-400" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-orange-500/20">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs de Vide de Pouvoir — Scores Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("succession_crisis_score")} label="Crise de Succession" color={ACCENT} />
            <GaugeRing value={avgScore("institutional_vacuum_score")} label="Vide Institutionnel" color="#fb923c" />
            <GaugeRing value={avgScore("proxy_war_attraction_score")} label="Attrait Guerre Proxy" color="#fdba74" />
            <GaugeRing value={avgScore("regional_destabilization_score")} label="Déstabilisation Régionale" color="#ea580c" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-orange-500/20">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Vide de Pouvoir</h2>
            <div className="space-y-2">
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-orange-900/10 border border-orange-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold mb-2" style={{ color: ACCENT }}>Alertes Vides de Pouvoir Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_vacuum_index}</span>
              </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Vides de Pouvoir Géopolitiques · {data.last_analysis}</p>
      </div>
    </div>
  );
}
