...
interface Entity {
  ...
  hrd_killings_disappearances_score: number;
  impunity_accountability_deficit_score: number;
  reprisals_intimidation_scale_score: number;
  protection_mechanism_gap_score: number;
  estimated_human_rights_defenders_assassination_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_human_rights_defenders_assassination_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Meurtres &amp; Disparitions DH", value: entity.hrd_killings_disappearances_score, weight: "0.30" },
    { label: "Déficit Redevabilité &amp; Impunité", value: entity.impunity_accountability_deficit_score, weight: "0.25" },
    { label: "Représailles &amp; Intimidation", value: entity.reprisals_intimidation_scale_score, weight: "0.25" },
    { label: "Lacune Mécanismes Protection", value: entity.protection_mechanism_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_human_rights_defenders_assassination_index === "number" ? entity.estimated_human_rights_defenders_assassination_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Assassinats DH</div>
                </div>
              </div>
...
export default function HumanRightsDefendersAssassinationPage() {
  ...
  useEffect(() => {
    fetch("/api/human-rights-defenders-assassination-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Assassinats Défenseurs Droits Humains…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_human_rights_defenders_assassination_index
    ?? (Object.entries(data ?? {}).find(([k]) => k.startsWith("avg_estimated_"))?.[1] as number)
    ?? avg(allEntities.map(e => e.estimated_human_rights_defenders_assassination_index));
  ...
  const avgKillings = avg(allEntities.map(e => e.hrd_killings_disappearances_score));
  const avgImpunity = avg(allEntities.map(e => e.impunity_accountability_deficit_score));
  const avgReprisals = avg(allEntities.map(e => e.reprisals_intimidation_scale_score));
  const avgProtection = avg(allEntities.map(e => e.protection_mechanism_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Assassinats DH", value: typeof avgIndex === "number" ? avgIndex.toFixed(2) : "—" },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Assassinats de Défenseurs des Droits Humains</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Protection HRD · Représailles · Impunité · Front Line Defenders — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgKillings} label="Meurtres & Disparitions" />
          <GaugeRing value={avgImpunity} label="Impunité & Redevabilité" />
          <GaugeRing value={avgReprisals} label="Représailles" />
          <GaugeRing value={avgProtection} label="Lacune Protection" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Assassinats DH: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_human_rights_defenders_assassination_index === "number" ? e.estimated_human_rights_defenders_assassination_index.toFixed(2) : "—"}</span>
            </div>
