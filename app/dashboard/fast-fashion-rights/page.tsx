...
interface Entity {
  ...
  textile_pollution_score?: number;
  wage_theft_score?: number;
  overconsumption_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "labor_exploitation_score", label: "Exploitation des travailleurs" },
  { key: "textile_pollution_score", label: "Pollution textile" },
  { key: "wage_theft_score", label: "Vol de salaires" },
  { key: "overconsumption_score", label: "Surconsommation" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits contre la fast fashion : exploitation des travailleurs du textile, pollution industrielle, vol de salaires dans les chaînes d&apos;approvisionnement et surconsommation forcée affectant les populations vulnérables.</p>
              <p className="text-slate-400">Source : Fast Fashion Rights Engine · Wave 270</p>
            </div>
          )}
        </div>
      </div>
...
export default function FastFashionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/fast-fashion-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits contre la Fast Fashion</h1>
          <p className="text-slate-400 mt-1 text-sm">Exploitation des Travailleurs · Pollution Textile · Vol de Salaires · Surconsommation</p>
        </div>
