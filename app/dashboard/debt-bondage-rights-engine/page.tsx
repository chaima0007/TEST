...
interface Entity {
  ...
  bonded_labor_debt_coercion_severity_score: number;
  migrant_debt_recruitment_exploitation_scale_score: number;
  legal_protection_debt_bondage_absence_score: number;
  corporate_supply_chain_debt_bondage_gap_score: number;
  estimated_debt_bondage_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_debt_bondage_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Travail Lié Dette", value: entity.bonded_labor_debt_coercion_severity_score, weight: "0.30" },
    { label: "Dette Recrutement Migrant", value: entity.migrant_debt_recruitment_exploitation_scale_score, weight: "0.25" },
    { label: "Protection Légale", value: entity.legal_protection_debt_bondage_absence_score, weight: "0.25" },
    { label: "Chaîne Approvisionnement", value: entity.corporate_supply_chain_debt_bondage_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_debt_bondage_rights_index === "number" ? entity.estimated_debt_bondage_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Servitude Dette</div>
                </div>
              </div>
...
export default function DebtBondageRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/debt-bondage-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Servitude Dette…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_debt_bondage_rights_index ?? avg(allEntities.map(e => e.estimated_debt_bondage_rights_index));
  ...
  const avgBondedLabor = avg(allEntities.map(e => e.bonded_labor_debt_coercion_severity_score));
  const avgMigrantDebt = avg(allEntities.map(e => e.migrant_debt_recruitment_exploitation_scale_score));
  const avgLegalProtection = avg(allEntities.map(e => e.legal_protection_debt_bondage_absence_score));
  const avgSupplyChain = avg(allEntities.map(e => e.corporate_supply_chain_debt_bondage_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Servitude Dette", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Debt Bondage Rights Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Travail lié par dette, exploitation des migrants et lacunes de protection légale — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgBondedLabor} label="Travail Lié Dette" />
          <GaugeRing value={avgMigrantDebt} label="Dette Recrutement Migrant" />
          <GaugeRing value={avgLegalProtection} label="Protection Légale" />
          <GaugeRing value={avgSupplyChain} label="Chaîne Approvisionnement" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Servitude Dette: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_debt_bondage_rights_index === "number" ? e.estimated_debt_bondage_rights_index.toFixed(2) : "—"}</span>
            </div>
