...
interface SREEntity {
  ...
  historical_atrocity_scale_score: number;
  intergenerational_harm_persistence_score: number;
  restitution_refusal_entrenchment_score: number;
  institutional_accountability_gap_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_slavery_reparations_index: number;
  last_updated: string;
  [key: string]: unknown;
}
...
interface SRESummary {
  ...
  avg_estimated_slavery_reparations_index: number;
...
function DetailModal({ entity, onClose }: { entity: SREEntity; onClose: () => void }) {
  ...
  const scores = [
    { label: "Atrocité Historique", key: "historical_atrocity_scale_score" },
    { label: "Préjudice Intergénérationnel", key: "intergenerational_harm_persistence_score" },
    { label: "Refus Restitution", key: "restitution_refusal_entrenchment_score" },
    { label: "Gap Institutionnel", key: "institutional_accountability_gap_score" },
  ];
  return (
    <div
    ...
    >
      <div
        className="bg-slate-900 border border-rose-500/20 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl"
        ...
        <div className="p-6">
          {tab === 0 && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>
                    {entity.estimated_slavery_reparations_index.toFixed(1)}
                  </div>
                  <div className="text-xs text-slate-400 mt-1">Index Réparations</div>
                </div>
              </div>
          ...
          {tab === 1 && (
            <div className="flex flex-col items-center gap-4 py-4">
              <GaugeRing value={entity.estimated_slavery_reparations_index} max={10} />
              <div className="text-center">
                <div className="text-3xl font-bold" style={{ color: ACCENT }}>
                  {entity.estimated_slavery_reparations_index.toFixed(2)}
                </div>
                <div className="text-sm text-slate-500 mt-1">Index Réparations</div>
              </div>
            ...
            <div className="space-y-3">
              <div className="rounded-lg border border-slate-700/40 bg-slate-800/50 p-4">
                <p className="text-sm leading-relaxed text-slate-400">
                  Analyse de la dette historique et de la justice réparatrice pour{" "}
...
export default function SlaveryReparationsEnginePage() {
  const [data, setData] = useState<SREData>({});
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<SREEntity | null>(null);

  useEffect(() => {
    fetch("/api/slavery-reparations-engine")
      .then((r) => r.json())
      .then((d) => { setData(d.payload ?? d); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);

  const summary = (data as { summary?: SRESummary }).summary;
  const rawEntities: SREEntity[] = (data as { entities?: SREEntity[] }).entities ?? summary?.entities ?? [];

  const PATTERN_KEYS = [
    "historical_atrocity_scale",
    "intergenerational_harm_persistence",
    "restitution_refusal_entrenchment",
  ...
  const avgIndex = avg(rawEntities.map((e) => e.estimated_slavery_reparations_index));
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h1 className="text-2xl font-bold tracking-tight">Slavery Reparations Engine</h1>
            </div>
            <p className="text-slate-400 text-sm ml-6">
              Analyse des réparations pour l&apos;esclavage et les crimes coloniaux — dette historique &amp; justice réparatrice
            </p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {kpis.map((kpi) => (
            ...
            >
              ...
              <div className="mt-2 text-2xl font-bold text-rose-400">{kpi.value}</div>
            </div>
          ))}
        </div>
            ...
            {entities.map((entity) => (
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="text-xs text-slate-500">Index Réparations</span>
                ...
                <div className="mt-2 space-y-1">
                  {[
                    { key: "historical_atrocity_scale_score", label: "Atrocité Historique" },
                    { key: "intergenerational_harm_persistence_score", label: "Préjudice Intergénérationnel" },
                    { key: "restitution_refusal_entrenchment_score", label: "Refus Restitution" },
                    { key: "institutional_accountability_gap_score", label: "Gap Institutionnel" },
                  ].map(({ key, label }) => (
