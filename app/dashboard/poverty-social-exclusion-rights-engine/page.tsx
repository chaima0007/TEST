...
interface Entity {
  ...
  extreme_poverty_essential_services_denial_severity_score: number;
  social_protection_floor_absence_scale_score: number;
  homelessness_vagrancy_criminalization_score: number;
  poverty_trap_structural_inequality_redress_deficit_gap_score: number;
  estimated_poverty_social_exclusion_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_poverty_social_exclusion_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Pauvreté Extrême & Déni Services", value: entity.extreme_poverty_essential_services_denial_severity_score, weight: "0.30" },
    { label: "Absence Plancher Protection Sociale", value: entity.social_protection_floor_absence_scale_score, weight: "0.25" },
    { label: "Criminalisation Sans-Abri", value: entity.homelessness_vagrancy_criminalization_score, weight: "0.25" },
    { label: "Pièges Pauvreté & Inégalités", value: entity.poverty_trap_structural_inequality_redress_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_poverty_social_exclusion_rights_index === "number" ? entity.estimated_poverty_social_exclusion_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Pauvreté & Exclusion</div>
                </div>
              </div>
...
export default function PovertySocialExclusionRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/poverty-social-exclusion-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits contre Pauvreté & Exclusion Sociale…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_poverty_social_exclusion_rights_index ?? avg(allEntities.map(e => e.estimated_poverty_social_exclusion_rights_index));
  ...
  const avgExtremePoverty = avg(allEntities.map(e => e.extreme_poverty_essential_services_denial_severity_score));
  const avgSocialProtection = avg(allEntities.map(e => e.social_protection_floor_absence_scale_score));
  const avgHomelessness = avg(allEntities.map(e => e.homelessness_vagrancy_criminalization_score));
  const avgPovertyTrap = avg(allEntities.map(e => e.poverty_trap_structural_inequality_redress_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Pauvreté & Exclusion", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits contre Pauvreté & Exclusion Sociale</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Poverty & Social Exclusion Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgExtremePoverty} label="Pauvreté Extrême & Déni Services" />
          <GaugeRing value={avgSocialProtection} label="Absence Protection Sociale" />
          <GaugeRing value={avgHomelessness} label="Criminalisation Sans-Abri" />
          <GaugeRing value={avgPovertyTrap} label="Pièges Pauvreté & Inégalités" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Pauvreté & Exclusion: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_poverty_social_exclusion_rights_index === "number" ? e.estimated_poverty_social_exclusion_rights_index.toFixed(2) : "—"}</span>
            </div>
