...
interface Entity {
  ...
  forced_eviction_scale_score: number;
  indigenous_territorial_dispossession_score: number;
  agroindustry_land_grab_score: number;
  legal_remedy_access_failure_score: number;
...
export default function Page() {
  ...
  if (!data) return (
    <div className="bg-slate-950 min-h-screen flex items-center justify-center">
      <div className="text-amber-700 animate-pulse">Initialisation du Moteur Droits Fonciers…</div>
    </div>
  );
  ...
  const confidence = typeof data.confidence_score === "number"
    ...
    : "—";
  const avgIndex = typeof data.avg_estimated_land_rights_index === "number"
    ? data.avg_estimated_land_rights_index.toFixed(2)
    : "—";
  ...
  const kpis = [
    ...
    { label: "Index Droits Fonciers", value: avgIndex },
    ...
    <div className="bg-slate-950 min-h-screen text-slate-100 p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Fonciers</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Évictions forcées, dépossession territoriale autochtone et accaparement agroindustriel des terres
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {(() => {
            ...
            return [
              { label: "Évictions Forcées", key: "forced_eviction_scale_score" as keyof Entity },
              { label: "Dépossession Autochtone", key: "indigenous_territorial_dispossession_score" as keyof Entity },
              { label: "Accaparement Agroindustriel", key: "agroindustry_land_grab_score" as keyof Entity },
              { label: "Échec Recours Légaux", key: "legal_remedy_access_failure_score" as keyof Entity },
            ].map(s => (
        ...
        {entities.map(e => (
          <div key={e.entity_id} onClick={() => { setSelected(e); setTab(0); }}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Fonciers: <span className="font-medium text-amber-600">{e.estimated_land_rights_index}</span>
            </div>
          </div>
        ))}
      </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="ml-auto text-right">
                      <div className="text-lg font-bold text-amber-600">{selected.estimated_land_rights_index}</div>
                      <div className="text-xs text-slate-500">Index Droits Fonciers</div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { label: "Évictions Forcées", value: selected.forced_eviction_scale_score },
                      { label: "Dépossession Autochtone", value: selected.indigenous_territorial_dispossession_score },
                      { label: "Accaparement Agroindustriel", value: selected.agroindustry_land_grab_score },
                      { label: "Échec Recours Légaux", value: selected.legal_remedy_access_failure_score },
                    ].map(s => (
