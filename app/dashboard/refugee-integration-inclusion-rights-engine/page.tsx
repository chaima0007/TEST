...
interface Entity {
  ...
  work_authorization_economic_exclusion_severity_score: number;
  education_language_integration_barrier_scale_score: number;
  housing_segregation_camp_confinement_score: number;
  social_protection_civic_participation_deficit_gap_score: number;
  estimated_refugee_integration_inclusion_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_refugee_integration_inclusion_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Exclusion Économique & Autorisation Travail", value: entity.work_authorization_economic_exclusion_severity_score, weight: "0.30" },
    { label: "Barrières Éducation & Intégration Linguistique", value: entity.education_language_integration_barrier_scale_score, weight: "0.25" },
    { label: "Ségrégation Logement & Confinement Camp", value: entity.housing_segregation_camp_confinement_score, weight: "0.25" },
    { label: "Déficit Protection Sociale & Participation Civique", value: entity.social_protection_civic_participation_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_refugee_integration_inclusion_rights_index === "number" ? entity.estimated_refugee_integration_inclusion_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Intégration Réfugiés</div>
                </div>
              </div>
...
export default function RefugeeIntegrationInclusionRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/refugee-integration-inclusion-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Intégration des Réfugiés & Droits d&apos;Inclusion…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_refugee_integration_inclusion_rights_index ?? avg(allEntities.map(e => e.estimated_refugee_integration_inclusion_rights_index));
  ...
  const avgWorkAuth = avg(allEntities.map(e => e.work_authorization_economic_exclusion_severity_score));
  const avgEducation = avg(allEntities.map(e => e.education_language_integration_barrier_scale_score));
  const avgHousing = avg(allEntities.map(e => e.housing_segregation_camp_confinement_score));
  const avgSocialProt = avg(allEntities.map(e => e.social_protection_civic_participation_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Intégration Réfugiés", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Intégration des Réfugiés & Droits d&apos;Inclusion</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Refugee Integration Inclusion Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgWorkAuth} label="Exclusion Économique" />
          <GaugeRing value={avgEducation} label="Éducation & Langue" />
          <GaugeRing value={avgHousing} label="Logement & Camps" />
          <GaugeRing value={avgSocialProt} label="Protection Sociale" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Intégration Réfugiés: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_refugee_integration_inclusion_rights_index === "number" ? e.estimated_refugee_integration_inclusion_rights_index.toFixed(2) : "—"}</span>
            </div>
