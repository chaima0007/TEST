...
export default function TortureCruelTreatmentDetentionPage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#3d0000";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", "modéré": "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/torture-cruel-treatment-detention-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_torture_cruel_treatment_detention_index"] as number) ?? 0;
