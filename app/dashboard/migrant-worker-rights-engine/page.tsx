...
interface MWREntity {
  ...
  wage_theft_exploitation_score: number;
  recruitment_fee_debt_score: number;
  kafala_system_dependency_score: number;
  labor_rights_access_failure_score: number;
  risk_level: string;
...
function EntityCard({ entity, onClick }: { entity: MWREntity; onClick: (e: MWREntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.wage_theft_exploitation_score}</p>
          <p className="text-[10px] text-gray-500">Vol Salaires</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.kafala_system_dependency_score}</p>
          <p className="text-[10px] text-gray-500">Kafala/Parrainage</p>
        </div>
      </div>
      <div className="flex items-center justify-between text-xs">
        <span className="text-gray-400">Score: <span className={`font-bold ${rc}`}>{entity.composite_score}</span></span>
        <span className="text-gray-500 truncate ml-2">Migrants: {entity.avg_estimated_migrant_worker_rights_index}/10</span>
      </div>
    </button>
  );
}

function DetailModal({ entity, onClose }: { entity: MWREntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Vol Salaires", value: `${entity.wage_theft_exploitation_score}/100` },
                  { label: "Dette Recrutement", value: `${entity.recruitment_fee_debt_score}/100` },
                  { label: "Kafala/Parrainage", value: `${entity.kafala_system_dependency_score}/100` },
                  { label: "Accès Droits", value: `${entity.labor_rights_access_failure_score}/100` },
                  { label: "Indice Migrants", value: `${entity.avg_estimated_migrant_worker_rights_index}/10` },
            ...
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-300 leading-relaxed">
                  Protéger les travailleurs migrants contre l'exploitation, abolir les systèmes de parrainage abusifs et garantir l'accès équitable aux droits du travail et aux recours juridiques.
                </p>
              </div>
...
export default function MigrantWorkerRightsEnginePage() {
  const [data, setData] = useState<MWRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<MWREntity | null>(null);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Migrant Worker Rights Engine</h1>
            <p className="text-sm text-gray-400 mt-1">Droits des Travailleurs Migrants &amp; Exploitation Systémique</p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Index Moyen Migrants" value={`${s.avg_estimated_migrant_worker_rights_index?.toFixed(1) ?? "—"}/10`} sub="droits migrants" accent="text-amber-600" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Droits Travailleurs Migrants</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_migrant_worker_rights_index ?? 0} max={10} label="Index Migrants" color="#b45309" />
                </div>
              </div>
              ...
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl px-5 py-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-400">
                    {s.critical_alerts} entité{s.critical_alerts > 1 ? "s" : ""} en situation critique de violation des droits des travailleurs migrants
                  </p>
                </div>
              </div>
            )}
