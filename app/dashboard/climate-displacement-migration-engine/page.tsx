...
interface Entity {
  ...
  risk_level: string;
  estimated_climate_displacement_migration_index: number;
}
...
interface EngineData {
  ...
  avg_estimated_climate_displacement_migration_index: number;
        ...
        {tab === "metriques" && (
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span style={{ color: accent }} className="font-bold">{entity.estimated_climate_displacement_migration_index}</span>
            </div>
          </div>
        )}
        {tab === "sources" && (
...
export default function ClimateDisplacementMigrationPage() {
  ...
  useEffect(() => {
    fetch("/api/climate-displacement-migration-engine")
      .then(r => r.json())
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white">Déplacements Climatiques &amp; Migration</h1>
            <p className="text-slate-400 text-sm mt-1">
              {data.total_entities} entités · Confiance {(data.confidence_score * 100).toFixed(0)}% · MAJ 2026-06-21
            </p>
          </div>
          <GaugeRing value={data.avg_estimated_climate_displacement_migration_index} accent={ACCENT} />
        </div>
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-slate-900 rounded-xl p-4 border border-cyan-900/30">
            <h3 className="text-cyan-400 font-semibold text-sm mb-3">Alertes critiques</h3>
