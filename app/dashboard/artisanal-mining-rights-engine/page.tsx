...
interface AMREntity {
  ...
  child_labor_exploitation_scale_score: number;
  mercury_toxic_exposure_severity_score: number;
  legal_formalization_absence_score: number;
  armed_group_coercion_pattern_score: number;
  estimated_artisanal_mining_rights_index: number;
  risk_level: string;
...
interface AMRData {
  ...
  data_sources: string[];
  entities: AMREntity[];
  avg_estimated_artisanal_mining_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function ArtisanalMiningRightsPage() {
  const [data, setData] = useState<AMRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<AMREntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/artisanal-mining-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-yellow-400 animate-pulse">Chargement des données minières…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "child_labor_exploitation_scale_score", label: "Travail Enfant" },
    { key: "mercury_toxic_exposure_severity_score", label: "Exposition Mercure" },
    { key: "legal_formalization_absence_score", label: "Absence Formalisation" },
    { key: "armed_group_coercion_pattern_score", label: "Coercition Groupes Armés" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits des Mineurs Artisanaux</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Artisanal Mining Rights Engine</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_artisanal_mining_rights_index ?? "—" },
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-500">idx {entity.estimated_artisanal_mining_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droits des Mineurs Artisanaux · {data.last_analysis}</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Droits Mineurs Artisanaux</div>
                    <div className="text-lg font-bold text-yellow-400">{selected.estimated_artisanal_mining_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
