...
interface Entity {
  ...
  country: string;
  sector: string;
  colonial_extractive_economic_harm_severity_score: number;
  reparations_acknowledgment_deficit_gap_score: number;
  historical_memory_denial_suppression_score: number;
  transitional_justice_mechanism_absence_score: number;
  estimated_colonial_reparations_historical_justice_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_colonial_reparations_historical_justice_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Préjudice Économique Colonial", value: entity.colonial_extractive_economic_harm_severity_score, weight: "0.30" },
    { label: "Déficit Reconnaissance Réparations", value: entity.reparations_acknowledgment_deficit_gap_score, weight: "0.25" },
    { label: "Déni Mémoire Historique", value: entity.historical_memory_denial_suppression_score, weight: "0.25" },
    { label: "Absence Justice Transitionnelle", value: entity.transitional_justice_mechanism_absence_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.colonial_extractive_economic_harm_severity_score === "number" ? entity.colonial_extractive_economic_harm_severity_score.toFixed(1) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Préjudice Économique</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_colonial_reparations_historical_justice_index === "number" ? entity.estimated_colonial_reparations_historical_justice_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Justice Historique</div>
                </div>
              </div>
...
export default function ColonialReparationsHistoricalJusticeEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/colonial-reparations-historical-justice-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Réparations Coloniales &amp; Justice Historique…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.colonial_extractive_economic_harm_severity_score));
  const avgIndex = data?.avg_estimated_colonial_reparations_historical_justice_index ?? avg(allEntities.map(e => e.estimated_colonial_reparations_historical_justice_index));
  ...
  const avgColonialHarm = avg(allEntities.map(e => e.colonial_extractive_economic_harm_severity_score));
  const avgReparations = avg(allEntities.map(e => e.reparations_acknowledgment_deficit_gap_score));
  const avgMemory = avg(allEntities.map(e => e.historical_memory_denial_suppression_score));
  const avgTransitional = avg(allEntities.map(e => e.transitional_justice_mechanism_absence_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Justice Historique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Réparations Coloniales &amp; Justice Historique</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Colonial Reparations &amp; Historical Justice Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgColonialHarm} label="Préjudice Économique Colonial" />
          <GaugeRing value={avgReparations} label="Déficit Reconnaissance Réparations" />
          <GaugeRing value={avgMemory} label="Déni Mémoire Historique" />
          <GaugeRing value={avgTransitional} label="Absence Justice Transitionnelle" />
        </div>
      </div>
          ...
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.colonial_extractive_economic_harm_severity_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.colonial_extractive_economic_harm_severity_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Justice Historique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_colonial_reparations_historical_justice_index === "number" ? e.estimated_colonial_reparations_historical_justice_index.toFixed(2) : "—"}</span>
            </div>
