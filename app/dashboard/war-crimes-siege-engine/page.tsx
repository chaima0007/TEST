...
interface Entity {
  ...
  deliberate_civilian_targeting_score: number;
  siege_starvation_tactics_score: number;
  protected_site_attack_score: number;
  war_crimes_impunity_score: number;
  estimated_war_crimes_siege_index: number;
  risk_level: string;
...
const PATTERN_LABELS: Record<string, string> = {
  siege_famine_arme: "Siège Famine Arme",
  attaques_hopitaux_protegees: "Attaques Sites Protégés",
  boucliers_humains_forces: "Boucliers Humains",
  execution_prisonniers_guerre: "Exécutions Sommaires",
  droit_humanitaire_respecte: "DIH Respecté",
};
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/war-crimes-siege-engine")
      .then(r => r.json())
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">War Crimes Siege Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Surveillance des crimes de guerre, sièges et violations du DIH</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    {[
                      { label: "Ciblage Civils", value: selected.deliberate_civilian_targeting_score },
                      { label: "Siège / Famine", value: selected.siege_starvation_tactics_score },
                      { label: "Sites Protégés", value: selected.protected_site_attack_score },
                      { label: "Impunité Crimes", value: selected.war_crimes_impunity_score },
                    ].map(s => (
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index War Crimes Siege</div>
                    <div className="text-lg font-bold" style={{ color: ACCENT }}>{typeof selected.estimated_war_crimes_siege_index === "number" ? selected.estimated_war_crimes_siege_index.toFixed(1) : "—"}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
