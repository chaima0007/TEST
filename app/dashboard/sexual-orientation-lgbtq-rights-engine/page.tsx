...
interface Entity {
  ...
  lgbtq_criminalization_imprisonment_severity_score: number;
  lgbtq_violence_hate_crime_impunity_scale_score: number;
  same_sex_partnership_legal_recognition_absence_score: number;
  lgbtq_asylum_protection_deficit_gap_score: number;
  estimated_sexual_orientation_lgbtq_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_sexual_orientation_lgbtq_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Criminalisation & Emprisonnement LGBTQ+", value: entity.lgbtq_criminalization_imprisonment_severity_score, weight: "0.30" },
    { label: "Violence & Crimes Haine Impunis", value: entity.lgbtq_violence_hate_crime_impunity_scale_score, weight: "0.25" },
    { label: "Absence Reconnaissance Légale", value: entity.same_sex_partnership_legal_recognition_absence_score, weight: "0.25" },
    { label: "Déficit Protection Asile LGBTQ+", value: entity.lgbtq_asylum_protection_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_sexual_orientation_lgbtq_rights_index === "number" ? entity.estimated_sexual_orientation_lgbtq_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits LGBTQ+</div>
                </div>
              </div>
...
export default function SexualOrientationLgbtqRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/sexual-orientation-lgbtq-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits LGBTQ+ & Orientation Sexuelle…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_sexual_orientation_lgbtq_rights_index ?? avg(allEntities.map(e => e.estimated_sexual_orientation_lgbtq_rights_index));
  ...
  const avgCriminalization = avg(allEntities.map(e => e.lgbtq_criminalization_imprisonment_severity_score));
  const avgViolence = avg(allEntities.map(e => e.lgbtq_violence_hate_crime_impunity_scale_score));
  const avgLegalRecognition = avg(allEntities.map(e => e.same_sex_partnership_legal_recognition_absence_score));
  const avgAsylum = avg(allEntities.map(e => e.lgbtq_asylum_protection_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits LGBTQ+", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits LGBTQ+ & Orientation Sexuelle</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Sexual Orientation & LGBTQ+ Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgCriminalization} label="Criminalisation LGBTQ+" />
          <GaugeRing value={avgViolence} label="Violence & Crimes Haine" />
          <GaugeRing value={avgLegalRecognition} label="Reconnaissance Légale" />
          <GaugeRing value={avgAsylum} label="Protection Asile" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits LGBTQ+: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_sexual_orientation_lgbtq_rights_index === "number" ? e.estimated_sexual_orientation_lgbtq_rights_index.toFixed(2) : "—"}</span>
            </div>
