...
interface Entity {
  ...
  police_brutality_severity_score: number;
  racial_profiling_systemic_score: number;
  impunity_accountability_gap_score: number;
  community_impact_resilience_score: number;
  estimated_racial_justice_police_brutality_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_racial_justice_police_brutality_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Sévérité Brutalité Policière", value: entity.police_brutality_severity_score, weight: "0.30" },
    { label: "Profilage Racial Systémique", value: entity.racial_profiling_systemic_score, weight: "0.25" },
    { label: "Déficit Impunité & Responsabilité", value: entity.impunity_accountability_gap_score, weight: "0.25" },
    { label: "Impact Communauté & Résilience", value: entity.community_impact_resilience_score, weight: "0.20" },
  ];
  return (
        ...
        <div className="flex border-b border-slate-800">
          {tabs.map((t) => (
            <button key={t.key} onClick={() => setTab(t.key)}
              ...
              style={tab === t.key ? { borderColor: ACCENT_DISPLAY, color: ACCENT_DISPLAY } : {}}>
              {t.label}
            </button>
          ))}
        </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT_DISPLAY }}>{entity.composite_score.toFixed(1)}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Composite</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT_DISPLAY }}>{typeof entity.estimated_racial_justice_police_brutality_index === "number" ? entity.estimated_racial_justice_police_brutality_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Justice Raciale</div>
                </div>
              </div>
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-sm font-medium" style={{ color: ACCENT_DISPLAY }}>{entity.primary_pattern}</div>
              </div>
            </div>
          )}
          {tab === "metriques" && (
              ...
              {subScores.map((s) => (
                <div key={s.label} className="bg-slate-800/50 rounded-lg p-3">
                  ...
                  <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${Math.min(typeof s.value === "number" ? s.value : 0, 100)}%`, background: ACCENT_DISPLAY }} />
                  </div>
                </div>
              ))}
            </div>
                ...
                <ul className="space-y-2">
                  {(entity.key_signals ?? []).map((sig, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-slate-300">
                      <span className="mt-1 w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: ACCENT_DISPLAY }} />
                      {sig}
                    </li>
                  ))}
                </ul>
...
export default function RacialJusticePoliceBrutalityPage() {
  ...
  useEffect(() => {
    fetch("/api/racial-justice-police-brutality-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT_DISPLAY }}>Initialisation Justice Raciale & Brutalité Policière…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_racial_justice_police_brutality_index ?? avg(allEntities.map(e => e.estimated_racial_justice_police_brutality_index));
  ...
  const avgBrutalite = avg(allEntities.map(e => e.police_brutality_severity_score));
  const avgProfiling = avg(allEntities.map(e => e.racial_profiling_systemic_score));
  const avgImpunite = avg(allEntities.map(e => e.impunity_accountability_gap_score));
  const avgCommunaute = avg(allEntities.map(e => e.community_impact_resilience_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Justice Raciale", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          <div className="w-3 h-8 rounded-full" style={{ background: ACCENT_DISPLAY }} />
          <h1 className="text-2xl font-bold tracking-tight">Justice Raciale & Brutalité Policière</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Racial Justice Police Brutality Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {kpis.map(k => (
          <div key={k.label} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            <div className="text-xl font-bold" style={{ color: ACCENT_DISPLAY }}>{k.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgBrutalite} label="Brutalité Policière" />
          <GaugeRing value={avgProfiling} label="Profilage Racial" />
          <GaugeRing value={avgImpunite} label="Impunité" />
          <GaugeRing value={avgCommunaute} label="Impact Communauté" />
        </div>
      </div>
      ...
      {(data?.critical_alerts ?? []).length > 0 && (
        <div className="bg-red-950/30 border border-red-500/30 rounded-xl p-4">
          <h3 className="text-red-400 text-xs uppercase tracking-wide mb-2 font-semibold">Alertes Critiques</h3>
      ...
      <div className="flex gap-2 flex-wrap">
        {filters.map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { background: ACCENT_DISPLAY } : {}}>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.composite_score, 100)}%`, background: ACCENT_DISPLAY }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Justice Raciale: <span className="font-medium" style={{ color: ACCENT_DISPLAY }}>{typeof e.estimated_racial_justice_police_brutality_index === "number" ? e.estimated_racial_justice_police_brutality_index.toFixed(2) : "—"}</span>
            </div>
