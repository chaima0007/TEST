...
interface Entity {
  ...
  torture_prevalence_severity_score: number;
  legal_protection_failure_score: number;
  detention_conditions_inhumane_score: number;
  impunity_accountability_failure_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_anti_torture_index: number;
  last_updated: string;
  [key: string]: unknown;
}
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_anti_torture_index: number;
}
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/anti-torture-engine").then(r => r.json()).then(j => setData(j.data ?? j)).catch(console.error);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Anti-Torture Engine</h1>
        <p className="text-slate-400 mt-1">Convention CAT · Rapporteur ONU · Mécanisme SPT</p>
      </div>

      {/* KPI Grid */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Total Entités</p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Cas Critiques</p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_anti_torture_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold mt-1 text-red-400">{data.critical_alerts.length}</p>
        </div>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { key: "torture_prevalence_severity_score", label: "Prévalence Torture" },
                      { key: "legal_protection_failure_score", label: "Défaillance Protection" },
                      { key: "detention_conditions_inhumane_score", label: "Conditions Inhumaines" },
                      { key: "impunity_accountability_failure_score", label: "Impunité" },
                    ].map(({ key, label }) => (
                      <div key={key}>
