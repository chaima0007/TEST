...
interface Entity {
  ...
  estimated_plastic_pollution_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_plastic_pollution_index?: number;
        ...
        <div className="p-6">
          {tab === "analyse" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_plastic_pollution_index === "number" ? entity.estimated_plastic_pollution_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Pollution Plastique</div>
                </div>
              </div>
          ...
          {tab === "entites" && (
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                <div className="text-xs text-slate-400 mb-3">Index Pollution Plastique (sur 10)</div>
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-3 bg-slate-700 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${Math.min((entity.estimated_plastic_pollution_index / 10) * 100, 100)}%`, background: ACCENT }} />
                  </div>
                  <span className="text-sm font-bold text-white w-12 text-right">{typeof entity.estimated_plastic_pollution_index === "number" ? entity.estimated_plastic_pollution_index.toFixed(2) : "—"}</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-xl p-4">
                ...
                <div className="text-sm text-slate-300 space-y-1">
                  <div>Production Plastique & Déversements Illégaux × 0.30</div>
                  <div>Impact Santé Communautés Vulnérables × 0.25</div>
                  <div>Impunité Industrielle & Lobbying × 0.25</div>
                  <div>Dégradation Écosystèmes Marins × 0.20</div>
                </div>
              </div>
              {entity.last_updated && (
...
export default function PlasticPollutionRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/plastic-pollution-rights")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits contre la Pollution Plastique…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_plastic_pollution_index ?? avg(allEntities.map(e => e.estimated_plastic_pollution_index));
  ...
  const kpis = [
    ...
    { label: "Index Pollution Plastique", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits contre la Pollution Plastique</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Plastic Pollution Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <GaugeRing value={avgIndex * 10} label="Index Pollution" />
        ...
        {filtered.map(e => (
          <div key={e.id ?? e.entity} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Pollution Plastique: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_plastic_pollution_index === "number" ? e.estimated_plastic_pollution_index.toFixed(2) : "—"}</span>
            </div>
