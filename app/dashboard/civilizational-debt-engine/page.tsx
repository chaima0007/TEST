...
interface Entity {
  ...
  ecological_debt_score: number;
  financial_debt_score: number;
  social_debt_score: number;
  institutional_debt_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_civdebt_index: number;
  last_updated: string;
}
...
interface SummaryData {
  ...
  data_sources: string[];
  entities: Entity[];
  avg_estimated_civdebt_index: number;
}
...
const PATTERN_COLORS: Record<string, string> = {
  effondrement_generationnel: "#ef4444",
  transfert_massif_risques: "#f97316",
  erosion_heritages: "#f59e0b",
  accumulation_silencieuse: "#8b5cf6",
  equilibre_intergenerationnel: "#10b981",
};
      ...
      >
        ...
        {tab === "scores" && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            {(
              [
                ["Dette Écologique", entity.ecological_debt_score, "#10b981"],
                ["Dette Financière", entity.financial_debt_score, "#f97316"],
                ["Dette Sociale", entity.social_debt_score, "#8b5cf6"],
                ["Dette Institutionnelle", entity.institutional_debt_score, "#06b6d4"],
            ...
            <div className="col-span-2 bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-300/60 text-xs mb-1">Dette Civilisationnelle Composite</div>
              <div className="text-white font-bold text-2xl">
                {entity.composite_score.toFixed(2)}
              </div>
              <div className="text-slate-400 text-xs mt-1">
                Index Dette Civ.: {entity.estimated_civdebt_index.toFixed(2)}/10
              </div>
            </div>
          </div>
        )}
        ...
        {tab === "signaux" && (
          <div className="space-y-3">
            <div className="flex gap-2 flex-wrap mb-3">
              ...
              <span
                className="px-2 py-0.5 rounded text-xs font-medium bg-slate-800 text-violet-300"
            ...
            {entity.key_signals.map((sig, i) => (
              <div
                key={i}
                className="bg-slate-900 border border-violet-700/20 rounded-lg p-3 text-sm text-slate-200 leading-relaxed"
              >
                {sig}
              </div>
            ))}
        ...
        {tab === "actions" && (
          <div className="space-y-3 text-sm">
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-300/60 text-xs mb-1">Patron de Dette Principal</div>
              <div
            ...
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-300/60 text-xs mb-1">Dernière mise à jour</div>
              <div className="text-white font-medium">{entity.last_updated}</div>
            </div>
            <div className="bg-slate-900 border border-violet-700/20 rounded-lg p-3">
              <div className="text-violet-300/60 text-xs mb-1">Pays</div>
...
export default function CivDebtDashboard() {
  const [data, setData] = useState<SummaryData | null>(null);
  const [filter, setFilter] = useState<string>("Tous");
  const [selected, setSelected] = useState<Entity | null>(null);

  useEffect(() => {
    fetch("/api/civilizational-debt-engine")
      .then((r) => r.json())
      .then((json) => {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-violet-400 text-lg animate-pulse">
          Initialisation du Moteur Dette Civilisationnelle...
        </div>
      </div>
    );
  }
  ...
  const avgEcological =
    data.entities.reduce((s, e) => s + e.ecological_debt_score, 0) / (data.entities.length || 1);
  const avgFinancial =
    data.entities.reduce((s, e) => s + e.financial_debt_score, 0) / (data.entities.length || 1);
  const avgSocial =
    data.entities.reduce((s, e) => s + e.social_debt_score, 0) / (data.entities.length || 1);
  const avgInstitutional =
    data.entities.reduce((s, e) => s + e.institutional_debt_score, 0) / (data.entities.length || 1);
  ...
  const dist4 = [
    ...
    },
    {
      ...
      colors: {
        intergenerational_justice_index: "#8b5cf6",
        ecological_debt_tracker: "#10b981",
        sovereign_debt_monitor: "#f97316",
      },
    },
    {
      title: "Entités à Dette Critique",
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-violet-400">
          Civilizational Debt Engine — Intelligence de la Dette Intergénérationnelle
        </h1>
        <p className="text-violet-300/50 text-sm mt-1">
          Dette Écologique · Dette Financière · Dette Sociale · Dette Institutionnelle — Concept Caelum Partners
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {(
          [
            ...
            [
              "Index CivDebt",
              `${data.avg_estimated_civdebt_index.toFixed(2)}/10`,
              "text-amber-400",
            ],
            [
              "Confiance",
      ...
      <div className="bg-slate-900 border border-violet-700/30 rounded-xl p-5">
        <div className="grid grid-cols-4 gap-4">
          <GaugeRing value={avgEcological} label="Dette Écol. Moy." color="#10b981" />
          <GaugeRing value={avgFinancial} label="Dette Fin. Moy." color="#f97316" />
          <GaugeRing value={avgSocial} label="Dette Soc. Moy." color="#8b5cf6" />
          <GaugeRing value={avgInstitutional} label="Dette Inst. Moy." color="#06b6d4" />
        </div>
      </div>
        ...
        {filtered.map((e) => (
          ...
          >
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs text-violet-400/60">{e.entity_id}</span>
            </div>
            ...
            <div className="text-xs text-violet-400/60 mb-2">
              Index CivDebt: {e.estimated_civdebt_index.toFixed(2)}/10
            </div>
            <div className="text-xs text-violet-400 font-medium mb-2">
              Écol: {e.ecological_debt_score.toFixed(0)} · Fin: {e.financial_debt_score.toFixed(0)} ·
              Soc: {e.social_debt_score.toFixed(0)} · Inst: {e.institutional_debt_score.toFixed(0)}
            </div>
