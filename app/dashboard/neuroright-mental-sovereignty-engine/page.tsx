...
interface NMSEntity {
  ...
  brain_data_collection_consent_absence_score: number;
  neurotechnology_regulation_gap_score: number;
  mental_state_manipulation_risk_score: number;
  cognitive_liberty_violation_scale_score: number;
  estimated_neuroright_mental_sovereignty_index: number;
  risk_level: string;
...
interface NMSData {
  ...
  data_sources: string[];
  entities: NMSEntity[];
  avg_estimated_neuroright_mental_sovereignty_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function NeurorightMentalSovereigntyPage() {
  const [data, setData] = useState<NMSData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<NMSEntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/neuroright-mental-sovereignty-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données de neurodroits…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "brain_data_collection_consent_absence_score", label: "Collecte Données Cérébrales" },
    { key: "neurotechnology_regulation_gap_score", label: "Lacunes Réglementation" },
    { key: "mental_state_manipulation_risk_score", label: "Manipulation État Mental" },
    { key: "cognitive_liberty_violation_scale_score", label: "Liberté Cognitive" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Neuroright &amp; Souveraineté Mentale</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Surveillance neurotechnologique &amp; libertés cognitives</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_neuroright_mental_sovereignty_index ?? "—" },
            ...
            <div key={k.label} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              <div className="text-xl font-bold text-violet-400">{k.value}</div>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#a78bfa" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Neurosurveillance</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#a78bfa", "#f97316", "#eab308", "#ef4444", "#10b981"];
        ...
        {(data.critical_alerts?.length ?? 0) > 0 && (
          <div className="border border-violet-500/30 bg-violet-500/10 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-violet-400 mb-2">Alertes Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#a78bfa" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_neuroright_mental_sovereignty_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Neuroright &amp; Souveraineté Mentale · {data.last_analysis}</p>
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Aperçu", "Métriques", "Sources"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`px-6 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-violet-500 text-violet-400" : "text-slate-500 hover:text-slate-300"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#a78bfa" />
                    <div>
                      <div className="text-2xl font-bold text-violet-400">{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Neurodroits</div>
                    <div className="text-lg font-bold text-violet-400">{selected.estimated_neuroright_mental_sovereignty_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
