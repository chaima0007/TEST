...
interface DRAEntity {
  ...
  forced_institutionalization_autonomy_denial_score: number;
  disability_discrimination_employment_access_score: number;
  medical_model_rights_based_deficit_score: number;
  disability_legal_protection_enforcement_gap_score: number;
  estimated_disability_rights_ableism_index: number;
  risk_level: string;
...
interface DRAData {
  ...
  data_sources: string[];
  entities: DRAEntity[];
  avg_estimated_disability_rights_ableism_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function DisabilityRightsAbleismEnginePage() {
  const [data, setData] = useState<DRAData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<DRAEntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/disability-rights-ableism-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données droits handicap & validisme…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "forced_institutionalization_autonomy_denial_score", label: "Institutionnalisation Forcée" },
    { key: "disability_discrimination_employment_access_score", label: "Discrimination Emploi & Accès" },
    { key: "medical_model_rights_based_deficit_score", label: "Modèle Médical vs Droits" },
    { key: "disability_legal_protection_enforcement_gap_score", label: "Déficit Application Légale" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Handicap & Validisme Systémique</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Institutionnalisation Forcée · Discrimination Emploi & Accès · Modèle Médical vs Droits · Déficit Application Légale
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Validisme", value: data.avg_estimated_disability_rights_ableism_index ?? "—" },
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-500">idx {entity.estimated_disability_rights_ableism_index}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droits Handicap & Validisme Systémique · {data.last_analysis}</p>
      </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Validisme</div>
                    <div className="text-lg font-bold" style={{ color: ACCENT }}>{selected.estimated_disability_rights_ableism_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
