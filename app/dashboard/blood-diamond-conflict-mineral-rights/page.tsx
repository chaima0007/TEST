...
interface BDCMREntity {
  id: string;
  name: string;
  country: string;
  composite_score: number;
  armed_group_revenue_capture_score: number;
  civilian_forced_labor_extraction_score: number;
  conflict_financing_supply_chain_score: number;
  due_diligence_failure_scale_score: number;
  estimated_blood_diamond_conflict_mineral_rights_index: number;
  risk_level: string;
...
interface BDCMRData {
  ...
  data_sources: string[];
  entities: BDCMREntity[];
  avg_estimated_blood_diamond_conflict_mineral_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function BloodDiamondConflictMineralRightsPage() {
  const [data, setData] = useState<BDCMRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<BDCMREntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/blood-diamond-conflict-mineral-rights")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-red-400 animate-pulse">Chargement des données minerais conflit…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "armed_group_revenue_capture_score", label: "Capture Revenue Groupes Armés" },
    { key: "civilian_forced_labor_extraction_score", label: "Travail Forcé Civils" },
    { key: "conflict_financing_supply_chain_score", label: "Financement Conflit Chaîne" },
    { key: "due_diligence_failure_scale_score", label: "Échec Diligence Raisonnée" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits — Diamants du Sang</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Blood Diamond &amp; Conflict Mineral Rights Engine</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Entités", value: data.total_entities },
            ...
            { label: "Index Moyen", value: data.avg_estimated_blood_diamond_conflict_mineral_rights_index ?? "—" },
            { label: "Confidence", value: `${Math.round((data.confidence_score ?? 0) * 100)}%` },
          ].map(k => (
            <div key={k.label} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              <div className="text-xs text-slate-500 mb-1">{k.label}</div>
              <div className="text-xl font-bold text-red-400">{k.value}</div>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#f87171" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns d&apos;Exploitation</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#f87171", "#ef4444", "#dc2626", "#fca5a5", "#fecaca"];
        ...
        <div className="flex gap-2 flex-wrap">
          {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-full text-sm border transition-all capitalize ${filter === f ? "bg-red-500 border-red-400 text-white" : "bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-500"}`}>
              {f}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => { setSelected(entity); setTab(0); }}
              className={`bg-slate-900 border rounded-xl p-4 cursor-pointer hover:border-red-500/50 transition-all ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#f87171" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_blood_diamond_conflict_mineral_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droits Diamants Sang &amp; Minerais Conflit · {data.last_analysis}</p>
      </div>
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-slate-800 flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">{selected.country}</p>
            ...
            <div className="flex border-b border-slate-800">
              {["Vue générale", "Sous-scores", "Alertes"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`px-6 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-red-500 text-red-400" : "text-slate-500 hover:text-slate-300"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#f87171" />
                    <div>
                      <div className="text-2xl font-bold text-red-400">{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Droits Diamants Sang</div>
                    <div className="text-lg font-bold text-red-400">{selected.estimated_blood_diamond_conflict_mineral_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
                    ...
                    return (
                      <div key={key}>
                        ...
                        <div className="h-2 bg-slate-800 rounded-full">
                          <div className="h-full rounded-full bg-red-500" style={{ width: `${val}%` }} />
                        </div>
                      </div>
                    );
                  })}
