...
interface CREntity {
  ...
  historical_atrocity_magnitude_score: number;
  reparation_refusal_persistence_score: number;
  ongoing_structural_impact_score: number;
  accountability_recognition_failure_score: number;
  last_updated: string;
}
...
function EntityCard({ entity, onClick }: { entity: CREntity; onClick: (e: CREntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.historical_atrocity_magnitude_score ?? "—"}</p>
          <p className="text-[10px] text-gray-500">Magnitude Atrocités</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.reparation_refusal_persistence_score ?? "—"}</p>
          <p className="text-[10px] text-gray-500">Refus Réparations</p>
        </div>
      </div>
...
function DetailModal({ entity, onClose }: { entity: CREntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Magnitude Atrocités", value: `${entity.historical_atrocity_magnitude_score ?? "—"}/100` },
                  { label: "Refus Réparations", value: `${entity.reparation_refusal_persistence_score ?? "—"}/100` },
                  { label: "Impact Structurel Persistant", value: `${entity.ongoing_structural_impact_score ?? "—"}/100` },
                  { label: "Déficit Reconnaissance", value: `${entity.accountability_recognition_failure_score ?? "—"}/100` },
                  { label: "Indice CR", value: `${entity.estimated_colonial_reparations_index ?? "—"}/10` },
          ...
          {tab === "contexte" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 mt-1"><span className="text-gray-300 font-medium">Indice CR :</span> {entity.estimated_colonial_reparations_index ?? "—"}/10</p>
...
export default function Page() {
  ...
  const [modal, setModal] = useState<CREntity | null>(null);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Colonial Reparations Engine</h1>
            <p className="text-sm text-gray-400 mt-1">Réparations Coloniales &amp; Justice Historique</p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Indice CR moyen" value={`${s.avg_estimated_colonial_reparations_index ?? "—"}/10`} sub="score normalisé" accent="text-amber-600" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                ...
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_colonial_reparations_index ?? 0} max={10} label="Indice CR" color="#d97706" />
                </div>
              </div>
              ...
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                ...
                <div className="mt-5 pt-4 border-t border-white/8 grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-white/4 rounded-lg p-3">
                    ...
                    <p className="text-lg font-bold mt-0.5" style={{ color: "#d97706" }}>{s.engine_version ?? "—"}</p>
                  </div>
                </div>
              </div>
            </div>
