...
const ACCENT = "#1a0d26";
const INDEX_KEY = "estimated_transgender_rights_index";
...
const FALLBACK_ENTITIES = [
  { id: "TGR-001", name: "Iran", country: "Moyen-Orient", sector: "Chirurgie forcée légalisée, emprisonnement & exécutions — identité de genre criminalisée", composite_score: 89, risk_level: "critique", estimated_transgender_rights_index: 8.9 },
  { id: "TGR-002", name: "Nigeria", country: "Afrique de l&apos;Ouest", sector: "SSMPA 2014 — 14 ans prison, violences et meurtres trans non-poursuivis", composite_score: 86, risk_level: "critique", estimated_transgender_rights_index: 8.6 },
  { id: "TGR-003", name: "Saudi Arabia", country: "Moyen-Orient", sector: "Identité trans interdite, arrestations systématiques & discrimination institutionnelle totale", composite_score: 84, risk_level: "critique", estimated_transgender_rights_index: 8.4 },
  { id: "TGR-004", name: "Uganda", country: "Afrique de l&apos;Est", sector: "Anti-Homosexuality Act 2023 — peine de mort, exil massif & organisations LGBTi fermées", composite_score: 81, risk_level: "critique", estimated_transgender_rights_index: 8.1 },
  { id: "TGR-005", name: "Turkey", country: "Europe/Asie", sector: "Marches Pride interdites, détentions arbitraires & recul législatif depuis 2015", composite_score: 59, risk_level: "élevé", estimated_transgender_rights_index: 5.9 },
  { id: "TGR-006", name: "Brazil", country: "Amérique du Sud", sector: "Taux de meurtre trans le plus élevé du monde malgré décisions STF progressistes", composite_score: 57, risk_level: "élevé", estimated_transgender_rights_index: 5.7 },
  { id: "TGR-007", name: "USA", country: "Amérique du Nord", sector: "500+ lois anti-trans 2023-2024, interdictions soins médicaux mineurs & ban militaire", composite_score: 36, risk_level: "modéré", estimated_transgender_rights_index: 3.6 },
  { id: "TGR-008", name: "Iceland", country: "Europe du Nord", sector: "Reconnaissance légale simplifiée, accès soins remboursés & protection discriminations robuste", composite_score: 7, risk_level: "faible", estimated_transgender_rights_index: 0.7 },
];
...
const FALLBACK_DATA = {
  ...
  avg_estimated_transgender_rights_index: 6.24,
  confidence_score: 0.84,
  last_analysis: "2026-06-22",
  data_sources: ["transgender_europe_tgeu", "ilga_world", "human_rights_watch_lgbti", "trans_murder_monitoring"],
  critical_alerts: ["Uganda: anti-homosexuality_act_peine_mort", "Nigeria: violence_trans_impunité_totale", "Iran: chirurgie_forcée_légalisée"],
...
interface DashData {
  ...
  avg_estimated_transgender_rights_index?: number;
        ...
        <div style={{ padding: "20px 24px 24px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
            {[
              ...
              { label: "Index TGR", value: indexVal !== undefined ? `${indexVal}/10` : "—" },
          ...
          <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "12px 16px" }}>
            ...
            <p style={{ fontSize: 11, color: "#475569", margin: "6px 0 0" }}>
              Reconnaissance &amp; protection des droits trans
            </p>
          </div>
        </div>
      </div>
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/transgender-rights-engine")
      .then(r => r.json())
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020817", color: "white", padding: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#c4b5fd", margin: "0 0 4px" }}>
            Droits Trans &mdash; Reconnaissance &amp; Protection
          </h1>
          <p style={{ fontSize: 13, color: "#64748b", margin: 0 }}>
            Transgender Rights Engine &middot; Wave 212
          </p>
        </div>
      ...
      {loading && (
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: 200 }}>
          <div style={{ width: 32, height: 32, border: "2px solid #c4b5fd", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
        </div>
      )}
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 28 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px", display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              <GaugeRing value={dash.avg_composite} stroke="#c4b5fd" />
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
              <p style={{ fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 6px" }}>Index TGR Moyen</p>
              <p style={{ fontSize: 28, fontWeight: 700, color: "#c4b5fd", margin: 0 }}>{dash.avg_estimated_transgender_rights_index ?? "—"}</p>
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
              <p style={{ fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 6px" }}>Entit&eacute;s</p>
              <p style={{ fontSize: 28, fontWeight: 700, color: "#c4b5fd", margin: 0 }}>{dash.total_entities}</p>
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
          ...
          {(dash.data_sources ?? []).length > 0 && (
            <div style={{ background: "rgba(196,181,253,0.05)", border: "1px solid rgba(196,181,253,0.15)", borderRadius: 12, padding: "14px 18px", marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {(dash.data_sources ?? []).map((src: string, i: number) => (
                  <span key={i} style={{ fontSize: 12, color: "#c4b5fd", background: "rgba(196,181,253,0.08)", border: "1px solid rgba(196,181,253,0.15)", borderRadius: 6, padding: "3px 10px" }}>
                    {src}
                  </span>
                ))}
              </div>
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, gap: 8 }}>
                    ...
                    <div style={{ flexShrink: 0 }}>
                      <GaugeRing value={e.composite_score} stroke={RISK_COLORS[e.risk_level] ?? "#c4b5fd"} />
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: cfg.color }}>{cfg.label}</span>
                    <span style={{ fontSize: 12, color: "#64748b" }}>
                      Index: <span style={{ fontWeight: 700, color: "#c4b5fd" }}>{indexVal ?? "—"}</span>
                    </span>
                  </div>
                </button>
              );
