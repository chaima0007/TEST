...
const SIGNALS = [
  ...
  "Carousel click attribution",
];

const ACTIONS = [
  "Limit swipe tracking",
  "Audit product data",
  "Enforce multi-card consent",
  "Cap engagement profiling",
  "Review click attribution",
];
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque carouselads advertising : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_index.toFixed(2)} / 10`,
    `Conformité plateforme carouselads : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 419 — Carousel Ads Advertising Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Carousel Ads Advertising · Wave 419</div>
          </div>
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            {ACTIONS.map((action, i) => (
              <div key={i} style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(8,145,178,0.15)" }}>
...
export default function CarouseladsAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/carouselads-advertising")
      .then((r) => r.json())
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Carousel Ads Advertising…
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Carousel Ads Advertising Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#0891b299", margin: 0 }}>
          Caelum Partners · Wave 419 · Analyse publicitaire carouselads · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Carousel Ads Advertising Intelligence · Wave 419
      </div>
