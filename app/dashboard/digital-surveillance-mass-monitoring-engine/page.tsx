...
export default function DigitalSurveillanceMassMonitoringDashboard() {
  const [data, setData] = useState<DomainData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch("/api/digital-surveillance-mass-monitoring-engine")
      .then(r => r.json())
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm font-medium uppercase tracking-widest" style={{ color: ACCENT }}>
              Wave 170 — Surveillance Numérique
            </span>
          </div>
          ...
          <p className="text-slate-400 text-sm">
            Monitoring &amp; évaluation des systèmes de surveillance numérique à l&apos;échelle mondiale
          </p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="mt-auto pt-2 border-t border-slate-800 flex justify-between items-center">
                  <span className="text-xs text-slate-500">Index Surveillance</span>
                  <span className="text-sm font-bold" style={{ color: ACCENT }}>
                    {entity.estimated_surveillance_rights_index.toFixed(2)}
                  </span>
                </div>
              </div>
            )
