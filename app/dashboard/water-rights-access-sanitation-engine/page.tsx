...
interface Entity {
  ...
  country: string;
  sector: string;
  water_access_denial_civilian_severity_score: number;
  sanitation_infrastructure_collapse_score: number;
  water_privatization_rights_erosion_score: number;
  water_conflict_weaponization_score: number;
  estimated_water_rights_access_sanitation_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_water_rights_access_sanitation_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Déni Accès Eau Civils", value: entity.water_access_denial_civilian_severity_score, weight: "0.30" },
    { label: "Effondrement Sanitaire", value: entity.sanitation_infrastructure_collapse_score, weight: "0.25" },
    { label: "Privatisation Eau", value: entity.water_privatization_rights_erosion_score, weight: "0.25" },
    { label: "Eau Armée Conflit", value: entity.water_conflict_weaponization_score, weight: "0.20" },
  ];
  return (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.water_access_denial_civilian_severity_score === "number" ? entity.water_access_denial_civilian_severity_score.toFixed(1) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Score Déni Accès Eau</div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_water_rights_access_sanitation_index === "number" ? entity.estimated_water_rights_access_sanitation_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droit à l&apos;Eau</div>
                </div>
              </div>
...
export default function WaterRightsAccessSanitationEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/water-rights-access-sanitation-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droit à l&apos;Eau &amp; Assainissement…</div>
      </div>
    );
  }
  ...
  const avgComposite = data?.avg_composite ?? avg(allEntities.map(e => e.water_access_denial_civilian_severity_score));
  const avgIndex = data?.avg_estimated_water_rights_access_sanitation_index ?? avg(allEntities.map(e => e.estimated_water_rights_access_sanitation_index));
  ...
  const avgWaterAccess = avg(allEntities.map(e => e.water_access_denial_civilian_severity_score));
  const avgSanitation = avg(allEntities.map(e => e.sanitation_infrastructure_collapse_score));
  const avgPrivatization = avg(allEntities.map(e => e.water_privatization_rights_erosion_score));
  const avgConflict = avg(allEntities.map(e => e.water_conflict_weaponization_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droit à l'Eau", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droit à l&apos;Eau &amp; Assainissement</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Water Rights Access &amp; Sanitation Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgWaterAccess} label="Déni Accès Eau Civils" />
          <GaugeRing value={avgSanitation} label="Effondrement Sanitaire" />
          <GaugeRing value={avgPrivatization} label="Privatisation Eau" />
          <GaugeRing value={avgConflict} label="Eau Armée Conflit" />
        </div>
      </div>
          ...
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right ml-3 flex-shrink-0">
                <div className="text-xl font-bold text-white">{e.water_access_denial_civilian_severity_score.toFixed(1)}</div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mt-2">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(e.water_access_denial_civilian_severity_score, 100)}%`, background: ACCENT }} />
            </div>
            <div className="text-xs text-slate-500 mt-2">
              Index Droit à l&apos;Eau: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_water_rights_access_sanitation_index === "number" ? e.estimated_water_rights_access_sanitation_index.toFixed(2) : "—"}</span>
            </div>
