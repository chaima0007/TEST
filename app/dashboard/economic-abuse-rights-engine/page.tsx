...
interface Entity {
  ...
  last_updated: string;
  financial_control_exploitation_score?: number;
  employment_sabotage_score?: number;
  debt_coercion_score?: number;
  economic_isolation_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "financial_control_exploitation_score", label: "Exploitation contrôle financier" },
  { key: "employment_sabotage_score", label: "Sabotage emploi" },
  { key: "debt_coercion_score", label: "Coercition par la dette" },
  { key: "economic_isolation_score", label: "Isolation économique" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de l&apos;abus économique : exploitation par le contrôle financier, sabotage de l&apos;emploi, coercition par la dette et isolation économique comme forme de violence domestique.</p>
              <p className="text-slate-400">Source : Economic Abuse Rights Engine · Wave 262</p>
            </div>
          )}
        </div>
      </div>
...
export default function EconomicAbuseRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/economic-abuse-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Economic Abuse Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Contrôle Financier · Emploi · Dette · Isolation</p>
        </div>
        ...
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${filter === f
              ? "border-yellow-900/40 text-yellow-300"
              : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            style={filter === f ? { backgroundColor: "#ca8a0420" } : {}}>
