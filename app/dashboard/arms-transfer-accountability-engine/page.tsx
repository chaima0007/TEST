...
interface DashData { agent?: string; total_entities?: number; avg_composite?: number; confidence_score?: number; risk_distribution?: Record<string,number>; entities?: Entity[]; [key: string]: unknown; }
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/arms-transfer-accountability-engine").then(r => r.json()).then(d => { setData(d); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Transferts d&apos;Armes</h1>
        <p className="text-slate-400 mt-1">Arms Transfer Accountability Engine — Analyse de la responsabilité dans les transferts d&apos;armements vers des régimes répressifs</p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800"><p className="text-slate-400 text-sm">Indice Armes Moyen</p><p className="text-2xl font-bold mt-1" style={{ color: ACCENT }}>{loading ? "…" : (data as any).avg_estimated_arms_transfer_accountability_index?.toFixed(2) ?? "—"}</p><p className="text-slate-500 text-xs mt-1">/10</p></div>
          ...
          <div className="bg-slate-900 rounded-2xl border border-slate-700 max-w-2xl w-full max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-slate-800">
              <div className="flex items-start justify-between gap-4">
                <div><p className="text-xs text-slate-400 mb-1">{selected.entity_id} · {selected.country}</p><h2 className="text-lg font-bold leading-snug">{selected.name}</h2></div>
            ...
            <div className="p-6">
              {tab === 0 && (<div className="space-y-3"><div className="flex justify-between text-sm"><span className="text-slate-400">Score composite</span><span className="font-bold">{selected.composite_score}/100</span></div><div className="flex justify-between text-sm"><span className="text-slate-400">Niveau de risque</span><span className={`font-bold uppercase ${RC[selected.risk_level]}`}>{selected.risk_level}</span></div><div className="flex justify-between text-sm"><span className="text-slate-400">Pattern principal</span><span className="text-slate-200">{selected.primary_pattern?.replace(/_/g," ")}</span></div><div className="pt-3 border-t border-slate-800"><p className="text-slate-400 text-xs mb-2">SECTEUR</p><p className="text-slate-300 text-sm">{selected.sector as string}</p></div></div>)}
