...
interface Entity {
  ...
  child_marriage_prevalence_harm_severity_score: number;
  forced_marriage_coercion_consent_violation_scale_score: number;
  girl_education_disruption_exclusion_score: number;
  legal_minimum_age_enforcement_deficit_gap_score: number;
  estimated_forced_marriage_child_marriage_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_forced_marriage_child_marriage_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Prévalence Mariage Enfants & Gravité", value: entity.child_marriage_prevalence_harm_severity_score, weight: "0.30" },
    { label: "Coercition & Violation Consentement", value: entity.forced_marriage_coercion_consent_violation_scale_score, weight: "0.25" },
    { label: "Exclusion Éducative des Filles", value: entity.girl_education_disruption_exclusion_score, weight: "0.25" },
    { label: "Déficit Application Âge Légal", value: entity.legal_minimum_age_enforcement_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_forced_marriage_child_marriage_rights_index === "number" ? entity.estimated_forced_marriage_child_marriage_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Mariage Forcé</div>
                </div>
              </div>
...
export default function ForcedMarriageChildMarriageRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/forced-marriage-child-marriage-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Mariage Forcé &amp; Mariage Enfants…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_forced_marriage_child_marriage_rights_index ?? avg(allEntities.map(e => e.estimated_forced_marriage_child_marriage_rights_index));
  ...
  const avgPrevalence = avg(allEntities.map(e => e.child_marriage_prevalence_harm_severity_score));
  const avgCoercition = avg(allEntities.map(e => e.forced_marriage_coercion_consent_violation_scale_score));
  const avgEducation = avg(allEntities.map(e => e.girl_education_disruption_exclusion_score));
  const avgLegal = avg(allEntities.map(e => e.legal_minimum_age_enforcement_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Mariage Forcé", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Mariage Forcé &amp; Mariage Enfants</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Forced Marriage &amp; Child Marriage Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgPrevalence} label="Prévalence Mariage Enfants" />
          <GaugeRing value={avgCoercition} label="Coercition &amp; Consentement" />
          <GaugeRing value={avgEducation} label="Exclusion Éducative Filles" />
          <GaugeRing value={avgLegal} label="Déficit Application Légale" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Mariage Forcé: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_forced_marriage_child_marriage_rights_index === "number" ? e.estimated_forced_marriage_child_marriage_rights_index.toFixed(2) : "—"}</span>
            </div>
