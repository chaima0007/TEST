...
export default function Page() {
  ...
  useEffect(() => { fetch("/api/right-to-be-forgotten-rights-engine").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
          ...
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 200px", display: "flex", flexDirection: "column", gap: 10, justifyContent: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#94a3b8" }}>Index RBF</span><strong style={{ color: ACCENT }}>{data.avg_estimated_right_to_be_forgotten_rights_index?.toFixed(2)}</strong></div>
          </div>
          ...
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 280px" }}>
            ...
            {[
              { label: "Refus effacement données", key: "data_erasure_denial" },
              { label: "Rétention données surveillance", key: "surveillance_data_retention" },
              { label: "Préjudice réputation numérique", key: "digital_reputation_harm" },
              { label: "Lacune conformité plateformes", key: "platform_compliance_gap" },
            ].map(m => (
