...
interface Entity {
  ...
  last_updated: string;
  indigenous_cultural_theft_score?: number;
  commercial_exploitation_sacred_score?: number;
  ip_protection_gap_score?: number;
  reparation_mechanism_absence_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "indigenous_cultural_theft_score", label: "Vol culturel autochtone" },
  { key: "commercial_exploitation_sacred_score", label: "Exploitation commerciale du sacré" },
  { key: "ip_protection_gap_score", label: "Lacunes en protection de la propriété intellectuelle" },
  { key: "reparation_mechanism_absence_score", label: "Absence de mécanismes de réparation" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse de l&apos;appropriation culturelle : vol culturel autochtone, exploitation commerciale du sacré, lacunes en protection de la propriété intellectuelle et absence de mécanismes de réparation.</p>
              <p className="text-slate-400">Source : Cultural Appropriation Rights Engine · Wave 251</p>
            </div>
          )}
        </div>
      </div>
...
export default function CulturalAppropriationRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/cultural-appropriation-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Cultural Appropriation Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Vol Culturel · Exploitation Commerciale · Propriété Intellectuelle · Réparation</p>
        </div>
