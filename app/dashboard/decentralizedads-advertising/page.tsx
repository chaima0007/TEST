...
type Entity = {
  ...
  risk_level: string;
  estimated_decentralizedads_index: number;
};
...
const SIGNALS = [
  "DAO governance consent",
  "P2P ad transparency",
  "Decentralized identity",
  "Community ad moderation",
  "Protocol fee disclosure",
];

const ACTIONS = [
  "Enforce DAO consent",
  "Ensure P2P transparency",
  "Protect decentralized identity",
  "Enable community moderation",
  "Disclose protocol fees",
];
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque decentralized ads : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_decentralizedads_index.toFixed(2)} / 10`,
    `Conformité decentralized ads : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 424 — Decentralized Ads Intelligence`,
  ];
}
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: COLOR, marginTop: 2 }}>Decentralized Ads · Wave 424</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: entity.estimated_decentralizedads_index * 10, color: COLOR },
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {ACTIONS.map((action, i) => (
              <div key={i} style={{ background: "#1e293b", borderRadius: 8, padding: 12, fontSize: 13, color: "#cbd5e1", border: "1px solid rgba(124,58,237,0.15)" }}>
                {action}
              </div>
            ))}
          </div>
...
export default function DecentralizedadsAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/decentralizedads-advertising")
      .then((r) => r.json())
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + e.estimated_decentralizedads_index, 0) / entities.length
    : 0;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Decentralized Ads Intelligence
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#7c3aed99", margin: 0 }}>
          Caelum Partners · Wave 424 · Analyse publicité décentralisée · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
        ...
        {filtered.map((entity, i) => (
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
              ...
              <span style={{ color: COLOR, fontWeight: 600 }}>{entity.estimated_decentralizedads_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Decentralized Ads Intelligence · Wave 424
      </div>
