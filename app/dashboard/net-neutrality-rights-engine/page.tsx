...
export default function Page() {
  ...
  useEffect(() => { fetch("/api/net-neutrality-rights-engine").then(r => r.json()).then(d => setData(d.payload ?? d)); }, []);
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
          ...
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 200px", display: "flex", flexDirection: "column", gap: 10, justifyContent: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#94a3b8" }}>Index NNR</span><strong style={{ color: ACCENT }}>{data.avg_estimated_net_neutrality_rights_index?.toFixed(2)}</strong></div>
          </div>
          ...
          <div style={{ background: "#0f172a", borderRadius: 8, padding: 20, flex: "1 1 280px" }}>
            ...
            {[
              { label: "Discrimination trafic", key: "traffic_discrimination" },
              { label: "Inégalité zero-rating", key: "zero_rating_inequity" },
              { label: "Harm priorisation payante", key: "paid_prioritization_harm" },
              { label: "Inégalité d&apos;accès", key: "access_inequality" },
            ].map(m => (
