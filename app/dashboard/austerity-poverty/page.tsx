...
interface Entity { entity: string; risk_level: string; avg_composite: number; estimated_austerity_poverty_index: number; }
...
export default function AusterityPovertyPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/austeritypoverty");
  ...
  return (
    <div className="p-6 space-y-6 text-slate-100">
      <div>
        <h1 className="text-2xl font-bold text-white">Austérité, Pièges à Pauvreté &amp; Droits Sociaux</h1>
        <p className="text-slate-400 text-sm mt-1">Coupes de prestations, banques alimentaires et exclusion sociale</p>
      </div>
