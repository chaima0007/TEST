...
interface Entity {
  ...
  last_updated: string;
  nonconsensual_treatment_score?: number;
  electroshock_coercion_score?: number;
  overmedication_control_score?: number;
  seclusion_restraint_abuse_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "nonconsensual_treatment_score", label: "Traitement non-consenti" },
  { key: "electroshock_coercion_score", label: "Coercition électrochocs" },
  { key: "overmedication_control_score", label: "Surmédicamentation contrôle" },
  { key: "seclusion_restraint_abuse_score", label: "Abus isolement et contention" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des abus psychiatriques : traitements non-consentis, coercition aux électrochocs, surmédicamentation comme outil de contrôle et abus d&apos;isolement et de contention.</p>
              <p className="text-slate-400">Source : Psychiatric Abuse Rights Engine · Wave 262</p>
            </div>
          )}
        </div>
      </div>
...
export default function PsychiatricAbuseRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/psychiatric-abuse-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Psychiatric Abuse Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Traitement Forcé · Électrochocs · Surmédicamentation · Isolement</p>
        </div>
