...
interface LIEntity {
  ...
  composite_score: number; lifespan_gap_score: number;
  health_access_inequality_score: number; longevity_tech_exclusion_score: number;
  demographic_bifurcation_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_longevity_fracture_index: number; last_updated: string;
}
interface LISummary {
  ...
  entities: LIEntity[]; avg_estimated_longevity_fracture_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: LIEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Écart d'Espérance de Vie", v: entity.lifespan_gap_score, c: "#ef4444" },
              { l: "Inégalité Accès Soins", v: entity.health_access_inequality_score, c: "#f97316" },
              { l: "Exclusion Technologie Longévité", v: entity.longevity_tech_exclusion_score, c: "#eab308" },
              { l: "Bifurcation Démographique", v: entity.demographic_bifurcation_score, c: "#f43f5e" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Fracture Longévité Caelum</span>
              <span className="text-rose-400 font-bold">{entity.estimated_longevity_fracture_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Plan d&apos;urgence de réduction des inégalités de santé — accès universel prioritaire</p><p>› Régulation des technologies de longévité pour éviter la bifurcation biologique</p></>}
            {entity.risk_level === "élevé" && <><p>› Programme d&apos;accès équitable aux soins de longévité</p><p>› Politiques de redistribution des bénéfices des technologies de santé</p></>}
            {entity.risk_level === "modéré" && <><p>› Stratégie de réduction des écarts d&apos;espérance de vie</p><p>› Investissement dans les infrastructures de santé préventive</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien des politiques d&apos;équité en santé et monitoring des nouvelles fractures</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function LongevityInequalityEnginePage() {
  const [data, setData] = useState<LISummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<LIEntity | null>(null);

  useEffect(() => {
    fetch("/api/longevity-inequality-engine")
      .then(r => r.json())
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données inégalités de longévité"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse des inégalités de longévité mondiales…</div></div>;
  ...
  const avgScore = (k: keyof LIEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Longevity Inequality Engine — Fractures Biologiques &amp; Exclusion de Longévité</h1>
          <p className="text-slate-400 text-sm">Mesure de la bifurcation mortelle : espérance de vie · accès soins · exclusion longévité · v{data.engine_version}</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { l: "Zones Analysées", v: data.total_entities, g: "from-slate-700 to-slate-600", t: "text-white" },
            { l: "Score Inégalité Moyen", v: `${data.avg_composite}%`, g: "from-violet-900 to-violet-800", t: "text-violet-300" },
            { l: "Alertes Critiques", v: data.critical_alerts.length, g: "from-red-900 to-red-800", t: "text-red-300" },
            { l: "Index Fracture Longévité", v: data.avg_estimated_longevity_fracture_index, g: "from-purple-900 to-purple-800", t: "text-purple-300" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs d&apos;Inégalité de Longévité Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("lifespan_gap_score")} label="Écart d'Espérance de Vie" color="#ef4444" />
            <GaugeRing value={avgScore("health_access_inequality_score")} label="Inégalité Accès Soins" color="#f97316" />
            <GaugeRing value={avgScore("longevity_tech_exclusion_score")} label="Exclusion Technologie Longévité" color="#eab308" />
            <GaugeRing value={avgScore("demographic_bifurcation_score")} label="Bifurcation Démographique" color="#f43f5e" />
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Distribution Risques Longévité</h2>
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Fracture Biologique</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={data.total_entities} color={["#ef4444", "#f97316", "#eab308", "#10b981", "#f43f5e"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-rose-900/10 border border-rose-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-rose-400 mb-2">Alertes Bifurcation Mortelle Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_longevity_fracture_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Longevity Inequality &amp; Fractures Biologiques · {data.last_analysis}</p>
      </div>
    </div>
  );
}
