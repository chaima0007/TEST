...
interface Entity {
  ...
  nuclear_testing_civilian_contamination_severity_score: number;
  reactor_accident_compensation_denial_scale_score: number;
  nuclear_waste_indigenous_land_disposal_score: number;
  radiation_information_transparency_deficit_gap_score: number;
  estimated_nuclear_radiation_civilian_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_nuclear_radiation_civilian_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Essais Nucléaires & Contamination", value: entity.nuclear_testing_civilian_contamination_severity_score, weight: "0.30" },
    { label: "Refus Compensation Accidents Réacteurs", value: entity.reactor_accident_compensation_denial_scale_score, weight: "0.25" },
    { label: "Déchets Nucléaires Terres Autochtones", value: entity.nuclear_waste_indigenous_land_disposal_score, weight: "0.25" },
    { label: "Déficit Transparence Radiation", value: entity.radiation_information_transparency_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_nuclear_radiation_civilian_rights_index === "number" ? entity.estimated_nuclear_radiation_civilian_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Civils Nucléaire</div>
                </div>
              </div>
...
export default function NuclearRadiationCivilianRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/nuclear-radiation-civilian-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Civils Nucléaire & Radiations…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_nuclear_radiation_civilian_rights_index ?? avg(allEntities.map(e => e.estimated_nuclear_radiation_civilian_rights_index));
  ...
  const avgNuclearTesting = avg(allEntities.map(e => e.nuclear_testing_civilian_contamination_severity_score));
  const avgReactorAccident = avg(allEntities.map(e => e.reactor_accident_compensation_denial_scale_score));
  const avgNuclearWaste = avg(allEntities.map(e => e.nuclear_waste_indigenous_land_disposal_score));
  const avgRadiationTransparency = avg(allEntities.map(e => e.radiation_information_transparency_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Civils Nucléaire", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Civils Nucléaire & Radiations</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Nuclear Radiation & Civilian Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgNuclearTesting} label="Essais Nucléaires" />
          <GaugeRing value={avgReactorAccident} label="Accidents Réacteurs" />
          <GaugeRing value={avgNuclearWaste} label="Déchets Nucléaires" />
          <GaugeRing value={avgRadiationTransparency} label="Transparence Radiation" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Civils Nucléaire: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_nuclear_radiation_civilian_rights_index === "number" ? e.estimated_nuclear_radiation_civilian_rights_index.toFixed(2) : "—"}</span>
            </div>
