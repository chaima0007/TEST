...
interface Entity {
  ...
  estimated_nuclear_testing_legacy_index: number;
  radiation_contamination_scale_score: number;
  health_cancer_mortality_score: number;
  displaced_communities_score: number;
  state_denial_reparations_score: number;
...
interface EngineResult {
  ...
  avg_estimated_nuclear_testing_legacy_index: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Contamination Radiation", value: entity.radiation_contamination_scale_score },
    { label: "Santé & Mortalité Cancer", value: entity.health_cancer_mortality_score },
    { label: "Communautés Déplacées", value: entity.displaced_communities_score },
    { label: "Déni État & Réparations", value: entity.state_denial_reparations_score },
  ];
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <div className="w-20 h-20 shrink-0">
                  <GaugeRing value={entity.estimated_nuclear_testing_legacy_index ?? entity.composite_score} />
                </div>
                <div>
                  <p className="text-slate-400 text-sm">Index Héritage Nucléaire</p>
                  <p className="text-white text-2xl font-bold">
                    {(entity.estimated_nuclear_testing_legacy_index ?? entity.composite_score).toFixed(1)}
                  </p>
...
export default function NuclearTestingLegacyEnginePage() {
  const [data, setData] = useState<EngineResult | null>(null);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<Entity | null>(null);

  useEffect(() => {
    fetch("/api/nuclear-testing-legacy-engine")
      .then((r) => r.json())
  ...
  if (!data) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="text-slate-400 animate-pulse">Chargement du module Héritage Tests Nucléaires…</div>
      </div>
    );
  }
  ...
  const kpis = [
    ...
    { label: "Index Héritage Nucléaire", value: data.avg_estimated_nuclear_testing_legacy_index?.toFixed(2) ?? "—" },
    ...
    <div className="bg-slate-950 min-h-screen text-slate-100 p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Héritage Tests Nucléaires</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Contamination radiation · Santé &amp; cancer · Communautés déplacées · Déni &amp; réparations — Caelum Partners
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6 flex items-center gap-6">
        ...
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1">
            Index Héritage: {(data.avg_estimated_nuclear_testing_legacy_index ?? 0).toFixed(2)}
          </p>
        </div>
      </div>
          ...
          {filtered.map((entity) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-2">
                <span className="text-slate-400 text-xs">Index Héritage Nucléaire</span>
              ...
              <div className="mt-2 grid grid-cols-2 gap-1 text-xs text-slate-500">
                <span>Radiation: {(entity.radiation_contamination_scale_score ?? 0).toFixed(1)}</span>
                <span>Santé: {(entity.health_cancer_mortality_score ?? 0).toFixed(1)}</span>
                <span>Déplacés: {(entity.displaced_communities_score ?? 0).toFixed(1)}</span>
                <span>Déni: {(entity.state_denial_reparations_score ?? 0).toFixed(1)}</span>
              </div>
            </button>
          ))}
        </div>
