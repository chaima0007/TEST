...
const API_URL = "/api/nanoinfluencer-advertising";
const TITLE = "Nano-Influencer Advertising";
const COLOR = "#16a34a";
const DOMAIN = "nanoinfluencer-advertising";
const INDEX_KEY = "estimated_nanoinfluencer_index";

const SIGNALS = [
  "Community trust consent",
  "Local audience data",
  "Nano creator rights",
  "Word-of-mouth disclosure",
  "Small audience profiling",
];

const ACTIONS = [
  "Enforce community consent",
  "Protect local audience data",
  "Secure nano creator rights",
  "Require word-of-mouth disclosure",
  "Limit small audience profiling",
];
...
function DetailModal({ entity, color, onClose }: { entity: Entity; color: string; onClose: () => void }) {
  ...
  const indexValue = typeof entity[INDEX_KEY] === "number" ? (entity[INDEX_KEY] as number) : null;
  return (
      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: 24, width: 520, maxWidth: "90vw", maxHeight: "80vh", overflowY: "auto" }}>
        ...
        {tab === "apercu" && (
          <div>
            ...
            {indexValue !== null && <p style={{ marginBottom: 8 }}>Index {DOMAIN} : <strong>{indexValue}</strong></p>}
            <div style={{ marginTop: 16 }}>
              <p style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Signaux de risque :</p>
        ...
        {tab === "indicateurs" && (
          <p>Conformité {TITLE} — les nano-influenceurs et les marques qui les mandatent doivent respecter le consentement des communautés locales, protéger les données des petites audiences et garantir la divulgation complète du bouche-à-oreille sponsorisé conformément aux droits des créateurs et aux lois publicitaires applicables.</p>
        )}
        {tab === "recommandations" && (
          <div>
            <p style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Actions prioritaires :</p>
            <ul style={{ paddingLeft: 20, fontSize: 13, color: "#374151" }}>
              {ACTIONS.map(a => <li key={a} style={{ marginBottom: 4 }}>{a}</li>)}
            </ul>
          </div>
        )}
        {tab === "certifications" && (
...
export default function NanoinfluencerAdvertisingDashboard() {
