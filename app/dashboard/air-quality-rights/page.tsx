...
interface Entity {
  ...
  last_updated: string;
  particulate_matter_score?: number;
  emission_regulation_score?: number;
  clean_air_access_score?: number;
  respiratory_health_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "particulate_matter_score", label: "Particules fines (PM2.5/PM10)" },
  { key: "emission_regulation_score", label: "Régulation des émissions" },
  { key: "clean_air_access_score", label: "Accès à l&apos;air pur" },
  { key: "respiratory_health_score", label: "Santé respiratoire" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits à la qualité de l&apos;air : exposition aux particules fines, régulation des émissions industrielles, accès à un air pur et impacts sur la santé respiratoire des populations.</p>
              <p className="text-slate-400">Source : Air Quality Rights Engine · Wave 264</p>
            </div>
          )}
        </div>
      </div>
...
export default function AirQualityRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/air-quality-rights").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Droits à la Qualité de l&apos;Air</h1>
          <p className="text-slate-400 mt-1 text-sm">Particules Fines · Émissions Industrielles · Accès Air Pur · Santé Respiratoire</p>
        </div>
