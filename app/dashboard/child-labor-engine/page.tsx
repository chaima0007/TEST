...
interface ChildLaborEntity {
  id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  hazardous_child_labor_rate_score: number;
  forced_labor_recruitment_score: number;
  school_access_deprivation_score: number;
  regulatory_enforcement_failure_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_child_labor_index: number;
  last_updated: string;
}

interface ChildLaborSummary {
  ...
  data_sources: string[];
  avg_estimated_child_labor_index: number;
}

interface ChildLaborData {
  entities: ChildLaborEntity[];
  summary: ChildLaborSummary;
}
...
function EntityCard({ entity, onClick }: { entity: ChildLaborEntity; onClick: (e: ChildLaborEntity) => void }) {
    ...
    <button onClick={() => onClick(entity)}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.hazardous_child_labor_rate_score}</p>
          <p className="text-[10px] text-gray-500">Dangereux</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.forced_labor_recruitment_score}</p>
          <p className="text-[10px] text-gray-500">Forcé</p>
        </div>
        <div className="bg-white/4 rounded-lg p-2 text-center">
          <p className="text-sm font-bold text-white">{entity.school_access_deprivation_score}</p>
          <p className="text-[10px] text-gray-500">École</p>
        </div>
      </div>
...
function DetailModal({ entity, onClose }: { entity: ChildLaborEntity; onClose: () => void }) {
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Travail Dangereux Enfants", value: `${entity.hazardous_child_labor_rate_score}/100` },
                  { label: "Recrutement Forcé", value: `${entity.forced_labor_recruitment_score}/100` },
                  { label: "Privation Scolaire", value: `${entity.school_access_deprivation_score}/100` },
                  { label: "Échec Réglementation", value: `${entity.regulatory_enforcement_failure_score}/100` },
                  { label: "Indice CL", value: `${entity.estimated_child_labor_index}/10` },
              ...
              <div className="bg-white/4 rounded-lg p-4">
                ...
                <p className="text-[11px] text-gray-500 mt-1.5">
                  Formule : dangereux×0.30 + forcé×0.25 + école×0.25 + réglementation×0.20
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "contexte" && (
            <div className="space-y-3">
              <div className="bg-white/4 border border-white/10 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 mt-1"><span className="text-gray-300 font-medium">Indice CL :</span> {entity.estimated_child_labor_index}/10</p>
              </div>
              <div className={`rounded-xl border p-4 ${cfg.bg} ${cfg.border}`}>
                <p className={`text-sm font-semibold ${cfg.color}`}>{entity.primary_pattern}</p>
                <p className="text-xs text-gray-400 mt-2">Renforcer l'accès à l'éducation, éliminer le travail dangereux et renforcer les mécanismes d'application de la réglementation.</p>
              </div>
            </div>
          )}
        </div>
...
export default function ChildLaborEnginePage() {
  const [data, setData] = useState<ChildLaborData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<ChildLaborEntity | null>(null);

  useEffect(() => {
    fetch("/api/child-labor-engine")
      .then((r) => r.json())
      .then((json) => {
        const payload = json?.data ?? json;
        setData(payload as ChildLaborData);
        setLoading(false);
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Child Labor Engine</h1>
            <p className="text-sm text-gray-400 mt-1">
              Travail des Enfants &amp; Exploitation Économique Infantile
            </p>
          </div>
        ...
        {!loading && s && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <KpiCard label="Indice CL moyen" value={`${s.avg_estimated_child_labor_index ?? "—"}/10`} sub="child labor index" accent="text-amber-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/4 border border-white/10 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-gray-300 mb-6">Indicateurs Travail des Enfants</h2>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <GaugeRing value={s.avg_estimated_child_labor_index ?? 0} max={10} label="Indice CL" color={ACCENT} />
                </div>
              </div>
            ...
            {s.critical_alerts > 0 && (
              <div className="rounded-xl px-5 py-4 flex items-start gap-3" style={{ backgroundColor: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)" }}>
                <div className="w-2 h-2 rounded-full mt-1.5 shrink-0 animate-pulse" style={{ backgroundColor: ACCENT }} />
                <div>
                  <p className="text-sm font-medium" style={{ color: ACCENT }}>
                    {s.critical_alerts} entité{s.critical_alerts > 1 ? "s" : ""} en situation critique de travail des enfants
                  </p>
                  <p className="text-xs mt-0.5" style={{ color: `${ACCENT}b3` }}>
                    Intervention immédiate requise — protection de l'enfance prioritaire
                  </p>
                </div>
              </div>
            )}
