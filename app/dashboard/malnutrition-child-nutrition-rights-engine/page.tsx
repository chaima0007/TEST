...
interface MCNREntity {
  entity_id: string;
  name: string;
  country: string;
  sector: string;
  composite_score: number;
  acute_malnutrition_prevalence_score: number;
  stunting_wasting_severity_score: number;
  food_system_access_gap_score: number;
  child_mortality_nutrition_score: number;
  estimated_malnutrition_child_nutrition_rights_index: number;
  risk_level: string;
...
interface MCNRData {
  ...
  data_sources: string[];
  entities: MCNREntity[];
  avg_estimated_malnutrition_child_nutrition_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function MalnutritionChildNutritionRightsPage() {
  const [data, setData] = useState<MCNRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<MCNREntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/malnutrition-child-nutrition-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-amber-400 animate-pulse">Chargement des données malnutrition…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "acute_malnutrition_prevalence_score", label: "Prévalence Malnutrition Aiguë" },
    { key: "stunting_wasting_severity_score", label: "Retard Croissance / Émaciation" },
    { key: "food_system_access_gap_score", label: "Accès Système Alimentaire" },
    { key: "child_mortality_nutrition_score", label: "Mortalité Infantile Nutrition" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: ACCENT, boxShadow: `0 0 6px #f59e0b` }} />
          <h1 className="text-2xl font-bold tracking-tight">Malnutrition &amp; Droits Nutritionnels de l&apos;Enfant</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">UNICEF · OMS · PAM · 148M enfants stunted · Objectifs nutrition 2025</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_malnutrition_child_nutrition_rights_index ?? "—" },
            { label: "Confidence", value: `${Math.round((data.confidence_score ?? 0) * 100)}%` },
          ].map(k => (
            <div key={k.label} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              <div className="text-xs text-slate-500 mb-1">{k.label}</div>
              <div className="text-xl font-bold text-amber-400">{k.value}</div>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#f59e0b" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns Nutritionnels</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#f59e0b", "#d97706", "#b45309", "#fcd34d", "#fde68a"];
        ...
        {(data.critical_alerts?.length ?? 0) > 0 && (
          <div className="border border-amber-500/30 bg-amber-500/10 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-amber-400 mb-2">Alertes Critiques</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              className={`bg-slate-900 border rounded-xl p-4 cursor-pointer hover:border-amber-500/50 transition-all ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#f59e0b" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_malnutrition_child_nutrition_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Malnutrition &amp; Droits Nutritionnels Enfant · {data.last_analysis}</p>
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Vue générale", "Sous-scores", "Alertes"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`px-6 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-amber-500 text-amber-400" : "text-slate-500 hover:text-slate-300"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#f59e0b" />
                    <div>
                      <div className="text-2xl font-bold text-amber-400">{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Malnutrition &amp; Droits Nutritionnels Enfant</div>
                    <div className="text-lg font-bold text-amber-400">{selected.estimated_malnutrition_child_nutrition_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
                    ...
                    return (
                      <div key={key}>
                        ...
                        <div className="h-2 bg-slate-800 rounded-full">
                          <div className="h-full rounded-full bg-amber-500" style={{ width: `${val}%` }} />
                        </div>
                      </div>
                    );
                  })}
