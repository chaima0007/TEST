...
interface Entity {
  ...
  femicide_rate_impunity_score: number
  legal_protection_enforcement_deficit_score: number
  gender_violence_systemic_failure_score: number
  state_accountability_prevention_score: number
...
interface DomainData {
  ...
  avg_estimated_femicide_index?: number
  data_sources?: string[]
}
...
function EntityCard({ entity, onClick }: { entity: Entity; onClick: () => void }) {
  return (
    <div onClick={onClick} style={{ background: "#0f172a", border: `1px solid ${RC[entity.risk_level]}33`, borderRadius: 12, padding: 16, cursor: "pointer", transition: "border-color 0.2s" }}>
      ...
      <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
        <div style={{ background: "#1e293b", borderRadius: 6, padding: "4px 8px", fontSize: 11, color: "#94a3b8" }}>
          Index: <span style={{ color: ACCENT, fontWeight: 700 }}>{entity.estimated_femicide_index.toFixed(2)}</span>
        </div>
        <div style={{ background: "#1e293b", borderRadius: 6, padding: "4px 8px", fontSize: 11, color: "#94a3b8" }}>
...
function Modal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Taux Féminicide & Impunité", value: entity.femicide_rate_impunity_score, weight: "0.30" },
    { label: "Déficit Protection Légale", value: entity.legal_protection_enforcement_deficit_score, weight: "0.25" },
    { label: "Échec Systémique Violence Genre", value: entity.gender_violence_systemic_failure_score, weight: "0.25" },
    { label: "Responsabilité État & Prévention", value: entity.state_accountability_prevention_score, weight: "0.20" },
  ]
  return (
          ...
          {tab === "apercu" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div style={{ background: "#1e293b", borderRadius: 12, padding: 16, textAlign: "center" }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: ACCENT }}>{entity.estimated_femicide_index.toFixed(2)}</div>
                  <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>Index Féminicide</div>
                </div>
              </div>
...
export default function FemicideGenderBasedKillingsPage() {
  ...
  useEffect(() => {
    fetch("/api/femicide-gender-based-killings-engine")
      .then(r => r.json())
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 16px" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
            ...
            <div>
              <h1 style={{ fontSize: 28, fontWeight: 800, color: "#f1f5f9", margin: 0 }}>Féminicide & Crimes de Genre</h1>
              <p style={{ fontSize: 14, color: "#64748b", margin: 0, marginTop: 4 }}>Wave 184 — Surveillance mondiale des féminicides et violences létales basées sur le genre</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ borderTop: "1px solid #1e293b", paddingTop: 16, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          <span style={{ fontSize: 12, color: "#475569" }}>Caelum Partners — Wave 184 — Féminicide & Crimes de Genre</span>
