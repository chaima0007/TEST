...
interface Entity {
  ...
  nuclear_testing_radiation_civilian_harm_severity_score: number;
  nuclear_arsenal_modernization_proliferation_scale_score: number;
  nuclear_deterrence_civilian_threat_score: number;
  tpnw_npt_disarmament_compliance_deficit_gap_score: number;
  estimated_nuclear_weapons_disarmament_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_nuclear_weapons_disarmament_rights_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Tests Nucléaires & Victimes Civiles", value: entity.nuclear_testing_radiation_civilian_harm_severity_score, weight: "0.30" },
    { label: "Modernisation Arsenal & Prolifération", value: entity.nuclear_arsenal_modernization_proliferation_scale_score, weight: "0.25" },
    { label: "Dissuasion & Menace Populations Civiles", value: entity.nuclear_deterrence_civilian_threat_score, weight: "0.25" },
    { label: "Déficit Conformité TPNW/TNP", value: entity.tpnw_npt_disarmament_compliance_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_nuclear_weapons_disarmament_rights_index === "number" ? entity.estimated_nuclear_weapons_disarmament_rights_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Désarmement Nucléaire</div>
                </div>
              </div>
...
export default function NuclearWeaponsDisarmamentRightsEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/nuclear-weapons-disarmament-rights-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Armes Nucléaires &amp; Désarmement…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_nuclear_weapons_disarmament_rights_index ?? avg(allEntities.map(e => e.estimated_nuclear_weapons_disarmament_rights_index));
  ...
  const avgTesting = avg(allEntities.map(e => e.nuclear_testing_radiation_civilian_harm_severity_score));
  const avgArsenal = avg(allEntities.map(e => e.nuclear_arsenal_modernization_proliferation_scale_score));
  const avgDeterrence = avg(allEntities.map(e => e.nuclear_deterrence_civilian_threat_score));
  const avgCompliance = avg(allEntities.map(e => e.tpnw_npt_disarmament_compliance_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Désarmement", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Armes Nucléaires &amp; Désarmement</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Nuclear Weapons &amp; Disarmament Rights Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgTesting} label="Tests &amp; Victimes Civiles" />
          <GaugeRing value={avgArsenal} label="Modernisation Arsenal" />
          <GaugeRing value={avgDeterrence} label="Dissuasion Civile" />
          <GaugeRing value={avgCompliance} label="Conformité TPNW/TNP" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Désarmement: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_nuclear_weapons_disarmament_rights_index === "number" ? e.estimated_nuclear_weapons_disarmament_rights_index.toFixed(2) : "—"}</span>
            </div>
