...
interface SwarmData { engine: string; avg_composite: number; distribution: Record<string, number>; entities: Entity[]; estimated_sovereign_debt_index: number; }
...
export default function SovereignDebtPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/sovereigndebt");
  ...
  return (
    <div className="p-6 space-y-6 text-slate-100">
      <div>
        <h1 className="text-2xl font-bold text-white">Dette Souveraine &amp; Droits Humains</h1>
        <p className="text-slate-400 text-sm mt-1">Austérité forcée, restructurations et impact sur les droits sociaux</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        {data.estimated_sovereign_debt_index != null && (
          <p className="text-xs text-slate-500 mt-3">Indice dette souveraine : <span className="text-indigo-300 font-semibold">{data.estimated_sovereign_debt_index}/10</span></p>
        )}
      </div>
    </div>
  );
