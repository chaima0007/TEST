...
interface Entity {
  ...
  domestic_care_burden_gender_gap_score: number;
  economic_recognition_unpaid_work_absence_score: number;
  childcare_eldercare_infrastructure_gap_score: number;
  pension_social_protection_care_exclusion_score: number;
  estimated_unpaid_care_work_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  entities: Entity[];
  avg_estimated_unpaid_care_work_rights_index: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "domestic_care_burden_gender_gap_score", label: "Charge Domestique Genrée" },
  { key: "economic_recognition_unpaid_work_absence_score", label: "Absence Reconnaissance Économique" },
  { key: "childcare_eldercare_infrastructure_gap_score", label: "Gap Infrastructure Care" },
  { key: "pension_social_protection_care_exclusion_score", label: "Exclusion Retraite/Protection" },
];
        ...
        <div className="flex border-b border-slate-800">
          {["Aperçu", "Métriques", "Sources"].map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-pink-500/50 text-pink-400" : "text-slate-400 hover:text-white"}`}>
              {t}
            </button>
          ))}
        </div>
          ...
          {tab === 0 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Indice Care Non Rémunéré</span>
                <span className="font-semibold" style={{ color: ACCENT }}>{entity.estimated_unpaid_care_work_rights_index}/10</span>
              </div>
              <div className="flex items-center justify-between text-sm">
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/unpaid-care-work-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Travail de Care Non Rémunéré</h1>
          <p className="text-slate-400 mt-1 text-sm">Charge Domestique · Reconnaissance Économique · Infrastructure Care · Protection Sociale</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice UCW</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_unpaid_care_work_rights_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        {filtered.map(e => {
          ...
          return (
            <button key={e.entity_id} onClick={() => setSelected(e)}
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold uppercase ${cfg.color}`}>{cfg.label}</span>
                <span className="text-xs text-slate-500">Indice: <span className="font-bold" style={{ color: ACCENT }}>{e.estimated_unpaid_care_work_rights_index}</span></span>
              </div>
            </button>
          );
        })}
