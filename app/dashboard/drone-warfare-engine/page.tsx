...
type DWEntity = {
  ...
  targeted_killing_program_score: number;
  autonomous_lethal_systems_score: number;
  civilian_casualty_rate_score: number;
  drone_proliferation_export_score: number;
  estimated_drone_warfare_index: number;
  risk_level: string;
          ...
          {tab === "scores" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>
                    {entity.estimated_drone_warfare_index.toFixed(1)}
                  </div>
                  <div className="text-xs text-slate-400 mt-1">Index Drone Warfare</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Programme Assassinats Ciblés", value: entity.targeted_killing_program_score },
                  { label: "Systèmes Létaux Autonomes", value: entity.autonomous_lethal_systems_score },
                  { label: "Taux Victimes Civiles", value: entity.civilian_casualty_rate_score },
                  { label: "Prolifération & Export Drones", value: entity.drone_proliferation_export_score },
                ].map((s) => (
...
export default function DroneWarfareEnginePage() {
  const [entities, setEntities] = useState<DWEntity[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    fetch("/api/drone-warfare-engine")
      .then((r) => r.json())
      .then((json: ApiResponse) => {
        const rows = json?.data ?? (Array.isArray(json) ? json : []);
        setEntities(rows as DWEntity[]);
        setLoading(false);
      })
      .catch(() => {
  ...
  const avgTargetedKilling = avg(entities.map((e) => e.targeted_killing_program_score));
  const avgAutonomousLethal = avg(entities.map((e) => e.autonomous_lethal_systems_score));
  const avgCivilianCasualty = avg(entities.map((e) => e.civilian_casualty_rate_score));
  const avgProliferation = avg(entities.map((e) => e.drone_proliferation_export_score));
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-white">Drone Warfare Engine</h1>
        </div>
        <p className="text-slate-400 ml-5">
          Frappes Ciblées, Drones Kamikazes &amp; Systèmes Létaux Autonomes — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 rounded-2xl p-6 mb-8" style={{ border: `1px solid ${ACCENT}1a` }}>
        ...
        <div className="flex flex-wrap justify-around gap-6">
          <GaugeRing value={avgTargetedKilling} label="Assassinats Ciblés" color={ACCENT} />
          <GaugeRing value={avgAutonomousLethal} label="Systèmes Létaux Auto." color="#7c3aed" />
          <GaugeRing value={avgCivilianCasualty} label="Victimes Civiles" color="#0891b2" />
          <GaugeRing value={avgProliferation} label="Prolifération Drones" color="#b45309" />
        </div>
      </div>
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs text-slate-500">Index DWE</div>
                </div>
              </div>
              <div
