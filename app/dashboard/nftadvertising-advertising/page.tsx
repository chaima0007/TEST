...
const API_URL = "/api/nftadvertising-advertising";
const TITLE = "NFT Advertising";
const COLOR = "#06b6d4";
const INDEX_KEY = "estimated_nftadvertising_index";

const SIGNALS = [
  "NFT holder data consent",
  "Marketplace tracking",
  "Secondary sale transparency",
  "NFT audience profiling",
  "Royalty disclosure compliance",
];

const ACTIONS = [
  "Enforce NFT holder consent",
  "Limit marketplace tracking",
  "Disclose secondary sales",
  "Restrict NFT profiling",
  "Require royalty disclosure",
];

type Entity = { entity: string; composite_score: number; risk_level: string; [key: string]: unknown };
type ApiData = { entities: Entity[]; avg_composite: number; distribution: Record<string, number>; [key: string]: unknown };
...
function DetailModal({ entity, color, onClose }: { entity: Entity; color: string; onClose: () => void }) {
  ...
  const indexVal = typeof entity[INDEX_KEY] === "number" ? (entity[INDEX_KEY] as number) : null;
  return (
      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: 24, width: 520, maxWidth: "90vw", maxHeight: "80vh", overflowY: "auto" }}>
        ...
        {tab === "apercu" && (
          <div>
            ...
            <p style={{ marginBottom: 8 }}>Risque : <strong style={{ color }}>{entity.risk_level}</strong></p>
            {indexVal !== null && <p style={{ marginBottom: 8 }}>Index NFT Advertising : <strong>{indexVal}</strong></p>}
            <div style={{ marginTop: 16 }}>
              <p style={{ fontWeight: 600, marginBottom: 8, fontSize: 13 }}>Signaux surveillés :</p>
              <ul style={{ paddingLeft: 16, fontSize: 13, color: "#374151" }}>
                {SIGNALS.map(s => <li key={s} style={{ marginBottom: 4 }}>{s}</li>)}
              </ul>
            </div>
          </div>
        )}
        {tab === "indicateurs" && (
          <p>Conformité NFT Advertising — les marketplaces NFT et plateformes publicitaires associées doivent garantir le consentement des détenteurs de NFT, la transparence des ventes secondaires et la divulgation des royalties conformément aux standards émergents de gouvernance des actifs numériques.</p>
        )}
        {tab === "recommandations" && (
          <div>
            <p style={{ marginBottom: 12 }}>Actions prioritaires pour renforcer la conformité NFT Advertising :</p>
            <ul style={{ paddingLeft: 16, fontSize: 13, color: "#374151" }}>
              {ACTIONS.map(a => <li key={a} style={{ marginBottom: 6 }}>{a}</li>)}
            </ul>
          </div>
        )}
        {tab === "certifications" && (
...
export default function NftadvertisingAdvertisingDashboard() {
