...
interface Entity {
  ...
  forced_displacement_land_seizure_severity_score: number;
  legal_title_recognition_absence_scale_score: number;
  corporate_state_complicity_land_grab_score: number;
  indigenous_community_consultation_gap_score: number;
  estimated_land_grabbing_rights_index: number;
  risk_level: string;
...
interface DashData {
  ...
  critical_alerts: { length?: number } | number | null;
  confidence_score: number;
  entities: Entity[];
  avg_estimated_land_grabbing_rights_index: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "forced_displacement_land_seizure_severity_score", label: "Déplacement Forcé & Saisie Terres" },
  { key: "legal_title_recognition_absence_scale_score", label: "Absence Titre Légal" },
  { key: "corporate_state_complicity_land_grab_score", label: "Complicité Corp./État" },
  { key: "indigenous_community_consultation_gap_score", label: "Gap Consultation Autochtones" },
];
        ...
        <div className="flex border-b border-slate-800">
          {["Aperçu", "Métriques", "Sources"].map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-teal-500/50 text-teal-400" : "text-slate-400 hover:text-white"}`}>
              {t}
            </button>
          ))}
        </div>
          ...
          {tab === 0 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Indice Accaparement Terres</span>
                <span className="font-semibold" style={{ color: ACCENT }}>{entity.estimated_land_grabbing_rights_index}/10</span>
              </div>
              <div className="flex items-center justify-between text-sm">
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/land-grabbing-rights-engine").then(r=>r.json()).then(d=>{ setData(d.payload??d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Land Grabbing Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Déplacement Forcé · Titre Légal · Complicité Corp./État · Consultation Autochtones</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Entités analysées</p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col items-center">
          <p className="text-xs text-slate-500 uppercase tracking-wide mb-2">Score composite moyen</p>
          <GaugeRing value={data.avg_composite} stroke={ACCENT} />
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Indice Accaparement Terres</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_land_grabbing_rights_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Entités critique</p>
          <p className="text-3xl font-bold mt-1 text-red-400">{data.risk_distribution?.critique ?? 0}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Confiance IA</p>
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{(data.confidence_score * 100).toFixed(0)}%</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Alertes critiques</p>
          <p className="text-3xl font-bold mt-1 text-red-400">{(data.critical_alerts as { length?: number } | null)?.length ?? 0}</p>
        </div>
      </div>
        ...
        {filtered.map(e => {
          ...
          return (
            <button key={e.entity_id} onClick={() => setSelected(e)}
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold uppercase ${cfg.color}`}>{cfg.label}</span>
                <span className="text-xs text-slate-500">Indice: <span className="font-bold" style={{ color: ACCENT }}>{e.estimated_land_grabbing_rights_index}</span></span>
              </div>
            </button>
          );
        })}
