...
const ACCENT = "#92400e";
const TITRE = "Accaparement des Terres, Agrobusiness & Déplacements Paysans";
      ...
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 24, maxWidth: 600, width: "90%", maxHeight: "80vh", overflowY: "auto" }}
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {(["apercu", "metriques", "sources"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              style={{ padding: "6px 12px", borderRadius: 6, border: "none", cursor: "pointer", background: tab === t ? accent : "#1e293b", color: tab === t ? "#fff" : "#94a3b8" }}>
              {t}
            </button>
          ))}
        </div>
        {tab === "apercu" && (
          <div style={{ color: "#cbd5e1", fontSize: 14, lineHeight: 1.7 }}>
            <div>Score : <strong style={{ color: accent }}>{entity?.composite_score}</strong></div>
            <div>Risque : <strong style={{ color: accent }}>{entity?.risk_level}</strong></div>
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/land-grabbing-rights-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  return (
    <div style={{ background: "#020617", minHeight: "100vh", color: "#f1f5f9", fontFamily: "system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <p style={{ color: "#94a3b8", marginBottom: 32 }}>CaelumSwarm™ · CSDDD 2024/1760 · Wave 225</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 16 }}>
          {entities.map((e: any) => (
            <div key={e.entity_id ?? e.id} onClick={() => setSelected(e)}
              ...
              <div style={{ fontSize: 11, color: "#64748b", marginBottom: 4 }}>{e.entity_id ?? e.id}</div>
