...
interface NREntity {
  ...
  educational_inclusion_denial_severity_score: number;
  employment_discrimination_neurodivergent_scale_score: number;
  diagnostic_access_barrier_score: number;
  stigma_criminalization_autistic_pattern_score: number;
  estimated_neurodiversity_rights_index: number;
  risk_level: string;
...
interface NRData {
  ...
  data_sources: string[];
  entities: NREntity[];
  avg_estimated_neurodiversity_rights_index: number;
  engine_version: string;
  [key: string]: unknown;
}

export default function NeurodiversityRightsPage() {
  const [data, setData] = useState<NRData | null>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("tous");
  const [selected, setSelected] = useState<NREntity | null>(null);
  const [tab, setTab] = useState(0);

  useEffect(() => {
    fetch("/api/neurodiversity-rights-engine")
      .then(r => r.json())
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Chargement des données de neurodiversité…</div>
    </div>
  );
  if (!data) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  const subKeys = [
    { key: "educational_inclusion_denial_severity_score", label: "Déni Inclusion Scolaire" },
    { key: "employment_discrimination_neurodivergent_scale_score", label: "Discrimination Emploi" },
    { key: "diagnostic_access_barrier_score", label: "Accès au Diagnostic" },
    { key: "stigma_criminalization_autistic_pattern_score", label: "Stigmatisation Autistique" },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits à la Neurodiversité</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">Autisme, TDAH, dyslexie — inclusion scolaire et emploi</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Index Moyen", value: data.avg_estimated_neurodiversity_rights_index ?? "—" },
            ...
            <div key={k.label} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              <div className="text-xl font-bold text-indigo-400">{k.value}</div>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-wrap justify-around gap-6">
            {subKeys.map(({ key, label }) => (
              <div key={key} className="flex flex-col items-center gap-2">
                <GaugeRing value={avgSub(key)} stroke="#818cf8" />
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            ...
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution ?? {}).map(([k, v], i) => {
                const colors = ["#818cf8", "#f97316", "#eab308", "#ef4444", "#10b981"];
        ...
        {(data.critical_alerts?.length ?? 0) > 0 && (
          <div className="border border-indigo-500/30 bg-indigo-500/10 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-indigo-400 mb-2">Alertes Critiques</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => { setSelected(entity); setTab(0); }}
              className={`bg-slate-900 border rounded-xl p-4 cursor-pointer hover:border-indigo-500/50 transition-all ${RB[entity.risk_level] ?? "border-slate-800"}`}>
              <div className="flex justify-between items-start mb-3">
                ...
                <GaugeRing value={entity.composite_score} stroke="#818cf8" />
              </div>
              <div className="flex justify-between text-xs">
                <span className={RC[entity.risk_level]}>{entity.risk_level}</span>
                <span className="text-slate-500">idx {entity.estimated_neurodiversity_rights_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Droits à la Neurodiversité · {data.last_analysis}</p>
      </div>
            ...
            <div className="flex border-b border-slate-800">
              {["Aperçu", "Métriques", "Sources"].map((t, i) => (
                <button key={t} onClick={() => setTab(i)}
                  className={`px-6 py-3 text-sm font-medium transition-colors ${tab === i ? "border-b-2 border-indigo-500 text-indigo-400" : "text-slate-500 hover:text-slate-300"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <GaugeRing value={selected.composite_score} stroke="#818cf8" />
                    <div>
                      <div className="text-2xl font-bold text-indigo-400">{selected.composite_score.toFixed(1)}</div>
                  ...
                  <div className="bg-slate-800 rounded-lg p-3">
                    <div className="text-xs text-slate-500 mb-1">Index Neurodiversité</div>
                    <div className="text-lg font-bold text-indigo-400">{selected.estimated_neurodiversity_rights_index}</div>
                  </div>
                </div>
              )}
              {tab === 1 && (
