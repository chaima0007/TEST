...
export default function ReligiousPersecutionMinorityRightsPage() {
  const [data, setData] = useState<EngineData | null>(null);
  const ACCENT = "#581c87";
  const RC: Record<string, string> = { critique: "#ef4444", "élevé": "#f97316", "modéré": "#eab308", faible: "#22c55e" };

  useEffect(() => {
    fetch("/api/religious-persecution-minority-rights-engine").then(r => r.json()).then(d => setData(d.payload ?? d));
  }, []);
  ...
  const indexValue = (data["avg_estimated_religious_persecution_minority_rights_index"] as number) ?? 0;
