...
type Entity = {
  ...
  risk_level: string;
  estimated_carboncapture_index: number;
};
...
function deriveCerts(entity: Entity): string[] {
  return [
    ...
    `Niveau de risque publicitaire Carbon Capture : ${entity.risk_level}`,
    `Indice estimé Caelum : ${entity.estimated_carboncapture_index.toFixed(2)} / 10`,
    `Conformité campagne Carbon Capture : ${entity.composite_score >= 60 ? "Surveillance active" : "Monitoring standard"}`,
    `Certification Wave 390 — Carbon Capture Advertising Intelligence`,
  ];
}
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        style={{ background: "#0a0f1e", border: "1px solid rgba(100,116,139,0.3)", borderRadius: 16, width: "100%", maxWidth: 520, padding: 24, boxShadow: "0 25px 50px rgba(0,0,0,0.7)" }}
        onClick={(e) => e.stopPropagation()}
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700, color: "#fff" }}>{entity.name}</div>
            <div style={{ fontSize: 12, color: `${COLOR}99`, marginTop: 2 }}>Carbon Capture Advertising · Wave 390</div>
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { label: "Index Estimé", value: entity.estimated_carboncapture_index * 10, color: COLOR },
            ].map(({ label, value, color }) => (
              <div key={label} style={{ background: "#1e293b", borderRadius: 10, padding: 14, border: "1px solid rgba(100,116,139,0.2)" }}>
            ...
            <div style={{ gridColumn: "span 2", background: "#1e293b", borderRadius: 10, padding: 14, border: "1px solid rgba(100,116,139,0.2)" }}>
        ...
        {tab === "signaux" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              `Score composite ${entity.composite_score.toFixed(1)} — ${entity.risk_level === "critique" ? "surveillance publicitaire Carbon Capture urgente" : entity.risk_level === "élevé" ? "monitoring renforcé recommandé" : "suivi standard actif"}`,
              `Indice Caelum Carbon Capture Advertising : ${entity.estimated_carboncapture_index.toFixed(2)} / 10`,
              `Campagne Carbon Capture en ${entity.risk_level === "faible" ? "zone verte — performance nominale" : "zone d&apos;alerte — optimisation requise"}`,
            ].map((sig, i) => (
              <div key={i} style={{ background: "#1e293b", borderRadius: 8, padding: 12, fontSize: 13, color: "#e2e8f0", border: "1px solid rgba(100,116,139,0.15)", lineHeight: 1.5 }}>
                {sig}
              </div>
            ))}
          </div>
        ...
        {tab === "certs" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {certs.map((cert, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(100,116,139,0.15)" }}>
        ...
        {tab === "actions" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#cbd5e1" }}>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(100,116,139,0.15)" }}>
              <div style={{ fontSize: 11, color: "#93c5fd", marginBottom: 4 }}>Action Prioritaire</div>
              <div>{entity.risk_level === "critique" ? "Audit immédiat des campagnes Carbon Capture — conformité réglementaire urgente" : entity.risk_level === "élevé" ? "Révision des stratégies publicitaires Carbon Capture — renforcement compliance" : entity.risk_level === "modéré" ? "Optimisation continue des métriques publicitaires Carbon Capture" : "Maintien des bonnes pratiques — veille sectorielle Carbon Capture"}</div>
            </div>
            <div style={{ background: "#1e293b", borderRadius: 8, padding: 12, border: "1px solid rgba(100,116,139,0.15)" }}>
              <div style={{ fontSize: 11, color: "#93c5fd", marginBottom: 4 }}>Index Caelum</div>
              <div style={{ color: COLOR, fontWeight: 700, fontSize: 18 }}>{entity.estimated_carboncapture_index.toFixed(2)} / 10</div>
            </div>
          </div>
        )}
      </div>
...
export default function CarbonCaptureAdvertisingDashboard() {
  ...
  useEffect(() => {
    fetch("/api/carboncapture-advertising")
      .then((r) => r.json())
      .then((d) => { setData(d.payload ?? d); setLoading(false); })
      .catch(() => { setError("Erreur de chargement des données Carbon Capture Advertising."); setLoading(false); });
  }, []);

  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#030712", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        Chargement Carbon Capture Advertising&hellip;
      </div>
    );
  }
  ...
  const avgIndex = entities.length > 0
    ? entities.reduce((s, e) => s + e.estimated_carboncapture_index, 0) / entities.length
    : 0;
    ...
    <div style={{ minHeight: "100vh", background: "#030712", color: "#fff", padding: "32px 24px" }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", margin: 0 }}>
            Carbon Capture Advertising
          </h1>
        </div>
        <p style={{ fontSize: 13, color: `${COLOR}99`, margin: 0 }}>
          Caelum Partners · Wave 390 · Droits de l&apos;enfant — Secteur capture carbone · {data.generatedAt ? new Date(data.generatedAt).toLocaleDateString("fr-FR") : "2026-06-22"}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 32 }}>
        {[
          ...
          <div key={label} style={{ background: "#0f172a", border: "1px solid rgba(100,116,139,0.25)", borderRadius: 12, padding: "16px 14px" }}>
      ...
      <div style={{ background: "#0f172a", border: "1px solid rgba(100,116,139,0.25)", borderRadius: 14, padding: 20, marginBottom: 24 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: "#93c5fd99", textTransform: "uppercase", letterSpacing: 1, marginBottom: 16 }}>
          Métriques Globales Carbon Capture
        </div>
        <div style={{ display: "flex", gap: 24, flexWrap: "wrap", justifyContent: "space-around" }}>
          ...
          <GaugeRing value={avgIndex * 10} label="Index Moyen" color="#94a3b8" />
        ...
        {FILTER_OPTIONS.map((opt) => (
          <button
            ...
            style={{
              ...
              border: `1px solid ${filter === opt ? COLOR : "rgba(100,116,139,0.3)"}`,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14, marginBottom: 24 }}>
        {filtered.map((entity, i) => (
          <div
            ...
            style={{
              background: "#0f172a", border: "1px solid rgba(100,116,139,0.25)", borderRadius: 12, padding: 16,
              cursor: "pointer", transition: "border-color 0.2s",
            }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = COLOR; }}
            onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(100,116,139,0.25)"; }}
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b" }}>
              ...
              <span style={{ color: COLOR, fontWeight: 600 }}>{entity.estimated_carboncapture_index.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: "#334155" }}>
        Caelum Partners · Carbon Capture Advertising Intelligence · Wave 390
      </div>
