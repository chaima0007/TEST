...
interface Entity {
  ...
  last_updated: string;
  arbitrary_detention_score?: number;
  torture_in_detention_score?: number;
  fair_trial_denial_score?: number;
  prisoner_of_conscience_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "arbitrary_detention_score", label: "Détention arbitraire" },
  { key: "torture_in_detention_score", label: "Torture en détention" },
  { key: "fair_trial_denial_score", label: "Refus de procès équitable" },
  { key: "prisoner_of_conscience_score", label: "Prisonnier de conscience" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des droits des prisonniers politiques : détention arbitraire, torture en détention, refus de procès équitable et prisonniers de conscience.</p>
              <p className="text-slate-400">Source : Political Prisoner Rights Engine · Wave 247</p>
            </div>
          )}
        </div>
      </div>
...
export default function PoliticalPrisonerRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/political-prisoner-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Political Prisoner Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Détention Arbitraire · Torture · Procès Équitable · Conscience</p>
        </div>
