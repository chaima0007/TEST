...
interface Entity {
  ...
  institutionalization_forced_treatment_severity_score: number;
  physical_accessibility_exclusion_scale_score: number;
  disability_employment_discrimination_score: number;
  crpd_legal_capacity_guardianship_reform_deficit_gap_score: number;
  estimated_disability_rights_accessibility_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_disability_rights_accessibility_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Institutionnalisation & Traitement Forcé", value: entity.institutionalization_forced_treatment_severity_score, weight: "0.30" },
    { label: "Exclusion Accessibilité Physique", value: entity.physical_accessibility_exclusion_scale_score, weight: "0.25" },
    { label: "Discrimination Emploi Handicap", value: entity.disability_employment_discrimination_score, weight: "0.25" },
    { label: "Déficit Réforme Capacité Juridique", value: entity.crpd_legal_capacity_guardianship_reform_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_disability_rights_accessibility_index === "number" ? entity.estimated_disability_rights_accessibility_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Droits Handicap</div>
                </div>
              </div>
...
export default function DisabilityRightsAccessibilityEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/disability-rights-accessibility-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Droits Handicap & Accessibilité…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_disability_rights_accessibility_index ?? avg(allEntities.map(e => e.estimated_disability_rights_accessibility_index));
  ...
  const avgInstitutionalization = avg(allEntities.map(e => e.institutionalization_forced_treatment_severity_score));
  const avgAccessibility = avg(allEntities.map(e => e.physical_accessibility_exclusion_scale_score));
  const avgEmployment = avg(allEntities.map(e => e.disability_employment_discrimination_score));
  const avgCrpd = avg(allEntities.map(e => e.crpd_legal_capacity_guardianship_reform_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Droits Handicap", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Droits Handicap & Accessibilité</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Disability Rights & Accessibility Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgInstitutionalization} label="Institutionnalisation & Traitement" />
          <GaugeRing value={avgAccessibility} label="Exclusion Accessibilité" />
          <GaugeRing value={avgEmployment} label="Discrimination Emploi" />
          <GaugeRing value={avgCrpd} label="Réforme Capacité Juridique" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.entity_id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Droits Handicap: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_disability_rights_accessibility_index === "number" ? e.estimated_disability_rights_accessibility_index.toFixed(2) : "—"}</span>
            </div>
