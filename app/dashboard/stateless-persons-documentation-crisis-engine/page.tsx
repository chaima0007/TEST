...
interface Entity {
  ...
  risk_level: string
  estimated_stateless_documentation_index: number
}
...
export default function StatelessPersonsDocumentationCrisisPage() {
  const [data, setData] = useState<DomainData | null>(null)

  useEffect(() => {
    fetch("/api/stateless-persons-documentation-crisis-engine")
      .then((r) => r.json())
  ...
  if (!data) return (
    <div style={{ background: "#0f172a", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: ACCENT, fontSize: 18 }}>Chargement apatrides &amp; crise documentation…</div>
    </div>
  )
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "#f1f5f9" }}>
              Apatrides &amp; Crise de Documentation
            </h1>
          ...
          <div key={entity.id} style={{ background: "#1e293b", borderRadius: 12, padding: 22, display: "flex", gap: 20, alignItems: "flex-start", borderLeft: `4px solid ${ACCENT}` }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", gap: 16 }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: ACCENT }}>{entity.estimated_stateless_documentation_index.toFixed(2)}</div>
