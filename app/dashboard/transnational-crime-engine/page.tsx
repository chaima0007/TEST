...
interface Entity {
  ...
  narco_state_capture_score: number;
  money_laundering_score: number;
  human_trafficking_network_score: number;
  criminal_governance_score: number;
  risk_level: string;
  primary_pattern: string;
  key_signals: string[];
  estimated_transnational_crime_index: number;
  last_updated: string;
}
...
interface Data {
  ...
  avg_estimated_transnational_crime_index: number;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Capture Narco-État", v: entity.narco_state_capture_score, c: "#78716c" },
              { l: "Blanchiment d'Argent", v: entity.money_laundering_score, c: "#57534e" },
              { l: "Réseau Trafic Humain", v: entity.human_trafficking_network_score, c: "#44403c" },
              { l: "Gouvernance Criminelle", v: entity.criminal_governance_score, c: "#292524" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Crime Transnational Estimé</span>
              <span className="font-bold" style={{ color: "#a8a29e" }}>{entity.estimated_transnational_crime_index}</span>
            </div>
        ...
        {tab === "signaux" && (
          <div>
            <p className="text-xs text-slate-500 mb-3">Pattern principal: <span style={{ color: "#a8a29e" }}>{entity.primary_pattern}</span></p>
            <ul className="space-y-2">
              {entity.key_signals.map((s, i) => (
                <li key={i} className="flex gap-2 text-sm text-slate-300">
                  <span style={{ color: "#a8a29e" }}>›</span>{s}
                </li>
              ))}
            </ul>
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && (
              <>
                <p>› Activation des protocoles d'intervention urgente et coordination INTERPOL/FATF pour démantèlement des réseaux identifiés</p>
                <p>› Gel immédiat des flux financiers illicites et saisie coordonnée des actifs criminels transnationaux</p>
              </>
            )}
            {entity.risk_level === "élevé" && (
              <>
                <p>› Renforcement de la coopération judiciaire internationale et activation des mécanismes d'entraide pénale</p>
                <p>› Cartographie approfondie des réseaux de blanchiment et des structures de gouvernance criminelle</p>
              </>
            )}
            {entity.risk_level === "modéré" && (
              <p>› Surveillance accrue des flux financiers suspects et monitoring des réseaux de trafic identifiés</p>
            )}
            {entity.risk_level === "faible" && (
              <p>› Maintien de la veille sur les évolutions des économies parallèles et des structures criminelles émergentes</p>
            )}
          </div>
        )}
      </div>
...
export default function TransnationalCrimeEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/transnational-crime-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données Transnational Crime Engine"); setLoading(false); });
  }, []);

  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-slate-400 animate-pulse">Analyse du crime organisé transnational…</div>
    </div>
  );
  ...
  const kpis = [
    ...
    { l: "Index Crime Moyen", v: `${(data.avg_estimated_transnational_crime_index ?? 0).toFixed(2)}`, c: "text-stone-300" },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "#a8a29e" }}>Transnational Crime Engine</h1>
          <p className="text-slate-400 text-sm mt-1">Crime organisé transnational, capture étatique et économies parallèles criminelles</p>
        ...
        <div className="bg-slate-900 border border-stone-500/20 rounded-xl p-5">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Scores Crime Transnational Moyens</h2>
          <div className="flex flex-wrap justify-around gap-4">
            <GaugeRing value={avgScore("narco_state_capture_score")} label="Capture Narco-État" color={ACCENT} />
            <GaugeRing value={avgScore("money_laundering_score")} label="Blanchiment" color="#57534e" />
            <GaugeRing value={avgScore("human_trafficking_network_score")} label="Trafic Humain" color="#44403c" />
            <GaugeRing value={avgScore("criminal_governance_score")} label="Gouvernance Crim." color="#a8a29e" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900 border border-stone-500/20 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Patterns Criminalité Transnationale</h2>
            <div className="space-y-2">
              {Object.entries(patDist).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={entities.length}
                  color={["#78716c", "#57534e", "#44403c", "#a8a29e", "#d6d3d1"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
          ...
          {filtered.map(entity => (
            <div key={entity.entity_id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_transnational_crime_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Transnational Crime Engine · {data.last_analysis}</p>
      </div>
    </div>
  );
}
