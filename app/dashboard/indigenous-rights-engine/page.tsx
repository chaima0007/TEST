...
interface Entity {
  ...
  land_territory_dispossession_score: number;
  cultural_linguistic_erasure_score: number;
  self_determination_denial_score: number;
  violence_criminalization_score: number;
  risk_level: string;
...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: ACCENT }}>Indigenous Rights Engine</h1>
        <p className="text-slate-400 mt-1">UNDRIP · Rapporteur ONU · Mécanisme Permanent</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold mt-1" style={{ color: ACCENT }}>{data.avg_estimated_indigenous_rights_index}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              {tab === 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { key: "land_territory_dispossession_score", label: "Dépossession Terres" },
                      { key: "cultural_linguistic_erasure_score", label: "Effacement Culturel" },
                      { key: "self_determination_denial_score", label: "Déni Autodétermination" },
                      { key: "violence_criminalization_score", label: "Violence/Criminalisation" },
                    ].map(({ key, label }) => (
                      <div key={key}>
