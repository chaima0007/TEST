...
export default function CorporateHumanRightsDueDiligencePage() {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  useEffect(() => {
    fetch("/api/corporate-human-rights-due-diligence-engine")
      .then((r) => r.json())
  ...
  const avgIndex = (data.avg_estimated_corporate_human_rights_due_diligence_index as number) ?? 0;
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-6">
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: SKY }}>
            Devoir de Vigilance Droits Humains — Entreprises
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            UNGPs 2011 · EU CSDDD 2024 · OCDE Diligence · Business &amp; Human Rights Resource Centre
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Avg Composite", value: avgComposite.toFixed(2), unit: "/100" },
            { label: "Indice CSDDD", value: avgIndex.toFixed(2), unit: "/10" },
            ...
            <div key={label} className="rounded-xl bg-slate-900 border border-slate-800 p-4 text-center">
              ...
              <div className="text-2xl font-bold" style={{ color: SKY }}>{value}<span className="text-sm text-slate-500 ml-1">{unit}</span></div>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 flex flex-col items-center">
            <div className="text-xs text-slate-500 uppercase tracking-widest mb-4">Score Global Vigilance Entreprises</div>
            <div className="w-32 h-32"><GaugeRing value={avgComposite} color={SKY} /></div>
            <div className="mt-3 text-3xl font-bold" style={{ color: SKY }}>{avgComposite.toFixed(1)}</div>
          </div>
        ...
        {entities.length > 0 && (
          <div className="rounded-xl bg-slate-900 border border-slate-800 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-800">
              <div className="text-xs text-slate-500 uppercase tracking-widest">Chaînes d&apos;approvisionnement analysées</div>
            </div>
        ...
        <div className="rounded-xl border p-5" style={{ borderColor: SKY + "40", backgroundColor: SKY + "10" }}>
          <div className="text-xs font-semibold mb-1" style={{ color: SKY }}>EU CSDDD 2024 — Entrée en vigueur</div>
          <p className="text-xs text-slate-300 leading-relaxed">
            La directive européenne CSDDD oblige les grandes entreprises (500+ salariés) à identifier,
            prévenir et remédier aux violations droits humains dans leurs chaînes d&apos;approvisionnement
            mondiales. Sanctions jusqu&apos;à 5% du chiffre d&apos;affaires mondial.
          </p>
        </div>
      </div>
    </div>
