...
interface MMEntity {
  ...
  composite_score: number; healthcare_system_collapse_score: number;
  gender_inequality_score: number; obstetric_access_deficit_score: number;
  conflict_health_impact_score: number; risk_level: string;
  primary_pattern: string; key_signals: string[];
  estimated_maternal_risk_index: number; last_updated: string;
}
interface MMSummary {
  ...
  entities: MMEntity[]; avg_estimated_maternal_risk_index: number;
}
...
function DetailModal({ entity, onClose }: { entity: MMEntity; onClose: () => void }) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        {tab === "scores" && (
          <div className="space-y-3">
            {[
              { l: "Effondrement Système de Santé", v: entity.healthcare_system_collapse_score, c: "#f43f5e" },
              { l: "Inégalité de Genre", v: entity.gender_inequality_score, c: "#f97316" },
              { l: "Déficit Accès Obstétrique", v: entity.obstetric_access_deficit_score, c: "#eab308" },
              { l: "Impact Sanitaire des Conflits", v: entity.conflict_health_impact_score, c: "#f43f5e" },
            ].map(({ l, v, c }) => (
              <div key={l}>
            ...
            <div className="pt-2 flex justify-between text-sm">
              <span className="text-slate-400">Index Risque Mortalité Maternelle</span>
              <span className="text-rose-400 font-bold">{entity.estimated_maternal_risk_index}</span>
            </div>
          </div>
        )}
        {tab === "signaux" && (
        ...
        {tab === "actions" && (
          <div className="text-sm space-y-2 text-slate-300">
            ...
            {entity.risk_level === "critique" && <><p>› Plan d'urgence national pour l'accès aux soins obstétriques</p><p>› Déploiement immédiat de personnel médical qualifié en zones à risque</p></>}
            {entity.risk_level === "élevé" && <><p>› Renforcement des infrastructures de santé maternelle</p><p>› Programmes d'égalité de genre dans l'accès aux soins</p></>}
            {entity.risk_level === "modéré" && <><p>› Amélioration de la couverture obstétrique rurale</p></>}
            {entity.risk_level === "faible" && <><p>› Maintien des programmes de surveillance maternelle</p></>}
          </div>
        )}
      </div>
    </div>
...
export default function MaternalMortalityPage() {
  const [data, setData] = useState<MMSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [filter, setFilter] = useState("tous");
  const [sel, setSel] = useState<MMEntity | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/maternal-mortality-engine")
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
      .then(d => { setData(d.entities ? d : d.data ?? d); setLoading(false); })
      .catch(() => { setError("Erreur chargement données mortalité maternelle"); setLoading(false); });
  }, []);

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-slate-400 animate-pulse">Analyse de la mortalité maternelle mondiale…</div></div>;
  ...
  const avgScore = (k: keyof MMEntity) => Math.round(data.entities.reduce((s, e) => s + (e[k] as number), 0) / data.entities.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Mortalité Maternelle & Capacité Étatique</h1>
          <p className="text-slate-400 text-sm">Mortalité maternelle comme indicateur de faillite systémique de l'État · v{data.engine_version}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { l: "Index Risque Maternel", v: data.avg_estimated_maternal_risk_index, g: "from-pink-900 to-pink-800", t: "text-pink-300" },
        ...
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Vecteurs de Risque Mortalité Maternelle Moyens</h2>
          <div className="flex flex-wrap justify-around gap-6">
            <GaugeRing value={avgScore("healthcare_system_collapse_score")} label="Effondrement Système de Santé" color="#f43f5e" />
            <GaugeRing value={avgScore("gender_inequality_score")} label="Inégalité de Genre" color="#f97316" />
            <GaugeRing value={avgScore("obstetric_access_deficit_score")} label="Déficit Accès Obstétrique" color="#eab308" />
            <GaugeRing value={avgScore("conflict_health_impact_score")} label="Impact Sanitaire des Conflits" color="#f43f5e" />
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Distribution Risques Maternels</h2>
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Patterns de Faillite Systémique</h2>
            <div className="space-y-2">
              {Object.entries(data.pattern_distribution).map(([k, v], i) => (
                <DistBar key={k} label={k.replace(/_/g, " ")} value={v} total={data.total_entities} color={["#f43f5e", "#f97316", "#eab308", "#10b981", "#f43f5e"][i % 5]} />
              ))}
            </div>
          </div>
        </div>
        ...
        {data.critical_alerts.length > 0 && (
          <div className="bg-rose-900/10 border border-rose-500/20 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-rose-400 mb-2">Alertes Mortalité Maternelle Critiques</h2>
          ...
          {filtered.map(entity => (
            <div key={entity.id} onClick={() => setSel(entity)}
              ...
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span>idx {entity.estimated_maternal_risk_index}</span>
              </div>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-600 text-center">Caelum Partners · Mortalité Maternelle &amp; Capacité Étatique · {data.last_analysis}</p>
      </div>
    </div>
  );
}
