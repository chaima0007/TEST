...
interface Entity {
  ...
  forced_assimilation_child_removal_severity_score: number;
  language_cultural_suppression_scale_score: number;
  territorial_dispossession_violence_score: number;
  state_accountability_reparations_gap_score: number;
  estimated_indigenous_cultural_genocide_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_indigenous_cultural_genocide_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Assimilation Forcée Retrait Enfants", value: entity.forced_assimilation_child_removal_severity_score, weight: "0.30" },
    { label: "Suppression Langue & Culture", value: entity.language_cultural_suppression_scale_score, weight: "0.25" },
    { label: "Dépossession Territoire Violence", value: entity.territorial_dispossession_violence_score, weight: "0.25" },
    { label: "Lacune Responsabilité Réparations", value: entity.state_accountability_reparations_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_indigenous_cultural_genocide_index === "number" ? entity.estimated_indigenous_cultural_genocide_index.toFixed(2) : "—"}</div>
...
export default function IndigenousCulturalGenocideAssimilationEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/indigenous-cultural-genocide-assimilation-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Génocide Culturel Autochtone…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_indigenous_cultural_genocide_index ?? avg(allEntities.map(e => e.estimated_indigenous_cultural_genocide_index));
  ...
  const avgAssimilation = avg(allEntities.map(e => e.forced_assimilation_child_removal_severity_score));
  const avgLanguage = avg(allEntities.map(e => e.language_cultural_suppression_scale_score));
  const avgTerritory = avg(allEntities.map(e => e.territorial_dispossession_violence_score));
  const avgAccountability = avg(allEntities.map(e => e.state_accountability_reparations_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Génocide Culturel", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Indigenous Cultural Genocide & Assimilation Engine</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Pensionnats, assimilation forcée et génocide culturel autochtone — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles · Wave 181
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <GaugeRing value={avgLanguage} label="Suppression Culturelle" />
          <GaugeRing value={avgTerritory} label="Dépossession Terre" />
          <GaugeRing value={avgAccountability} label="Lacune Réparations" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Génocide Culturel: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_indigenous_cultural_genocide_index === "number" ? e.estimated_indigenous_cultural_genocide_index.toFixed(2) : "—"}</span>
            </div>
          </div>
        ))}
      </div>
