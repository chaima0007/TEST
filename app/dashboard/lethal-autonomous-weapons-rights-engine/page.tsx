...
interface Entity {
  ...
  last_updated: string;
  autonomous_kill_decision_score?: number;
  accountability_gap_score?: number;
  civilian_targeting_error_score?: number;
  arms_proliferation_control_score?: number;
  [key: string]: unknown;
}
...
const SUB_SCORES = [
  { key: "autonomous_kill_decision_score", label: "Décision de tuer autonome" },
  { key: "accountability_gap_score", label: "Lacune de responsabilité" },
  { key: "civilian_targeting_error_score", label: "Erreur de ciblage civil" },
  { key: "arms_proliferation_control_score", label: "Contrôle prolifération armements" },
];
        ...
        <div className="p-6 space-y-4">
          ...
          {tab === "context" && (
            <div className="space-y-3 text-sm text-slate-300">
              <p>Analyse des armes autonomes létales : décisions de tuer sans supervision humaine, lacunes de responsabilité juridique, erreurs de ciblage civil et risques de prolifération des systèmes d&apos;armes autonomes.</p>
              <p className="text-slate-400">Source : Lethal Autonomous Weapons Rights Engine · Wave 258</p>
            </div>
          )}
        </div>
      </div>
...
export default function LethalAutonomousWeaponsRightsPage() {
  ...
  useEffect(() => {
    fetch("/api/lethal-autonomous-weapons-rights-engine").then(r => r.json()).then(d => { setData(d.payload ?? d); setLoading(false); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: ACCENT }}>Lethal Autonomous Weapons Rights Engine</h1>
          <p className="text-slate-400 mt-1 text-sm">Armes Autonomes · Responsabilité · Ciblage Civil · Prolifération</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["tous", "critique", "élevé", "modéré", "faible"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { backgroundColor: "#dc262620" } : {}}>
