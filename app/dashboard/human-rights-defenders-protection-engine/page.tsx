...
interface Entity {
  ...
  hrd_killing_disappearance_severity_score: number;
  criminalization_legal_harassment_scale_score: number;
  digital_surveillance_doxxing_hrd_score: number;
  ngo_restriction_foreign_agent_law_deficit_gap_score: number;
  estimated_human_rights_defenders_protection_index: number;
  risk_level: string;
...
interface DashData {
  ...
  avg_estimated_human_rights_defenders_protection_index?: number;
...
function DetailModal({ entity, onClose }: { entity: Entity; onClose: () => void }) {
  ...
  const subScores = [
    { label: "Meurtres & Disparitions DH", value: entity.hrd_killing_disappearance_severity_score, weight: "0.30" },
    { label: "Criminalisation & Harcèlement Légal", value: entity.criminalization_legal_harassment_scale_score, weight: "0.25" },
    { label: "Surveillance Numérique & Doxxing", value: entity.digital_surveillance_doxxing_hrd_score, weight: "0.25" },
    { label: "Restrictions ONG & Lois Agent Étranger", value: entity.ngo_restriction_foreign_agent_law_deficit_gap_score, weight: "0.20" },
  ];
  return (
          ...
          {tab === "apercu" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold" style={{ color: ACCENT }}>{typeof entity.estimated_human_rights_defenders_protection_index === "number" ? entity.estimated_human_rights_defenders_protection_index.toFixed(2) : "—"}</div>
                  <div className="text-xs text-slate-400 mt-1">Index Protection DH</div>
                </div>
              </div>
...
export default function HumanRightsDefendersProtectionEnginePage() {
  ...
  useEffect(() => {
    fetch("/api/human-rights-defenders-protection-engine")
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-sm" style={{ color: ACCENT }}>Initialisation Protection Défenseurs Droits Humains…</div>
      </div>
    );
  }
  ...
  const avgIndex = data?.avg_estimated_human_rights_defenders_protection_index ?? avg(allEntities.map(e => e.estimated_human_rights_defenders_protection_index));
  ...
  const avgHrdKilling = avg(allEntities.map(e => e.hrd_killing_disappearance_severity_score));
  const avgCriminalization = avg(allEntities.map(e => e.criminalization_legal_harassment_scale_score));
  const avgSurveillance = avg(allEntities.map(e => e.digital_surveillance_doxxing_hrd_score));
  const avgNgo = avg(allEntities.map(e => e.ngo_restriction_foreign_agent_law_deficit_gap_score));

  const kpis = [
    { label: "Entités Analysées", value: data?.total_entities ?? allEntities.length },
    { label: "Score Moyen", value: avgComposite.toFixed(1) },
    { label: "Index Protection DH", value: avgIndex.toFixed(2) },
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Protection Défenseurs Droits Humains</h1>
        </div>
        <p className="text-slate-400 text-sm ml-6">
          Human Rights Defenders Protection Engine — Caelum Partners · Chaima Mhadbi, Fondatrice, Bruxelles
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <GaugeRing value={avgHrdKilling} label="Meurtres & Disparitions" />
          <GaugeRing value={avgCriminalization} label="Criminalisation" />
          <GaugeRing value={avgSurveillance} label="Surveillance Numérique" />
          <GaugeRing value={avgNgo} label="Restrictions ONG" />
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} onClick={() => setSelected(e)}
            ...
            <div className="text-xs text-slate-500 mt-2">
              Index Protection DH: <span className="font-medium" style={{ color: ACCENT }}>{typeof e.estimated_human_rights_defenders_protection_index === "number" ? e.estimated_human_rights_defenders_protection_index.toFixed(2) : "—"}</span>
            </div>
