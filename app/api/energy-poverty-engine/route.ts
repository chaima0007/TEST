...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    const entities = MOCK_ENTITIES.map(e => {
      ...
      const jus  = justiceScore(e);
      const tra  = transitionScore(e);
      const sys  = systemicScore(e);
      const comp = compositeScore(acc, jus, tra, sys);
      const risk = riskLevel(comp);
      const pat  = energyPattern(e);
  ...
  try {
    const upstream = await fetch(`${process.env.SWARM_API_URL}/energy-poverty-engine`);
    if (!upstream.ok) throw new Error(`Upstream ${upstream.status}`);
    return NextResponse.json(sealResponse(await upstream.json(), "energy-poverty-engine"));
  } catch {
    return NextResponse.json(
