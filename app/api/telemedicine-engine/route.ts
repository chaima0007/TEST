...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Telemedicine Agent")));
  }
  try {
    ...
    return sealResponse(NextResponse.json(sealResponse(data, "Telemedicine Agent")));
  } catch {
