...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Urban Heat Crisis Engine Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/urban-heat-crisis-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Urban Heat Crisis Engine Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Urban Heat Crisis Engine Agent"), { status: 502 }));
  }
}
