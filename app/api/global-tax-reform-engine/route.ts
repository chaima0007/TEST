...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Global Tax Reform Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/global-tax-reform-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Global Tax Reform Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Global Tax Reform Agent"), { status: 502 }));
  }
}
