...
export async function GET() {
  ...
  console.warn("[soil-degradation-engine] SWARM_API_URL non défini — mode dégradé activé");
};
    });
    ...
    return sealResponse(NextResponse.json(sealResponse({ entities, summary }, "soil-degradation-engine")));
  }

  try {
    const upstream = await fetch(`${process.env.SWARM_API_URL}/soil-degradation-engine`, { next: { revalidate: 30 } });
    if (!upstream.ok) throw new Error(`Upstream ${upstream.status}`);
    return sealResponse(NextResponse.json(sealResponse(await upstream.json(), "soil-degradation-engine")));
  } catch {
