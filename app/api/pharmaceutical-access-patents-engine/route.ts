...
if (!process.env.SWARM_API_URL) {
  console.warn("[pharmaceutical-access-patents-engine] SWARM_API_URL non défini — mode dégradé activé");
}
...
export async function GET() {
  ...
  try {
    ...
    return sealResponse(NextResponse.json(sealResponse({ engine: "PAP_ENGINE", entities: [] }), { status: 502 }));
  }
}
