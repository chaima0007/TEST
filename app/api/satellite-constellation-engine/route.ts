...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Satellite Constellation Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/satellite-constellation-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Satellite Constellation Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Satellite Constellation Agent"), { status: 502 }));
  }
}
