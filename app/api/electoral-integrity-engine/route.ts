...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Electoral Integrity Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/electoral-integrity-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Electoral Integrity Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Electoral Integrity Agent"), { status: 502 }));
  }
}
