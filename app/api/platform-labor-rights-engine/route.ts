...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return NextResponse.json(sealResponse(getMockData() as unknown as Record<string, unknown>, "Platform Labor Rights Agent"));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/platform-labor-rights-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return NextResponse.json(sealResponse(data as Record<string, unknown>, "Platform Labor Rights Agent"));
  } catch {
    return NextResponse.json(sealResponse(getMockData() as unknown as Record<string, unknown>, "Platform Labor Rights Agent"), { status: 502 });
  }
}
