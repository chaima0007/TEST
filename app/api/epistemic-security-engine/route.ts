...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Epistemic Security Engine Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/epistemic-security-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Epistemic Security Engine Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Epistemic Security Engine Agent"), { status: 502 }));
  }
}
