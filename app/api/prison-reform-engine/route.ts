...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    const entities = MOCK_ENTITIES.map(e => {
      const inc  = incarcerationScore(e);
      const rac  = racialScore(e);
      const reh  = rehabilitationScore(e);
      const sys  = systemicScore(e);
      const comp = compositeScore(inc, rac, reh, sys);
      const risk = riskLevel(comp);
      const pat  = justicePattern(e);
  ...
  try {
    const upstream = await fetch(`${process.env.SWARM_API_URL}/api/prison-reform-engine`);
    if (!upstream.ok) throw new Error(`Upstream ${upstream.status}`);
    return NextResponse.json(sealResponse(await upstream.json(), "prison-reform-engine"));
  } catch {
    return NextResponse.json(
