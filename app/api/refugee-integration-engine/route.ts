...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return NextResponse.json(sealResponse(getMockData(), "Refugee Integration Agent"));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/refugee-integration-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return NextResponse.json(sealResponse(data, "Refugee Integration Agent"));
  } catch {
    return NextResponse.json(sealResponse(getMockData(), "Refugee Integration Agent"), { status: 502 });
  }
}
