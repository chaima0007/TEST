...
if (!process.env.SWARM_API_URL) {
  console.warn("[gaming-advertising-child-labor-rights] SWARM_API_URL non défini — mode dégradé activé");
}

export async function GET() {
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/gaming_advertising_child_labor_rights`, { next: { revalidate: 30 } });
