...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Longevity Inequality Engine Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/longevity-inequality-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Longevity Inequality Engine Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Longevity Inequality Engine Agent"), { status: 502 }));
  }
}
