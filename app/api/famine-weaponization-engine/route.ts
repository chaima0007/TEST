...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Famine Weaponization Engine Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/famine-weaponization-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Famine Weaponization Engine Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Famine Weaponization Engine Agent"), { status: 502 }));
  }
}
