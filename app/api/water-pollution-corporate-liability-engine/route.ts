...
if (!process.env.SWARM_API_URL) {
  console.warn("[water-pollution-corporate-liability-engine] SWARM_API_URL non défini — mode dégradé activé");
}
...
export async function GET() {
  ...
  try {
    ...
    return sealResponse(NextResponse.json(sealResponse({ engine: "WPC_ENGINE", entities: [] }), { status: 502 }));
  }
}
