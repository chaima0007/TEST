...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    const entities = MOCK_ENTITIES.map(e => {
      const pred = predationScore(e);
      const cont = contentScore(e);
      const plat = platformScore(e);
      const gov  = governanceScore(e);
      const comp = compositeScore(pred, cont, plat, gov);
      const risk = riskLevel(comp);
      const pat  = safetyPattern(e);
  ...
  try {
    const upstream = await fetch(`${process.env.SWARM_API_URL}/api/child-online-safety-engine`);
    if (!upstream.ok) throw new Error(`Upstream ${upstream.status}`);
    return NextResponse.json(sealResponse(await upstream.json(), "child-online-safety-engine"));
  } catch {
    return NextResponse.json(
