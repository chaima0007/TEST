...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    const entities = MOCK_ENTITIES.map(e => {
      const res   = resistanceScore(e);
      const pipe  = pipelineScore(e);
      const trans = transmissionScore(e);
      const sys   = systemicScore(e);
      const comp  = compositeScore(res, pipe, trans, sys);
      const risk  = riskLevel(comp);
      const pat   = amrPattern(e);
    ...
    return NextResponse.json(sealResponse({ entities, summary }, "antibiotic-resistance-engine"));
  }

  try {
    const upstream = await fetch(`${process.env.SWARM_API_URL}/antibiotic-resistance-engine`);
    if (!upstream.ok) throw new Error(`Upstream ${upstream.status}`);
    return NextResponse.json(sealResponse(await upstream.json(), "antibiotic-resistance-engine"));
  } catch {
