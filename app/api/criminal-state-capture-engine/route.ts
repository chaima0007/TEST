...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Criminal State Capture Engine Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/criminal-state-capture-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Criminal State Capture Engine Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Criminal State Capture Engine Agent"), { status: 502 }));
  }
}
