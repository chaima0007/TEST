...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Food System Sovereignty Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/food-system-sovereignty-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Food System Sovereignty Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Food System Sovereignty Agent"), { status: 502 }));
  }
}
