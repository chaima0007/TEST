...
if (!process.env.SWARM_API_URL) {
  console.warn("[identityresolution] SWARM_API_URL non défini — vérifier .env");
}

export async function GET() {
  try {
    const upstream = process.env.SWARM_API_URL
      ? await fetch(`${process.env.SWARM_API_URL}/api/identityresolution`, {
          next: { revalidate: 30 },
        }).then((r) => r.json())
      : null;
    ...
    const data = upstream ?? {
      ...
      estimated_identityresolution_index: 6.1,
      avg_composite: 61.03,
      risk_level: "critique",
      entities: [],
    };
