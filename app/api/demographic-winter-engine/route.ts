...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Demographic Winter Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/demographic-winter-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Demographic Winter Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Demographic Winter Agent"), { status: 502 }));
  }
}
