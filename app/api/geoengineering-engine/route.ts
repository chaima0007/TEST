...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return NextResponse.json(sealResponse(getMockData(), "Geoengineering Agent"));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/geoengineering-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return NextResponse.json(sealResponse(data, "Geoengineering Agent"));
  } catch {
    return NextResponse.json(sealResponse(getMockData(), "Geoengineering Agent"), { status: 502 });
  }
}
