...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return NextResponse.json(sealResponse(getMockData(), "Social Media Mental Health Agent"));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/social-media-mental-health-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return NextResponse.json(sealResponse(data, "Social Media Mental Health Agent"));
  } catch {
    return NextResponse.json(sealResponse(getMockData(), "Social Media Mental Health Agent"), { status: 502 });
  }
}
  ...
  const entities = [
    {
      ...
      estimated_mentalhealth_index: 7.24,
