...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    const entities = MOCK_ENTITIES.map(e => {
      const jur  = jurisdictionScore(e);
      const con  = conservationScore(e);
      const enf  = enforcementScore(e);
      const eq   = equityScore(e);
      const comp = compositeScore(jur, con, enf, eq);
      ...
      return {
        ...
        conservation_score:             con,
        enforcement_score:              enf,
        equity_score:                   eq,
  ...
  try {
    const upstream = await fetch(`${process.env.SWARM_API_URL}/api/ocean-governance-engine`);
    if (!upstream.ok) throw new Error(`Upstream ${upstream.status}`);
    return NextResponse.json(sealResponse(await upstream.json(), "ocean-governance-engine"));
  } catch {
    return NextResponse.json(
