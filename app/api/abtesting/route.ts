...
if (!process.env.SWARM_API_URL) {
  console.warn("[abtesting] SWARM_API_URL non défini — mode dégradé activé");
}
  ...
  if (SWARM_API_URL) {
    try {
      ...
      if (res.ok) {
        ...
        return sealResponse(NextResponse.json({ source: "live", ...data }));
      }
    } catch {
      // fall through to mock
    }
  }
  return sealResponse(NextResponse.json(MOCK_REPORT));
}
...
export async function POST(req: Request) {
  ...
  return sealResponse(NextResponse.json({ status: "recorded", agent_id }));
}
