...
if (!SWARM_API_URL) console.warn("[environmentaldefenders] SWARM_API_URL is not set");

export async function GET() {
  try {
    if (!SWARM_API_URL) throw new Error("SWARM_API_URL not configured");
    const res = await fetch(`${SWARM_API_URL}/environmentaldefenders`, { next: { revalidate: 30 } });
