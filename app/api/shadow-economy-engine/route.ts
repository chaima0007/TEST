...
export async function GET() {
  if (!process.env.SWARM_API_URL) {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Shadow Economy Engine Agent")));
  }
  try {
    const res = await fetch(`${process.env.SWARM_API_URL}/shadow-economy-engine`, { next: { revalidate: 30 } });
    if (!res.ok) throw new Error(`Upstream ${res.status}`);
    const data = await res.json();
    return sealResponse(NextResponse.json(sealResponse(data, "Shadow Economy Engine Agent")));
  } catch {
    return sealResponse(NextResponse.json(sealResponse(getMockData(), "Shadow Economy Engine Agent"), { status: 502 }));
  }
}
