...
if (!process.env.SWARM_API_URL) {
  console.warn("[human-trafficking-modern-slavery-engine] SWARM_API_URL non défini — mode dégradé activé");
}
...
export async function GET() {
  ...
  try {
    ...
    return sealResponse(NextResponse.json(sealResponse({ engine: "HTM_ENGINE", entities: [] }), { status: 502 }));
  }
}
