#!/usr/bin/env python3
"""CaelumSwarm™ — Climate Sinking Islands Rights Engine

Domaine : populations des îles englouties par la montée des eaux
(déplacement forcé, perte de nationalité et de territoire, réfugiés climatiques).
"""
import random
import json
...
def run_engine(n: int = 50_000, seed: int = 42) -> dict:
    ...
    names = ["Tuvalu", "Kiribati", "Maldives", "Îles Marshall",
             "Vanuatu", "Nauru", "Tokelau", "Carteret (PNG)"]
    ...
    for r in results:
        dist[r["level"]] += 1
    return {
        "engine": "climate_sinking_islands_rights_engine",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "n_simulations": n,
        "avg_composite": avg_composite,
        "distribution": dist,
        "entities": [{"entity": r["entity"], "level": r["level"], "final": r["final"]} for r in results],
        "estimated_sinking_islands_index": round(avg_composite / 100 * 10, 2),
    }
