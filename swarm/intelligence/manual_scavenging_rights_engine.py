#!/usr/bin/env python3
"""CaelumSwarm™ — Manual Scavenging Rights Engine

Domaine : assainissement manuel (nettoyage manuel des égouts et latrines,
discrimination de caste, morts par asphyxie dans les fosses septiques).
"""
import random
import json
...
def run_engine(n: int = 50_000, seed: int = 42) -> dict:
    ...
    names = ["Inde", "Pakistan", "Bangladesh", "Népal",
             "Nigéria", "Kenya", "Ghana", "Sénégal"]
    ...
    for r in results:
        dist[r["level"]] += 1
    return {
        "engine": "manual_scavenging_rights_engine",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "n_simulations": n,
        "avg_composite": avg_composite,
        "distribution": dist,
        "entities": [{"entity": r["entity"], "level": r["level"], "final": r["final"]} for r in results],
        "estimated_manual_scavenging_index": round(avg_composite / 100 * 10, 2),
    }
