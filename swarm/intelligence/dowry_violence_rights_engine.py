...
def run_engine(n: int = 50_000, seed: int = 42) -> dict:
    ...
    entities = [
        ("Inde", 99, 97, 95, 93),
        ("Pakistan", 93, 90, 88, 86),
        ("Bangladesh", 85, 82, 80, 78),
        ("Népal", 80, 77, 75, 73),
        ("Sri Lanka", 61, 58, 56, 54),
        ("Iran", 51, 48, 46, 44),
        ("Afghanistan", 32, 29, 27, 25),
        ("Yémen", 13, 11, 9, 7),
    ]
    results = []
    ...
    for r in results:
        dist[r["level"]] += 1
    return {
        "engine": "dowry_violence_rights_engine",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "n_simulations": n,
        "avg_composite": avg_composite,
        "distribution": dist,
        "entities": [{"entity": r["entity"], "level": r["level"], "final": r["final"]} for r in results],
        "estimated_dowry_violence_index": round(avg_composite / 100 * 10, 2),
    }
