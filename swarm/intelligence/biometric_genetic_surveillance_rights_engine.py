#!/usr/bin/env python3
"""
Engine: biometric_genetic_surveillance_rights
Wave: 498 — Surveillance biométrique & génétique
...
SUB_LABELS = [
    "collecte_biométrique_non_consentie",
    "profilage_ethnique_génétique",
    "cadre_légal_protection",
    "droit_refus_effacement",
]
...
def run_engine(n: int = 50_000, seed: int = 42):
    ...
    estimated_biometric_surveillance_index = round(avg_composite / 100 * 10, 2)
    ...
    return {
        ...
        "estimated_biometric_surveillance_index": estimated_biometric_surveillance_index,
        "entities":          results,
    }
...
if __name__ == "__main__":
    ...
    print(f"Index             : {out['estimated_biometric_surveillance_index']}")
    for r in out["entities"]:
        fb = " [FALLBACK]" if r["fallback"] else ""
        print(f"  {r['entity']:<20} {r['level']:<8} {r['final']:.4f}{fb}")
