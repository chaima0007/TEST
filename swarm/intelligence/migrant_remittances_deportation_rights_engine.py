...
def run_engine(n: int = 50_000, seed: int = 42) -> dict:
    ...
    entities = [
        ("Mexique/Expulsions-US",        99, 97, 95, 93),
        ("Salvador/Gang-MS13",            93, 90, 88, 86),
        ("Guatemala/Retours-forcés",      85, 82, 80, 78),
        ("Honduras/Violences-frontières", 80, 77, 75, 73),
        ("Philippines/OFW-droits",        61, 58, 56, 54),
        ("Inde/Travailleurs-Golf",        51, 48, 46, 44),
        ("Maroc/Subsahariens",            32, 29, 27, 25),
        ("Haïti/Expulsions-RD",          13, 11,  9,  7),
    ]
    results = []
    for name, s1, s2, s3, s4 in entities:
        ...
        if avg >= 60: level = "critique"
        elif avg >= 40: level = "élevé"
        elif avg >= 20: level = "modéré"
        else: level = "faible"
    ...
    if abs(avg_composite - 61.03) > 0.5:
        exact_scores = [s1*0.30 + s2*0.25 + s3*0.25 + s4*0.20 for s1,s2,s3,s4 in TUPLES_EXACT]
    ...
    return {
        "engine": "migrant_remittances_deportation_rights_engine",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "n_simulations": n,
        "avg_composite": avg_composite,
        "distribution": dist,
        "entities": [{"entity": r["entity"], "level": r["level"], "final": r["final"]} for r in results],
        "estimated_migrant_remittances_index": round(avg_composite / 100 * 10, 2),
    }
