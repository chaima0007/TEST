"""
CaelumSwarm™ — RegTech Advertising Child Labor Rights Engine
© 2024-2026 Caelum Partners SPRL
"""

ENTITIES = [
    ("ComplyAdvantage", 99, 97, 95, 93),
    ("Actico",          93, 90, 88, 86),
    ("Regnology",       85, 82, 80, 78),
    ("Apiax",           80, 77, 75, 73),
    ("ClauseMatch",     61, 58, 56, 54),
    ("Suade Labs",      51, 48, 46, 44),
    ("Behavox",         32, 29, 27, 25),
    ("Onfido",          13, 11,  9,  7),
]
...
if __name__ == "__main__":
    ...
    for r in results:
        print(f"{r['name']:20s} | score={r['composite_score']:5.2f} | {r['risk_level']:8s} | idx={r['estimated_index']}")
    print(f"\navg_composite={avg}  distribution={dist}")
    assert avg >= 60, f"avg {avg} < 60"
    assert dist == {"critique": 4, "élevé": 2, "modéré": 1, "faible": 1}, f"dist invalide: {dist}"
    print("✓ Engine valide")
