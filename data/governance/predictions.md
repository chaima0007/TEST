# 🔮 Agent prédictif — anticiper, pas subir

*Généré le 2026-06-27. Anticipation par règles et signaux (pas de divination).*

## 1. Maintenance prédictive — 0 fait(s) à revoir dans les 60 prochains jours
- Aucun fait n'arrive à échéance bientôt ✅

## 2. Changements de loi attendus (veille)
- **Calendrier PEB/exigences énergétiques (bail)** (Wallonie, Bruxelles, Flandre) — Échéances PEB régulièrement réajustées ; obligations de rénovation évolutives.
- **Réforme du Code bruxellois du Logement (en vigueur 01/11/2024)** (Bruxelles) — Vérifier les arrêtés d'exécution et ajustements post-réforme.
- **Indexations annuelles (1er janvier)** (Belgique) — Seuils aide juridique, quotités (in)saisissables, montants fiscaux — réindexés chaque année.
- **Médiation de dettes amiable (loi du 3 mai 2024)** (Belgique) — Suivre les arrêtés et la mise en œuvre pratique.
- **Délais de préavis (statut unique, évolutions)** (Belgique) — Ajustements des règles de préavis / indemnités.

## 3. Prochains domaines à construire (prédiction de besoin)
- Fiscalité (impôts) — priorité moyenne (partiel)

## 4. Signaux saisonniers (pics de demande à préparer à l'avance)
- **1er janvier** : Indexations annuelles (loyers, aide juridique, quotités, fiscalité) → revoir les montants.
- **Janvier–mars** : Déclarations fiscales à préparer → contenu impôts/délais.
- **Avril–juin** : Conflits de voisinage (haies, bruit, jardins) → pic saisonnier.
- **Août–septembre** : Rentrée scolaire (inscriptions, frais, refus) → pic enseignement.
- **Hiver** : Énergie : difficultés de paiement, coupures → renforcer le contenu énergie/CPAS.

> Avant-gardiste = on prépare le contenu AVANT le pic de demande, et on met à jour AVANT la péremption.
