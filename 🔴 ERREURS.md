...
- **État au 2026-09-21 — et le mot « APPLIQUÉ » n'est PAS employé, exprès :** le garde-fou est désormais **en vigueur sur la branche `claude/charming-galileo-cqhkn1`** — `lib/agents/garde-fou.ts` présent, `verifierSansSurvente` appelé au point de sortie des **trois** rédacteurs (`hermes.ts:114`, `pacte.ts:101`, `relance.ts:100`), y compris sur le chemin **heuristique**, celui qui tourne réellement sans clé API. **81/81 tests verts**, dont les 4 tests adverses qui étaient rouges. **Il reste ABSENT de `main`** : `git cat-file -e origin/main:lib/agents/garde-fou.ts` échoue toujours. Le merge est une décision de Chaima (§10). Écrire « APPLIQUÉ » ici serait refaire exactement l'erreur que cette entrée consigne.
- **Quatrième fuite fermée au passage :** `hermes.ts` annonçait « **à partir de** 500 € » quand `pacte.ts` facture un « forfait tout compris ». Deux de nos propres textes se contredisaient, et celui qui part en premier promettait le mauvais sens — « à partir de » annonce au prospect une facture **plus élevée** (piège relevé par `avocat-du-client` le 2026-09-14). HERMES aligné sur ce que PACTE facture. **Le prix (`o.price`) n'est pas touché** : ce n'est pas une décision commerciale, c'est la levée d'une contradiction interne.
